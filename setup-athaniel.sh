...
# 5. Symlinks the 'athaniel' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Athaniel Brain Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/athaniel-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink athaniel into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up athaniel command..."

ATHANIEL_BIN="$SCRIPT_DIR/venv/bin/athaniel"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ATHANIEL_BIN" "$COMMAND_LINK_DIR/athaniel"
echo -e "${GREEN}✓${NC} Symlinked athaniel → $COMMAND_LINK_DISPLAY_DIR/athaniel"
...
# ============================================================================
# Seed bundled skills into ~/.athaniel/skills/
# ============================================================================

ATHANIEL_SKILLS_DIR="${ATHANIEL_HOME:-$HOME/.athaniel}/skills"
mkdir -p "$ATHANIEL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.athaniel/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ATHANIEL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  athaniel status        # Check configuration"
if is_termux; then
    echo "  athaniel gateway       # Run gateway in foreground"
else
    echo "  athaniel gateway install # Install gateway service (messaging + cron)"
fi
echo "  athaniel cron list     # View scheduled jobs"
echo "  athaniel doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m athaniel_cli.main setup
fi
