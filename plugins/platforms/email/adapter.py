"""
Email platform adapter for the Athaniel gateway.

Allows users to interact with Athaniel by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "athaniel-brain" "version" "{_athaniel_version}" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `athaniel config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `athaniel gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Athaniel Brain")
        ...
        msg_id = f"<athaniel-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# athaniel_cli/gateway.py, and the _send_email dispatch in
