"""ntfy platform adapter (Athaniel plugin).
...
a Athaniel dependency.

This adapter ships as a Athaniel platform plugin under
``plugins/platforms/ntfy/``. The Athaniel plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "athaniel-in"              # subscribe topic (incoming)
          publish_topic: "athaniel-out"     # optional — defaults to topic
...
_ECHO_TAG = "athaniel-brain"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``athaniel cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Athaniel dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `athaniel gateway status` without
