"""
LINE Messaging API platform adapter for Athaniel Brain.
...
to Athaniel Brain. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ATHANIEL_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            athaniel_home = Path(get_athaniel_home()).resolve()
        except Exception:
            athaniel_home = Path.home().joinpath(".athaniel").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``athaniel status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``athaniel status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``athaniel setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.athaniel/.env`` via ``athaniel_cli.config``.
    """
    print()
    ...
    try:
        from athaniel_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("athaniel_cli.config not available; set LINE_* vars manually in ~/.athaniel/.env")
        return
