...
def build_agent_card(
    *,
    ...
    card: dict[str, Any] = {
        ...
        "version": "1.0.0",
        "provider": {
            "organization": os.getenv("A2A_PROVIDER_ORG", "Athaniel Brain"),
    ...
    if isinstance(toolsets, dict):
        for ts_name in sorted(toolsets.keys()):
            ...
            skills.append({
                ...
                "name": ts_name,
                "description": f"Athaniel '{ts_name}' capabilities",
        ...
        for ts in sorted(set(toolsets or [])):
            skills.append({
                "id": f"toolset.{ts}",
                "name": ts,
                "description": f"Athaniel '{ts}' capabilities",
                "tags": [ts],
            })
    if not skills:
        skills.append({
...
def _conv_dir() -> Path:
    try:
        ...
        base = Path(get_athaniel_home())
    except Exception:
        base = Path(os.path.expanduser("~/.athaniel"))
