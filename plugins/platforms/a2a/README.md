# A2A — Agent-to-Agent protocol for Athaniel
...
peer (another Athaniel, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
athaniel gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Athaniel serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.athaniel/a2a_audit.jsonl`.
- Conversations persist to `~/.athaniel/a2a_conversations/` — they survive context
