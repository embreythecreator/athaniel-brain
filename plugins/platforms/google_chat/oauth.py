...
- Per-user tokens (keyed by sender email):
    ``${ATHANIEL_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ATHANIEL_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ATHANIEL_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ATHANIEL_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${ATHANIEL_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ATHANIEL_HOME helper so the token follows the user's
# profile (e.g. tests can override via ATHANIEL_HOME=/tmp/...).
try:
    from athaniel_constants import display_athaniel_home, get_athaniel_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where athaniel_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _athaniel_home.py shim).
...
def _athaniel_home() -> Path:
    """Resolve ATHANIEL_HOME at call time (NOT module import).

    Tests and ``ATHANIEL_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.athaniel/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _athaniel_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _athaniel_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _athaniel_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _athaniel_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _athaniel_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `athaniel setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `athaniel setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ATHANIEL_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_athaniel_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_athaniel_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Athaniel (native attachment delivery)"
    )
