...
# ``model_tools`` import time, so ``athaniel status``, ``athaniel chat``, etc.
...
# ``athaniel_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Athaniel is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a athaniel MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "athaniel-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ATHANIEL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from athaniel_constants import get_athaniel_home as _get_athaniel_home
            _athaniel_home = _get_athaniel_home()
        except (ModuleNotFoundError, ImportError):
            _athaniel_home = _Path.home() / ".athaniel"
        self._thread_count_store = _ThreadCountStore(
            _athaniel_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Athaniel is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ATHANIEL_HOME", str(_Path.home() / ".athaniel"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ATHANIEL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a athaniel MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Athaniel is thinking…' marker message.
        ...
        "Athaniel is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Athaniel is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Athaniel is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Athaniel is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``athaniel setup``.

    The setup wizard at ``athaniel_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.athaniel/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. athaniel-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.athaniel/.env")
    print_info("Restart the gateway: athaniel gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``athaniel cron`` running as a
    separate process from ``athaniel gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Athaniel plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `athaniel setup` to install Google Chat support.",
        ...
        # the "Athaniel is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Athaniel is thinking…' marker message "
