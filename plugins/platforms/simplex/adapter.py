"""SimpleX Chat platform adapter (Athaniel plugin).
...
This adapter ships as a Athaniel platform plugin under
``plugins/platforms/simplex/``. The Athaniel plugin loader scans the
...
Optional environment variables:
    ...
    ATHANIEL_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``athaniel setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Athaniel — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ATHANIEL_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``athaniel setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.athaniel/.env`` via
    ``athaniel_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "athaniel_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.athaniel/.env"
        )
        return
