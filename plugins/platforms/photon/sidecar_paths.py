...
under an immutable ``/opt/athaniel`` (read-only for the athaniel user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$ATHANIEL_HOME/photon/sidecar`` (the durable data volume,
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".athaniel-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_athaniel_home() / "photon" / "sidecar"
    try:
