...
Small Node helper that bridges Athaniel Brain to Photon's Spectrum SDK
(`spectrum-ts`).  Athaniel is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Athaniel-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Athaniel can receive inbound messages
...
The Athaniel plugin's `athaniel photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Athaniel is Python, so replies go through this sidecar
