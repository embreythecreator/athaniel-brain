...
// `message.content.text` before Athaniel can see it. We rewrite the two inbound
...
const MARKER = "Athaniel patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Athaniel to patch; keep the legacy v8 path below for older pinned installs.
    if (
