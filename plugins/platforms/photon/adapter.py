"""
Photon Spectrum (iMessage) platform adapter for Athaniel Brain.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Athaniel dep
...
#
# Out-of-process senders (cron subprocesses, `athaniel send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_athaniel_home() honors profile overrides — never hardcode ~/.athaniel.
    from athaniel_constants import get_athaniel_home

    return get_athaniel_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (athaniel status, test collection, plugin discovery).
...
# artifacts so Athaniel sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `athaniel photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `athaniel photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `athaniel setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, athaniel status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: athaniel photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: athaniel photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `athaniel update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``athaniel photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Athaniel does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Athaniel defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Athaniel-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Athaniel checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `athaniel photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `athaniel photon setup`)",
                    code="SIDECAR_DEPS_MISSING",
                    retryable=False,
                )
        # A `athaniel update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `athaniel send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`athaniel send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Athaniel gateway (which spawns the sidecar and "
                    "records its address under <athaniel-home>/runtime/"
    ...
    headers = {"X-Athaniel-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Athaniel plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `athaniel photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: athaniel photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `athaniel gateway setup` alongside every other
    ...
    # Register CLI subcommands — `athaniel photon ...`
