...
This plugin connects Athaniel Brain to iMessage (and other Spectrum
...
TypeScript-only, Athaniel runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Athaniel channel,
...
Run `athaniel photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.athaniel/.env` (the same place every other
...
Management metadata lives in `~/.athaniel/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `athaniel photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Athaniel
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Athaniel coalesces
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage read
  after forwarding it to Athaniel, so the sender sees `Read` without waiting for
  a model/tool turn. Inbound receipts for Athaniel-sent messages are consumed as
  presence telemetry and never create an agent turn. Set
  `PHOTON_READ_RECEIPTS=false` to keep messages at `Delivered`.
- **Native polls are supported.** Athaniel posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `athaniel send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<athaniel-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `athaniel photon status`, a DM and a group roundtrip,
