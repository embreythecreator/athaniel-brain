"""
``athaniel photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Athaniel
...
# Writable sidecar runtime dir (mirrors to ATHANIEL_HOME on immutable
...
# wiring imports this module for `athaniel --help`).
...
# when called later (gateway start, athaniel status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `athaniel photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Athaniel Brain')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``athaniel photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Athaniel Brain" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.athaniel/.env,
    ...
    try:
        from athaniel_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  athaniel gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `athaniel photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`athaniel photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.athaniel/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `athaniel photon telemetry on` / `athaniel photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.athaniel/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  athaniel gateway restart")
    return 0
...
#
# `athaniel gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``athaniel photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `athaniel gateway setup` wizard."""
