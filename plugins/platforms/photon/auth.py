...
Credential storage mirrors every other Athaniel channel:

    * runtime SDK creds  -> ``~/.athaniel/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.athaniel/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a athaniel dependency
...
# registers Athaniel as its own client_id.
...
# Default name of the project Athaniel provisions for the operator.
DEFAULT_PROJECT_NAME = "Athaniel Brain"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of athaniel-brain.

def _auth_json_path() -> Path:
    """Resolve ``~/.athaniel/auth.json`` honouring the active Athaniel profile."""
    try:
        from athaniel_constants import get_athaniel_home
        return Path(get_athaniel_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.athaniel")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # athaniel_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from athaniel_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.athaniel/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.athaniel/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.athaniel/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: athaniel_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `athaniel photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `athaniel photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `athaniel photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `athaniel photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `athaniel photon setup`)"
