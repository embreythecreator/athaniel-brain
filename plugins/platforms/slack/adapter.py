...
# identify AthanielAgent traffic — matching other Athaniel outbound surfaces
# that already set ``AthanielAgent/<version>`` for platform-partner attribution.
try:
    from athaniel_cli import __version__ as _ATHANIEL_VERSION
except Exception:
    _ATHANIEL_VERSION = "unknown"
_ATHANIEL_SLACK_USER_AGENT_PREFIX = f"AthanielAgent/{_ATHANIEL_VERSION}"
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``athaniel config set`` and Railway persist YAML
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from athaniel_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|athaniel>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `athaniel slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `athaniel gateway setup` "
                "or in the active profile's ~/.athaniel/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `athaniel gateway setup` "
                "or add it to your active profile's ~/.athaniel/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `athaniel gateway setup` "
                "or in the active profile's ~/.athaniel/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `athaniel gateway setup` "
                "or add it to your active profile's ~/.athaniel/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_athaniel_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_ATHANIEL_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # Athaniel has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Athaniel can "
                    "be removed from the Slack app manifest via "
                    "`athaniel slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /athaniel prefix). A single regex
            ...
            # manifest (see `athaniel slack manifest`). In Socket Mode, Slack
            ...
            from athaniel_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/athaniel$")

            @self._app.command(_slash_pattern)
            async def handle_athaniel_command(ack, command):
            ...
            for _action_id in (
                "athaniel_approve_once",
                "athaniel_approve_session",
                "athaniel_approve_always",
                "athaniel_deny",
            ):
            ...
            for _action_id in (
                "athaniel_confirm_once",
                "athaniel_confirm_always",
                "athaniel_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("athaniel_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^athaniel_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("athaniel_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'athaniel slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Athaniel handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Athaniel session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Athaniel owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_athaniel_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_athaniel_no_thread_response"] = True
    ...
    async def _handle_slack_message_impl(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_athaniel_no_thread_response"):
        ...
        force_process = bool(event.get("_athaniel_force_process"))
        ...
        # the shape produced by peer Athaniel agents in Socket Mode on some
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "athaniel_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "athaniel_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "athaniel_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "athaniel_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "athaniel_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "athaniel_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``athaniel_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``athaniel_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"athaniel_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "athaniel_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "athaniel_confirm_once": "once",
            "athaniel_confirm_always": "always",
            "athaniel_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "athaniel_approve_once": "once",
            "athaniel_approve_session": "session",
            "athaniel_approve_always": "always",
            "athaniel_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "athaniel_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/athaniel <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/athaniel
        ...
        if slash_name in {"athaniel", ""}:
            # Legacy /athaniel <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from athaniel_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/athaniel   ``).
        ...
        # "Running /cmd…" ack shown by handle_athaniel_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/athaniel <question>" must produce public replies so
...
# ``athaniel_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_athaniel_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``athaniel_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from athaniel_cli.config import get_env_value, remove_env_value, save_env_value
    from athaniel_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ATHANIEL_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from athaniel_cli.slack_cli import _build_full_manifest
            from athaniel_constants import get_athaniel_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Athaniel",
                bot_description="Your Athaniel agent on Slack",
            )
            target = Path(get_athaniel_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `athaniel slack manifest --write` anytime to refresh after "
                "Athaniel adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `athaniel update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://athaniel-brain.embreythecreator.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``athaniel_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    """Plugin entry point — called by the Athaniel plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `athaniel setup` to install Slack support.",
        # Interactive setup wizard — replaces athaniel_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in athaniel_cli/gateway.py.
