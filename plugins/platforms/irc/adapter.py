"""
IRC Platform Adapter for Athaniel Brain.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Athaniel agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "athaniel-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Athaniel Brain")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Athaniel Brain shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: athaniel_, athaniel_1, athaniel_2...
...
def check_requirements() -> bool:
    ...
    # athaniel setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `athaniel gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Athaniel to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #athaniel — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.athaniel/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "athaniel-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "athaniel-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Athaniel Brain (cron)")
