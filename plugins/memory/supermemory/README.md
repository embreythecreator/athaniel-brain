...
- Hosted: API key from [app.supermemory.ai/integrations?connect=athaniel](http://app.supermemory.ai/integrations?connect=athaniel)
...
Before running `athaniel memory setup`, add the local endpoint to
...
Then run `athaniel memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ATHANIEL_HOME/supermemory.json`
...
| `container_tag` | `athaniel` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `athaniel-{identity}` → `athaniel-coder`). |
...
`https://api.supermemory.ai`. Athaniel resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: athaniel`, and document writes stamp
`metadata.sm_source: athaniel`. This is a **functional routing key, not telemetry**:
it groups Athaniel-written memories into a dedicated "Athaniel" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Athaniel profile:

```json
{
  "container_tag": "athaniel-{identity}"
}
```

For a profile named `coder`, this resolves to `athaniel-coder`. The default profile resolves to `athaniel-default`. Without `{identity}`, all profiles share the same container.
