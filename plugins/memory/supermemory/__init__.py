...
_DEFAULT_CONTAINER_TAG = "athaniel"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=athaniel"
...
def _load_supermemory_config(athaniel_home: str) -> dict:
    config = _default_config()
    config_path = Path(athaniel_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, athaniel_home: str) -> None:
    config_path = Path(athaniel_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "athaniel"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Athaniel writes into the "Athaniel" Space in the Supermemory
        ...
        merged = {"sm_source": "athaniel", **(metadata or {})}
...
def _resolve_container_tag_for_setup(athaniel_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(athaniel_home)
...
def _probe_supermemory_connection(api_key: str, athaniel_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(athaniel_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(athaniel_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `athaniel memory setup`.
        # All other options are documented for $ATHANIEL_HOME/supermemory.json
    ...
    def save_config(self, values, athaniel_home):
        ...
        _save_supermemory_config(sanitized, athaniel_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, athaniel_home)
    ...
    def post_setup(self, athaniel_home: str, config: dict) -> None:
        ...
        from athaniel_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, athaniel_home=athaniel_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from athaniel_constants import get_athaniel_home
        self._athaniel_home = kwargs.get("athaniel_home") or str(get_athaniel_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._athaniel_home)
