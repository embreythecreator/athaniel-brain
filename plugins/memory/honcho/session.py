...
def _auth_error_message(exc: BaseException) -> str:
    return (
        ...
        "Re-authenticate with 'athaniel honcho setup'."
    )
...
class HonchoSessionManager:
    ...
    Runs alongside athaniel' existing SQLite state and file-based memory,
    ...
    def get_or_create(self, key: str) -> HonchoSession:
        ...
        assistant_peer_id = self._sanitize_id(
            self._config.ai_peer if self._config else "athaniel-assistant"
        )
    ...
    def dialectic_query(
        ...
        try:
            ...
            # Only automatic injection uses the Athaniel-side character cap.
            if (
                apply_injection_cap
                and result
    ...
    def get_prefetch_context(self, session_key: str, user_message: str | None = None) -> dict[str, str]:
        ...
        # Also fetch AI peer's own representation so Athaniel knows itself.
        try:
    ...
    def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        ...
        Args:
            ...
            memory_dir: Path to the memories directory (~/.athaniel/memories/).
