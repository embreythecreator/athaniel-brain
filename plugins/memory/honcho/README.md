...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/athaniel>
...
```bash
athaniel memory setup honcho   # configure Honcho directly (works on a fresh install)
athaniel memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
athaniel config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.athaniel/.env
```

> `athaniel honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `athaniel memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `athaniel model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ATHANIEL_HOME/honcho.json` | Profile-local (isolated Athaniel instances) |
| 2 | `~/.athaniel/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Athaniel profile: `athaniel` (default) or `athaniel_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `athaniel honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Athaniel session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`athaniel-brain`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`athaniel`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-athaniel-brain`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening athaniel in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Athaniel session ID (timestamp + hex). Every `athaniel` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Athaniel profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`athaniel`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Athaniel profile: `athaniel` (default) or `athaniel_<profile>` (e.g. `athaniel -p coder` -> host key `athaniel_coder`). Older `athaniel.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ATHANIEL_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `athaniel-brain`) |
...
|---------|-------------|
| `athaniel memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `athaniel honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `athaniel memory setup`) |
| `athaniel honcho status` | Show resolved config for active profile |
| `athaniel honcho enable` / `disable` | Toggle Honcho for active profile |
| `athaniel honcho mode <mode>` | Change recall or observation mode |
| `athaniel honcho peer --user <name>` | Update user peer name |
| `athaniel honcho peer --ai <name>` | Update AI peer name |
| `athaniel honcho tokens --context <N>` | Set context token budget |
| `athaniel honcho tokens --dialectic <N>` | Set dialectic max chars |
| `athaniel honcho map <name>` | Map current directory to a session name |
| `athaniel honcho sync` | Create host blocks for all Athaniel profiles |
