...
``athaniel://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Athaniel OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "athaniel-brain"
...
def begin_authorization(
    ...
    *,
    ...
    (``athaniel-desktop`` / ``athaniel-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Athaniel.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ATHANIEL_HOME override can't reach.
