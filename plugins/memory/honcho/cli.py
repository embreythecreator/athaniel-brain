...
Handles: athaniel honcho setup | status | sessions | map | peer
"""
...
from athaniel_cli.config import cfg_get
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "athaniel" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Athaniel profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'athaniel honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'athaniel honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `athaniel update` -- no output, no exceptions.
    """
    try:
        from athaniel_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Athaniel profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ATHANIEL_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_athaniel_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        athaniel_host.get("pinUserPeer"),
        athaniel_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in athaniel_host:
        aliases_src = athaniel_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in athaniel_host:
        prefix_src = athaniel_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(athaniel_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom athaniel_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    athaniel_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        athaniel_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(athaniel_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = athaniel_host.get("userPeerAliases")
    prefix = athaniel_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(athaniel_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(athaniel_host)
    athaniel_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        athaniel_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        athaniel_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/athaniel (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Athaniel persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = athaniel_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            athaniel_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(athaniel_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'athaniel honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'athaniel honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'athaniel honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(athaniel_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'athaniel honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = athaniel_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        athaniel_host["peerName"] = new_peer

    current_ai = athaniel_host.get("aiPeer") or cfg.get("aiPeer", "athaniel")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        athaniel_host["aiPeer"] = new_ai

    current_workspace = athaniel_host.get("workspace") or cfg.get("workspace", "athaniel")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        athaniel_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Athaniel GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, athaniel_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Athaniel gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = athaniel_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(athaniel_host)
            athaniel_host["pinUserPeer"] = False
            if prior_aliases:
                athaniel_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                athaniel_host, current_prefix, prefix_from_root,
            ...
            if merged:
                athaniel_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                athaniel_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                athaniel_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = athaniel_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        athaniel_host["observationMode"] = new_obs
    else:
        athaniel_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(athaniel_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        athaniel_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        athaniel_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = athaniel_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        athaniel_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = athaniel_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        athaniel_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                athaniel_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(athaniel_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            athaniel_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        athaniel_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        athaniel_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        athaniel_host["dialecticReasoningLevel"] = new_reasoning
    else:
        athaniel_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = athaniel_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        athaniel_host["sessionStrategy"] = new_strat

    athaniel_host["enabled"] = True
    athaniel_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        athaniel_config = load_config()
        athaniel_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(athaniel_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: athaniel config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    athaniel honcho status     -- show full config")
    print("    athaniel honcho mode       -- change recall/observation mode")
    print("    athaniel honcho tokens     -- tune context and dialectic budgets")
    print("    athaniel honcho peer       -- update peer names")
    print("    athaniel honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if athaniel_cli internals move."""
    try:
        from athaniel_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(athaniel_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    athaniel_host["apiKey"] = cred.access_token
    athaniel_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        athaniel_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Athaniel profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("athaniel.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: athaniel honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'athaniel honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'athaniel honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: athaniel honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        athaniel = hosts.get(_host_key(), {})
        user = athaniel.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = athaniel.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = athaniel.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = athaniel.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Athaniel' identity in Honcho. Seed with 'athaniel honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: athaniel honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: athaniel honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = athaniel.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = athaniel.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = athaniel.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Athaniel asks Honcho's AI peer a question")
        ...
        print("\n  Set with: athaniel honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'athaniel honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'athaniel honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    athaniel honcho identity --show        — show both peer representations")
        print("    athaniel honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Athaniel + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Athaniel\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Athaniel persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  athaniel honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'athaniel honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'athaniel honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  athaniel honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'athaniel'")
        print("  with Honcho configured and no prior session history.")
        print("  (Athaniel calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    athaniel honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'athaniel honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Athaniel, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'athaniel honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    athaniel honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  athaniel honcho identity <path/to/SOUL.md>")
    ...
    print("    Athaniel:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Athaniel:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Athaniel:   Honcho observes every message and updates representations")
    ...
    print("    Athaniel:   per-session by default — each run gets its own session")
    print("              Map a custom name:  athaniel honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. athaniel honcho setup              — configure API key (required)")
        print("  2. athaniel honcho migrate            — re-run this walkthrough")
    else:
        print("  1. athaniel honcho status             — verify Honcho connection")
        print("  2. athaniel                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. athaniel honcho identity --show    — verify AI peer representation")
        print("  4. athaniel honcho tokens             — tune context and dialectic budgets")
        print("  5. athaniel honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'athaniel memory setup'...\n")
        from athaniel_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``athaniel honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``athaniel honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to athaniel memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Athaniel Honcho",
    )
