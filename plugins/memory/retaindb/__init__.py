...
Config (env vars or athaniel config.yaml under retaindb:):
...
class _Client:
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "athaniel-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "athaniel-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > config.yaml project > athaniel-<profile> > "default"
        ...
        if explicit:
            ...
            athaniel_home = str(kwargs.get("athaniel_home", ""))
            profile_name = os.path.basename(athaniel_home) if athaniel_home else ""
            project = f"athaniel-{profile_name}" if (profile_name and profile_name not in {"", ".athaniel"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "athaniel") or "athaniel"

        from athaniel_constants import get_athaniel_home
        athaniel_home_path = get_athaniel_home()
        db_path = athaniel_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = athaniel_home_path / "SOUL.md"
        if soul_path.exists():
