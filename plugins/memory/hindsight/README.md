...
Athaniel spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.athaniel/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p athaniel ui start
```
...
Config file: `~/.athaniel/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `athaniel` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `athaniel-{profile}` isolates memory per active Athaniel profile. Empty placeholders collapse cleanly (e.g. `athaniel-{user}` with no user becomes `athaniel`). |
...
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `athaniel`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.athaniel/.env` as `HINDSIGHT_LLM_API_KEY`.
