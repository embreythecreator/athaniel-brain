...
the ``athaniel`` integration, and applies a chosen manifest to the user's bank
...
def fetch_athaniel_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``athaniel`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "athaniel" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_athaniel_templates()
