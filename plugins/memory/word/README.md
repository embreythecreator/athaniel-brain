...
Backs Athaniel Brain's long-term memory with **Word**, the angel's memory
...
- **Writes**: durable SQLite write-behind queue (`$ATHANIEL_HOME/word_queue.db`).
...
- Memory notes land in the **Athaniel Memory** notebook (`note_type: ai`),
  created on first use.

## Setup

```bash
athaniel memory setup   # choose: word
```
