...
- OpenViking server running and reachable from Athaniel

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Athaniel can identify older servers that expose the legacy status-only health
...
connection values into Athaniel, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.athaniel/.env`; for a named profile use
...
OpenViking's server config is separate from Athaniel:
...
Athaniel-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Athaniel lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Athaniel sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Athaniel also sends `User-Agent: openviking-memory-athaniel/<version>` on
OpenViking requests. This standard harness identifier contains the Athaniel
...
does not ask for a peer ID. Without a configured peer, Athaniel sends neither
...
order remains environment, linked OpenViking config, then Athaniel YAML. To use
...
on the old implicit `athaniel` peer now use user memory for new writes. Without
...
Set `agent: athaniel` to restore peer-scoped writes. Memories written at user
...
current user). Athaniel caches a confirmed user only for the active connection.
If the probe fails, Athaniel uses the configured user, or `default`, for that
...
Athaniel built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Athaniel action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Athaniel
...
file URIs, such as
`viking://user/default/peers/athaniel/memories/preferences/mem_abc123.md` (any
