...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-athaniel/{_ATHANIEL_VERSION}"
...
_SYNC_TRACE_ENV = "ATHANIEL_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Athaniel slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Athaniel to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Athaniel refused the connection."
        ) from exc
...
def _load_athaniel_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ATHANIEL_HOME", "")).expanduser() if os.environ.get("ATHANIEL_HOME") else Path.home() / ".athaniel"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Athaniel did not start "
    ...
    try:
        ...
        # The Athaniel Desktop backend can include the Athaniel venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Athaniel venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Athaniel venv,
        # aborting `athaniel update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Athaniel will retry on a later access or when "
        "the config changes."
    )
...
def _save_athaniel_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Athaniel session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Athaniel only", "write values only to Athaniel .env"),
    ...
    _save_athaniel_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Athaniel .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See athaniel-brain#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_athaniel_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], athaniel_home: str) -> None:
    ...
    def post_setup(self, athaniel_home: str, config: dict) -> None:
        ...
        from athaniel_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        athaniel_home_path = Path(athaniel_home)
        env_path = athaniel_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_athaniel_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        athaniel_home = str(kwargs.get("athaniel_home") or "").strip()
        if not athaniel_home:
            try:
                from athaniel_constants import get_athaniel_home
                athaniel_home = str(get_athaniel_home())
            except Exception:
                athaniel_home = str(Path.home() / ".athaniel")
        self._athaniel_home = athaniel_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Athaniel will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.athaniel/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts athaniel (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Athaniel will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._athaniel_home:
            return None
        return Path(self._athaniel_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._athaniel_home:
            return None
        return Path(self._athaniel_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_athaniel_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Athaniel' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Athaniel canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See athaniel-brain#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/athaniel/.overview.md).
