...
```bash
athaniel memory setup    # select "mem0"
```
...
Behavioral settings live in `$ATHANIEL_HOME/mem0.json` (set them via `athaniel memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.athaniel/.env`.
...
| `user_id` | `athaniel-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Athaniel session and call `mem0_search` — it connects to your server.
...
```bash
athaniel memory setup mem0 --mode oss \
...
```bash
athaniel memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ATHANIEL_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.athaniel/mem0_qdrant"}}
  }
}
```
...
```bash
athaniel memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ATHANIEL_HOME/mem0.json`).
