...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # athaniel_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(athaniel_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(athaniel_home) / "mem0.json"
    existing = {}
...
def _setup_platform(athaniel_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "athaniel-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "athaniel"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.athaniel/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.athaniel/.env (or unset it) or Athaniel will keep "
    ...
    provider.save_config(provider_config, athaniel_home)

    if env_writes:
        _write_env(Path(athaniel_home) / ".env", env_writes)
...
def _setup_selfhosted(athaniel_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "athaniel-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "athaniel")
...
def _setup_oss(athaniel_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(athaniel_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "athaniel-user")
    ...
    if env_writes:
        _write_env(Path(athaniel_home) / ".env", env_writes)
    _save_mem0_json(athaniel_home, {"mode": "oss", "user_id": user_id, "agent_id": "athaniel", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, athaniel_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # athaniel_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "athaniel-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "athaniel"
...
def _setup_oss_interactive(athaniel_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], athaniel_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], athaniel_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'athaniel-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "athaniel-user")

    agent_id = input("  Agent ID [athaniel]: ").strip()
    agent_id = agent_id or "athaniel"
    ...
    if env_writes:
        _write_env(Path(athaniel_home) / ".env", env_writes)
    _save_mem0_json(athaniel_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(athaniel_home: str, config: dict) -> None:
    """Entry point called by athaniel memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(athaniel_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(athaniel_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(athaniel_home, config, flags)
        return
