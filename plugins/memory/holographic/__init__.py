"""athaniel-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $ATHANIEL_HOME/config.yaml (profile-scoped):
  plugins:
    athaniel-memory-store:
      db_path: $ATHANIEL_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "athaniel-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, athaniel_home):
        """Write config to config.yaml under plugins.athaniel-memory-store."""
        from pathlib import Path
        config_path = Path(athaniel_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["athaniel-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_athaniel_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _athaniel_home = str(get_athaniel_home())
        _default_db = _athaniel_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $ATHANIEL_HOME in user-supplied paths so config values like
        # "$ATHANIEL_HOME/memory_store.db" or "~/.athaniel/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$ATHANIEL_HOME", _athaniel_home)
            db_path = db_path.replace("${ATHANIEL_HOME}", _athaniel_home)
