"""Meta Model API (Muse Spark) provider plugin for Athaniel Brain.
...
Bundled from https://github.com/albertodepaola/athaniel-meta-provider. Athaniel'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to athaniel' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Athaniel' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in athaniel_cli/providers.py and the retention hint in
