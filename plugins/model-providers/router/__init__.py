"""Ramp Router (router.com) provider plugin for Athaniel Brain.
...
* **Responses API is the native wire.** Router serves ``GET /v1/models``
  ...
  in ``athaniel_cli/providers.py`` keep every path on the native wire —
...
``athaniel_cli/models.py``: cache-only lookups on the per-request hot path
...
def _resolve_api_key() -> str:
    ...
    raw environment when the athaniel_cli helper is unavailable (e.g. stripped
    test environments).
    """
    resolvers = []
    try:
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_athaniel_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
def _warm_efforts_async() -> None:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        # Match the canonical caps warmer (athaniel_cli/models.py): a mid-suite
...
router = RouterProfile(
    ...
    base_url=_base_url(),
    auth_type="api_key",
    # Identify Athaniel traffic to the gateway (Router attributes coding-agent
    ...
    default_headers={"User-Agent": f"Athaniel-Agent/{_ATHANIEL_VERSION}"},
