...
This plugin ships bundled with Athaniel but is **opt-in** — it only loads when
...
athaniel tools  # → Langfuse Observability
...
Set these in `~/.athaniel/.env` (or via `athaniel tools`):
...
athaniel chat -q "hello"              # then check Langfuse for a "Athaniel turn" trace
```

Generation observations include the Athaniel system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ATHANIEL_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
ATHANIEL_LANGFUSE_ENV=production       # environment tag
ATHANIEL_LANGFUSE_RELEASE=v1.0.0       # release tag
ATHANIEL_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
ATHANIEL_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
ATHANIEL_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ATHANIEL_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ATHANIEL_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
athaniel plugins disable observability/langfuse
```
