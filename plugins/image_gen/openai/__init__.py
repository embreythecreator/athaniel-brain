...
three virtual model IDs so the ``athaniel tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `athaniel tools` → Image "
                    "Generation → OpenAI to configure, or `athaniel setup` "
                    "to add the key."
                ),
