...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=athaniel``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`athaniel_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``athaniel tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from athaniel the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `athaniel tools` → Image "
                    "Generation → DeepInfra to configure, or `athaniel setup` "
                    "to add the key."
                ),
        ...
        from athaniel_cli.models import deepinfra_base_url
