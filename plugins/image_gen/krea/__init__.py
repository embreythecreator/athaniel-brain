...
# Athaniel uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _poll_krea_job(
    ...
    *,
    ...
    headers = {
        ...
        "User-Agent": "Athaniel-Agent/1.0 (krea-image-gen)",
    }
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `athaniel tools` → Image "
        ...
        headers = {
            "Authorization": f"Bearer {auth_token}",
            "Content-Type": "application/json",
            "User-Agent": "Athaniel Brain/1.0 (krea-image-gen)",
        }
        if managed is not None:
            ...
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "`athaniel tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
