...
agent's existing :func:`~athaniel_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``athaniel tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``athaniel tools``. Nous
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``athaniel tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `athaniel tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/embreythecreator/athaniel-brain",
