...
``athaniel_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__ATHANIEL_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``athaniel_cli/web_server.py``.

This means ``athaniel dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from athaniel_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``athaniel_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from athaniel_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``athaniel_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from athaniel_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from athaniel_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``ATHANIEL_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from athaniel_cli.kanban import _check_dispatcher_presence
                from athaniel_constants import get_athaniel_home

                # Scope the probe to the request's active home. The dashboard
                # backend can run under a different ATHANIEL_HOME than the
...
from athaniel_cli.kanban_db import (  # noqa: E402
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``athaniel kanban
...
# spawn failures, stuck-blocked). See athaniel_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``athaniel kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``athaniel kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``athaniel kanban specify <task_id>``.
    ...
    # ATHANIEL_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from athaniel_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``athaniel kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.athaniel/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Athaniel profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.athaniel/profiles/*`` on disk and every
...
def model_options():
    ...
    Thin wrapper around ``athaniel_cli.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of Athaniel
...
def list_kanban_projects():
    """List first-class Athaniel projects for board scoping.
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from athaniel_constants import get_athaniel_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_athaniel_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``athaniel profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from athaniel_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``athaniel kanban decompose <task_id>``.
    ...
    # ATHANIEL_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from athaniel_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.athaniel/config.yaml.
