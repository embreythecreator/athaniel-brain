"""`athaniel meet node ...` subcommand tree.

Wired into the existing ``athaniel meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``athaniel meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="athaniel-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``athaniel meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             athaniel meet node approve <name> ws://<host>:{args.port} {token}")
        try:
