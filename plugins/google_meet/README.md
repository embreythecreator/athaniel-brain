...
Let the athaniel agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where athaniel runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `athaniel meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `athaniel meet` CLI |
...
| `cli.py` | `athaniel meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ATHANIEL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `athaniel meet node {run,list,approve,remove,status,ping}` |
...
athaniel meet install                                      # pip + Chromium
athaniel meet setup                                        # preflight
athaniel meet auth                                         # optional
athaniel meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
athaniel meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.athaniel/.env
athaniel meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
athaniel meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
athaniel meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.athaniel/.env
athaniel meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, athaniel will **not** switch your system audio input automatically — the
...
athaniel plugins enable google_meet
athaniel meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
athaniel meet node ping my-mac
