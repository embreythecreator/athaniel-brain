...
session-scoped state file under ``$ATHANIEL_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ATHANIEL_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_athaniel_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this athaniel install, leave it first —
    ...
    env = os.environ.copy()
    env["ATHANIEL_MEET_URL"] = url
    env["ATHANIEL_MEET_OUT_DIR"] = str(out)
    env["ATHANIEL_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ATHANIEL_MEET_HEADED"] = "1"
    if auth_state:
        env["ATHANIEL_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ATHANIEL_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ATHANIEL_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ATHANIEL_MEET_MODE"] = mode
    if realtime_model:
        env["ATHANIEL_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ATHANIEL_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ATHANIEL_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    ...
    # meet_bot checks ATHANIEL_MEET_REALTIME_KEY before OPENAI_API_KEY.
    if not realtime_api_key:
        try:
            from agent.secret_scope import get_secret

            realtime_api_key = (
                get_secret("ATHANIEL_MEET_REALTIME_KEY")
                or get_secret("OPENAI_API_KEY")
            )
        except ImportError:  # pragma: no cover — secret_scope is in-repo
            pass
    if realtime_api_key:
        env["ATHANIEL_MEET_REALTIME_KEY"] = realtime_api_key
