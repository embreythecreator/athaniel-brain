"""disk_cleanup — ephemeral file cleanup for Athaniel Brain.
...
Rules:
  ...
  - empty dirs    → always delete (under ATHANIEL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ATHANIEL_HOME and /tmp/athaniel-*
Never touches: ~/.athaniel/logs/ or any system directory.
"""
...
try:
    ...
    def get_athaniel_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ATHANIEL_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".athaniel").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ATHANIEL_HOME/logs/``."""
    return get_athaniel_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ATHANIEL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ATHANIEL_HOME or ``/tmp/athaniel-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    athaniel_home = get_athaniel_home()
    try:
        path.resolve().relative_to(athaniel_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/athaniel-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("athaniel-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "athaniel-brain", "backups", "profiles", ".worktrees",
...
# Paths under $ATHANIEL_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ATHANIEL_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        athaniel_home = get_athaniel_home()
        for parent in ("cron", "cronjobs"):
            base = athaniel_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ATHANIEL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ATHANIEL_HOME, but never recurse into known
    # durable state trees.  Some installs place the Athaniel checkout, venv,
    # and desktop build under ATHANIEL_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    athaniel_home = get_athaniel_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in athaniel_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    athaniel_home = get_athaniel_home()
    try:
        rel = path.resolve().relative_to(athaniel_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "athaniel-brain",
        ...
        # Path isn't under ATHANIEL_HOME (e.g. /tmp/athaniel-*) — fall through.
        pass
