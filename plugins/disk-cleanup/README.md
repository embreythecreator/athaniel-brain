...
Auto-tracks and cleans up ephemeral files created during Athaniel Brain
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ATHANIEL_HOME` and `/tmp/athaniel-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ATHANIEL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ATHANIEL_HOME` or `/tmp/athaniel-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ATHANIEL_HOME/disk-cleanup/` is itself excluded
- `$ATHANIEL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
