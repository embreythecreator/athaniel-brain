...
``athaniel tools`` can prompt the user to install it.
...
GIL — so the whole Athaniel process freezes through Ctrl+C/SIGTERM. Each search
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``athaniel tools`` probes.
    ...
    if _test_hook:
        env["ATHANIEL_DDGS_ALLOW_TEST_HOOKS"] = "1"
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        ``primp`` call cannot freeze the Athaniel process (#36776, #68096).
        """
        try:
