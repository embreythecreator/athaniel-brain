...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `athaniel tools`
    ...
    existing tests and the ``athaniel tools`` setup flow.
    """
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            ...
            "run `athaniel tools` and select Nous Subscription as the web provider."
        )
    else:
