...
``{title, url, description, position}`` rows every other Athaniel web
provider produces.
...
prefers Athaniel-managed xAI Grok OAuth (via ``athaniel auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.athaniel/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``athaniel tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `athaniel auth` to sign in with "
