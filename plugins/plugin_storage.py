...
``<athaniel home>/plugins/<name>/``. That tree is the plugin *install* dir —
``athaniel plugins remove`` deletes it and ``athaniel plugins update`` git-pulls
...
``<athaniel home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Athaniel.
...
# Mirrors the plugin-name shape `athaniel plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<athaniel home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`athaniel_constants.get_athaniel_home` on every call. Don't cache the
    result across profile switches.
    """
    from athaniel_constants import get_athaniel_home

    root = get_athaniel_home() / "plugin-data" / _validate_name(name)
