"""
Timezone-aware clock for Athaniel.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``ATHANIEL_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.athaniel/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Athaniel never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Athaniel requires 3.9+)
    from backports.zoneinfo import ZoneInfo  # type: ignore[no-redef]

# Cached state, keyed to the active timezone source. This process can multiplex
# profiles by switching ATHANIEL_HOME (context override or env), so a single
...
def _timezone_cache_identity() -> Tuple[str, str]:
    ...
    tz_env = os.getenv("ATHANIEL_TIMEZONE", "").strip()
    if tz_env:
...
def get_timezone() -> Optional[ZoneInfo]:
    ...
    ``ATHANIEL_TIMEZONE`` override or the active profile's config path — so a
...
def reset_cache() -> None:
    ...
    config edit or ``ATHANIEL_TIMEZONE`` update) to force ``get_timezone()`` /
