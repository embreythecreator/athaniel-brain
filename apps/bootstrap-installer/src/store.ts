...
export const $athanielHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.athaniel/logs/bootstrap-installer.log')
    $athanielHome.set('~/.athaniel')
  ...
  try {
    const [logPath, athanielHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_athaniel_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $athanielHome.set(athanielHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Athaniel" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Athaniel-Setup.exe --update);
...
export async function launchAthanielDesktop(): Promise<void> {
  ...
  await invoke('launch_athaniel_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Athaniel repository' },
