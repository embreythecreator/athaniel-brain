...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Athaniel' : 'Setting up Athaniel Brain'

  const description = isUpdate
    ? 'Athaniel is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Athaniel installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="athaniel-fade-in flex h-full flex-col">
