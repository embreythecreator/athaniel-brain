...
/// read side does not see EOF until the last of them closes it. `athaniel update`
...
/// stranded with it.
///
/// Same bound `Invoke-AthanielStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `athaniel_home_override` propagates to the child as $ATHANIEL_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, athaniel_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = athaniel_home_override {
        cmd.env("ATHANIEL_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, athaniel_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = athaniel_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_athaniel_home() {
    ...
    /// child. `athaniel update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
