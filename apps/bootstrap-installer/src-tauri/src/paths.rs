...
//!
//! Mirrors `athaniel_constants.get_athaniel_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\athaniel
//!   macOS:   ~/.athaniel
//!   Linux:   ~/.athaniel  (override via $ATHANIEL_HOME)
//!
//! NOTE (macOS): Python's get_athaniel_home(), scripts/install.sh, and the
//! Electron desktop's resolveAthanielHome() ALL use ~/.athaniel on macOS — there
...
/// Returns the canonical Athaniel home directory, respecting $ATHANIEL_HOME if set.
pub fn athaniel_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ATHANIEL_HOME") {
    ...
    {
        // %LOCALAPPDATA%\athaniel — matches scripts/install.ps1's $AthanielHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("athaniel");
        }
    }

    // macOS + Linux + fallback: ~/.athaniel (matches Python get_athaniel_home(),
    // install.sh, and the Electron desktop's resolveAthanielHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".athaniel");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".athaniel")
}

pub fn log_dir() -> PathBuf {
    athaniel_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    athaniel_home().join("bootstrap-cache")
}
...
/// ATHANIEL_HOME so it survives repo checkout deletion (unlike anything under
/// athaniel-brain/).
///
/// On Windows this is `%LOCALAPPDATA%\athaniel\athaniel-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_athaniel`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ATHANIEL_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ATHANIEL_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    athaniel_home().join(".athaniel-update-in-progress")
}
...
pub fn copy_self_to_athaniel_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ATHANIEL_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ATHANIEL_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".athaniel-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ATHANIEL_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[athaniel-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ATHANIEL_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_athaniel_home() -> String {
    athaniel_home().to_string_lossy().into_owned()
}
