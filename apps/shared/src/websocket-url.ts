...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update Athaniel Desktop and try again.')
    }
...
export interface AthanielWebSocketUrlOptions {
...
export function buildAthanielWebSocketUrl(options: AthanielWebSocketUrlOptions): string {
