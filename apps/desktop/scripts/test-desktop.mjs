...
// no longer bundles the Athaniel Brain Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Athaniel.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Athaniel'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Athaniel.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Athaniel'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ATHANIEL_HOME for non-sandboxed runs -- matches main.ts's
// resolveAthanielHome(). On Windows it's %LOCALAPPDATA%\athaniel; elsewhere
// it's ~/.athaniel. The fresh-install sandbox launchFresh() sets its own
// ATHANIEL_HOME and never touches this.
const DEFAULT_ATHANIEL_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'athaniel')
  }
  return path.join(os.homedir(), '.athaniel')
})()
const VENV_ROOT = path.join(DEFAULT_ATHANIEL_HOME, 'athaniel-brain', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'athaniel-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ATHANIEL_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Athaniel-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Athaniel-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Athaniel-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Athaniel-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ATHANIEL_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ATHANIEL_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const athanielHome = path.join(sandbox, 'athaniel-home')
  ...
  env.ATHANIEL_DESKTOP_CWD = cwd
  env.ATHANIEL_DESKTOP_IGNORE_EXISTING = '1'
  env.ATHANIEL_DESKTOP_TEST_MODE = 'fresh-install'
  env.ATHANIEL_DESKTOP_USER_DATA_DIR = userDataDir
  env.ATHANIEL_HOME = athanielHome
  delete env.ATHANIEL_DESKTOP_ATHANIEL
  delete env.ATHANIEL_DESKTOP_ATHANIEL_ROOT
  ...
  console.log(`  ATHANIEL_HOME: ${athanielHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(athanielHome, 'athaniel-brain', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Athaniel Brain Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of athaniel_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'athaniel-brain', 'athaniel_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Athaniel
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ATHANIEL_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ATHANIEL_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
