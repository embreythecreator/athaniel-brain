 ...
 * product name (`Athaniel`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Athaniel'
 *
 * This is a hard failure with no obvious cause for the user — `athaniel desktop`
 ...
 * (the mac-unpacked Athaniel.app bundle) and Windows (win-unpacked), so we
 ...
 * Athaniel.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `athaniel desktop --build-only` (athaniel_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Athaniel.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Athaniel'}.exe`
