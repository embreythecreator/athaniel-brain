...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `athaniel-dev-mock-${Date.now()}`))
  ...
  return { root, athanielHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(athanielHome, mockUrl) {
  fs.writeFileSync(
    path.join(athanielHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(athanielHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.athanielHome, mock.url)
  console.log(`  ATHANIEL_HOME: ${sandbox.athanielHome}`)
  ...
  const env = {
    ...
    ATHANIEL_DESKTOP_APP_NAME: `AthanielDevMock-${Date.now()}`,
  }
