...
//   · its own ATHANIEL_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · ATHANIEL_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated ATHANIEL_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate ATHANIEL_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = athanielHome ?? mkTemp('athaniel-perf-home-')
  const userData = userDataDir ?? mkTemp('athaniel-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !athanielHome) {
    seedConfigFrom(join(homedir(), '.athaniel'), home)
  }
  ...
  try {
    ...
    // ATHANIEL_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set ATHANIEL_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      ATHANIEL_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.ATHANIEL_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'athaniel-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'athaniel-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.athaniel'), home)
