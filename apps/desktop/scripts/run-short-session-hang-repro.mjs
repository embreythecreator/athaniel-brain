...
const UPSTREAM_URL = 'https://github.com/embreythecreator/athaniel-brain.git'
...
const LEGACY_STORAGE_KEY = 'athaniel.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'athaniel.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.athanielDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `athaniel-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const athanielHome = join(sandbox, 'athaniel-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(athanielHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(athanielHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ATHANIEL_DESKTOP_APP_NAME: `AthanielShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ATHANIEL_DESKTOP_ATHANIEL_ROOT: target.targetRoot,
        ATHANIEL_DESKTOP_IGNORE_EXISTING: '1',
        ATHANIEL_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'athaniel-short-session-ab-'))
  const prepared = []
