#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Athaniel icon + version metadata onto the
// built Athaniel.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Athaniel.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `athaniel desktop`, the installer's
...
// path (which rebuilds via `athaniel desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Athaniel.exe>
//
...
// Stamp the Athaniel icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Athaniel',
      FileDescription: 'Athaniel',
      CompanyName: 'Athaniel',
      LegalCopyright: 'Copyright (c) 2026 Athaniel'
    }
  })

  console.log('[set-exe-identity] done — Athaniel icon + identity stamped')
}
