...
test('error page names the failure and carries a Reload button', () => {
  ...
  assert.match(html, /Athaniel couldn.t start the desktop UI/)
  ...
  assert.match(html, /athaniel desktop --force-build/)
...
test('error page reload button targets the real renderer URL when provided', () => {
  const html = buildRendererLoadErrorPage({
    ...
    reloadUrl: 'file:///C:/Athaniel%20Agent/dist/index.html'
  })
  ...
  assert.match(html, /location\.replace\("file:\/\/\/C:\/Athaniel%20Agent\/dist\/index\.html"\)/)
