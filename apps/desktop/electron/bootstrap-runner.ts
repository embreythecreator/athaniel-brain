 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Athaniel Brain by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.athaniel-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(athanielHome) {
  return path.join(athanielHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.athaniel/athaniel-brain. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(athanielHome) {
  if (!athanielHome) {
    return null
  }

  const candidate = path.join(athanielHome, 'athaniel-brain', 'scripts', installScriptName())
...
function cachedScriptPath(athanielHome, commit) {
  return path.join(bootstrapCacheDir(athanielHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/embreythecreator/athaniel-brain/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(athanielHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(athanielHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, athanielHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ATHANIEL_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ATHANIEL_HOME: athanielHome || process.env.ATHANIEL_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, athanielHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, athanielHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--athaniel-home', athanielHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, athanielHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, athanielHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, athanielHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(athanielHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, athanielHome, emit })
