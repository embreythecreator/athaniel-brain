...
test('primary SSH reuse rejects a descriptor with a different remote Athaniel path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteAthanielPath: '/srv/athaniel', user: 'alice' },
    profiles: {}
  })
    ...
    await reuseMatchingPrimarySshBackend({
      ...
      ensurePrimary: async () => ({
        ...
        ssh: {
          effectiveConfigFingerprint: 'same-effective-config',
          host: 'build-host',
          remoteAthanielPath: '/opt/athaniel',
          remoteProfile: '',
          user: 'alice'
        }
      }),
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    primary: 'athaniel-vps',
    launchMode: 'primary',
    lastUsed: 'athaniel-vps',
    connections: [
      { id: LOCAL_CONNECTION_ID, kind: 'local', label: 'This device' },
      { id: 'athaniel-vps', kind: 'ssh', label: 'Athaniel VPS', host: 'athaniel-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'athaniel-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'athaniel-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'athaniel-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  const base = {
    ...
    keyPath: '/keys/a',
    kind: 'ssh' as const,
    remoteAthanielPath: '/srv/athaniel',
    remoteProfile: 'alpha',
    user: 'root'
  }
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteAthanielPath: '/opt/athaniel' },
  ...
  assert.equal(resolve({ ...base, remoteAthanielPath: '/opt/athaniel' }), 'ssh-path')
...
// The electron and @athaniel/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@athaniel/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'athaniel-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('token only persists on token-auth remotes; oauth/cloud drop it', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'C', url: 'https://c.athaniel.cloud', authMode: 'oauth', token: { enc: 'x' } },
    registry
  )
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteAthanielPath, '/opt/athaniel')
...
test('remote input normalizes URL and auth mode; cloud keeps org', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'Cloud', url: 'https://foo.athaniel.cloud', authMode: 'oauth', org: 'nous' },
    registry
  )
  ...
  const input = {
    ...
    connections: [
      ...
      },
      {
        id: 'cloud-1',
        kind: 'cloud',
        label: 'Athaniel Cloud',
        url: 'https://a.athaniel.cloud',
        authMode: 'oauth',
        org: 'nous'
      },
...
test('migrate: v1 cloud keeps cloud provenance + org', () => {
  const registry = migrateV1ToRegistry({
    mode: 'cloud',
    remote: { url: 'https://a.athaniel.cloud', authMode: 'oauth', org: 'nous' }
  })
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'athaniel-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'athaniel-alex')
  assert.equal(applied.lastUsed, 'athaniel-alex')
})
