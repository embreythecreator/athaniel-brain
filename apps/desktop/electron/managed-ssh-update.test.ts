...
test('POSIX managed launcher is detached, correlation-scoped, and never publishes handoff exit 75', () => {
  ...
  assert.match(command, new RegExp(`ATHANIEL_UPDATE_CORRELATION_ID=.*${CORRELATION}`))
...
test('POSIX managed launcher executes the updater command and atomically publishes its status', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'athaniel-managed-launch-'))
...
test('Windows managed launcher starts a hidden child and leaves exit 75 to the external coordinator', () => {
  const command = buildWindowsManagedUpdateLaunch(
    {
      ...
      pythonPath: 'C:\\Athaniel\\python.exe'
    },
    CORRELATION
  )
  ...
  assert.match(wrapper, /ATHANIEL_UPDATE_WINDOWS_DETACHED/)
  assert.match(wrapper, /ATHANIEL_UPDATE_TAURI_READY_PATH/)
  assert.match(wrapper, /ATHANIEL_UPDATE_TAURI_OUTCOME_PATH/)
...
test('POSIX observer reads the exact correlation receipt and terminal marker from disk', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'athaniel-managed-update-'))
    ...
    const command = buildRemoteUpdateObservationCommand(
      {
        ssh: { exec: async () => '' },
        platform: 'Linux',
        athanielPath: '/opt/athaniel/athaniel',
        athanielHome: home
      },
      CORRELATION
    )
...
test('managed observer unwraps a named profile home for the install-wide marker', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'athaniel-managed-profile-marker-'))
  const profileHome = path.join(root, 'profiles', 'research')

  try {
    await mkdir(profileHome, { recursive: true })
    await writeFile(path.join(root, '.athaniel-update-in-progress'), `${process.pid}\n1\n`)
