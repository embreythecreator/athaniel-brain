 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `athaniel update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.athanielHome, 'Athaniel home')
  const athanielPath = validateRemoteValue(target.athanielPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(athanielPath)

  const updateCommand =
    `env ATHANIEL_HOME=${homeWord} ` +
    `ATHANIEL_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ATHANIEL_UPDATE_ORIGIN_PROFILE=default ' +
    `ATHANIEL_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ATHANIEL_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ATHANIEL_HOME=${psLiteral(home)}`,
    `$env:ATHANIEL_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ATHANIEL_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ATHANIEL_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ATHANIEL_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ATHANIEL_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ATHANIEL_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ATHANIEL_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(athanielPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.athaniel-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Athaniel updated and every managed SSH profile is ready.'
        : restoreOk
