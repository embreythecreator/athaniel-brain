...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', ATHANIEL_HOME: '/home/b/.athaniel' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'athaniel_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.athaniel/athaniel-brain/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.athaniel/athaniel-brain' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.athaniel/athaniel-brain'`,
      `exec '/home/b/.athaniel/athaniel-brain/venv/bin/python' '-m' 'athaniel_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\athaniel\\venv\\Scripts\\athaniel.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\athaniel\\venv\\Scripts\\athaniel.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
