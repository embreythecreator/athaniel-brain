...
export function markerPath(athanielHome) {
  return path.join(athanielHome, '.athaniel-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(athanielHome)
  let raw
 ...
 *
 * The Tauri-based athaniel-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(athanielHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(athanielHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Athaniel to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(athanielHome, opts)
