 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Athaniel dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Athaniel install on the remote (login-shell probe),
 ...
 *     `ATHANIEL_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForAthaniel in.
 *
 * The minted ATHANIEL_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ATHANIEL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.athaniel/desktop-ssh'
...
// Resolve the remote athaniel executable. An EXPLICIT path is honored strictly
...
async function locateAthaniel(ssh, remoteAthanielPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The athaniel binary or wrapper script
    ...
    //     the Athaniel version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/embreythecreator/athaniel-brain/issues/74411
    return candidate
  }
  ...
  if (remoteAthanielPath) {
    if (await isExecutable(remoteAthanielPath)) {
      return resolveLauncher(remoteAthanielPath)
    }

    const err: any = new Error(
      `The Athaniel path you set is not an executable on the remote host: "${remoteAthanielPath}". ` +
        'Check the path (it must be the full path to the `athaniel` binary on the remote, e.g. ' +
        '~/athaniel-brain/.venv/bin/athaniel), or clear it to auto-detect.'
    )

    err.kind = 'athaniel-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v athaniel')}`)).trim()
  ...
  candidates.push('~/.local/bin/athaniel')
  candidates.push('/usr/local/bin/athaniel')
  candidates.push('~/.athaniel/athaniel-brain/venv/bin/athaniel')
  ...
  const err: any = new Error(
    'Athaniel is not installed on the remote host (could not find a `athaniel` executable). ' +
      'Install it on the remote with:  curl -fsSL https://athaniel-brain.nousresearch.com/install.sh | sh  ' +
      '— or set the Athaniel path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<athaniel> --version`,
// e.g. "Athaniel Brain v0.18.2 ..."), or '' on failure. Surfaces WHICH athaniel a
// connection uses, so a stale/unexpected install is visible.
async function probeAthanielVersion(ssh, athanielPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(athanielPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Athaniel Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ATHANIEL_HOME the remote dashboard will use (explicit env wins, else
// ~/.athaniel). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteAthanielHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ATHANIEL_HOME:-$HOME/.athaniel}"')).trim().split('\n').pop()

    return out || '~/.athaniel'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Athaniel home.')
...
marker=home/'.athaniel-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Athaniel checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, athanielHome) {
  const home = assertSafeRemoteHome(athanielHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Athaniel install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Athaniel update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Athaniel update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteAthanielProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteAthanielHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Athaniel profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Athaniel home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'athanielPath', 'athanielHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !athanielPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(athanielPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/athaniel`, leaving neither
      // launcher nor ATHANIEL_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `athaniel_home=os.path.expanduser(${shq(athanielHome)}) if ${shq(athanielHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if athaniel_home:\n' +
      ' expected_entries.add(os.path.join(athaniel_home,"athaniel-brain","venv","bin","athaniel"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.athanielPath)
  const expectedHome = lock.athanielHome ? expandRemotePath(lock.athanielHome) : "''"
  ...
  const executableMatch = lock.athanielHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.athanielPath === expected.athanielPath &&
    lock.athanielHome === expected.athanielHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.athanielPath)})
athaniel_home=os.path.expanduser(${py(lock.athanielHome)})
expected_entries={expected_path,os.path.join(athaniel_home,"athaniel-brain","venv","bin","athaniel")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"athaniel-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(athanielPath, profile, opts: any = {}) {
  const athaniel = expandRemotePath(athanielPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.athanielHome || '~/.athaniel')}/.athaniel-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.athanielHome || '~/.athaniel')}/.athaniel-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ATHANIEL_DESKTOP=1 ${athaniel} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} athaniel-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, athanielPath) {
  const athaniel = expandRemotePath(athanielPath)

  const out = await ssh.exec(
    `help="$(${athaniel} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { athanielPath, profile, token, ownershipId, athanielHome = '~/.athaniel', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, athanielPath))) {
    const err: any = new Error(
      'The remote Athaniel install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Athaniel on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(athanielPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        athanielHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForAthaniel, a token-gated
...
async function connect(deps) {
  ...
  const athanielHome = await probeRemoteAthanielHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, athanielHome)
  const athanielPath = await locateAthaniel(ssh, remoteAthanielPath)
  log(`located athaniel at ${athanielPath}`)
  const athanielVersion = await probeAthanielVersion(ssh, athanielPath)

  if (athanielVersion) {
    log(`remote athaniel version: ${athanielVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Athaniel Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.athanielPath === athanielPath &&
      lock.athanielHome === athanielHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    athanielHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, athanielHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAthaniel(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
...
export {
  ...
  ownershipDirectory,
  pidIsOurDashboard,
  probeAthanielVersion,
  probeRemoteAthanielHome,
  probeRemotePlatform,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,
