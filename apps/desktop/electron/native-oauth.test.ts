...
test('resolveLoginStrategy returns native for native_pkce gateway with non-password provider', () => {
  const statusBody = { auth_required: true, auth_flows: ['cookie', 'native_pkce'] }
  const providers = [{ name: 'nous', displayName: 'Athaniel', supportsPassword: false }]
...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/athaniel', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/athaniel/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/athaniel'), 'https://gw.example.com/athaniel/auth/native/refresh')
})
