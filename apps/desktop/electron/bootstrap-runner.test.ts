...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'athaniel-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/athaniel-runner-test',
    sourceRepoRoot: null,
    athanielHome: '/tmp/athaniel-runner-test',
    logRoot: '/tmp/athaniel-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'athaniel-brain', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'athaniel-brain')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/athaniel-brain',
      athanielHome: '/tmp/athaniel'
    }),
    ['--dir', '/tmp/athaniel-brain', '--athaniel-home', '/tmp/athaniel', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/athaniel-brain', '--athaniel-home', '/tmp/athaniel', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/athaniel',
      athanielHome: '/tmp/home'
    }),
    ['--dir', '/tmp/athaniel', '--athaniel-home', '/tmp/home', '--branch', 'main']
  )
})
