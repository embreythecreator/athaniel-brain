...
const SCAN_MODULE = 'athaniel_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Athaniel processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'athaniel_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Athaniel process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Athaniel installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Athaniel windows and terminals, then retry.  If the problem\n' +
    'persists, run `athaniel update` in a terminal for detailed diagnostics.'
  )
}
