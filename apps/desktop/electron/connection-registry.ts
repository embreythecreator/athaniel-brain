 ...
 * runtime, remote gateways, Athaniel Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@athaniel/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteAthanielPath || '').trim() !== String(activeSsh.remoteAthanielPath || '').trim() ||
...
/** Turn `ls ~/.athaniel/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Athaniel Cloud
 * instances are platform-managed — we never run `athaniel update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteAthanielPath: input.remoteAthanielPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Athaniel-managed names; an empty result stores nothing.
 ...
 * `remoteAthanielPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Athaniel Cloud' : 'Remote gateway'),
