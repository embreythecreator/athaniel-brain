...
//   ATHANIEL_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ATHANIEL_WIN_SSH_USER   remote user
//   ATHANIEL_WIN_SSH_ATHANIEL absolute path to the remote athaniel.exe under test
const liveHost = process.env.ATHANIEL_WIN_SSH_HOST || ''
const liveUser = process.env.ATHANIEL_WIN_SSH_USER || ''
const configuredAthaniel = process.env.ATHANIEL_WIN_SSH_ATHANIEL || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Athaniel-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredAthaniel)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredAthaniel.replace('athaniel.exe', 'python.exe')}' -m athaniel_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredAthaniel.replace('athaniel.exe', 'python.exe')
        const terminate = `& '${python}' -m athaniel_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.athanielPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m athaniel_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m athaniel_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
