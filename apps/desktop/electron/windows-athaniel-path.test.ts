// Unit tests for the pure Windows `athaniel` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvAthanielCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `athaniel` shim shadows
//      the real athaniel.cmd/athaniel.exe.
...
//   3. resolveVenvAthanielCommand() — must probe the venv python via
//      canImportAthanielCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthaniel: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthaniel: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthaniel: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvAthaniel: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvAthaniel: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvAthaniel: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvAthanielCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportAthanielCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    athanielHome: '/fake/athaniel-home',
...
test('resolveVenvAthanielCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvAthanielCommand('/root/venv/Scripts/athaniel.exe', [], deps), null)
})

test('resolveVenvAthanielCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvAthanielCommand('/root/venv/Scripts/athaniel.cmd', [], deps), null)
})

test('resolveVenvAthanielCommand: returns null when the basename is not athaniel/athaniel.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAthanielCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvAthanielCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAthanielCommand('/root/venv/bin/athaniel.exe', [], deps), null)
})

test('resolveVenvAthanielCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvAthanielCommand('/root/venv/Scripts/athaniel.exe', [], deps), null)
})

test('resolveVenvAthanielCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportAthanielCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvAthanielCommand('/root/venv/Scripts/athaniel.exe', ['serve'], deps)
...
test('resolveVenvAthanielCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvAthanielCommand('/root/venv/Scripts/athaniel.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'athaniel_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvAthanielCommand: is case-insensitive on athaniel.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvAthanielCommand('/root/venv/Scripts/ATHANIEL.EXE', [], deps))
  assert.ok(resolveVenvAthanielCommand('/root/venv/SCRIPTS/athaniel.exe', [], deps))
})
