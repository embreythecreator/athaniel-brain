...
test('sandbox marker round-trips through the userData file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athaniel-sandbox-marker-'))
...
test('buildIcaclsGrantArgs targets ALL APPLICATION PACKAGES with inherited RX', () => {
  assert.deepEqual(buildIcaclsGrantArgs('C:\\Athaniel\\win-unpacked'), [
    'C:\\Athaniel\\win-unpacked',
    '/grant',
...
test('grantAllApplicationPackagesAcl is a no-op off Windows and reports exec failures', () => {
  ...
  const ok = grantAllApplicationPackagesAcl('C:\\Athaniel', {
    platform: 'win32',
  ...
  assert.deepEqual(calls[0]?.args, buildIcaclsGrantArgs('C:\\Athaniel'))

  const failed = grantAllApplicationPackagesAcl('C:\\Athaniel', {
    platform: 'win32',
    execFileSync() {
...
test('buildNoSandboxRelaunchArgs appends a single --no-sandbox flag', () => {
  assert.deepEqual(buildNoSandboxRelaunchArgs(['--foo', '--no-sandbox', 'athaniel://x']), [
    '--foo',
    'athaniel://x',
    '--no-sandbox'
  ])
