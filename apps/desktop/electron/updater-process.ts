 ...
 *
 * The staged Tauri `athaniel-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `athaniel update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `athaniel update`, swaps/relaunches
 * the app, and writes .athaniel-update-result.json. With the app gone before
 * the update starts, the ATHANIEL_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `athaniel update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ATHANIEL_HOME on *every* platform
 * (`athaniel-setup.exe` on Windows, `athaniel-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_athaniel_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `athaniel update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(athanielHome, 'athaniel-setup.exe')
 ...
 *
 * `copy_self_to_athaniel_home` deliberately no-ops during `--update`
 ...
 * "Another Athaniel update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `athaniel update`, which
 ...
 * the installer is written to ATHANIEL_HOME at install/repair time, making mtime
