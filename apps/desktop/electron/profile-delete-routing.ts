// Profile-delete routing logic for the `athaniel:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_athaniel_home() and
...
/** Parse a `athaniel:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ATHANIEL_HOME.
 *
...
/**
 * Route the next `athaniel:api` request away from the primary/window backend
 ...
 * ensure_athaniel_home() recreates the directory the delete just removed.
 */
