 ...
 * the three user-facing uninstall modes to the `athaniel uninstall` CLI flags,
 ...
 *              `athaniel uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `athaniel uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `athaniel uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `athaniel` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m athaniel_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `athaniel_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'athaniel_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Athaniel.app/Contents/MacOS/Athaniel  → …/Athaniel.app
 *   Windows: …\Athaniel\Athaniel.exe                 → …\Athaniel  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Athaniel.app/Contents/MacOS/Athaniel → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Athaniel.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Athaniel.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Athaniel$/i.test(dir) || /[\\/]athaniel-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/athaniel
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import athaniel_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, athanielHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export ATHANIEL_HOME=${q(athanielHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import athaniel_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Athaniel install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ATHANIEL_HOME=${String(athanielHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
