...
// hardening, ATHANIEL_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('athaniel:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('athaniel:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('athaniel:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('athaniel:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<ATHANIEL_HOME>/desktop-plugins`,
  // resolved from the main-process ATHANIEL_HOME (see resolveAthanielHome) — NOT from
  // the connected backend. A remote backend reports its own `athaniel_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped athaniel_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(athanielHome, 'profiles', profile) : athanielHome
  ...
  ipcMain.handle('athaniel:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<ATHANIEL_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where ATHANIEL_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('athaniel:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<ATHANIEL_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('athaniel:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('athaniel:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('athaniel:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('athaniel:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('athaniel:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('athaniel:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
