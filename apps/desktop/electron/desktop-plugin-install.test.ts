...
describe('resolvePluginGitUrl', () => {
  it('maps owner/repo shorthand to github git url', () => {
    expect(resolvePluginGitUrl('embreythecreator/athaniel-example-plugins')).toEqual({
      gitUrl: 'https://github.com/embreythecreator/athaniel-example-plugins.git',
      subdir: null
    })
  })
...
describe('resolveSubdirWithin', () => {
  it('rejects path traversal', () => {
    const root = mkdtemp('athaniel-plugin-root-')
...
describe('findDesktopEntry', () => {
  it('finds root plugin.js', () => {
    const root = mkdtemp('athaniel-plugin-detect-')
...
describe('detectPluginComponents', () => {
  ...
  it('detects agent-only layout', async () => {
    const root = mkdtemp('athaniel-plugin-agent-')
    roots.push(root)
  ...
  it('detects dual layout', async () => {
    const root = mkdtemp('athaniel-plugin-dual-')
    roots.push(root)
    fs.mkdirSync(path.join(root, 'desktop'), { recursive: true })
