...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ATHANIEL_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Athaniel-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.athaniel/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'athaniel-desktop-ssh')
  }

  return path.join(os.homedir(), '.athaniel', 'desktop-ssh')
}
