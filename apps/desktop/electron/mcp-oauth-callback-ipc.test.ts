...
test('listen binds a loopback listener and wait resolves with the redirect params', async () => {
  const { id, redirectUri } = (await invoke('athaniel:mcp-oauth:listen')) as { id: string; redirectUri: string }

  assert.match(redirectUri, /^http:\/\/127\.0\.0\.1:\d+\/callback$/)

  const waitPromise = invoke('athaniel:mcp-oauth:wait', id, 5000) as Promise<{
    code: null | string
    error: null | string
    state: null | string
  }>
  ...
  assert.match(await res.text(), /return to Athaniel/)
...
test('non-callback noise (favicon) does not settle the listener', async () => {
  ...
  const waitPromise = invoke('athaniel:mcp-oauth:wait', id, 5000) as Promise<{ code: null | string }>
...
test('cancel tears the listener down and wait reports listener not found afterwards', async () => {
  ...
  assert.equal(await invoke('athaniel:mcp-oauth:cancel', id), true)

  await assert.rejects(fetch(`${redirectUri}?code=x&state=s`))

  const result = (await invoke('athaniel:mcp-oauth:wait', id, 100)) as { error: null | string }

  assert.equal(result.error, 'listener not found')
})

test('wait times out when no callback arrives', async () => {
  const { id } = (await invoke('athaniel:mcp-oauth:listen')) as { id: string }

  const result = (await invoke('athaniel:mcp-oauth:wait', id, 1000)) as { code: null | string; error: null | string }
