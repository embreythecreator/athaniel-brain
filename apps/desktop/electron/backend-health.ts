...
export interface AthanielReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForAthanielReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Athaniel gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForAthanielReady().
 *
...
export function makeNousCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  ...
  const err = new Error(
    `Nous Cloud agent ${hostname} is down ` +
      ...
      'You can also reach out on Discord at discord.gg/embreythecreator ' +
...
/**
 * True when the backend URL points at a Nous-managed Athaniel Cloud instance
 ...
 * `isReauthRequired` so startAthaniel latches; `needsOauthLogin` alone only
...
export async function waitForAthanielReady(baseUrl: string, options: AthanielReadyOptions): Promise<void> {
  ...
  throw new Error(`Athaniel backend did not become ready: ${detail}`)
}
