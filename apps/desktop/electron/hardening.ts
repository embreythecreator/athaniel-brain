...
// Relative, not `@athaniel/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Athaniel Desktop cannot save remote gateway tokens. ' +
        ...
        'or set ATHANIEL_DESKTOP_REMOTE_URL and ATHANIEL_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set ATHANIEL_DESKTOP_REMOTE_URL and ATHANIEL_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// ATHANIEL_DESKTOP_REMOTE_* env vars.
