...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'athaniel-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'athaniel-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Athaniel Test'], { cwd: dir })
