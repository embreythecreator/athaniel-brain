...
// `athaniel serve` announces ATHANIEL_BACKEND_READY; the legacy `athaniel dashboard`
// backend announces ATHANIEL_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^ATHANIEL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `athaniel_cli.main` → `web_server` → FastAPI/
 ...
 * ATHANIEL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.ATHANIEL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `ATHANIEL_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs(), describeOutputTail = () => '') {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Athaniel backend: exited before port announcement (${signal || code})${describeOutputTail()}`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Athaniel backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
