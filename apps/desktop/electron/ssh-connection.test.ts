...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env ATHANIEL_DASHBOARD_SESSION_TOKEN=abc123deadbeef ATHANIEL_DESKTOP=1 athaniel dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /ATHANIEL_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /ATHANIEL_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Athaniel-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Athaniel-Session-Token: hdr_888'), /X-Athaniel-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Athaniel-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('athaniel', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v athaniel', 15000)
  assert.equal(args[args.length - 1], 'command -v athaniel')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `athaniel-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `athaniel-ssh-test-${process.pid}-${Date.now()}`)
