export type ParentWatchdogEnv = {
  ATHANIEL_PARENT_PID: string
  ATHANIEL_PARENT_START_MARKER?: string
  ATHANIEL_PARENT_NONCE?: string
  /** Spawn tag consumed by athaniel_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { ATHANIEL_PARENT_PID: String(pid) }
  env.ATHANIEL_SPAWN = spawnTag(pid, startMarker)
  ...
  env.ATHANIEL_PARENT_START_MARKER = startMarker
  env.ATHANIEL_PARENT_NONCE = nonce

  return env
}

/** Build the `ATHANIEL_SPAWN` tag mirrored by athaniel_cli/process_identity.py.
