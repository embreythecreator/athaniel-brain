...
export const ZOOM_STORAGE_KEY = 'athaniel:desktop:zoomLevel'
 ...
 * but forgot to emit 'athaniel:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('athaniel:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `athaniel:zoom:changed`, which never fired —
