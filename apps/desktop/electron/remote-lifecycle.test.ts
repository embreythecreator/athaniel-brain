...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    athanielPath: '~/.local/bin/athaniel',
    athanielHome: '~/.athaniel',
...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.athaniel-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Athaniel', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('ATHANIEL_HOME')) {
          return '/home/alice/.athaniel\n'
        }

        if (command.includes('.athaniel-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.athaniel/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.athaniel-update-in-progress/)
})
...
test('listRemoteAthanielProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/ATHANIEL_HOME/, '/Users/zillajr/.athaniel\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteAthanielProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteAthanielProfiles rejects a hostile ATHANIEL_HOME', async () => {
  const ssh = fakeSsh([[/ATHANIEL_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteAthanielProfiles(ssh),
    (err: any) => {
...
test('locateAthaniel prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/athaniel/, 'OK']])
  assert.equal(await locateAthaniel(ssh, '/opt/athaniel'), '/opt/athaniel')
})

test('locateAthaniel throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong athaniel" bug.
  const ssh = fakeSsh([
    [/command -v athaniel/, '/home/u/.local/bin/athaniel\n'],
    [/\[ -x .*\.local\/bin\/athaniel/, 'OK']
  ])

  await assert.rejects(
    () => locateAthaniel(ssh, '/bad/path/athaniel'),
    (err: any) => {
      assert.equal(err.kind, 'athaniel-not-found')
      assert.match(err.message, /\/bad\/path\/athaniel/)
...
test('locateAthaniel falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v athaniel/, '/home/u/.local/bin/athaniel\n'],
    [/\[ -x .*\.local\/bin\/athaniel/, 'OK']
  ])

  assert.equal(await locateAthaniel(ssh, ''), '/home/u/.local/bin/athaniel')
})

test('locateAthaniel preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ATHANIEL_BIN" "$ATHANIEL_ENTRYPOINT" "$@",
  // where $ATHANIEL_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.athaniel/athaniel-brain/venv/bin/python\n']
  ])

  assert.equal(await locateAthaniel(ssh, ''), '/home/u/.local/bin/athaniel')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateAthaniel must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateAthaniel returns an explicit remoteAthanielPath unchanged', async () => {
  // The override half of #74411: an explicit remoteAthanielPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/athaniel changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/athaniel/, 'OK'],
    [/python3 -c/, '/home/u/.athaniel/athaniel-brain/venv/bin/python\n']
  ])

  assert.equal(await locateAthaniel(ssh, '~/.local/bin/athaniel'), '~/.local/bin/athaniel')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteAthanielPath must never be rewritten')
})

test('locateAthaniel falls back to ~/.local/bin/athaniel when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateAthaniel(ssh, ''), '~/.local/bin/athaniel')
})

test('locateAthaniel tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/athaniel/, 'OK']])
  assert.equal(await locateAthaniel(ssh, ''), '~/.athaniel/athaniel-brain/venv/bin/athaniel')
})

test('locateAthaniel throws a athaniel-not-found error with an install hint', async () => {
...
test('locateAthaniel uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v athaniel/, '/x/athaniel'],
    [/\[ -x/, 'OK']
  ])

  await locateAthaniel(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.athaniel/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.athaniel/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.athaniel/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/athaniel'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/athaniel serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/athaniel'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/athaniel', '/Users/cd9c/.athaniel', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /athaniel-brain.*venv.*bin.*athaniel/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'athaniel wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'athaniel')
    const launcher = path.join(temp, 'athaniel launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/athaniel-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/athaniel', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ATHANIEL_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/athaniel', 'work', {
    athanielHome: '~/.athaniel',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.athaniel-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"athaniel-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /athaniel-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'athaniel-update-mutex-'))
  const athanielPath = path.join(directory, 'athaniel')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *athaniel-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(athanielPath, '', {
      athanielHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { athanielPath: '/x/athaniel', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ATHANIEL_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ATHANIEL_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { athanielPath: '/x/athaniel', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nATHANIEL_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.athaniel\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Athaniel Brain v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile athanielPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ athanielPath: '/old/stale/athaniel', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteAthanielPath: '/new/athaniel', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes athanielHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ATHANIEL_HOME/, '/home/alice/.athaniel\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"athanielHome":"\/home\/alice\/\.athaniel"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ATHANIEL_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/athaniel'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.athaniel/athaniel-brain/venv/bin/athaniel'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/athaniel'))
  assert.doesNotThrow(() => validateRemotePath('~/.athaniel/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/athaniel'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('athaniel'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/athaniel'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/athaniel\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/athaniel\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/athaniel\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/athaniel', '/usr/`id`/athaniel', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.athaniel/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/athaniel')
  assert.ok(result.includes('/usr/local/bin/athaniel'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/athaniel')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ATHANIEL_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/athaniel', 'work', {
    tokenFilePath: `~/.athaniel/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.athaniel\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/athaniel', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Athaniel', () => {
  const cmd = buildSpawnCommand('/x/athaniel', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ATHANIEL_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { athanielPath: '/x/athaniel', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { athanielPath: '/x/athaniel', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.athaniel/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.athaniel/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/athaniel serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/athaniel serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/athaniel'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/athaniel'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/athaniel'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/athaniel'), false)
})
