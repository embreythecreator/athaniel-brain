 ...
 * console. It writes ATHANIEL_HOME/.athaniel-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Athaniel within 30 minutes.
...
export function handoffResultPath(athanielHome: string): string {
  return path.join(athanielHome, '.athaniel-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(athanielHome)
  let raw: string
