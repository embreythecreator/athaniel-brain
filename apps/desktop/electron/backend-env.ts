...
/**
 * Athaniel-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\athaniel\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$ATHANIEL_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_athaniel_node_dirs()` in athaniel_constants.py, which the Electron main
 * process cannot import.
 */
function athanielManagedNodePathEntries(
  athanielHome,
  ...
  const root = pathModule.join(athanielHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const athanielNodeDirs = athanielManagedNodePathEntries(athanielHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([athanielNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeAthanielHomeRoot(athanielHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!athanielHome) {
    return athanielHome
  }

  const resolved = pathModule.resolve(String(athanielHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). athaniel_bootstrap sets this inside the child too, but only
