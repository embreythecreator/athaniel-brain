// Launching the Athaniel TUI in the user's OWN terminal emulator.
//
...
// `athaniel --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Athaniel runtime is often a venv Python invoked
//     as `python -m athaniel_cli.main`, not a `athaniel` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Athaniel
...
export function terminalScriptEnv(
  ...
  if (athanielHome) {
    out.ATHANIEL_HOME = athanielHome
  }
