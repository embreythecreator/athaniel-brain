 ...
 *   - the browser sees only a minimal "return to Athaniel" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Athaniel.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('athaniel:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('athaniel:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('athaniel:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
