...
// registry, and the athaniel:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ATHANIEL_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ATHANIEL_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Athaniel' TUI into
    ...
    env.TERM_PROGRAM = 'Athaniel'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a athaniel/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ATHANIEL_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ATHANIEL_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `athaniel:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('athaniel:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('athaniel:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('athaniel:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('athaniel:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('athaniel:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
