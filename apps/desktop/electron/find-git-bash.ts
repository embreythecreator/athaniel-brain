 ...
 *   1. ATHANIEL_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\athaniel\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ATHANIEL_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ATHANIEL_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'athaniel', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'athaniel', 'git', 'usr', 'bin', 'bash.exe'))
  }
