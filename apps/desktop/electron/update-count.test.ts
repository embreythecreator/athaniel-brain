...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'athaniel-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Athaniel Test')
    git('config', 'user.email', 'athaniel@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/embreythecreator/athaniel-brain.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/embreythecreator/athaniel-brain/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/embreythecreator/athaniel-brain.git',
