...
import { buildDesktopBackendEnv, athanielManagedNodePathEntries, normalizeAthanielHomeRoot } from './backend-env'
import {
  isReauthRequiredError,
  makeNousCloudBackendDownError,
  makeUnsignedOauthError,
  waitForAthanielReady
} from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import {
  canImportAthanielCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
  shouldTrustAthanielOverride,
  verifyAthanielCli
} from './backend-probes'
...
import {
  ...
  resolveVenvAthanielCommand
} from './windows-athaniel-path'
import {
...
const USER_DATA_OVERRIDE = process.env.ATHANIEL_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ATHANIEL_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ATHANIEL_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ATHANIEL_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[athaniel] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[athaniel] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ATHANIEL_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[athaniel] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[athaniel] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via ATHANIEL_DESKTOP_PASSWORD_STORE, bridged by the
// `athaniel desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[athaniel] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[athaniel] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `athaniel desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Athaniel sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[athaniel] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[athaniel] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[athaniel] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[athaniel] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[athaniel] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('athaniel:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Athaniel burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[athaniel] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[athaniel] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[athaniel] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[athaniel] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ATHANIEL_HOME — the user-facing root for everything Athaniel-related. Mirrors
// scripts/install.ps1's $AthanielHome and scripts/install.sh's $ATHANIEL_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\athaniel (matches install.ps1)
//   macOS / Linux: ~/.athaniel (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.athaniel directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\athaniel yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ATHANIEL_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ATHANIEL_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.athaniel / %LOCALAPPDATA%\athaniel.
function resolveAthanielHome() {
  if (process.env.ATHANIEL_HOME) {
    return normalizeAthanielHomeRoot(process.env.ATHANIEL_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'athaniel-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ATHANIEL_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\athaniel and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ATHANIEL_HOME')

    if (fromRegistry) {
      return normalizeAthanielHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'athaniel')
    const legacy = path.join(app.getPath('home'), '.athaniel')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.athaniel setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.athaniel')
}

const ATHANIEL_HOME = resolveAthanielHome()

function pathWithAthanielManagedNode(...entries) {
  const managed = athanielManagedNodePathEntries(ATHANIEL_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ATHANIEL_ROOT — the canonical mutable Athaniel install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ATHANIEL_ROOT = path.join(ATHANIEL_HOME, 'athaniel-brain')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ATHANIEL_ROOT, 'venv')
...
// boots, going straight to `resolveAthanielBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ATHANIEL_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ATHANIEL_ROOT, '.athaniel-bootstrap-complete')
...
// active-profile.json records which Athaniel profile the desktop launches its
// local backend as. When set, startAthaniel() passes `athaniel --profile <name>
// dashboard …`, which deterministically pins ATHANIEL_HOME (see
// _apply_profile_override in athaniel_cli/main.py) and bypasses the sticky
// ~/.athaniel/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors athaniel_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// athanielDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ATHANIEL_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by athaniel_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ATHANIEL_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (athaniel_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ATHANIEL_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ATHANIEL_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ATHANIEL_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ATHANIEL_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ATHANIEL_DESKTOP_APP_NAME || 'Athaniel'
...
// 'athaniel:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.embreythecreator.athaniel) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.embreythecreator.athaniel')
}

// Seed the native About panel with the live Athaniel version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `athaniel update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveAthanielVersion(),
  copyright: 'Copyright © 2026 Athaniel'
})
...
// startAthaniel(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ATHANIEL_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ATHANIEL_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `athaniel:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.ATHANIEL_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startAthaniel() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning athaniel serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Athaniel backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  athanielLog.push(...lines)

  if (athanielLog.length > 300) {
    athanielLog.splice(0, athanielLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-athaniel-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('athaniel:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the athaniel:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('athaniel:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ATHANIEL_ROOT
  })
}
...
// The Tauri updater writes ATHANIEL_HOME/.athaniel-update-in-progress for the whole
...
// (`force_kill_other_athaniel`, taskkill /IM athaniel.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ATHANIEL_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Athaniel will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ATHANIEL_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Athaniel update did not finish',
        `${result.message}\n\nDetails: ${path.join(ATHANIEL_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `athaniel` — must not shadow `athaniel.cmd`/`athaniel.exe`.
...
function unwrapWindowsVenvAthanielCommand(command, backendArgs) {
  return resolveVenvAthanielCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `athaniel serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `athaniel` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'athaniel_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `athaniel --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ATHANIEL_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveAthanielBackend step 4); execFileSync of a .cmd without
...
function isAthanielSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'athaniel_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ATHANIEL_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Athaniel deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the athaniel-import
...
// findGitBash — locate bash.exe on Windows. Resolves ATHANIEL_GIT_BASH_PATH
...
// normal ATHANIEL_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\athaniel\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'athaniel', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'athaniel', 'git', 'bin', 'git.exe'))
  }
...
function recentAthanielLog() {
  return athanielLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's athaniel root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ATHANIEL_ROOT.
// ATHANIEL_DESKTOP_ATHANIEL_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ATHANIEL_DESKTOP_ATHANIEL_ROOT && path.resolve(process.env.ATHANIEL_DESKTOP_ATHANIEL_ROOT),
    !IS_PACKAGED && isAthanielSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAthanielSourceRoot(ACTIVE_ATHANIEL_ROOT) ? ACTIVE_ATHANIEL_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ATHANIEL_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('athaniel:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'athaniel-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `athaniel update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ATHANIEL_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `athaniel update` can write
// fresh entry points. On Windows this is the file the running backend
// `athaniel.exe` holds open; on POSIX it's never mandatory-locked.
function venvAthanielShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'athaniel.exe')
    : path.join(updateRoot, 'venv', 'bin', 'athaniel')
}
...
// only signals the direct child, so on Windows a backend `athaniel.exe` that
// spawned its own grandchildren (a `athaniel` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Athaniel backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Athaniel backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.athanielBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Athaniel backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.athanielBackendIdentity
...
// `athanielProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `athaniel.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a athaniel
// REPL, a pty terminal, the gateway) keep `athaniel.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const athanielProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\athaniel.exe, but the
  // backend is `python.exe -m athaniel_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (athanielProcess && Number.isInteger(athanielProcess.pid)) {
    initialPids.push(athanielProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(athanielProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvAthanielShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentAthanielProcess = backendConnectionState.getProcess()

        if (currentAthanielProcess && Number.isInteger(currentAthanielProcess.pid)) {
          stragglers.push(currentAthanielProcess.pid)
        }
...
// `athaniel update`). Instead we spawn the staged Athaniel-Setup binary with
// --update and quit, so it can run `athaniel update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `athaniel update` → relaunch),
      ...
      // in: the ATHANIEL_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `athaniel update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `athaniel desktop`, never the Tauri installer that self-copies
      // athaniel-setup.exe into ATHANIEL_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `athaniel` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `athaniel update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `athaniel update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, athanielRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(ATHANIEL_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Athaniel — this window will close and the updater will open. Don’t reopen Athaniel yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(ATHANIEL_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // athaniel.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running athaniel, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Athaniel install open ' +
        '(a second Athaniel window or a terminal running athaniel?). Close it and retry.'
    ...
    // Athaniel processes running from this venv.  The updater normally refuses
    ...
    // `athaniel update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `athaniel update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          ATHANIEL_HOME,
          ATHANIEL_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithAthanielManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `athaniel update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(ATHANIEL_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Athaniel update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(ATHANIEL_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Athaniel will keep running — try again, or run \`athaniel update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAthaniel = path.join(venvBin, IS_WINDOWS ? 'athaniel.exe' : 'athaniel')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Athaniel launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the athaniel
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.athaniel-bootstrap-complete')),
      hasVenvAthaniel: fileExists(venvAthaniel),
...
// Python-level pre-update snapshot inside `athaniel update`.
function preflightStateDb(athanielHome, rememberLog) {
  const stateDbPath = path.join(athanielHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(athanielHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(athanielHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(athanielHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `athaniel update`, swaps/relaunches the app bundle, and writes
// .athaniel-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'athaniel update', percent: null })

    return { ok: true, manual: true, command: 'athaniel update', athanielRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      ATHANIEL_HOME,
      ATHANIEL_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithAthanielManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Athaniel — this window will close. Don’t reopen Athaniel yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_ATHANIEL_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isAthanielSourceRoot(ACTIVE_ATHANIEL_ROOT) &&
    fileExists(venvPython) &&
    canImportAthanielCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ATHANIEL_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `athaniel
...
function resolveWebDist() {
  const override = process.env.ATHANIEL_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ATHANIEL_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: athaniel desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: athaniel desktop --force-build`
  )
...
function resolveAthanielCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Athaniel.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ATHANIEL_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAthanielCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'athaniel_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ATHANIEL_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Athaniel at ${ACTIVE_ATHANIEL_ROOT}`,
    command,
    args: ['-m', 'athaniel_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      athanielHome: ATHANIEL_HOME,
      pythonPathEntries: [ACTIVE_ATHANIEL_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ATHANIEL_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveAthanielBackend(backendArgs) {
  // 1. Explicit override -- ATHANIEL_DESKTOP_ATHANIEL_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ATHANIEL_DESKTOP_ATHANIEL_ROOT && path.resolve(process.env.ATHANIEL_DESKTOP_ATHANIEL_ROOT)

  if (overrideRoot && isAthanielSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Athaniel source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `athaniel` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAthanielSourceRoot.)
  if (!IS_PACKAGED && isAthanielSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Athaniel source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ATHANIEL_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\athaniel\\athaniel-brain (Windows) or ~/.athaniel/athaniel-brain.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Athaniel runtime at ${ACTIVE_ATHANIEL_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `athaniel` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ATHANIEL_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ATHANIEL_DESKTOP_IGNORE_EXISTING !== '1') {
    let athanielCommand = null
    const athanielOverride = process.env.ATHANIEL_DESKTOP_ATHANIEL

    if (athanielOverride) {
      const resolvedOverride = findOnPath(athanielOverride)

      if (resolvedOverride) {
        athanielCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(athanielOverride, { isWsl: IS_WSL })) {
        athanielCommand = athanielOverride
      } else {
        rememberLog(`Ignoring Windows Athaniel override under WSL: ${athanielOverride}`)
      }
    } else {
      athanielCommand = findOnPath('athaniel')
    }

    if (athanielCommand) {
      if (looksLikeDesktopAppBinary(athanielCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Athaniel CLI: ${athanielCommand}`)
        athanielCommand = null
      }
    }

    if (athanielCommand) {
      const unwrapped = unwrapWindowsVenvAthanielCommand(athanielCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `athaniel` shim
      ...
      const shellForProbe = isCommandScript(athanielCommand)

      // ATHANIEL_DESKTOP_ATHANIEL is an explicit deployment override (used by
      ...
      if (shouldTrustAthanielOverride(athanielOverride) || verifyAthanielCli(athanielCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvAthanielCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Athaniel CLI at ${athanielCommand}`,
          command: athanielCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Athaniel CLI at ${athanielCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed athaniel_cli module via system Python.
  ...
  if (python) {
    ...
    // having athaniel_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\athaniel\athaniel-brain\venv.
    if (canImportAthanielCli(python)) {
      return {
        kind: 'python',
        label: `installed athaniel_cli module via ${python}`,
        command: python,
        args: ['-m', 'athaniel_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: athaniel_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAthanielBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Athaniel Brain not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_ATHANIEL_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAthanielBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Athaniel install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Athaniel recovery was handed off to Athaniel Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      athanielHome: ATHANIEL_HOME,
      logRoot: path.join(ATHANIEL_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Athaniel install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Athaniel bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ATHANIEL_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startAthaniel() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // athaniel:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveAthanielBackend(backend.args))
  }
  ...
  if (!isAthanielSourceRoot(ACTIVE_ATHANIEL_ROOT)) {
    throw new Error(
      `Athaniel install at ${ACTIVE_ATHANIEL_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Athaniel' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\athaniel\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `athaniel` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Athaniel on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Athaniel.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isAthanielSourceRoot()
    // plus an importable athaniel_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Athaniel venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Athaniel at ${ACTIVE_ATHANIEL_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Athaniel runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----athaniel-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Athaniel backend URL protocol: ${parsed.protocol}`))
        ...
        const req = client.request(
          parsed,
          {
            ...
            headers: {
              ...
              'X-Athaniel-Session-Token': token,
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Athaniel backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Athaniel backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Athaniel-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Athaniel-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('athaniel:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAthanielCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('athaniel:preview-file-changed', payload)
}
...
// waitForAthanielReady how to read a 401 — rejected session vs gated route.
...
async function waitForAthaniel(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForAthanielReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('athaniel:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('athaniel:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('athaniel:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('athaniel:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Athaniel...".
function sendPowerResume() {
  ...
  webContents.send('athaniel:power-resume')
}
...
// 'athaniel:power-battery' push below.
ipcMain.handle('athaniel:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('athaniel:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('athaniel:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('athaniel:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('athaniel:context-menu-spellcheck', {
...
//
// Hosted Athaniel gateways gate the dashboard behind an OAuth provider (e.g.
// Athaniel) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``athaniel_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / athaniel #37247).
//     Both are set as HttpOnly cookies (``athaniel_session_at`` ~15 min,
//     ``athaniel_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startAthaniel → the renderer's
// single-shot boot() with no retry) surfaced as the "Athaniel couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Athaniel
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Athaniel Cloud agent…' : 'Sign in to Athaniel gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Athaniel backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in athaniel_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Athaniel Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:athaniel-remote-oauth`:
...
// convention (athaniel_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Athaniel surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.ATHANIEL_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Athaniel gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for athaniel_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Athaniel Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Athaniel Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Athaniel Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Athaniel Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Athaniel Cloud. Open Settings → Gateway, choose Athaniel Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Athaniel Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Athaniel Cloud session has expired. Sign in to Athaniel Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Athaniel Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every athaniel:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteAthanielPath`/`remoteProfile` —
...
// a valid stored value (pins the root ATHANIEL_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ATHANIEL_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ATHANIEL_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Athaniel Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteAthanielPath: (ssh || savedSsh)?.remoteAthanielPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ATHANIEL_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Athaniel Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Athaniel Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteAthanielPath: input.sshRemoteAthanielPath ?? existingBlock.remoteAthanielPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (athaniel #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Athaniel gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Athaniel gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.ATHANIEL_DESKTOP_REMOTE_TOKEN,
      url: process.env.ATHANIEL_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      athanielPath: result.athanielPath,
      athanielHome: result.athanielHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { athanielPath: result.athanielPath, athanielHome: result.athanielHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteAthanielPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteAthanielPath: sshConfig.remoteAthanielPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForAthaniel: (baseUrl, token) => waitForAthaniel(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        athanielVersion: result.athanielVersion || '',
        ...
        // per-profile pools also use bare keys. Only startAthaniel' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.athanielVersion || 'athaniel (version unknown)'} at ${result.athanielPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteAthanielVersion: result.athanielVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (ATHANIEL_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.ATHANIEL_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startAthaniel() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Athaniel
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Athaniel" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteAthanielPath: input.sshRemoteAthanielPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteAthanielPath || '')
          let athanielPath
          let athanielVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            athanielPath = runtime.athanielPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.athanielPath])
            athanielVersion = inspection.version
            supported = inspection.supported
          } else {
            athanielPath = await remoteLifecycle.locateAthaniel(ssh, sshConfig.remoteAthanielPath || '')
            athanielVersion = await remoteLifecycle.probeAthanielVersion(ssh, athanielPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, athanielPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Athaniel on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteAthanielPath: athanielPath,
            remoteAthanielVersion: athanielVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAthaniel())
  ...
  // connect to Athaniel gateway". Mirror the renderer's connect here so the test
...
function resetAthanielConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const athanielProcess = backendConnectionState.invalidate()
  stopBackendChild(athanielProcess)
...
// startAthaniel() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetAthanielConnection() invalidates it.
  const athanielProcess = backendConnectionState.getProcess()
  const dying = athanielProcess && !athanielProcess.killed ? athanielProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('athaniel:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('athaniel:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startAthaniel()
  ...
  // spawning, so two children never share one profile's ATHANIEL_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Athaniel backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteAthanielPath: source.remoteAthanielPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Athaniel backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. athaniel:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: athanielLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForAthaniel(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.athanielPath || !state?.athanielHome) {
  ...
  return {
    ...
    athanielPath: state.athanielPath,
    athanielHome: state.athanielHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteAthanielPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.athanielPath ? platform : await probeWindowsRemote(ssh, config.remoteAthanielPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          athanielPath: runtime.athanielPath,
          athanielHome: runtime.athanielHome,
          pythonPath: runtime.python
        }
      }
    }

    const athanielPath = await remoteLifecycle.locateAthaniel(ssh, config.remoteAthanielPath || '')
    const athanielHome = await remoteLifecycle.probeRemoteAthanielHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, athanielPath, athanielHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { athanielPath: state.athanielPath, athanielHome: state.athanielHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForAthaniel(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startAthaniel() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForAthaniel(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited ATHANIEL_HOME env (see _apply_profile_override
  // step 3 in athaniel_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveAthanielBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const athanielCwd = resolveAthanielCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(ATHANIEL_HOME, 'profiles', key))
  )

  rememberLog(`Starting Athaniel backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: athanielCwd,
        ATHANIEL_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        ATHANIEL_WEB_DIST: webDist,
        ...(readyFile ? { ATHANIEL_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Athaniel backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Athaniel backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Athaniel backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForAthaniel(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Athaniel backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Athaniel backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_athaniel_home() recreates the deleted profile directory.
//
...
async function startAthaniel() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Athaniel Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startAthaniel() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Athaniel backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Athaniel backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Athaniel backend at ${remote.baseUrl}`, 24)
      await waitForAthaniel(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Athaniel backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, athanielLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.athaniel/active_profile file) and
    // resolves ATHANIEL_HOME the same way `athaniel -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Athaniel runtime', 28)

        return resolveAthanielBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Athaniel backend via ${backend.label}`, 84)
    rememberLog(`Starting Athaniel backend via ${backend.label}`)
    ...
    const athanielProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ATHANIEL_HOME for the child so Python's get_athaniel_home()
          // resolves to the SAME location our resolveAthanielHome() picked. Without
          // this pin, Python falls back to ~/.athaniel on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\athaniel, which differs from C:\Users\<u>\.athaniel.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ATHANIEL_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(athanielProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, athanielProcess)

    if (!processOwner) {
      stopBackendChild(athanielProcess)
      await waitForBackendExit(athanielProcess)
      releaseBackendChild(athanielProcess)
      throw new Error('Athaniel backend start was superseded by a newer connection attempt.')
    }

    athanielProcess.stdout.on('data', rememberLog)
    athanielProcess.stderr.on('data', rememberLog)
    ...
    athanielProcess.once('error', error => {
      releaseBackendChild(athanielProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Athaniel backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Athaniel backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Athaniel backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Athaniel backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    athanielProcess.once('exit', (code, signal) => {
      releaseBackendChild(athanielProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Athaniel backend exit (${signal || code})`)
      ...
      rememberLog(`Athaniel backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Athaniel backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Athaniel backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAthanielLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Athaniel backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(athanielProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Athaniel backend to become ready', 90)
    await Promise.race([waitForAthaniel(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => athanielProcess.exitCode === null && !athanielProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Athaniel backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Athaniel backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('athaniel:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Athaniel is
...
// pushes pet state over IPC (athaniel:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via athaniel:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Athaniel app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Athaniel icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see athaniel:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Athaniel in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('athaniel:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Athaniel can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('athaniel:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('athaniel:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[athaniel] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('athaniel:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `athaniel:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, athaniel:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('athaniel:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('athaniel:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('athaniel:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'athaniel desktop --force-build',
  ...
  startAthaniel().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('athaniel:connection', async (_event, profile) => {
...
ipcMain.handle('athaniel:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'athaniel:connection', keyed by the composite
...
ipcMain.on('athaniel:connection:active-route', (event, route) => {
...
// "Starting Athaniel…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('athaniel:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetAthanielConnection({ soft: true }),
...
ipcMain.handle('athaniel:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('athaniel:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('athaniel:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('athaniel:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('athaniel:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('athaniel:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveAthanielBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Athaniel is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `athaniel-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, ATHANIEL_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('athaniel:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('athaniel:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('athaniel:zoom:get', event => {
...
ipcMain.on('athaniel:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('athaniel:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startAthaniel() call restarts the
...
ipcMain.handle('athaniel:bootstrap:repair', async () => {
  // Forceful repair: force the next startAthaniel() through the full installer
  ...
  // "bypass active runtime" path inside startAthaniel() and fall through
  ...
  resetAthanielConnection()

  return { ok: true }
})
ipcMain.handle('athaniel:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('athaniel:bootstrap:cancel', async () => {
...
ipcMain.handle('athaniel:boot-progress:get', async () => bootProgressState)
ipcMain.handle('athaniel:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('athaniel:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('athaniel:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('athaniel:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('athaniel:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('athaniel:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('athaniel:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('athaniel:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('athaniel:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('athaniel:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('athaniel:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('athaniel:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('athaniel:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('athaniel:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('athaniel:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteAthanielPath: entry.remoteAthanielPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startAthaniel()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteAthanielPath: connection.remoteAthanielPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteAthanielProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root ATHANIEL_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('athaniel:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// athaniel:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('athaniel:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('athaniel:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `athaniel update` to every eligible registered connection at once.
...
ipcMain.handle('athaniel:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/athaniel/update', {}, { timeoutMs: 15_000 })
...
// analogue of the athaniel:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('athaniel:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('athaniel:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startAthaniel() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('athaniel:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Athaniel Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('athaniel:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('athaniel:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('athaniel:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('athaniel:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of athaniel:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('athaniel:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('athaniel:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ATHANIEL_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('athaniel:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('athaniel:requestMicrophoneAccess', async () => {
...
ipcMain.handle('athaniel:window:readBelow', async event => {
...
async function handleAthanielApiRequest(request) {
  ...
  // backend calls ensure_athaniel_home() which recreates the profile directory,
...
ipcMain.handle('athaniel:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its ATHANIEL_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleAthanielApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleAthanielApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('athaniel:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('athaniel:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('athaniel:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as athaniel://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('athaniel:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('athaniel:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('athaniel:data-url-read-max:get', () => ({
...
ipcMain.handle('athaniel:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('athaniel:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('athaniel:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('athaniel:readFileText', async (_event, filePath) => {
...
// `athaniel:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('athaniel:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('athaniel:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('athaniel:writeClipboard', (_event, text) => {
...
ipcMain.handle('athaniel:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('athaniel:readClipboard', () => clipboard.readText())

ipcMain.handle('athaniel:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('athaniel:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('athaniel:context-menu:edit', (event, command) => {
...
ipcMain.handle('athaniel:context-menu:copy-image', event => {
...
ipcMain.handle('athaniel:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('athaniel:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('athaniel:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('athaniel:saveClipboardImage', async () => {
...
ipcMain.handle('athaniel:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('athaniel:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('athaniel:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('athaniel:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('athaniel:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('athaniel:titlebar-theme', (_event, payload) => {
...
ipcMain.on('athaniel:native-theme', (_event, mode) => {
...
ipcMain.on('athaniel:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('athaniel:translucency', (_event, payload) => {
...
ipcMain.on('athaniel:keep-awake', (_event, on) => {
...
ipcMain.handle('athaniel:quick-entry:settings:get', async () => {
...
ipcMain.handle('athaniel:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('athaniel:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('athaniel:quick-entry:submit', {
...
ipcMain.on('athaniel:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('athaniel:quick-entry:state', payload)
  }
})

ipcMain.on('athaniel:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('athaniel:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('athaniel:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `athaniel:window:openSession`).
...
ipcMain.handle('athaniel:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('athaniel:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('athaniel:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('athaniel:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAthanielCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('athaniel:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAthanielCwd()
}))

ipcMain.handle('athaniel:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('athaniel:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('athaniel:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('athaniel:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('athaniel:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('athaniel:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('athaniel:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: athanielLog.slice(-200) }))
...
ipcMain.on('athaniel:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (athaniel:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('athaniel:updates:check', async () =>
...
ipcMain.handle('athaniel:updates:apply', async (_event, payload) =>
...
ipcMain.handle('athaniel:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('athaniel:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Athaniel version (the one `release.py` bumps in
// athaniel_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Athaniel version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAthanielVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'athaniel_cli', '__init__.py')
...
// Renderer-bundle skew: `athaniel update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Athaniel version and push it into the native About panel
// just before showing it, so an in-place `athaniel update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveAthanielVersion()} — app build out of date, update the desktop app`
        : resolveAthanielVersion(),
      copyright: 'Copyright © 2026 Athaniel'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('athaniel:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveAthanielVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    athanielRoot: resolveUpdateRoot(),
...
// the actual removal via `athaniel uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (athaniel_cli/uninstall.py + athaniel_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ATHANIEL_ROOT
  ...
  const fallback = () => ({
    athaniel_home: ATHANIEL_HOME,
    agent_installed: isAthanielSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'athaniel_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ATHANIEL_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Athaniel agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import athaniel_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_ATHANIEL_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_ATHANIEL_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_ATHANIEL_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `athaniel-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `athaniel-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('athaniel:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('athaniel:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('athaniel:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('athaniel:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// athaniel:// deep links (e.g. athaniel://blueprint/morning-brief?time=08:00,
// athaniel://mcp/install?name=NAME&config=B64 — the vendor "Add to Athaniel"
// button, or athaniel://plugin/install?repo=owner/repo). Dev
// (`ATHANIEL_DESKTOP_DEV_SERVER`) registers athaniel-dev:// instead — bare
// Electron or a stale OS handler often owns athaniel:// on dev machines.
...
// ---------------------------------------------------------------------------
const ATHANIEL_PROTOCOL = DEV_SERVER ? 'athaniel-dev' : 'athaniel'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['athaniel-dev', 'athaniel'] : ['athaniel']
...
function handleDeepLink(url) {
  ...
  // athaniel://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('athaniel:deep-link', payload)
...
ipcMain.handle('athaniel:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ATHANIEL_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ATHANIEL_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(ATHANIEL_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${ATHANIEL_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `athaniel://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startAthaniel), whose
...
app.whenReady().then(() => {
  ...
  // startAthaniel() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching athaniel:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Athaniel never keeps another app's chord hostage.
