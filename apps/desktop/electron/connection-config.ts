 ...
 *     `X-Athaniel-Session-Token` header; WS uses `?token=`.
...
// athaniel_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - athaniel_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - athaniel_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / athaniel #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-athaniel_session_at', '__Secure-athaniel_session_at', 'athaniel_session_at']
const RT_COOKIE_VARIANTS = ['__Host-athaniel_session_rt', '__Secure-athaniel_session_rt', 'athaniel_session_rt']

// The Nous portal (NAS) does NOT use Athaniel gateway session cookies — it is a
...
// Keep this aligned with athaniel_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['athaniel', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Athaniel profile the remote SSH dashboard should actually run as.
...
// Athaniel Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteAthanielPath = String(entry.remoteAthanielPath || '').trim()

  if (remoteAthanielPath) {
    out.remoteAthanielPath = remoteAthanielPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Athaniel installation. Preserve an explicit mapping
  // when it is a valid Athaniel profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_athaniel_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ATHANIEL_HOME` scopes it.
 *
 ...
 * `athaniel:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://athaniel.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `athaniel:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a athaniel session ACCESS-token cookie
 ...
 * the Athaniel gateway session cookies, so cloud sign-in / discovery liveness
