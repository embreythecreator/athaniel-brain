/**
 * windows-athaniel-path.ts
 *
 * Pure, dependency-injected pieces of Windows `athaniel` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvAthanielCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `athaniel` shim shadowed the real
 *      athaniel.cmd/athaniel.exe; the shim then failed the --version probe and
 ...
 *      and Athaniel launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvAthanielCommand() — unwrapWindowsVenvAthanielCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `athaniel`) must
 * not shadow `athaniel.cmd`/`athaniel.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Athaniel launcher that drives `athaniel update`.
...
export interface ResolveVenvAthanielCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportAthanielCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `athaniel`/`athaniel.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/athaniel(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m athaniel_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportAthanielCli(). A
 * venv whose update died mid-`pip install` still has python.exe + athaniel.exe
 ...
 *
 * Returns null when `command` is not a venv athaniel shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvAthanielCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvAthanielCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^athaniel(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Athaniel at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Athaniel Python at ${python}`,
    command: python,
