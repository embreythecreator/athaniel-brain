  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (const entry of entries) {
        ...
        // if it reaches reapOrphans (see main.ts startAthaniel + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:athaniel(?:\.exe)?|athaniel_cli\.main|athaniel_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
