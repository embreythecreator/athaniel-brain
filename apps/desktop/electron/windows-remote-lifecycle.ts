...
const READY_RE = /^ATHANIEL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitAthanielPath = '') {
  const explicit = psLiteral(explicitAthanielPath)
  ...
  const script = [
    ...
    '$athanielHome=$env:ATHANIEL_HOME',
    'if(-not $athanielHome){$athanielHome=Join-Path $env:LOCALAPPDATA "athaniel"}',
    'Assert-NoReparse $athanielHome $true',
    '$candidate=[IO.Path]::Combine($athanielHome, "athaniel-brain\\venv\\Scripts\\athaniel.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "athaniel-brain\\.venv\\Scripts\\athaniel.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $athanielHome "athaniel-brain\\venv\\Scripts\\athaniel.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "athaniel-brain\\.venv\\Scripts\\athaniel.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command athaniel.exe -ErrorAction SilentlyContinue',
    ...
    '$athaniel=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$athaniel=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $athaniel){throw "Athaniel is not installed on the remote Windows host."}',
    'Assert-NoReparse $athaniel $false',
    'if($explicit -and $athaniel -ne $explicit){throw "The configured Athaniel path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($athaniel), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;athanielHome=$athanielHome;athanielPath=$athaniel;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(athanielHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class AthanielMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(athanielHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".athaniel-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[AthanielMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, athanielHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(athanielHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitAthanielPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitAthanielPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'athaniel_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'athaniel_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'athaniel_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.athanielHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".athaniel-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};athanielPath=${psLiteral(reservation.athanielPath)};athanielHome=${psLiteral(reservation.athanielHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.athanielPath === 'string' &&
    typeof lock.athanielHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.athanielPath === runtime.athanielPath &&
    lock.athanielHome === runtime.athanielHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteAthanielPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.athanielHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.athanielPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Athaniel on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.athanielPath = inspection.path
  const athanielVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located athaniel at ${runtime.athanielPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, athanielPath: runtime.athanielPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAthaniel(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Athaniel SSH"', 'powershell.exe -NoLogo')
