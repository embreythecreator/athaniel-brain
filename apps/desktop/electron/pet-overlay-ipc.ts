...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('athaniel:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('athaniel:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('athaniel:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('athaniel:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('athaniel:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('athaniel:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('athaniel:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('athaniel:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('athaniel:pet-overlay:control', payload)
  })
}
