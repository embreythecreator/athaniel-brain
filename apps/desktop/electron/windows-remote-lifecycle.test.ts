...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    athanielHome: 'C:\\Users\\andre\\.athaniel',
    python: 'C:\\Users\\andre\\.athaniel\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.athaniel-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      athanielPath: 'C:\\Athaniel\\athaniel.exe',
      athanielHome: 'C:\\Users\\andre\\.athaniel',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command athaniel.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          athanielHome: 'C:\\Users\\alice\\.athaniel',
          athanielPath: 'C:\\Athaniel\\athaniel.exe',
          python: 'C:\\Athaniel\\python.exe'
        })
      }

      if (script.includes('.athaniel-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('athaniel_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.athaniel\\profiles\\research')
  assert.match(script, /\.athaniel-update-in-progress/)
...
test('Windows probe validates Athaniel and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $athanielHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Athaniel is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Athaniel is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Athaniel's\\python.exe" }, 'inspect', [
    'C:\\x y\\athaniel.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'athaniel_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Athaniel''s/)
  assert.match(script, /C:\\x y\\athaniel\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { athanielPath: lock.athanielPath, athanielHome: lock.athanielHome }
