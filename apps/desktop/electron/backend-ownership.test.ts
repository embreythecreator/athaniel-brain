...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'athaniel serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/athaniel serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m athaniel_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/athaniel --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Athaniel Runtime\\athaniel.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('athaniel chat --query serve'), false)
