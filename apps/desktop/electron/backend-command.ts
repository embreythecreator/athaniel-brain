// Backend subcommand routing for the desktop-managed Athaniel process.
//
// The desktop app launches its own headless backend via `athaniel serve` — it
...
// app hasn't updated yet, or an older `athaniel` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Athaniel profile to pin via `--profile`.
 */
 ...
 * `-m athaniel_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `athaniel_cli/subcommands/dashboard.py` source registers
