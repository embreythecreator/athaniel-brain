...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'ATHANIEL_DESKTOP_REMOTE_URL is set but ATHANIEL_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Athaniel backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `athaniel serve` the desktop never
