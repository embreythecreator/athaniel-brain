 ...
 * packaged binary) with a real `athaniel serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → athaniel serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Athaniel title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Athaniel')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { athanielDesktop?: Record<string, unknown> }).athanielDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → athaniel serve is spawned → WS connects → config
