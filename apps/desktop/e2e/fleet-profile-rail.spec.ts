 ...
 * second gateway, "Homelab", is a REAL second `athaniel serve` this spec spawns
 * with its own ATHANIEL_HOME, profiles and session token, registered in the v2
...
function findAthanielBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'athaniel')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['athaniel'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('athaniel binary not found: create the repo venv (uv sync) or put athaniel on PATH')
}
...
/**
 * Spawn a second, fully real `athaniel serve` as the remote gateway. Its
 * session token is pinned through ATHANIEL_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        ATHANIEL_HOME: home,
        ATHANIEL_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote athaniel serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Athaniel-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote athaniel serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.athanielHome, mock.url)
    ...
    seedProfiles(sandbox.athanielHome, ['research'])
