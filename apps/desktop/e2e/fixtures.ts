/**
 * Shared E2E fixtures for the Athaniel desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → athaniel serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `athaniel desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `athaniel-e2e-${prefix}-${Math.random()}`))
  const athanielHome = path.join(root, 'athaniel-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(athanielHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(athanielHome, 'config.yaml')
 ...
 */
export function writeEnvFile(athanielHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(athanielHome, '.env')
 ...
 */
function writeEmptyConfig(athanielHome: string): void {
  const configPath = path.join(athanielHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ATHANIEL_HOME → sandbox athaniel-home (isolated config/sessions)
 *  - ATHANIEL_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ATHANIEL_DESKTOP_IGNORE_EXISTING=1 → don't pick up `athaniel` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ATHANIEL_DESKTOP_ATHANIEL_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ATHANIEL_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ATHANIEL_HOME: sandbox.athanielHome,
    ATHANIEL_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ATHANIEL_DESKTOP_IGNORE_EXISTING: '1',
    ATHANIEL_DESKTOP_ATHANIEL_ROOT: REPO_ROOT,
    ATHANIEL_DESKTOP_APP_NAME: `AthanielE2E-${Date.now()}`,
    ...
    ATHANIEL_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ATHANIEL_HOME + userData
 * @param env      - the process environment (already has ATHANIEL_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.athanielHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.athanielHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.athanielHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ATHANIEL_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`athaniel serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.athanielHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ATHANIEL_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Athaniel backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Athaniel.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Athaniel.app', 'Contents', 'MacOS', 'Athaniel')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'athaniel')
}
 ...
 * progress without spawning a real Athaniel backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ATHANIEL_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ATHANIEL_DESKTOP_BOOT_FAKE: '1',
    ATHANIEL_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + athaniel-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ATHANIEL_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ATHANIEL_DESKTOP_ATHANIEL
  delete (env as Record<string, string | undefined>).ATHANIEL_DESKTOP_ATHANIEL_ROOT
