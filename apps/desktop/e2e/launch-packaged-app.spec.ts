...
/**
 * E2E smoke tests for the packaged Athaniel desktop app.
 *
 ...
 * (credential stripping, isolated ATHANIEL_HOME + userData, unique app name).
 *
...
test('window opens with the Athaniel title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      athanielDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).athanielDesktop?.hud?.open({ sessionId: null })
  )
