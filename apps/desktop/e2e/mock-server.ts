 ...
 *
 * Implements just enough of the /v1/* surface for `athaniel serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Athaniel should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `athaniel-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
  return {
    path,
    release: () => {
