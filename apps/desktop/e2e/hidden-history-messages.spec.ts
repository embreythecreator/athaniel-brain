...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.athanielHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.athanielHome)
  const builder = await RealSessionBuilder.start(sandbox.athanielHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.athanielHome, mock.url)
  fs.appendFileSync(path.join(sandbox.athanielHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.athanielHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
