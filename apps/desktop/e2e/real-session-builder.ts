 ...
 * AIAgent loop, using the E2E mock provider configured in `athanielHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(athanielHome: string) {
  ...
  static async start(athanielHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(athanielHome)
