 ...
 * dir (macOS `~/Library/Application Support/Athaniel/connection.json`, Windows
 * `AppData\Roaming\Athaniel\connection.json`). Anything that can read the file
 ...
 *      or in ATHANIEL_HOME (logs included).
 ...
 * `X-Athaniel-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add Athaniel desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'athaniel-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'AthanielE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Athaniel-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Athaniel gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-athaniel-session-token']
 ...
 * backend, so we skip spawning `athaniel serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startAthaniel() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    ATHANIEL_DESKTOP_APP_NAME: STABLE_APP_NAME,
    ATHANIEL_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { athanielDesktop?: Record<string, unknown> }).athanielDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Athaniel should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { athanielDesktop: any }).athanielDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('with keychain encryption opted IN, a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // ATHANIEL_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.athanielHome, needles),
      'the gateway session token leaked into a ATHANIEL_HOME file (logs included)',
    ).toEqual([])
