...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Athaniel E2E'], { cwd: repo })
...
function configureRepoCwd(athanielHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(athanielHome, mockUrl)
  fs.appendFileSync(path.join(athanielHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(athanielHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.athanielHome, mock.url, repo)
