...
async function seedBot(athanielHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(athanielHome, 'profiles', name)
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.athanielHome, mock.url, 'alpha')
  await seedBot(sandbox.athanielHome, mock.url, 'beta')
