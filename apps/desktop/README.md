...
<p align="center">
  <a href="https://github.com/embreythecreator/athaniel-brain/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Athaniel Brain](../../README.md) — the self-improving AI agent from [Athaniel](https://embreythecreator.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Athaniel surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Athaniel and hear it back.</td></tr>
...
### Install with Athaniel (recommended)

Already have the Athaniel CLI? Just run:

```bash
athaniel desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Athaniel gateway or install Athaniel locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Athaniel Desktop website.](https://athaniel-brain.embreythecreator.com/).
...
```bash
# throwaway ATHANIEL_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Athaniel Brain runtime into `ATHANIEL_HOME`
(`~/.athaniel`, or `%LOCALAPPDATA%\athaniel` on Windows), using the same layout as a
CLI install.
...
- **Athaniel Brain** runs as a headless `athaniel serve` process and exposes the
...
1. `ATHANIEL_DESKTOP_ATHANIEL_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ATHANIEL_DESKTOP_ATHANIEL`, or `athaniel` on `PATH`
5. a system Python that can import the Athaniel runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Athaniel
...
screen offers **Connect to existing Athaniel** before starting the local installer.
...
terminal commands, and file operations run against the remote Athaniel host, not
...
```json
{
  "mode": "remote",
  "remote": {
    "url": "https://athaniel.example.com",
    "authMode": "token",
...
transport- or Athaniel-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Athaniel-Session-Token`.
...
Boot logs land in `ATHANIEL_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\athaniel\athaniel-brain\.athaniel-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\athaniel\athaniel-brain\venv"
```

> The default Athaniel home on Windows is `%LOCALAPPDATA%\athaniel`. Set the `ATHANIEL_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://athaniel-brain.embreythecreator.com/docs/)
...
Built by [Athaniel](https://embreythecreator.com).
