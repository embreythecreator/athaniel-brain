...
import { getAthanielConfigRecord, type AthanielConfigRecord, saveAthanielConfig } from '@/athaniel'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AthanielConfigRecord>
  saveConfig: (config: AthanielConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.athanielDesktop?.api) {
      return Promise.resolve({})
    }

    return getAthanielConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.athanielDesktop?.api) {
    ...
    return saveAthanielConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: AthanielConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AthanielConfigRecord, locale: Locale): AthanielConfigRecord {
