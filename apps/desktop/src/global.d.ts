import type { GatewayWsUrlResult } from '@athaniel/shared'
import type { TranslucencyState } from '@athaniel/shared/translucency'
...
declare global {
  interface Window {
    athanielDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<AthanielConnection>
      ...
      // Resume this session in the user's own terminal emulator (`athaniel --tui
      ...
      // Athaniel Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ATHANIEL_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: AthanielApiRequest) => Promise<T>
      notify: (payload: AthanielNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<AthanielReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<AthanielReadFileTextResult>
      selectPaths: (options?: AthanielSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AthanielPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AthanielPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<AthanielPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: AthanielActiveWork) => void
      setTitleBarTheme?: (payload: AthanielTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AthanielReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ATHANIEL_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ATHANIEL_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<ATHANIEL_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<AthanielGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<AthanielGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<AthanielGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<AthanielRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: AthanielReviewScope, baseRef?: null | string) => Promise<AthanielReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: AthanielReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<AthanielReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<AthanielRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<AthanielPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: AthanielTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AthanielTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: AthanielWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: AthanielPreviewFileChanged) => void) => () => void
...
export interface AthanielTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AthanielTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `athaniel update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  athanielRoot?: string
...
export interface AthanielConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface AthanielTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface AthanielActiveWork {
  count: number
  titles: string[]
}

export interface AthanielWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Athaniel Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Athaniel Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Athaniel Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the athaniel gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Athaniel Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface AthanielApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface AthanielNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface AthanielPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AthanielReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AthanielPreviewWatch {
  id: string
  path: string
}
...
export interface AthanielGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface AthanielGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface AthanielGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface AthanielRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface AthanielRepoStatus {
  branch: null | string
...
// turn began.
export type AthanielReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface AthanielReviewFile {
  path: string
  added: number
  removed: number
...
export interface AthanielReviewList {
  files: AthanielReviewFile[]
...
export interface AthanielReviewPr {
  url: string
  state: string
  number: number
}
...
export interface AthanielBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface AthanielRepoPullRequests {
  ghReady: boolean
  prs: AthanielBranchPullRequest[]
}
...
export interface AthanielPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface AthanielReviewShipInfo {
  ghReady: boolean
  pr: AthanielReviewPr | null
}

export interface AthanielReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AthanielReadDirResult {
  entries: AthanielReadDirEntry[]
  error?: string
}

export interface AthanielPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AthanielSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
