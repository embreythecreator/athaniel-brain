...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { athanielDesktop: { readDir: typeof readDir } }).athanielDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AthanielReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { athanielDesktop: unknown }).athanielDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AthanielReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AthanielReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: AthanielReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: AthanielReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: AthanielReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AthanielReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.athanielDesktop is missing', async () => {
    delete (window as unknown as { athanielDesktop?: unknown }).athanielDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: AthanielReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: AthanielReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: AthanielReadDirResult) => void) | undefined
