import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@athaniel/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { AthanielConnection } from '@/global'
import { AthanielGateway } from '@/athaniel'
...
import type { RpcEvent } from '@/types/athaniel'
...
export function primaryRuntimeConnectionId(connection: Pick<AthanielConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.athanielDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AthanielGateway | null) => void
  refreshAthanielConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: AthanielConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Athaniel...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Athaniel…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Athaniel backend'
        )
        ...
        // as an opaque "Could not connect to Athaniel gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshAthanielConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshAthanielConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startAthaniel re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new AthanielGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Athaniel…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshAthanielConfig(),
          ...
          // retry it), not brick boot behind the "Athaniel couldn't start"
