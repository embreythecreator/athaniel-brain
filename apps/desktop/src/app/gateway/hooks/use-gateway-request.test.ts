...
vi.mock('@/athaniel', async importOriginal => {
  const actual = await importOriginal<typeof AthanielModule>()

  class FakeAthanielGateway {
  ...
  return { ...actual, AthanielGateway: FakeAthanielGateway }
})

import type * as AthanielModule from '@/athaniel'
import type { AthanielGateway } from '@/athaniel'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as AthanielGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as AthanielGateway, 'default')
  $gateway.set(primary as unknown as AthanielGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.athanielDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as AthanielGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { athanielDesktop: unknown }).athanielDesktop = { getConnection }
    $gateway.set(dropped)
