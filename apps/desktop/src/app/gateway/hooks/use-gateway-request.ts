import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@athaniel/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as AthanielGateway | null
  const gatewayRef = useRef<AthanielGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.athanielDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<AthanielGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as AthanielGateway | null
      }),
    []
  )
