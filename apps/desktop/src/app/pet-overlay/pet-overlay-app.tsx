...
export function PetOverlayApp() {
  ...
  const setIgnore = (ignore: boolean) => {
    if (ignoreRef.current !== ignore) {
      ...
      window.athanielDesktop?.petOverlay?.setIgnoreMouse(ignore)
    }
  }
  ...
  useEffect(() => {
    const off = window.athanielDesktop?.petOverlay?.onState(payload => {
    ...
    window.athanielDesktop?.petOverlay?.control({ type: 'ready' })
  ...
  useEffect(() => {
    ...
    window.athanielDesktop?.petOverlay?.setFocusable(composerOpen)
  ...
  const onPetPointerMove = (e: React.PointerEvent) => {
    ...
    window.athanielDesktop?.petOverlay?.setBounds({
      height: drag.height,
      width: drag.width,
  ...
  const onPetPointerUp = (e: React.PointerEvent) => {
    ...
    if (drag.moved) {
      ...
      window.athanielDesktop?.petOverlay?.control({
    ...
    if (e.shiftKey) {
      window.athanielDesktop?.petOverlay?.control({ type: 'pop-in' })
    ...
    if (clickTimerRef.current) {
      clearTimeout(clickTimerRef.current)
      clickTimerRef.current = undefined
      window.athanielDesktop?.petOverlay?.control({ type: 'toggle-app' })
  ...
  const send = () => {
    ...
    if (text) {
      window.athanielDesktop?.petOverlay?.control({ text, type: 'submit' })
    }
  ...
  const openApp = () => {
    ...
    setUnread(false)
    window.athanielDesktop?.petOverlay?.control({ type: 'open-app' })
  }
  ...
  const onScale = useCallback((next: number, anchor: PetZoomAnchor) => {
    ...
    window.athanielDesktop?.petOverlay?.control({ scale: next, type: 'scale' })
  }, [])
  ...
  useEffect(() => {
    ...
    window.athanielDesktop?.petOverlay?.setBounds(bounds)
    window.athanielDesktop?.petOverlay?.control({ bounds, type: 'bounds' })
