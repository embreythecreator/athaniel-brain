...
import type { SkillInfo, ToolsetInfo } from '@/types/athaniel'
...
export function SkillsView({
  ...
  const registryBridge = window.athanielDesktop?.connections
  const rosterBridge = window.athanielDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Athaniel (default)' : p.name,
      value: p.name
    }))
