...
import { pathFromAthanielDeepLink, resolveAthanielOpenPath } from '@/lib/athaniel-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.athanielDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.athanielDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // athaniel://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future athanielDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveAthanielOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (athaniel://index-network/intent/1, athaniel://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromAthanielDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.athanielDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.athanielDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
