...
import { getAthanielConfigRecord, saveMcpServers } from '@/athaniel'
...
import { setAthanielConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `athaniel://mcp/install` deep links. The payload is
 ...
 * confirming lets Athaniel spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getAthanielConfigRecord())
      ...
      setAthanielConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
