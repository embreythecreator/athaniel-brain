...
import { getLatestSessionMessages } from '@/athaniel'
...
import { useAthanielConfig } from '../session/hooks/use-athaniel-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshAthanielConfig, sttEnabled, voiceMaxRecordingSeconds } = useAthanielConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('athaniel:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('athaniel:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshAthanielConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshAthanielConfig])
