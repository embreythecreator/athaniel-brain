...
// lib/personalities (mirrors athaniel_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__athaniel_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Athaniel & Nous-trained models',
  ...
  { prefix: 'ATHANIEL_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // athaniel_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  browser: {
    useRealProfile:
      "Local browsing uses your real logins. Athaniel copies your default browser's profile (cookies, logins, preferences) into a managed snapshot and drives it with its packaged Chromium — your live profile is never opened directly, and the copy is refreshed from it on each run. Also lets the agent open a local real-profile session on request even when a cloud browser backend is configured. Only Chromium browsers (Chrome, Edge, Brave, Brave Origin, Chromium) are supported; a non-Chromium default fails with a clear message. Off by default."
  },
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Athaniel stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Athaniel can read from one file request.',
  approvals: {
    mode: 'How Athaniel handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Athaniel updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
