...
const getAthanielConfigRecord = vi.fn()
const saveAthanielConfig = vi.fn()
...
vi.mock('@/athaniel', () => ({
  ...
  getAthanielConfigRecord: () => getAthanielConfigRecord(),
  saveAthanielConfig: (config: unknown) => saveAthanielConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'athaniel-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['athaniel-4', 'athaniel-4-mini'],
        authenticated: true,
        capabilities: { 'athaniel-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'athaniel-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'nous', model: 'athaniel-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'athaniel-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getAthanielConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveAthanielConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nous', model: 'athaniel-4' })
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getAthanielConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveAthanielConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['athaniel-4'],
          authenticated: true,
          capabilities: { 'athaniel-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'athaniel-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · athaniel-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'nous', model: 'athaniel-4', enabled: false }),
