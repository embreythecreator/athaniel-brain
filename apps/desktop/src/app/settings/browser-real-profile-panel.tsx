...
import { type ProfileScope, saveAthanielConfigRecord } from '@/athaniel'
 ...
 * the default browser's profile (cookies/logins) into a Athaniel-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useAthanielConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveAthanielConfigRecord(next, profile)
        notify({
          kind: 'info',
