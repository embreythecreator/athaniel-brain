...
import { saveAthanielConfig } from '@/athaniel'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { AthanielConfigRecord } from '@/types/athaniel'

import { setAthanielConfigCache, useAthanielConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: AthanielConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useAthanielConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<AthanielConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setAthanielConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="athaniel-terminal-font-families"
          ...
          <datalist id="athaniel-terminal-font-families">
