...
import { getEnvVars, getAthanielConfigSchema } from '@/athaniel'
...
import { useAthanielConfigRecord } from '../hooks/use-config-record'
...
export function useSettingsSearchCatalog(enabled: boolean) {
  const { t } = useI18n()
  const configQuery = useAthanielConfigRecord()
