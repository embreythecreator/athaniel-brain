...
// Athaniel Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.athanielDesktop.setSecretStorageEncryption(on)
  ...
  // --- Athaniel Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.athanielDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteAthanielPath: state.sshRemoteAthanielPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.athanielDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.athanielDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'athaniel-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.athanielDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.athanielDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.athanielDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.athanielDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.athanielDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.athanielDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.athanielDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.athanielDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.athanielDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.athanielDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Athaniel Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/athaniel"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteAthanielPath: event.target.value }))}
                placeholder={g.sshAthanielPathPlaceholder}
                value={state.sshRemoteAthanielPath}
              />
            }
            description={g.sshAthanielPathDesc}
            title={g.sshAthanielPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.athanielDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
