...
import type { AthanielConfigRecord, SessionInfo } from '@/types/athaniel'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.athanielDesktop) {
      return
    }

    let alive = true

    void getAthanielConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveAthanielConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Athaniel wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.athanielDesktop` is
    ...
    const settings = window.athanielDesktop?.settings
