...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: AthanielConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: AthanielConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // ATHANIEL_QWEN_ shares the ATHANIEL_ stem with other integrations.
      expect(providerGroup('ATHANIEL_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: AthanielConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: AthanielConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: AthanielConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: AthanielConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: AthanielConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: AthanielConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: AthanielConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: AthanielConfigRecord = { toolsets: ['memory'] }
      const next: AthanielConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: AthanielConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: AthanielConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: AthanielConfigRecord = { toolsets: [] }
      const next: AthanielConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: AthanielConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: AthanielConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `athaniel config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: AthanielConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: AthanielConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: AthanielConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: AthanielConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: AthanielConfigRecord = {}
      const draft: AthanielConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: AthanielConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: AthanielConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: AthanielConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: AthanielConfigRecord = { toolsets: ['memory'] }
