 ...
 * agent sources (local runtime + any number of remote gateways / Athaniel Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.athanielDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.athanielDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `athaniel update` to every eligible source; per-connection results
