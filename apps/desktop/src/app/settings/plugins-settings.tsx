...
import { getProfiles } from '@/athaniel'
...
// User-installed plugins first — mirrors `athaniel plugins list --user`.
...
function reveal(file: string) {
  void window.athanielDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // athaniel_home breaks against a remote backend (#66899).
    const dir = await window.athanielDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.athanielDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's athaniel home (profile-aware), so the
// path comes from the gateway — not from the renderer's local ATHANIEL_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.athanielDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Athaniel (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
