...
import { disconnectOAuthProvider, listOAuthProviders } from '@/athaniel'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/athaniel'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.athanielDesktop?.terminal)
...
//      in athaniel_cli/provider_catalog.py) — the SAME provider identity
//      `athaniel model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Athaniel can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Athaniel can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Athaniel never deletes creds another tool owns behind a silent API call.
