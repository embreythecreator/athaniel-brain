...
const getAthanielConfigSchema = vi.fn()
const saveAthanielConfig = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/athaniel', () => ({
  getAthanielConfigRecord: () => getAthanielConfigRecord(),
  getAthanielConfigSchema: () => getAthanielConfigSchema(),
  saveAthanielConfig: (config: unknown, profile?: string) => saveAthanielConfig(config, profile),
...
beforeEach(() => {
  ...
  getAthanielConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getAthanielConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveAthanielConfig).toHaveBeenCalledTimes(1))
      expect(saveAthanielConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveAthanielConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveAthanielConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
