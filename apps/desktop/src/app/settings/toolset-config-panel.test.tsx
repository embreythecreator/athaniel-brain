...
import type { ToolsetConfig } from '@/types/athaniel'
...
beforeEach(() => {
  ...
  getAthanielConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getAthanielConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveAthanielConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveAthanielConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
