...
import { getElevenLabsVoices, getAthanielConfigSchema, saveAthanielConfig } from '@/athaniel'
...
import type { ConfigFieldSchema, AthanielConfigRecord } from '@/types/athaniel'

import { athanielConfigCacheWriter, useAthanielConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<AthanielConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useAthanielConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => athanielConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['athaniel-config-schema'] : ['athaniel-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getAthanielConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `athaniel config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<AthanielConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveAthanielConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: AthanielConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: AthanielConfigRecord) => {
