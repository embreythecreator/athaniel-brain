...
import { getAthanielConfigDefaults, getAthanielConfigRecord, saveAthanielConfig } from '@/athaniel'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getAthanielConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'athaniel-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveAthanielConfig(await getAthanielConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
