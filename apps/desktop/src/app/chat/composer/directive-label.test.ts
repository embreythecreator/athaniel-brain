...
import { athanielDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
function sentLabel(draft: string) {
  return athanielDirectiveFormatter
    .parse(draft)
...
describe('one label per reference, on every surface', () => {
  ...
  it('an agent mention inserts as plain @name text, never an @simple: chip', () => {
    ...
    expect(athanielDirectiveFormatter.serialize(item)).toBe('@mr-tester')
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/embreythecreator/athaniel-brain/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/embreythecreator/athaniel-brain/pull/74533'
