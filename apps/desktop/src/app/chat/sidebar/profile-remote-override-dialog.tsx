...
export function ProfileRemoteOverrideDialog({ profileNames }: { profileNames: string[] }) {
  ...
  useEffect(() => {
    ...
    window.athanielDesktop?.connections
      ?.list()
      .then(registry => {
        if (cancelled) {
          return
  ...
  const performSave = async () => {
    ...
    try {
      await window.athanielDesktop.applyConnectionConfig({
        mode: 'remote',
        profile,
  ...
  const removeOverride = async () => {
    ...
    try {
      await window.athanielDesktop.applyConnectionConfig({ mode: 'local', profile })
