...
import type { SessionInfo } from '@/athaniel'
...
// zero Athaniel sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, AthanielGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, AthanielGitWorktree[]>>({})
