...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__athanielTourEngine) {
    ${driverIife}
    w.__athanielTourHolder = {};
    w.__athanielTourCollect = (${collectTourTargets.toString()});
    w.__athanielTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__athaniel-tour-style')) {
    var style = document.createElement('style');
    style.id = '__athaniel-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__athanielTourEngine(
    w.driver.js.driver,
    w.__athanielTourHolder,
    ${JSON.stringify(action)},
    w.__athanielTourCollect,
    document
  ));
})()`
}
