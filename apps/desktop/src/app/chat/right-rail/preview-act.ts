...
const preamble = () => `  var w = window;
  ...
  w.__athanielActHolder = w.__athanielActHolder || {};
  w.__athanielAct = ${actEngineSource()};
  w.__athanielWatch_fn = (${watchInPage.toString()});
  w.__athanielWatchTag = ${WATCH_TAG};
  var holder = w.__athanielActHolder;
  var act = function (a) { return w.__athanielAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__athanielWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  w.__athanielHit = null;
  document.addEventListener('pointerdown', function (e) {
    w.__athanielHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ATHANIEL-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
