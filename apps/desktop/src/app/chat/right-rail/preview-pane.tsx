        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.athanielDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Athaniel to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.athanielDesktop?.watchPreviewFile ||
      !window.athanielDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.athanielDesktop.onPreviewFileChanged(payload => {
    ...
    void window.athanielDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.athanielDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.athanielDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:athaniel-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.athanielDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.athanielDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.athanielDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.athanielDesktop?.openExternal(currentUrl)
                : undefined
            }
