...
function setWorkspaceTransfer(transferring: boolean): void {
  window.athanielDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.athanielDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.athanielDesktop?.hud?.beginMove?.()
}
 ...
 * (see athaniel:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.athanielDesktop?.hud?.endMove?.()
      }
