...
import { getAthanielConfig } from '@/athaniel'
...
import { useAthanielConfig } from './use-athaniel-config'

vi.mock('@/athaniel', () => ({
  getAthanielConfig: vi.fn(),
  getAthanielConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'athaniel.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getAthanielConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getAthanielConfig>>)

describe('useAthanielConfig refreshAthanielConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Athaniel' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useAthanielConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshAthanielConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useAthanielConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getAthanielConfig>>>()
    vi.mocked(getAthanielConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useAthanielConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshAthanielConfig(true)
    })
    expect(getAthanielConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getAthanielConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getAthanielConfig>>>()
    vi.mocked(getAthanielConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useAthanielConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshAthanielConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getAthanielConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getAthanielConfig>>>()
    vi.mocked(getAthanielConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshAthanielConfig(true)
      refreshC = result.current.refreshAthanielConfig(true)
    })
