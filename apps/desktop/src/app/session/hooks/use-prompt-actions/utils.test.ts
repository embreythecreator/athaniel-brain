...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.athaniel/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.athaniel/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'Athaniel TUI Status\n\nSession ID: s-1\nModel: nous-athaniel-3 (unknown)'
