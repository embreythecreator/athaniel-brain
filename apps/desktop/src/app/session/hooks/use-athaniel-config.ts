...
import { getAthanielConfig, getAthanielConfigDefaults } from '@/athaniel'
...
interface AthanielConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useAthanielConfig({ activeSessionIdRef }: AthanielConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshAthanielConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getAthanielConfig(), getAthanielConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Athaniel' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshAthanielConfig, sttEnabled, voiceMaxRecordingSeconds }
}
