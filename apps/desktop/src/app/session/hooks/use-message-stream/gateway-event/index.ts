import { registryBackendScopeKey } from '@athaniel/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshAthanielConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshAthanielConfig — two REST calls (config + defaults) per event, per
