import type { BillingBlock } from '@athaniel/shared'
 ...
 * billing-specific toast — never the generic "Athaniel error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Athaniel reported an error',
