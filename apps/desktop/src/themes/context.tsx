...
const SKIN_KEY = 'athaniel-desktop-theme-v2'
const MODE_KEY = 'athaniel-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'athaniel-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'athaniel-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'athaniel-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.athanielTheme = skinName
  root.dataset.athanielMode = rendered
  ...
  window.athanielDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('athaniel-boot-background', chromeBg)
    window.localStorage.setItem('athaniel-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.athanielThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.athanielDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Athaniel authoring/activating a skin from a
