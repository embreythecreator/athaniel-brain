/**
 * Live skin sync from the Athaniel backend.
 *
 * The backend resolves the active skin (built-in or `$ATHANIEL_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Athaniel
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { AthanielSkin } from '@athaniel/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: AthanielSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as AthanielSkin)
