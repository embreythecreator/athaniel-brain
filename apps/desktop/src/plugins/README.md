...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `AthanielPlugin` and
...
`$ATHANIEL_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`athaniel-desktop-plugins` skill.
