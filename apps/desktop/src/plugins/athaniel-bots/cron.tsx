/**
 * Routines: the Athaniel cron jobs scoped to the bot you're chatting with — the
...
// athaniel -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `athaniel -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Athaniel-native schedule string; Advanced exposes it raw.
...
/** Compose the Athaniel schedule string from picker state. */
