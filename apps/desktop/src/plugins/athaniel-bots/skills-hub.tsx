...
import { Button, host, Input } from '@athaniel/plugin-sdk'
import { useEffect, useRef, useState } from 'react'

import { useBots } from './i18n'

// ── skills hub section: the REAL hub page (docs) embedded as a picker ──────
// https://athaniel-brain.nousresearch.com/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'athaniel-skill-pick', ...} to us (athaniel-brain#86243). We validate
...
export function HubSkillsSection({ forProfile, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'athaniel-skill-pick' || !data.name) {
        return
      }
