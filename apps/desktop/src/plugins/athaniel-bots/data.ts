...
import { atom, host, queryClient, useQuery, useValue } from '@athaniel/plugin-sdk'
...
export const BOT_ATTENTION_HINTS: Record<string, string> = {
  ...
  missing_config: 'Provider not configured — run athaniel model',
 ...
 *  protocol into the system prompt itself (athaniel-brain bot_mode_probe).
...
export function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (athaniel-brain #86875) also expose the union
 ...
 *  callable alias is 'athaniel' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'athaniel' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`athaniel profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Athaniel"
 *  can never hijack the primary profile's @athaniel alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'athaniel'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['athaniel-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Athaniel Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'athaniel') {
      token = byForm.has('athaniel') ? 'athaniel' : token
    }
