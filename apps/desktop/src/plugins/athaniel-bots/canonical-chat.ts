...
import * as sdk from '@athaniel/plugin-sdk'
import { host } from '@athaniel/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['athaniel-bots'].chat) were
 ...
 *  click path's tile-staleness probe (athaniel-brain#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Athaniel Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (athaniel-brain#89617) — clicking Retry
  ...
  // until an app restart (athaniel-brain#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Athaniel Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['athaniel-bots']
