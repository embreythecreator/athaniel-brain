...
import { Button, host, Input, useI18n } from '@athaniel/plugin-sdk'
...
// and the row falls back to the "run athaniel mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.athanielDesktop && window.athanielDesktop.mcpOauth
        ? window.athanielDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.athanielDesktop && window.athanielDesktop.openExternal) {
        window.athanielDesktop.openExternal(authUrl)
      } else {
