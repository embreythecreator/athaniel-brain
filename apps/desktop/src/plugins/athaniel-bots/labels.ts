...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Athaniel". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `athaniel profile rename
  ...
  // never generic "Athaniel" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Athaniel (the agent it is)
