/**
 * Athaniel Bot Mode — a "one chat per agent" roster for the Athaniel desktop.
 *
 * Left pane "Bots": one row per Athaniel profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Athaniel cron jobs) scoped to the
 ...
 * bot-initiated sends use `athaniel -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@athaniel/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@athaniel/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @athaniel.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.athanielDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.athanielDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Athaniel' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['athaniel-bots']?.title || bot.title || ''
            ).trim()
