...
describe('routing', () => {
  ...
  it('resolves @athaniel to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@athaniel take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @athaniel', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @athaniel,/)
    ...
    expect(peer).toMatch(/group chat with @athaniel/)
  })
