 ...
 * `{type:'athaniel-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as AthanielSdk from '@athaniel/plugin-sdk'
import { cleanup, fireEvent, render, screen } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

const HUB_ORIGIN = 'https://athaniel-brain.nousresearch.com'
...
vi.mock('@athaniel/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof AthanielSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'nous/web-research', name: 'Web Research', type: 'athaniel-skill-pick' },
      {
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'athaniel-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'nous/web-research', type: 'athaniel-skill-pick' }, { source: frame.contentWindow })
