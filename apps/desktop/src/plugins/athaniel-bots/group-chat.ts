...
import { atom, host } from '@athaniel/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'athaniel-bots-groups'
...
/** The v3 envelope stored under the default profile's `athaniel-bots-groups`
 ...
 *  clients (Athaniel Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Athaniel"
 *  (community report, Aug 21 2026: renamed default still read "Athaniel is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Athaniel (matching displayName and the
 *  @athaniel handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`athaniel profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Athaniel' : trimmed
}
