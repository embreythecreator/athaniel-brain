...
import { host, LruCache } from '@athaniel/plugin-sdk'
...
// to (local, remote URL, SSH, Athaniel Cloud, docker) must be able to find
...
// athaniel_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['athaniel-bots']?.title || profile?.display_name || ''),
