...
import { forgetSessionUnread, host, queryClient } from '@athaniel/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['athaniel-bots']
...
/** Permanently delete a bot's Athaniel profile, then remove plugin-local state
 ...
 * directory (athaniel-brain#52279). That is the "can't delete a bot" error. */
