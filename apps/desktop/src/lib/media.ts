...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.athanielDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.athanielDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.athanielDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.athanielDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `athaniel-media://remote/${file}${scope ? `?${scope}` : ''}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `athaniel-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.athaniel/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.athanielDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.athanielDesktop.saveGatewayFile({
