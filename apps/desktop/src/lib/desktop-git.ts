import type {
  AthanielGitBaseBranch,
  AthanielGitBranch,
  AthanielGitWorktree,
  AthanielRepoPullRequests,
  AthanielRepoStatus,
  AthanielReviewList,
  AthanielReviewShipInfo
} from '@/global'
import { athanielApi } from '@/athaniel'
...
// (window.athanielDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['athanielDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.athanielDesktop

  if (!desktop) {
    throw new Error('Athaniel Desktop bridge is unavailable')
  }

  return athanielApi<T>(
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: AthanielGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: AthanielGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: AthanielGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<AthanielRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<AthanielReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<AthanielReviewShipInfo>('review/ship-info', { path: repoPath }),

    prList: (repoPath, branches, numbers) =>
      gitPost<AthanielRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.athanielDesktop?.git
}
