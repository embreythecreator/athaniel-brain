...
// Athaniel is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.athanielMode
 ...
 * the only carriers of Athaniel' palette — are dropped so the paste target's
