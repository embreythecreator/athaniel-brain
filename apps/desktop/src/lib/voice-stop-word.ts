...
//
// When someone is in a hands-free "Hey Athaniel" voice chat, the natural way to
...
// phrase (optionally addressed to Athaniel), so a real turn that merely contains
...
// Optional address prefixes so "athaniel stop" / "ok stop" / "hey athaniel, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey athaniel', 'hey athaniel,', 'athaniel', 'athaniel,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("athaniel") is not a stop command on its own.
      continue
    }
 ...
 * to Athaniel). Returns false for anything that merely contains "stop" as part of
