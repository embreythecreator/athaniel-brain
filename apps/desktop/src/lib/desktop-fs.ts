import { athanielApi } from '@/api/client'
import type {
  AthanielConnection,
  AthanielReadDirResult,
  AthanielReadFileTextResult,
  AthanielSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: AthanielConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: AthanielConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<AthanielReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<AthanielReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<AthanielReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<AthanielReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.athanielDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: AthanielSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
