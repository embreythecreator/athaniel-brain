...
import { getApiRequestConnection, getApiRequestProfile, athanielApi } from '@/athaniel'
...
export async function fetchVoiceClientConfig(): Promise<null | VoiceClientConfig> {
  ...
  const promise = (async () => {
    try {
      // athanielApi carries connectionScoped(); profileScoped() adds the
      // profile — the same routing every relay audio call uses, so the
      // config comes from the backend the user is actually talking to.
      const response = await athanielApi<{ ok: boolean } & VoiceClientConfig>({
        ...profileScoped(),
