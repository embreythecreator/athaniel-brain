...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>Athaniel Athaniel</p>')
    performScopedFind(surface, 'athaniel', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'athaniel', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>athaniel</p>')
    performScopedFind(surface, 'athaniel', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">Athaniel</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>athaniel</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'athaniel')).toBe(true)
  })
