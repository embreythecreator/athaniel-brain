 ...
 * `athaniel://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `athaniel://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `athaniel://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type AthanielOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const ATHANIEL_PROTOCOL = 'athaniel:'
...
export function normalizeAthanielOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('athaniel://') || trimmed.startsWith(`${ATHANIEL_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // athaniel://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // athaniel://index-network/intent/1 → /index-network/intent/1
...
export function resolveAthanielOpenPath(target: AthanielOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeAthanielOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeAthanielOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeAthanielOpenString(target.path)
 ...
 * (`athaniel://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromAthanielDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveAthanielOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveAthanielOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `athaniel://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromAthanielDeepLink('open', name, params)
}
