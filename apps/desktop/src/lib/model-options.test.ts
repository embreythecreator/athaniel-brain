...
import { getGlobalModelOptions } from '@/athaniel'
...
const globalOptions = { model: 'athaniel-4', provider: 'nous', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'athaniel-local', provider: 'athaniel-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['athaniel-local'], name: 'Athaniel Local vLLM', slug: 'athaniel-local' }]
    }
    ...
    await expect(requestModelOptions({ gateway: gateway as never, sessionId: 'session-1' })).resolves.toEqual({
      ...restPayload,
      model: 'athaniel-local',
      provider: 'athaniel-local'
    })
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'athaniel-local', provider: 'athaniel-local', providers: [] }
  ...
  it('scopes REST recovery to the catalog owner profile', async () => {
    const restPayload = {
      model: 'berry-local',
      provider: 'athaniel-local',
      providers: [{ models: ['berry-local'], name: 'Athaniel Local', slug: 'athaniel-local' }]
    }
...
describe('manualPickRemoved', () => {
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'nous', 'athaniel-4')).toBe(false)
  })
