...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __athanielWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__athanielWatch || (win.__athanielWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('athaniel-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('athaniel-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __athanielWatchTag?: number }).__athanielWatchTag
