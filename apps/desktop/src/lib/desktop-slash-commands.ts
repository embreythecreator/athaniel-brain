...
/** Why a known Athaniel command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Athaniel profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownAthanielSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Athaniel' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownAthanielSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Athaniel and were never asked for. It is
