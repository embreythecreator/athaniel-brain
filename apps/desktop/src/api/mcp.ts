import type { McpCatalogResponse, McpServerSummary } from '@/types/athaniel'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.athanielDesktop.api<McpTestResult>({
 ...
 *  `saveAthanielConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string, profile?: ProfileScope): Promise<McpOAuthFlow> {
  return window.athanielDesktop.api<McpOAuthFlow>({
    ...capabilityScoped(profile),
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return athanielApi<{ ok: boolean; status: string }>({
...
// `athaniel mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveAthanielConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return athanielApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return athanielApi<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope): Promise<McpCatalogResponse> {
  return window.athanielDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.athanielDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),
