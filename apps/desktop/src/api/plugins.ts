...
import { getApiRequestConnection, getApiRequestProfile, athanielApi, profileScoped } from './client'
 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<AthanielConnection> {
  ...
  return withTimeout(
    window.athanielDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `athanielDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see AthanielApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.athanielDesktop?.api) {
    throw new Error('Athaniel desktop bridge unavailable')
  }
