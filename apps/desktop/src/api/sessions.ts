...
import { capabilityScoped, getApiRequestConnection, athanielApi, type ProfileScope, profileScoped } from './client'
...
export async function listSessions(
  ...
  const result = await athanielApi<PaginatedSessions>({
    ...profileScoped(),
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await athanielApi<SidebarSessionsResponse>({
      ...profileScoped(),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return athanielApi<SessionSearchResponse>({
...
export function getSession(id: string, profile?: ProfileScope): Promise<SessionInfo> {
  ...
  return athanielApi<SessionInfo>({
...
export function getSessionMessages(
  ...
  return athanielApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return athanielApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
