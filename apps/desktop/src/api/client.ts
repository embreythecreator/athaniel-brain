import { JsonRpcGatewayClient } from '@athaniel/shared'

import type { AthanielApiRequest } from '@/global'
...
// "Timed out connecting to Athaniel backend" that hangs the UI (#48504).
//
...
export class AthanielGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Athaniel gateway connection closed',
      connectErrorMessage: 'Could not connect to Athaniel gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Athaniel gateway is not connected',
...
// process to route them to that host (athaniel:api's registry branch). Null
...
export function athanielApi<T>(request: AthanielApiRequest): Promise<T> {
  return window.athanielDesktop.api<T>({ ...connectionScoped(), ...request })
}
