...
const athanielApi = vi.mocked(client.athanielApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    athanielApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'athaniel-pi' })

    await deleteSession('sess-1', { connectionId: 'athaniel-pi', profile: 'tommy' })

    expect(athanielApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'athaniel-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((athanielApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = athanielApi.mock.calls[0][0] as { body: Record<string, unknown> }
