...
import { capabilityScoped, athanielApi, type ProfileScope, profileScoped } from './client'
...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return athanielApi<MemoryProviderConfig>({
...
export function startMemoryProviderOAuth(
  ...
  return athanielApi<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `athaniel memory` / `athaniel curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return athanielApi<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return athanielApi<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return athanielApi<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return athanielApi<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function runCurator(): Promise<ActionResponse> {
  return athanielApi<ActionResponse>({
    ...profileScoped(),
...
export function updateAthaniel(): Promise<ActionResponse> {
  return athanielApi<ActionResponse>({
    ...profileScoped(),
    path: '/api/athaniel/update',
    method: 'POST'
  })
}
...
export function checkAthanielUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return athanielApi<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/athaniel/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: ProfileScope): Promise<ActionStatusResponse> {
  return window.athanielDesktop.api<ActionStatusResponse>({
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return athanielApi<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return athanielApi<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return athanielApi<ElevenLabsVoicesResponse>({
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return athanielApi<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `athaniel doctor` / `athaniel security
// audit` / `athaniel backup` / `athaniel debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return athanielApi<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return athanielApi<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return athanielApi<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return athanielApi<DebugShareResponse>({
