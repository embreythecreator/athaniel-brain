...
import { connectionScoped, athanielApi, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'
...
// Cron jobs are stored per-profile (<ATHANIEL_HOME>/cron/jobs.json), and the
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await athanielApi<{ runs: SessionInfo[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await athanielApi<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return athanielApi<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
    ...connectionScoped(),
