...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'athaniel:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.athanielDesktop?.saveImageFromUrl) {
        if (await window.athanielDesktop.saveImageFromUrl(src)) {
