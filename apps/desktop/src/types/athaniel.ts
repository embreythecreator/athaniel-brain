...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Athaniel doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in athaniel_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `athaniel model` uses — instead of
...
export interface AthanielConfig {
  agent?: {
...
export type AthanielConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `athaniel model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Athaniel simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (athaniel.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors athaniel_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Athaniel, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `athaniel tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Athaniel); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** athaniel-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** athaniel-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/athaniel/update/check` — the backend's own update state.
