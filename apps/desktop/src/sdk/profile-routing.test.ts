...
vi.mock('@/athaniel', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), athanielApi: vi.fn() }))
...
const { deleteProfile, athanielApi } = await import('@/athaniel')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its ATHANIEL_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { athanielDesktop: unknown }).athanielDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(athanielApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(athanielApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(athanielApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
