/**
 * @athaniel/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__ATHANIEL_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, athanielApi, type AthanielGateway } from '@/athaniel'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/athaniel'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Athaniel profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.athanielDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.athanielDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and AthanielGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (athaniel-brain#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.athanielDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Athaniel Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.athanielDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Athaniel Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Athaniel Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (athaniel-brain#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.athanielDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('Athaniel Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return athanielApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return athanielApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Athaniel gateway unavailable')
    }
 ...
 *  "ATHANIEL AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { AthanielGateway } from '@/athaniel'
...
export type { AthanielOpenTarget } from '@/lib/athaniel-open-target'
...
/** Athaniel' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/athaniel'
