...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Athaniel Brain Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Athaniel docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Athaniel Brain Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Athaniel docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://athaniel-brain.embreythecreator.com/docs' },
    ...
    expect(view.title).toBe('Failed to open athaniel-brain.embreythecreator.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://athaniel-brain.embreythecreator.com/docs' },
        result: { success: true, url: 'https://athaniel-brain.embreythecreator.com/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened athaniel-brain.embreythecreator.com/docs')
  })
})
