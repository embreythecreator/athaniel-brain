...
describe('frameSizeFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'athaniel-inline-preview-size',
    token: 'tok',
    height: 500,
    width: 300,
    ...over
...
describe('intentFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'athaniel-inline-preview-intent',
    token: 'tok',
  ...
  it('rejects wrong token, wrong type, empty, and hostile shapes', () => {
    ...
    expect(intentFromMessage(msg({ type: 'athaniel-inline-preview-size' }), 'tok')).toBeNull()
...
describe('withInlineChrome intent wiring', () => {
  it('injects athaniel.send and the data-athaniel-send click bridge', () => {
    const framed = withInlineChrome('<html><body><h1>w</h1></body></html>', 'tok', '')

    expect(framed).toContain('window.athaniel={send:send}')
    expect(framed).toContain('data-athaniel-send')
    expect(framed).toContain('athaniel-inline-preview-intent')
  })
})
