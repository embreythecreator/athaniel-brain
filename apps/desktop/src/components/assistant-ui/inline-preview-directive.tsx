 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.athaniel.send(prompt)` (or declarative
 * `data-athaniel-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'athaniel-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'athaniel-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `athaniel.send(prompt)`.
 ...
 *  wires `data-athaniel-send` so declarative HTML works with zero script:
 *  `<button data-athaniel-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.athaniel={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-athaniel-send]"):null;' +
    'if(el)send(el.getAttribute("data-athaniel-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `athaniel serve` — renders live.)
