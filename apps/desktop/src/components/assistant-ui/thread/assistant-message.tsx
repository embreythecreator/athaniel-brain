...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ATHANIEL_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.athanielDesktop?.logsRoot?.()
      ...
      const result = await window.athanielDesktop?.openDir?.(root)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {window.athanielDesktop?.logsRoot && (
