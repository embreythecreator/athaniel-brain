...
import { formatRefValue, athanielDirectiveFormatter } from './directive-text'
...
describe('athanielDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = athanielDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = athanielDirectiveFormatter.parse('@file:src/main.tsx the entry point')
  ...
  it('parses session links with profile/id values', () => {
    const segments = athanielDirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...athanielDirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = athanielDirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...athanielDirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
