...
const ATHANIEL_REF_TYPES = WIRE_REFERENCE_KINDS
type AthanielRefType = (typeof ATHANIEL_REF_TYPES)[number]
...
const ATHANIEL_DIRECTIVE_RE = referenceRe()
...
export const athanielDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ATHANIEL_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...athanielDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Athaniel directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.athanielDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.athanielDesktop?.readFileDataUrl(id)
