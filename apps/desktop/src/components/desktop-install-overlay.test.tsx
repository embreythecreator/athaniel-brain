...
function installDesktopMock(state: DesktopBootstrapState) {
  ...
  Object.defineProperty(window, 'athanielDesktop', {
    configurable: true,
    value: desktop
  })
...
afterEach(() => {
  cleanup()
  vi.useRealTimers()
  Reflect.deleteProperty(window, 'athanielDesktop')
})
...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'win32', activeRoot: 'C:\\Users\\me\\AppData\\Local\\athaniel\\athaniel-brain' }
      })
    )

    render(<DesktopInstallOverlay />)

    expect(await screen.findByText('Set up Athaniel Desktop')).toBeTruthy()
    expect(screen.getByText('Connect to existing Athaniel')).toBeTruthy()
    expect(screen.getByText('Install Athaniel locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when Install Athaniel locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install Athaniel locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up Athaniel Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up Athaniel Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install Athaniel locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart Athaniel Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install Athaniel locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart Athaniel Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install Athaniel locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart Athaniel Desktop and try again.')
    ).toBeTruthy()
    ...
    act(() => {
      desktop.emitBootstrapEvent({
        ...
        active: false,
        platform: 'win32',
        activeRoot: 'C:\\Users\\me\\AppData\\Local\\athaniel\\athaniel-brain-repaired'
      })
    })

    expect(screen.queryByText('Local installation could not start. Restart Athaniel Desktop and try again.')).toBeNull()
  })

  it('opens the remote connection form from the first-run choice', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'linux', activeRoot: '/home/me/.athaniel/athaniel-brain' }
      })
    )

    render(<DesktopInstallOverlay />)

    fireEvent.click(await screen.findByText('Connect to existing Athaniel'))
  ...
  it('requires a successful token connection test before applying remote config', async () => {
    ...
    desktop.probeConnectionConfig.mockResolvedValue({
      authMode: 'token',
      baseUrl: 'https://gateway.example.com/athaniel',
      error: null,
      providers: [],
      reachable: true,
      version: '0.17.0'
    ...
    fireEvent.click(await screen.findByText('Connect to existing Athaniel'))
    fireEvent.change(await screen.findByPlaceholderText('https://gateway.example.com/athaniel'), {
      target: { value: 'https://gateway.example.com/athaniel' }
    })
    ...
    await waitFor(() => {
      expect(desktop.testConnectionConfig).toHaveBeenCalledWith({
        ...
        remoteUrl: 'https://gateway.example.com/athaniel'
      })
    })

    await screen.findByText('Connected to https://gateway.example.com/athaniel (0.17.0).')
  ...
  it('ignores a completed probe after the gateway URL becomes invalid', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing Athaniel'))
    const urlInput = await screen.findByPlaceholderText('https://gateway.example.com/athaniel')
    fireEvent.change(urlInput, { target: { value: 'https://gateway.example.com/athaniel' } })
  ...
  it('does not enable Apply when credentials change during a connection test', async () => {
    ...
    await act(async () => {
      resolveTest?.({ baseUrl: 'https://gateway.example.com/athaniel', ok: true, version: '0.17.0' })
      await pendingTest
    })

    expect(screen.queryByText('Connected to https://gateway.example.com/athaniel (0.17.0).')).toBeNull()
    expect(apply.disabled).toBe(true)
  })
  ...
  it('signs in, tests, and applies a password-style remote gateway', async () => {
    ...
    await waitFor(() => {
      expect(desktop.oauthLoginConnectionConfig).toHaveBeenCalledWith('https://gateway.example.com/athaniel')
    })
    ...
    await screen.findByText('Connected to https://gateway.example.com/athaniel.')
    const apply = screen.getByText('Apply and reconnect').closest('button') as HTMLButtonElement
    expect(apply.disabled).toBe(false)
    fireEvent.click(apply)
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    const desktop = installDesktopMock(
      bootstrapState({
        unsupportedPlatform: {
          platform: 'darwin',
          activeRoot: '/Users/me/.athaniel/athaniel-brain',
    ...
    expect(await screen.findByText('Athaniel needs a one-time install')).toBeTruthy()
    ...
    fireEvent.change(screen.getByPlaceholderText('https://gateway.example.com/athaniel'), {
      target: { value: 'https://gateway.example.com/athaniel' }
    })
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('Athaniel needs a one-time install')).toBeNull()
  })
})
