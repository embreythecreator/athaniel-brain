...
describe('isRemoteReauthError', () => {
  ...
  it('ignores non-auth boot errors and nullish', () => {
    expect(isRemoteReauthError('Athaniel background process exited during startup.')).toBe(false)
...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'nous', displayName: 'Athaniel', supportsPassword: false }])).toEqual({
      isPassword: false,
      providerLabel: 'Athaniel'
    })
  })
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'nous', displayName: 'Athaniel', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / Athaniel')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Athaniel' })).toBe(
      'Sign in with Athaniel'
    )
  })
