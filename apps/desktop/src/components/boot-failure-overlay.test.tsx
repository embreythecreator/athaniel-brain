...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Athaniel gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.athanielDesktop
  Object.defineProperty(window, 'athanielDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'athanielDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('shows the Nous Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    ...
    try {
      ...
      expect(screen.getByText(/ares-3009\.agents\.embreythecreator\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
