...
function sessionsBotsTree() {
  ...
  registerPane('athaniel-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'athaniel-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('athaniel-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('athaniel-bots:pane')
    expect($hiddenStripTabs.get()).toContain('athaniel-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('athaniel-bots:pane')

    expect(setStripTabHidden('athaniel-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('athaniel-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('athaniel-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('athaniel-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('athaniel.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('athaniel-bots:pane')
    ...
    revealTreePane('athaniel-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('athaniel-bots:pane')
    expect(window.localStorage.getItem('athaniel.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'athaniel-bots:pane', title: 'athaniel-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'athaniel-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
