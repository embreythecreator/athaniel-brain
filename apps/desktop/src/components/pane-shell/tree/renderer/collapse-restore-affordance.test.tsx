...
describe('Sessions/Bots strip — #91223', () => {
  beforeEach(() => {
    ...
    registerPane('athaniel-bots:pane', { hideOnly: true, placement: 'left' }, 'Bots')
  ...
  it('double-clicking the Sessions tab leaves the strip and both chips', () => {
    ...
    expect(tabEl('athaniel-bots:pane')).toBeTruthy()
  ...
  it('double-clicking the Bots tab leaves the strip too', () => {
    ...
    tap(tabEl('athaniel-bots:pane')!)
    doubleTap(tabEl('athaniel-bots:pane')!)
...
describe('docked tool tile — collapsing keeps the restore chip', () => {
  beforeEach(() => {
    registerPane('workspace', { placement: 'main', uncloseable: true }, 'Chat')
    registerPane('athaniel-bots:routines', { placement: 'main', width: '250px' }, 'Cronjobs')
    $layoutTree.set(
      split('row', [
        group(['workspace'], { active: 'workspace', id: 'g-main' }),
        group(['athaniel-bots:routines'], { active: 'athaniel-bots:routines', id: 'g-routines' })
      ])
    )
  })
  ...
  it('clicking the active tab does not collapse the tile or drop its label', () => {
    ...
    expect(tabEl('athaniel-bots:routines')).toBeTruthy()
    tap(tabEl('athaniel-bots:routines')!)

    expect(zoneAt(1).minimized).toBeFalsy()
    expect(tabEl('athaniel-bots:routines')).toBeTruthy()
    expect(tabEl('athaniel-bots:routines')?.textContent).toMatch(/cronjobs/i)
  })
  ...
  it('collapsing via the store does not dismiss the pane from the tree', () => {
    collapseTreePane('athaniel-bots:routines')

    expect(zoneAt(1).minimized).toBe(true)
    expect($dismissedPanes.get().has('athaniel-bots:routines')).toBe(false)
    expect(zoneAt(1).panes).toContain('athaniel-bots:routines')
