...
const TREE_KEY = 'athaniel.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'athaniel.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'athaniel.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['athaniel-bots:pane'], active: 'athaniel-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'athaniel-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'athaniel-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","athaniel-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['athaniel-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['athaniel-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'athaniel-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['athaniel-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'athaniel-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'athaniel-bots:pane'],
          active: 'athaniel-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'athaniel-bots:pane', 'athaniel-bots:routines'],
          active: 'athaniel-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'athaniel-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'athaniel-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'athaniel-bots:pane'])
    expect(botsGroup.active).toBe('athaniel-bots:pane')
    expect(routinesGroup.panes).toEqual(['athaniel-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['athaniel-bots:routines'],
          active: 'athaniel-bots:routines'
        }
      ]
    }
