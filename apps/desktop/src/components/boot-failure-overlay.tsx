...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.athanielDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.athanielDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.athanielDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Athaniel Cloud must reuse its portal session
  ...
  const openLogs = () => void window.athanielDesktop?.revealLogs().catch(() => undefined)
  ...
  if (remoteReauth) {
    ...
    actions = [
      ...
      { ...retryAction, variant: 'secondary' },
      {
        ...
        onClick: () => openExternalLink('https://discord.gg/embreythecreator'),
        variant: 'ghost'
      },
