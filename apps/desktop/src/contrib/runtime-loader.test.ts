...
import type { AthanielReadDirResult } from '@/global'
import type * as AthanielModule from '@/athaniel'
...
// getStatus would supply the connected backend's athaniel_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ athaniel_home: '/remote/box/.athaniel' }))

vi.mock('@/athaniel', async importActual => ({
  ...(await importActual<typeof AthanielModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const agentPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<AthanielReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { athanielDesktop: unknown }).athanielDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { athanielDesktop?: unknown }).athanielDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend athaniel_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.athaniel/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.athaniel/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.athaniel/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.athaniel/plugins')
    // The remote backend's athaniel_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.athaniel/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.athaniel/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.athaniel/plugins/my-feature' }] }
      }

      if (dir === '/local/.athaniel/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.athaniel/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.athaniel/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.athaniel/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.athaniel/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.athaniel/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.athaniel/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.athaniel/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.athanielDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.athaniel/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.athaniel/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.athaniel/plugins/uni' }] }
      }

      if (dir === '/local/.athaniel/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.athaniel/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.athaniel/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.athaniel/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.athaniel/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend athaniel_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.athaniel/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.athaniel/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.athaniel/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.athaniel/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.athaniel/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.athaniel/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.athanielDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.athaniel/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.athaniel/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.athaniel/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'athaniel-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "athaniel-bots", name: "Bot Mode", register() {} }',
        'athaniel-bots',
        { file: '/local/.athaniel/desktop-plugins/athaniel-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['athaniel-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['athaniel-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.athaniel/desktop-plugins/athaniel-bots/plugin.js'
      })
