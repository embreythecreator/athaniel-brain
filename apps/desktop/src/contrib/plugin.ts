 ...
 * `AthanielPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/athaniel'
...
export type { PluginRestOptions } from '@/athaniel'
 ...
 *  under `athaniel.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.athanielDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Athaniel — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `athaniel://index-network/intent/1`),
...
export interface AthanielPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `athaniel.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.athanielDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.athanielDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.athanielDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.athanielDesktop
