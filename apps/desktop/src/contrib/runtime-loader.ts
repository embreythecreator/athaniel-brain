 ...
 *   (`@athaniel/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default AthanielPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<athaniel home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<athaniel home>/plugins/<name>/desktop/
...
import { createPluginContext, type AthanielPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.athaniel/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@athaniel/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @athaniel/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: AthanielPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default AthanielPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of athaniel-bots predating its adoption in-tree) must
...
//  - `<athaniel home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<athaniel home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's athaniel_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.athaniel/plugins` is installed-but-inert until the
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Athaniel Desktop to load larger plugins"
    )
  }
...
async function resolveDiskPluginEntry(
  desktop: Window['athanielDesktop'],
  folderPath: string,
