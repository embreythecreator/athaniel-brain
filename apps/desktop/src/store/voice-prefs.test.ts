...
vi.mock('@/athaniel', () => ({
  getAthanielConfigRecord: vi.fn(async () => ({})),
  saveAthanielConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye athaniel', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye athaniel')
  })
