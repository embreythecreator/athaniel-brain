...
// reaches the gateway); everything else routes through window.athanielDesktop.git,
...
function file(path: string, over: Partial<AthanielReviewFile> = {}): AthanielReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as AthanielReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.athanielDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as AthanielReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.athanielDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: AthanielReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as AthanielReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { athanielDesktop?: unknown }).athanielDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as AthanielReviewShipInfo)
