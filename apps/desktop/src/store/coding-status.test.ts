...
import type { AthanielRepoStatus } from '@/global'
...
const sampleStatus: AthanielRepoStatus = {
...
const otherStatus: AthanielRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<AthanielRepoStatus | null>) {
  ;(window as unknown as { athanielDesktop?: unknown }).athanielDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: AthanielRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: AthanielRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
...
describe('repoChangeKindForPath', () => {
  it('does not notify a row when only another path changes', () => {
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'b.ts', untracked: true } as AthanielRepoStatus['files'][number]]
      }
    })
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'a.ts', untracked: true } as AthanielRepoStatus['files'][number]]
      }
    })
