...
import type { AthanielActiveWork } from '@/global'
import { $sessions } from '@/store/session'
import { $workingSessionIds } from '@/store/session-states'

const $activeWork = computed([$workingSessionIds, $sessions], (workingIds, sessions): AthanielActiveWork => {
...
if (typeof window !== 'undefined') {
  ...
  $activeWork.subscribe(work => {
    ...
    lastSent = next
    window.athanielDesktop?.setActiveWork?.(work)
  })
}
