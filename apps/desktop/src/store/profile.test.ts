...
const { getProfiles } = await import('@/athaniel')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/athaniel/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<AthanielConnection> = {}): AthanielConnection =>
  ({ baseUrl: 'https://athaniel-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as AthanielConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { athanielDesktop: { getConnection } })
