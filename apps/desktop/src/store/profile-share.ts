 ...
 *
 * The archive is the CLI's own `athaniel profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/athaniel'
...
import type { ProfileDesktopOverlay } from '@/types/athaniel'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Athaniel profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.athanielDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.athanielDesktop?.selectPaths?.({
