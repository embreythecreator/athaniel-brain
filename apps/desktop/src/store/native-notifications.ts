...
import { type AthanielOpenTarget, resolveAthanielOpenPath } from '@/lib/athaniel-open-target'
...
export type { AthanielOpenTarget }
...
const STORAGE_KEY = 'athaniel:native-notifications'
...
// "Backgrounded" = the user isn't on Athaniel. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `athaniel://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.athanielDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `athaniel://index-network/intent/1`). */
  activate?: AthanielOpenTarget
 ...
 *  user is away from Athaniel — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveAthanielOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveAthanielOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.athanielDesktop
