...
import type { AthanielGitBaseBranch, AthanielGitBranch } from '@/global'
import { getAthanielConfig, athanielApi, type AthanielGateway } from '@/athaniel'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/athaniel'
...
const PROJECT_SCOPE_KEY = 'athaniel.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Athaniel gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Athaniel profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await athanielApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<AthanielGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<AthanielGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Athaniel
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getAthanielConfig(context.profile))
...
// repo, returning where Athaniel should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<AthanielGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<AthanielGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.athanielDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.athanielDesktop?.writeClipboard?.(path)
  }
}
