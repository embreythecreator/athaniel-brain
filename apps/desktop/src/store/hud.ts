 ...
 * composer with the reply scrolling above it, so Athaniel can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.athanielDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.athanielDesktop?.hud
...
export function resetHudLayout(): void {
  void window.athanielDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.athanielDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.athanielDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
