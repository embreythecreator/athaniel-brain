...
const checkAthanielUpdateSpy = vi.fn()
const updateAthanielSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/athaniel', () => ({
  checkAthanielUpdate: (...args: unknown[]) => checkAthanielUpdateSpy(...args),
  updateAthaniel: (...args: unknown[]) => updateAthanielSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkAthanielUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkAthanielUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'athaniel update',
      message: null,
    ...
    expect(checkAthanielUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkAthanielUpdateSpy).not.toHaveBeenCalled()
  })
})

// The ⌘K "Update Athaniel" row. It used to call applyBackendUpdate() flat, which
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updateAthanielSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkAthanielUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      athanielDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('applies the CLIENT update in local mode, never the backend', async () => {
    ...
    expect(updateAthanielSpy).not.toHaveBeenCalled()
  ...
  it('applies the BACKEND update in remote mode', async () => {
    ...
    requestActiveUpdate()
    await vi.waitFor(() => expect(updateAthanielSpy).toHaveBeenCalled())
...
describe('applyEverythingUpdate', () => {
  ...
  it('remote mode: updates the backend, then the still-behind client — the stale-GUI gap', async () => {
    ...
    expect(updateAthanielSpy).toHaveBeenCalledTimes(1)
  ...
  it('a failed backend leg does not strand the fan-out or the client', async () => {
    setRemote(true)
    $mockConnectionsRegistry.set(registryOf(['local', 'vps']))
    updateAthanielSpy.mockRejectedValue(new Error('backend gone'))
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      athanielDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'athaniel update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('athaniel update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Athaniel to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updateAthanielSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== athaniel-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updateAthanielSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'athaniel-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== athaniel-update started now ===', `step ${attempt}`],
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updateAthanielSpy.mockResolvedValue({ ok: true, name: 'athaniel-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== athaniel-update started now ===', '✓ Already up to date!'],
      name: 'athaniel-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== athaniel-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'athaniel-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== athaniel-update completed ${'c'.repeat(32)} ===`],
        name: 'athaniel-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== athaniel-update completed ${actionId} ===`],
        name: 'athaniel-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== athaniel-update completed ${actionId} ===`],
      name: 'athaniel-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('athaniel-update', 2000)
  })
  ...
  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkAthanielUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkAthanielUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== athaniel-update started now ===', 'still running'],
        name: 'athaniel-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== athaniel-update started now ===', 'Update complete!'],
        name: 'athaniel-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updateAthanielSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'athaniel-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== athaniel-update started now ===', '✓ Already up to date!'],
      name: 'athaniel-update',
      pid: 1,
      running: false
    })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== athaniel-update started now ===', 'update failed'],
      name: 'athaniel-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateAthanielSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkAthanielUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      athanielDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },
