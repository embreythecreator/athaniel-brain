...
import { getAthanielConfigRecord, saveAthanielConfig } from '@/athaniel'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getAthanielConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveAthanielConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
