 ...
 * `@athaniel/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-athaniel-glass]`
 * block in styles.css).
 */
...
} from '@athaniel/shared/translucency'
 ...
 * sets `athanielDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.athanielDesktop?.glassSupported === 'boolean'
    ? window.athanielDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.athanielDesktop?.translucencySupported === 'boolean'
    ? window.athanielDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-athaniel-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-athaniel-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-athaniel-glass-on', glassLive)
  root.toggleAttribute('data-athaniel-glass', glassOn)
  root.toggleAttribute('data-athaniel-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-athaniel-glass-scope', scope)
  } else {
    root.removeAttribute('data-athaniel-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.athanielDesktop?.setTranslucency?.(state)
  })
