...
import { checkAthanielUpdate, getActionStatus, updateAthaniel } from '@/athaniel'
...
import type { BackendUpdateCheckResponse } from '@/types/athaniel'
...
const UPDATE_TOAST_SNOOZE_KEY = 'athaniel:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'athaniel:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'athaniel:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateAthaniel'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.athanielDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkAthanielUpdate(true))
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.athanielDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `athaniel update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'athaniel update',
        command: result.command ?? 'athaniel update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== athaniel-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateAthaniel()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'athaniel update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkAthanielUpdate(true)
