...
/**
 * Feature store for backend (agent) plugins — the native Athaniel plugins plus
 ...
 * primitives `athaniel plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Athaniel. */
  portable?: boolean
}
 ...
 *  ATHANIEL_HOME. Always refetches (it's a cheap local disk scan on the
