...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@athaniel/shared'
...
const TILES_KEY = 'athaniel.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'athaniel.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'athaniel.desktop.botChatSessions.v1'
 ...
 *  (athaniel-brain#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_athaniel_home() re-creates
 * the profile directory the delete just removed (athaniel-brain#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (athaniel-brain#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('athaniel-bots:pane') is not a
...
// Dev hook for automation (mirrors __ATHANIEL_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ATHANIEL_SESSION_TILES__ = {
