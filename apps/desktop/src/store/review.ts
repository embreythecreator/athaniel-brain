...
import type { AthanielReviewFile, AthanielReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Athaniel' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'athaniel.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'athaniel.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'athaniel.desktop.reviewTreeMode'
const SELECTED_KEY = 'athaniel.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<AthanielReviewFile[]>([])
...
export const $reviewShipInfo = atom<AthanielReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['athanielDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: AthanielReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly AthanielReviewFile[], path: string): AthanielReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.athanielDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.athanielDesktop?.openExternal?.(url)
    }
