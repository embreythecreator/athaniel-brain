import type { ConnectionState } from '@athaniel/shared'
...
import type { SessionInfo, UsageStats } from '@/types/athaniel'
...
const COMPOSER_MODEL_KEY = 'athaniel.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'athaniel.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'athaniel.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'athaniel.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'athaniel.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'athaniel.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'athaniel.desktop.lastRoute'
...
function workspaceCwdKey(connection: AthanielConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.athanielDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.athanielDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<AthanielConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'athaniel.desktop.sessionOwnerHints.v1'
...
export const setConnection = (next: Updater<AthanielConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useAthanielConfig.
