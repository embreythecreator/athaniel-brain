...
const agentConn = (over: Partial<AthanielConnection> = {}): AthanielConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as AthanielConnection

const localConn = (over: Partial<AthanielConnection> = {}): AthanielConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as AthanielConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<AthanielConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<AthanielConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { athanielDesktop: { getConnection, getConnectionFor } })
})
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<AthanielConnection>(() => undefined))
