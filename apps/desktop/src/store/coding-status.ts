...
import type { AthanielGitWorktree, AthanielRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: AthanielGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, AthanielRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, AthanielGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<AthanielRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<AthanielGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<AthanielRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<AthanielGitWorktree[]>>()
const $noRepoStatus = atom<AthanielRepoStatus | null>(null)
const $noWorktrees = atom<AthanielGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<AthanielRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<AthanielGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: AthanielRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: AthanielGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<AthanielRepoStatus | null>
  seq: number
  target: string
}
