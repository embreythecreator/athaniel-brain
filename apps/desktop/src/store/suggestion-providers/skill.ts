...
import { getSkills } from '@/athaniel'
 ...
 *  Working in `~/www/athaniel-brain`, the draft says "athaniel-brain" constantly
 *  — a "Use skill: athaniel-brain" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`athaniel-brain-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/athaniel-brain-dev`, `plugin:skill`). */
