...
describe('collidesWithWorkspace', () => {
  it('suppresses a skill named exactly like the cwd folder', () => {
    expect(collidesWithWorkspace('athaniel-brain', '/Users/b/www/athaniel-brain')).toBe(true)
  })

  it('suppresses inside worktree-suffixed folders too', () => {
    expect(collidesWithWorkspace('athaniel-brain', '/Users/b/www/athaniel-brain-suggest')).toBe(true)
  })
  ...
  it('does not suppress on substring-only overlap', () => {
    ...
    expect(collidesWithWorkspace('clean', '/Users/b/www/athaniel-brain')).toBe(false)
  })

  it('never collides when detached (empty cwd)', () => {
    expect(collidesWithWorkspace('athaniel-brain', '')).toBe(false)
  })
})
...
describe('skillTouchedInMessages', () => {
  ...
  it('matches qualified skill names (category/name, plugin:name)', () => {
    expect(
      skillTouchedInMessages('athaniel-brain-dev', [toolCall('skill_view', { name: 'github/athaniel-brain-dev' })])
    ).toBe(true)
    expect(
