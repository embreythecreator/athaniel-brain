...
export function setZoomPercent(percent: number): void {
  window.athanielDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.athanielDesktop?.zoom) {
  void window.athanielDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.athanielDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
