...
import type { AthanielBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/athaniel'
...
export const $pullRequestsByBranch = atom<Record<string, AthanielBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'athaniel.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('athaniel.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: AthanielBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
