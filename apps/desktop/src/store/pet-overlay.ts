 ...
 * app's bounds and stays visible while Athaniel is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'athaniel.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'athaniel.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.athanielDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.athanielDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.athanielDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.athanielDesktop?.petOverlay?.close()
}
