import { LOCAL_CONNECTION_ID } from '@athaniel/shared'
import { atom, batch, computed } from 'nanostores'

import type { AthanielConnection } from '@/global'
import { getProfiles, athanielApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/athaniel'
...
import type { ProfileInfo } from '@/types/athaniel'
...
// /api/profiles/active `current`). "default" is the root ~/.athaniel. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'athaniel.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'athaniel.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await athanielApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ATHANIEL_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.athanielDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<AthanielConnection | null> {
  const getConnection = window.athanielDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<AthanielConnection | null> {
  const getConnectionFor = window.athanielDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'athaniel.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.athanielDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.athanielDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.athaniel) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.athanielDesktop?.touchBackend?.(target).catch(() => undefined)
}
