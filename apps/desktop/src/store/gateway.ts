import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@athaniel/shared'
import { atom } from 'nanostores'

import type { AthanielConnection } from '@/global'
import { AthanielGateway, setApiRequestConnection } from '@/athaniel'
...
const isOpen = (gateway: AthanielGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: AthanielConnection) => void
  /**
...
interface Secondary {
  ...
  connection: AthanielConnection | null
  gateway: AthanielGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: AthanielGateway | null
  ...
  $gateway: ReturnType<typeof atom<AthanielGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('athaniel.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<AthanielGateway | null>(null),
...
export function setPrimaryGateway(gateway: AthanielGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<AthanielConnection, 'connectionId'> | null): void {
...
export function activeGateway(): AthanielGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the athaniel module (null
...
function publishActiveConnection(connection: AthanielConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new AthanielGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: AthanielGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Athaniel gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.athanielDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Athaniel Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<AthanielGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Athaniel gateway is not connected" on the Sessions `+` action (#88880).
