...
React + Ink terminal UI for Athaniel. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ATHANIEL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `athaniel setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.athaniel/.athaniel_history` or under `ATHANIEL_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `athaniel setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/athaniel-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external athaniel setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.athaniel/tui-parent.log
    ...
    types/
      athaniel-ink.d.ts            type declarations for @athaniel/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AthanielCLI subprocess for slash commands
```
