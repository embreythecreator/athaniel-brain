import type { InputEvent, Key } from '@athaniel/ink'
import * as Ink from '@athaniel/ink'
 ...
 * but that accepts `key.meta` on macOS — and athaniel-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ATHANIEL_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which athaniel-ink decodes as meta+'d'; without this branch it
