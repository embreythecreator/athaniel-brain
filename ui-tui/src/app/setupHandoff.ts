import type { RunExternalProcess } from '@athaniel/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`athaniel ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching athaniel: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`athaniel ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
