...
import { terminalBackgroundHex } from '@athaniel/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ATHANIEL_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ATHANIEL_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ATHANIEL_TUI_BACKGROUND', process.env.ATHANIEL_TUI_BACKGROUND ?? '(unset)'],
            ['ATHANIEL_TUI_THEME', process.env.ATHANIEL_TUI_THEME ?? '(unset)'],
