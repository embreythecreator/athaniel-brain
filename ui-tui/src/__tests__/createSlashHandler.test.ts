...
// DASHBOARD_TUI_MODE resolves once at module load from ATHANIEL_TUI_DASHBOARD,
...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'Athaniel TUI Status' }))
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Athaniel TUI Status', 'Status')
    })
  })
  ...
  it('falls through to command.dispatch for skill commands, sending the body but showing the invocation', async () => {
    const skillMessage =
      '[IMPORTANT: The user has invoked the "athaniel-brain-dev" skill, indicating they want you to follow its instructions.\n' +
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({
                ...
                name: 'athaniel-brain-dev',
                display: '/athaniel-brain-dev'
              })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/athaniel-brain-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.send).toHaveBeenCalledWith(skillMessage, true, '/athaniel-brain-dev')
    })
  ...
  it('/history pages the current TUI transcript (user + assistant)', () => {
    ...
    expect(body).toContain('[Athaniel #2]')
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/athaniel_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/athaniel_conversation_test.json')
    })
  })
