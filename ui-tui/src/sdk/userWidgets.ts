...
import { Box, Text } from '@athaniel/ink'
...
/**
 * User widget apps — Athaniel authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ATHANIEL_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.athaniel/plugins/`: files under ATHANIEL_HOME execute
...
const widgetsDir = () => join(process.env.ATHANIEL_HOME?.trim() || join(homedir(), '.athaniel'), 'tui-widgets')
 ...
 *  every change, so a widget Athaniel writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Athaniel) ever writes must hot-load too — a 10s poll here read
