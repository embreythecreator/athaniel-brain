 ...
 * `athaniel-boot-background` / `athaniel-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ATHANIEL_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.athaniel matches get_athaniel_home()'s default.
const bootFilePath = () => join(process.env.ATHANIEL_HOME ?? join(homedir(), '.athaniel'), 'tui-theme-boot.json')

// Never touch the user's real ~/.athaniel from test runs (the TS suite has no
// ATHANIEL_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ATHANIEL_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ATHANIEL_TUI_THEME || env.ATHANIEL_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.ATHANIEL_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.ATHANIEL_TUI_BACKGROUND
  ) {
    env.ATHANIEL_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ATHANIEL_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ATHANIEL_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ATHANIEL_TUI_THEME.
