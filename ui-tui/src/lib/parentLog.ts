...
const logDir = join(process.env.ATHANIEL_HOME?.trim() || join(homedir(), '.athaniel'), 'logs')
...
// a real ~/.athaniel (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('athaniel-tui: parent lifecycle log unavailable\n')
    }
  }
}
