...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because ATHANIEL_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.ATHANIEL_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.athaniel', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `athaniel-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.ATHANIEL_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.ATHANIEL_HEAPDUMP_MAX_BYTES?.trim()
