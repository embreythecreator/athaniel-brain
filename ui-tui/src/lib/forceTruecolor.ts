...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ATHANIEL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Athaniel on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ATHANIEL_TUI_TRUECOLOR=1.
