...
Write path is the CLI (``athaniel cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_athaniel_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`athaniel cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `athaniel cron notepad {job_id} delete "
