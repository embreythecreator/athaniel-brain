...
``athaniel_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import athaniel_state (cycle) — shared
module-level constants live in athaniel_state_common.
"""
...
# Moved methods logged under the "athaniel_state" logger before the split;
...
def schema_read_probe_statements() -> tuple:
    ...
    ``athaniel_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `athaniel update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._athaniel_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._athaniel_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`athaniel sessions optimize-storage` with the gateway stopped. "
                        "`athaniel doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from athaniel_constants import get_athaniel_home
            cache_path = get_athaniel_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `athaniel sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `athaniel db optimize`) must keep its EXISTING
