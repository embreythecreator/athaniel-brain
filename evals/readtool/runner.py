"""Run the read-tool eval through the REAL Athaniel AIAgent.

For each task: fresh temp ATHANIEL_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    athaniel_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".athaniel"
    ...
    try:
        ...
        shutil.rmtree(athaniel_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.athaniel/.env; set +a  — then relaunch."
        )
