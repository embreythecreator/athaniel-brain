# Athaniel Brain - Development Guide

Instructions for AI coding assistants and developers working on the athaniel-brain codebase.

**Never give up on the right solution.**

## What Athaniel Is

Athaniel is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Athaniel ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`athaniel tools`, `athaniel setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `ATHANIEL_HOME`. Mocks hide integration bugs.
...
- **New `ATHANIEL_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `athaniel tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.athaniel/plugins/` (or via a
  pip entry point), and promote them in the Athaniel Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `athaniel <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `athaniel webhook`, `athaniel cron`, `athaniel tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.athaniel/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Athaniel Cloud. Only the first two are spawned
by us and carry `ATHANIEL_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
Athaniel desktop app."*

The pattern that works:

- **The toolset is the surface gate.** Keep the tools off `_ATHANIEL_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `ATHANIEL_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`athaniel --tui` against that same backend) is
...
`$HOME/.athaniel/athaniel-brain/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _ATHANIEL_CORE_TOOLS list
├── cli.py                # AthanielCLI class — interactive CLI orchestrator (~11k LOC)
├── athaniel_state.py       # SessionDB — SQLite session store (FTS5 search)
├── athaniel_constants.py   # get_athaniel_home(), display_athaniel_home() — profile-aware paths
├── athaniel_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── athaniel_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── athaniel-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `athaniel --tui`
...
**User config:** `~/.athaniel/config.yaml` (settings), `~/.athaniel/.env` (API keys only).
**Logs:** `~/.athaniel/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_athaniel_home()`.
Browse with `athaniel logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Athaniel: desktop, TUI, website, and future TS packages.
...
- **Default CLI avatar** (`athaniel_cli/banner.py`, `ATHANIEL_SIGIL`) is embedded dithered Braille art with a continuous orb contour derived from Face's `core-idle.webp`, preserving its bright crown, dark centre, and lower reflection. Its 30-column, 13-row grid compensates for terminal line height; the lower hemisphere is dimmed to emphasize the crown. It inherits terminal foreground and uses Braille blanks to preserve alignment; conversion details live beside the constant. No runtime Face/Pillow dependency. Keep `banner_hero` skin overrides authoritative.
- **KawaiiSpinner** (`agent/display.py`) — animated faces during API calls, `┊` activity feed for tool results
- `load_cli_config()` in cli.py merges hardcoded defaults + user config YAML
- **Skin engine** (`athaniel_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `AthanielCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.athaniel/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`athaniel_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/athaniel` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `athaniel_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `AthanielCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `athaniel --tui` or `ATHANIEL_TUI=1`.
...
npm run dev       # watch mode (rebuilds athaniel-ink + tsx --watch)
npm start         # production
npm run build     # full build (athaniel-ink + tsc)
...
### TUI in the Dashboard (`athaniel dashboard` → `/chat`)

The dashboard embeds the real `athaniel --tui` — **not** a rewrite.  See `athaniel_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `athaniel_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `athaniel --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `athaniel --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@athaniel/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `athaniel serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `ATHANIEL_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `athaniel` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `athaniel --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Athaniel built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Athaniel core.
Use the plugin route instead: create `~/.athaniel/plugins/<name>/plugin.yaml`
and `~/.athaniel/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Athaniel tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_ATHANIEL_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_ATHANIEL_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_athaniel_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `ATHANIEL_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_athaniel_home()` for the base directory — never `Path.home() / ".athaniel"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `athaniel_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `athaniel_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `athaniel tools`, `athaniel setup`, most CLI subcommands | `athaniel_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`athaniel_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
athaniel_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.athaniel/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Athaniel gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `athaniel_cli/skin_engine.py`:
...
Users create `~/.athaniel/skins/<name>.yaml`:
...
Athaniel has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.athaniel/plugins/` and pip-installed entry points.

### General plugins (`athaniel_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.athaniel/plugins/`, `./.athaniel/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `athaniel` at startup so
  `athaniel <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$ATHANIEL_HOME/plugins/`, `./.athaniel/plugins/` (opt-in via
`ATHANIEL_ENABLE_PROJECT_PLUGINS`), and `athaniel_brain.memory_providers` entry
...
`post_setup(athaniel_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `athaniel <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `athaniel --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `athaniel_cli/main.py`, etc.).
...
into `~/.athaniel/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `athaniel memory setup` / `post_setup()` without
landing in this tree. PRs that add a new directory under
`plugins/memory/` will be closed with a pointer to publish the
provider as its own repo. Existing in-tree providers stay; bug fixes
to them are welcome.

**Word (`plugins/memory/word/`) implements the optional hooks
`on_memory_write` and `on_delegation`** (WO-POSTURE/1 1.2): every
completed `delegate_task` child is mirrored into Word as a
`[delegation]` note with a provenance block (child/parent session,
posture). The dispatch lives in `tools/delegate_tool.py` → `memory_manager.on_delegation`;
nothing in core changed. The provider's write half is
`capture_note(title, content, metadata=) -> vault_id|None` (1.1): sha256
dedup over title+content via the `captured` ledger in `word_queue.db`,
sync write when Word is healthy, durable enqueue otherwise; the queue
dead-letters a row after `_MAX_WRITE_ATTEMPTS`. Core reaches it only
through `agent/word_seam.capture(agent, …)` (1.3) — never import the
plugin. The capture point is the host-private frame in
`athaniel_cli/middleware.run_tool_execution_middleware` (1.5): `tool_executor`
passes `capture_agent=agent`, the frame pops it before any plugin callback,
and for `CAPTURE_TOOLS` (`web_search`, `web_extract`, `read_file`,
`search_files`) wraps `next_call` so the successful result lands in Word and
the model's tool result carries `vault_id`. Capture failures never alter the
tool result; the frame sits above the no-callbacks short-circuit on purpose.
Results longer than `ELIDE_CHARS` whose capture landed are replaced by
`{vault_id, title, head, elided_chars, note}` (1.6) — the model reads the
rest with `word_memory_read`. The frame stamps `posture` from the live
`agent._execution_policy`; PLAN-posture and web-origin (`T3`) captures are
filed in the quarantine notebook (`_QUARANTINE_NOTEBOOK`; D-4 pending)
AND flagged `quarantined=1` in the `captured` ledger — the ledger is the
truth (Word's note payload carries no notebook id), and `prefetch` drops
any hit whose id is quarantined (1.7). `/plan approve` rewrites into the
EXECUTING turn; the provider's `prefetch` sees that state on the session
and calls `promote_session(session_id)` — ledger-only, since Word has no
note-move endpoint and re-filing would churn `vault_id`s (1.8).
Streaming SSE callbacks in `api_server.py`
run on the agent thread — always `_stream_q.put_threadsafe`, never bare `.put` (0.1).

**Postures (WO-POSTURE/1 2.1):** `ExecutionPosture` is `ACT | PLAN | CHAT`.
`CHAT` is a per-session flag on `ExecutionPolicy` (`store.set_chat`), not a
lock state: `policy_deny_message` treats it exactly like ACT
(`tools_unrestricted`), plan mode outranks it while active, and the flag
rides through `enter_planning`/`finish` so the session returns to chat when
a plan ends. The Face sends `posture: chat|act` on every real turn and
`APIServerAdapter._apply_posture` flips the flag at the three request
entry points (2.3); the MoA chat preset keys off `posture is CHAT` (Phase 3).
...
`~/.athaniel/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Athaniel Discord
...
2. User: `$ATHANIEL_HOME/plugins/model-providers/<name>/`
...
[`athaniel-example-plugins`](https://github.com/embreythecreator/athaniel-example-plugins)
companion repo, not in this tree.

### Bot Mode (`apps/desktop/src/plugins/athaniel-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a Athaniel
...
`ui_meta['athaniel-bots'].chat` and verified it per click; five hardening
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `athaniel skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.athaniel.tags`, `metadata.athaniel.category`,
`metadata.athaniel.related_skills`, `metadata.athaniel.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.athaniel.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Athaniel tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Athaniel Brain" is the secondary collaborator. If the
   contributor's commit shows "Athaniel Brain" as author (because they
   used Athaniel to draft the skill), replace it with their actual name
...
lives in the `athaniel-brain-dev` skill at
...
`"messaging"`); `_ATHANIEL_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `athaniel tools` (the curses UI) or the
...
Athaniel returns a delegation id immediately and the result re-enters the
...
**Plan-mode children (WO-POSTURE/1 4.3):** when the parent is in PLAN
posture, `_build_child_agent` gives the child only the toolsets carrying
`PLAN_CHILD_ALLOWED_TOOLS` (read/search/skills/web — never delegation,
debate, todo, clarify, or save_plan) AND persists a `PLAN(plan_child=True)`
policy row under the child's own session id. The conversation loop reloads
the policy from the store every turn, so an attribute on the child would be
overwritten — the row is what makes name-level denial (e.g. `write_file`
inside the shared `file` toolset) hold in the child. If the row cannot be
written the child falls back to zero toolsets.

**Plan-frame tasks (WO-POSTURE/1 4.2):** `delegate_task` `tasks[].frame`
(one of `model_council.PLAN_FRAMES`) resolves through
`resolve_plan_frame_slot` → `_resolve_frame_credentials`, and that seat's
`model/provider/base_url/api_key/api_mode` replaces the hoisted delegation
creds for that task only. The child is built with `model_pin=True`, so the
MoA guard may retarget provider/base_url/key off the `moa` virtual but never
`effective_model`. A framed task with no `output_schema` gets
`plan_frame_output_schema(frame)`. Unknown frame / keyless seat is a
`tool_error` before any child is built.

**NeuroArxiv per frame (WO-POSTURE/1 4.5):** `PLAN_MODE_INSTRUCTIONS` step 2
embeds `model_council.render_plan_frame_briefs()` — one `delegate_task`
`tasks[]` entry per `PLAN_FRAMES` frame with `frame` set; each brief names
the lens, `skill_view('neuroarxiv')`, `web_extract`-only fetches, and the
D-8 read cap from `PLAN_FRAME_READ_CAPS` (12/frame, contrarian 6). Change
caps there, not in the prompt.

## Curator (skill lifecycle)

Background skill-maintenance system that tracks usage on agent-created
skills and auto-archives stale ones. Users never lose skills; archives
go to `~/.athaniel/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `athaniel_cli/curator.py` wires `athaniel curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.athaniel/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `athaniel cron <verb>`
...
- File lock at `~/.athaniel/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `athaniel kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `athaniel_cli/kanban.py` wires `athaniel kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`athaniel-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `ATHANIEL_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
## Update Pipeline (`athaniel update`)
...
- **Plan** (`athaniel_cli/update_inventory.py`, `athaniel update --plan`):
...
- **Snapshot** (`athaniel_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `athaniel-gateway*` unit / `ai.athaniel.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.athaniel/logs/update_receipts/` (`latest.json` pointer; steps,
...
`athaniel serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_athaniel_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
Athaniel Brain ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `ATHANIEL_BACKGROUND_NOTIFICATIONS` env var):
...
Athaniel supports **profiles** — multiple fully isolated instances, each with its own
`ATHANIEL_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `athaniel_cli/main.py` sets
`ATHANIEL_HOME` before any module imports. All `get_athaniel_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_athaniel_home()` for all ATHANIEL_HOME paths.** Import from `athaniel_constants`.
   NEVER hardcode `~/.athaniel` or `Path.home() / ".athaniel"` in code that reads/writes state.
   ```python
   # GOOD
   from athaniel_constants import get_athaniel_home
   config_path = get_athaniel_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".athaniel" / "config.yaml"
   ```

2. **Use `display_athaniel_home()` for user-facing messages.** Import from `athaniel_constants`.
   This returns `~/.athaniel` for default or `~/.athaniel/profiles/<name>` for profiles.
   ```python
   # GOOD
   from athaniel_constants import display_athaniel_home
   print(f"Config saved to {display_athaniel_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.athaniel/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_athaniel_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_athaniel_home()`,
   not `Path.home() / ".athaniel"`.

4. **Tests that mock `Path.home()` must also set `ATHANIEL_HOME`** — since code now uses
   `get_athaniel_home()` (reads env var), not `Path.home() / ".athaniel"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"ATHANIEL_HOME": str(tmp_path / ".athaniel")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not ATHANIEL_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".athaniel" / "profiles"`, NOT `get_athaniel_home() / "profiles"`.
   This is intentional — it lets `athaniel -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `athaniel_cli.update_cmd._athaniel_holder_subcommand`
  (top-level subcommand of any Athaniel argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.athaniel` paths
Use `get_athaniel_home()` from `athaniel_constants` for code paths. Use `display_athaniel_home()`
for user-facing print/log messages. Hardcoding `~/.athaniel` breaks profiles — each profile
has its own `ATHANIEL_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `athaniel_cli/curses_ui.py`. See `athaniel_cli/tools_config.py` for an example.
...
canary ledger, athaniel#85796 / gateway-gateway#210). Violating any of them
...
with actual imports (not mocks) against a temp `ATHANIEL_HOME`.

### Tests must not write to `~/.athaniel/`
The `_isolate_athaniel_home` autouse fixture in `tests/conftest.py` redirects `ATHANIEL_HOME` to a temp dir. Never hardcode `~/.athaniel/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_athaniel_home()` resolve within the temp dir.
Use the pattern from `tests/athaniel_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    home = tmp_path / ".athaniel"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("ATHANIEL_HOME", str(home))
    return home
```
...
subprocess (`--file-retries`, default 1; `ATHANIEL_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.athaniel/` | Your real config+auth.json                  | Temp dir per test                         |
...
Athaniel supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/athaniel_cli/test_venv_holder_windows_live.py` on a real
...
### Tool activity diff previews (THREAD-DONORS-2 audit, 2026-09-05)

The API adapter's completed tool activity event keeps redacted `resultPreview`
up to 64,000 characters for `patch` and `write_file`, so ordinary unified diffs
arrive intact. Other tools retain the 300-character preview. Diff receipts are parsed and their diff text redacted before JSON encoding
(the string redactor must not consume escaped newlines). Only success + diff
are exposed in these receipts. Other previews use `_redact_api_error_text`; `resultTokens` still measures the full original result.
Oversized patch previews remain bounded and consumers must label truncation.
Coverage: `tests/gateway/test_api_server.py` runs the actual SSE handler for
ordinary results, patch/write receipts, redaction, and oversized diffs.

### Durable Face forks (WO-THREAD/BRANCH-1, 2026-09-06)

Schema 27 adds nullable `sessions.fork_message_id`; the schema reconciler adds
it to existing databases. `_insert_session_row`, compact/list projections and
session export/import preserve it. Only explicit forks set this value.
`SessionDB.fork_session` owns one transaction for source/lease validation,
cutoff selection, transcript copying, lineage and source end-reason publication.
Exact role/content/occurrence anchors understand Face's prepared-user header
and multimodal text parts. Regenerate excludes the user from the copy because
the next completion resends it. Manual forks retain their selected message and
any tool results needed to close its tool calls. Repeated regeneration at the
same fork point shares a lineage parent. Never guess a stale/compacted cutoff.
The API returns 409 for an active turn or compression lease; invalid cutoffs or
titles fail without partial children. Tests live in `tests/gateway/test_session_api.py`.

Live audit verification: the supervised gateway restart completed after the
branch changes, `/health` returned OK, and a read-only check of the active
`~/.athaniel/state.db` confirmed `fork_message_id` is present. No transcript
was altered by that verification.
