...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with athaniel-brain)
2. User plugins: ``$ATHANIEL_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``athaniel_brain.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ATHANIEL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ATHANIEL_HOME/plugins/model-providers/`` if it exists."""
    try:
        from athaniel_constants import get_athaniel_home

        d = get_athaniel_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ATHANIEL_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_athaniel_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``athaniel_brain.plugins``
    ...
    * **Provider targets only.** The ``athaniel_brain.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ATHANIEL_HOME``) keep their
    ...
    try:
        from athaniel_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ATHANIEL_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``athaniel_brain.plugins``
    ...
    #    filesystem steps means a bundled or ``$ATHANIEL_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with athaniel-brain.
    ...
    # 2. User plugins — under $ATHANIEL_HOME/plugins/model-providers/<name>/.
