...
def _resolve_athaniel_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.athaniel",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ATHANIEL_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ATHANIEL_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because ATHANIEL_HOME is exactly where
    ...
    """
    athaniel_home = _resolve_athaniel_home()

    # Reject absolute paths — they bypass the ATHANIEL_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ATHANIEL_HOME)",
            relative_path,
        )
        return False

    host_path = athaniel_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape ATHANIEL_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, athaniel_home)
    ...
    # inside ATHANIEL_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.athaniel/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="athaniel-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching athaniel_constants.get_athaniel_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_athaniel_dir()`` for backward compatibility with old directory layouts.
    """
    from athaniel_constants import get_athaniel_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_athaniel_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_athaniel_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.athaniel``,
    SSH ``<remote_home>/.athaniel``, etc.) and whether translation is wanted.
...
def to_agent_visible_cache_path(
    ...
    backend's Athaniel cache lands):

    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.athaniel`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.athaniel`` is shell-expanded by the remote shell, so tool
    ...
    * singularity — NOT translated: Apptainer auto-binds the host home, so
    ...
    if backend in ("docker", "modal"):
        pass  # /root/.athaniel default
    elif backend in ("ssh", "daytona", "vercel_sandbox"):
        container_base = "~/.athaniel"
    else:
