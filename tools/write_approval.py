...
(see ``athaniel_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<ATHANIEL_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_athaniel_home() / "pending" / subsystem
