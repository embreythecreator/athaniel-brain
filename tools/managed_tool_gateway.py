...
def auth_json_path():
    """Return the Athaniel auth store path, respecting ATHANIEL_HOME overrides."""
    return get_athaniel_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`athaniel tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Athaniel release, and
