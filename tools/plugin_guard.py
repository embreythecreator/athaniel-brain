...
warn / fail). Athaniel already scans hub-installed *skills* via
``tools/skills_guard.py``; this module extends the same static-analysis
engine to ``athaniel plugins install`` and ``athaniel plugins update``, which
...
- Exempts the "reads own env secret" / "HTTP call with key" pattern
  ...
  foreign credential-store access (~/.ssh, ~/.aws, ~/.athaniel/.env),
...
#
# ``athaniel_env_access`` (a reference to ``~/.athaniel/.env``) is the DOCUMENTED
...
def scan_plugin(plugin_dir: Path, source: str = "") -> ScanResult:
    ...
    Args:
        plugin_dir: Path to the plugin directory (typically the temp clone,
            before it is moved into ``~/.athaniel/plugins/``).
