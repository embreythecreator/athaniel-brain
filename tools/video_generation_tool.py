...
provider** — turn on a backend by enabling a plugin (``athaniel plugins
enable video_gen/<name>``) and selecting it in ``athaniel tools`` → Video
Generation.
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `athaniel tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `athaniel tools` → "
...
# `athaniel tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`athaniel tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `athaniel tools` → Video Generation."
        )
        return {
            "description": "\n".join(parts),
            "parameters": {
