...
``~/.athaniel/skills/.curator_ledger.jsonl`` describing who changed what, with
before/after file manifests whose contents are stored content-addressed
(sha256-deduped) under ``~/.athaniel/.curator_backups/blobs/``.
...
Design decisions (Teknium-approved):
  ...
  - The ledger covers ALL actors, tagged ``curator`` / ``agent`` / ``user``.
    ...
    ``athaniel curator rollback <entry-id>``.
...
# the tree first) makes `athaniel curator rollback` restore a shell of a
...
def ledger_path() -> Path:
    return get_athaniel_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_athaniel_home() / ".curator_backups" / "blobs"
...
def fill_snapshot_from_curator_backup(
    ...
    *root* itself when it is known (delete/archive/purge all restore to the
    ...
    stay under ``skills/`` and ATHANIEL_HOME.
    """
    ...
    for rel, data in extra.items():
        ...
        if not _is_within(skills, dest) or not _is_within(get_athaniel_home(), dest):
            continue
        try:
            rel_key = (
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """All paths in an entry must live under ATHANIEL_HOME. Defense in depth —
...
def rollback_entry(entry_id: str) -> Tuple[bool, str]:
    ...
    # paths. The filled set is re-validated against ATHANIEL_HOME below.
