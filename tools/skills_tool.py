...
# Cache validation (mirrors athaniel_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.athaniel/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set ATHANIEL_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped ATHANIEL_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ATHANIEL_HOME/.env."""
    env_path = get_athaniel_home() / ".env"
    ...
    # the canonical readers in athaniel_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ATHANIEL_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ATHANIEL_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ATHANIEL_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ATHANIEL_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_athaniel_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.athaniel/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ATHANIEL_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ATHANIEL_PLATFORM`` environment variable
    3. ``ATHANIEL_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from athaniel_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ATHANIEL_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.athaniel/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``athaniel skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile ATHANIEL_HOME; the module-level
    ...
    for scan_dir in dirs_to_scan:
        ...
        for skill_md in _iter:
            ...
            try:
                ...
                seen_names.add(name)
                try:
                    # Face skills deck reads this as a per-skill cost hint
                    # (~bytes/4 tokens to load the SKILL.md).
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from athaniel_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: athaniel plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `athaniel skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.athaniel/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `athaniel skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.athaniel.* first (agentskills.io convention), fall back to top-level
        athaniel_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            athaniel_meta = metadata.get("athaniel", {}) or {}

        tags = _parse_tags(athaniel_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            athaniel_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `athaniel sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
