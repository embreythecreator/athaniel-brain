...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``athaniel tools`` repaint, tool-registration scans,
    ...
    2. ``~/.athaniel/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``athaniel auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_athaniel_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.athaniel/.env`` first, then ``os.environ``.

    Wraps :func:`athaniel_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from athaniel_cli.config import get_env_value as _athaniel_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _athaniel_get_env_value(name)
    return value if value is not None else default


def athaniel_xai_user_agent() -> str:
    """Return a stable Athaniel-specific User-Agent for xAI HTTP calls."""
    try:
        from athaniel_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Athaniel Brain/{__version__}"


def athaniel_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Athaniel Brain,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": athaniel_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Athaniel config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Athaniel home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_athaniel_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``ATHANIEL_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`athaniel_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("ATHANIEL_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Athaniel-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``athaniel_cli.config.get_env_value`` so keys
    stored in ``~/.athaniel/.env`` (the standard Athaniel location) are honored —
    ...
    honors ``ATHANIEL_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
