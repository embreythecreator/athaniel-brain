...
Athaniel Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own ATHANIEL root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in athaniel_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "athaniel" if profile == "default" else profile
    if (
...
def _athaniel_cli() -> str:
    """Resolve the athaniel CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``athaniel``
    entrypoint. A bare ``"athaniel"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("athaniel.exe" if sys.platform == "win32" else "athaniel")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("athaniel")
    if found:
        return found
    return "athaniel"
...
# turns concurrently: deliveries spawn separate ``athaniel`` subprocesses, so
