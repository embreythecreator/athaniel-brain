...
<url>``) is spawned as a direct child of the Athaniel process. Athaniel's own
...
Athaniel process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Athaniel startup's opt-in
