...
``athaniel mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Athaniel desktop app. Use the "
            "terminal instead: `athaniel mcp install <name>` for catalog entries, "
            "`athaniel mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "unanswered, continue without it. Catalog names: `athaniel mcp "
