"""
Lazy dependency installer for opt-in Athaniel Brain backends.

Many Athaniel features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``athaniel-brain[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``athaniel tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ATHANIEL_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/athaniel`` read-only), setting
  ``ATHANIEL_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Athaniel, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey Athaniel") engines ──────────────────────────────────
    ...
    # (ATHANIEL_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (athaniel trace upload / /upload-trace).
    #
    ...
    # presence, the `athaniel update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "ATHANIEL_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ATHANIEL_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ATHANIEL_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``athaniel update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Athaniel under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``athaniel update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="athaniel-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``athaniel_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = athaniel_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $ATHANIEL_HOME/bin is never on PATH, so a bare
        # which() misses the uv Athaniel installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Athaniel
    ...
    # deployment sets ATHANIEL_MANAGED=true *and* ATHANIEL_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from athaniel_cli.config import get_managed_system
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Athaniel cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Athaniel instead)."
            )
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``ATHANIEL_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``ATHANIEL_DISABLE_LAZY_INSTALLS=1``) and sets
      ``ATHANIEL_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``athaniel update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``athaniel update``. Never raises; lazy-install failures
