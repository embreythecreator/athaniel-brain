...
it is automatically downloaded from GitHub releases to $ATHANIEL_HOME/bin/tirith.
...
def _get_athaniel_home() -> str:
    """Return the Athaniel home directory, respecting ATHANIEL_HOME env var."""
    return str(get_athaniel_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_athaniel_home(), ".tirith-install-failed")
...
def _athaniel_bin_dir() -> str:
    """Return $ATHANIEL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_athaniel_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ATHANIEL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_athaniel_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ATHANIEL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ATHANIEL_HOME/bin/tirith
    ...
    athaniel_bin = os.path.join(_athaniel_bin_dir(), "tirith")
    if os.path.isfile(athaniel_bin) and os.access(athaniel_bin, os.X_OK):
        _resolved_path = athaniel_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return athaniel_bin
