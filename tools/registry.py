"""Central registry for all athaniel-brain tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (athaniel_constants itself is stdlib-only, so no cycle).
        from athaniel_constants import get_athaniel_home

        return Path(get_athaniel_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``athaniel tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Athaniel-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("ATHANIEL_SESSION_ID", ""),
            get_session_env("ATHANIEL_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("ATHANIEL_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``athaniel tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved ATHANIEL_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("athaniel_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``athaniel_plugins.{name}``), not the exact module string.
                # A handler defined in ``athaniel_plugins.pkg.handlers`` is
                # still owned by the ``athaniel_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``athaniel tools enable foo``)
