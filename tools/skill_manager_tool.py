...
~/.athaniel/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `athaniel config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.athaniel/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch ATHANIEL_HOME and
    ...
    live profile-scoped ATHANIEL_HOME on every call.
    """
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``athaniel-brain``) are treated as permanently pinned: the system prompt
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to Athaniel (the agent's own "
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`athaniel curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`athaniel curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `athaniel curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `athaniel curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.athaniel/skills/) first, then any
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `athaniel sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `athaniel sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `athaniel sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `athaniel sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Athaniel profiles.
    ...
    # Default profile (~/.athaniel/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.athaniel/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"profiles (`athaniel -p {other_profile}`) or edit the file "
            ...
            base += (
                ...
                f"Switch profiles (`athaniel -p <name>`) to edit there, or "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "to match Athaniel skill standards."
    )
...
def _patch_skill(
    ...
    if not old_string:
        ...
        # how to recover. Upstream: embreythecreator/athaniel-brain#33064.
        return {
            "success": False,
            "error": (
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.athaniel/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `athaniel curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `athaniel curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"{display_athaniel_home()}/skills/; must precede that skill's other "
