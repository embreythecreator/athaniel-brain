...
`athaniel skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.athaniel.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.athaniel.blueprint`` is a mapping containing
    ...
    athaniel = meta.get("athaniel") if isinstance(meta, dict) else None
    blueprint = athaniel.get("blueprint") if isinstance(athaniel, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.athaniel.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.athaniel.blueprint`` block) they can hand
    to ``athaniel skills publish`` to share. ``body`` is the plain-language
