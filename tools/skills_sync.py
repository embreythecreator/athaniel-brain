...
Copies bundled skills from the repo's skills/ directory into ~/.athaniel/skills/
...
The manifest lives at ~/.athaniel/skills/.bundled_manifest.
"""
...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from athaniel_constants import get_bundled_skills_dir, get_athaniel_home, get_optional_skills_dir
...
# cron, kanban workers) import this module once under the launch ATHANIEL_HOME
# and later scope requests to a different profile via
# set_athaniel_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped ATHANIEL_HOME on every call.
_ATHANIEL_HOME_AT_IMPORT = ATHANIEL_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _athaniel_home() -> Path:
    """Return the active profile's ATHANIEL_HOME at call time."""
    configured = Path(ATHANIEL_HOME)
    if configured != _ATHANIEL_HOME_AT_IMPORT:
        return configured
...
# Marker file written by `athaniel profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.athaniel profile).
# When present in ATHANIEL_HOME, sync_skills() is a no-op so neither the
# installer, `athaniel update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# athaniel_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _essential_names() -> frozenset:
    ...
    try:
        ...
        return frozenset({"athaniel-brain"})


def _get_bundled_dir() -> Path:
    """Locate the bundled skills/ directory.

    Checks ATHANIEL_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.athaniel/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.athaniel/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``athaniel skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `athaniel skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.athaniel/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.athaniel) that wrote the
    ...
    # ``athaniel-brain`` skill is the agent's own operating manual and the
    ...
    essential_only = (_athaniel_home() / NO_BUNDLED_SKILLS_MARKER).exists()
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.athaniel/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `athaniel update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `athaniel skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``ATHANIEL_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.athaniel/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``ATHANIEL_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `athaniel skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `athaniel update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``athaniel update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``athaniel update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``athaniel skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `athaniel skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ATHANIEL_HOME/.no-bundled-skills so the
    installer, ``athaniel update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``athaniel skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _athaniel_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _athaniel_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`athaniel skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `athaniel update` (or `athaniel skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_athaniel_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.athaniel/skills/ ...")
