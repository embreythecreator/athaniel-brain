...
# WO-POSTURE/1 4.4 — the fixed contract every deep-research frame returns.
# Enums keep Phase-2 scoring a Python sort/cluster (no aggregator model).
_LEVELS = ("low", "medium", "high")
FRAME_EVIDENCE_MAX = 12


def plan_frame_output_schema(frame: str) -> Dict[str, Any]:
    """JSON Schema for a plan-frame child's final answer.

    ``frame`` is echoed back so the host can reject a child that answered
    under the wrong lens. Bounded ``evidence[]`` of ``{arxiv_id|vault_id, claim}``.
    """
    frame = str(frame or "").strip()
    return {
        "type": "object",
        "additionalProperties": False,
        "required": ["frame", "summary", "relevance", "practicality", "rigor", "evidence"],
        "properties": {
            "frame": {"type": "string", "enum": [frame]},
            "summary": {"type": "string", "minLength": 1, "maxLength": 2000},
            "relevance": {"type": "string", "enum": list(_LEVELS)},
            "practicality": {"type": "string", "enum": list(_LEVELS)},
            "rigor": {"type": "string", "enum": list(_LEVELS)},
            "evidence": {
                "type": "array",
                "maxItems": FRAME_EVIDENCE_MAX,
                "items": {
                    "type": "object",
                    "additionalProperties": False,
                    "required": ["claim"],
                    "anyOf": [{"required": ["arxiv_id"]}, {"required": ["vault_id"]}],
                    "properties": {
