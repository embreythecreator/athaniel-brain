...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `athaniel model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``athaniel auth add
    ...
    2. The environment / ``~/.athaniel/.env``. Under a multiplexed gateway turn
       ...
       ``athaniel_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``athaniel auth
    ...
    when omitted, ``athaniel_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `athaniel auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``athaniel auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `athaniel tools` provider string for a config section.
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via athaniel "
        f"tools), but {failure}. Run 'athaniel tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.athaniel/.env`` (via
    ``athaniel_cli.config.get_env_value`` when available) so tool-side
