...
# Must match athaniel_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def count_active_subagents_for_session(session_id: str) -> int:
    """How many live subagents belong to *session_id* (WO-PLAN/BUSY-1).

    Used by the API server's /plan busy rejection so the operator is told
    WHAT is holding the session instead of a bare "agent is running".
    """
    if not session_id:
        return 0
    with _active_subagents_lock:
        owned = [
            r for r in _active_subagents.values()
            if r.get("owner_session_id") == session_id
        ]
        if owned:
            return len(owned)
        # ponytail: unowned records can't be attributed — report the global
        # count only when nothing at all is owned, so a busy message never
        # says "0 subagents" while avatars are visibly running.
        return 0 if any(
            r.get("owner_session_id") for r in _active_subagents.values()
        ) else len(_active_subagents)
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``athaniel-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``athaniel-cli`` because those also contain
...
def _build_child_agent(
    ...
    role: str = "leaf",
    # WO-POSTURE/1 4.2: a plan-frame child's model is pinned by its council
    # seat. The MoA guard below may still swap provider/base_url/key, never
    # effective_model.
    ...
    # Plan mode admits delegation only for isolated cognitive fan-out (for
    # example the ADHD skill). Children must be pure reasoning processes:
    # no inherited tools, no nested delegation, and therefore no side-effect
    # path around the parent's execution policy.
    try:
        from agent.execution_policy import ExecutionPosture

        parent_policy = getattr(parent_agent, "_execution_policy", None)
        plan_reasoning_only = (
            parent_policy is not None
            and parent_policy.posture is ExecutionPosture.PLAN
        )
    except (AttributeError, ImportError):
        plan_reasoning_only = False
    if plan_reasoning_only:
        effective_role = "leaf"
    ...
    if plan_reasoning_only:
        # 4.3: the toolsets that carry PLAN_CHILD_ALLOWED_TOOLS, bounded by
        # what the parent has. Name-level enforcement (write_file lives in
        # the same "file" toolset as read_file) is the child's own PLAN
        # policy row, written below once its session id exists.
        from agent.execution_policy import PLAN_CHILD_ALLOWED_TOOLS
        import model_tools as _mt

        wanted = {
            ts for name in PLAN_CHILD_ALLOWED_TOOLS
            if (ts := _mt.get_toolset_for_tool(name)) is not None
        }
        child_toolsets = sorted(wanted & _expand_parent_toolsets(parent_toolsets))
    elif toolsets:
        # Intersect with parent — subagent must not gain tools the parent lacks.
        # Expand composite toolsets (e.g. athaniel-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (athaniel-cli,
    # athaniel-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # virtual provider. A child that fans out its own advisor slots is a
    # category violation — children are isolated reasoning branches, advisor
    # fan-out belongs to the parent's turn — and each such child would
    # multiply the reference-slot cost per iteration. Both the inherited and
    # the explicitly-overridden provider paths land here; the child is
    # retargeted onto the MoA aggregator's real model instead.
    if (effective_provider or "").strip().lower() == "moa":
        _moa_rt = _resolve_moa_child_runtime(parent_agent)
        if not model_pin and (
            not model
            or str(model).strip().lower().startswith("moa")
            or model == getattr(parent_agent, "model", None)
        ):
            effective_model = _moa_rt["model"]
        effective_provider = _moa_rt["provider"]
        effective_base_url = _moa_rt["base_url"]
        effective_api_key = _moa_rt["api_key"]
        if override_api_mode is None:
            override_api_mode = _moa_rt["api_mode"]
        logger.info(
            "Delegate child retargeted off the MoA virtual provider onto "
            "aggregator %s:%s",
            effective_provider,
            effective_model,
        )
    ...
    # child Athaniel/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    elif _effective_provider_norm in {"nous", "nous-portal", "embreythecreator"}:
    ...
    if plan_reasoning_only:
        # 4.3 / D-7: the conversation loop reloads the policy from the store
        # every turn by session id, so an attribute on the child would be
        # overwritten — persist a PLAN(child) row for the child's session.
        try:
            from agent.execution_policy import (
                ExecutionPolicy, ExecutionPolicyStore, PlanModeState,
            )
            ExecutionPolicyStore(
                getattr(child, "_session_db", None) or parent_session_db
            ).save(
                child_session_ref["session_id"],
                ExecutionPolicy(
                    state=PlanModeState.PLANNING,
                    task=getattr(parent_policy, "task", "") or "",
                    plan_child=True,
                ),
            )
        except Exception as _exc:
            logger.warning("plan-child policy not persisted (%s); child keeps no tools", _exc)
    ...
    try:
        from athaniel_cli.lifecycle import invoke_hook as _invoke_hook
        _invoke_hook(
            "subagent_start",
            parent_session_id=getattr(parent_agent, "session_id", None),
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.athaniel/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_athaniel_dir("cache/delegation", "delegation_cache")
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("ATHANIEL_UI_SESSION_ID", "") or None
            except Exception:
    ...
    try:
        ...
        # WO-PLAN/BUSY-1: a plan-frame child got a PLANNING(plan_child) row
        # at spawn (~L1955). Finish it now or it leaks forever (29 stale rows
        # found in the 2026-08-31 audit).
        try:
            _child_sid = getattr(child, "session_id", "") or ""
            if _child_sid:
                from agent.execution_policy import (
                    ExecutionPolicyStore, PlanModeState,
                )
                _store = ExecutionPolicyStore(
                    getattr(child, "_session_db", None)
                    or getattr(parent_agent, "_session_db", None)
                )
                if getattr(_store.load(_child_sid), "plan_child", False):
                    _store.finish(_child_sid)
        except Exception as _cleanup_exc:
            logger.debug("plan-child policy cleanup skipped: %s", _cleanup_exc)
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from athaniel_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # WO-POSTURE/1 4.2: per-task plan frame → pinned council seat + frame
    # output contract. None for frameless tasks (hoisted creds apply).
    task_frame_creds: List[Optional[Dict[str, Any]]] = []
    for i, task in enumerate(task_list):
        ...
        frame = str(task.get("frame") or "").strip()
        frame_creds = None
        if frame:
            try:
                frame_creds = _resolve_frame_credentials(frame)
            except (KeyError, ValueError) as exc:
                return tool_error(f"Task {i} frame invalid: {exc}")
            if raw_schema is None:
                from tools.delegation_output_schema import plan_frame_output_schema

                raw_schema = plan_frame_output_schema(frame)
        task_frame_creds.append(frame_creds)
    ...
    # ATHANIEL_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Athaniel-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("ATHANIEL_UI_SESSION_ID", "")
    except Exception:
    ...
    for i, t in enumerate(task_list):
        ...
        # 4.2: a frame's seat replaces the hoisted creds for this task only.
        _task_creds = (task_frame_creds[i] if i < len(task_frame_creds) else None) or creds
        ...
        try:
            child = _build_child_preserving_parent_tools(
                ...
                role=effective_role,
                model_pin=_task_creds is not creds,
            )
        except ValueError as exc:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Athaniel-Session-Id once the batch completes.
            ...
            # ATHANIEL_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `athaniel -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("ATHANIEL_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("ATHANIEL_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ATHANIEL_SESSION_KEY is not in the CLI
...
def _resolve_moa_child_runtime(parent_agent) -> dict:
    """Resolve the real credential bundle behind a MoA parent's aggregator slot.

    Delegate children must never run the `moa` virtual provider (WO-MOA/2
    universal invariant — see the guard in ``_build_child_agent``). The MoA
    parent's ``model`` attribute is a preset name, and its base_url is the
    fake ``moa://local``, so nothing about the parent's own credentials is
    usable by a child. Instead: resolve the parent's active MoA preset, take
    the aggregator slot's provider:model, and resolve full credentials via
    the runtime provider system — the same path delegation.provider uses.

    Raises ValueError when the aggregator provider cannot be resolved; a MoA
    session whose aggregator credentials are broken cannot run MoA turns
    either, so failing loudly here reports an already-broken session rather
    than silently spawning a MoA-fanout child.
    """
    from athaniel_cli.config import load_config
    from athaniel_cli.moa_config import resolve_moa_preset
    from athaniel_cli.runtime_provider import resolve_runtime_provider

    moa_cfg = (load_config() or {}).get("moa")
    preset_name = str(getattr(parent_agent, "model", "") or "").strip() or None
    try:
        preset = resolve_moa_preset(moa_cfg, preset_name)
    except Exception:
        # Parent's model attr wasn't a known preset name — fall back to the
        # configured default preset rather than refusing to spawn.
        preset = resolve_moa_preset(moa_cfg, None)
    ...
    try:
        ...
        raise ValueError(
            "Delegate children cannot run the MoA virtual provider, and the "
            f"MoA aggregator's provider '{agg_provider}' failed to resolve: "
            f"{exc}"
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            "Delegate children cannot run the MoA virtual provider, and the "
            f"MoA aggregator provider '{agg_provider}' resolved with no API "
            "key. Set its credentials or run 'athaniel auth'."
        )
    resolved_provider = (
        agg_provider
        if runtime.get("provider") == _RUNTIME_PROVIDER_CUSTOM
        else (runtime.get("provider") or agg_provider)
    )
    return {
        "provider": resolved_provider,
        "model": agg_model or runtime.get("model") or "",
...
def _resolve_frame_credentials(frame: str) -> dict:
    """WO-POSTURE/1 4.2 — council seat for a plan frame, as a delegation
    credential bundle (same shape as _resolve_delegation_credentials).

    KeyError on an unknown frame (from resolve_plan_frame_slot); ValueError
    when the seat's provider resolves without a key.
    """
    from athaniel_cli.model_council import resolve_plan_frame_slot
    from athaniel_cli.runtime_provider import resolve_runtime_provider

    slot = resolve_plan_frame_slot(frame)
    runtime = resolve_runtime_provider(requested=slot["provider"], target_model=slot["model"])
    api_key = runtime.get("api_key", "")
    if not api_key:
        raise ValueError(
            f"plan frame {frame!r} seat {slot['provider']}:{slot['model']} resolved with no API key"
        )
    provider = (
        slot["provider"]
        if runtime.get("provider") == _RUNTIME_PROVIDER_CUSTOM
        else (runtime.get("provider") or slot["provider"])
    )
    return {
        "model": slot["model"],
        "provider": provider,
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'athaniel auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Athaniel config.

    Prefer the shared persistent loader because it follows the active
    ATHANIEL_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ATHANIEL_IGNORE_USER_CONFIG=1`` (``athaniel chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ATHANIEL_IGNORE_USER_CONFIG") == "1"
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "frame": {
                            ...
                            "description": (
                                "Deep-research lens (plan mode). Pins this task "
                                "to the frame's council seat (model cannot be "
                                "swapped) and, unless output_schema is given, "
                                "requires the frame's evidence contract."
                            ),
                        },
                        "output_schema": {
                            "type": "object",
