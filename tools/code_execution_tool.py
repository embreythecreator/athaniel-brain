...
Lets the LLM write a Python script that calls Athaniel tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `athaniel_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `athaniel_tools.py` with file-based RPC stubs
...
# available on every platform Athaniel itself runs on.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_athaniel_dir("cache/exec", "exec_spill")
...
# match a safe prefix, the operational ATHANIEL_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Athaniel
# code can lose the DB-layer Kanban mutation guard while still inheriting
# ATHANIEL_HOME.
#
# NB: the broad "ATHANIEL_" prefix was deliberately removed (#27303) — it leaked
# ATHANIEL_*-named config that lacks a secret substring (e.g. ATHANIEL_BASE_URL,
# ATHANIEL_KANBAN_DB, ATHANIEL_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ATHANIEL_CHILD_ALLOWED below; ATHANIEL_RPC_SOCKET /
# ATHANIEL_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ATHANIEL_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set athaniel_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_ATHANIEL_CHILD_ALLOWED = frozenset({
    "ATHANIEL_HOME",
    "ATHANIEL_PROFILE",
    "ATHANIEL_CONFIG",
    "ATHANIEL_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ATHANIEL_* vars (_ATHANIEL_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ATHANIEL_* vars dropped by the tightened allowlist (#27303). The
    # broad "ATHANIEL_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ATHANIEL_KANBAN_DB / ATHANIEL_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _ATHANIEL_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ATHANIEL_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ATHANIEL_* var.
            _dropped_athaniel.append(k)
    if _dropped_athaniel:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ATHANIEL_* var(s) from the "
            ...
            len(_dropped_athaniel),
            ", ".join(sorted(_dropped_athaniel)),
        )
...
# ---------------------------------------------------------------------------
# athaniel_tools.py code generator
# ---------------------------------------------------------------------------
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    athaniel_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'athaniel_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_athaniel_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the athaniel_tools.py stub module.
...
"""Auto-generated Athaniel tools RPC stubs."""
...
def _connect():
    ...
    ATHANIEL_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ATHANIEL_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ATHANIEL_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (ATHANIEL_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("ATHANIEL_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Athaniel tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ATHANIEL_RPC_DIR") or os.path.join(tempfile.gettempdir(), "athaniel_rpc")
_seq = 0
...
def _execute_remote(
    ...
    sandbox_dir = f"{temp_dir}/athaniel_exec_{sandbox_id}"
    ...
    try:
        ...
        # --- Session-kernel path (athaniel-brain#96873) -------------------
        ...
        tools_src = generate_athaniel_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/athaniel_tools.py", tools_src)
        ...
        env_prefix = (
            f"ATHANIEL_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"ATHANIEL_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ATHANIEL_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["ATHANIEL_RPC_SOCKET"] = rpc_endpoint
    child_env["ATHANIEL_RPC_TOKEN"] = rpc_token
    ...
    # ATHANIEL_TIMEZONE is an internal Athaniel setting and must not leak
    # into child processes.
    _tz_name = os.getenv("ATHANIEL_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("ATHANIEL_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``athaniel_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Athaniel's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Athaniel's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Athaniel-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Athaniel-
    ...
    _strip_athaniel_owned_pythonpath(child_env)
    _athaniel_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_athaniel_python_environment(child_python):
        _pp_parts.append(_athaniel_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import athaniel_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Athaniel "
            "environment; athaniel root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.athaniel.gateway`, but the
    ...
    # --- Set up temp directory with athaniel_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="athaniel_sandbox_")
    ...
    # selector from ATHANIEL_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"athaniel_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated athaniel_tools module.
        ...
        tools_src = generate_athaniel_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "athaniel_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ATHANIEL_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.athaniel/.env')).
...
def _load_config() -> dict:
    ...
    ``athaniel --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (athaniel-brain's python). Reproducible and the
...
# Interpreter paths already reported as outside the Athaniel environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the athaniel root from every
...
def _uses_athaniel_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Athaniel's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``athaniel tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and athaniel-brain's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.athaniel/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "Athaniel's own python (the common case — stdlib plus Athaniel's "
    ...
    description = (
        "Run Python that calls Athaniel tools programmatically. Use when you "
        ...
        "loses that state.\n\n"
        f"Available via `from athaniel_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from athaniel_tools import {import_str}` "
