...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``athaniel_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from athaniel_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where athaniel previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_athaniel_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Athaniel.
...
_athaniel_config_resolved: str | None = None
_athaniel_config_resolved_loaded = False


def _get_athaniel_config_resolved() -> str | None:
    """Return the resolved absolute path of the Athaniel config file (cached)."""
    global _athaniel_config_resolved, _athaniel_config_resolved_loaded
    if _athaniel_config_resolved_loaded:
        return _athaniel_config_resolved
    _athaniel_config_resolved_loaded = True
    try:
        from athaniel_cli.config import get_config_path
        _athaniel_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _athaniel_config_resolved = str(Path(_expand_tilde("~/.athaniel/config.yaml")).resolve())
        except Exception:
            _athaniel_config_resolved = None
    return _athaniel_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Athaniel config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    athaniel_config = _get_athaniel_config_resolved()
    if athaniel_config and (resolved == athaniel_config or normalized == athaniel_config):
        return (
            f"Refusing to write to Athaniel config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.athaniel/config.yaml directly or use 'athaniel config' instead."
        )
    return None
...
# .cursorrules (or a project-local .athaniel config tree) outlives the current
...
_real_athaniel_home_cached: str | None = None
_real_athaniel_home_loaded = False


def _get_real_athaniel_home() -> str | None:
    """Return the realpath of the authoritative Athaniel home (cached)."""
    global _real_athaniel_home_cached, _real_athaniel_home_loaded
    if _real_athaniel_home_loaded:
        return _real_athaniel_home_cached
    _real_athaniel_home_loaded = True
    try:
        from athaniel_constants import get_athaniel_home
        _real_athaniel_home_cached = os.path.realpath(str(get_athaniel_home()))
    except Exception:
        try:
            _real_athaniel_home_cached = os.path.realpath(_expand_tilde("~/.athaniel"))
        except Exception:
            _real_athaniel_home_cached = None
    return _real_athaniel_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from athaniel_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.athaniel home is governed by its own guards
    ...
    # ``.athaniel`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_athaniel_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .athaniel config dirs (e.g. <repo>/.athaniel/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.athaniel`` — matching
        # any ancestor named .athaniel would gate every write inside a
        # checkout that happens to live under ~/.athaniel (e.g. the
        # athaniel-brain repo itself at ~/.athaniel/athaniel-brain).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".athaniel":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Athaniel mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Athaniel state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Athaniel process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside Athaniel scope.
    """
    try:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Athaniel internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under ATHANIEL_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ATHANIEL_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with athaniel_tools.write_file() for very "
            "large files."
        )
