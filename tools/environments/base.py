"""Base class for all Athaniel execution environment backends.
...
# ATHANIEL_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ATHANIEL_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ATHANIEL_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_athaniel_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._athaniel_stdin_errors`` while stdin is still closed
    ...
    proc._athaniel_stdin_errors = errors
    ...
    proc._athaniel_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__ATHANIEL_CWD_{session_id}__"
...
# the FIRST session's ATHANIEL_SESSION_ID into the snapshot makes every LATER
...
# ATHANIEL_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Athaniel' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is ATHANIEL_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ATHANIEL_SESSION_|ATHANIEL_UI_SESSION_ID|ATHANIEL_CRON_AUTO_DELIVER_|"
    "ATHANIEL_CRON_SESSION|ATHANIEL_BROWSER_CONTROL_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ATHANIEL_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ATHANIEL_SESSION_*} ${!ATHANIEL_CRON_AUTO_DELIVER_*} "
        "${!ATHANIEL_BROWSER_CONTROL_*} "
        # AI_AGENT / ATHANIEL_AGENT are per-command attribution markers
        ...
        "AI_AGENT ATHANIEL_AGENT "
        f"ATHANIEL_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Athaniel backends.
    ...
    # True only when commands execute on the SAME host as the Athaniel process
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/athaniel-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/athaniel-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__athaniel_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__athaniel_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__athaniel_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__athaniel_fns\" ] && declare -f $__athaniel_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_ATHANIEL_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under Athaniel via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Athaniel-specific
        # ``ATHANIEL_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``athaniel-brain`` — see huggingface.js
        ...
        # inherited from the Athaniel process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Athaniel running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-athaniel-brain}" '
            'ATHANIEL_AGENT="${ATHANIEL_AGENT:-true}"'
        )
        ...
        parts.append("__athaniel_ec=$?")
        # Restrict Athaniel metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__athaniel_snap_tmp=$(mktemp {_snap_tmp_template}) && "
        ...
        parts.append("exit $__athaniel_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ATHANIEL_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        *,
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_athaniel_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via ATHANIEL_DEBUG_INTERRUPT=1) -------------
        ...
        stdin_thread = getattr(proc, "_athaniel_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_athaniel_stdin_errors", None)
        if stdin_errors:
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ATHANIEL_CWD_{session}__ marker from stdout output.
