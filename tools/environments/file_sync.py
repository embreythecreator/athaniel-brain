...
_FORCE_SYNC_ENV = "ATHANIEL_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.athaniel") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.athaniel to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.athaniel", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ATHANIEL_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, athaniel_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.athaniel/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(athaniel_home=athaniel_home)

    def _sync_back_transaction(self, athaniel_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .athaniel/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (athaniel_home or get_athaniel_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="athaniel-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.athaniel/skills/a.md`` →
        ``~/.athaniel/skills/a.md``, a new remote file at
        ``/root/.athaniel/skills/b.md`` maps to ``~/.athaniel/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
