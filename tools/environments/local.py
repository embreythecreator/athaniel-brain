...
from athaniel_constants import get_process_athaniel_home
...
#
# get_temp_dir() now defaults to ATHANIEL_HOME/cache/terminal (real storage)
...
#
# Background-process artifacts come in triplets (athaniel_bg_<id>.log/.pid/
...
_BG_GROUP_RE = re.compile(r"^(athaniel_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return ATHANIEL_HOME/cache/terminal, or None if unresolvable."""
    try:
        from athaniel_constants import get_athaniel_home
        return get_athaniel_home() / "cache" / "terminal"
    except Exception:
        return None
...
def cleanup_terminal_temp_cache(
    ...
    Only prunes the managed default dir (``ATHANIEL_HOME/cache/terminal``).
    ...
    # Newest mtime per athaniel_bg_<id> group, so a live server's fresh .log
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Athaniel was already launched from (for example ``athaniel-brain``
    while the process cwd is ``~/.athaniel/athaniel-brain``), passing it through
    unchanged makes the wrapper run ``cd athaniel-brain`` *inside* the project
    ...
    # Common recovery for config values like ``athaniel-brain`` when Athaniel was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./athaniel-brain``; use the current directory instead.
...
# Athaniel-internal env vars that should NOT leak into terminal subprocesses.
_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX = "_ATHANIEL_FORCE_"

# Athaniel-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Athaniel inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from athaniel_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "ATHANIEL_DASHBOARD_SESSION_TOKEN",
    ...
    # Athaniel-managed inference credential — Claude subscription auth is not a
    # working Athaniel provider path.  Stripping it broke agent-spawned
    ...
    # the :func:`athaniel_subprocess_env` Tier-2 strip (browser / TUI host /
...
_ATHANIEL_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# ``buzz`` (the gateway's ``ATHANIEL_SESSION_PLATFORM`` ContextVar; concurrency
...
# :func:`athaniel_subprocess_env` (browser / TUI host / copilot-executor
...
# from the blocklist entirely because it is NOT a Athaniel credential; these ARE
# Athaniel-managed first-party platform credentials, so they stay IN the
...
def _buzz_terminal_context_active() -> bool:
    ...
    * ``BUZZ_MANAGED_AGENT`` in the process env — set exclusively by Buzz
      Desktop's buzz-acp harness when it spawns ``athaniel acp`` (#76243).
      Gateway / CLI / cron / kanban processes never carry it.
    * The live session's platform is ``buzz`` — the gateway's
      ``ATHANIEL_SESSION_PLATFORM`` ContextVar via
    ...
    try:
        ...
        return get_session_env("ATHANIEL_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# project's dependencies into the Athaniel venv path instead of the project's own
# ``.venv`` — silently clobbering the Athaniel environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Athaniel venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Athaniel venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Athaniel itself treats PYTHONHOME as contamination in its own
...
# _strip_athaniel_owned_pythonpath() which removes only Athaniel-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_athaniel_internal_secret(key: str) -> bool:
    """Return True for Athaniel-internal secrets injected under *dynamic* names.

    ``_ATHANIEL_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Athaniel-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`athaniel_subprocess_env` helper all call it, so the
...
def _inject_context_athaniel_home(env: dict) -> None:
    """Bridge the context-local Athaniel home override into subprocess env."""
    try:
        from athaniel_constants import get_athaniel_home_override

        value = get_athaniel_home_override()
        if value:
            env["ATHANIEL_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (ATHANIEL_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Athaniel-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_athaniel_internal_secret(key):
            continue
        if key in _plugin_strip:
            continue
        first_party = _is_terminal_first_party_env(key)
        passthrough = _is_passthrough(key)
        if key in _ATHANIEL_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_athaniel_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_athaniel_internal_secret(key):
            continue
    ...
    _inject_context_athaniel_home(sanitized)

    from athaniel_constants import apply_subprocess_home_env
    ...
    # the separate Athaniel runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_athaniel_owned_pythonpath_and_runtime_markers(sanitized)

    # Keep bare ``athaniel`` invocations available to child jobs even when the
    ...
    if path_key is not None:
        sanitized[path_key] = _prepend_athaniel_bin_dir(sanitized.get(path_key, ""))
...
# legitimate child Athaniel spawns needs them, and they are the highest-value
...
# narrow subset of _ATHANIEL_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in athaniel_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_athaniel_internal_secret, but _ID has no secret suffix, so it must be
...
def athaniel_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_ATHANIEL_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Athaniel spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_ATHANIEL_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and Athaniel-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_athaniel_internal_secret`.
    for key in list(env):
        if key.startswith(_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_athaniel_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _ATHANIEL_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_athaniel_home(env)
    from athaniel_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_athaniel_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose ATHANIEL_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's ATHANIEL_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`athaniel_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``ATHANIEL_HOME`` / subprocess ``HOME`` contract) and the
    Athaniel secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_athaniel_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``ATHANIEL_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Athaniel home override into ``ATHANIEL_HOME`` and apply the
      subprocess HOME contract (``athaniel_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``ATHANIEL_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs ATHANIEL_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ATHANIEL_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale ATHANIEL_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\athaniel\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\athaniel\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _athaniel_portable_git = os.path.join(_local_appdata, "athaniel", "git") if _local_appdata else ""
    if _athaniel_portable_git:
        for candidate in (
            os.path.join(_athaniel_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_athaniel_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # ATHANIEL_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\athaniel\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "ATHANIEL_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Athaniel Brain requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ATHANIEL_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Athaniel. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``athaniel`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_ATHANIEL_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_athaniel_bin_dir() -> str | None:
    """Return the directory holding the ``athaniel`` console-script, or None.
    ...
    athaniel install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``athaniel``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``athaniel`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``athaniel`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("athaniel")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``athaniel`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _ATHANIEL_BIN_DIR
    if _ATHANIEL_BIN_DIR is not _SENTINEL:
        return _ATHANIEL_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("athaniel")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "athaniel" or base.startswith("athaniel."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "athaniel.exe" if _IS_WINDOWS else "athaniel"
    ...
    _ATHANIEL_BIN_DIR = candidate
    return candidate


def _prepend_athaniel_bin_dir(existing_path: str) -> str:
    """Prepend the athaniel install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_athaniel_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Athaniel-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Athaniel installs for
    itself, so on a machine where Athaniel provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$ATHANIEL_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$ATHANIEL_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_athaniel_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_athaniel_managed_node``, a first browser install).
    """
    try:
        from athaniel_constants import get_athaniel_home, iter_athaniel_node_dirs

        candidates = [*iter_athaniel_node_dirs(), get_athaniel_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Athaniel-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Athaniel
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_athaniel_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_athaniel_internal_secret(k):
            continue
        else:
            first_party = _is_terminal_first_party_env(k)
            passthrough = _is_passthrough(k)
            if k in _ATHANIEL_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the athaniel install dir is reachable so plugins can shell out
        # to bare ``athaniel`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_athaniel_bin_dir(new_path)

    _inject_context_athaniel_home(run_env)
    ...
    _strip_athaniel_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_athaniel_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by Athaniel launchers.

    ``gateway_windows._preserve_athaniel_home_path`` maps a physical path under
    the resolved ATHANIEL_HOME back onto the configured ATHANIEL_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of ATHANIEL_HOME as Athaniel-owned.
    ...
    # lexically the same way get_default_athaniel_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of ATHANIEL_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\athaniel\athaniel-brain
    # -> C:\...\athaniel-brain) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Athaniel-owned.  Fail-closed: a
...
# --- Athaniel venv / repo-root detection (module-level, computed once) ---

#: The Athaniel repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import athaniel_cli``,
#: etc.  Subprocesses that are NOT the Athaniel backend don't need it and it
#: can shadow local packages.
_athaniel_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Athaniel launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Athaniel-owned paths under
#: the configured ATHANIEL_HOME spelling (which may be a junction to another
#: drive — see ``athaniel_cli/gateway_windows.py::_preserve_athaniel_home_path``).
...
_athaniel_repo_root_aliases: tuple[Path, ...] = _build_athaniel_repo_root_aliases(
    _athaniel_repo_root,
...
#: subprocess spawn.
_athaniel_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Athaniel repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _athaniel_repo_root_aliases):
        return None
...
def _get_athaniel_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Athaniel runtime.
    ...
    """
    global _athaniel_site_packages
    if _athaniel_site_packages is not None:
        result = list(_athaniel_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _athaniel_site_packages = list(result)
...
def _strip_athaniel_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Athaniel-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_athaniel_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_athaniel_owned_pythonpath(env: dict) -> None:
    """Remove Athaniel-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Athaniel repo root and the Athaniel venv's
    ...
    user entries, so only entries proven Athaniel-owned are removed:
    ...
    athaniel_site_packages = _get_athaniel_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Athaniel-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Athaniel venv site-packages ---
        ...
        for sp in athaniel_site_packages:
        ...
        # resolved and unresolved (ATHANIEL_HOME/junction) spellings count as
        # Athaniel-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _athaniel_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped Athaniel-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Athaniel trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Athaniel host itself — controller-side platform
    ...
    def _additional_profile_scoped_passthrough_names(self) -> tuple[str, ...]:
        ...
        ``athaniel-snap-<id>.sh`` and a later command from profile B sharing
    ...
    def get_temp_dir(self) -> str:
        ...
        **Default (no override set):** a dedicated cache dir under
        ``ATHANIEL_HOME`` (``~/.athaniel/cache/terminal``) rather than ``/tmp``.
        On several distros (Arch and friends) ``/tmp`` is a small RAM-backed
        tmpfs, and Athaniel session artifacts — background-process logs,
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ATHANIEL_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ATHANIEL_HOME.  Using
            ...
            try:
                from athaniel_constants import get_athaniel_home
                cache_dir = get_athaniel_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "athaniel_terminal"
        ...
        # Honored ahead of the generic TMPDIR so users can redirect Athaniel' temp
        ...
        # Default: ATHANIEL_HOME/cache/terminal — real storage, mirroring the
        # Windows branch above. /tmp is only a last-resort fallback now
        # because RAM-backed tmpfs /tmp fills up under Athaniel load.
        try:
            from athaniel_constants import get_athaniel_home
            cache_dir = get_athaniel_home() / "cache" / "terminal"
            cache_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._athaniel_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_athaniel_pgid", None)
                    if pgid is None:
                        raise
