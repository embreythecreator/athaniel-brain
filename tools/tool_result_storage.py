...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$ATHANIEL_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   Athaniel-owned caches (images, audio, documents, ...) instead of littering
...
STORAGE_DIR = "/tmp/athaniel-results"
SPILLOVER_SUBDIR = "cache/spillover"
SPILLOVER_MAX_AGE_HOURS = 24
HEREDOC_MARKER = "ATHANIEL_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $ATHANIEL_HOME/cache/spillover as a Path (not created)."""
    from athaniel_constants import get_athaniel_home

    return get_athaniel_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $ATHANIEL_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/athaniel-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ATHANIEL_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $ATHANIEL_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other Athaniel-owned
