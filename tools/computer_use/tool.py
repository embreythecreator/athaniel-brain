...
`health_report` (surfaced by `athaniel computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Athaniel runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Athaniel-session cached backends. Each backend owns its own cua-driver
...
# See embreythecreator/athaniel-brain#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Athaniel's explicit approval bypass onto Cua's immutable mode.

    Athaniel has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "ATHANIEL_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown ATHANIEL_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Athaniel YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Athaniel sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Athaniel process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `athaniel computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (embreythecreator/athaniel-brain#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of embreythecreator/athaniel-brain#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$ATHANIEL_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_athaniel_dir("cache/vision", "temp_vision_images")
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Athaniel' media cache and return its absolute path.
    ...
    try:
        ...
        cache_dir = get_athaniel_dir("cache/images", "image_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
        try:
            captures = sorted(
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_athaniel_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `athaniel computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
