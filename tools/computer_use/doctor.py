"""
`athaniel computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Athaniel side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Athaniel provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Athaniel-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `ATHANIEL_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Athaniel identity under athaniel_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["athaniel_identity"] = identity
