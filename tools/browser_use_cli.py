...
_PRIVATE_BROWSER_SENTINEL = "_ATHANIEL_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# athaniel: pin this named session to its own tab (once per daemon process)
def _athaniel_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "athaniel-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_athaniel_ensure_own_tab()
del _athaniel_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Athaniel's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Athaniel's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_athaniel_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `athaniel tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Athaniel' own bin dir ($ATHANIEL_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from athaniel_constants import get_athaniel_home

        return str(Path(get_athaniel_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Athaniel' own ``$ATHANIEL_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Athaniel-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Athaniel' managed uv (bootstrapped on demand via
    ``athaniel_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$ATHANIEL_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Athaniel-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through athaniel tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_athaniel_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_lightpanda_cdp(
    ...
    """Point the harness at a Athaniel-spawned ``lightpanda serve``.
    ...
    try:
        ...
        return (
            ...
            "to use local Chrome, or switch backends via `athaniel tools` → "
            "Browser Automation."
        )
...
def _resolve_backend_cdp(
    ...
    4. ``browser.engine: lightpanda``: a Athaniel-spawned ``lightpanda serve``
    ...
    try:
        ...
        return (
            ...
            "switch backends via `athaniel tools` → Browser Automation."
        )
    cdp = str((session_info or {}).get("cdp_url") or "")
    if not cdp:
        return (
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Athaniel launches on a
    SNAPSHOT of their real profile (see athaniel_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Athaniel-managed copy of "
