...
from athaniel_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Athaniel secret
...
def _build_browser_env() -> dict:
    ...
    Strips Athaniel-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``athaniel_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = athaniel_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    athaniel_node_bin = str(athaniel_home / "node" / "bin")
    athaniel_node_root = str(athaniel_home / "node")
    athaniel_nm_bin = str(athaniel_home / "node_modules" / ".bin")
    return [athaniel_node_bin, athaniel_node_root, athaniel_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``athaniel doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.athaniel/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Athaniel profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.athaniel/plugins/browser/<vendor>/``) participate
...
from athaniel_constants import is_termux as _is_termux_environment
...
# comparison sites (four here, plus athaniel_cli/tools_config.py and
# athaniel_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    if bu_mode:
        return True, "Browser Use mode: Athaniel spawns `lightpanda serve` per session"
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Athaniel silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Athaniel-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Athaniel CLI where sys.stdout and sys.stderr both report
...
_REAL_PROFILE_SESSION = "athaniel-real-profile"
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from athaniel_cli.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a athaniel-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from athaniel_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous athaniel process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from athaniel_cli.browser_connect import _PROFILE_LOCKED_PREFIX
            ...
            if err and err.startswith(_PROFILE_LOCKED_PREFIX):
                ...
                return None, (
                    ...
                    "`athaniel browser close-profile`, then retry."
                )
        ...
        from athaniel_cli.browser_connect import chromium_executable
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-athaniel_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a athaniel
...
# Hard ceiling for a daemon whose owning athaniel process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed athaniel processes — this way every clean athaniel exit sweeps
    ...
    # Sweep orphans from other crashed athaniel processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live athaniel processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current athaniel PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live athaniel process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    athaniel restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning athaniel process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced athaniel PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent athaniel instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-athaniel_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live athaniel process and must not be touched — but "owner alive"
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a athaniel-owned SNAPSHOT of their real profile —
    ...
    # ``browser.engine: lightpanda`` that endpoint is a Athaniel-spawned
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Athaniel-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Athaniel-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_athaniel_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Athaniel-managed
    ...
    # node_modules binary, which disappears on the next ``athaniel update`` and
    ...
    # Build an extended search PATH including Athaniel-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_athaniel_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_athaniel_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_athaniel_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``athaniel update`` (or ``athaniel doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Athaniel holds, while running registry-fetched
    npm code on every ``athaniel update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `athaniel update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this athaniel PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_athaniel_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = athaniel_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Athaniel
    ...
    from athaniel_constants import get_athaniel_dir
    screenshots_dir = get_athaniel_dir("cache/screenshots", "browser_screenshots")
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if session_info:
        ...
        # A Lightpanda session is a process Athaniel spawned itself (Browser
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/embreythecreator/athaniel-brain:latest")
                else:
