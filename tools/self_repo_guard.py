...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Athaniel's live source checkout "
        ...
        "checkout, stop Athaniel, run the command externally, then restart "
        "Athaniel."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(athaniel_home).expanduser() if athaniel_home else Path.home() / ".athaniel"
