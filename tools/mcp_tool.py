...
transport, discovers their tools, and registers them into the athaniel-brain
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.athaniel/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.athaniel/logs/mcp-stderr.log), tagged with the
...
def sdk_httpx():
    ...
    side by side with Athaniel' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Athaniel' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Athaniel explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Athaniel and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            athaniel_home = os.path.expanduser(
                os.getenv(
                    "ATHANIEL_HOME", os.path.join(os.path.expanduser("~"), ".athaniel")
                )
            )
            candidates = [
                os.path.join(athaniel_home, "node", "bin", resolved_command),
                ...
                # image (which the Athaniel Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Athaniel MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Athaniel gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Athaniel document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`athaniel_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Athaniel profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Athaniel' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (ATHANIEL_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit ATHANIEL_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Athaniel' logging (agent.log via athaniel_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `athaniel setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Athaniel process (kill -9, crash, force-quit)
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from athaniel_cli.process_identity import register_child
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"athaniel-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Athaniel' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Athaniel is fully
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``athaniel mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Athaniel' pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `athaniel mcp login {server_name}` (or delete the tokens "
        f"file under ~/.athaniel/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Athaniel processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_athaniel_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ATHANIEL_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_athaniel_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_athaniel_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ATHANIEL_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_athaniel_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Athaniel config file.
    ...
    ``os.environ`` (which includes ``~/.athaniel/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Athaniel' MEDIA: tag convention and out to messaging
            ...
            # Athaniel' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Athaniel's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Athaniel uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Athaniel registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``athaniel tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Athaniel registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple athaniel
