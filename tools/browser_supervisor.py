...
One ``CDPSupervisor`` runs per Athaniel ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Athaniel logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "athaniel-dialog-bridge.invalid"
...
(() => {
  if (window.__athanielDialogBridgeInstalled) return;
  window.__athanielDialogBridgeInstalled = true;
  const ENDPOINT = "http://athaniel-dialog-bridge.invalid/";
