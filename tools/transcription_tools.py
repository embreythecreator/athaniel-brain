...
LOCAL_STT_COMMAND_ENV = "ATHANIEL_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ATHANIEL_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via athaniel_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``ATHANIEL_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the Athaniel process user cannot "
            "write to the virtual environment. Try running manually as the "
            "venv owner: `stat -c '%%u' '$(dirname $(dirname $(which python3)))'` "
            "then `su - <owner> -c 'VIRTUAL_ENV=/opt/athaniel/.venv "
...
#
# The single-env-var ``ATHANIEL_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Athaniel's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__ATHANIEL_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"athaniel-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ATHANIEL_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `athaniel plugins list` to see "
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from athaniel_cli.plugins import has_hook, invoke_hook
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="athaniel-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="athaniel-stt-idle-unload", daemon=True
        )
...
def _transcribe_local_command(
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="athaniel-local-stt-") as output_dir:
            ...
            # Scrub Athaniel secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from tools.environments.local import athaniel_subprocess_env

            child_env = athaniel_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``ATHANIEL_LOCAL_STT_LANGUAGE`` (env).
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="athaniel-stt-") as work_dir:
                try:
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `athaniel model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``athaniel_cli.models`` helpers so every DeepInfra surface resolves the
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="athaniel-stt-trim-")
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `athaniel tools` remediation —
