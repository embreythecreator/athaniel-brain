...
409), and is driven by:
  ...
  * the ``athaniel sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``athaniel_cli/``) so the
...
is toggled via ``athaniel sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.athaniel/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``athaniel sync enable`` — it is
...
# namespace from athaniel-brain's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the ATHANIEL_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: ATHANIEL_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("ATHANIEL_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Athaniel Cloud instance can be set
...
# precedence as base_url: the ATHANIEL_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   ATHANIEL_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ATHANIEL_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ATHANIEL_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``ATHANIEL_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Athaniel Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("ATHANIEL_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``ATHANIEL_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``athaniel sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "ATHANIEL_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ATHANIEL_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``athaniel sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Athaniel Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("ATHANIEL_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.athaniel/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.athaniel/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Athaniel Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``ATHANIEL_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.athaniel/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.athaniel/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``athaniel sync device --name`` to set an explicit
    label."""
    ...
    # Athaniel Cloud (and any templated deployment) can seed the label
    # declaratively via ATHANIEL_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `athaniel sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("ATHANIEL_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.athaniel/skills/.sync_device_id, overwriting
...
# ~/.athaniel/skills/.sync_state as JSON.
#
...
# path under ~/.athaniel/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.athaniel/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Athaniel speaks "
            f"{WIRE_VERSION} — update Athaniel to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "athaniel skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (athaniel sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.athaniel/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "athaniel skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (ATHANIEL_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``athaniel sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.athaniel/skills/_org/
...
#
# `athaniel sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.athaniel/skills/_org/<org_id>/``.
