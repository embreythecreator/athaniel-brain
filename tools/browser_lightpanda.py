...
left behind by a crashed Athaniel.
"""
...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_athaniel_home()) / "bin" / "lightpanda")
    except Exception as e:  # pragma: no cover - defensive
        logger.debug("athaniel home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    ``$ATHANIEL_HOME/bin/lightpanda``. Lightpanda has no Windows build.
    """
    if os.name == "nt":
...
def _state_dir() -> Path:
    ...
    path = Path(get_athaniel_home()) / "cache" / "browser-use" / "lightpanda"
...
def launch_lightpanda(
    ...
    ``$ATHANIEL_HOME/cache/browser-use/lightpanda/<session>.log`` so a chatty
    ...
    if not binary:
        if os.name == "nt":
            return None, (
                ...
                "build. Set browser.engine to auto (or run Athaniel under WSL2)."
            )
        return None, (
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Athaniel is gone.

    Records are written by :func:`launch_lightpanda`; a live owner (another
    Athaniel process, or this one still tracking the session) is never
