#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Athaniel Skills Hub.
...
Used by athaniel_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _athaniel_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            athaniel_meta = metadata.get("athaniel", {})
            if isinstance(athaniel_meta, dict):
                tags = athaniel_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            athaniel_meta = metadata.get("athaniel", {})
            if isinstance(athaniel_meta, dict):
                raw_tags = athaniel_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Athaniel) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.athaniel/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "embreythecreator/athaniel-brain"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Athaniel-native bundle
        # under pbakaus/impeccable:.athaniel/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``athaniel update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.athaniel.upstream`` in its SKILL.md
        frontmatter:
            ...
            metadata:
              athaniel:
                upstream:
                  ...
                  path: .athaniel/skills/impeccable
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.athaniel.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(meta_block, dict):
            return None
        athaniel_meta = meta_block.get("athaniel")
        if not isinstance(athaniel_meta, dict):
            return None
        upstream = athaniel_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``athaniel_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Athaniel centralized index source
# ---------------------------------------------------------------------------

ATHANIEL_INDEX_URL = "https://athaniel-brain.embreythecreator.com/docs/api/skills-index.json"
ATHANIEL_INDEX_TTL = 6 * 3600  # 6 hours


def _athaniel_index_cache_file() -> Path:
    return _index_cache_dir() / "athaniel-index.json"


def _load_athaniel_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ATHANIEL_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    athaniel_index_cache_file = _athaniel_index_cache_file()
    if athaniel_index_cache_file.exists():
        try:
            age = time.time() - athaniel_index_cache_file.stat().st_mtime
            if age < ATHANIEL_INDEX_TTL:
                return json.loads(athaniel_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Athaniel index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Athaniel index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Athaniel index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        athaniel_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        athaniel_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class AthanielIndexSource(SkillSource):
    """Skill source backed by the centralized Athaniel Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_athaniel_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "athaniel-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "athaniel-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        AthanielIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "athaniel-index"
