...
Tracks per-skill usage metadata in a sidecar JSON file (~/.athaniel/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.athaniel/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.athaniel/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.athaniel/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``athaniel update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Athaniel metadata, VCS, virtualenv/dependency, and cache dirs
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.athaniel/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``athaniel curator
    list-archived`` to help users pass a name to ``athaniel curator restore``.
    """
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``athaniel curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``athaniel curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``athaniel curator
...
def adopt_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    if is_bundled(skill_name):
        ...
        # claim Athaniel' own shipped skill was agent-authored and change nothing
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.athaniel/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.athaniel/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.athaniel/skills/<skill>/SKILL.md) and category-nested
    (~/.athaniel/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
