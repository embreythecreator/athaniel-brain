...
teammate agent (another Athaniel profile on this install, or an agent on a
...
DM each other via a prompt-injected ``athaniel -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``athaniel -p`` / ``athaniel peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``athaniel -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``athaniel peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'athaniel' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('athaniel' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "athaniel":
...
def message_agent_tool(
    ...
    if peer_match or bare_peer:
        ...
        # Pin the registry-owning profile (#93935): `athaniel peer` resolves
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "athaniel-dm-*.txt"),
        (temp_root, "athaniel-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``athaniel -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative athaniel next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("athaniel", "athaniel.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("ATHANIEL_HOME") or os.path.expanduser("~/.athaniel"))
    return acquire_turn_lock(_athaniel_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("ATHANIEL_HOME") or os.path.expanduser("~/.athaniel")
