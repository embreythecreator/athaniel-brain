...
Design contract (matches the Athaniel "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    athaniel_meta = meta.get("athaniel") if isinstance(meta, dict) else None
    if not isinstance(athaniel_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.athaniel.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in athaniel_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.athaniel.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("athaniel", "agent", "athaniel agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Athaniel Brain' (proper caps) "
