...
machine as Athaniel itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Athaniel ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Athaniel's own uv call sites: this
    ...
    # local.py puts the Athaniel-managed $ATHANIEL_HOME/bin there, so a managed-only
