...
This is an additive Athaniel extension surfaced under ACP ``_meta.athaniel`` so
...
compression rotates the internal Athaniel head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.athaniel.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_athaniel_session_id: The live internal Athaniel DB session id
            (``state.agent.session_id``).
        previous_athaniel_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"athaniel": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_athaniel_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_athaniel_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_athaniel_session_id}
    ...
    rotated = bool(
        ...
        and previous_athaniel_session_id != current_athaniel_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentAthanielSessionId": current_athaniel_session_id,
        "rootAthanielSessionId": root_id,
        "parentAthanielSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_athaniel_session_id:
        provenance["previousAthanielSessionId"] = previous_athaniel_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"athaniel": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"athaniel": {"sessionProvenance": prov}}
