"""CLI entry point for the athaniel-brain ACP adapter.

Loads environment variables from ``~/.athaniel/.env``, configures logging
...
try:
    ...
    # shadowing Athaniel's own modules — ``athaniel acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from ATHANIEL_HOME (default ``~/.athaniel``)."""
    from athaniel_cli.env_loader import load_athaniel_dotenv

    athaniel_home = get_athaniel_home()
    loaded = load_athaniel_dotenv(athaniel_home=athaniel_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", athaniel_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="athaniel-acp",
        description="Run Athaniel Brain as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Athaniel version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Athaniel provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.athaniel/node/ "
...
def _print_version() -> None:
    ...
    print(athaniel_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import AthanielACPAgent  # noqa: F401

    print("Athaniel ACP check OK")


def _run_setup() -> None:
    from athaniel_cli.main import main as athaniel_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "athaniel", "model"]
        athaniel_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting athaniel-brain ACP adapter")
    ...
    import acp
    from .server import AthanielACPAgent
    ...
    if os.environ.get("ATHANIEL_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
