"""ACP auth helpers — detect and advertise Athaniel authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "athaniel-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Athaniel runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Athaniel can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Athaniel.
    ...
    not have Athaniel provider credentials configured yet, so Athaniel always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Athaniel using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Athaniel provider",
            description=(
                "Open Athaniel' interactive model/provider setup in a terminal. "
                "Use this when Athaniel has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
