...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(
        material,
        b"athaniel-hosted-room-installation-grant-v1",
        hashlib.sha256,
    ).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local ATHANIEL_HOME override. The process
        # environment retains the installation root and is the authority here.
        root = os.environ.get("ATHANIEL_HOME") or get_athaniel_home()
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(
        ...
        b"athaniel-hosted-room-grant-v1",
        hashlib.sha256,
    ).digest()
...
def catalog_mapping(
    *,
    ...
    persistent_process = bool(persistent_process and os.getenv("ATHANIEL_DESKTOP") != "1")
    checked_policy = RoomExecutionPolicy.from_mapping(
        execution_policy
        or execution_policy_mapping(
            target_profile=(
                str(target_profile or "").strip()
                or (os.getenv("ATHANIEL_PROFILE") or "default").strip()
                or "default"
            )
        )
    )
...
def _configured_room_link_url() -> str | None:
    ...
    override = os.getenv("ATHANIEL_ROOM_LINK_URL")
