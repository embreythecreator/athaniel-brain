...
class PlatformEntry:
    ...
    # status displays (``athaniel setup``, ``athaniel status``, the dashboard
    ...
    # Env vars this platform needs (for ``athaniel setup`` display).
    ...
    # built-ins).  Used by ``athaniel gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ATHANIEL_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `athaniel`
        # invocation -- including plain `athaniel chat`, which never touches any
        ...
        # `athaniel setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``athaniel setup``/``gateway status``, channel
