...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.athaniel to the active profile home."""
    ...
    if raw.startswith("~/.athaniel/"):
        return athaniel_home / raw.removeprefix("~/.athaniel/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return athaniel_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ATHANIEL_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from athaniel_constants import get_athaniel_home

    scripts_root = (get_athaniel_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.athaniel" or raw_text.startswith("~/.athaniel/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__athaniel_ignore__") is True
        ):
            return False, None
