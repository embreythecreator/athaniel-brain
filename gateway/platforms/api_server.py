...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Athaniel-Session-Id header; opt-in long-term memory scoping via X-Athaniel-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Athaniel-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists the versioned Athaniel model name and configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Athaniel sessions
- POST /api/sessions               — create an empty Athaniel session
- GET/PATCH/DELETE /api/sessions/{session_id} — read/update/delete a session
- GET/PUT /api/sessions/{session_id}/plan-artifact — read/edit the active plan
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to athaniel-brain
...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    """
    try:
        from athaniel_cli.profiles import profile_matches_home
...
_BROWSER_CONTROL_WS_PROTOCOL = "athaniel-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "athaniel-browser-control-ticket."
...
_TEXT_ARTIFACT_WRITE_TOOLS = frozenset({"patch", "save_plan", "write_file"})
_MARKDOWN_ARTIFACT_SUFFIXES = frozenset({".md", ".markdown", ".mdown", ".mkd", ".txt"})
...
_MAX_STREAMED_ARTIFACT_CHARS = 512_000
...
def _artifact_type_for_path(path: str) -> Optional[str]:
    suffix = Path(path).suffix.lower()
    if suffix in _MARKDOWN_ARTIFACT_SUFFIXES:
        return "markdown"
...
def _artifact_event_from_tool_result(
    ...
    """Normalize successful text-file writes into one client-safe event."""
    tool_name = str(function_name or "")
    if tool_name not in _TEXT_ARTIFACT_WRITE_TOOLS:
        return None

    args = _json_object(function_args)
    result = _json_object(function_result)
    ...
    if tool_name == "save_plan":
        ...
        if not path or not isinstance(content, str):
            return None
        return {
            "id": str(result.get("plan_id") or path),
            "type": "plan",
            "title": str(args.get("title") or Path(path).name),
            "content": content[:_MAX_STREAMED_ARTIFACT_CHARS],
            "path": path,
            "revision": int(result.get("revision") or 0),
            "short_id": str(result.get("short_id") or ""),
            "editable": True,
        }
    ...
    path = str(
        ...
        or (modified[0] if isinstance(modified, list) and modified else "")
        or args.get("path")
        or ""
    )
    artifact_type = _artifact_type_for_path(path)
    if not path or artifact_type is None:
        return None

    # The credential read-denylist is wider than the write-denylist, so a
    # permitted patch/write of e.g. ~/.athaniel/auth.json must not stream the
    # file back out — that would leak more than the model itself ever saw.
    from agent.file_safety import get_read_block_error

    if get_read_block_error(path):
        return None

    content = args.get("content") if tool_name == "write_file" else None
    if not isinstance(content, str):
        try:
            candidate = Path(path)
            if candidate.is_file() and candidate.stat().st_size <= _MAX_STREAMED_ARTIFACT_CHARS * 4:
...
def _athaniel_version() -> str:
    """Return the canonical Athaniel Brain version string.

    ``athaniel_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``athaniel_brain-*.dist-info`` after a source update until the environment is
    ...
    try:
        ...
        return version("athaniel-brain")
    except Exception:
        return "dev"


def _default_athaniel_model_name() -> str:
    version = _athaniel_version().strip()
    if not version or version == "dev":
        return "Athaniel-dev"
    return f"Athaniel-v{version}"
...
def _is_action_result_turn(request: "web.Request") -> bool:
    raw = request.headers.get("X-Athaniel-Turn-Type", "")
...
def _limb_rail_telemetry_path() -> Path:
    ...
    return get_athaniel_home() / "logs" / "limb_rail.jsonl"
...
# config.yaml accept (athaniel_constants.VALID_REASONING_EFFORTS); wire-level
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from athaniel_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("ATHANIEL_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``athaniel-brain``)
    ...
    ``direct_model_requests`` config value here, while Athaniel-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Athaniel-aware and is
    always honored.
    """
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_athaniel_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted ATHANIEL_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # athaniel_state._WAL_INCOMPAT_MARKERS).
        from athaniel_state import apply_wal_with_fallback
...
def _tool_activity_result_preview(function_name: str, result: Any) -> str:
    """Keep a bounded, redacted diff receipt as valid JSON when it fits."""
    if function_name in {"patch", "write_file"}:
        receipt = _json_object(result)
        diff = receipt.get("diff")
        if isinstance(diff, str):
            # Redact before JSON encoding: an env assignment in an escaped
            # JSON string otherwise consumes the entire remaining diff.
            return json.dumps({
                "success": receipt.get("success") is not False,
                "diff": _redact_api_error_text(diff),
            }, ensure_ascii=False)[:64_000]
    return _redact_api_error_text(result, limit=300)
...
def _evidence_stale(policy: Any) -> bool:
    """True when a plan carries pins whose set no longer matches its digest."""
    try:
...
def _derive_chat_session_id(
    ...
    the same Athaniel session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through athaniel-brain's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Athaniel-native session-chat
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from athaniel_cli.config import cfg_get, load_config
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: versioned Athaniel model name

        Delegates the tiered fallthrough to
        :func:`athaniel_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from athaniel_cli.model_switch import resolve_effective_model
        ...
        return resolve_effective_model(
            explicit, profile_name, _default_athaniel_model_name()
        )
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from athaniel_cli.auth import has_usable_secret
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `athaniel peer dm mini/researcher` answered by the mini's default
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_athaniel_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from athaniel_cli.profiles import get_profile_dir
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            ("GET", "/api/sessions/{session_id}/plan-artifact", self._handle_get_plan_artifact),
            ("PUT", "/api/sessions/{session_id}/plan-artifact", self._handle_put_plan_artifact),
            ("GET", "/api/sessions/{session_id}/plan-artifact/pins", self._handle_get_plan_pins),
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Athaniel-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Athaniel-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Athaniel-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Athaniel-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Athaniel-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``athaniel sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_athaniel_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_athaniel_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``athaniel-brain``) means "use
        ...
        400s ("athaniel-brain is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "athaniel-brain") is
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "athaniel_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "athaniel_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the athaniel-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Athaniel-Session-Key``).  Unlike ``session_id``
        ...
        # athaniel_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `athaniel auth add openai-codex` without `athaniel model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from athaniel_cli.models import get_default_model_for_provider
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "athaniel-brain", "version": _athaniel_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "athaniel-brain",
            "version": _athaniel_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list athaniel-brain and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Athaniel provider/model inventory.
        ...
        Athaniel provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Athaniel version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "athaniel.api_server.capabilities",
            "platform": "athaniel-brain",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Athaniel AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Athaniel-Session-Id",
                "session_key_header": "X-Athaniel-Session-Key",
            ...
            "endpoints": {
                ...
                "session_plan_artifact_read": {
                    "method": "GET",
                    "path": "/api/sessions/{session_id}/plan-artifact",
                },
                "session_plan_artifact_update": {
                    "method": "PUT",
                    "path": "/api/sessions/{session_id}/plan-artifact",
                },
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<ATHANIEL_HOME>/plugin-data/.../artifacts``-style controlled root),
        ...
        try:
            ...
            # under the Athaniel home.
            try:
                from athaniel_state import get_athaniel_home

                root = Path(get_athaniel_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    def _session_response(session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe_keys = (
            ...
            "api_call_count", "parent_session_id", "fork_message_id", "last_active", "preview",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Athaniel sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `athaniel peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `athaniel peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Athaniel session row.
        ...
        # (e.g. "provider::athaniel-brain") are split, and the virtual model
        # alias (self._model_name, e.g. "athaniel-brain") is nulled out
        # there — a bare "athaniel-brain" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "athaniel.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "athaniel.session", "session": payload})

    def _execution_policy_store(self):
        from agent.execution_policy import ExecutionPolicyStore
        # `self._session_db` is never assigned after __init__ — the live handle
        # lives in the per-profile `_session_dbs` cache behind _ensure_session_db.
        return ExecutionPolicyStore(self._ensure_session_db())

    @staticmethod
    def _plan_artifact_title(
        content: str, path: str, task: str = "", saved_title: str = ""
    ) -> str:
        # The title the author passed to save_plan wins — the H1 scan below has
        # no fence tracking, so a `# Rebuild the container` inside a ```bash
        # block would rename the artifact. Kept as a fallback for plans saved
        # before the title was persisted.
        ...
        stem = Path(path).stem
        stem = re.sub(r"^\d{4}-\d{2}-\d{2}_\d{6}-", "", stem)
        return stem.replace("-", " ").strip().title() or task.strip() or "Plan"

    def _plan_artifact_payload(self, session_id: str) -> tuple[Dict[str, Any], Optional[str]]:
        store = self._execution_policy_store()
        policy = store.load(session_id)
        plan = self._plan_block(session_id)
        if not policy.plan_path or not policy.plan_id:
            return {"plan": plan, "artifact": None}, None

        try:
            from agent.execution_policy import MAX_PLAN_CONTENT_CHARS
            path = Path(policy.plan_path)
            if path.stat().st_size > MAX_PLAN_CONTENT_CHARS * 4:
                return {}, f"Plan artifact exceeds {MAX_PLAN_CONTENT_CHARS} characters."
            content = path.read_text(encoding="utf-8")
        except (OSError, UnicodeError) as exc:
            return {}, f"Plan artifact is unreadable: {exc}"
        ...
        return {
            ...
            "artifact": {
                "id": policy.plan_id,
                "type": "plan",
                "title": self._plan_artifact_title(
                    content, policy.plan_path, policy.task, policy.title
                ),
                "content": content,
    ...
    async def _handle_get_plan_artifact(self, request: "web.Request") -> "web.Response":
        """GET the artifact owned by this session's active plan policy."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if err:
            return err
        payload, artifact_err = await asyncio.to_thread(
            self._plan_artifact_payload, session_id
        )
        if artifact_err:
            return web.json_response(
                _openai_error(artifact_err, code="plan_artifact_unreadable"),
                status=409,
            )
        return web.json_response(payload)

    async def _handle_put_plan_artifact(self, request: "web.Request") -> "web.Response":
        """Revision-checked edit of the exact file already owned by a plan."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        session_id = request.match_info["session_id"]
        ...
        if err:
            return err
        if self._session_busy(session_id):
            return web.json_response(
                _openai_error(
                    "The plan cannot be edited while its session is running.",
                    code="plan_session_busy",
                ),
                status=409,
            )
        body, err = await self._read_json_body(request)
        if err:
            return err
        unknown = sorted(set(body) - {"content", "expected_revision"})
        if unknown:
            return web.json_response(
                _openai_error(
                    f"Unsupported plan artifact fields: {', '.join(unknown)}",
                    code="unsupported_plan_artifact_field",
                ),
                status=400,
            )
        updated, update_err = await asyncio.to_thread(
            self._execution_policy_store().replace_plan_content,
            session_id,
        ...
        if update_err:
            status = 409 if (
                "revision conflict" in update_err.lower()
                or "digest mismatch" in update_err.lower()
            ) else 400
            return web.json_response(
                _openai_error(
                    update_err,
                    code="plan_artifact_conflict" if status == 409 else "invalid_plan_artifact",
                ),
                status=status,
            )
        payload, artifact_err = await asyncio.to_thread(
    ...
    async def _handle_patch_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "athaniel.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "athaniel.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        if self._session_busy(source_id):
            return web.json_response(_openai_error("Finish the current turn before branching", code="session_busy"), status=409)
        anchor = body.get("anchor")
        fork_message_id = body.get("fork_message_id")
        if (anchor is not None and not isinstance(anchor, dict)) or (
            fork_message_id is not None and (not isinstance(fork_message_id, str) or len(fork_message_id) > 256)
        ) or type(body.get("exclude_anchor", False)) is not bool:
            return web.json_response(_openai_error("Invalid fork point", code="invalid_fork"), status=400)
        title = body.get("title")
        if title is None:
            title = await asyncio.to_thread(db.get_next_title_in_lineage, source.get("title") or "fork")
        from athaniel_state import CompressionSessionBusyError, SessionTurnLeaseLostError
        try:
            await asyncio.to_thread(db.fork_session, source_id, fork_id, title=title,
                                   fork_message_id=fork_message_id, anchor=anchor,
                                   exclude_anchor=body.get("exclude_anchor", False))
        except (CompressionSessionBusyError, SessionTurnLeaseLostError) as exc:
            return web.json_response(_openai_error(str(exc), code="session_busy"), status=409)
        except ValueError as exc:
            return web.json_response(_openai_error(str(exc), code="invalid_fork"), status=400)
        fork = await asyncio.to_thread(db.get_session, fork_id) or {"id": fork_id, "parent_session_id": source_id}
        return web.json_response({"object": "athaniel.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_busy_detail(self, session_id: str) -> str:
        """Human-readable account of WHAT is holding a busy session.

        WO-PLAN/BUSY-1: the visible turn can be long over while delegation
        wake self-posts run hidden turns on the same session — a bare
        "agent is running" reads as a lie from the operator's chair.
        """
        n = 0
        try:
            from tools.delegate_tool import count_active_subagents_for_session
            n = count_active_subagents_for_session(session_id)
        except Exception:
            pass
        if n:
            return (
                f"Athaniel is still finishing background work — {n} "
                f"subagent{'s' if n != 1 else ''} active; their results are "
                "processed in a background turn when they complete."
            )
        return (
            "Athaniel is still finishing a background turn on this session "
            "(delegation results or a scheduled self-post)."
        )
    ...
    def _plan_block(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            policy = self._execution_policy_store().load(session_id)
        except Exception:
            return None
        if policy.state is PlanModeState.OFF:
            return None
        # WO-PLAN/BUSY-1: busy is the same truth the /plan guard enforces, so
        # clients can disable/queue their Approve UI instead of inviting a
        # click that will bounce. active_subagents feeds the reason line.
        busy = False
        subagents = 0
        try:
            busy = self._session_busy(session_id)
            from tools.delegate_tool import count_active_subagents_for_session
            subagents = count_active_subagents_for_session(session_id)
        except Exception:
            pass
        return {
            ...
            # WO-POSTURE/1 4.9 — ids only; the full pin table is the
            # plan-artifact/pins route. Stale = the pinned set no longer
            # matches its recorded digest (4.8 treats stale as contradicted).
            "evidence_pins": [p.get("id") for p in policy.evidence_pins if isinstance(p, dict)],
            "evidence_stale": _evidence_stale(policy),
        }

    def _plan_pins_payload(self, session_id: str) -> Dict[str, Any]:
        policy = self._execution_policy_store().load(session_id)
        return {
            "plan_id": policy.plan_id,
            "revision": policy.revision,
            ...
            "evidence_stale": _evidence_stale(policy),
            "pins": [dict(p) for p in policy.evidence_pins if isinstance(p, dict)],
        }

    async def _handle_get_plan_pins(self, request: "web.Request") -> "web.Response":
        """GET the evidence pins behind this session's plan (4.9)."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        session_id = request.match_info["session_id"]
        ...
        if err:
            return err
        payload = await asyncio.to_thread(self._plan_pins_payload, session_id)
        return web.json_response(payload)

    def _flesh_confirm_block(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Client-safe pending Flesh T3 approval for the Face confirm card,
        or None. Same contract as _plan_block: the key is always present on
        completion payloads so the Face clears its card on None."""
        try:
            from plugins.flesh.approval import FleshApprovalStore
            return FleshApprovalStore().confirm_payload(session_id)
        except Exception:
            return None

    def _seal_confirm_block(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Client-safe pending Seal T2/T3 grant for the Face approval card, or
        None. Same always-present-key contract as _plan_block and
        _flesh_confirm_block: the key rides every completion payload so the Face
        clears its card on None rather than leaving a stale copy behind.

        Face holds no approval state — it renders this and posts back a
        (nonce, digest) pair, both of which the store re-verifies. See
        agent/seal_approval.py for why verifying the digest alone is not enough.
        """
        try:
            from agent.seal_approval import SealApprovalStore
            return SealApprovalStore().confirm_payload(session_id)
        except Exception:
            return None

    def _seal_gate(self, session_id: str, final_text: str) -> List[str]:
        """Emission-time Seal gate -> the action ids the Face may execute.

        MUST run before _seal_confirm_block on the same payload: it is what
        opens the pending grant that block then reports. Reversing the order
        ships the card one turn late.

        Returns [] on any failure, which means the Face awaits rather than
        executes — the gate fails closed. See agent/seal_approval.py for why
        this has to be computed here instead of trusted from a field inside the
        model-authored envelope.
        """
        try:
            from agent.seal_approval import gate_space_actions
            return gate_space_actions(session_id, final_text or "")
        except Exception:
            logger.warning("seal gate unavailable; withholding all actions", exc_info=True)
            return []
    ...
    def _apply_posture(self, body: Any, session_id: str) -> None:
        """WO-POSTURE/1 2.3 — the client's resting posture (chat|act) becomes
        the session's chat flag. Anything else is ignored; plan mode is the
        Brain's own state and never arrives this way. Never raises."""
        posture = body.get("posture") if isinstance(body, dict) else None
        if posture not in ("chat", "act"):
            return
        try:
            store = self._execution_policy_store()
            if store.load(session_id).chat != (posture == "chat"):
                store.set_chat(session_id, posture == "chat")
        except Exception as exc:
            logger.debug("posture flag not applied for %s: %s", session_id, exc)

    def _apply_plan_mode(self, user_message: Any, session_id: str) -> tuple[Optional[str], Any]:
        ...
        if text == "/plan" or text.startswith("/plan "):
            ...
            if args.lower() != "status" and self._session_busy(session_id):
                return (
                    f"{self._session_busy_detail(session_id)} "
                    "Your /plan command was not applied — retry once the "
                    "session is idle (the plan card re-enables itself).",
                    user_message,
                )
            ...
            try:
                from athaniel_cli.config import load_config
                from athaniel_cli import codex_runtime_switch as crs
                runtime_is_codex = (
        ...
        # /flesh — same exact-token interception pattern as /plan. T3 arming
        # (approve) and denial happen here; the dispatch-side consume check in
        # plugins/flesh/handlers.py is the actual enforcement boundary.
        if text == "/flesh" or text.startswith("/flesh "):
            args = text[len("/flesh"):].strip()
            if args.lower() not in ("", "status") and self._session_busy(session_id):
                return (
                    "Agent is running — wait for the turn to finish, "
                    "then use /flesh.",
                    user_message,
                )
            from agent.flesh_mode import handle_flesh_command
            result = handle_flesh_command(args, session_id)
            if result.reply is not None:
                return result.reply, user_message
            return None, result.rewritten_message or ""

        # /seal — same exact-token interception as /flesh. The enforcement
        # boundary is SealApprovalStore.approve's (nonce, digest) re-check, not
        # this dispatch, so no surface can arm a grant just by reaching here.
        if text == "/seal" or text.startswith("/seal "):
            args = text[len("/seal"):].strip()
            if args.lower() not in ("", "status", "ledger") and self._session_busy(session_id):
                return (
                    "Agent is running — wait for the turn to finish, "
                    "then use /seal.",
                    user_message,
                )
            from agent.seal_mode import handle_seal_command
            result = handle_seal_command(args, session_id)
            if result.reply is not None:
                return result.reply, user_message
            return None, result.rewritten_message or ""
        try:
            ...
            # A bare ExecutionPolicyStore() opens a fresh connection to the
            # multi-hundred-MB state.db and re-runs the whole schema DDL on
            # every message. It also binds the import-time DEFAULT_DB_PATH,
            # so under multiplex /p/<profile>/ it reads the wrong profile.
            policy = ExecutionPolicyStore(self._ensure_session_db()).load(session_id)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        await asyncio.to_thread(self._apply_posture, body, session_id)
        ...
        if plan_reply is not None:
            ...
            headers = {"X-Athaniel-Session-Id": session_id}
            if gateway_session_key:
                headers["X-Athaniel-Session-Key"] = gateway_session_key
            return web.json_response(
                {
                    "object": "athaniel.session.chat.completion",
                    "session_id": session_id,
        ...
        headers = {"X-Athaniel-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Athaniel-Session-Key"] = gateway_session_key
        runtime = {}
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Athaniel-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Athaniel-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "athaniel.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Athaniel-Session-Key.  This
        # is independent of X-Athaniel-Session-Id: the key persists across
        ...
        # WO-SESSION/YIELD-1: machine turns declare themselves so the durable
        # session-turn lease can classify the holder. The gateway's own wake
        # self-posts send "wake"; anything else (absent, unknown) is an
        # operator turn. Worst case for a mislabeled caller is being asked to
        # yield — safe, so no auth gate beyond what the route already has.
        turn_kind = request.headers.get("X-Athaniel-Turn-Kind", "").strip().lower()
        if turn_kind not in ("wake", "background"):
            turn_kind = "operator"

        # Allow caller to continue an existing session by passing X-Athaniel-Session-Id.
        ...
        if provided_session_id:
            ...
            # conversation map to the same Athaniel session.  The first user
        ...
        if plan_reply is not None:
            ...
            flesh_block = await asyncio.to_thread(self._flesh_confirm_block, session_id)
            seal_block = await asyncio.to_thread(self._seal_confirm_block, session_id)
            control_headers = {"X-Athaniel-Session-Id": session_id}
            if gateway_session_key:
                control_headers["X-Athaniel-Session-Key"] = gateway_session_key
            if not stream:
                return web.json_response(
                    {
                        "id": completion_id,
                        ...
                        # WO-PLAN/BUSY-1: synthesized control reply, not model
                        # prose — clients may render it as a system notice.
                        "control": "plan",
                        "plan": plan_block,
                        "flesh_confirm": flesh_block,
                        # No agent ran, so no block was emitted and nothing is
                        # authorized. The key still ships: presence is the
                        # contract, so the Face clears rather than going stale.
                        "seal_authorized": [],
                        "seal_confirm": seal_block,
                    },
            ...
            for chunk in (
                ...
                {**base_chunk, "choices": [{"index": 0, "delta": {}, "finish_reason": "stop"}], "control": "plan", "plan": plan_block, "flesh_confirm": flesh_block, "seal_authorized": [], "seal_confirm": seal_block},
            ):
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``athaniel.tool.progress`` with ``status: running``.
            ...
            def _on_tool_complete(tool_call_id, function_name, function_args, function_result):
                ...
                # Additive optional field (#6972 clients ignore it): short
                # redacted preview of the tool result for activity displays.
                try:
                    if function_result is not None:
                        payload["resultPreview"] = _tool_activity_result_preview(
                            function_name, function_result
                        )
                        # True token weight of the FULL result, counted before
                        # truncation — the preview alone undercounts and the
                        # Face can only estimate from it. Same (chars+3)//4
                        # arithmetic as agent.context_breakdown._chars_to_tokens.
                        payload["resultTokens"] = (len(str(function_result)) + 3) // 4
                except Exception:
                    pass
                # put_threadsafe, matching _on_tool_start above: this callback
                # runs on the agent thread, and a bare .put() into an
                # asyncio.Queue from the wrong thread both drops the event
                # (#16588 lifecycle pairs) and warns under pytest.
                _stream_q.put_threadsafe(("__tool_progress__", payload))
                artifact = _artifact_event_from_tool_result(
                    function_name,
                    function_args,
                    function_result,
                )
                if artifact is not None:
                    _stream_q.put_threadsafe(("__artifact__", artifact))

            def _on_reasoning_delta(text):
                """Live reasoning deltas → ``event: athaniel.activity``.

                Reasoning text must only ever travel on the activity event,
                never inside ``delta.content`` (reasoning-leak incidents
                #8878/#9568).
                """
                if text:
                    _stream_q.put_threadsafe(("__activity__", {
                        "type": "reasoning.delta",
                        "text": str(text),
                        "ts": time.time(),
                    }))

            def _on_thinking_status(text):
                # thinking_callback fires "" to clear the status line;
                # forward the clear too so clients can blank their status.
                _stream_q.put_threadsafe(("__activity__", {
                    "type": "status.update",
                    "text": str(text or ""),
                    "ts": time.time(),
            ...
            def _on_agent_status(kind, message):
                _stream_q.put_threadsafe(("__activity__", {
                    "type": "status.update",
                    "kind": str(kind or ""),
                    "text": str(message or ""),
            ...
            agent_task = asyncio.ensure_future(self._run_agent(
                ...
                thinking_callback=_on_thinking_status,
        ...
        response_headers = {
            "X-Athaniel-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Athaniel-Session-Key"] = gateway_session_key
        if result.get("yielded_to_operator") is True:
            # WO-SESSION/YIELD-1: tells the wake self-post caller
            # (gateway/wake.py) this turn yielded to an operator message so
            # it can requeue the remainder after the operator turn runs.
            response_headers["X-Athaniel-Yielded"] = "true"
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["athaniel"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Athaniel-Completed"] = "false"
            response_headers["X-Athaniel-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Athaniel-specific extras.
        response_data = {
            "id": completion_id,
            "object": "chat.completion",
            "created": created,
            ...
            "flesh_confirm": await asyncio.to_thread(
                self._flesh_confirm_block, result.get("session_id", session_id) or session_id
            ),
            # Gate first, then report: _seal_gate is what opens the pending
            # grant that _seal_confirm_block turns into the card.
            "seal_authorized": await asyncio.to_thread(
                self._seal_gate, result.get("session_id", session_id) or session_id, final_response
            ),
            "seal_confirm": await asyncio.to_thread(
                self._seal_confirm_block, result.get("session_id", session_id) or session_id
            ),
            # Parity with the streaming finish_chunk site. Two separate dict
            # literals, one shared helper — see _suggested_follow_ups.
            "suggested_follow_ups": await self._suggested_follow_ups(
                user_message, final_response
            ),
        }
        if is_partial or is_failed or not completed:
            response_data["athaniel"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
                "error": err_msg,
            ...
            response_headers["X-Athaniel-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Athaniel-Error"] = _redact_api_error_text(err_msg, limit=200)

        return web.json_response(response_data, headers=response_headers)

    async def _suggested_follow_ups(
        self, user_message: Any, assistant_response: Any
    ) -> list:
        """Return up to three follow-up suggestions for the finished turn.

        Emitted as an additive key on BOTH terminal payloads — the streaming
        ``finish_chunk`` and the non-streaming ``response_data``. Those are two
        separate dict literals and parity between them is manual, which is why
        this lives in one helper rather than being inlined twice.

        Always returns a list. The generator swallows its own failures, and the
        ``to_thread`` wrapper is guarded too: a suggestion row must never be
        able to fail a turn that already produced a good answer.
        """
        ...
        if not user_message or not assistant_response:
            return []
        try:
            from agent.follow_up_generator import generate_follow_ups

            return await asyncio.to_thread(
                generate_follow_ups, user_message, assistant_response
            )
        except Exception:
    ...
    async def _write_sse_chat_completion(
        ...
        if cors:
            sse_headers.update(cors)
        if session_id:
            sse_headers["X-Athaniel-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Athaniel-Session-Key"] = gateway_session_key
        response = web.StreamResponse(status=200, headers=sse_headers)
        await response.prepare(request)
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: athaniel.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: athaniel.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                elif isinstance(item, tuple) and len(item) == 2 and item[0] == "__artifact__":
                    event_data = json.dumps(item[1], ensure_ascii=False)
                    await response.write(
                        f"event: athaniel.artifact\ndata: {event_data}\n\n".encode("utf-8")
                    )
                elif isinstance(item, tuple) and len(item) == 2 and item[0] == "__activity__":
                    # Reasoning/status activity events (see _on_reasoning_delta
                    # and friends). Same custom-SSE-event pattern as
                    # athaniel.tool.progress — data:-only OpenAI clients
                    # skip lines with an event: name they don't know.
                    event_data = json.dumps(item[1])
                    await response.write(
                        f"event: athaniel.activity\ndata: {event_data}\n\n".encode()
                    )
                    ...
                    if agent_task.done():
                        ...
                        while True:
                            try:
                                ...
                                break
                        break
                    # Poll the transport so a client disconnect is noticed
                    # within ~0.5s instead of at the next failed write (the
                    # 30s keepalive, or the first delta after a long thinking
                    # or session-lease wait) — the window where a stopped
                    # turn used to keep generating to completion. Raising
                    # here funnels into the same disconnect cleanup as a
                    # failed write.
                    _transport = request.transport
                    if _transport is None or _transport.is_closing():
                        raise ConnectionResetError("SSE client transport closed")
            ...
            finish_chunk = {
                ...
                "flesh_confirm": await asyncio.to_thread(self._flesh_confirm_block, _plan_session_id),
                # Gate first, then report — see the non-streaming site. The
                # blocks were streamed as text already; nothing executes on the
                # Face until this list says so.
                "seal_authorized": await asyncio.to_thread(
                    self._seal_gate,
                    _plan_session_id,
                    (result.get("final_response") or "") if isinstance(result, dict) else "",
                ),
                "seal_confirm": await asyncio.to_thread(self._seal_confirm_block, _plan_session_id),
                # Additive key on the terminal chunk, NOT a new SSE event — a new
                # event name would miss ACTIVITY_EVENT_NAMES on the Face and be
                # parsed as a completion chunk. Parity with the non-streaming
                # response_data site below is manual; both call one helper.
                "suggested_follow_ups": await self._suggested_follow_ups(
                    user_message,
                    (result.get("final_response") or "") if isinstance(result, dict) else "",
                ),
                    ...
                    if agent_task.done():
                        ...
                        while True:
                            try:
                                ...
                                break
                        break
                    # Same ~0.5s disconnect poll as the chat-completions SSE
                    # writer: funnel a dead transport into the disconnect
                    # cleanup instead of waiting for a failed write.
                    _transport = request.transport
                    if _transport is None or _transport.is_closing():
                        raise ConnectionResetError("SSE client transport closed")
                    if time.monotonic() - last_activity >= CHAT_COMPLETIONS_SSE_KEEPALIVE_SECONDS:
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Athaniel-Session-Id header.  Without this,
        ...
        response_headers = {"X-Athaniel-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Athaniel-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `athaniel cron run` / cronjob(action='run', prompt=...) — same length
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Athaniel agent; they are replayed for structured tool
    ...
    async def _run_agent(
        ...
        *turn_kind* (WO-SESSION/YIELD-1) classifies the turn for the durable
        ...
        an operator message is waiting; "operator" (default) turns never do.
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # WO-SESSION/YIELD-1: stamp EVERY turn (agents are cached
                    # per session) so a wake turn's class can never leak onto
                    # the operator turn that follows it.
                    agent._session_turn_kind = (
                        turn_kind if turn_kind in ("wake", "background") else "operator"
                    )
                    ...
                    # (e.g. X-Athaniel-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_athaniel_api_runtime", {}) or {})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Athaniel-Relay
            ...
            # [overlay] athaniel-brain settings routes — guarded so a bad import
            ...
            # that is the exact surface the athaniel-0day campaign abused to write
            # ~/.athaniel/config.yaml and plant persistence. Sandboxing (Docker /
