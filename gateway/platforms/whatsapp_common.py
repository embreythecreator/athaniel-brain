...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "✶ *Athaniel Brain*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ATHANIEL_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/athaniel), this function
    mirrors the bridge source to a writable ATHANIEL_HOME location and returns that
    ...
    # Try ATHANIEL_HOME location first
    athaniel_home = get_athaniel_home()
    athaniel_home_bridge = athaniel_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ATHANIEL_HOME if needed
    if athaniel_home_bridge.exists():
        return athaniel_home_bridge

    # Mirror the bridge source to ATHANIEL_HOME
    try:
        athaniel_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            athaniel_home_bridge,
            dirs_exist_ok=False,
        )
        return athaniel_home_bridge
    except Exception:
