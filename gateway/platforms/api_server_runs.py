...
_ROOM_RETENTION_REQUEST_KEY = (
    RequestKey("athaniel.room_run_retention_until", float)
    if RequestKey is not None
    else "athaniel.room_run_retention_until"
)
...
def _remember_room_retention(request: "web.Request", claims: dict[str, Any]) -> None:
    ...
    try:
        ...
        setattr(request, "_athaniel_room_run_retention_until", value)
...
def _room_retention_until(request: "web.Request") -> float:
    try:
        ...
        value = getattr(request, "_athaniel_room_run_retention_until", 0)
...
async def _handle_runs(
    ...
    *,
    ...
    response_headers = (
        {"X-Athaniel-Session-Key": gateway_session_key} if gateway_session_key else {}
    )
    return web.json_response(
...
async def _handle_run_approval(
    ...
    *,
    ...
    return web.json_response({
        "object": "athaniel.run.approval_response",
        "run_id": run_id,
        "choice": choice,
        **({"request_id": request_id} if request_id else {}),
        "resolved": resolved,
...
async def _handle_steer_run(
    ...
    *,
    ...
    return web.json_response({"object": "athaniel.run.steer", "run_id": run_id, "accepted": True})
