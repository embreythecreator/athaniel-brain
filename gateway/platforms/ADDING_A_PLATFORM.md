...
There are two ways to add a platform to the Athaniel gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.athaniel/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Athaniel code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `athaniel gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `athaniel_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Athaniel core.
...
And add it to the `athaniel-gateway` composite:

```python
"athaniel-gateway": {
    "includes": [..., "athaniel-your-platform"]
}
```
...
## 12. Status Display (`athaniel_cli/status.py`)
...
## 13. Gateway Setup Wizard (`athaniel_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ athaniel_cli/ toolsets.py \
  --include="*.py" -l | sort -u
