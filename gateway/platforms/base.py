...
# Audio file extensions Athaniel recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Athaniel but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Athaniel' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Athaniel-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_athaniel_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``ATHANIEL_SESSION_PLATFORM`` contextvar — that contextvar is
...
from athaniel_constants import get_default_athaniel_root, get_athaniel_dir, get_athaniel_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.athaniel/.env manually."
)
...
IMAGE_CACHE_DIR = get_athaniel_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_athaniel_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_athaniel_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from athaniel_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; AthanielAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_athaniel_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_athaniel_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_athaniel_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_athaniel_dir("cache/screenshots", "browser_screenshots")
...
_ATHANIEL_ROOT = get_default_athaniel_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ATHANIEL_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ATHANIEL_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ATHANIEL_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ATHANIEL_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ATHANIEL_HOME / "image_cache",
    _ATHANIEL_HOME / "audio_cache",
    _ATHANIEL_HOME / "video_cache",
    _ATHANIEL_HOME / "document_cache",
    _ATHANIEL_HOME / "browser_screenshots",
    ...
    _ATHANIEL_HOME / "cache" / "images",
    _ATHANIEL_HOME / "cache" / "audio",
    _ATHANIEL_HOME / "cache" / "videos",
    _ATHANIEL_HOME / "cache" / "documents",
    _ATHANIEL_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Athaniel root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* ATHANIEL_HOME's cache, so a gateway running at
    the root (e.g. ``ATHANIEL_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when ATHANIEL_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _ATHANIEL_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("ATHANIEL_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _ATHANIEL_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Athaniel profile and shared Athaniel root both contain control
    ...
    # ATHANIEL_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.athaniel
    ...
    for athaniel_root in (_ATHANIEL_HOME, _ATHANIEL_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(athaniel_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Athaniel
    secrets (``~/.athaniel/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Athaniel cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.athaniel/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Athaniel cache dirs (``/root/.athaniel/...``), the session's
    ...
    # In-process gateways (Desktop backend, `athaniel serve`) may not have
    ...
    # are longer prefixes, so /root/.athaniel/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.athaniel/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.athaniel"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``ATHANIEL_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Athaniel-managed cache, under an operator-allowlisted root
    (``ATHANIEL_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Athaniel root (~/.athaniel/.env,
    ...
    # ``MEDIA:~/.athaniel/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?athaniel\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+athaniel[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``ATHANIEL_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Athaniel commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-ATHANIEL_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("ATHANIEL_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ATHANIEL_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ATHANIEL_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Athaniel sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-ATHANIEL_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(athaniel --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Athaniel home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Athaniel home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.athaniel/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from athaniel_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ATHANIEL_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ATHANIEL_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ATHANIEL_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ATHANIEL_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ATHANIEL_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ATHANIEL_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_athaniel_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # ATHANIEL_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _athaniel_run_generation is set on
    ...
    def build_source(
        ...
        per-profile ATHANIEL_HOME isolation.
        """
