...
Defenses 1 and 2 (the ``_ATHANIEL_GATEWAY`` guard on ``athaniel gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.athaniel.gateway")``,
...
State lives in ``<ATHANIEL_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_athaniel_home() / "gateway" / "restart_loop.json"
