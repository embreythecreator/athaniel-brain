...
# path eagerly freezes it to whatever ATHANIEL_HOME/profile context existed
# at that exact import moment for the rest of the process's lifetime --
# even if a context-local override (see athaniel_constants.set_athaniel_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``athaniel pairing`` CLI) re-imports this module later with the final
...
# working (embreythecreator/athaniel-brain#93449).
#
...
def _default_pairing_dir() -> Path:
    ...
    if PAIRING_DIR is not None:
        return PAIRING_DIR
    return get_athaniel_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``athaniel_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from athaniel_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ATHANIEL_HOME}/pairing`` while newer code/docs may
    write ``{ATHANIEL_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own ATHANIEL_HOME using the same legacy/consolidated layout rules
    as ``athaniel -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current ATHANIEL_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ATHANIEL_HOME
        ...
        if profile:
            ...
            # ``athaniel -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `athaniel` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> athaniel pairing approve ...` as root, "
                    "re-run with `docker exec -u athaniel <container> ...` and "
                    "chown the existing file to the athaniel user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``athaniel
