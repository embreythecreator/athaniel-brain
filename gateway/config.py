...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from athaniel_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (athaniel_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Athaniel is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_athaniel_home() / "sessions")
    ...
    # gateway behaves exactly as before — single ATHANIEL_HOME, no profile stamping.
    ...
    # advertise a route. ATHANIEL_ROOM_LINK_URL remains the operator override.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.athaniel/config.yaml (primary user-facing config)
    3. ~/.athaniel/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_athaniel_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through athaniel_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `athaniel config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `athaniel config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``athaniel config set gateway.streaming.*``
