...
Module-level run.py helpers a handler needs (``_athaniel_home``,
...
from athaniel_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: athaniel gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Athaniel commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            from athaniel_cli.tips import get_random_tip
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`athaniel gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _athaniel_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _athaniel_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _athaniel_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Athaniel Brain version."""
        from athaniel_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _athaniel_home, _load_gateway_config
        ...
        from athaniel_cli.providers import get_label
        ...
        # via the shared single-owner parser (athaniel_cli.model_switch).
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _athaniel_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from athaniel_cli.context_switch_guard import (
                        ...
                        from athaniel_cli.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from athaniel_cli.route_identity import should_clear_context_pin_async
                        ...
                        from athaniel_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Athaniel default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        # Persist the selection only — athaniel_cli.personality never writes
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from athaniel_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `athaniel cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from athaniel_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Athaniel itself changed. ``--stat`` limits output to the summary.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from gateway.run import _athaniel_home
        from athaniel_cli.config import read_user_config_raw
        config_path = _athaniel_home / "config.yaml"
        try:
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.athaniel/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``athaniel skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _athaniel_home
        from athaniel_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `athaniel skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.athaniel/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from athaniel_cli.models import model_supports_fast_mode
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _athaniel_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Athaniel is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while Athaniel is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while Athaniel is busy."
            return EphemeralReply(
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _athaniel_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from athaniel_cli.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from athaniel_cli.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="athaniel_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from athaniel_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (athaniel_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `athaniel bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``athaniel debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Athaniel Brain to the latest version.

        Spawns ``athaniel update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``athaniel update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _athaniel_home, _resolve_athaniel_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Athaniel Brain')}"
        ...
        athaniel_cmd = _resolve_athaniel_bin()
        if not athaniel_cmd:
            return t("gateway.update.athaniel_cmd_not_found")

        pending_path = _athaniel_home / ".update_pending.json"
        output_path = _athaniel_home / ".update_output.txt"
        exit_code_path = _athaniel_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `athaniel update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `athaniel update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through athaniel_cmd (venv\Scripts\athaniel.exe): the shim
                ...
                athaniel_cmd_str = " ".join(shlex.quote(part) for part in athaniel_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {athaniel_cmd_str} update --gateway"
