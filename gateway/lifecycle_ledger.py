...
``<ATHANIEL_HOME>/state/gateway.lifecycle.json``:
...
def _process_athaniel_home() -> Path:
    """ATHANIEL_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<ATHANIEL_HOME>/state/gateway.lifecycle.json``."""
...
def check_state_db_integrity(home: Optional[Path] = None) -> str:
    ...
    ``_enforce_macos_synchronous_full`` in :mod:`athaniel_state` — macOS
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``athaniel_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
