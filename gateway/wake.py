...
* Stateless request/response adapters (the API server,
  ...
  ``X-Athaniel-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Athaniel-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Athaniel-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Athaniel-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-Athaniel-Session-Id is rejected (403) on an unauthenticated API "
    ...
    headers = {
        ...
        # WO-SESSION/YIELD-1: declare the machine class so the session-turn
        # lease can ask this turn to yield when an operator message waits.
        "X-Athaniel-Turn-Kind": "wake",
    }

    async def _post_once(post_text: str) -> bool:
        """Deliver one wake turn; True when the turn yielded to an operator."""
        payload = {
            "model": str(getattr(adapter, "_model_name", "") or "athaniel-brain"),
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as http:
                    async with http.post(url, json=payload, headers=headers) as resp:
                        ...
                        return resp.headers.get("X-Athaniel-Yielded") == "true"
    ...
    # WO-SESSION/YIELD-1: a wake turn that yielded to an operator message
    # checkpointed its progress into the session transcript and released the
    # lease early. Requeue the remainder — after a head-start delay so the
    # waiting operator turn (polling the lease at ~1s) acquires first — a
    # bounded number of times.
    # ponytail: fixed 3-resume cap and 8s head start; make them config if a
    # real workload ever exhausts them.
    _MAX_YIELD_RESUMES = 3
    post_text = text
    for resume in range(_MAX_YIELD_RESUMES + 1):
        if not await _post_once(post_text):
            return
        if resume == _MAX_YIELD_RESUMES:
            break
        logger.info(
            "wake turn for session %s yielded to an operator; requeueing "
            "remainder (resume %d/%d)",
            session_id,
            resume + 1,
            _MAX_YIELD_RESUMES,
        )
        ...
        post_text = (
            "RESUME: this background turn yielded the session to an operator "
            "message earlier and its progress so far is in the transcript. "
            "Continue the remaining work. Original notification: " + text
        )
    logger.warning(
        "wake turn for session %s still yielding after %d resumes; leaving "
        "the remainder to the transcript",
        session_id,
        _MAX_YIELD_RESUMES,
    )
