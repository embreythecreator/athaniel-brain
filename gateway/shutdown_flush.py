...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<athaniel_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active ATHANIEL_HOME."""
    from athaniel_constants import get_athaniel_home

    flush_dir = get_athaniel_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<athaniel_home>/pending_messages/``), so a runtime cap rotation no
