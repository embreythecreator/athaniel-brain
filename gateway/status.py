...
The PID file lives at ``{ATHANIEL_HOME}/gateway.pid``.  ATHANIEL_HOME defaults to
``~/.athaniel`` but can be overridden via the environment variable.  This means
separate ATHANIEL_HOME directories naturally get separate PID files — a property
...
from athaniel_constants import get_athaniel_home, _get_platform_default_athaniel_home
...
_GATEWAY_KIND = "athaniel-gateway"
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_athaniel_home() / "gateway-starts.log"
...
def _get_process_athaniel_home() -> Path:
    """Return the process-level ATHANIEL_HOME, skipping context-local overrides.
    ...
    ``get_athaniel_home()`` honors ``_ATHANIEL_HOME_OVERRIDE`` contextvar used for
...
def _canonical_athaniel_home(path: Path | str) -> Path:
    """Return a stable absolute ATHANIEL_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_athaniel_home(left: Path | str, right: Path | str) -> bool:
    """Compare ATHANIEL_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_athaniel_home(left))) == os.path.normcase(
        str(_canonical_athaniel_home(right))
    )


# Mirrors athaniel_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (athaniel_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a ATHANIEL_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.athaniel/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_athaniel_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_athaniel_home(canonical, get_default_athaniel_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_athaniel_home(canonical, _get_platform_default_athaniel_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    ATHANIEL_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``athaniel_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ATHANIEL_HOME."""
    home = _get_process_athaniel_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ATHANIEL_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "athaniel" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Athaniel' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Athaniel gateway lifecycle subcommand from a command line.
    ...
    test also matched ``athaniel_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\athaniel-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Athaniel's
    ...
    for token in tokens:
        ...
        if basename in ("athaniel-gateway", "athaniel-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "athaniel_cli.main" in joined
        or "athaniel_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("athaniel", "athaniel.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `athaniel gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Athaniel-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Athaniel gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a ATHANIEL_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.athaniel`` or ``$ATHANIEL_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``athaniel_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ATHANIEL_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a ATHANIEL_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"athaniel_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit ATHANIEL_HOME= on the argv. ATHANIEL_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "athaniel_home=" in command_lc and f"athaniel_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # ATHANIEL_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "athaniel_home": str(_canonical_athaniel_home(_get_process_athaniel_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``athaniel_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `athaniel update` (and the
...
def _pid_record_belongs_to_current_profile(
    ...
    """Return True when the PID record's ``athaniel_home`` matches the current process.

    PID records written by ``_build_pid_record()`` include the gateway's
    ``athaniel_home`` at write time. If a profile gateway was started (or recorded)
    under a different ATHANIEL_HOME, the record belongs to a different profile
    ...
    Records that predate the ``athaniel_home`` field (pre-#74872 gateways) are
    accepted conservatively (no field → assume current profile).
    """
    if not isinstance(record, dict):
        return False
    record_home = record.get("athaniel_home")
    if not record_home:
        # Records without athaniel_home belong to a pre-#74872 gateway;
        # accept them conservatively.
        return True
    return _same_athaniel_home(record_home, _get_process_athaniel_home())
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `athaniel status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ATHANIEL_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``athaniel_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ATHANIEL_HOME.  Pass it when validating *another* profile's state file (the
    ...
    # verify the persisted record's athaniel_home matches the current process.
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ATHANIEL_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from athaniel_home.
    profile = _profile_label_for_home(_get_process_athaniel_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``athaniel.service`` + ``athaniel-
# gateway.service``).
#
...
def _get_takeover_marker_path(athaniel_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``athaniel_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = athaniel_home or _get_process_athaniel_home()
    return _canonical_athaniel_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-ATHANIEL_HOME --replace while
    ...
    our_home = _get_process_athaniel_home()
    target_home = record.get("target_athaniel_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_athaniel_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_athaniel_home")
        if replacer_home is not None and not _same_athaniel_home(
    ...
    # ``athaniel gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's ATHANIEL_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_athaniel_home(
            target_home or _get_process_athaniel_home()
        )
        ...
        record = {
            ...
            "target_athaniel_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_athaniel_home": str(
                _canonical_athaniel_home(_get_process_athaniel_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target ATHANIEL_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("athaniel_home")
    if not isinstance(pid_record_home, str) or not _same_athaniel_home(
