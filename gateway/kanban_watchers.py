...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`athaniel pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                ...
                def _collect():
                    ...
                    # ATHANIEL_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `athaniel kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours ATHANIEL_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        try:
            ...
            return
        env_override = os.environ.get("ATHANIEL_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via ATHANIEL_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `athaniel kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Athaniel profile
            ...
            PATH, missing venv, credential loss for a real Athaniel profile).
            """
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from athaniel_cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("ATHANIEL_KANBAN_BOARD")
                try:
                    os.environ["ATHANIEL_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("ATHANIEL_KANBAN_BOARD", None)
                    else:
                        os.environ["ATHANIEL_KANBAN_BOARD"] = prev_env
            return successes
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`athaniel pause`): skip auto-decompose
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`athaniel kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
