...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{ATHANIEL_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``ATHANIEL_HOME`` is a **durable** store — on Athaniel
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``athaniel gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting ATHANIEL_HOME."""
    base = home if home is not None else get_athaniel_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable ATHANIEL_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (ATHANIEL_HOME is a
    durable Fly volume on Athaniel Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    ATHANIEL_HOME volume, or an expired same-epoch orphan (#85433), is
