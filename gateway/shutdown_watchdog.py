...
2. An event-loop heartbeat file at ``<ATHANIEL_HOME>/state/gateway.heartbeat`` so
...
def _process_athaniel_home() -> Path:
    """ATHANIEL_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ATHANIEL_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_athaniel_home()
...
def get_loop_tick_socket_path(
    ...
    ``<ATHANIEL_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from athaniel_logging import drain_log_queue
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``athaniel_cli.gateway.probe_gateway_loop_liveness`` for the
