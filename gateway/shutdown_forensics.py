    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if athaniel_home_str:
            takeover_path = Path(athaniel_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(athaniel_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(
    ...
    upgraded athaniel-brain but never re-ran ``athaniel setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../athaniel-gateway.service"
    ...
    # that's the common case for athaniel.
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by athaniel_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
