...
#
# Wired into the image as /etc/cont-init.d/01-athaniel-setup by the
...
ATHANIEL_HOME="${ATHANIEL_HOME:-/opt/data}"
INSTALL_DIR="/opt/athaniel"

# Drop to athaniel via s6-setuidgid, but skip it when already non-root.
as_athaniel() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid athaniel "$@"; }
...
# starts non-root. The baked install tree under /opt/athaniel is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ATHANIEL_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ATHANIEL_UID/ATHANIEL_GID — or the PUID/PGID aliases — which the
...
# `id -u athaniel` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# athaniel UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u athaniel)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-athaniel UID).
...
and the baked /opt/athaniel install tree is intentionally root-owned and
...
# --- Bootstrap ATHANIEL_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid athaniel mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ATHANIEL_HOME paths whose parents only
# root can create (e.g. `ATHANIEL_HOME=/home/athaniel/.athaniel` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ATHANIEL_UID/ATHANIEL_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ATHANIEL_UID/
# ATHANIEL_GID still win when both are set.  See #15290, salvages #25872.
ATHANIEL_UID="${ATHANIEL_UID:-${PUID:-}}"
ATHANIEL_GID="${ATHANIEL_GID:-${PGID:-}}"

if [ -n "${ATHANIEL_UID:-}" ] && validate_uid_gid "$ATHANIEL_UID" && [ "$ATHANIEL_UID" != "$(id -u athaniel)" ]; then
    echo "[stage2] Changing athaniel UID to $ATHANIEL_UID"
    usermod -u "$ATHANIEL_UID" athaniel
fi
if [ -n "${ATHANIEL_GID:-}" ] && validate_uid_gid "$ATHANIEL_GID" && [ "$ATHANIEL_GID" != "$(id -g athaniel)" ]; then
    echo "[stage2] Changing athaniel GID to $ATHANIEL_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ATHANIEL_GID" athaniel 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised athaniel user (UID 10000)
...
# athaniel process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes athaniel. Idempotent across container
...
#   - athaniel is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G athaniel 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] athaniel already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" athaniel 2>/dev/null; then
        echo "[stage2] Added athaniel to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group athaniel failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ATHANIEL_UID is remapped or the top-level $ATHANIEL_HOME isn't owned by
# the runtime athaniel UID, restore ownership to athaniel — but ONLY for the
# directories athaniel actually writes to. The full $ATHANIEL_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of athaniel-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_athaniel_uid=$(id -u athaniel)

path_has_symlink_component() {
    path="$1"
    root="${2:-$ATHANIEL_HOME}"
...
chown_athaniel_tree() {
    ...
    chown -R athaniel:athaniel "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_athaniel_owner() {
    target="$1"
    find "$target" \( ! -user athaniel -o ! -group athaniel \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ATHANIEL_HOME" 2>/dev/null)" != "$actual_athaniel_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ATHANIEL_HOME (targeted) to athaniel ($actual_athaniel_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ATHANIEL_HOME: chown the directory itself (not its contents)
    # so athaniel can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ATHANIEL_HOME"; then
        :
    else
        chown athaniel:athaniel "$ATHANIEL_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ATHANIEL_HOME failed (rootless container?) — continuing"
    fi
    # Athaniel-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by athaniel (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ATHANIEL_HOME/$sub" ] && tree_has_non_athaniel_owner "$ATHANIEL_HOME/$sub"; then
            chown_athaniel_tree "$ATHANIEL_HOME/$sub"
        fi
    done
fi
...
# athaniel user. Hosted/container instances keep mutable state under
# $ATHANIEL_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ATHANIEL_DISABLE_LAZY_INSTALLS=1. Keeping /opt/athaniel root-owned and
...
# $ATHANIEL_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ATHANIEL_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to athaniel
...
# Always reset ownership of $ATHANIEL_HOME/profiles to athaniel on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> athaniel …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as athaniel and walks
...
if [ -d "$ATHANIEL_HOME/profiles" ] && tree_has_non_athaniel_owner "$ATHANIEL_HOME/profiles"; then
    chown_athaniel_tree "$ATHANIEL_HOME/profiles"
fi

# Always reset ownership of $ATHANIEL_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged athaniel runtime even
...
# gate as profiles/).
if [ -d "$ATHANIEL_HOME/cron" ] && tree_has_non_athaniel_owner "$ATHANIEL_HOME/cron"; then
    chown_athaniel_tree "$ATHANIEL_HOME/cron"
fi

# Always ensure logs/gateways is athaniel-owned (#45258). Formerly healed by
...
# top-level $ATHANIEL_HOME is mis-owned, so a warm volume with athaniel-owned
# ATHANIEL_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid athaniel mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as athaniel.
if [ -d "$ATHANIEL_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ATHANIEL_HOME/logs/gateways"; then
        :
    else
        chown athaniel:athaniel "$ATHANIEL_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# athaniel pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged athaniel gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ATHANIEL_HOME is
...
# and cron/.
if [ -d "$ATHANIEL_HOME/platforms/pairing" ] && tree_has_non_athaniel_owner "$ATHANIEL_HOME/platforms/pairing"; then
    chown_athaniel_tree "$ATHANIEL_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ATHANIEL_HOME/pairing" ] && tree_has_non_athaniel_owner "$ATHANIEL_HOME/pairing"; then
    chown_athaniel_tree "$ATHANIEL_HOME/pairing"
fi

# Reset ownership of athaniel-owned top-level state files on every boot.
# The targeted data-volume chown above only covers athaniel-owned
# *subdirectories*; loose state files living directly under $ATHANIEL_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> athaniel …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged athaniel runtime then hits
...
# sweep so host-owned files in a bind-mounted $ATHANIEL_HOME are never
...
# entries of athaniel_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ATHANIEL_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ATHANIEL_HOME/$f"; then
            :
        else
            chown athaniel:athaniel "$ATHANIEL_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the athaniel runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ATHANIEL_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ATHANIEL_HOME/config.yaml"; then
        :
    else
        chown athaniel:athaniel "$ATHANIEL_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ATHANIEL_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as athaniel user ---
# Run as athaniel via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ATHANIEL_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_athaniel mkdir -p \
    "$ATHANIEL_HOME/backups" \
    "$ATHANIEL_HOME/cron" \
    "$ATHANIEL_HOME/sessions" \
    "$ATHANIEL_HOME/logs" \
    "$ATHANIEL_HOME/logs/gateways" \
    "$ATHANIEL_HOME/hooks" \
    "$ATHANIEL_HOME/memories" \
    "$ATHANIEL_HOME/skills" \
    "$ATHANIEL_HOME/skins" \
    "$ATHANIEL_HOME/plans" \
    "$ATHANIEL_HOME/workspace" \
    "$ATHANIEL_HOME/home" \
    "$ATHANIEL_HOME/pairing" \
    "$ATHANIEL_HOME/platforms/pairing" \
    "$ATHANIEL_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/athaniel/.install_method (see Dockerfile), NOT written here into
# $ATHANIEL_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ATHANIEL_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.athaniel:/opt/data) and sometimes shared with a
...
# 'athaniel update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ATHANIEL_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/athaniel stamp, so deleting the data-dir copy is safe).
if [ -f "$ATHANIEL_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ATHANIEL_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ATHANIEL_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ATHANIEL_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ATHANIEL_HOME/$dest"; then
            :
        else
            as_athaniel cp "$INSTALL_DIR/$src" "$ATHANIEL_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/athaniel/.env.example being present
...
# not generate one. Athaniel loads $ATHANIEL_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ATHANIEL_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ATHANIEL_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ATHANIEL_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ATHANIEL_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ATHANIEL_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ATHANIEL_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ATHANIEL_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ATHANIEL_HOME/.env"; then
        :
    else
        if [ ! -f "$ATHANIEL_HOME/.env" ]; then
            ...
            (umask 077 && as_athaniel touch "$ATHANIEL_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ATHANIEL_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ATHANIEL_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ATHANIEL_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ATHANIEL_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ATHANIEL_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ATHANIEL_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ATHANIEL_HOME/.env"; then
        :
    else
        chown athaniel:athaniel "$ATHANIEL_HOME/.env" 2>/dev/null || true
        chmod 600 "$ATHANIEL_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `athaniel` user — without this it gets EACCES on every
...
# athaniel group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp athaniel /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted athaniel group access to the Fly Machines API socket"
    else
...
# $ATHANIEL_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `athaniel update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ATHANIEL_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ATHANIEL_HOME/config.yaml" ]; then
    s6-setuidgid athaniel "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ATHANIEL_HOME/auth.json" ] && [ -n "${ATHANIEL_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ATHANIEL_HOME/auth.json"; then
        :
    else
        printf '%s' "$ATHANIEL_AUTH_JSON_BOOTSTRAP" > "$ATHANIEL_HOME/auth.json"
        chown athaniel:athaniel "$ATHANIEL_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ATHANIEL_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ATHANIEL_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ATHANIEL_HOME/auth.json" ] && [ -n "${ATHANIEL_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ATHANIEL_HOME/auth.json"; then
        :
    else
        s6-setuidgid athaniel "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ATHANIEL_HOME/auth.json" \
...
# can set ATHANIEL_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ATHANIEL_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ATHANIEL_HOME/gateway_state.json" ] && \
        [ "${ATHANIEL_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ATHANIEL_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ATHANIEL_HOME/gateway_state.json"
        chown athaniel:athaniel "$ATHANIEL_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ATHANIEL_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_athaniel "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/athaniel/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Athaniel spawns for the
...
# main-wrapper.sh propagates it into the supervised ``athaniel`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-athaniel, dashboard, etc.).
