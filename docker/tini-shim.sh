...
# Athaniel WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ATHANIEL_TINI_SHIM_TARGET overrides the `/init` path (and
# ATHANIEL_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ATHANIEL_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ATHANIEL_TINI_SHIM_WRAPPER:-/opt/athaniel/docker/main-wrapper.sh}"
