...
Extracted verbatim from athaniel_state.py so the SessionDB mixin modules
(athaniel_state_search / athaniel_state_schema / athaniel_state_portability) can
reference them without importing athaniel_state (which would be a cycle).
athaniel_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``athaniel sessions optimize-storage``. A legacy DB sits at
...
CREATE TABLE IF NOT EXISTS session_turn_leases (
    ...
    acquired_at REAL NOT NULL,
    expires_at REAL NOT NULL,
    -- WO-SESSION/YIELD-1: holder class. 'operator' turns are real people;
    -- 'wake'/'background' turns (async-delegation self-posts) must yield the
    -- session when an operator waiter requests it.
    kind TEXT NOT NULL DEFAULT 'operator',
    -- Set by a waiting operator turn; polled by wake-class holders, which
    -- checkpoint and release within one API-call boundary.
...
# `athaniel sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `athaniel db optimize`. This is
...
#
# Several independent Athaniel processes routinely share one state.db (gateway
# service, the Desktop app's `athaniel serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `athaniel_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not athaniel_state)
# because the search/schema mixins cannot import athaniel_state (cycle).
#
...
logger = logging.getLogger("athaniel_state")
