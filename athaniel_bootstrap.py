"""Windows UTF-8 bootstrap for Athaniel entry points.
...
should be imported at the very top of every Athaniel entry point
(``athaniel``, ``athaniel-brain``, ``athaniel-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``athaniel_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``athaniel_bootstrap``, never ``athaniel_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Athaniel modules.

    Athaniel ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Athaniel source root
    ...
    """
    root = src_root or os.environ.get("ATHANIEL_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ATHANIEL_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ATHANIEL_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import athaniel_bootstrap``
# (or ``from athaniel_bootstrap import apply_windows_utf8_bootstrap``) at
