...
const BACKEND = process.env.ATHANIEL_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `athaniel dashboard` server injects a one-shot
 * session token into `index.html` (see `athaniel_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__ATHANIEL_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function athanielDevToken(): Plugin {
  const TOKEN_RE = /window\.__ATHANIEL_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__ATHANIEL_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "athaniel:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[athaniel] Could not find session token in ${BACKEND} — ` +
              `is \`athaniel dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__ATHANIEL_SESSION_TOKEN__="${match[1]}";` +
              `window.__ATHANIEL_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[athaniel] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`athaniel dashboard\` or set ATHANIEL_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@athaniel/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `athaniel dashboard` must serve these; Vite has no
