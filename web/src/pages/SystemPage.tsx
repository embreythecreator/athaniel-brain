...
import { AthanielConsoleModal } from "@/components/AthanielConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `athaniel import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_athaniel === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkAthanielUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_athaniel],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_athaniel === false) {
      showToast(
        "Athaniel updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateAthaniel();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "athaniel-update");
  ...
  const canUpdateAthaniel = status?.can_update_athaniel !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateAthaniel && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Athaniel?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'athaniel update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'athaniel update' (${updateInfo?.update_command ?? "athaniel update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Athaniel</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.athaniel_version}</span>
                  {canUpdateAthaniel &&
                    updateInfo &&
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">athaniel portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$ATHANIEL_HOME/backups/athaniel-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Athaniel backup?"
              description={`This will overwrite your current Athaniel configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Athaniel team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
