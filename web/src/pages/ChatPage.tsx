/**
 * ChatPage — embeds `athaniel --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (athaniel_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "athaniel.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ATHANIEL_SESSION_TOKEN__ &&
    !window.__ATHANIEL_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `athaniel dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("athaniel-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("athaniel-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ATHANIEL_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Athaniel TUI has enabled xterm mouse-events
    ...
    //      payloads upload to ATHANIEL_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to ATHANIEL_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[athaniel-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ATHANIEL_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ATHANIEL_HOME pointed at the
    ...
    // forwarding them into Athaniel. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="athaniel-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
