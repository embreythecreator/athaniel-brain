import { buildAthanielWebSocketUrl } from "@athaniel/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/athaniel/). The Python backend
// injects ``window.__ATHANIEL_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ATHANIEL_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ATHANIEL_BASE_PATH = readBasePath();
const BASE = ATHANIEL_BASE_PATH;
...
declare global {
  interface Window {
    __ATHANIEL_SESSION_TOKEN__?: string;
    __ATHANIEL_BASE_PATH__?: string;
    ...
    __ATHANIEL_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Athaniel-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ATHANIEL_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``athaniel update``, ``athaniel gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ATHANIEL_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ATHANIEL_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ATHANIEL_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``athaniel_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ATHANIEL_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Athaniel-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``athaniel_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ATHANIEL_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildAthanielWebSocketUrl({
...
export const api = {
  ...
  updateAthaniel: () =>
    fetchJSON<ActionResponse>("/api/athaniel/update", { method: "POST" }),
  checkAthanielUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/athaniel/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** athaniel-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``athaniel update``. */
  can_update_athaniel?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ATHANIEL_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ATHANIEL_HOME volume — no staleness dimension, so no
 * sampled_at. */
