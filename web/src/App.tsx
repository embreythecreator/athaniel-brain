...
const SIDEBAR_COLLAPSED_KEY = "athaniel-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="athaniel-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="athaniel-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateAthaniel = status?.can_update_athaniel === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'athaniel update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "athaniel update";
    return (
      t.status.updateAthanielConfirmMessage ??
      `This will run 'athaniel update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateAthanielConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateAthaniel) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateAthaniel,
      runningLabel: t.status.updatingAthaniel,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Athaniel gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateAthanielConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateAthanielConfirmTitle ?? `${t.status.updateAthaniel}?`}
    />
    </>
  );
}
