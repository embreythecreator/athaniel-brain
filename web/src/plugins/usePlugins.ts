...
import { api, ATHANIEL_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "athaniel:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ATHANIEL_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ATHANIEL_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?athaniel_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-athaniel-plugin", manifest.name);
