...
interface AthanielConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function AthanielConsoleModal({ open, onClose }: AthanielConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("athaniel> ");
  const inputPromptRef = useRef("athaniel> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "athaniel> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Athaniel Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="athaniel-console-title"
    >
      <div
        className={cn(
          themedBody,
