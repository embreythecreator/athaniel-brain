# Behavioral test for install.ps1's athaniel launcher staging (PR #92092,
...
#
# The test lifts the real Install-AthanielCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $AthanielHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-AthanielCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-AthanielCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'athaniel-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'athaniel-brain'
    ...
    $missingThrew = $false
    try {
        Install-AthanielCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing athaniel.exe fails the launcher stage'
    ...
    $athanielV1 = [byte[]](77, 90, 1)
    $athanielV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'athaniel.exe'), $athanielV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-AthanielCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $athanielV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'athaniel.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'athaniel-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'athaniel.exe'), $athanielV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'athaniel-acp.exe'), $acp)
    Install-AthanielCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $athanielV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'athaniel.exe'))) `
        'installer refreshes an existing Athaniel launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'athaniel-acp.exe'))) `
    ...
    Install-AthanielCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'athaniel.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'athaniel.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'athaniel.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'athaniel.exe')) -and $cmdBody.Contains('%*')) `
