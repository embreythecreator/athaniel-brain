...
# tree generates llms.txt — the index every LLM (Athaniel included, via the
# athaniel-brain skill) reads to learn what Athaniel can do — and
...
_MCP_CATALOG_FILES = {"athaniel_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Athaniel-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``athaniel serve`` backend, the Docker image)
