...
# changing PATH, installing software, or touching the user's Athaniel home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("athaniel-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$AthanielHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -AthanielHome $AthanielHome -InstallDir $InstallDir
