#!/usr/bin/env bash
# Canonical test runner for athaniel-brain. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.athaniel/...)
#
# Usage:
...
# (ATHANIEL_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.athaniel/athaniel-brain/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.athaniel/athaniel-brain/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ATHANIEL_PYTHON:-}" ] && [ -x "$ATHANIEL_PYTHON" ] \
    && "$ATHANIEL_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ATHANIEL_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `athaniel` binary rather
  # than the devShell hook.
  PYTHON="$ATHANIEL_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ATHANIEL_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ATHANIEL_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.athaniel/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.athaniel"
...
#
#   * ATHANIEL_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ATHANIEL_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ATHANIEL_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ATHANIEL_RUN_SLOW_PET_TESTS / ATHANIEL_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ATHANIEL_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ATHANIEL_TEST_IMAGE ATHANIEL_TEST_WORKERS ATHANIEL_TEST_PATHS \
  ATHANIEL_TEST_FILE_TIMEOUT ATHANIEL_TEST_FILE_RETRIES ATHANIEL_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ATHANIEL_RUN_SLOW_PET_TESTS:+ATHANIEL_RUN_SLOW_PET_TESTS="$ATHANIEL_RUN_SLOW_PET_TESTS"} \
  ${ATHANIEL_E2E_BROWSER:+ATHANIEL_E2E_BROWSER="$ATHANIEL_E2E_BROWSER"} \
