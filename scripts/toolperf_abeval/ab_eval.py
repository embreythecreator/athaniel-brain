...
measured in the production session DB) through `athaniel chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ATHANIEL_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Athaniel home with credentials for the
...
(tracker: embreythecreator/athaniel-brain#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "ATHANIEL_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "athaniel_cli.main", "chat", "--query", q,
