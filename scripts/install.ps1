# ============================================================================
# Athaniel Brain Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://athaniel-brain.embreythecreator.com/install.ps1)
#
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$AthanielHome = $(if ($env:ATHANIEL_HOME) { $env:ATHANIEL_HOME } else { "$env:LOCALAPPDATA\athaniel" }),
    [string]$InstallDir = $(if ($env:ATHANIEL_HOME) { "$env:ATHANIEL_HOME\athaniel-brain" } else { "$env:LOCALAPPDATA\athaniel\athaniel-brain" }),
    ...
    # builds apps/desktop into a launchable Athaniel.exe.
    #
    # Why opt-in:
    #   * Athaniel-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Athaniel.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Athaniel.exe
    ...
    #     `athaniel desktop` already builds on demand.
...
# ATHANIEL_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[athaniel] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'AthanielInstall.LongPath').Type) {
            Add-Type -Namespace 'AthanielInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [AthanielInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -AthanielHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('AthanielHome')) {
    $AthanielHome = ConvertTo-LongPath $AthanielHome
} else {
    $AthanielHome = ConvertTo-LongPath $(
        if ($env:ATHANIEL_HOME) { $env:ATHANIEL_HOME } else { "$env:LOCALAPPDATA\athaniel" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ATHANIEL_HOME) { "$env:ATHANIEL_HOME\athaniel-brain" } else { "$env:LOCALAPPDATA\athaniel\athaniel-brain" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: AthanielHome=$AthanielHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:embreythecreator/athaniel-brain.git"
$RepoUrlHttps = "https://github.com/embreythecreator/athaniel-brain.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Athaniel Brain Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Athaniel.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AthanielHome)) {
        New-Item -ItemType Directory -Force -Path $AthanielHome | Out-Null
    }
    $envFile = Join-Path $AthanielHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `athaniel update`'s npx
    ...
    $prefixDir = Join-Path $AthanielHome "node"
...
function Install-Uv {
    # Athaniel owns its own uv at $AthanielHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (athaniel_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `athaniel update` stay in sync.
    $managedUv = Join-Path $AthanielHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AthanielHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AthanielHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AthanielHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AthanielHome "bin"
        ...
        # (athaniel_cli/managed_uv.py looks only at $AthanielHome\bin\uv.exe).
...
# Put the Athaniel-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone athaniel-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Athaniel-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Athaniel-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("athaniel-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ATHANIEL_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Athaniel-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Athaniel setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Athaniel uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\athaniel\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Athaniel needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Athaniel copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\athaniel\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ATHANIEL_GIT_BASH_PATH`` (User scope) so Athaniel can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Athaniel-managed PortableGit install instead..."
    }

    # Download PortableGit into $AthanielHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AthanielHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Athaniel features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Athaniel needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ATHANIEL_GIT_BASH_PATH`` (User env scope) so Athaniel can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AthanielHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AthanielHome\git\usr\bin\bash.exe
    $candidates += "$AthanielHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AthanielHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ATHANIEL_GIT_BASH_PATH", $candidate, "User")
            $env:ATHANIEL_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ATHANIEL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Athaniel may not find Git Bash."
    Write-Info "If needed, set ATHANIEL_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Athaniel-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Athaniel requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Athaniel requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Athaniel-managed Node.js installation instead..."

    # Prefer a Athaniel-managed Node from a previous run over a too-old system one.
    $managedNode = "$AthanielHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AthanielHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$AthanielHome\node"
        Write-Success "Node.js $version found (Athaniel-managed)"
        ...
        Update-ManagedNpm "$AthanielHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Athaniel-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AthanielHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AthanielHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\athaniel-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$AthanielHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$AthanielHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$AthanielHome\node") {
                    try {
                        Rename-Item "$AthanielHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Athaniel-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$AthanielHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AthanielHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\athaniel-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `athaniel
                    # update` path (#4735).
                    ...
                    $stashName = "athaniel-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. athaniel-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``athaniel update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Athaniel behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running athaniel processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/embreythecreator/athaniel-brain/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/embreythecreator/athaniel-brain/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/embreythecreator/athaniel-brain/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\athaniel-brain-$zipLabel.zip"
                $extractPath = "$env:TEMP\athaniel-brain-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `athaniel update` (see the notes at the shared clone-path
    ...
    # next `athaniel update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Athaniel-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running athaniel process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running athaniel processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Athaniel_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (athaniel.exe) plus its child tree.
            & taskkill /F /T /IM athaniel.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM athaniel.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m athaniel_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not athaniel.exe.
            ...
            # Athaniel children re-exec through .athaniel-runtime, so killing only
            ...
            # user (see athaniel_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Athaniel " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `athaniel update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Athaniel (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `athaniel.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'athaniel-brain\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install athaniel-brain package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and athaniel.exe broken with
    ...
    # here, not 30 seconds later when the user runs `athaniel`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Athaniel processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Athaniel processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Athaniel and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register athaniel.exe in dist-info/RECORD but fail to
        ...
        # `athaniel` command while athaniel-brain.exe / athaniel-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m athaniel_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `athaniel dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\athaniel_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `athaniel dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `athaniel dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: athaniel_cli/web_server.py"
        }
    }
...
function Install-AthanielCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the athaniel launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring athaniel.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "athaniel.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the athaniel command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # athaniel_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("athaniel", "athaniel-acp")) {
    ...
    $requiredExe = Join-Path $Destination "athaniel.exe"
    $requiredCmd = Join-Path $Destination "athaniel.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the athaniel command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up athaniel command..."
    
    if ($NoVenv) {
        $athanielBin = "$InstallDir"
    } else {
        # $AthanielHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `athaniel update`'s autostash
        ...
        # installer staged under athaniel-brain\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-AthanielCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $athanielBin = "$AthanielHome\bin"
        Install-AthanielCommandLaunchers -Root $InstallDir -Destination $athanielBin | Out-Null
    }
    ...
    #   athaniel-brain\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The athaniel-brain\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept athaniel via $athanielBin)"
        }
    }
    
    if ($currentPath -notlike "*$athanielBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$athanielBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $athanielBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ATHANIEL_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\athaniel instead
    # of the Unix default ~/.athaniel
    $currentAthanielHome = [Environment]::GetEnvironmentVariable("ATHANIEL_HOME", "User")
    if (-not $currentAthanielHome -or $currentAthanielHome -ne $AthanielHome) {
        [Environment]::SetEnvironmentVariable("ATHANIEL_HOME", $AthanielHome, "User")
        Write-Success "Set ATHANIEL_HOME=$AthanielHome"
    }
    $env:ATHANIEL_HOME = $AthanielHome
    
    # Update current session
    $env:Path = "$athanielBin;$env:Path"
    
    Write-Success "athaniel command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.athaniel-bootstrap-complete which tells the Athaniel
    ...
    # Athaniel-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `athaniel update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".athaniel-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ATHANIEL_HOME directory structure ($AthanielHome, default %LOCALAPPDATA%\athaniel)
    New-Item -ItemType Directory -Force -Path "$AthanielHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AthanielHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AthanielHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AthanielHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Athaniel's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$AthanielHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in athaniel_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $AthanielHome"
    
    # Seed bundled skills into $AthanielHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AthanielHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $AthanielHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AthanielHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AthanielHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Athaniel-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'athaniel setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\athaniel-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\athaniel-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\athaniel-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `athaniel tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $AthanielHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Athaniel' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Athaniel' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'athaniel tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `athaniel
...
# paths still lazy-install via install_cua_driver() (athaniel_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `athaniel computer-use install` runs. Bounded
        ...
        # Athaniel' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'athaniel tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: athaniel computer-use install"
            }
        } else {
...
# the final `electron` -> `Athaniel` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Athaniel.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_athaniel_desktop command
    # resolves apps/desktop/release/win-unpacked/Athaniel.exe directly,
    ...
    # enforces the supported Node lines and prepends the Athaniel-managed Node to
    ...
    # The --dir mode produces an unpacked Athaniel.exe in
    ...
    # are NOT trying to work around). The Athaniel icon + product name are
    # stamped onto Athaniel.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\athaniel-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Athaniel.exe",
        "$desktopDir\release\win-arm64-unpacked\Athaniel.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Athaniel.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Athaniel icon + identity are stamped onto Athaniel.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, athaniel-brain#38216).
    ...
    #    Athaniel.exe. We deliberately do NOT point them at `athaniel desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Athaniel.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Athaniel.lnk')
        )
        ...
        # with the Athaniel icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.athaniel/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'athaniel setup'."
        return
    }
    ...
    # Run athaniel setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m athaniel_cli.main setup
    } else {
        python -m athaniel_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $athanielCmd = "$InstallDir\venv\Scripts\athaniel.exe"
    if (-not (Test-Path $athanielCmd)) {
        $athanielCmd = "athaniel"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AthanielHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'athaniel whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: athaniel gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AthanielHome\logs\gateway.log"
            Start-Process -FilePath $athanielCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AthanielHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: athaniel gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: athaniel gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AthanielHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AthanielHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AthanielHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AthanielHome\athaniel-brain\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   athaniel              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   athaniel setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   athaniel config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   athaniel config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   athaniel gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   athaniel update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Athaniel repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Athaniel-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Athaniel to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://athaniel-brain.embreythecreator.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
