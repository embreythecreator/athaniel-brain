...
unprivileged `athaniel` user, giving the layout most people have —
$ATHANIEL_HOME/athaniel-brain plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  ATHANIEL_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .athaniel-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `athaniel setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${ATHANIEL_DEV_SANDBOX_UPSTREAM:-https://github.com/embreythecreator/athaniel-brain.git}"
...
GIT_ROOT="${ATHANIEL_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${ATHANIEL_DEV_SANDBOX_DIR:-.athaniel-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t athaniel-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t athaniel-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/athaniel-brain.nousresearch.com"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/athaniel-brain.nousresearch.com/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/athaniel-brain.nousresearch.com/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://athaniel-brain.nousresearch.com/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/athaniel-brain.git update-ref refs/heads/main "$next_main"; then
...
if [ "$RUN_AS_USER" = true ]; then
  SANDBOX_UID=1000
  SANDBOX_GID=1000
  SANDBOX_USER=athaniel
  SANDBOX_HOME=/home/athaniel
else
  SANDBOX_UID=0
  SANDBOX_GID=0
  SANDBOX_USER=root
...
# A user-level install writes the `athaniel` launcher to ~/.local/bin and the
# checkout to $ATHANIEL_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/athaniel-brain.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q athaniel-brain.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t athaniel-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Athaniel sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/athaniel-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=Athaniel dev sandbox CA' \
