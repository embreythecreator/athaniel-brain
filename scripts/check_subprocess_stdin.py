...
When Athaniel runs in TUI mode, the gateway child process communicates with
...
# ``get_athaniel_home() / "plugins"`` (user) and ``./.athaniel/plugins/`` (project,
# gated behind ``ATHANIEL_ENABLE_PROJECT_PLUGINS``) — see
# ``athaniel_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``athaniel_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import athaniel_constants (this script
    ...
    # plugins in get_athaniel_home()/plugins/ and project plugins in
    # ./.athaniel/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_athaniel_home() / "plugins"]
    if os.environ.get("ATHANIEL_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".athaniel" / "plugins")
