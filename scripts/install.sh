#!/bin/bash
# ============================================================================
# Athaniel Brain Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://athaniel-brain.embreythecreator.com/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Athaniel terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:embreythecreator/athaniel-brain.git"
REPO_URL_HTTPS="https://github.com/embreythecreator/athaniel-brain.git"
ATHANIEL_HOME="${ATHANIEL_HOME:-$HOME/.athaniel}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ATHANIEL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ATHANIEL_INSTALL_DIR"
...
#   code at /usr/local/lib/athaniel-brain, command at /usr/local/bin/athaniel,
#   data still at /root/.athaniel (ATHANIEL_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ATHANIEL_HOME/.no-bundled-skills so future"
            echo "                   'athaniel update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Athaniel.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.athaniel/athaniel-brain"
            echo "                   default (root, Linux): /usr/local/lib/athaniel-brain"
            echo "  --athaniel-home PATH  Data directory (default: ~/.athaniel, or \$ATHANIEL_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Athaniel installs the code under"
            echo "  /usr/local/lib/athaniel-brain and links the command into"
            echo "  /usr/local/bin/athaniel (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ATHANIEL_HOME"
            echo "  (default /root/.athaniel).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ATHANIEL_HOME/athaniel-brain are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Athaniel Brain Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Athaniel.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `athaniel update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Athaniel-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Athaniel Brain","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install athaniel command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `athaniel` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ATHANIEL_HOME/athaniel-brain
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ATHANIEL_HOME/athaniel-brain
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/athaniel-brain
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ATHANIEL_HOME/athaniel-brain — then preserve it)
#
# Always no-op when the user set --dir or $ATHANIEL_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ATHANIEL_HOME.
    if is_termux; then
        INSTALL_DIR="$ATHANIEL_HOME/athaniel-brain"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ATHANIEL_HOME/athaniel-brain/.git" ]; then
            INSTALL_DIR="$ATHANIEL_HOME/athaniel-brain"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/athaniel-brain)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/athaniel-brain"
        ...
        # /usr/local/bin/athaniel wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/athaniel"
        log_info "  Data:    $ATHANIEL_HOME (unchanged)"
...
# Point a Athaniel-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ATHANIEL_HOME/node/bin — which is NOT on
...
# Athaniel's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Athaniel-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ATHANIEL_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ATHANIEL_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ATHANIEL_HOME/node/etc/npmrc"
}

get_athaniel_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/athaniel" ]; then
        echo "$link_dir/athaniel"
    else
        echo "athaniel"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://athaniel-brain.embreythecreator.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Athaniel owns its own uv at $ATHANIEL_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (athaniel_cli/managed_uv.py) looks in the same
    # place, so install.sh and `athaniel update` stay in sync.
    local _managed_uv="$ATHANIEL_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ATHANIEL_HOME/bin ..."
    mkdir -p "$ATHANIEL_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/athaniel-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/athaniel-uv-installer.$$.sh")"
    ...
    # directly into $ATHANIEL_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ATHANIEL_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# replaced with the Athaniel-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Athaniel-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Athaniel-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Athaniel-managed Node from a previous run over a too-old system one.
    if [ -x "$ATHANIEL_HOME/node/bin/node" ] && [ -x "$ATHANIEL_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$ATHANIEL_HOME/node/bin/node" --version)"; then
        export PATH="$ATHANIEL_HOME/node/bin:$PATH"
        log_success "Node.js $("$ATHANIEL_HOME/node/bin/node" --version) found (Athaniel-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Athaniel-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Athaniel requires Node 22.22+, 24.11+, or 26+) — installing Athaniel-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.athaniel/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.athaniel/node/..."
    ...
    # Place into ~/.athaniel/node/ and symlink binaries into the same bin dir
    # the athaniel command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ATHANIEL_HOME/node"
    mkdir -p "$ATHANIEL_HOME"
    mv "$extracted_dir" "$ATHANIEL_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$ATHANIEL_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ATHANIEL_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ATHANIEL_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$ATHANIEL_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.athaniel/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Athaniel install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Athaniel Brain itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `athaniel update` path
                # (#4735).
                ...
                local stash_name
                stash_name="athaniel-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``athaniel update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Athaniel behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up athaniel command..."

    if [ "$USE_VENV" = true ]; then
        ATHANIEL_BIN="$INSTALL_DIR/venv/bin/python"
        ATHANIEL_ENTRYPOINT="$INSTALL_DIR/athaniel"
    else
        ATHANIEL_BIN="$(which athaniel 2>/dev/null || echo "")"
        if [ -z "$ATHANIEL_BIN" ]; then
            log_warn "athaniel not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$ATHANIEL_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ATHANIEL_ENTRYPOINT" ]; }; then
        log_warn "Athaniel launcher prerequisites not found"
    ...
    # Create a user-facing shim for the athaniel command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ATHANIEL_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ATHANIEL_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/athaniel"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/athaniel" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHANIEL_BIN" "$ATHANIEL_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/athaniel" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/athaniel"
    log_success "Installed athaniel launcher → $command_link_display_dir/athaniel"

    # Also expose `athaniel-brain`. The `athaniel-brain` console script declared in
    ...
    rm -f "$command_link_dir/athaniel-brain"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/athaniel-brain" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHANIEL_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/athaniel-brain" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHANIEL_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/athaniel-brain"
    log_success "Installed athaniel-brain launcher → $command_link_display_dir/athaniel-brain"

    # Also expose `athaniel-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `athaniel-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Athaniel as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/athaniel-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/athaniel-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHANIEL_BIN" "$ATHANIEL_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/athaniel-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATHANIEL_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/athaniel-acp"
    log_success "Installed athaniel-acp launcher → $command_link_display_dir/athaniel-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "athaniel command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v athaniel' \
                >/dev/null 2>&1; then
        ...
        log_info "athaniel not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Athaniel Brain — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Athaniel Brain — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Athaniel Brain — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so athaniel works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.athaniel directory structure (config at top level, code in subdir)
    mkdir -p "$ATHANIEL_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.athaniel/.env (top level, easy to find)
    if [ ! -f "$ATHANIEL_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ATHANIEL_HOME/.env"
            log_success "Created ~/.athaniel/.env from template"
        else
            touch "$ATHANIEL_HOME/.env"
            log_success "Created ~/.athaniel/.env"
        fi
    else
        log_info "~/.athaniel/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ATHANIEL_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.athaniel/config.yaml (top level, easy to find)
    if [ ! -f "$ATHANIEL_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ATHANIEL_HOME/config.yaml"
            log_success "Created ~/.athaniel/config.yaml from template"
        fi
    else
        log_info "~/.athaniel/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in athaniel_cli/default_soul.py — the
    ...
    if [ ! -f "$ATHANIEL_HOME/SOUL.md" ]; then
        cat > "$ATHANIEL_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.athaniel/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.athaniel/"

    # Seed bundled skills into ~/.athaniel/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `athaniel update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'athaniel update'." \
            > "$ATHANIEL_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ATHANIEL_HOME/.no-bundled-skills"
        log_info "  Future 'athaniel update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.athaniel/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.athaniel/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ATHANIEL_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ATHANIEL_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.athaniel/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$ATHANIEL_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Athaniel Brain browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Athaniel will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Athaniel Brain browser tools — explicit browser override."
...
# install — the same closure `athaniel update` installs
# (athaniel_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `athaniel update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `athaniel tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$ATHANIEL_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Athaniel' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$ATHANIEL_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'athaniel tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `athaniel tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `athaniel computer-use install` on their own).
    ...
    # Same upstream installer `athaniel computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Athaniel install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Athaniel'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'athaniel tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: athaniel computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'athaniel setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m athaniel_cli.main setup < /dev/tty
    else
        python -m athaniel_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ATHANIEL_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Athaniel to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ATHANIEL_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'athaniel whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ATHANIEL_CMD="$(get_athaniel_command_path)"
                $ATHANIEL_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'athaniel whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'athaniel gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ATHANIEL_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ATHANIEL_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: athaniel gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: athaniel gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ATHANIEL_CMD gateway > "$ATHANIEL_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.athaniel/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: athaniel gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: athaniel gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.athaniel-bootstrap-complete, which tells the Athaniel
    ...
    local marker_path="$INSTALL_DIR/.athaniel-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ATHANIEL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ATHANIEL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ATHANIEL_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}athaniel${NC}              Start chatting"
    echo -e "   ${GREEN}athaniel setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}athaniel config${NC}       View/edit configuration"
    echo -e "   ${GREEN}athaniel config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}athaniel gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}athaniel update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'athaniel' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'athaniel' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'athaniel' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ATHANIEL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ATHANIEL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ATHANIEL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ATHANIEL_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Athaniel` rename
...
# _purge_electron_build_cache() used by `athaniel desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Athaniel-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `athaniel desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Athaniel" ]; then
            app="$desktop_dir/release/linux-unpacked/Athaniel"
        elif [ -x "$desktop_dir/release/linux-unpacked/athaniel" ]; then
            app="$desktop_dir/release/linux-unpacked/athaniel"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Athaniel.app" \
            "$desktop_dir/release/mac/Athaniel.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `athaniel desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ATHANIEL_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ATHANIEL_HOME="$ATHANIEL_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from athaniel_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `athaniel update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Athaniel-managed Node
            # provisioned during prerequisites/node-deps (at $ATHANIEL_HOME/node/bin)
        ...
        complete)
            ...
            # $ATHANIEL_HOME. $ATHANIEL_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'athaniel update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ATHANIEL_HOME.
    # $ATHANIEL_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'athaniel update' on this host install.
