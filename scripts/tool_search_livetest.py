#!/usr/bin/env python3
"""Live test harness for Athaniel Brain's Tool Search feature.
...
# Force-isolate the test environment BEFORE any athaniel imports.
ORIGINAL_HOME = os.environ.get("ATHANIEL_HOME")
ORIGINAL_AUTH = Path.home() / ".athaniel" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.athaniel/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.athaniel/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="athaniel_ts_live_"))
    athaniel_home = home_dir / ".athaniel"
    athaniel_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, athaniel_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".athaniel" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, athaniel_home / ".env")
        ...
        load_athaniel_dotenv(athaniel_home=str(Path.home() / ".athaniel"))
    ...
    (athaniel_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new ATHANIEL_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "athaniel_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ATHANIEL_HOME"] = str(home)
...
def main():
    ...
    # Restore original ATHANIEL_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ATHANIEL_HOME"] = ORIGINAL_HOME
    else:
