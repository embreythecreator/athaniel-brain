"""Run a real Athaniel CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_athaniel_executable(athaniel_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "athaniel",
        Path(".venv") / "Scripts" / "athaniel.exe",
    ):
        candidate = athaniel_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("athaniel")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Athaniel executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Athaniel source checkout containing .venv/bin/athaniel",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ATHANIEL_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["athaniel.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['athaniel.client.active']}"
        )
    [model] = by_name["athaniel.model_route.count"]
    expected_model = {
        "name": "athaniel.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['athaniel.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["athaniel.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['athaniel.task_run.started']}"
        )
    [terminal] = by_name["athaniel.task_run.finished"]
    ...
    [tool] = by_name["athaniel.tool_call.count"]
    ...
    lifecycle = by_name["athaniel.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["athaniel.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Athaniel development environment requires jsonschema"
        ) from exc
    ...
    if metrics["athaniel.client.active"] != [
        {
            "name": "athaniel.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['athaniel.client.active']}"
        )
    [model] = metrics["athaniel.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['athaniel.model_route.count']}"
        )
    [terminal] = metrics["athaniel.task_run.finished"]
    ...
    [tool] = metrics["athaniel.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["athaniel.skill.lifecycle.count"]
    ...
    loads = metrics["athaniel.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    athaniel_repo = args.athaniel_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    athaniel = _resolve_athaniel_executable(athaniel_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="athaniel-relay-shared-metrics-"))
    home = root / "athaniel-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["ATHANIEL_HOME"] = str(home)
        python_paths = [str(athaniel_repo)]
    ...
    (root / "athaniel.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "athaniel.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Athaniel exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Athaniel model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Athaniel did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Athaniel did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from athaniel_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "athaniel.shared_metrics.v2.schema.json",
    )

    print("Athaniel -> NeMo Relay shared-metrics smoke test passed")
