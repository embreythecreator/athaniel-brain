...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Athaniel\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (athaniel-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "athaniel@nousresearch.com",
    "athaniel-audit@example.com",
    "nousbot@nousresearch.com",
    "athaniel@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "embreythecreator/athaniel-brain",
                "--state", "merged",
