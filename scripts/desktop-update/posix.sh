...
#
# The whole job: wait for the Desktop to exit, run `athaniel update`, tell the
...
# the ATHANIEL_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (ATHANIEL_HOME/athaniel-brain)
...
ATHANIEL_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
ATHANIEL_HOME="${ATHANIEL_HOME:-${TMPDIR:-/tmp}}"
MARKER="$ATHANIEL_HOME/.athaniel-update-in-progress"
LOG_DIR="$ATHANIEL_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$ATHANIEL_HOME/.athaniel-update-result.json"
STATUS="${TMPDIR:-/tmp}/athaniel-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"Athaniel update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "Athaniel update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="Athaniel update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "Athaniel update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
find_browser() {
  ...
  # email included — inside this window that is titled "Athaniel" (#88410).
...
start_ui() {
  ...
  # teardown TERM killed the shim ~1s into `athaniel update` (2026-08-14 16:44,
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/athaniel-update-ui-$$" \
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${ATHANIEL_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen Athaniel to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/Athaniel.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/Athaniel.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall Athaniel (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE Athaniel also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen Athaniel to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${ATHANIEL_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${ATHANIEL_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `athaniel update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch Athaniel while the
...
# so the shim server and the later `athaniel update` subprocess both inherit
...
# update_lock.py (the `athaniel update` child adopts it via process ancestry).
# The Desktop supplies one acquisition time for the whole ownership chain.
NOW="$(date +%s)"
STARTED_AT="${ATHANIEL_UPDATE_STARTED_AT:-$NOW}"
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the Athaniel window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close Athaniel fully and try again."
...
ATHANIEL_BIN="$INSTALL_ROOT/venv/bin/athaniel"
[ -x "$ATHANIEL_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $ATHANIEL_BIN is missing. The install needs repair (run the Athaniel installer or athaniel doctor)."; log "$FINAL_MSG"; exit 3; }

# Run FROM the install root: `athaniel update` resolves the tree it mutates
...
# "close all Athaniel windows" sentinel.
KEEP_STASH=""
if "$ATHANIEL_BIN" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed athaniel predates --keep-stash; running without it"
fi
log "running: athaniel update --yes --gateway $KEEP_STASH --branch $BRANCH"
publish_stage "Updating code and dependencies"
OUT="$("$ATHANIEL_BIN" update --yes --gateway $KEEP_STASH --branch "$BRANCH" 2>&1)"; CODE=$?
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "athaniel update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all Athaniel windows") is not retryable.
  #
  ...
  if printf '%s' "$OUT" | grep -q "CODE UPDATE SKIPPED"; then
    log "athaniel update skipped (checkout parked on a non-target branch); not retrying"
    FINAL_CODE=8
...
# Truthful completion: `athaniel update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside athaniel update; retrying build"
  publish_stage "Rebuilding Desktop"
  "$ATHANIEL_BIN" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run athaniel desktop --force-build from a terminal to retry."
    exit 6
  }
fi

if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
else FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run athaniel debug share in a terminal to send a report."; fi
exit "$FINAL_CODE"
