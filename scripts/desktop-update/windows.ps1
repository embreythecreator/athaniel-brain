...
# (%ATHANIEL_HOME%\athaniel-setup.exe). That binary has no self-update path --
# copy_self_to_athaniel_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `athaniel update` refreshes
...
#     -InstallRoot <path>   repo checkout (ATHANIEL_HOME\athaniel-brain)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Athaniel.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .athaniel-update-in-progress in place (tests)
#
...
# .athaniel-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim ATHANIEL_HOME\.athaniel-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining ATHANIEL_UPDATE_STARTED_AT from the Desktop hand-off.
# athaniel_cli/update_lock.py's ancestry rule lets our
# `athaniel update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Athaniel.exe pid.
try {
    Add-Type -Namespace AthanielHandoff -Name Win32 -MemberDefinition @'
...
$AthanielHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $AthanielHome ".athaniel-update-in-progress"
$LogDir = Join-Path $AthanielHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $AthanielHome ".athaniel-update-result.json"
$script:Ui = $null
$script:UiStage = "Athaniel will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale athaniel-update-ui-* leftovers from interrupted
    ...
    # update marker (.athaniel-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "athaniel-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-AthanielStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("athaniel-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [AthanielHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `athaniel debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"athaniel debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [AthanielHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Athaniel themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Athaniel.exe before writing the replacement,
    ...
    # DO NOT spawn Athaniel.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [AthanielHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [AthanielHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [AthanielHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Athaniel process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [AthanielHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `athaniel update` deliberately runs its build steps with stdout
# inherited (athaniel_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:ATHANIEL_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:ATHANIEL_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:ATHANIEL_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:ATHANIEL_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `athaniel update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (athaniel_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:ATHANIEL_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:ATHANIEL_UPDATE_PROGRESS_LOG
}
...
if (-not ("AthanielUpdateJob" -as [type])) {
...
public static class AthanielUpdateJob {
...
function Invoke-AthanielStep([string]$Exe, [string[]]$AthanielArgs, [string]$Tag) {
    ...
    # afterwards, where `athaniel debug share` picks it up.
    #
    ...
    # .athaniel-update-result.json, clearing .athaniel-update-in-progress,
    ...
    # Athaniel" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($AthanielArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [AthanielUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `athaniel update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [AthanielUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; ATHANIEL_SELFTEST_FAIL=1 exercises the
# error state, ATHANIEL_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:ATHANIEL_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ATHANIEL_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:ATHANIEL_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-AthanielStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Athaniel install to
...
#            update log, the shape of every real `athaniel update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:ATHANIEL_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ATHANIEL_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:ATHANIEL_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:ATHANIEL_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "athaniel-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "athaniel-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "athaniel-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "athaniel-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "athaniel-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "athaniel-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "athaniel-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "athaniel-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `athaniel update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-AthanielStep $powershell @(
    ...
    $flood = Invoke-AthanielStep $powershell @(
    ...
    $stall = Invoke-AthanielStep $powershell @(
    ...
    try {
        $logstall = Invoke-AthanielStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-AthanielStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-AthanielStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:ATHANIEL_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Athaniel to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Athaniel window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Athaniel fully and try again."
    ...
    Publish-UiProgress "Preparing Athaniel files"
    $shim = Join-Path $InstallRoot "venv\Scripts\athaniel.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Athaniel install open (venv\Scripts\athaniel.exe locked after 20s). Nothing was changed. Close other Athaniel windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the athaniel.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\athaniel.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_athaniel_exe must first rename the running athaniel.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that athaniel.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Athaniel window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Athaniel.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m athaniel_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Athaniel installer or `athaniel doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "athaniel_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Athaniel windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m athaniel_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed athaniel predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-AthanielStep $pythonExe $updateArgs "update"
    Write-HandoffLog "athaniel update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-AthanielUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `athaniel update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "athaniel update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-AthanielStep $pythonExe @("-m", "athaniel_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `athaniel desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `athaniel debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Athaniel is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Athaniel was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Athaniel."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Athaniel"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Athaniel to finish (it could not restart itself)."
