"""Shared utility functions for athaniel-brain."""
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.athaniel/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
...
def warn_if_credential_file_broadly_readable(
    ...
    *,
    ...
    Secret-bearing files that users create by hand (or that older Athaniel
...
def atomic_roundtrip_yaml_save(
    ...
    * Non-dict values (lists, scalars) are overwritten wholesale — list
    ...
    Shares the fail-closed contract ``athaniel_cli.config.atomic_config_write``
    ...
    ``athaniel_cli.config`` itself imports from this module.
    """
    from ruamel.yaml import YAML
    from ruamel.yaml.comments import CommentedMap
    from ruamel.yaml.scalarstring import DoubleQuotedScalarString

    from athaniel_cli.config import require_readable_config_before_write
