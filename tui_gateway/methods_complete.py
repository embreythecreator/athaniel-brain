...
def _(rid, params: dict) -> dict:
    ...
    paste_dir = _athaniel_home / "pastes"
...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'athaniel' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "athaniel".startswith(prefix.lower()) and "athaniel" not in seen:
                out.append(
                    {
                        "text": "@athaniel",
                        "display": "@athaniel",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from athaniel_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from athaniel_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from athaniel_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `athaniel model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.athaniel/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from athaniel_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
