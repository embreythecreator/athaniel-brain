...
_athaniel_home = get_athaniel_home()
load_athaniel_dotenv(
    athaniel_home=_athaniel_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.athaniel/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_athaniel_home, "logs", "tui_gateway_crash.log")
...
try:
    from athaniel_cli.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("ATHANIEL_TUI_SLASH_TIMEOUT_S") or "45")
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    ``ATHANIEL_TUI_WS_ORPHAN_REAP_GRACE_S`` env var is kept as an internal
    override for backward compatibility and wins when set.
    """
    raw = os.environ.get("ATHANIEL_TUI_WS_ORPHAN_REAP_GRACE_S")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ATHANIEL_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("athaniel_gateway_runtime_session_record", default=None)
)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar(
    "athaniel_gateway_rpc_method", default=""
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Athaniel' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Athaniel-managed CLIs (browser-use, uvx, uv) even when the parent
    ...
    try:
        ...
        managed_bin = str(Path(get_athaniel_home()) / "bin")
    except Exception:
        pass
...
class _SlashWorker:
    """Persistent AthanielCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        self._closed = False
        from athaniel_cli._subprocess_compat import windows_hide_flags

        # slash_worker runs the Athaniel agent → needs provider credentials.
        ...
        # gateway's launch ATHANIEL_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["ATHANIEL_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            athaniel_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"ATHANIEL_HOME": str(profile_home)} if profile_home else None,
        )
        # Prepend the Athaniel venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Athaniel-managed CLIs
...
def _notify_session_boundary(
    ...
    try:
        from athaniel_cli.lifecycle import finalize_session, invoke_hook
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "Athaniel could not safely reserve this session. Try again."
)
...
def _claim_active_session_slot(
    ...
    *,
    ...
    try:
        from athaniel_cli.active_sessions import try_acquire_active_session
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    lease = session.get("active_session_lease")
    try:
        from athaniel_cli.active_sessions import (
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from athaniel_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from athaniel_cli.lifecycle import invoke_hook
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if current.get("close_on_disconnect"):
                    ...
                    # `athaniel --tui` keeps real _stdio. UNLESS another window
...
try:
    _SESSION_TTL_S = float(os.environ.get("ATHANIEL_TUI_SESSION_TTL_S") or 6 * 3600)
...
# A `athaniel serve` killed mid-update used to lose every un-flushed in-memory
...
try:
    _EXIT_FLUSH_BUDGET_S = float(
        os.environ.get("ATHANIEL_TUI_EXIT_FLUSH_BUDGET_S") or 5.0
    )
except (TypeError, ValueError):
...
try:
    _INCREMENTAL_FLUSH_INTERVAL_S = float(
        os.environ.get("ATHANIEL_TUI_SESSION_FLUSH_INTERVAL_S") or _REAPER_SCAN_S
    )
except (TypeError, ValueError):
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    can never block exit longer than ``ATHANIEL_TUI_EXIT_FLUSH_BUDGET_S``
    ...
    worker = threading.Thread(target=_run, daemon=True, name="athaniel-exit-flush")
    worker.start()
    worker.join(budget)
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``athaniel serve`` / dashboard startup before uvicorn takes over
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `athaniel --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from athaniel_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from athaniel_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from athaniel_cli.active_sessions import coerce_max_concurrent_sessions
...
def _sweep_orphaned_session_rows() -> list[str]:
    ...
    ago (``ATHANIEL_TUI_SESSION_TTL_S``). A freshly created row that copied
...
_HEARTBEAT_REFRESH_S = float(
    os.environ.get("ATHANIEL_GATEWAY_HEARTBEAT_REFRESH_S") or 60.0
)
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    thread = threading.Thread(
        ...
        args=(stop_event,),
        name="athaniel-gateway-heartbeat",
        daemon=True,
    )
    thread.start()
...
def _schedule_startup_orphan_sweep() -> None:
    ...
    reads the DB. ``ATHANIEL_TUI_WS_ORPHAN_REAP_GRACE_S=0`` (park forever)
    and ``ATHANIEL_TUI_SESSION_TTL_S=0`` both suppress the sweep; so does
...
# ``profile`` on those calls; we open that profile's db and bind its ATHANIEL_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_athaniel_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s ATHANIEL_HOME around a handler.

    Pets (config + sprites) and projects (projects.db, discovery policy) both
    resolve via ``get_athaniel_home``. The desktop sends ``profile`` so a single
    ...
    def wrapper(rid, params):
        ...
        token = set_athaniel_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from athaniel_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``athaniel dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ATHANIEL_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `athaniel` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ATHANIEL_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_athaniel_home_override(profile_home)
                try:
...
def _ensure_session_db_row(session: dict) -> bool:
    ...
    * A terminal session (``athaniel`` / ``athaniel --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from athaniel_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from athaniel_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from athaniel_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``athaniel_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _athaniel_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_athaniel_home_override()
        home = override if isinstance(override, str) and override else _athaniel_home
...
def _load_cfg() -> dict:
    ...
    ``athaniel_cli.config.load_config_readonly`` — managed-scope overlay and
    ...
    try:
        from athaniel_cli.config import _expand_env_vars
...
def _apply_managed(cfg: dict) -> dict:
    ...
    athaniel_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    override = get_athaniel_home_override()
    home = Path(override) if isinstance(override, str) and override else _athaniel_home
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read ATHANIEL_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # ATHANIEL_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ATHANIEL_GATEWAY_SESSION"] = "1"
    os.environ["ATHANIEL_EXEC_ASK"] = "1"
    os.environ["ATHANIEL_INTERACTIVE"] = "1"
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Athaniel Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Athaniel Desktop app and start a new "
...
def resolve_skin() -> dict:
    try:
        from athaniel_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``athaniel config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_athaniel_home_override()
    return Path(override if isinstance(override, str) and override else _athaniel_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Athaniel activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="athaniel-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("ATHANIEL_MODEL", "")
        or os.environ.get("ATHANIEL_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from athaniel_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ATHANIEL_DESKTOP=1`` and ``ATHANIEL_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ATHANIEL_DESKTOP_TERMINAL=1`` → "tui"
        (``athaniel --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``athaniel --tui``.)
    """
    if is_truthy_value(os.environ.get("ATHANIEL_DESKTOP")) and not is_truthy_value(
        os.environ.get("ATHANIEL_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ATHANIEL_MODEL /
    ATHANIEL_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`athaniel --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads ATHANIEL_MODEL /
    # ATHANIEL_INFERENCE_MODEL, which `athaniel --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("ATHANIEL_TUI_PROVIDER", "").strip()
    ...
    try:
        from athaniel_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ATHANIEL_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from athaniel_state import _BARE_BILLING_PROVIDERS
...
def _overrides_have_routable_provider(overrides: dict) -> bool:
    ...
    if not provider:
        return False
    try:
        from athaniel_cli.runtime_provider import is_routable_provider
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from athaniel_cli.runtime_provider import (
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind ATHANIEL_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_athaniel_home_override(profile_home) if profile_home else None
    )
...
def _load_approval_mode() -> str:
    ...
    canonical ``athaniel_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`athaniel_constants.resolve_reasoning_config` (per-model override >
    global ``agent.reasoning_effort``; YAML boolean False = disabled).
    Closes #21256.
    """
    from athaniel_constants import resolve_reasoning_config
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ATHANIEL_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_ATHANIEL_CORE_TOOLS`` — every other
    ...
    paths run with ``ATHANIEL_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Athaniel desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("ATHANIEL_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Athaniel): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `athaniel --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ATHANIEL_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from athaniel_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ATHANIEL_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ATHANIEL_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ATHANIEL_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _ATHANIEL_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off athaniel-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ATHANIEL_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from athaniel_cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from athaniel_cli.config import get_compatible_custom_providers, load_config
    ...
    if agent:
        try:
            from athaniel_cli.context_switch_guard import merge_preflight_compression_warning
    ...
    #
    # We deliberately do NOT write process-global env vars (ATHANIEL_MODEL /
    # ATHANIEL_INFERENCE_MODEL / ATHANIEL_TUI_PROVIDER / ATHANIEL_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `athaniel --tui -m`
...
def _compress_session_history(
    ...
    from athaniel_cli.partial_compress import (
...
def _get_usage(agent) -> dict:
    ...
    # ATHANIEL_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ATHANIEL_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from athaniel_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from athaniel_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from athaniel_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from athaniel_cli.banner import get_update_result
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Athaniel window, and answers
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from athaniel_cli.config import save_env_value_secure
...
def _render_personality_prompt(value) -> str:
    """Delegates to athaniel_cli.personality (single owner of rendering)."""
    from athaniel_cli.personality import render_personality_prompt

    return render_personality_prompt(value)


def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via athaniel_cli.personality (single owner)."""
    from athaniel_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as athaniel_cli.personality.resolve_personality — (name,
    ...
    """
    from athaniel_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to athaniel_cli.personality (single owner).
    """
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    from athaniel_cli.config import resolve_turn_limit as _resolve_turn_limit
    # Env var override (highest priority)
    env_val = os.environ.get("ATHANIEL_TUI_MAX_TURNS")
    if env_val:
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ATHANIEL_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``AthanielCLI.__init__`` and ``gateway/run.py``:
    ...
    """
    from athaniel_cli.fallback_config import get_fallback_chain
...
def _resolve_runtime_with_fallback(
    ...
    """
    from athaniel_cli.auth import AuthError
    from athaniel_cli.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from athaniel_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses athaniel_cli.mcp_startup; TUI stdio keeps
    # its existing tui_gateway.entry-owned thread.
    try:
        from athaniel_cli.mcp_startup import wait_for_mcp_discovery
    ...
    cfg = _load_cfg()
    from athaniel_cli.config import resolve_ephemeral_system_prompt_from_config
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `athaniel skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ATHANIEL_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ATHANIEL_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ATHANIEL_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ATHANIEL_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped ATHANIEL_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / ATHANIEL_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The ATHANIEL_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_athaniel_home)
...
def _finalize_superseded_runtimes(stale: list[tuple[str, dict]]) -> None:
    ...
    athaniel_state_common._RECOVERABLE_END_REASONS so canonical Bot Chat
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Athaniel has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_athaniel_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ATHANIEL_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $ATHANIEL_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_athaniel_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from athaniel_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=ATHANIEL_SESSION_KEY`` (see
    ...
    if not session_key or session.get("_finalized"):
        return []
    try:
        from athaniel_cli import kanban_db as _kb
    except Exception:
        return []
    texts: list = []
    try:
    ...
    # DB when ATHANIEL_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``ATHANIEL_UI_SESSION_ID`` as
...
def _plan_goal_compression_recovery(
    ...
    *,
    ...
    from athaniel_cli.goals import GoalManager
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ATHANIEL_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_athaniel_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from athaniel_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from athaniel_cli.voice import is_continuous_active
                ...
                try:
                    ...
                    if sid_key:
                        ...
                        if goal_mgr.is_active():
                            try:
                                from athaniel_cli.goals import gather_background_processes as _gather_bg
            ...
            if status == "complete":
                try:
                    from athaniel_cli.loops import LoopManager
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: athaniel_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific ATHANIEL_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile ATHANIEL_HOME
    override, so ``get_athaniel_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _athaniel_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile ATHANIEL_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from athaniel_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from athaniel_cli.models import resolve_fast_mode_overrides
    ...
    if key == "focus":
        ...
        from athaniel_cli.focus_view import (
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ATHANIEL_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["ATHANIEL_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("ATHANIEL_YOLO_MODE", None)
                    nv = "0"
    ...
    if key == "reasoning":
        try:
            from athaniel_constants import parse_reasoning_effort
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through athaniel_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from athaniel_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under ATHANIEL_HOME (~/.athaniel by
    ...
    athaniel_home = os.path.realpath(str(get_athaniel_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == athaniel_home
        or real.startswith(athaniel_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ATHANIEL_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: ATHANIEL_HOME itself and the dirs in
    ...
    athaniel_home = os.path.normcase(os.path.realpath(str(get_athaniel_home())))
    return real in _non_workspace_dirs() or real == athaniel_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Athaniel sessions still appear in the sidebar (#81723). Mirrors the
        ...
        try:
            for dirpath, dirnames, _filenames in os.walk(root):
                ...
                if ".git" in dirnames:
                    ...
                    # Not a repo: skip hidden dirs (e.g. .athaniel) and node_modules.
...
def _discover_repos_payload(
    ...
    repos even with zero athaniel sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (athaniel home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Athaniel sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Athaniel
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `athaniel` is interactive — use `/athaniel chat -q …` or run `athaniel` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`athaniel setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`athaniel gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`athaniel sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`athaniel config edit` needs $EDITOR in a real terminal"
    return None


def _resolve_name(name: str) -> str:
    try:
        from athaniel_cli.commands import resolve_command
...
def _model_picker_context(agent):
    ...
    from athaniel_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Athaniel" if role == "assistant" else role.title()
...
def _format_live_help_output() -> str:
    try:
        from athaniel_cli.commands import COMMANDS_BY_CATEGORY
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ATHANIEL_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ATHANIEL_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``athaniel_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["ATHANIEL_VOICE"] = "0"
                    os.environ["ATHANIEL_VOICE_TTS"] = "0"
                    try:
                        from athaniel_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via athaniel_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from athaniel_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ATHANIEL_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["ATHANIEL_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from athaniel_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from athaniel_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import athaniel_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from athaniel_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from athaniel_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
