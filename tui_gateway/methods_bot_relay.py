...
(local, remote URL, SSH, Athaniel Cloud, docker) must be able to find every
...
def _(rid, params: dict) -> dict:
    ...
    ``athaniel -p <profile> chat -c "Bot Chat"`` transport local DMs use and
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "athaniel" else profile
        if resolved not in known:
            return _err(rid, 4092, f"no profile '{profile}' on this gateway")

        fd, tmp = tempfile.mkstemp(prefix="athaniel-relay-dm-", suffix=".txt", text=True)
        try:
