...
# from shadowing Athaniel's own top-level modules.  ``athaniel_bootstrap`` lives at
...
import athaniel_bootstrap

athaniel_bootstrap.harden_import_path()
...
# shared owner in athaniel_cli.mcp_startup — the wait/in-flight/join helpers
...
# athaniel_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ATHANIEL_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ATHANIEL_TUI_SIDECAR_URL")
...
# tested; override via ``ATHANIEL_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ATHANIEL_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ATHANIEL_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``athaniel --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``athaniel_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from athaniel_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → athaniel_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's ATHANIEL_HOME first), so a launch profile with no
    ...
    try:
        from athaniel_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``athaniel --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``athaniel dashboard`` spawn theirs via
    ``athaniel_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``athaniel_cli.mcp_startup`` thread used by the
    ...
    try:
        from athaniel_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from athaniel_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's ATHANIEL_HOME override — the shared owner in
    ``athaniel_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Athaniel activates mid-conversation.
    ...
    try:
        from athaniel_cli.model_switch import prewarm_picker_cache_async
