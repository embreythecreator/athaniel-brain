...
The underlying token machinery is the *same* one the CLI ``athaniel mcp login``
uses — ``athaniel_cli.mcp_config._probe_single_server`` under
...
def _start_loopback_listener(flow) -> "http.server.HTTPServer":
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to Athaniel.</p>"
            status = 200
            try:
...
def _worker(session_id: str, athaniel_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    ATHANIEL_HOME override + secret-scope + force_interactive_oauth +
    ...
    try:
        ...
        home_token = set_athaniel_home_override(athaniel_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(athaniel_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(server_name, athaniel_home=athaniel_home)
                    ...
                    manager.restore_entry(server_name, previous_entry, athaniel_home=athaniel_home)
                    raise
        finally:
...
def start_flow(
    ...
    *,
    ...
    OAuth-capable). ``athaniel_home`` is the already-resolved profile home dir
    ...
    with _sessions_lock:
        ...
        if any(
            ...
            and r["athaniel_home"] == athaniel_home
    ...
    threading.Thread(
        target=_worker,
        args=(session_id, athaniel_home, server_name, dict(cfg), reconnect_live),
        daemon=True,
