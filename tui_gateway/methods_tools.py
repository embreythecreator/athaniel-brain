...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.athaniel/.env`` into the gateway process via
    ``athaniel_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from athaniel_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m athaniel_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "athaniel_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m athaniel_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=athaniel_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "plan":
        ...
        # .athaniel/plans/ via write_file. Works on any backend.
    ...
    if name == "moa":
        ...
        try:
            from athaniel_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind ATHANIEL_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_athaniel_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("ATHANIEL_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ATHANIEL_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_athaniel_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off ATHANIEL_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from athaniel_constants import set_athaniel_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from athaniel_constants import reset_athaniel_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``athaniel journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from athaniel_cli.skills_hub import do_install
        ...
        if action == "browse":
            from athaniel_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from athaniel_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `athaniel mcp` offers, so
    ...
    try:
        ...
        from athaniel_cli import mcp_catalog
...
# (athaniel_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes ATHANIEL_HOME via set_athaniel_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses athaniel_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from athaniel_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from athaniel_cli.config import load_config, save_config, save_env_value
        from athaniel_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from athaniel_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.athanielDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``athaniel mcp login`` uses
    ...
    try:
        ...
        athaniel_home = str(get_athaniel_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(
            athaniel_home, name, cfg, client_redirect_uri=client_redirect_uri
        )
        return _ok(
            rid,
            {
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``athaniel plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.athaniel/plugins/`` (non-interactive).
    ...
    plugins live under each profile's ATHANIEL_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Athaniel plugin.
