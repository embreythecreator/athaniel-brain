...
handlers delegate to the same `athaniel_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Athaniel profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(db, profile_path):
        ...
        describe the same session (athaniel-brain#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(db):
        ...
        sessions never surface in conversation lists (athaniel-brain#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's athaniel-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``athaniel setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (athaniel_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local ATHANIEL_HOME override — the same
        ...
        try:
            ...
            token = set_athaniel_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from athaniel_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from athaniel_cli.config import load_config_readonly, read_user_config_raw
            from athaniel_cli.web_routers.profiles import _write_profile_model
            from athaniel_constants import (
                reset_athaniel_home_override,
                set_athaniel_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the ATHANIEL_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from athaniel_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `athaniel tools`
            ...
            # composites (athaniel-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from athaniel_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `athaniel tools` / Settings is where
            ...
            description = ""
            try:
                from athaniel_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from athaniel_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from athaniel_cli.skills_config import save_disabled_skills
