...
# cwd at all, or one whose folder can't be promoted (the bare home dir, ATHANIEL
...
def build_tree(
    ...
    *,
    ...
    bare home dir, the ATHANIEL_HOME subtree). ``is_junk_cwd`` is the narrower
    policy for non-git session folders: selected descendants may be intentional
    workspaces even when their parent tree contains Athaniel state. User-created
