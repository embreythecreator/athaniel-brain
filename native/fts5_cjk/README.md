...
storage discipline as the other indexes). On a populated database, run
    ...
    athaniel sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.athaniel/config.yaml` to disable. Override
the .so location with `ATHANIEL_FTS5_CJK_SO`.
