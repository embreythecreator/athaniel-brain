...
# IMPORTANT: athaniel_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See athaniel_bootstrap.py for full rationale.
try:
    import athaniel_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when athaniel_bootstrap isn't registered in the venv
    # yet — happens during partial ``athaniel update`` where git-reset landed
...
# WO-SESSION/YIELD-1: the soft-interrupt message a wake-class turn receives
# when an operator's waiting message asks it to yield the session (or its
# soft runtime ceiling expires). The turn winds down at the next API-call
# boundary with full persistence; gateway/wake.py requeues the remainder by
# reading the `yielded_to_operator` result flag, never this text.
SESSION_TURN_YIELD_NOTICE = (
    "⏸️ Yielding this session to a waiting operator message; "
    "background work pauses here and resumes shortly."
)
import os
import re
import sys
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``athaniel -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("ATHANIEL_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("ATHANIEL_SESSION_SOURCE", "")
...
from athaniel_cli.timeouts import (
...
_loaded_env_paths = load_athaniel_dotenv(athaniel_home=_athaniel_home, project_env=_project_env)
...
# (athaniel_state._rows_to_conversation), so any code that mutates a loaded or
...
def _routermint_headers() -> dict:
    ...
    from athaniel_cli import __version__ as _ATHANIEL_VERSION

    return {
        "User-Agent": f"AthanielAgent/{_ATHANIEL_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Athaniel-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.athaniel/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[athaniel-brain: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `athaniel --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from athaniel_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``athaniel chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``athaniel chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ATHANIEL_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ATHANIEL_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ATHANIEL_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ATHANIEL_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ATHANIEL_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``ATHANIEL_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("ATHANIEL_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        athaniel-brain #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See athaniel-brain#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from athaniel_cli.models import _should_use_copilot_responses_api
    ...
    def _flush_messages_to_session_db_unlocked(
        ...
        try:
            ...
            if _batch_rows:
                self._append_batch_with_compression_patience(_batch_rows)
            ...
            if isinstance(e, CompressionSessionClosedError):
                ...
                return False
            # Retain the raw exception too, for older diagnostic paths.
            self._last_persistence_error = e

            logger.warning("Session DB append_message failed: %s", e)
            return False

    # Transcript appends are refused while another writer holds the session's
    # compression lock. That refusal is CORRECT (appending mid-compression
    # would corrupt the transcript) but it is transient — the lock is a lease
    # held for the duration of one compaction. The Brain already shows this
    # patience in the other direction (preflight compression defers when a
    # lock is held); without it here, a turn that merely collided with a
    # compaction was thrown away as if the disk had failed.
    #
    # Bounded on purpose: a big stop-the-world compaction can run ~80s, and
    # blocking a live turn that long is worse than failing it. This waits out
    # the common short lock; `compression.micro_compact` is the setting that
    # prevents the long ones from forming.
    _COMPRESSION_LOCK_RETRY_DELAYS = (0.25, 0.5, 1.0, 2.0, 4.0)

    def _append_batch_with_compression_patience(self, batch_rows: list) -> None:
        """Append the turn's rows, waiting out a transient compression lock.

        Re-raises the original error once patience is exhausted, or
        immediately for any other failure (a real disk/permission fault must
        not be retried into a long stall).
        """
        from athaniel_state import SessionCompressionInProgressError

        attempts = len(self._COMPRESSION_LOCK_RETRY_DELAYS) + 1
        for attempt in range(attempts):
            try:
                ...
                if attempt:
                    logger.info(
                        "Session transcript append succeeded after waiting "
                        "out a compression lock (attempt %d/%d).",
                        attempt + 1,
                        attempts,
                    )
                return
            except SessionCompressionInProgressError:
                if attempt >= len(self._COMPRESSION_LOCK_RETRY_DELAYS):
                    raise
                time.sleep(self._COMPRESSION_LOCK_RETRY_DELAYS[attempt])
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Athaniel against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Athaniel can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ATHANIEL_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from athaniel_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``ATHANIEL_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.athaniel/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Athaniel-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ATHANIEL_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than Athaniel' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Athaniel model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Athaniel-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``ATHANIEL_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("ATHANIEL_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.athaniel/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ATHANIEL_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("ATHANIEL_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        persistence_error: Exception = None,  # back-compat alias (fork retention)
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "compression_locked_waited":
                # Fork retention: a compaction held the writer lock past our bounded patience.
                return (
                    prefix
                    + "the turn was stopped because a context compaction was "
                    "still holding this session's transcript lock. Nothing is "
                    "wrong with your disk and no history was lost — the "
                    "compaction finishes on its own. Send your message again. "
                    "If this recurs, enable `compression.micro_compact` so "
                    "context is trimmed continuously instead of in one long "
                    "stop-the-world pass."
                )
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Athaniel process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Athaniel process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `athaniel doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.athaniel/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.athaniel/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`athaniel doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (ATHANIEL_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (ATHANIEL_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ATHANIEL_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ATHANIEL_DEV_CREDITS"))
        ...
        if _dev:
            # ATHANIEL_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `athaniel logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Athaniel YOLO selected unrestricted
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from athaniel_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``athaniel auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                from athaniel_cli.auth import resolve_codex_runtime_credentials

                singleton_now = resolve_codex_runtime_credentials(
                    refresh_if_expiring=False,
                )
            else:
                from athaniel_cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(
        ...
        *,
        ...
        try:
            from athaniel_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("ATHANIEL_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.athaniel/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``athaniel setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from athaniel_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from athaniel_cli.auth import _resolve_zai_base_url
            ...
            try:
                from athaniel_cli.runtime_provider import _get_named_custom_provider
            except ImportError:
                return False
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from athaniel_cli.models import copilot_default_headers
            ...
            from tools.xai_http import athaniel_xai_default_headers

            self._client_kwargs["default_headers"] = athaniel_xai_default_headers()
        else:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from athaniel_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from athaniel_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from athaniel_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated athaniel_logging._session_context (a
            ...
            try:
                from athaniel_logging import set_session_context
            ...
            # ATHANIEL_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from athaniel_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        # WO-SESSION/YIELD-1: classify this turn for the durable lease. The
        # gateway stamps _session_turn_kind per request ("wake" for
        # async-delegation self-posts); everything else is an operator turn.
        # Reset the per-turn yield flag here so a cached agent can never
        # report a stale yield from a previous turn.
        _turn_lease_kind = str(getattr(self, "_session_turn_kind", "") or "operator")
        if _turn_lease_kind not in ("operator", "wake", "background"):
            _turn_lease_kind = "operator"
        self._session_turn_yielded = False
        ...
        try:
            # Serialize the full load -> run -> flush region across Athaniel
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Athaniel process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Athaniel process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    # WO-SESSION/YIELD-1: operator waiters ask a wake-class
                    # holder to yield; machine waiters never do.
                    kind=_turn_lease_kind,
                    request_yield_on_wait=(_turn_lease_kind == "operator"),
                ):
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Athaniel process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Athaniel process kept this session busy too "
                ...
                if _turn_lease_kind != "operator":
                    # WO-SESSION/YIELD-1: say so the moment a machine turn
                    # takes the session, so the Face can show WHY the
                    # composer is busy instead of silence.
                    self._emit_status(
                        "🔁 Processing background results — this turn yields "
                ...
                # WO-SESSION/YIELD-1: wake-class turns poll for yield requests
                # far more often than the lease refresh cadence, so an operator
                # waits ~one poll + one API-call boundary, not a refresh cycle.
                # Operator turns keep the original wake-up cadence exactly.
                _yieldable_turn = _turn_lease_kind in ("wake", "background")
                # min() so a test-shrunk refresh interval shrinks the poll too;
                # production (60s refresh) polls yield every 5s.
                _yield_poll_interval = (
                    min(5.0, _lease_refresh_interval)
                    if _yieldable_turn
                    else _lease_refresh_interval
                )
                try:
                    _wake_soft_ceiling = float(
                        os.environ.get("ATHANIEL_WAKE_TURN_SOFT_CEILING_S", "")
                        or 600.0
                    )
                except (TypeError, ValueError):
                    _wake_soft_ceiling = 600.0
                _turn_lease_started_monotonic = time.monotonic()

                def _refresh_durable_turn_lease() -> None:
                    def _interrupt_turn(
                        message: str, *, hard_cancel: bool = True
                    ) -> None:
                        ...
                        with durable_turn_lease_activity_lock:
                            ...
                            try:
                                self.interrupt(message, hard_cancel=hard_cancel)
                            except Exception:
                                self._interrupt_requested = True
                                self._interrupt_message = message

                    yield_fired = False
                    next_refresh_at = time.monotonic() + _lease_refresh_interval
                    while not durable_turn_lease_stop.wait(_yield_poll_interval):
                        now_monotonic = time.monotonic()
                        if now_monotonic >= next_refresh_at:
                            next_refresh_at = now_monotonic + _lease_refresh_interval
                            try:
                                ...
                                return
                        if _yieldable_turn and not yield_fired:
                            requested = False
                            try:
                                requested = _turn_db.session_turn_yield_requested(
                                    getattr(self, "session_id", None) or session_id,
                                    durable_turn_lease,
                                )
                            except Exception:
                                logger.debug(
                                    "session turn yield poll failed; retrying",
                                    exc_info=True,
                                )
                            over_ceiling = (
                                _wake_soft_ceiling > 0
                                and (now_monotonic - _turn_lease_started_monotonic)
                                >= _wake_soft_ceiling
                            )
                            if requested or over_ceiling:
                                # Soft interrupt: the loop finishes the
                                # in-flight API call/tool, persists, and exits
                                # at the next boundary. Keep looping so the
                                # lease stays refreshed while it winds down.
                                yield_fired = True
                                self._session_turn_yielded = True
                                logger.info(
                                    "wake turn yielding session %s (%s)",
                                    getattr(self, "session_id", None) or session_id,
                                    "operator waiting" if requested else "soft ceiling",
                                )
                                _interrupt_turn(
                                    SESSION_TURN_YIELD_NOTICE, hard_cancel=False
                                )
            ...
            if getattr(self, "_session_turn_yielded", False) and isinstance(result, dict):
                # WO-SESSION/YIELD-1: surface the yield to the caller — the
                # api_server turns this into an X-Athaniel-Yielded response
                # header so gateway/wake.py can requeue the remainder.
                result["yielded_to_operator"] = True
            return result
