...
Pure logic: walks a Athaniel config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `athaniel doctor` and a future `athaniel migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Athaniel config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Athaniel config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `athaniel migrate xai` reports nothing to migrate
