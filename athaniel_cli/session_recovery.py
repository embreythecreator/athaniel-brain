"""Offline, non-destructive recovery for a damaged Athaniel session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``athaniel_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``athaniel_state._backup_db_file``.
    """
    from athaniel_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="athaniel-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Athaniel process using this profile and retry. "
                "This includes the interactive `athaniel` CLI session this "
                ...
                "Athaniel process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `athaniel` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from athaniel_cli.session_lost_and_found import (
