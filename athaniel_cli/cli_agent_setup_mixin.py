"""Agent-construction and session-resume display methods for ``AthanielCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.athaniel/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``AthanielCLI``. ``self.*``
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    ...
    ``athaniel chat -q`` runs one turn without ever building the
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``AthanielCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'athaniel model' to choose a provider, or "
                      "'athaniel setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: athaniel setup")
            return False
        ...
        # `athaniel chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `athaniel auth add openai-codex`
        # without `athaniel model`), fall back to the provider's first catalog
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``athaniel model`` (which fronts Quick Setup / Nous
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'athaniel model' or 'athaniel setup' any time.")
            return False
        ...
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'athaniel model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'athaniel model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'athaniel model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (athaniel sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (athaniel sessions list).{_RST}")
                return False
        ...
        try:
            ...
            from athaniel_constants import partial_update_hint
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "event":
                ...
                lines.append("  ◆ Athaniel: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Athaniel: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
