"""
athaniel fallback — manage the fallback provider chain.
...
Subcommands:
  athaniel fallback [list]   Show the current fallback chain (default when no subcommand)
  athaniel fallback add      Pick provider + model via the same picker as `athaniel model`,
                           then append the selection to the chain
  athaniel fallback remove   Pick an entry to delete from the chain
  athaniel fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.athaniel/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from athaniel_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `athaniel model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  athaniel fallback add")
        print()
        return
    ...
    print("  Docs: https://athaniel-brain.embreythecreator.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `athaniel model`, then append the selection to the chain."""
    from athaniel_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `athaniel model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `athaniel fallback list` to view, or `athaniel fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``athaniel fallback [subcommand]``."""
