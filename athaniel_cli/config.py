"""
Configuration management for Athaniel Brain.

Config files are stored in ~/.athaniel/ for easy access:
- ~/.athaniel/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.athaniel/.env         - API keys and secrets

This module provides:
- athaniel config          - Show current configuration
- athaniel config edit     - Open config in editor
- athaniel config get      - Print a resolved configuration value
- athaniel config set      - Set a specific value
- athaniel config unset    - Remove a user configuration value
- athaniel config wizard   - Re-run setup wizard
"""
...
from athaniel_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``athaniel config set`` (which rewrites
    ...
    ``config.yaml`` in place: athaniel never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.athaniel/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``athaniel logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (athaniel config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  athaniel config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Athaniel makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Athaniel itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``athaniel update``, the TUI build.
...
#   on every plugin install / ``athaniel update``.
...
# * ``ATHANIEL_HOME`` / ``ATHANIEL_PROFILE`` / ``ATHANIEL_CONFIG`` /
#   ``ATHANIEL_ENV`` — Athaniel runtime location flags. Writing these into
...
#
# IMPORTANT: ``ATHANIEL_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ATHANIEL_LANGFUSE_PUBLIC_KEY,
# ATHANIEL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Athaniel runtime location — never via dashboard env writer.
    # NOT a ATHANIEL_* blanket: integration credentials (ATHANIEL_GEMINI_*,
    # ATHANIEL_LANGFUSE_*, ATHANIEL_SPOTIFY_*, ...) ARE allowed.
    "ATHANIEL_HOME", "ATHANIEL_PROFILE", "ATHANIEL_CONFIG", "ATHANIEL_ENV",
    "ATHANIEL_CONFIG_PATH", "ATHANIEL_ENV_PATH",
    ...
    "ATHANIEL_COPILOT_ACP_COMMAND", "ATHANIEL_COPILOT_ACP_ARGS",
    # Athaniel security policy / approval-routing context. These remain available
    ...
    "ATHANIEL_YOLO_MODE", "ATHANIEL_ACCEPT_HOOKS", "ATHANIEL_REDACT_SECRETS",
    "ATHANIEL_INTERACTIVE", "ATHANIEL_EXEC_ASK", "ATHANIEL_GATEWAY_SESSION",
    "ATHANIEL_CRON_SESSION", "ATHANIEL_SINGLE_QUERY_SESSION",
    "ATHANIEL_SESSION_KEY", "ATHANIEL_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Athaniel runtime location "
            "and security policy (ATHANIEL_HOME, ATHANIEL_YOLO_MODE, ...) "
            "cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.athaniel/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ATHANIEL_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `athaniel plugins enable
    # observability/langfuse` or `athaniel tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ATHANIEL_LANGFUSE_ENV",
    "ATHANIEL_LANGFUSE_RELEASE",
    "ATHANIEL_LANGFUSE_SAMPLE_RATE",
    "ATHANIEL_LANGFUSE_MAX_CHARS",
    "ATHANIEL_LANGFUSE_CAPTURE",
    "ATHANIEL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "ATHANIEL_ACP_AUTH_METHOD",
    "ATHANIEL_ACP_AUTO_APPROVE",
    "ATHANIEL_COPILOT_ACP_COMMAND",
    "ATHANIEL_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from athaniel_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ATHANIEL_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ATHANIEL_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_athaniel_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # ATHANIEL_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Athaniel is running in package-manager-managed mode.

    Two signals: the ATHANIEL_MANAGED env var (set by the systemd service),
    or a .managed marker file in ATHANIEL_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Athaniel through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``athaniel_cli/`` — i.e. the git checkout for source
    installs, ``/opt/athaniel`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ATHANIEL_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Athaniel was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$ATHANIEL_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ATHANIEL_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.athaniel``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ATHANIEL_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.athaniel:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ATHANIEL_HOME``, a
    ...
    ``docker`` and ``athaniel update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``embreythecreator/athaniel-brain`` image bakes a ``docker``
        stamp into ``/opt/athaniel`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ATHANIEL_HOME.
    try:
    ...
    #    host install whose shared $ATHANIEL_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``athaniel update``.
    try:
        method = (
            (get_athaniel_home() / ".install_method")
    ...
    # detect Nix installs that don't set ATHANIEL_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``athaniel_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ATHANIEL_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/athaniel`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Athaniel lives in a ``uv tool`` layout.

    ``uv tool install athaniel-brain`` places the install at
    ``.../uv/tools/athaniel-brain/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``athaniel-brain`` happens to be uv-tool-installed on the machine while
    the *active* Athaniel is a regular pip/venv install, causing
    ``athaniel update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/athaniel-brain/" in norm + "/"
...
# banner, the TUI/desktop session info panel, and ``athaniel update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://athaniel-brain.embreythecreator.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``athaniel update`` / ``athaniel update --check`` so the wording — and the
...
# Long-form text for ``athaniel update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# athaniel_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Athaniel, it doesn't update
...
✗ ``athaniel update`` doesn't apply inside the Docker container.

Athaniel Brain runs as a published image (embreythecreator/athaniel-brain), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull embreythecreator/athaniel-brain:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate athaniel-brain
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm embreythecreator/athaniel-brain:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/embreythecreator/athaniel-brain/tags
  • Your config and session history live under ``$ATHANIEL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``athaniel update`` inside Docker.
...
def format_managed_message(action: str = "modify this Athaniel installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Athaniel installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Athaniel."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ATHANIEL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, athaniel_bin
    or None if container mode is not active, we're already inside the
    container, or ATHANIEL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ATHANIEL_DEV") == "1":
        return None
    ...
    container_mode_file = get_athaniel_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "athaniel-brain")
    exec_user = info.get("exec_user", "athaniel")
    athaniel_bin = info.get("athaniel_bin", "/data/current-package/bin/athaniel")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "athaniel_bin": athaniel_bin,
    }
...
# Re-export from athaniel_constants — canonical definition lives there.
from athaniel_constants import get_athaniel_home, get_process_athaniel_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_athaniel_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_athaniel_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_athaniel_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ATHANIEL_UID / ATHANIEL_GID env vars set by Docker deployments.

    Docker containers running Athaniel commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ATHANIEL_HOME once, but
    subdirectories created at runtime by ``ensure_athaniel_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ATHANIEL_UID", "").strip()
    gid_str = os.environ.get("ATHANIEL_GID", "").strip()
    try:
...
def _chown_to_athaniel_uid(path) -> None:
    """Chown ``path`` to ``ATHANIEL_UID:ATHANIEL_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_athaniel_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_athaniel_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the athaniel group can
    share state with the gateway service.

    The mode can be overridden via the ATHANIEL_HOME_MODE environment variable
    (e.g. ATHANIEL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ATHANIEL_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ATHANIEL_UID``/``ATHANIEL_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ATHANIEL_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_athaniel_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Athaniel runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ATHANIEL_CONTAINER") or os.environ.get("ATHANIEL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ATHANIEL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ATHANIEL_HOME, upgrading legacy empty templates.
...
# ensure_athaniel_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ATHANIEL_HOME_ENSURED: set = set()


def ensure_athaniel_home():
    """Ensure ~/.athaniel directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ATHANIEL_HOME`` we only re-run
    ...
    ``get_athaniel_home()`` and therefore re-run for the new path.
    """
    home = get_athaniel_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``athaniel profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from athaniel_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _ATHANIEL_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_athaniel_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _ATHANIEL_HOME_ENSURED.add(key)


def _ensure_athaniel_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ATHANIEL_HOME {home} does not exist."
        )
...
from athaniel_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401

# Athaniel Model Council — one Crown orchestrator, six worker wings, and a
# separate operational fallback slot. Runtime code routes by role names;
# provider/model IDs live here or in ATHANIEL_* env overrides. Layered onto
# DEFAULT_CONFIG here (rather than in config_defaults.py, a pure-data leaf
# module documented as never importing from athaniel_cli.config) because
# building the defaults requires calling into athaniel_cli.model_council.
# The built-in "default" MoA preset is likewise replaced with the Model
# Council mapping (the six wings as reference advisors, Crown as aggregator)
# instead of config_defaults.py's hardcoded upstream preset.
DEFAULT_CONFIG["model_council"] = default_model_council_config()
DEFAULT_CONFIG["moa"]["presets"]["default"] = model_council_moa_preset()
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``athaniel config set`` uses ``.`` as the nesting separator, so a key that
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `athaniel config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.athaniel.config`` entries, then checks
    ...
    try:
        ...
        # should never break `athaniel update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Athaniel' own config writer has historically emitted it
    ...
    # Athaniel (``_save_discovered_models_to_config``), not hand-curated.
    # Older Athaniel versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Athaniel versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``athaniel_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (athaniel_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (athaniel_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (athaniel_cli/setup.py)
    "known_plugin_toolsets", # written/read by athaniel_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (athaniel_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'athaniel setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Athaniel won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, athaniel send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'athaniel doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_athaniel_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "athaniel update / athaniel -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from athaniel_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ athaniel config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in athaniel_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from athaniel_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from athaniel_cli.toolset_validation import validate_platform_toolsets
        from athaniel_cli.toolset_scope import toolset_allowed_for_platform
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: athaniel config set <key> <value>")
    ...
    # `athaniel update`, but only the version bump is persisted.
    ...
    # metadata.athaniel.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.athaniel/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_athaniel_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``AthanielCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``athaniel config set``
    ...
    backends) — while display paths (``athaniel status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the ATHANIEL_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _athaniel_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in athaniel_cli.personality.
from athaniel_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`athaniel_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from athaniel_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from athaniel_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``ATHANIEL_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`athaniel_cli.personality` (single owner).
    """
    from athaniel_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.athaniel/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_athaniel_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.athaniel/config.yaml.
    ...
    (which change ``ATHANIEL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Athaniel host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/athaniel/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_athaniel_dotenv()
...
#   openai-codex (OAuth — athaniel auth) — OpenAI Codex
#   nous         (OAuth — athaniel auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.athaniel/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Athaniel writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.athaniel/.env.
    ...
    interactive menu render (`athaniel tools`, `athaniel setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `athaniel tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.athaniel/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.athaniel/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ATHANIEL_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ATHANIEL_CUSTOM_{slug}_API_KEY" if slug else "ATHANIEL_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.athaniel/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.athaniel/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Athaniel — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Athaniel vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.athaniel/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.athaniel/.env`` over ``os.environ``.

    Used for Athaniel-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ✶ Athaniel Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from athaniel_cli.auth import get_anthropic_key
    ...
    # Warn on stale ATHANIEL_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ATHANIEL_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ATHANIEL_MAX_ITERATIONS={_env_ghost} "
                f"(run 'athaniel doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from athaniel_cli.personality import active_personality_name
    ...
    print(color("  athaniel config edit     # Edit config file", Colors.DIM))
    print(color("  athaniel config set <key> <value>", Colors.DIM))
    print(color("  athaniel setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``ATHANIEL_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("ATHANIEL_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `athaniel cron list`, then pin the "
        "intended values with `athaniel cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (athaniel_cli/plugins_cmd.py, athaniel_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `athaniel config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``athaniel config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``athaniel config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``athaniel config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   athaniel config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   athaniel config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``athaniel config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``athaniel config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    athaniel config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    athaniel config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    try:
    ...
    # so a fresh `athaniel config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_athaniel_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `athaniel config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `athaniel config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: athaniel config get <key> [--json]")
            print()
            print("Examples:")
            print("  athaniel config get model")
            print("  athaniel config get terminal.backend")
            print("  athaniel config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: athaniel config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  athaniel config set model anthropic/claude-sonnet-4")
            print("  athaniel config set terminal.backend docker")
            print("  athaniel config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: athaniel config unset <key>")
            print()
            print("Examples:")
            print("  athaniel config unset model")
            print("  athaniel config unset terminal.backend")
            print("  athaniel config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'athaniel config migrate' to add them")
        ...
        print("  athaniel config           Show current configuration")
        print("  athaniel config edit      Open config in editor")
        print("  athaniel config get <key>          Print a resolved config value")
        print("  athaniel config set <key> <value>   Set a config value")
        print("  athaniel config unset <key>        Remove a config value")
        print("  athaniel config check     Check for missing/outdated config")
        print("  athaniel config migrate   Update config with new options")
        print("  athaniel config path      Show config file path")
        print("  athaniel config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``athaniel config`` UI so users
