"""Per-terminal session breadcrumbs for ``athaniel -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$ATHANIEL_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``athaniel -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_athaniel_home() / "terminal-sessions"
