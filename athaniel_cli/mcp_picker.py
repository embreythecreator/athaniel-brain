"""MCP picker — interactive `athaniel mcp picker` (also the default `athaniel mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``athaniel mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `athaniel plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from athaniel_cli.cli_output import prompt_yes_no
from athaniel_cli.curses_ui import curses_single_select
from athaniel_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Athaniel session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from athaniel_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `athaniel mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: athaniel mcp install <name>    Picker: athaniel mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Athaniel is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Athaniel — run `athaniel update` "
...
def show_catalog() -> None:
    """`athaniel mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`athaniel mcp picker` (and default `athaniel mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`athaniel mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `athaniel mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
