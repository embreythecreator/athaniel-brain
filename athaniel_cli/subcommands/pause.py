"""``athaniel pause`` / ``athaniel resume`` — the global emergency stop.

``athaniel pause`` writes the ESTOP sentinel at ``$ATHANIEL_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``athaniel resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Athaniel paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `athaniel resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Athaniel resumed — dispatch picks up on the next tick.")
    else:
        print(f"Athaniel is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`athaniel resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `athaniel pause`",
