"""``athaniel update`` subcommand parser.

Extracted verbatim from ``athaniel_cli/main.py:main()`` (god-file Phase 2).
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Athaniel Brain to the latest version",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "kind (git/docker/nix), every running Athaniel service across all "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Force a FULL pre-update backup (quick state snapshot + ATHANIEL_HOME zip) for this run, regardless of updates.pre_update_backup",
    )
    update_parser.add_argument(
        "--yes",
        "-y",
        action="store_true",
        default=False,
        help="Run without blocking on prompts: accepts the config-migration and stash-restore prompts, skips the fork-upstream prompt without adding a remote. API-key entry is skipped; run 'athaniel config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--keep-stash",
        action="store_true",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "If the local checkout is on a different branch, athaniel will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another athaniel.exe is detected. The concurrent process will likely cause WinError 32 warnings. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
        "--force-venv",
        action="store_true",
