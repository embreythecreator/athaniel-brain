"""``athaniel peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Athaniel gateway (any machine: homelab, Spark, Athaniel
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    athaniel peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    athaniel peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``athaniel -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.athaniel/.env`` as
  ``ATHANIEL_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"ATHANIEL_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _request(
    ...
    *,
    ...
    # The peer URL is user-registered (``athaniel peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's athaniel-brain, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "athaniel.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (athaniel peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: athaniel peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: athaniel peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.athaniel/.env)"
        )
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.athaniel/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  athaniel peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.athaniel/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: athaniel peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    print("Unknown peer action. See: athaniel peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Athaniel gateways)",
        description=(
            "Register other Athaniel gateways as peers and message their agents. "
            "'athaniel peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'athaniel -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.athaniel/.env."
        ),
        epilog=(
            "Examples:\n"
            "  athaniel peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  athaniel peer list\n"
            '  athaniel peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  athaniel peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  athaniel peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  athaniel peer status spark run_abc123\n"
            "  athaniel peer stop spark run_abc123\n"
            "  athaniel peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.athaniel/.env)")
    ...
    status_p.add_argument("run_id", help="Run ID returned by 'athaniel peer run'")
    status_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
    ...
    stop_p.add_argument("run_id", help="Run ID returned by 'athaniel peer run'")
    stop_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
