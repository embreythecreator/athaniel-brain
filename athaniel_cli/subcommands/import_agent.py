"""``athaniel import-agent`` subcommand parser.

Follows the ``athaniel claw`` pattern (see ``athaniel_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``athaniel_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Athaniel",
        description=(
            "One-command import of another coding agent's setup into Athaniel. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Athaniel equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'athaniel setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Athaniel items on name conflicts (default: skip)",
    )
    parser.add_argument(
