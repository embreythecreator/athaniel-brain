"""``athaniel debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Athaniel Brain. Use 'athaniel debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    athaniel debug share              Upload debug report (asks for confirmation)
    athaniel debug share --yes        Skip confirmation (for scripts/CI)
    athaniel debug share --lines 500  Include more log lines
    athaniel debug share --expire 30  Keep paste for 30 days
    athaniel debug share --local      Print report locally (no upload)
    athaniel debug share --no-redact  Disable upload-time secret redaction
    athaniel debug share --nous       Upload to Nous-internal storage (private)
    athaniel debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'athaniel debug share'",
    )
