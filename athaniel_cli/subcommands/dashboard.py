"""``athaniel dashboard`` / ``athaniel serve`` subcommand parsers.
...
``athaniel_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `athaniel dashboard` / `athaniel serve` cmdlines and SIGTERM them directly —
    # the same path `athaniel update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Athaniel web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Athaniel web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Athaniel Brain web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Athaniel desktop app shells (<= 0.15.x) spawn the
    # backend as `athaniel dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Athaniel couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Athaniel Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Athaniel backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Athaniel backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports ATHANIEL_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `athaniel dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.athaniel/.env.
    # Nested subparser so bare `athaniel dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ATHANIEL_DASHBOARD_OAUTH_CLIENT_ID into ~/.athaniel/.env, and prints "
            "how to engage the login gate. Requires being logged in (athaniel setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://athaniel.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via ATHANIEL_DASHBOARD_PORTAL_URL. Mainly for "
