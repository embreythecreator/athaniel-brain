"""``athaniel plugins`` subcommand parser.
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description=(
            "Install, update, remove, list, or validate native Athaniel plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help=(
            "Git URL, owner/repo shorthand (e.g. anpicasso/athaniel-plugin-chrome-profiles), "
            "or a bare plugin name resolved through the community index "
            "(see `athaniel plugins search`)"
        ),
    )
    plugins_install.add_argument(
        "--force",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `athaniel plugins enable <name>`",
    )
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack",
        help="Declarative, shareable plugin sets (athaniel-pack.yaml)",
        description=(
    ...
    pack_install.add_argument(
        "source", help="Path to a athaniel-pack.yaml file, or an https:// URL"
    )
    pack_install.add_argument(
        "--force",
        "-f",
