"""``athaniel sync`` subcommand parser — Skill Sync.

Cloned from ``athaniel_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    athaniel sync status                 show gate/opt-in/head state
    athaniel sync pull                   pull and materialize opted-in skills
    athaniel sync push                   push opted-in skills
    athaniel sync now                    reconcile: pull then push
    athaniel sync enable <skill>         opt a skill into sync
    athaniel sync disable <skill>        opt a skill out of sync
    athaniel sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    athaniel sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  athaniel sync status            what is synced, and from where\n"
            "  athaniel sync enable my-skill   include a skill in your sync\n"
            "  athaniel sync now               pull, then push\n"
            "  athaniel sync propose my-skill  share a skill with your team\n"
        ),
