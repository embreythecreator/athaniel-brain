"""``athaniel plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.athaniel/plugins/``.
...
from athaniel_cli._subprocess_compat import noninteractive_git_env
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Athaniel subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_athaniel_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.athaniel/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Athaniel never auto-installs plugin pip
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from athaniel_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_athaniel_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_athaniel_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_athaniel_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_athaniel_home() / "plugins" / _INSTALL_METADATA_FILE
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Athaniel.",
                ) from None
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `athaniel plugins update {plugin_name}`."
            )
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from athaniel_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `athaniel plugins search <term>` to "
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Athaniel plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `athaniel plugins enable {installed_name}` to activate.[/dim]",
        )
    ...
    console.print("[dim]  athaniel gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`athaniel plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `athaniel plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``athaniel setup`` / ``athaniel plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``athaniel plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    if name in LEGACY_RELAY_PLUGIN_KEYS:
        console.print(
            ...
            f"by Athaniel core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."
        )
        sys.exit(1)
    ...
    # Athaniel core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from athaniel_cli.plugin_capabilities import parse_declared_capabilities
    ...
    for entry in _discover_all_plugins():
        ...
        if entry[5] == key or entry[0] == key:
            if entry[3] == "entrypoint":
                from athaniel_cli.plugins import discover_entrypoint_manifests
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from athaniel_cli.plugin_capabilities import CAPABILITY_REGISTRY
    ...
    *,
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        console.print(
            ...
            f"`athaniel plugins capabilities {plugin_id}` to review and "
            f"`athaniel plugins enable {plugin_id}` to grant interactively."
        )
        return False
    ...
    console.print(
        ...
        f"`athaniel plugins enable {plugin_id}` to grant later.[/dim]"
    )
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``athaniel plugins capabilities [<id>]`` — declared vs granted."""
    from rich.console import Console

    from athaniel_cli.plugin_capabilities import granted_capabilities
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`athaniel plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``athaniel_brain.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.athaniel/plugins``. Include package metadata here
    so ``athaniel plugins list`` can show and enable them.
    """
    from athaniel_cli.plugins import ENTRY_POINTS_GROUP
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``athaniel plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] athaniel plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] athaniel plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] athaniel plugins")
    console.print("[dim]Enable/disable:[/dim] athaniel plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Athaniel
    ...
    try:
        from athaniel_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] athaniel plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``athaniel plugins enable web/firecrawl`` cleared
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected,
    ...
    from athaniel_cli.curses_ui import flush_stdin
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.athaniel/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.athaniel/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `athaniel plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``athaniel plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``athaniel_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``athaniel update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "athaniel-plugin-update-autostash",
            )
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.athaniel/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from athaniel_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from athaniel_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: athaniel plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")


def plugins_command(args) -> None:
    """Dispatch athaniel plugins subcommands."""
    ...
    if action == "install":
        ...
        from athaniel_cli.plugin_packs import pack_command
