"""
Profile management for multiple isolated Athaniel instances.

Each profile is a fully independent ATHANIEL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.athaniel/profiles/<name>/`` by default.

The "default" profile is ``~/.athaniel`` itself — backward compatible,
zero migration needed.

Usage::

    athaniel profile create coder          # fresh profile + bundled skills
    athaniel profile create coder --clone  # also copy config, .env, SOUL.md, skills
    athaniel profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    athaniel -p coder chat                 # or via flag
    athaniel profile use coder             # set as sticky default
    athaniel profile delete coder          # remove profile + alias + service
"""
...
from athaniel_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See athaniel_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.athaniel``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   athaniel-brain  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `athaniel backup` archives
...
# Marker file written by `athaniel profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `athaniel
...
# `athaniel skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Athaniel infrastructure directories that only the default profile
         (``~/.athaniel``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_athaniel_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.athaniel) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "athaniel-brain",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when ATHANIEL_HOME equals the
...
# into the export. The set below identifies the *known Athaniel profile
# artifacts* at the root of ATHANIEL_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``athaniel_constants``.
...
_RESERVED_NAMES = frozenset({
    "athaniel", "default", "test", "tmp", "root", "sudo",
})

# Athaniel subcommands that cannot be used as profile names/aliases
_ATHANIEL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the athaniel root, NOT to the current ATHANIEL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ATHANIEL_HOME points outside
    ``~/.athaniel``, profiles live under ``ATHANIEL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_athaniel_home() / "profiles"


def _get_default_athaniel_home() -> Path:
    """Return the default (pre-profile) ATHANIEL_HOME path.

    In standard deployments this is ``~/.athaniel``.
    In Docker/custom deployments where ATHANIEL_HOME is outside ``~/.athaniel``
    (e.g. ``/opt/data``), returns ATHANIEL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_athaniel_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Athaniel installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ATHANIEL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_athaniel_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Athaniel home
    (:func:`athaniel_constants.get_athaniel_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, athaniel subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ATHANIEL_SUBCOMMANDS:
        return f"'{canon}' conflicts with a athaniel subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            athaniel_exe = shutil.which("athaniel") or "athaniel"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(athaniel_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Athaniel. If we
    ...
    try:
        ...
        # not be migrated. The next `athaniel doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``athaniel -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``athaniel profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``athaniel -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``athaniel profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        source = get_athaniel_home() / "config.yaml"
        ...
        # Creation must not fail over this; `athaniel model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ATHANIEL_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Athaniel config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_athaniel_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, athaniel_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ATHANIEL_HOME. When
    ...
    The returned ``athaniel_home`` is the path to pass to
    ``set_athaniel_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_athaniel_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``athaniel update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.athaniel)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.athaniel/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `athaniel -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Athaniel profile.\n"
    ...
    if not soul_path.exists():
        try:
            from athaniel_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `athaniel update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`athaniel profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Athaniel (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `athaniel profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `athaniel -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ATHANIEL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``athaniel profile create
    ...
    only the essential skills (e.g. ``athaniel-brain``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ATHANIEL_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_athaniel_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Athaniel *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Athaniel backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ATHANIEL_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `athaniel -p <canon> profile
    ...
    athaniel_markers = ("athaniel_cli.main", "athaniel-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('athaniel')` resolution) spawns the
    ...
    # interpreter, not "athaniel", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _ATHANIEL_CONSOLE_SCRIPT_NAMES = frozenset({"athaniel", "athaniel-brain", "athaniel-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Athaniel process: either an entrypoint marker in argv, a
            # resolved executable named `athaniel`, or a python interpreter
            # directly exec'ing a `athaniel`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_athaniel = (
                any(marker in joined for marker in athaniel_markers)
                or exe_name == "athaniel"
                or exe_name.startswith("athaniel")
            )
            if not is_athaniel and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: athaniel, athaniel-brain,
                # athaniel-acp) rather than a bare `startswith("athaniel")` --
                ...
                # "athaniel-notes.py" or "athaniel-unrelated-tool" as the shim,
                ...
                is_athaniel = script_stem in _ATHANIEL_CONSOLE_SCRIPT_NAMES
            if not is_athaniel:
                continue
            ...
            # ...or by ATHANIEL_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ATHANIEL_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.athaniel).\n"
            "To remove everything, use: athaniel uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_athaniel_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ATHANIEL_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from athaniel_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ATHANIEL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ATHANIEL_HOME")
    try:
        os.environ["ATHANIEL_HOME"] = str(profile_dir)
        from athaniel_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ATHANIEL_HOME"] = old_home
        elif "ATHANIEL_HOME" in os.environ:
            del os.environ["ATHANIEL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.athaniel/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: athaniel profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ATHANIEL_HOME.

    Returns ``"default"`` if ATHANIEL_HOME is not set or points to ``~/.athaniel``.
    Returns the profile name if ATHANIEL_HOME points into ``~/.athaniel/profiles/<name>``.
    Returns ``"custom"`` if ATHANIEL_HOME is set to an unrecognized path.
    """
    from athaniel_constants import get_athaniel_home
    athaniel_home = get_athaniel_home()
    resolved = athaniel_home.resolve()

    default_resolved = _get_default_athaniel_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ATHANIEL_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Athaniel file the
      user may have lying alongside ATHANIEL_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``athaniel sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``ATHANIEL_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.athaniel itself — its parent is ~/ and its
        # directory name is ".athaniel", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: athaniel profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.athaniel itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.athaniel). "
            "Specify a different name: athaniel profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="athaniel_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"athaniel_{old_name}"
    legacy_old_host = f"athaniel.{old_name}"
    new_host = f"athaniel_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_athaniel_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("athaniel_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ATHANIEL_HOME path string.

    Called early in the CLI entry point, before any athaniel modules
    are imported, to set the ATHANIEL_HOME environment variable.

    When ATHANIEL_HOME is already set, the configured spelling IS the
