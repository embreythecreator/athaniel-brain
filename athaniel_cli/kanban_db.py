...
``<root>`` is the **shared Athaniel root** (the parent of any active
...
``athaniel -p <profile>`` joins the same board as the dispatcher that
...
the profile's Athaniel home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ATHANIEL_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ATHANIEL_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``athaniel kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.athaniel``. In Docker / custom
deployments where ``ATHANIEL_HOME`` points outside ``~/.athaniel`` (e.g.
``/opt/athaniel``), ``<root>`` is ``ATHANIEL_HOME``. Legacy env-var
overrides still work:

* ``ATHANIEL_KANBAN_DB`` — pin the database file path directly.
* ``ATHANIEL_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ATHANIEL_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ATHANIEL_KANBAN_DB``,
``ATHANIEL_KANBAN_WORKSPACES_ROOT``, and ``ATHANIEL_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/athaniel-kanban-v1-spec.pdf`` for the full
...
from athaniel_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``athaniel_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from athaniel_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("ATHANIEL_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active ATHANIEL_HOME so dispatcher- and
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from athaniel_cli.lifecycle import has_hook
...
# ``ATHANIEL_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ATHANIEL_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ATHANIEL_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ATHANIEL_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ATHANIEL_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ATHANIEL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "ATHANIEL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "athaniel_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``athaniel-brain``
...
def kanban_home() -> Path:
    """Return the shared Athaniel root that anchors the kanban board.

    Resolution order:

    1. ``ATHANIEL_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_athaniel_root()``, which already returns ``<root>``
       when ``ATHANIEL_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ATHANIEL_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ATHANIEL_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ATHANIEL_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from athaniel_constants import get_default_athaniel_root
    return get_default_athaniel_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``athaniel kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ATHANIEL_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``athaniel kanban boards
    ...
    env = os.environ.get("ATHANIEL_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``athaniel kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ATHANIEL_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ATHANIEL_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ATHANIEL_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ATHANIEL_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ATHANIEL_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("ATHANIEL_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``athaniel kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``athaniel_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``ATHANIEL_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (athaniel_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ATHANIEL_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ATHANIEL_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``athaniel_cli.sqlite_safe_read``).
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``athaniel gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from athaniel_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see athaniel_cli.sqlite_safe_read), so it must only run once the board has
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Athaniel treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``athaniel kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ATHANIEL_KANBAN_DB`` env → ``ATHANIEL_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `athaniel kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        from athaniel_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See athaniel_state._WAL_INCOMPAT_MARKERS for detection logic.
                from athaniel_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``athaniel kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ATHANIEL_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from athaniel_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"athaniel_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``athaniel --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from athaniel_cli import projects_db as _pdb
    ...
    # `athaniel --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``athaniel kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `athaniel kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``athaniel kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``athaniel kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``athaniel kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``athaniel kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``athaniel kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ATHANIEL_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Athaniel' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Athaniel-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``athaniel kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Athaniel checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Athaniel treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Athaniel creates/reuses that linked worktree. With
      no ``workspace_path``, Athaniel anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Athaniel uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``ATHANIEL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Athaniel
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``athaniel kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``athaniel kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Athaniel profile.
    ...
    if not rows:
        return False
    try:
        from athaniel_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Athaniel ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Athaniel
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``athaniel kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``ATHANIEL_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from athaniel_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from athaniel_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Athaniel profile.
        # `_default_spawn` invokes ``athaniel -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Athaniel
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `athaniel kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Athaniel bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from athaniel_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_athaniel_argv() -> list[str]:
    """Return the interpreter-bound Athaniel CLI invocation."""
    # ``athaniel_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``athaniel`` package — there is no
    # ``athaniel`` package to import.
    return [sys.executable, "-m", "athaniel_cli.main"]


def _absolute_athaniel_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Athaniel shim."""
...
def _athaniel_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Athaniel executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_athaniel_argv()
    return [_absolute_athaniel_path(path)]


def _resolve_athaniel_argv() -> list[str]:
    """Resolve the ``athaniel`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ATHANIEL_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("athaniel")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\athaniel.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m athaniel_cli.main`` — fallback for setups where
       Athaniel is launched from a venv and the ``athaniel`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_athaniel_bin`` for the same reason. Kept
    local (not imported from gateway) because ``athaniel_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ATHANIEL_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _athaniel_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _athaniel_path_argv(resolved_env_bin)
        return _module_athaniel_argv()

    athaniel_bin = _safe_which_no_cwd("athaniel") if _IS_WINDOWS else shutil.which("athaniel")
    if athaniel_bin:
        return _athaniel_path_argv(athaniel_bin)
    return _module_athaniel_argv()
...
def _resolve_worker_cli_toolsets(athaniel_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``ATHANIEL_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        token = set_athaniel_home_override(athaniel_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for ATHANIEL_HOME=%r (%s)",
            athaniel_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``athaniel -p <profile> chat -q ...`` subprocess.
    ...
    ``ATHANIEL_KANBAN_DB`` / ``ATHANIEL_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ATHANIEL_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `athaniel -p <name>` the
    # _apply_profile_override() runs *before* athaniel_constants is imported.
    # If ATHANIEL_HOME is absent from the child's env, get_athaniel_home() falls
    # back to Path.home() / ".athaniel" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from athaniel_cli.profiles import resolve_profile_env
    try:
        env["ATHANIEL_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ATHANIEL_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ATHANIEL_HOME never had profiles created.
        pass
    if task.tenant:
        env["ATHANIEL_TENANT"] = task.tenant
    env["ATHANIEL_KANBAN_TASK"] = task.id
    env["ATHANIEL_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `athaniel kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["ATHANIEL_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ATHANIEL_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ATHANIEL_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ATHANIEL_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["ATHANIEL_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ATHANIEL_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`athaniel -p <name>`
    # rewrites ATHANIEL_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_athaniel_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ATHANIEL_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ATHANIEL_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ATHANIEL_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ATHANIEL_KANBAN_BOARD"] = resolved_board
    # ATHANIEL_PROFILE is the author the kanban_comment tool defaults to.
    # `athaniel -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ATHANIEL_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited ATHANIEL_TUI=1
    ...
    # older athaniel builds on PATH that predate the flag's precedence.
    env.pop("ATHANIEL_TUI", None)

    cmd = [
        *_resolve_athaniel_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped ATHANIEL_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("ATHANIEL_HOME"))
    if worker_toolsets:
    ...
    # `athaniel kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`athaniel` executable not found on PATH. "
            "Install Athaniel Brain or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``athaniel kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``athaniel kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `athaniel kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ATHANIEL_PROFILE values like "athaniel-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Athaniel root exists

    Reads profile paths directly so this module has no import dependency on
    ``athaniel_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from athaniel_constants import get_default_athaniel_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``athaniel kanban assignees`` for the terminal.
