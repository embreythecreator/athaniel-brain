"""``athaniel doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from athaniel_cli.doctor import ATHANIEL_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (ATHANIEL_HOME / "node" / "bin",
                          ATHANIEL_HOME / "node",
                          ATHANIEL_HOME / "node_modules" / ".bin"):
    ...
    # athaniel_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``athaniel mcp test``.
    """
