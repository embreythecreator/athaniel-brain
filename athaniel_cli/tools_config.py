"""
Unified tool configuration for Athaniel Brain.

`athaniel tools` and `athaniel setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.athaniel/config.yaml under
...
from athaniel_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from athaniel_cli.nous_account import format_nous_portal_entitlement_message
from athaniel_cli.toolset_scope import (
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``athaniel tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`athaniel_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from athaniel_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ATHANIEL_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `athaniel tools` → Video Generation, which walks
...
# `athaniel tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `athaniel tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from athaniel_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``athaniel tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from athaniel_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from athaniel_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``athaniel tools`` checklist actually offers
    for ``platform``.
    ...
    so ``athaniel tools`` never claims to add or remove a toolset the user was
...
from athaniel_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Athaniel routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "tag": "Zig headless browser spawned by Athaniel, text-only (no screenshots)",
                "env_vars": [],
                "browser_provider": "local",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ATHANIEL_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ATHANIEL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ATHANIEL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ATHANIEL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ATHANIEL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ATHANIEL_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `athaniel model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ATHANIEL_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``ATHANIEL_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Athaniel telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Athaniel' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $ATHANIEL_HOME/bin is never on PATH, so a bare which()
    # misses the uv Athaniel installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``athaniel update`` and
      by ``athaniel computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``athaniel update`` sets
    ...
    a further ~600s wait on Windows). ``athaniel computer-use install
    ...
    ``athaniel update`` already prints a contextual line before its update
    ...
    ``athaniel update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `athaniel update` calls this
    ...
    override = os.environ.get("ATHANIEL_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    ATHANIEL_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Athaniel' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Athaniel' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `athaniel update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Athaniel cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("ATHANIEL_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "athaniel computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `athaniel computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `athaniel update` (require_confirmed_update=True) keeps the
    ...
    # `athaniel computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: athaniel computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: athaniel computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``athaniel update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `athaniel update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``athaniel update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `athaniel update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `athaniel update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.athaniel/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Athaniel process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Athaniel-managed ``$ATHANIEL_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Athaniel installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key == "lightpanda":
        # Browser Use mode drives Lightpanda directly (Athaniel spawns
        ...
        if lightpanda_bin:
            ...
            if os.name == "nt":
                _print_info("    Lightpanda has no native Windows build; run Athaniel under WSL2.")
        return
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/Athaniel-managed node -> npx) rather than a bare
        # shutil.which — Athaniel-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/embreythecreator/athaniel-brain:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Athaniel-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Athaniel-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.athaniel/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.athaniel/config.yaml")
        ...
        # Run the full `athaniel auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.athaniel/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from athaniel_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: athaniel auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `athaniel auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: athaniel auth spotify")
        except Exception as exc:
        ...
        try:
            from athaniel_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: athaniel plugins enable observability/langfuse")
        _print_info("    Restart Athaniel for tracing to take effect.")
        _print_info("    Verify: athaniel plugins list")
        ...
        try:
            ...
            _print_info("    Run later: athaniel auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `athaniel auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: athaniel auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: athaniel auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``athaniel tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``athaniel tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: athaniel tools post-setup <key>")
        return 2
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from athaniel_cli.plugins import (
            get_plugin_manager,
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``athaniel-discord``, which contains those
...
#:
#: Saving ``athaniel tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[athaniel-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``athaniel tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"athaniel-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (athaniel-acp, athaniel-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``athaniel-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"athaniel-{platform}"
    ...
    # explicit config like ``[athaniel-cli, a2a]`` must survive filtering just
    ...
    # "athaniel-cli" (which include all _ATHANIEL_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[athaniel-cli, spotify]`` after enabling Spotify via ``athaniel
        ...
        # (e.g. "athaniel-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``athaniel tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `athaniel tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"athaniel-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("athaniel-")}
    ...
    # shipped as a bundled plugin but user must opt in via `athaniel tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `athaniel tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by athaniel tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `athaniel config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `athaniel` instead of
    # `athaniel-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `athaniel update`/`athaniel doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `athaniel tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (athaniel-cli, athaniel-telegram, etc.)
    ...
    # Opening `athaniel tools` is the user's opt-in to reconfigure tools, so treat
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from athaniel_constants import athaniel_home_key

    scope = athaniel_home_key()
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `athaniel tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``athaniel tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``athaniel tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from athaniel_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``athaniel tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from athaniel_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (athaniel_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `athaniel model`).
    if managed_feature:
        from athaniel_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``athaniel model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``athaniel model`` aux-task
    ...
    """
    try:
        from athaniel_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `athaniel model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `athaniel model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `athaniel tools` and `athaniel setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("✶ Athaniel Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://athaniel-brain.embreythecreator.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_athaniel_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'athaniel' or gateway restart.", Colors.DIM))
    print()
