"""CLI handlers for ``athaniel egress ...``.
...
The top-level command is ``athaniel egress``.  Note that the inbound OAuth
reverse-proxy command (``athaniel proxy``) lives elsewhere in
``athaniel_cli/main.py`` — different direction, different purpose.
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from athaniel_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``athaniel_cli.main`` as part of building the top-level
    ``athaniel egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``athaniel proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`athaniel egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `athaniel secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.athaniel/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `athaniel egress setup` finds the same keys the
        ...
        if loaded:
            console.print(
                ...
                f"~/.athaniel/.env for discovery.[/dim]"
            )
    ...
    # explicitly requested rotation.  This prevents re-running `athaniel
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Athaniel sandbox.  They will start "
    ...
    # `athaniel egress reload` apply future ruleset changes without a
    ...
    # rotation), running `athaniel egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]athaniel egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]athaniel egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]athaniel egress start[/cyan]\n"
        "  Restart: [cyan]athaniel egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]athaniel egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]athaniel egress status[/cyan]\n"
        "  Stop:    [cyan]athaniel egress stop[/cyan]\n"
        "  Disable: [cyan]athaniel egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `athaniel egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `athaniel egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`athaniel egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `athaniel secrets bitwarden setup` to configure the "
                "project, or switch back via `athaniel egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``athaniel egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `athaniel egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `athaniel egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `athaniel egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]athaniel egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `athaniel egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.athaniel/.env`` into ``os.environ``.

    ``athaniel egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.athaniel/.env`` (which the agent
