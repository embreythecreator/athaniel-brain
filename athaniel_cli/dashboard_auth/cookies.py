...
Three cookies in play:
  - athaniel_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - athaniel_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Nous Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / athaniel
  ...
  - athaniel_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/athaniel) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/athaniel`` covers
...
setter wrote ``__Secure-athaniel_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "athaniel_session_at"
SESSION_RT_COOKIE = "athaniel_session_rt"
...
SESSION_PROVIDER_COOKIE = "athaniel_session_provider"
PKCE_COOKIE = "athaniel_session_pkce"
...
SSO_ATTEMPT_COOKIE = "athaniel_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /athaniel`` we want ``Path=/athaniel`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (athaniel #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/athaniel``)
...
def set_pkce_cookie(
    ...
    # showed the browser sending the intact quoted cookie while Athaniel
