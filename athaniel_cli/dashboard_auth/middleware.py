...
from athaniel_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /athaniel``, the
    ``login_url`` is prefixed (``/athaniel/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["athaniel.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``athaniel_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``athaniel_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from athaniel_cli.dashboard_auth.cookies import clear_session_cookies
        from athaniel_cli.dashboard_auth.prefix import prefix_from_request
