...
from athaniel_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``ATHANIEL_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /athaniel`` (Mission Control deploys) — we
...
def _prefix(request: Request) -> str:
    ...
    ``athaniel_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from athaniel_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("athaniel_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from athaniel_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from athaniel_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from athaniel_cli.dashboard_auth import list_session_providers
    from athaniel_cli.dashboard_auth.base import RefreshExpiredError
