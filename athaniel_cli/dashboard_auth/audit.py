...
Profile-aware location: ``$ATHANIEL_HOME/logs/dashboard-auth.log``.
...
from ``athaniel_constants`` or other athaniel_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ATHANIEL_HOME/logs/dashboard-auth.log``.

    Uses ``athaniel_constants.get_athaniel_home()`` (a leaf module — no import
    ...
    return get_athaniel_home() / "logs" / "dashboard-auth.log"
