"""
Baked-in build metadata for Athaniel Brain.

Source installs report their git revision live via ``git rev-parse`` (see
``athaniel_cli/dump.py`` and ``athaniel_cli/banner.py``).  That doesn't work inside
...
To make ``athaniel dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$ATHANIEL_GIT_SHA``
arg into ``<project_root>/.athaniel_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``ATHANIEL_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".athaniel_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    rev-parse`` for source installs, the baked ``.athaniel_build_sha`` for
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.athaniel_build_sha`` if present.  The file is
    written by the Dockerfile's ``ATHANIEL_GIT_SHA`` build-arg and contains
