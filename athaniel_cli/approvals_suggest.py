"""``athaniel approvals suggest`` — mine approval history into allowlist proposals.

Athaniel has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.athaniel/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, athaniel self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/athaniel home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_athaniel_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_athaniel_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  athaniel approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.athaniel/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``athaniel approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``athaniel approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from athaniel_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: athaniel approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `athaniel approvals <subcommand> -h` for details."
    )
    return 1
