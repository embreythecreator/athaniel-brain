...
A system-level directory (default ``/etc/athaniel``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.athaniel/config.yaml`` and ``~/.athaniel/.env`` on a per-leaf-key basis.

This is DISTINCT from ``athaniel_cli.config.is_managed()`` / ``ATHANIEL_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/athaniel")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/athaniel`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``ATHANIEL_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$ATHANIEL_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/athaniel`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("ATHANIEL_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through athaniel_cli.config.load_config) to honor managed
    scope. Mirrors athaniel_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from athaniel_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
