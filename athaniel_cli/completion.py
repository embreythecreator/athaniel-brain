"""Shell completion script generation for athaniel CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_athaniel_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Athaniel Brain bash completion
# Add to ~/.bashrc:
#   eval "$(athaniel completion bash)"

_athaniel_profiles() {{
    local profiles_dir="$HOME/.athaniel/profiles"
...
_athaniel_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_athaniel_profiles)" -- "$cur"))
        return
    fi
...
complete -F _athaniel_completion athaniel
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef athaniel
# Athaniel Brain zsh completion
# Add to ~/.zshrc:
...
_athaniel_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.athaniel/profiles" ]]; then
        profiles+=($HOME/.athaniel/profiles/*(N/:t))
    fi
...
_athaniel() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_athaniel_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'athaniel command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Athaniel Brain fish completion",
        "# Add to your config:",
        "#   athaniel completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __athaniel_profiles",
        "    echo default",
        "    if test -d $HOME/.athaniel/profiles",
        "        for d in $HOME/.athaniel/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c athaniel -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c athaniel -f -s p -l profile"
        " -d 'Profile name' -xa '(__athaniel_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__athaniel_profiles)' -d 'Profile name'"
                )
