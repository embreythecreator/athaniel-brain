...
def active_session_limit_message(
    ...
    return (
        f"Athaniel is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_athaniel_home())
...
class ActiveSessionLease:
    ...
    # root ``ATHANIEL_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile ATHANIEL_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``athaniel dashboard`` / ``serve``) never trips that
