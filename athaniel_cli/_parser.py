"""
Top-level argparse construction for the athaniel CLI.
...
# argparse runs (it sets ``ATHANIEL_HOME`` and strips itself from ``sys.argv``),
...
# tests/athaniel_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``athaniel --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``athaniel_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    athaniel                        Start interactive chat
    athaniel chat -q "Hello"        Single query mode
    athaniel --tui                  Launch the modern TUI (or set display.interface: tui)
    athaniel --cli                  Force the classic REPL (overrides display.interface: tui)
    athaniel -c                     Resume the most recent session
    athaniel -c "my project"        Resume a session by name (latest in lineage)
    athaniel --resume <session_id>  Resume a specific session by ID
    athaniel --resume latest        Resume the most recent session (same as -c)
    athaniel --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    athaniel setup                  Run setup wizard
    athaniel logout                 Clear stored authentication
    athaniel auth add <provider>    Add a pooled credential
    athaniel auth list              List pooled credentials
    athaniel auth remove <p> <t>    Remove pooled credential by index, id, or label
    athaniel auth reset <provider>  Clear exhaustion status for a provider
    athaniel model                  Select default model
    athaniel fallback [list]        Show fallback provider chain
    athaniel fallback add           Add a fallback provider (same picker as `athaniel model`)
    athaniel fallback remove        Remove a fallback provider from the chain
    athaniel config                 View configuration
    athaniel config edit            Edit config in $EDITOR
    athaniel config set model gpt-4 Set a config value
    athaniel gateway                Run messaging gateway
    athaniel -s athaniel-brain-dev,github-auth
    athaniel -w                     Start in isolated git worktree
    athaniel gateway install        Install gateway background service
    athaniel sessions list          List past sessions
    athaniel sessions browse        Interactive session picker
    athaniel sessions rename ID T   Rename/title a session
    athaniel logs                   View agent.log (last 50 lines)
    athaniel logs -f                Follow agent.log in real time
    athaniel logs errors            View errors.log
    athaniel logs --since 1h        Lines from the last hour
    athaniel debug share             Upload debug report for support
    athaniel console                Open the safe Athaniel command console
    athaniel update                 Update to latest version
    athaniel dashboard              Start web UI dashboard (port 9119)
    athaniel dashboard --stop       Stop running dashboard processes
    athaniel dashboard --status     List running dashboard processes

For more help on a command:
    athaniel <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="athaniel",
        description="Athaniel Brain - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `athaniel chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ATHANIEL_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `athaniel setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to ATHANIEL_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.athaniel/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Athaniel Brain",
    )
    ...
    # top-level parser: when the user writes `athaniel -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/athaniel_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also ATHANIEL_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.athaniel/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Athaniel itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
