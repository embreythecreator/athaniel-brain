...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `athaniel model` to authenticate via OAuth\n"
...
# (``/api/model/options``, ``athaniel tools``) probe Copilot auth several times
...
def _jwt_disk_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_athaniel_home()) / _JWT_DISK_FILENAME
    except Exception:
        return None
