"""
Single source of truth for provider identity in Athaniel Brain.
...
2. **Athaniel overlays** — transport type, auth patterns, aggregator flags,
...
# -- Athaniel overlay ----------------------------------------------------------
# Athaniel-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AthanielOverlay:
    """Athaniel-specific provider metadata layered on top of models.dev."""
...
ATHANIEL_OVERLAYS: Dict[str, AthanielOverlay] = {
    "moa": AthanielOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": AthanielOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "openai-codex": AthanielOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": AthanielOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": AthanielOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": AthanielOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ATHANIEL_QWEN_BASE_URL",
    ),
    "lmstudio": AthanielOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": AthanielOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AthanielOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AthanielOverlay(
    ...
    "kimi-for-coding": AthanielOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": AthanielOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": AthanielOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": AthanielOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": AthanielOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": AthanielOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": AthanielOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": AthanielOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_override="https://opencode.ai/zen/v1",
        keyless=True,
    ),
    "kilo": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": AthanielOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": AthanielOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": AthanielOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "athaniel", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Athaniel overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Athaniel overlay
    ...
    overlay = ATHANIEL_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + athaniel extra
    ...
    if overlay is not None:
        # Athaniel-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and ATHANIEL_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Athaniel overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in ATHANIEL_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Athaniel provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from athaniel_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
