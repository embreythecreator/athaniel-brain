...
def _is_athaniel_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Athaniel ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Athaniel gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``athaniel update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_athaniel_gateway_run_argv(argv):
        return argv
