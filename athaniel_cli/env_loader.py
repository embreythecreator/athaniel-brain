"""Helpers for loading Athaniel .env files consistently across entrypoints."""
...
# load_athaniel_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_athaniel_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_athaniel_dotenv().
# Used by setup / `athaniel model` flows to label detected credentials so
...
# ATHANIEL_HOME paths we've already pulled external secrets for during this
# process.  ``load_athaniel_dotenv()`` is called at module-import time from
# several hot modules (cli.py, athaniel_cli/main.py, run_agent.py,
...
def _known_athaniel_env_keys() -> set[str]:
    """Return the combined set of known Athaniel env-var keys.
    ...
    ``load_athaniel_dotenv()`` calls.
    """
    from athaniel_cli.config import _EXTRA_ENV_KEYS
    from athaniel_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Athaniel process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Athaniel keys absent from ``.env``.
    ...
    parent Athaniel process injects and that silently change *which provider
    ...
    ``tests/athaniel_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_athaniel_dotenv()`` call.  Returns
...
def get_secret_source_values(
    athaniel_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``athaniel_home``."""
    home_key = str(Path(athaniel_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(athaniel_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_athaniel_dotenv()'s .op.env bootstrap: the 1Password
        ...
        local_env["ATHANIEL_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which ATHANIEL_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `athaniel setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``athaniel_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from athaniel_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_athaniel_dotenv(
    *,
    athaniel_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Athaniel environment files with user config taking precedence.

    Behavior:
    - `~/.athaniel/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(athaniel_home or os.getenv("ATHANIEL_HOME", Path.home() / ".athaniel"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Athaniel keys
    ...
    #   EnvironmentFile=-/path/to/.athaniel/.op.env
    ...
    # 2. A fresh ``athaniel update`` retry just completed a deferred dependency
    ...
    from athaniel_cli import _early_recovery
    ...
    # TERMINAL_ENV=docker left in ~/.athaniel/.env (e.g. written by an older
    # `athaniel setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_athaniel_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``athaniel_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process ATHANIEL_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_athaniel_dotenv(athaniel_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_athaniel_home().resolve():
            return
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of ATHANIEL_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Athaniel reads
    ...
    owns the once-per-ATHANIEL_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_athaniel_dotenv()`` runs at import
    time from several hot modules (cli.py, athaniel_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_athaniel_dotenv() call instead of never.
        return
    ...
    # a generic dict entry must not force crypto load on every athaniel launch.
    ...
    # so the 3-5 import-time load_athaniel_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `athaniel model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `athaniel` startup, so populating the
    # shared cache here lets main.py's early bridge and athaniel_logging reuse
    ...
    if home_path == _process_athaniel_home():
        try:
...
def _process_athaniel_home() -> Path:
    """The ATHANIEL_HOME the shared config cache is keyed to."""
    try:
