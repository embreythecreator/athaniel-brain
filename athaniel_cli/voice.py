...
# lookup, and athaniel-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and athaniel-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when ATHANIEL_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("ATHANIEL_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def speak_text(text: str, stop_event: Optional[threading.Event] = None) -> None:
    ...
    try:
        ...
        os.makedirs(os.path.join(tempfile.gettempdir(), "athaniel_voice"), exist_ok=True)
