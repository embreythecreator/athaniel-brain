"""`athaniel checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.athaniel/checkpoints/``.  Actions:

    athaniel checkpoints               # same as `status`
    athaniel checkpoints status        # total size, project count, breakdown
    athaniel checkpoints list          # per-project checkpoint counts + workdir
    athaniel checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    athaniel checkpoints clear [-f]    # nuke the entire base (asks first)
    athaniel checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    athaniel checkpoints
    athaniel checkpoints prune --retention-days 3 --max-size-mb 200
    athaniel checkpoints clear -f
...
from athaniel_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: athaniel checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``athaniel checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `athaniel checkpoints` → status
