...
the tool registry or launching Athaniel (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``athaniel-cli`` to the non-existent ``athaniel``. ``resolve_toolset('athaniel')``
...
from athaniel_cli.platforms import PLATFORMS
...
def _platform_default_toolset(platform: object) -> str:
    ...
    return info.default_toolset if info is not None else f"athaniel-{platform}"
...
def _platform_default_is_valid(
    ...
    # though their synthesized athaniel-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``athaniel-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``athaniel`` instead of ``athaniel-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `athaniel tools` to configure explicitly."
            )
            if platform_valid_count == 0:
                warnings.append(
                    ...
                    f"Run `athaniel tools` to reconfigure."
                )
            continue
        names = raw
        for name in names:
        ...
        if platform_valid_count == 0:
            ...
            warnings.append(
                ...
                f"tools on this platform. Run `athaniel tools` to reconfigure."
            )

    if valid_count == 0:
        warnings.append(
            "platform_toolsets resolves to zero valid toolsets — the agent will "
            "have no tools. Run `athaniel tools` to reconfigure."
        )
    return warnings
