"""Unified provider-credential lifecycle across every store Athaniel reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.athaniel/.env``                     — the canonical secret store
    2. ``~/.athaniel/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.athaniel/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `athaniel auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``athaniel auth add``.
    ...
    ran ``athaniel auth add <provider> --type api-key`` separately. This makes
    the Desktop save's effect on disk match what ``athaniel auth add`` does.
    """
    from athaniel_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from athaniel_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from athaniel_cli.config import load_env, remove_env_value
