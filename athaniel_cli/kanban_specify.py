...
Used by ``athaniel kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``athaniel_cli/goals.py`` — same aux
...
ATHANIEL_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("ATHANIEL_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Athaniel Brain board.
...
def _profile_author() -> str:
    """Mirror of ``athaniel_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ATHANIEL_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=ATHANIEL_KANBAN_SPECIFY_MAX_TOKENS,
