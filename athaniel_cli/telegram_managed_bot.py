...
manual BotFather token copy-paste. Athaniel talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.athaniel-brain.embreythecreator.com"
...
DEFAULT_MANAGER_BOT = "AthanielSetupBot"

DEFAULT_BOT_NAME = "Athaniel Brain"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``athaniel_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``athaniel_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"athaniel_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Athaniel Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Athaniel Telegram onboarding service.")
