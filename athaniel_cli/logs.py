"""``athaniel logs`` — view and filter Athaniel log files.
...
Usage examples::
    ...
    athaniel logs                    # last 50 lines of agent.log
    athaniel logs -f                 # follow agent.log in real time
    athaniel logs errors             # last 50 lines of errors.log
    athaniel logs gateway -n 100    # last 100 lines of gateway.log
    athaniel logs gui -f            # follow gui.log (dashboard/pty/ws)
    athaniel logs desktop -f        # follow desktop.log (Electron app boot/backend)
    athaniel logs --level WARNING    # only WARNING+ lines
    athaniel logs --session abc123   # filter by session ID substring
    athaniel logs --component tools  # only tool-related lines
    athaniel logs --since 1h         # lines from the last hour
    athaniel logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_athaniel_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Athaniel runs — try 'athaniel chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_athaniel_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_athaniel_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_athaniel_home()}/logs/")
        return

    print(f"Log files in {display_athaniel_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'athaniel chat' to generate logs)")
