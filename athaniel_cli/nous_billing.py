...
billing screens drive. Companion to ``athaniel_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Athaniel already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``ATHANIEL_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# athaniel_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``ATHANIEL_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("ATHANIEL_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `athaniel portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
