"""CLI subcommand: `athaniel curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`athaniel curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `athaniel curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`athaniel curator status` and run `athaniel curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.set_pinned(args.skill, True):
        print(
            ...
            "`athaniel curator list-unmanaged` shows which skills the curator tracks."
        )
        return 1
    if not skill_usage.is_curator_managed(args.skill):
        ...
        print(
            ...
            f"`athaniel curator adopt {args.skill}` to put it under curator "
            "management)"
        )
        return 0
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `athaniel curator adopt <name>`, "
          "or all with `athaniel curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`athaniel curator unpin {args.skill}`"
        )
        return 1
    tok = skill_ledger.set_ledger_actor("user")
    try:
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.athaniel/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    print(
        "\nRoll back a single mutation with `athaniel curator rollback <id>`; "
        "whole-tree snapshots remain available via `athaniel curator rollback --list`."
    )
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `athaniel curator ledger` for entry ids, or use "
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`athaniel curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.athaniel/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.athaniel/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.athaniel/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `athaniel curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`athaniel curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by athaniel_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="athaniel curator")
    register_cli(parser)
