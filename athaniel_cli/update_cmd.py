"""Athaniel update pipeline — extracted from ``athaniel_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``athaniel_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``athaniel_cli.main`` reference — so existing call sites and test monkeypatches
that target ``athaniel_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``athaniel_cli.main``.
...
Imports are one-way: ``athaniel_cli.main`` imports this module, never the reverse
...
from athaniel_constants import get_default_athaniel_root, venv_python_path

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``athaniel_cli.main`` reference.

    Lets callers keep patching ``athaniel_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``athaniel_cli.main`` -> ``athaniel_cli.update_cmd`` stays one-way
    at import time.
    """
    from athaniel_cli import main
...
#: ``_purge_stale_athaniel_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "athaniel_cli.athaniel_logging",
    }
)


def _purge_stale_athaniel_modules() -> None:
    """Evict every cached Athaniel module after the checkout changed in-place.

    ``athaniel update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from athaniel_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``athaniel_cli.gateway`` does
    ``from athaniel_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Athaniel
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Athaniel module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Athaniel modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``athaniel update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``athaniel update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``athaniel_cli.config_defaults``,
    ``athaniel_cli.config``, and ``athaniel_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``athaniel_cli._subprocess_compat`` and
    ``athaniel_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "athaniel_cli.config_defaults",
        "athaniel_cli.config",
        "athaniel_cli.config_migrations",
        "athaniel_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from athaniel_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from athaniel_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``athaniel update`` historically
    ...
    caller): scope config reads/writes via the context-local ATHANIEL_HOME
    ...
    try:
        ...
        from athaniel_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_athaniel_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_athaniel_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'athaniel config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'athaniel config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'athaniel config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: athaniel config migrate")
        else:
            print()
            print("Skipped. Run 'athaniel config migrate' later to configure.")
    else:
    ...
    # context-local ATHANIEL_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from athaniel_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from athaniel_cli.backup import restore_cron_jobs_all_profiles
...
# Critical files that Athaniel must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "athaniel_cli/main.py",
    "athaniel_cli/config.py",
    "athaniel_cli/__init__.py",
    "athaniel_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "athaniel_constants.py",
)
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``athaniel.exe`` has
    ...
    Skipping is safe because Athaniel pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="athaniel-syntax-check-") as tmpdir:
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``athaniel`` startup; if any of them
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``athaniel update`` can be driven by a
    ...
    a later one.
    """
    from athaniel_constants import FIRST_PARTY_MODULE_ROOTS

    import secrets

    marker = f"__ATHANIEL_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # The root set is injected from athaniel_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('athaniel_') or %r:\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``athaniel update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `athaniel update`.
    ...
    print("  Preview now:  athaniel curator run --dry-run")
    print("  Pause it:     athaniel curator pause")
    print(
        "  Docs:         https://athaniel-brain.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `athaniel update`.
    ...
    try:
        ...
        return
    db_path = get_athaniel_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `athaniel sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    athaniel sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  athaniel sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``athaniel update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``athaniel update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: athaniel curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from athaniel_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    athaniel dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``athaniel_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.athaniel-update-staging"
    backup = f"{dst}.athaniel-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `athaniel update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``athaniel_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``athaniel update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && athaniel update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("ATHANIEL_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== athaniel-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Athaniel launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["athaniel.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Athaniel")
    print("  Desktop, a gateway, or another athaniel REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `athaniel` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``athaniel.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.athaniel-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".athaniel-update-staging", ".athaniel-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `athaniel update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Athaniel process the updater's drain
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from athaniel_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (athaniel gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Athaniel Brain by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `athaniel update "
            f"--branch {branch}`, or update against main with `athaniel update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/embreythecreator/athaniel-brain/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="athaniel-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"athaniel-brain-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for athaniel-brain itself
        ...
        # GitHub ZIPs extract to athaniel-brain-<branch>/
        extracted = os.path.join(tmp_dir, f"athaniel-brain-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Athaniel.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `athaniel update` to retry; if the agent won't start, "
            "reinstall from https://athaniel-brain.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    if uv_bin:
        ...
        from athaniel_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `athaniel update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: athaniel skills list-modified  "
    ...
    try:
        from athaniel_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from athaniel_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_athaniel_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_athaniel_home())
    ...
    try:
        from athaniel_cli.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "athaniel-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``athaniel update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Athaniel agent unexecutable.")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Athaniel behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes athaniel completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Athaniel couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Athaniel couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Athaniel behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Athaniel couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Athaniel couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/embreythecreator/athaniel-brain.git",
    "git@github.com:embreythecreator/athaniel-brain.git",
    "https://github.com/embreythecreator/athaniel-brain",
    "git@github.com:embreythecreator/athaniel-brain",
}

OFFICIAL_REPO_URL = "https://github.com/embreythecreator/athaniel-brain.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_athaniel_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_athaniel_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Athaniel repository.")
        print("  This means you may miss updates from embreythecreator/athaniel-brain.")
        print()
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/embreythecreator/athaniel-brain.git"
            )
            return False
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/embreythecreator/athaniel-brain.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/embreythecreator/athaniel-brain.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``athaniel update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from athaniel_constants import get_default_athaniel_root

    default_home = get_default_athaniel_root()
...
# ``fleet_restart_pending`` lives under ATHANIEL_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """ATHANIEL_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_athaniel_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from athaniel_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `athaniel update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `athaniel update` or `athaniel gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every athaniel-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_athaniel_modules()
    except Exception:
        pass
    try:
        from athaniel_cli.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from athaniel_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``athaniel update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: athaniel gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "athaniel.exe"
    lines = ["✗ Another athaniel.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Athaniel Desktop, exit any open `athaniel` REPLs, and")
    lines.append("  stop the gateway (`athaniel gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `athaniel update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from athaniel_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``athaniel update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Athaniel Desktop backend child, an unrelated ``athaniel`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``athaniel tools``."""
    try:
        from athaniel_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Athaniel Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``athaniel tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Athaniel Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Athaniel Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``athaniel update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `athaniel update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `athaniel update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Athaniel' editable-install extras or
    ...
    try:
        from athaniel_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from athaniel_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ATHANIEL_HOME/bin/uv``, but on Termux the official
    ...
    """
    from athaniel_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `athaniel update` is exactly the step
...
def _npm_lockfile_changed(athaniel_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `athaniel
    ...
    try:
        ...
        cache_file = athaniel_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(athaniel_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    athaniel update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `athaniel update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from athaniel_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `athaniel update`.")
            failed = []
            if any(
    ...
    # Athaniel profile using this checkout. Keep one per-checkout cache under the
    # shared Athaniel root rather than rerunning npm once per named profile.
    shared_athaniel_root = get_default_athaniel_root()
    ...
    # `athaniel update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_athaniel_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `athaniel update`.")
        return list(labels)
    ...
    nixos_env = with_athaniel_node_path(_m()._nixos_build_env())
    ...
    # `athaniel update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_athaniel_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.athaniel/logs/update.log`` only, never the terminal.

    During ``athaniel update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``athaniel update --check``: fetch and report without installing.
    ...
    from athaniel_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from athaniel_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``athaniel update`` without requiring a reinstall.  The
    ...
    ``athaniel`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/athaniel.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v athaniel'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/athaniel, code at /usr/local/lib/athaniel-brain).
    fhs_link = Path("/usr/local/bin/athaniel")
    ...
    path_comment = (
        "# Athaniel Brain — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``athaniel-acp`` launcher next to the ``athaniel`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``athaniel update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``athaniel-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Athaniel as not installed even when it is.

    The shim simply delegates to the sibling ``athaniel`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``athaniel`` / ``athaniel-acp``
    launchers into the managed binary dir ``$AthanielHome\bin`` and puts THAT
    ...
    ``athaniel_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``athaniel-acp`` is already present next to the
    ``athaniel`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        athaniel_cmd = bin_dir / "athaniel"
        acp_cmd = bin_dir / "athaniel-acp"
        try:
            if not (athaniel_cmd.is_file() or athaniel_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ATHANIEL_HOME under
      ``backups/`` (restorable via ``athaniel import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from athaniel_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from athaniel_constants import
        # get_athaniel_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from athaniel_cli.config import get_athaniel_home as _get_home
        ...
        try:
            from athaniel_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from athaniel_cli.update_receipt import record_step
    ...
    try:
        from athaniel_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from athaniel_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_athaniel_home so the user sees ~/.athaniel/...
    try:
        from athaniel_constants import get_athaniel_home, display_athaniel_home

        home = get_athaniel_home()
        try:
            display_path = f"{display_athaniel_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  athaniel import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``athaniel update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``athaniel update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run athaniel from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.athaniel-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".athaniel-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The athaniel.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m athaniel_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``athaniel
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Athaniel candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "athaniel.exe"}
        ):
            continue
        ...
        # this venv's path, or a `-m athaniel_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "athaniel_cli.main" in cmdline_low:
            try:
        ...
        # the `-m athaniel_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``athaniel_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``athaniel.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``athaniel`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `athaniel` launch will complete it")
...
def _athaniel_holder_subcommand(cmdline: str) -> str | None:
    """The actual Athaniel SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``athaniel_cli.main`` / ``athaniel(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("athaniel_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("athaniel", "athaniel.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``athaniel dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Athaniel processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Athaniel backend (if the Desktop app is open, close it)",
        "dashboard": "  ← athaniel dashboard (stop it: athaniel dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _athaniel_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Athaniel desktop app / other Athaniel terminals, then re-run:"
    )
    lines.append("    athaniel update")
    lines.append("  (or use `athaniel update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``athaniel update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from athaniel_cli._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``athaniel update`` through its terminal tool.
    ...
    try:
        from athaniel_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `athaniel update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Athaniel spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from athaniel_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    try:
        ...
        if scripts_dir is not None:
            for name in ("athaniel.exe", "athaniel"):
                candidate = scripts_dir / name
                if candidate.is_file():
                    athaniel = str(candidate)
                    break
    except Exception:
        athaniel = None
    if athaniel is None:
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (athaniel serve --host <ip> --port <port>)."
        )
    try:
        from athaniel_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning athaniel-setup, and the update-in-progress marker
    ...
    the update with "Athaniel is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Athaniel backend (``athaniel_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.athaniel-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "athaniel_cli.main" in argv_low and (
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_athaniel at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``athaniel_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Athaniel ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``athaniel.exe``) is open, NOTHING legitimate
    ...
    - Only Athaniel backends (``athaniel_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``athaniel.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.athaniel-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from gateway.status import get_process_start_time
    from athaniel_cli._subprocess_compat import pid_is_athaniel, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_athaniel(
                pid,
                expected_start_time=expected_start_time,
            ):
                logger.debug(
                    "Skipping taskkill of non-Athaniel or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``athaniel serve`` / ``athaniel dashboard`` argv.
    ...
    """
    if "athaniel_cli.main" not in (cmdline or "").lower():
        return False
    return _athaniel_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    athaniel.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Athaniel...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m athaniel_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: athaniel gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``athaniel gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from athaniel_cli import gateway_windows
        from athaniel_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``athaniel-gateway*.service``/``athaniel-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("athaniel-serve")`` alone would also accept the
        # unrelated ``athaniel-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "athaniel-gateway.service"
            or unit.startswith("athaniel-gateway-")
            or unit == "athaniel-serve.service"
            or unit.startswith("athaniel-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``athaniel-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``athaniel-serve*`` units (#83438) don't, so sending them
    ...
    (``athaniel-gateway-helper`` is fine — profile units are
    ``athaniel-gateway-<profile>`` — but ``athaniel-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "athaniel-gateway" or svc_name.startswith("athaniel-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from athaniel_cli.gateway import is_macos
    ...
    print("    athaniel gateway status")
    if any(not name.startswith("ai.athaniel.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.athaniel.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: athaniel gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: athaniel gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.athaniel.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``athaniel_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``athaniel-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"athaniel-gateway-{profile}",
        f"ai.athaniel.gateway-{profile}",
        f"gateway-{profile}",
    }
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``athaniel_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``athaniel update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "athaniel_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["ATHANIEL_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_ATHANIEL_GATEWAY", "ATHANIEL_GATEWAY", "ATHANIEL_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``athaniel_cli.gateway`` from the freshly pulled checkout)
    ...
    print("    athaniel gateway restart")
    print("    athaniel gateway status")
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``athaniel update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``athaniel-setup.exe``) executes
    ``$ATHANIEL_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_athaniel_home()) / "bootstrap-cache"
...
def _portable_git_candidates() -> list:
    ...
    The Athaniel-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped ATHANIEL_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``athaniel update`` must
    ...
    candidates = []
    try:
        for root in (get_default_athaniel_root(), Path(get_athaniel_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    Athaniel-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``athaniel update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``athaniel update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `athaniel desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "athaniel_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Athaniel-managed Node on PATH: when
    # `athaniel update` runs inside the desktop updater chain (Desktop →
    # athaniel-setup → athaniel update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from athaniel_constants import with_athaniel_node_path

    build_env = with_athaniel_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `athaniel desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from athaniel_constants import display_athaniel_home as _dhh
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo athaniel`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``athaniel update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/athaniel`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Athaniel broken.")
    print("  This usually happens after running athaniel or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    athaniel update")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from athaniel_cli.update_receipt import begin_update_receipt
    ...
    # running Athaniel runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another athaniel.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_athaniel_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `athaniel serve` backend) will keep .pyd
    ...
    # the athaniel.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Athaniel backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Athaniel backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .athaniel-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Athaniel is still
        ...
        if _venv_holders:
            ...
            # `athaniel serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Athaniel backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_athaniel_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Athaniel backend process(es) "
    ...
    # process by design (a CLI `athaniel update` IS the venv python), and an
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://athaniel-brain.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by athaniel.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Athaniel windows/gateways and re-run: athaniel update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Athaniel gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in athaniel_cli/config.py bricked
        # every user who ran ``athaniel update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``athaniel
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``athaniel update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``athaniel update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``athaniel doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && athaniel update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``athaniel update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_athaniel_home added to athaniel_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``athaniel`` launch finishes the install
        ...
        # symbols in athaniel_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `athaniel` again "
        ...
        if not import_ok:
            ...
            print("    Run `athaniel update` again — if it persists, reinstall:")
            print("    https://athaniel-brain.nousresearch.com")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Athaniel re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.embreythecreator.athaniel` (repeat "
        ...
        try:
            from athaniel_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_athaniel_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.athaniel/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ATHANIEL_HOME so
        # it is not affected by sync_skills()'s module-level ATHANIEL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ATHANIEL_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from athaniel_cli.profiles import backfill_profile_envs
        ...
        # check `athaniel curator status`. Self-stamps after printing so it
        ...
        # Self-heal the athaniel-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Athaniel on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("athaniel-acp launcher self-heal failed: %s", e)

        # Migrate the Windows athaniel launchers to the managed binary dir
        # (the default Athaniel root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (athaniel-brain\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `athaniel`
        ...
        # the managed clone under the default Athaniel root).
        try:
            from athaniel_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``athaniel update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from athaniel_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `athaniel update`
                # must stay fast; `athaniel computer-use install --upgrade`
        ...
        # When running as ``athaniel update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Athaniel modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... athaniel-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... athaniel-gateway*)
            ...
            try:
                from athaniel_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all athaniel-gateway* units (default + profiles) plus
            # athaniel-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: athaniel gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # athaniel-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `athaniel update` restart it automatically, allow\n"
                        ...
                        # path in `athaniel gateway restart`
            ...
            # Restart EVERY ai.athaniel.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: athaniel gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: athaniel -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_athaniel_home() / ".update_exit_code"
                    try:
            ...
            try:
                from athaniel_cli.update_receipt import record_gateway_restart
        ...
        # Warn if legacy Athaniel gateway unit files are still installed.
        # When both athaniel.service (from a pre-rename install) and the
        # current athaniel-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `athaniel update` surfaces the issue until the user migrates.
        try:
            from athaniel_cli.gateway import (
                has_legacy_athaniel_units,
                _find_legacy_athaniel_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_athaniel_units():
                print()
                print("⚠ Legacy Athaniel gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_athaniel_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (athaniel.service) fight the current")
                print("  athaniel-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    athaniel gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes athaniel-serve*,
        ...
        print("  athaniel model              # Select provider and model")
        ...
        try:
            from athaniel_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import athaniel_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``athaniel update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from athaniel_cli.main import main; main()" update --yes'
                    )
            try:
                from athaniel_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``athaniel_cli.gateway`` no longer imports,
