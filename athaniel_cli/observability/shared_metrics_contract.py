"""Bounded product contract for the first Athaniel shared-metrics slice."""
...
SCHEMA_KEY = "athaniel.metrics.schema_version"
SCHEMA_VERSION = "athaniel.metrics.event.v2"
MODEL_CALL_SCOPE = "athaniel.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "athaniel.task_run"
TOOL_CALL_SCOPE = "athaniel.tool_call"
CLIENT_ACTIVE_MARK = "athaniel.client.active"
TOOL_APPROVAL_MARK = "athaniel.tool_approval"
SKILL_LIFECYCLE_MARK = "athaniel.skill.lifecycle"
SKILL_LOAD_MARK = "athaniel.skill.load"
SUBSCRIBER_NAME = "athaniel.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "athaniel.client.active"
LEGACY_MODEL_CALL_METRIC = "athaniel.model_call.count"
MODEL_ROUTE_METRIC = "athaniel.model_route.count"
TASK_STARTED_METRIC = "athaniel.task_run.started"
TASK_FINISHED_METRIC = "athaniel.task_run.finished"
TOOL_CALL_METRIC = "athaniel.tool_call.count"
TOOL_APPROVAL_METRIC = "athaniel.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "athaniel.skill.lifecycle.count"
SKILL_LOAD_METRIC = "athaniel.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Athaniel installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(athaniel_version or "").strip()
    ...
    return {
        ...
        "athaniel_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("athaniel_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Athaniel logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("athaniel.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from athaniel_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Athaniel terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Athaniel registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "athaniel-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Athaniel tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Athaniel."""
