"""Direct NeMo Relay integration for Athaniel shared client metrics."""
...
class _Runtime:
    """Own shared-metrics state layered on the Athaniel core Relay host."""

    def __init__(self, host: relay_runtime.RelayRuntime | None = None) -> None:
        resolved_host = host or relay_runtime.get_runtime()
        if resolved_host is None:
            raise RuntimeError("Athaniel core Relay runtime is unavailable")
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Athaniel task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # physical attempt. Provider fallback resets Athaniel's
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Athaniel retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Athaniel shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Athaniel shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Athaniel shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_tool_call(
        ...
        try:
            ...
            logger.warning(
                "Athaniel shared-metrics tool call close failed",
                exc_info=True,
            )
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Athaniel shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Athaniel shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Athaniel shared metrics operation failed", exc_info=True)
            return None


def enabled() -> bool:
    """Return the shared-metrics policy for the active Athaniel profile."""
    profile_key = relay_runtime.current_profile_key()
    try:
        from athaniel_cli.config import read_raw_config_readonly
        ...
        logger.debug("Unable to read Athaniel shared-metrics policy", exc_info=True)
        value = False
    else:
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Athaniel lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning(
            "Athaniel shared metrics hook failed: %s", hook_name, exc_info=True
        )


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Athaniel's runtime registry."""
...
def start_task_run(
    *,
    ...
    """Start task metrics at the outer Athaniel execution boundary."""
    if not enabled():
        return
    ...
    *,
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Athaniel shared metrics initialization failed", exc_info=True)
