"""athaniel hooks — inspect and manage shell-script hooks.

Usage::

    athaniel hooks list
    athaniel hooks test <event> [--for-tool X] [--payload-file F]
    athaniel hooks revoke <command>
    athaniel hooks doctor

Consent records live under ``~/.athaniel/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.athaniel/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``athaniel hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: athaniel hooks {list|test|revoke|doctor}")
        print("Run 'athaniel hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.athaniel/config.yaml.")
        print("See `athaniel hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.athaniel/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `athaniel hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `athaniel hooks test` and `athaniel hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Athaniel wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `athaniel hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `athaniel hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `athaniel hooks doctor`.")
