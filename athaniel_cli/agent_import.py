"""athaniel import-agent — import Claude Code / Codex CLI setups into Athaniel.

Usage:
    athaniel import-agent                       # auto-detect ~/.claude or ~/.codex
    athaniel import-agent claude-code           # import from ~/.claude
    athaniel import-agent codex                 # import from ~/.codex
    athaniel import-agent claude-code --dry-run # preview only, no changes
    athaniel import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``athaniel claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ATHANIEL_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ATHANIEL_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ATHANIEL_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ATHANIEL_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ATHANIEL_HOME/skills/codex-imports/<name>/
...
deliberately via ``athaniel setup`` or config.yaml.
"""
...
# Same entry delimiter as the Athaniel memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`athaniel config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_athaniel.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Athaniel command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Athaniel glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Athaniel equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        athaniel_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ATHANIEL_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``athaniel import-agent`` (invoked from athaniel_cli.main)."""
    from athaniel_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: athaniel import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: athaniel import-agent claude-code   or   athaniel import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Athaniel — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: athaniel import-agent "
    ...
    print_info("Secrets:     never imported — run 'athaniel setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: athaniel import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'athaniel setup' "
               "to configure providers, or add them to ~/.athaniel/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from athaniel_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'athaniel setup' or ~/.athaniel/.env.")
        print()
