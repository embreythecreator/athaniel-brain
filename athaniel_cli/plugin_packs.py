...
A pack is a single YAML file (``athaniel-pack.yaml``) that pins a set of
...
* Capability consent is NEVER bulk-granted: after each plugin installs,
  ...
  as a normal ``athaniel plugins install`` (#64228).
    ...
    for entry in pack.plugins:
        ...
        try:
            if index_entries is None:
                from athaniel_cli.plugin_index import load_index

                index_entries, _src = load_index()
            from athaniel_cli.plugin_index import resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `athaniel skills install <id>`.[/dim]"
        )
    console.print(
...
def install_pack_plugins(
    ...
    *,
    ...
    single install (:func:`athaniel_cli.plugins_cmd._run_capability_consent`).
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-athaniel-pack") -> tuple[str, List[str]]:
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `athaniel plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def cmd_pack_show(source: str) -> None:
    """``athaniel plugins pack show <path-or-url>`` — dry-run review."""
    ...
    console.print("\n[dim]Dry run only. Install with `athaniel plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``athaniel plugins pack install <path-or-url>``.
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-athaniel-pack") -> None:
    """``athaniel plugins pack export [--enabled-only]`` — pack YAML on stdout."""
    from rich.console import Console
...
def pack_command(args) -> None:
    """Dispatch ``athaniel plugins pack <action>``."""
    ...
    if action == "install":
        ...
        cmd_pack_export(
            ...
            name=getattr(args, "name", None) or "my-athaniel-pack",
        )
    elif action == "show":
        ...
        Console().print(
            "[red]Error:[/red] Usage: athaniel plugins pack {install|export|show}"
        )
        sys.exit(1)
