"""Windows ConPTY bridge for the `athaniel dashboard` chat tab.

Drop-in counterpart to ``athaniel_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``athaniel_cli.web_server`` consumes: ``spawn``, ``read``,
