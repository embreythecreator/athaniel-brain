"""
Interactive setup wizard for Athaniel Brain.
...
from athaniel_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
from athaniel_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from athaniel_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://athaniel-brain.embreythecreator.com/docs"
...
# display_athaniel_home imported lazily at call sites (stale-module safety during athaniel update)
...
from athaniel_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from athaniel_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("✶ Athaniel Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Athaniel using environment variables or config commands:")
    print_info("  athaniel config set model.provider custom")
    print_info("  athaniel config set model.base_url http://localhost:8080/v1")
    print_info("  athaniel config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'athaniel setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "athaniel_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``ATHANIEL_NONINTERACTIVE=1`` (see ``athaniel_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ATHANIEL_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ATHANIEL_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from athaniel_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'athaniel setup')")


def _print_setup_summary(config: dict, athaniel_home):
    ...
    try:
        from athaniel_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Athaniel cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    athaniel model            (pick any provider/model)")
        print_info("    athaniel setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'athaniel setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from athaniel_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `athaniel tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from athaniel_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'athaniel setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'athaniel setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'athaniel tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'athaniel setup terminal'"))
    ...
    # Spotify (OAuth via athaniel auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'athaniel setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {athaniel_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('athaniel setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('athaniel setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('athaniel setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('athaniel setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('athaniel setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('athaniel config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('athaniel config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('athaniel config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('athaniel', Colors.GREEN)}              Start chatting")
    print(f"   {color('athaniel gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('athaniel doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `athaniel tools` and `athaniel setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``athaniel model``)
    ...
    provider added to ``athaniel model`` is automatically available here.
    ...
    # Delegate to the shared athaniel model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from athaniel_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: athaniel model")
    ...
    # on demand via `athaniel auth add`, `athaniel setup` vision, and
    # `athaniel setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from athaniel_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``athaniel model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from athaniel_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.athaniel/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Athaniel auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via athaniel setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'athaniel setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Athaniel runs shell commands and code.")
    ...
    # under ~/.athaniel/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `athaniel setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `athaniel setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `athaniel egress setup` then `athaniel egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `athaniel egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'athaniel-brain[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $ATHANIEL_HOME/bin is never on PATH, so a bare
            # which() misses the uv Athaniel installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'athaniel-brain[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ATHANIEL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ATHANIEL_MAX_ITERATIONS")
    ...
    print_info("  Run `athaniel setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ATHANIEL_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.athaniel/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from athaniel_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_athaniel_home(Path(get_athaniel_home()))
    except Exception:
        pass
...
def _profile_name_from_athaniel_home(athaniel_home) -> str | None:
    """Return the active profile name when ATHANIEL_HOME is a profile dir."""
    if athaniel_home.parent.name == "profiles":
        return athaniel_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Athaniel delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Athaniel to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from athaniel_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://athaniel-brain.embreythecreator.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://athaniel-brain.embreythecreator.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  athaniel config edit")
    print_info("   Open config in your editor:  athaniel config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from athaniel_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Athaniel from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'athaniel setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     athaniel config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `athaniel import` or `athaniel setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `athaniel setup tools` and `athaniel tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Athaniel profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``athaniel_cli.auth`` — lists every supported
    ...
    """
    try:
        from athaniel_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from athaniel_cli.config
    ...
    if section_key == "model":
        ...
        from athaniel_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_athaniel.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_athaniel migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_athaniel", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Athaniel.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Athaniel to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Athaniel at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Athaniel at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Athaniel at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Athaniel at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Athaniel equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Athaniel config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Athaniel.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Athaniel's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(athaniel_home: Path) -> bool:
    ...
    print_info("Athaniel can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: athaniel claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=athaniel_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: athaniel claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Athaniel configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Athaniel config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Athaniel (use athaniel claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``athaniel setup --portal`` and ``athaniel portal``. This is the
    ...
    Athaniel session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``athaniel setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``athaniel model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``athaniel portal`` always offers a model picker,
    ...
    print(color("│     ✶ Athaniel Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `athaniel portal` == quick setup's Nous step.
    try:
        from athaniel_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `athaniel portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `athaniel portal`: %s", exc)
        print()
    ...
    print_info("  Run `athaniel portal info` to inspect routing.")
    print_info("  Run `athaniel` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from athaniel_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``athaniel model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      athaniel setup           — full or quick (auto-detected)
      athaniel setup model     — just model/provider
      athaniel setup tts       — just text-to-speech
      athaniel setup terminal  — just terminal backend
      athaniel setup gateway   — just messaging platforms
      athaniel setup tools     — just tool configuration
      athaniel setup telemetry — just local shared metrics
      athaniel setup agent     — just agent settings
    """
    from athaniel_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ✶ Athaniel Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ✶ Athaniel Brain Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Athaniel Brain installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, athaniel_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Athaniel configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'athaniel setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(athaniel_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Athaniel?",
            [
                ...
                [
                    (
                        ...
                        lambda: _run_first_time_quick_setup(
                            config, athaniel_home, is_existing
                        ),
                    )
                ]
            )
    ...
    print_info(f"Data folder:  {athaniel_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'athaniel config edit'")
    ...
    # Tune later with `athaniel setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from athaniel_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, athaniel_home)


def _run_first_time_quick_setup(config: dict, athaniel_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``athaniel setup <section>``
    or switch providers with ``athaniel model``.
    """
    from athaniel_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'athaniel setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `athaniel import` from another machine).
        from athaniel_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    athaniel setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  athaniel setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Athaniel.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Athaniel update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``athaniel-brain`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Athaniel) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``athaniel tools`` (which
    ...
    try:
        ...
        from athaniel_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``athaniel setup agent`` /
    ``athaniel config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, athaniel_home, is_existing: bool):
    ...
    print_info("essential `athaniel-brain` skill is always kept so the agent")
    print_info("can help you drive and configure Athaniel itself.")
    print()
    ...
    if path == 0:
        ...
        # `athaniel update` runs don't re-inject them. Essential skills (the
        # `athaniel-brain` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        athaniel tools")
        print_info("  Seed skills:         athaniel skills opt-in --sync")
        print_info("  Add MCP servers:     athaniel mcp add")
        print_info("  Enable plugins:      athaniel plugins")
        print_info("  Tune agent settings: athaniel setup agent")
        print()
        _print_setup_summary(config, athaniel_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, athaniel_home)


def _blank_slate_walkthrough(config: dict, athaniel_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `athaniel-brain` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `athaniel-brain`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`athaniel update` runs from re-injecting them. Opt back in any")
            print_info("time with `athaniel skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `athaniel tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `athaniel plugins list` / `athaniel plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `athaniel plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `athaniel mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `athaniel mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   athaniel tools")
    print_info("  Seed skills:         athaniel skills opt-in --sync")
    print_info("  Add MCP servers:     athaniel mcp add")
    print_info("  Tune agent settings: athaniel setup agent")
    print()
...
def _run_quick_setup(config: dict, athaniel_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'athaniel setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Athaniel to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'athaniel setup gateway'.")
