"""Implementation of the ``athaniel bundles`` CLI subcommand.

Mirrors the structure of ``athaniel_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `athaniel bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  athaniel bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``athaniel bundles`` argparse tree.

    Called from ``athaniel_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `athaniel bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``athaniel bundles <subcommand>`` to the right handler."""
