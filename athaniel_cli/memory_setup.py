"""athaniel memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``athaniel memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_athaniel_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as athaniel tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``athaniel update`` heals the active memory provider
    ...
    # data volume (ATHANIEL_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'athaniel memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(athaniel_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.athaniel/plugins/ and try again.\n")
        return
...
def _write_env_vars(
    env_writes: dict,
    athaniel_home: str | os.PathLike[str] | None = None,
) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    Delegates to ``athaniel_cli.config.save_env_value`` so every key flows
    ...
    ``ATHANIEL_HOME`` / etc.), CR/LF stripping on the value, and the atomic
    ...
    ``.env`` -> ``os.environ`` chain on the next Athaniel startup, and the
    ...
    ``athaniel_home`` may be supplied by plugin ``post_setup`` hooks that
    already received an explicit home directory (e.g. a non-default
    profile). It is applied through the context-local Athaniel home override
    ...
    token = set_athaniel_home_override(athaniel_home) if athaniel_home is not None else None
    try:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.athaniel/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.athaniel/plugins/")
