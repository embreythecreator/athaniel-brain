"""CLI handlers for ``athaniel secrets bitwarden ...``.
...
# that process is ``athaniel update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``athaniel_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``athaniel_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import athaniel_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Athaniel process."
    )
    console.print(
        "  Status:  [cyan]athaniel secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]athaniel secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]athaniel secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]athaniel secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Athaniel will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `athaniel secrets bitwarden setup` to pick a "
    ...
    console.print(
        ...
        "Takes effect on the next Athaniel invocation."
    )
    if not bw_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the Bitwarden integration is currently disabled — "
            "run `athaniel secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`athaniel secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]athaniel[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]athaniel secrets bitwarden setup[/cyan] and pick EU or "
