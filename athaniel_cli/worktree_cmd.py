"""``athaniel worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``athaniel_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    athaniel worktree list             # audit: verdict + reason per tree
    athaniel worktree prune            # reap safe trees + merged branches
    athaniel worktree prune --dry-run  # show the plan, change nothing
    athaniel worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `athaniel worktree prune`."
        )
