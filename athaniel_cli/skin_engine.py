"""Athaniel skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Athaniel itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.athaniel/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.athaniel/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Athaniel Brain"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ✶"              # Shown on exit
      response_label: " ✶ Athaniel "       # Response box header label
...
.. code-block:: python
    ...
    from athaniel_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Athaniel Brain"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.athaniel/skins/
...
Drop a YAML file in ``~/.athaniel/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces ATHANIEL_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces ATHANIEL_SIGIL)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Athaniel — plain monochrome",
        "colors": {
            ...
            # Deliberately does not use "light_colors" (see SkinConfig):
            # every value here is either "default" (follows the terminal's
            # own fg, light or dark) or a neutral grey that reads fine on
            # either background, so there is nothing to overlay.
        ...
        "branding": {
            "agent_name": "Athaniel Brain",
            "welcome": "Welcome to Athaniel Brain! Type your message or /help for commands.",
            "goodbye": "Goodbye! ✶",
            "response_label": " ✶ Athaniel ",
            "prompt_symbol": "❯",
...
def _skins_dir() -> Path:
    ...
    return get_athaniel_home() / "skins"
