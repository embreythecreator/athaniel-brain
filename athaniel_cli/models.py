...
Add, remove, or reorder entries here — both `athaniel setup` and
`athaniel` provider-selection will pick up the change automatically.
"""
...
from athaniel_cli.urllib_security import open_credentialed_url, url_origin
...
_ATHANIEL_USER_AGENT = f"athaniel-cli/{_ATHANIEL_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from athaniel_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `athaniel model`
    flow without maintaining a separate static list.
    """
    from athaniel_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $ATHANIEL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Athaniel-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ATHANIEL_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see athaniel_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Athaniel that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Athaniel that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_athaniel_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$ATHANIEL_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  athaniel model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `athaniel model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `athaniel model` TUI
...
#
# Some vendors expose several Athaniel provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `athaniel model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``athaniel model``, the
...
# In-repo fallback for the model Athaniel silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from athaniel_cli.model_catalog import get_default_model_from_cache
...
# ``athaniel model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in athaniel_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``athaniel auth add openai-codex`` without
    ``athaniel model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``athaniel model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    athaniel-brain is tool-calling-first — every provider path assumes the model
...
# copy, `athaniel -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from athaniel_constants import get_athaniel_home
    return get_athaniel_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _ATHANIEL_USER_AGENT}
    try:
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from athaniel_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — athaniel-brain
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Athaniel lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from athaniel_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from athaniel_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return athaniel-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; athaniel's picker expects
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from athaniel_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``athaniel model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from athaniel_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:athaniel-3                           →  ("nous", "athaniel-3")
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _ATHANIEL_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from athaniel_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Athaniel' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``athaniel_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Athaniel should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``athaniel auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.athaniel/.env``.
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from athaniel_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Athaniel release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from athaniel_cli.codex_models import get_codex_model_ids
        ...
        # a Athaniel release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from athaniel_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Athaniel release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``athaniel model`` shows.
            from athaniel_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $ATHANIEL_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from athaniel_constants import get_athaniel_home
    return get_athaniel_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ATHANIEL_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `athaniel config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ATHANIEL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Athaniel' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Athaniel' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from athaniel_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def _fetch_opencode_free_models(
    ...
    from athaniel_cli.urllib_security import open_credentialed_url
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ATHANIEL_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"athaniel-brain/{_ATHANIEL_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=athaniel"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from athaniel_constants import get_athaniel_home
    return get_athaniel_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `athaniel moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Athaniel will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Athaniel will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Athaniel
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Athaniel cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Athaniel actually ships.  (#46850)
            #
            ...
            # `athaniel chat` already accepts these models via
