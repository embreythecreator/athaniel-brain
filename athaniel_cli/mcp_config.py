"""
MCP Server Management CLI — ``athaniel mcp`` subcommand.

Implements ``athaniel mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.athaniel/config.yaml under the ``mcp_servers`` key.
"""
...
from athaniel_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_athaniel_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from athaniel_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.athaniel/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``athaniel mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import AthanielTokenStorage
        return AthanielTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # athaniel_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  athaniel mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  athaniel mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  athaniel mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_athaniel_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: athaniel mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_athaniel_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `athaniel mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  athaniel mcp add <name> --url <endpoint>')
        _info('  athaniel mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``athaniel mcp login`` and
    ``athaniel mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `athaniel mcp remove` + `athaniel mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `athaniel mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Athaniel desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `athaniel mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``athaniel mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``athaniel mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: athaniel mcp reauth <name>   |   athaniel mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── athaniel mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'athaniel mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``athaniel mcp`` subcommands."""
    ...
    if action == "picker":
        from athaniel_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from athaniel_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from athaniel_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `athaniel plugin`.
        from athaniel_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("athaniel mcp                                    Open the catalog picker (default)")
        _info("athaniel mcp catalog                            List Nous-approved MCPs")
        _info("athaniel mcp install <name>                     Install a catalog MCP")
        _info("athaniel mcp serve                              Run as MCP server")
        _info("athaniel mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("athaniel mcp add <name> --command <cmd>         Add a stdio server")
        _info("athaniel mcp add <name> --preset <preset>       Add from a known preset")
        _info("athaniel mcp remove <name>                      Remove a server")
        _info("athaniel mcp list                               List configured servers")
        _info("athaniel mcp test <name>                        Test connection")
        _info("athaniel mcp configure <name>                   Toggle tools")
        _info("athaniel mcp login <name>                       Re-authenticate OAuth")
        _info("athaniel mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
