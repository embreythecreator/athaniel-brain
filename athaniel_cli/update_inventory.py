...
One read-only pass that answers, BEFORE any mutation: what Athaniel runtimes
...
`detect_install_method`) — so `athaniel update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `athaniel update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `athaniel serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Athaniel runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "athaniel update"
...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from athaniel_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"athaniel -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        ...
        try:
            from athaniel_cli.image_provenance import read_image_provenance
    ...
    try:
        from athaniel_cli.gateway import _get_service_pids
    ...
    try:
        from athaniel_cli.gateway import find_windows_gateway_services
    ...
    try:
        from athaniel_cli.gateway import find_profile_gateway_processes
    ...
    # launched `athaniel serve --host <ip>` for a remote Desktop, or a
    # long-lived `athaniel dashboard`. Every serve/dashboard registers itself
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Athaniel services: none detected — code swap only.")
        return
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "athaniel-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "athaniel-gateway" in svc)
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    print("      athaniel gateway restart                # active profile")
    print("      athaniel -p <profile> gateway restart   # named profile")
    return True
