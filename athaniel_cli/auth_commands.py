...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_athaniel_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:athaniel_pkce",
    ...
    if provider == "nous":
        ...
        # <athaniel-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `athaniel --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``athaniel auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``athaniel auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`athaniel auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Athaniel reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `athaniel auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `athaniel auth` is called bare."""
