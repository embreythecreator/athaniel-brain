"""CLI handlers for ``athaniel migrate ...``.

Currently exposes only ``athaniel migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``athaniel migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: athaniel migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `athaniel migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `athaniel doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
