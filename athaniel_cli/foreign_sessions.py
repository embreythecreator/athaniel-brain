...
``athaniel sessions import`` (and ``--resume @claude`` / ``--resume @codex``)
let a user pull a conversation they started in another agent CLI into
Athaniel and continue it here.
...
role-alternation invariant Athaniel enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Athaniel SessionDB.

    Returns the new Athaniel session id.  The foreign file is only read.
...
def pick_foreign_session(
    ...
    if not sys.stdin.isatty():
        print(
            ...
            "  athaniel sessions import --from claude|codex <path>"
        )
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`athaniel sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  athaniel --resume {session_id}")
    return session_id
