"""``athaniel slack ...`` CLI subcommands.

Today only ``athaniel slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ athaniel slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Athaniel. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Athaniel deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Athaniel on Slack's older Assistant messaging
    ...
    """
    from athaniel_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Athaniel in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Athaniel in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Athaniel agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``athaniel_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ATHANIEL_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Athaniel")
    ...
    """
    name = getattr(args, "name", None) or "Athaniel"
    description = getattr(args, "description", None) or "Your Athaniel agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "athaniel slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"athaniel slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "athaniel slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "athaniel slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Athaniel app\n"
