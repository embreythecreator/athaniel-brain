"""``athaniel portal`` — the human-readable entry point for Nous Portal.

Running ``athaniel portal`` with no subcommand performs the one-shot Portal
...
``athaniel auth add nous --type oauth`` (which still works), is identical to
``athaniel setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``athaniel auth`` or ``athaniel tools``. It's the onboarding + discovery
...
DOCS_URL = "https://athaniel-brain.embreythecreator.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `athaniel model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `athaniel portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `athaniel auth add nous --type
    oauth`. It reuses the exact wiring behind `athaniel setup --portal` (which in
    ...
    """
    from athaniel_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `athaniel portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `athaniel portal` is the
        # human-readable alias for `athaniel auth add nous --type oauth` /
    ...
    print("Run `athaniel portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `athaniel portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `athaniel portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `athaniel auth add "
            "nous --type oauth`, identical to `athaniel setup --portal`). "
