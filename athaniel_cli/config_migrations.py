...
``athaniel_cli.config.migrate_config``. Each step is a function
...
``athaniel_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``athaniel_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``athaniel_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``athaniel_cli.config`` (e.g.
``patch("athaniel_cli.config.read_raw_config", ...)``) keep working, because
...
#: ATHANIEL_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_athaniel_home()}/config.yaml and run `athaniel setup` to "
...
def _cfg():
    """Return the live ``athaniel_cli.config`` module (lazy, cycle-free)."""
    from athaniel_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_athaniel_home = _c.get_athaniel_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$ATHANIEL_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `athaniel plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `athaniel curator status` has no
    ...
    #   3. Creates `~/.athaniel/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_athaniel_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_athaniel_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `athaniel config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `athaniel config set`"
                )
...
# behavior set it to true explicitly via `athaniel config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from athaniel_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "ATHANIEL_NEMO_RELAY_PLUGINS_TOML."
    )
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # provider surface (`video_generate`, `athaniel tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `athaniel tools` → "
