...
to the resolved absolute path of the client binary.  Athaniel' interpreter is
...
* Dynamically-linked builds look up ``libpython`` via
  ...
  no dylib — every athaniel command, including update/doctor, died in dyld
  (#95425).
...
from athaniel_cli.managed_uv import _RUNTIME_DIR_NAME
...
def _install_anchor(venv_dir: Path, source_file: Path) -> None:
    ...
    try:
        ...
        try:
            from athaniel_cli.managed_uv import _macos_sign_managed_python
...
def tcc_anchor_state(project_root: Path | None = None) -> tuple[str, str]:
    """Report the anchor state for ``athaniel doctor``.
