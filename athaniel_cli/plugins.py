"""
Athaniel Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with athaniel-brain;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.athaniel/plugins/<name>/``
3. **Project plugins** – ``./.athaniel/plugins/<name>/`` (opt-in via
   ``ATHANIEL_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``athaniel_brain.plugins``
   entry-point group.
...
from athaniel_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from athaniel_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from athaniel_cli.plugin_capabilities import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``ATHANIEL_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ATHANIEL_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ATHANIEL_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.athaniel/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ATHANIEL_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ATHANIEL_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Athaniel loggers.
    """
    ...
    logger.debug(
        "ATHANIEL_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Athaniel' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by athaniel_cli.kanban_db when a task
    ...
    # separate `athaniel -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `athaniel kanban dispatch`),
    ...
    # ``athaniel kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: athaniel_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "athaniel_brain.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "athaniel_brain.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``athaniel_brain.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- athaniel-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- athaniel-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- athaniel-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``athaniel:<event>``.
ATHANIEL_EVENT_NAMESPACE = "athaniel"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Athaniel home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "athaniel", "depends",
}

# Highest manifest schema version this Athaniel understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Athaniel "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `athaniel plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.athaniel/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``athaniel plugins list``. For a flat plugin at
    ...
    # ``athaniel_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Athaniel never
    ...
    # (``athaniel plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Athaniel paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_athaniel_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from athaniel_cli import config as config_mod
        ...
        # threads. Other Athaniel config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from athaniel_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Athaniel profile name (e.g. ``"default"``).

        Derived from ``ATHANIEL_HOME`` via
        :func:`athaniel_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.athaniel/profiles/<name>``, or ``"custom"`` when
        ``ATHANIEL_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Athaniel core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``athaniel honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``athaniel <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`athaniel_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from athaniel_cli.dashboard_auth import DashboardAuthProvider
        from athaniel_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_athaniel_dotenv()`` startup — after
        ``~/.athaniel/.env`` loads, before Athaniel reads credentials — when
        ...
        NOTE ON TIMING: ``load_athaniel_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_athaniel_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Athaniel' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Athaniel' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``athaniel model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: athaniel_cli.main imports plugins indirectly
        from athaniel_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``athaniel:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``athaniel:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, ATHANIEL_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{ATHANIEL_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``athaniel:<event>``
    ...
    def register_skill(
        ...
        ``~/.athaniel/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or athaniel_home_key()
        ...
        # by resolved athaniel home). The ledger is therefore keyed per manager
        # — i.e. per (athaniel_home, plugin_id) — and every release/restore
        ...
        # contributed, so `athaniel plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("ATHANIEL_SAFE_MODE"):
                logger.info("ATHANIEL_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_athaniel_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from athaniel_cli.env_loader import load_athaniel_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Athaniel plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Athaniel core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Athaniel core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Athaniel Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with athaniel and must
            ...
            # all ~20 of them added several seconds to every `athaniel`
            # invocation — including plain `athaniel chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Athaniel ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `athaniel plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from athaniel_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_athaniel_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_athaniel_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.athaniel/plugins/)
        user_dir = get_athaniel_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.athaniel/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("ATHANIEL_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".athaniel" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ATHANIEL_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("ATHANIEL_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from athaniel_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from athaniel_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Athaniel process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``athaniel_brain.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``athaniel plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`athaniel plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``athaniel tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `athaniel plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Athaniel validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Athaniel does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `athaniel plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from athaniel_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_athaniel_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``athaniel_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``athaniel_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Athaniel home (profile switch reusing a slug
        # like "athaniel-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"athaniel-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved Athaniel home -> PluginManager. Athaniel supports
# multiple profiles via different ATHANIEL_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_athaniel_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# athaniel_constants.set_athaniel_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_athaniel_home() / "plugins"`` and some
    plugins (notably context engines such as athaniel-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Athaniel home (env var *or* the context-local
    ``set_athaniel_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Athaniel home instead of
    being one process-wide singleton.
    """
    try:
        return get_athaniel_home().expanduser().resolve()
    except Exception:
        return get_athaniel_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``athaniel_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``athaniel_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Athaniel profile/home.
    ...
    profile switch — via ``ATHANIEL_HOME`` or the context-local
    ``set_athaniel_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=athaniel_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_athaniel_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from athaniel_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="athaniel-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``athaniel:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``athaniel tools`` TUI so plugin-provided toolsets appear
