...
_TASK_NAME_DEFAULT = "Athaniel_Gateway"
_TASK_DESCRIPTION = "Athaniel Brain Gateway - Messaging Platform Integration"
...
def _preserve_athaniel_home_path(path: str | Path) -> str:
    """Render Athaniel-owned paths under the configured ATHANIEL_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\athaniel`` as a symlink/junction to
    ...
    path lives under ATHANIEL_HOME.
    """
    candidate = Path(path)
    try:
        from athaniel_cli.config import get_athaniel_home

        home = Path(get_athaniel_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Athaniel profile."""
    from athaniel_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "athaniel_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ATHANIEL_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ATHANIEL_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ATHANIEL_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ATHANIEL_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ATHANIEL_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ATHANIEL_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ATHANIEL_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ATHANIEL_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ATHANIEL_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Athaniel_Gateway``
    Named profile X: ``Athaniel_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during athaniel_cli boot.
    from athaniel_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\athaniel\\gateway-service\\<task_name>.cmd``
    (or ``<ATHANIEL_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Athaniel installs stay self-contained).
    """
    _assert_windows()
    from athaniel_cli.config import get_athaniel_home

    script_dir = Path(get_athaniel_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ATHANIEL_HOME`` whenever it
    ...
    source checkout only if ``ATHANIEL_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ATHANIEL_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m athaniel_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``athaniel gateway run``.
    ...
    lines.append(f'set "ATHANIEL_HOME={athaniel_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ATHANIEL_GATEWAY_DETACHED=1"')
    lines.append('set "ATHANIEL_SUPERVISED_CHILD=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports athaniel_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_athaniel_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_athaniel_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_athaniel_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "athaniel_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_athaniel_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_athaniel_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('ATHANIEL_HOME')}) = {_quote_vbs_string(athaniel_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('ATHANIEL_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('ATHANIEL_SUPERVISED_CHILD')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_athaniel_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_athaniel_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    athaniel_home = str(Path(get_athaniel_home()))
    profile_arg = _profile_arg(athaniel_home)

    content = _build_gateway_cmd_script(python_path, working_dir, athaniel_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, athaniel_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Athaniel builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_athaniel_home_path(get_python_path())
    )
    project_root = _preserve_athaniel_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    athaniel_home = str(Path(get_athaniel_home()))
    profile_arg = _profile_arg(athaniel_home)

    argv = [python_exe, "-m", "athaniel_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "ATHANIEL_GATEWAY_DETACHED": "1",
        "ATHANIEL_SUPERVISED_CHILD": "1",
        "VIRTUAL_ENV": _preserve_athaniel_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_athaniel_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (ATHANIEL_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m athaniel_cli.main
    ...
    from athaniel_cli.config import get_athaniel_home
    from athaniel_cli.gateway import PROJECT_ROOT
    ...
    try:
        athaniel_home = str(Path(get_athaniel_home()).resolve())
    except Exception:
        athaniel_home = ""
    ...
    if athaniel_home:
        env_overlay["ATHANIEL_HOME"] = athaniel_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m athaniel_cli.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_athaniel_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ATHANIEL_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ATHANIEL_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from athaniel_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `athaniel -p <profile> gateway install` must be safe.
    ...
    from athaniel_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"athaniel {profile_arg} gateway start" if profile_arg else "athaniel gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: athaniel gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Athaniel gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: athaniel gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: athaniel gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from athaniel_cli.config import get_athaniel_home
        print(f"    type {Path(get_athaniel_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_athaniel_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from athaniel_cli.config import get_athaniel_home

    athaniel_home = Path(get_athaniel_home())
    print()
    print("Next steps:")
    print("  athaniel gateway status                      # Check status")
    print(f"  type {athaniel_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Athaniel gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: athaniel gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: athaniel gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: athaniel gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `athaniel gateway status`."
        )
