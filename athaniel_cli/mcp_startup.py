...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from athaniel_cli.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local ATHANIEL_HOME override (profile
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``athaniel -z "..."`` / ``-q``), the larger
    ...
    try:
        from athaniel_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``athaniel dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``athaniel -z``) can construct
