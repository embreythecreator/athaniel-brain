"""
Gateway subcommand for athaniel CLI.

Handles: athaniel gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_athaniel_home is imported lazily at call sites to avoid ImportError
# when athaniel_constants is cached from a pre-update version during `athaniel update`.
...
from athaniel_cli.colors import Colors, color
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.athaniel.gateway*`` LaunchAgent — the update path needs the whole
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # mirrors the systemd branch's ``athaniel-gateway*`` pattern above.
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.athaniel.gateway*
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "athaniel CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `athaniel update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``athaniel gateway status`` from falsely counting the ``athaniel`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``athaniel gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_athaniel_home().resolve())
    # Forward slashes on both sides of the ATHANIEL_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"athaniel_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ATHANIEL_HOME
        ...
        # ATHANIEL_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "athaniel_home=" in command_lc
            and f"athaniel_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``athaniel update`` hung exactly there
            ...
            # watchdog probe.
            from athaniel_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``athaniel update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Athaniel profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    *,
    ...
    """Return running gateway PIDs mapped to Athaniel profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid, get_running_pid_identity_strict
        from athaniel_cli.profiles import list_profiles
    except Exception:
        if strict:
            raise
        return processes
...
def find_windows_gateway_services(
    *,
    ...
    validated profile gateway PID comes from Athaniel's own PID file, and its
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "athaniel_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m athaniel_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``athaniel update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Athaniel's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ATHANIEL_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``athaniel update`` users
    # with no running gateway until they re-invoke ``athaniel gateway``
    ...
    from athaniel_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (ATHANIEL_HOME,
    ...
    if sys.platform == "win32":
        try:
            from athaniel_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ATHANIEL_HOME) the windowless
        # base interpreter needs to import athaniel_cli.  Empty on POSIX, where
...
def _athaniel_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ATHANIEL_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("ATHANIEL_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_athaniel_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ATHANIEL_HOME``.

    Under ``sudo``, ``ATHANIEL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_athaniel_home` falls back to ``/root/.athaniel`` — the wrong
    profile. The unit file pins ``ATHANIEL_HOME`` for the actual gateway
    ...
    unit_home = (_athaniel_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ATHANIEL_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ATHANIEL_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ATHANIEL_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}athaniel gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}athaniel gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from athaniel_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: athaniel gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `athaniel gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``athaniel gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from athaniel_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (athaniel_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Athaniel_Gateway /
            # Athaniel_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `athaniel gateway install` deployment.
            from athaniel_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Athaniel_Gateway"
    ...
    try:
        ...
        # whole ai.athaniel.gateway* fleet — not just the current profile's
    ...
    # healthy standalone gateway (no service supervisor — e.g. `athaniel
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ATHANIEL_HOME-scoped).
...
from athaniel_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Athaniel Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``athaniel gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ATHANIEL_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ATHANIEL_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from athaniel_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "athaniel-gateway"
SERVICE_DESCRIPTION = "Athaniel Brain Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ATHANIEL_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ATHANIEL_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from athaniel_constants import get_default_athaniel_root

    home = get_athaniel_home().resolve()
    default = get_default_athaniel_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ATHANIEL_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(athaniel_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ATHANIEL_HOME is a named profile.

    For ``~/.athaniel/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        athaniel_home: Optional explicit ATHANIEL_HOME path. Defaults to the current
            ``get_athaniel_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Athaniel root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_athaniel_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from athaniel_constants import get_default_athaniel_root

    home = Path(athaniel_home or str(get_athaniel_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_athaniel_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(athaniel_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".athaniel"
    try:
        Path(athaniel_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(athaniel_home, default_root=target_root)
    except ValueError:
        return _profile_arg(athaniel_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this ATHANIEL_HOME.

    Default ``~/.athaniel`` returns ``athaniel-gateway`` (backward compatible).
    Profile ``~/.athaniel/profiles/coder`` returns ``athaniel-gateway-coder``.
    Any other ATHANIEL_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Athaniel installs that predate the
# athaniel-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (athaniel-gateway-*.service) and unrelated third-party
# "athaniel" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("athaniel.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "athaniel_cli.main gateway",
    "athaniel_cli/main.py gateway",
    "gateway/run.py",
    " athaniel gateway ",
    "/athaniel gateway ",
)
...
def _find_legacy_athaniel_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Athaniel gateway units.

    Detects unit files installed by older Athaniel versions that used a
    different service name (e.g. ``athaniel.service`` before the rename to
    ``athaniel-gateway.service``). When both a legacy unit and the current
    ``athaniel-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``athaniel-gateway-coder.service`` and unrelated third-party
      ``athaniel-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``athaniel.service`` running an unrelated
...
def has_legacy_athaniel_units() -> bool:
    """Return True when any legacy Athaniel gateway unit files exist."""
    return bool(_find_legacy_athaniel_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Athaniel gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_athaniel_units()
    if not legacy:
        return
    print_warning("Legacy Athaniel gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current athaniel-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    athaniel gateway migrate-legacy")


def remove_legacy_athaniel_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Athaniel gateway unit files.

    Iterates over whatever ``_find_legacy_athaniel_units()`` returns — which is
    ...
    legacy = _find_legacy_athaniel_units()
    if not legacy:
        print("No legacy Athaniel gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Athaniel gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: athaniel gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo athaniel gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    athaniel gateway uninstall")
    print_info("    sudo athaniel gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: athaniel gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``athaniel setup`` (end of wizard)
    and ``athaniel import``: if this host supports a service manager and no
    ...
    ``athaniel gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   athaniel gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: athaniel gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: athaniel gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Athaniel often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.athaniel`` → ``ai.athaniel.gateway.plist`` (backward compatible).
    Profile ``~/.athaniel/profiles/coder`` → ``ai.athaniel.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.athaniel.gateway-{suffix}" if suffix else "ai.athaniel.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    ``get_default_athaniel_root()``), NOT by globbing ``~/Library/LaunchAgents``:
    the LaunchAgents directory is shared per-user, so a sandboxed
    ``ATHANIEL_HOME`` (tests, capture sandboxes, side-by-side installs) must
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.athaniel.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.athaniel.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # athaniel_constants cached from before this symbol existed. See
            # _reload_athaniel_constants() in athaniel_cli/managed_uv.py.
            from athaniel_cli.managed_uv import _reload_athaniel_constants

            venv_python_path = _reload_athaniel_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.athaniel/athaniel-brain  -> /home/alice/.athaniel/athaniel-brain
      /opt/athaniel                 -> /opt/athaniel  (kept as-is)
...
def _athaniel_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ATHANIEL_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_athaniel_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.athaniel                    → /home/alice/.athaniel
      /root/.athaniel/profiles/coder     → /home/alice/.athaniel/profiles/coder
      /opt/custom-athaniel               → /opt/custom-athaniel  (kept as-is)
    """
    current_athaniel_raw = os.environ.get("ATHANIEL_HOME", "").strip()
    current_athaniel = (
        Path(current_athaniel_raw).expanduser()
        if current_athaniel_raw
        else get_athaniel_home()
    )
    ...
    # different ATHANIEL_HOME into the generated service unit.
    current_default = Path.home() / ".athaniel"
    target_default = Path(target_home_dir) / ".athaniel"

    # Default ~/.athaniel → remap to target user's default
    if current_athaniel == current_default:
        return str(target_default)

    # Profile or subdir of ~/.athaniel → preserve the relative structure
    try:
        relative = current_athaniel.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.athaniel) — keep as-is
        return str(current_athaniel)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    athaniel_node = athaniel_home / "node" / "bin"
    if _is_dir(athaniel_node):
        candidates.append(str(athaniel_node))
    athaniel_nm = athaniel_home / "node_modules" / ".bin"
    if _is_dir(athaniel_nm):
        candidates.append(str(athaniel_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m athaniel_cli.main``, so module
    ...
    dir, or a clone that ``athaniel update`` later relocates/removes — the path
    ...
    ``ATHANIEL_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ATHANIEL_HOME
...
def _systemd_watchdog_seconds(athaniel_home: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = None
    reset_home_override = None
    if athaniel_home is not None:
        from athaniel_constants import (
            reset_athaniel_home_override,
            set_athaniel_home_override,
        )

        override_token = set_athaniel_home_override(athaniel_home)
        reset_home_override = reset_athaniel_home_override
    try:
...
def _systemd_watchdog_service_fields(
    athaniel_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(athaniel_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], athaniel_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Athaniel-managed Node under ``$ATHANIEL_HOME/node`` goes first when it
    ...
    *athaniel_root* is the Athaniel home the unit will run against. System units
    ...
    """
    from athaniel_constants import (
        athaniel_managed_node_tree_present,
        iter_athaniel_node_dirs,
    )

    managed_node_present = athaniel_managed_node_tree_present(athaniel_root)
    for directory in iter_athaniel_node_dirs(athaniel_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target Athaniel home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Athaniel home is known — probing here would stat the calling
    ...
    if system:
        ...
        athaniel_home = _athaniel_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            athaniel_home
        )
        profile_arg = _profile_arg_for_target_user(athaniel_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ATHANIEL_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(athaniel_home) if athaniel_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped athaniel_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(athaniel_home) if athaniel_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m athaniel_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="ATHANIEL_HOME={athaniel_home}"
Environment="ATHANIEL_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
"""
    ...
    athaniel_home = str(get_athaniel_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        athaniel_home
    )
    profile_arg = _profile_arg(athaniel_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m athaniel_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ATHANIEL_HOME={athaniel_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned ATHANIEL_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo athaniel gateway … --system``, ATHANIEL_HOME is often stripped
    # and falls back to ``/root/.athaniel``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ATHANIEL_HOME comparisons
    ...
    _sync_athaniel_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ATHANIEL_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ATHANIEL_HOME
    ...
    exported a throwaway ``ATHANIEL_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``athaniel gateway restart``
    with ``ATHANIEL_HOME=/tmp/athaniel-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ATHANIEL_HOME=..."`` lines and launchd
    ``<key>ATHANIEL_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ATHANIEL_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ATHANIEL_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ATHANIEL_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ATHANIEL_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ATHANIEL_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ATHANIEL_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only ATHANIEL_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ATHANIEL_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../athaniel_test`` path into
    # ``Environment="ATHANIEL_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/athaniel_test"' in new_unit
        or "/athaniel_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # ATHANIEL_HOME into the unit (manual E2E homes like /tmp/athaniel-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Athaniel install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo athaniel gateway uninstall --system")
    print_info("         athaniel gateway install")
    print_info("         athaniel gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("ATHANIEL_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    env_raw = os.getenv("ATHANIEL_CRON_DRAIN_TIMEOUT")
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("ATHANIEL_RESTART_AFTER_TURN_TIMEOUT")
    if env_raw is not None and str(env_raw).strip() != "":
...
def systemd_install(
    ...
    # Offer to remove legacy units (athaniel.service from pre-rename installs)
    # before installing the new athaniel-gateway.service. If both remain, they
    ...
    if has_legacy_athaniel_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_athaniel_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin ATHANIEL_HOME; adopt it before any
    ...
    # ``sudo athaniel gateway install --system --force`` would bake /root/.athaniel
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}athaniel gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}athaniel gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}athaniel gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # ATHANIEL_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `athaniel gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `athaniel gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}athaniel gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}athaniel gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}athaniel gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}athaniel gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.athaniel.gateway-{suffix}" if suffix else "ai.athaniel.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``athaniel gateway start/stop/status`` call).
...
#      background process (the `nohup athaniel gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_athaniel_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_athaniel_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m athaniel_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ATHANIEL_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "athaniel_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``athaniel update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup athaniel gateway run --replace`
    ...
    working.
    """
    from athaniel_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_athaniel_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup athaniel gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `athaniel gateway start`)
    ...
    nofile_block = ""
    try:
        from athaniel_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>ATHANIEL_HOME</key>
        <string>{athaniel_home}</string>
        <key>ATHANIEL_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.athaniel/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See athaniel-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_athaniel_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Athaniel install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  athaniel gateway status             # Check status")
    from athaniel_constants import display_athaniel_home as _dhh
...
def launchd_stop():
    ...
    # `athaniel gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `athaniel update`
...
def launchd_status(deep: bool = False):
    ...
    # Athaniel PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Athaniel PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Athaniel install")
    else:
        print("⚠ Service definition is stale relative to the current Athaniel install")
        print("  Run: athaniel gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: athaniel gateway stop")
            else:
    ...
    if deep:
        log_file = get_athaniel_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ATHANIEL_S6_SUPERVISED_CHILD``.
...
def named_profile_served_by_running_multiplexer() -> bool:
    ...
    try:
        from athaniel_constants import get_default_athaniel_root
        default_root = get_default_athaniel_root()
    except Exception:
        return False
    ...
    try:
        ...
        from athaniel_cli.profiles import normalize_profile_name
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``athaniel gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``athaniel gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``athaniel gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ATHANIEL_HOME roots.
    """
    ...
    if pid is None:
        ...
        # ATHANIEL_HOME (via _pid_record_belongs_to_current_profile). When no
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                stale_home = stale.get("athaniel_home", "<unknown>")
                logger.warning(
                    "PID file belongs to another profile (athaniel_home=%s). "
    ...
    print("  Use 'athaniel gateway restart' to replace it,")
    print("  or 'athaniel gateway stop' first.")
    print("  Or use 'athaniel gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ATHANIEL_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Athaniel gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'athaniel' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/athaniel/docker/entrypoint.sh before the Athaniel command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ATHANIEL_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ATHANIEL_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `athaniel gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ATHANIEL_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("ATHANIEL_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `athaniel gateway restart` which already
    ...
    # `athaniel gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ✶ Athaniel Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # ATHANIEL_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ATHANIEL_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``ATHANIEL_GATEWAY_MAX_STARTS`` /
    # ``ATHANIEL_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("ATHANIEL_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ATHANIEL_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``athaniel gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. athaniel) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Athaniel delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Athaniel connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: athaniel pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Athaniel will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``athaniel setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run athaniel under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.athaniel/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_athaniel_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from athaniel_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from athaniel_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'athaniel pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: athaniel pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'athaniel gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Athaniel will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Athaniel will store the returned account_id/token in ~/.athaniel/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `athaniel gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_athaniel_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `athaniel pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `athaniel pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "AthanielAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``athaniel_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from athaniel_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.athaniel/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.athaniel/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: athaniel gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: athaniel gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: athaniel gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo athaniel gateway install --system"
                        )
                    print_info("  Or run in foreground:  athaniel gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: athaniel gateway run")
                print_info(
                    "  For persistence:   tmux new -s athaniel 'athaniel gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: athaniel gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup athaniel gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'athaniel gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`athaniel_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from athaniel_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ATHANIEL_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``athaniel gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``athaniel gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``athaniel gateway run`` are unaffected.
      2. ``ATHANIEL_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``athaniel gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ATHANIEL_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ATHANIEL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    ...
    # `docker logs` and ${ATHANIEL_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ATHANIEL_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ATHANIEL_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-athaniel service's
    # pattern in docker/s6-rc.d/main-athaniel/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: athaniel gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: athaniel gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s athaniel 'athaniel gateway run'"
                )
                print()
            ...
            print()
            print(
                "  athaniel gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s athaniel 'athaniel gateway run'         # persistent via tmux"
            )
            print(
                "  nohup athaniel gateway run > ~/.athaniel/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from athaniel_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  athaniel profile create <name>     # creates the s6 service slot")
                print("  athaniel -p <name> gateway start   # bring it up via s6")
                print("  athaniel status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: athaniel gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: athaniel gateway run")
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `athaniel gateway uninstall` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: athaniel gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  athaniel profile delete <name>     # tears down the s6 service slot")
                print("  athaniel -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `athaniel -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: athaniel gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        # The supervised probe also PASSES a plain foreground `athaniel gateway run`
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `athaniel gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `athaniel gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: athaniel gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  athaniel gateway install")
                else:
                    print("To install as a service:")
                    print("  athaniel gateway install")
                    print("  sudo athaniel gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  athaniel gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup athaniel gateway run > ~/.athaniel/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s athaniel 'athaniel gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  athaniel gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  athaniel gateway install  # Install as user service")
                    print(
                        "  sudo athaniel gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Athaniel gateway unit files from
        # pre-rename installs (e.g. athaniel.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_athaniel_units(interactive=not yes, dry_run=dry_run)
