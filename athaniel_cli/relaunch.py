"""
Unified self-relaunch for Athaniel CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``athaniel sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``athaniel`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from athaniel_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched athaniel."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_athaniel_bin() -> Optional[str]:
    """Find the athaniel entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("athaniel")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m athaniel_cli.main``.
    ...
    ``athaniel.exe`` on PATH or the ``sys.executable -m athaniel_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("athaniel")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with athaniel.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh athaniel invocation.
    ...
    relaunch contract wants: "run athaniel again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``athaniel.exe`` (a Python console-script shim that wraps
    ``python -m athaniel_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "athaniel exited, then
    new athaniel started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``athaniel`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nAthaniel relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run athaniel.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
