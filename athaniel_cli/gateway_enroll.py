"""``athaniel gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.athaniel/auth.json``) — the same path ``athaniel dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.athaniel/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `athaniel auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from athaniel_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `athaniel gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.athaniel/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_athaniel_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in athaniel_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_athaniel_root()).resolve()
        home = Path(get_athaniel_home()).resolve()
        try:
