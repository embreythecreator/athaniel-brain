...
Three layers that make every long-lived Athaniel process positively
identifiable, so reapers (``athaniel update``, Desktop startup sweeps) never
have to guess lineage from PPID archaeology or cmdline pattern-matching:

1. **Spawn tag** (``ATHANIEL_SPAWN`` env var): every spawner stamps its children
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Athaniel root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Athaniel install on the
    same machine without path comparisons at scan time.
    """
    if project_root is None:
        try:
            from athaniel_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``ATHANIEL_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``ATHANIEL_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_athaniel_root()) / LEDGER_FILENAME
    except Exception:
        from athaniel_cli.config import get_athaniel_home

        return Path(get_athaniel_home()) / LEDGER_FILENAME
...
def register_self(
    ...
    *,
    ...
    if spawner_pid is None:
        ...
        # ATHANIEL_PARENT_PID (+ optional `winms:<ms>` start marker) for its
        # parent-death watchdog. Reuse it as spawner identity so ledger
        # lineage works with every Desktop version, no TS change needed.
        try:
            raw = int(os.environ.get("ATHANIEL_PARENT_PID", ""))
            if raw > 0:
                spawner_pid = raw
        except (TypeError, ValueError):
            pass
        marker = os.environ.get("ATHANIEL_PARENT_START_MARKER", "")
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `athaniel serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import Athaniel code). The entry records the
