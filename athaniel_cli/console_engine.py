"""Safe Athaniel Console command engine.

This module backs ``athaniel console`` and is intentionally narrower than the
full Athaniel CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["AthanielConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'athaniel doctor'")
        and last.startswith("Run 'athaniel setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from athaniel_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="athaniel", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `athaniel "):
        return ""
    return summary
...
# dashboard opens a fresh AthanielConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("athaniel_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["AthanielConsoleEngine", list[str]], str]:
    def handler(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "AthanielConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["AthanielConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `athaniel {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `athaniel {usage}`?",
        )


class AthanielConsoleEngine:
    """Curated line-command executor for Athaniel Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "athaniel":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Athaniel Console does not run shell syntax. Use one supported "
                    "Athaniel command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Athaniel component status.", _status)
        self.register(("version",), "version", "Show Athaniel version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Athaniel logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Athaniel configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Athaniel Console."""

        extracted = {
            "dump": (
                "athaniel_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "athaniel_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "athaniel_cli.subcommands.prompt_size",
            ...
            "insights": (
                "athaniel_cli.subcommands.insights",
            ...
            "security": (
                "athaniel_cli.subcommands.security",
            ...
            "backup": (
                "athaniel_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "athaniel_cli.subcommands.import_cmd",
            ...
            "config": (
                "athaniel_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "athaniel_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "athaniel_cli.subcommands.plugins",
            ...
            "skills": (
                "athaniel_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "athaniel_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "athaniel_cli.subcommands.pairing",
            ...
            "webhook": (
                "athaniel_cli.subcommands.webhook",
            ...
            "hooks": (
                "athaniel_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "athaniel_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "athaniel_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Athaniel configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "athaniel_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("athaniel_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "athaniel_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("athaniel_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("athaniel_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Athaniel Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Athaniel Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`athaniel {first}` is not available in Athaniel Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Athaniel Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Athaniel Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Athaniel Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Athaniel Console.",
            ("skills", "publish"): "`skills publish` is not available in Athaniel Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Athaniel Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Athaniel Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Athaniel Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Athaniel Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Athaniel Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Athaniel Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Athaniel Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Athaniel Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Athaniel Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Athaniel Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Athaniel Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Athaniel Console.")
...
def _version(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "version")
    from athaniel_cli._startup_fast import print_fast_version_info

    return _capture_output(lambda: print_fast_version_info(check_updates=True))


def _status(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Athaniel Console.")
    ...
    from athaniel_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from athaniel_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from athaniel_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _config_migrate(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: AthanielConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from athaniel_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="athaniel_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from athaniel_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from athaniel_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from athaniel console")
...
def _cron_resume(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _athaniel_now, rearm_oneshot, resume_job

    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _athaniel_now().isoformat() if ns.run_now else ns.at)
        else:
...
def _cron_run(_engine: AthanielConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``athaniel console`` REPL."""
    ...
    engine = AthanielConsoleEngine()
    if interactive:
        print("Athaniel Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("athaniel> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
