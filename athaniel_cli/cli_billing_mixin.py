...
``cli.py``'s ``AthanielCLI`` class. ``AthanielCLI`` inherits
...
Import discipline mirrors ``athaniel_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded; honors ATHANIEL_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _show_subscription(self):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                print("  Run `athaniel portal` to log in, then /subscription.")
            return
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``athaniel_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from athaniel_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `athaniel portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from athaniel_cli import nous_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `athaniel portal` to log in, then /topup.")
            return
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Athaniel Brain page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Athaniel Brain page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Athaniel to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `athaniel portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `athaniel portal` to log in again.")
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Athaniel Brain page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `athaniel portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Athaniel Brain page, then run /topup again.")
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Athaniel to charge {card.masked} "
