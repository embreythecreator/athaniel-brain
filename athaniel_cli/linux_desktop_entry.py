"""Install and remove the Linux desktop entry (``athaniel.desktop``).

``athaniel desktop`` builds and launches the Electron app. On Linux, a
...
``athaniel uninstall --gui`` removes the entry again.

Two values must be absolute for the entry to work:

  - ``Exec`` — the launcher runs without shell ``PATH`` customizations, so
    a bare ``athaniel desktop`` fails when athaniel lives in ``~/.local/bin``
...
DESKTOP_ENTRY_NAME = "athaniel.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``athaniel.desktop`` entry lives."""
...
def _can_import_athaniel_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``athaniel_cli.main`` unaided.
    ...
    try:
        result = subprocess.run(
            [key, "-I", "-c", "import athaniel_cli.main"],
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``athaniel_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``athaniel desktop``.

    Prefer the real ``athaniel`` executable (argv[0] or PATH). When Athaniel
    ...
    bytes. ``resolve_athaniel_bin()`` prefers ``sys.argv[0]``, which differs
    ...
    """
    from athaniel_cli.relaunch import resolve_athaniel_bin

    bin_path = _resolve_athaniel_bin_for_desktop_entry(
        resolve_athaniel_bin, checkout_root=project_root
    )
    interpreter = _running_interpreter()
    if not _can_import_athaniel_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import athaniel_cli.main
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `athaniel` script with
            ...
            # sys.executable is the interpreter actually running Athaniel (the
...
def _resolve_athaniel_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`athaniel_cli.relaunch.resolve_athaniel_bin` with one
    desktop-entry-specific rule: an ``argv[0]`` that points inside this
    checkout is a launch-context artifact (the repo ``athaniel`` script the
    ...
    if resolve_fn is None:
        from athaniel_cli.relaunch import resolve_athaniel_bin as resolve_fn
    ...
    def _inside_checkout(candidate: str) -> bool:
        try:
            ...
            return False
        # The repo `athaniel` script and anything else shipped in the tree is
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m athaniel_cli.main` relaunch context surfaces the
    ...
    # interpreter is an external launcher (e.g. /opt/.../bin/athaniel from
    ...
    if rerouted is None and primary:
        # argv[0] was checkout-internal AND PATH had no `athaniel` — common
...
def _wrapper_targets_checkout(wrapper: Path, checkout_root: Path) -> bool:
    ...
    ``<checkout>/venv/bin/python <checkout>/athaniel``; a venv console
    ...
    for root in roots:
        ...
        # <root>/athaniel …): a path-continuation boundary is also a match.
        if root + "/" in text:
            return True
    return False
...
def _known_wrapper_candidates():
    ...
    (``$PREFIX/bin``). The wrapper is always named ``athaniel``.
    """
    candidates = []
    home = Path.home()
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "athaniel")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/athaniel"))
    candidates.append(home / ".local" / "bin" / "athaniel")
    return candidates


def _project_root() -> Path:
    """This file lives at ``<checkout>/athaniel_cli/linux_desktop_entry.py``.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see Athaniel' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Athaniel\n"
        "GenericName=Athaniel Desktop\n"
        "Comment=Launch Athaniel Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Athaniel\n"
    )
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    ...
    The freedesktop icon lookup finds an installed ``apps/athaniel.png``
    by the unqualified name ``athaniel``, so the entry can reference the
    ...
    try:
        ...
        dest = _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "athaniel.png"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Athaniel desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "athaniel"
