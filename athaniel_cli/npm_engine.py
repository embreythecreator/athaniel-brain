...
Scope of the repair is deliberately narrow. Athaniel only upgrades an npm that
lives inside its **own** managed Node tree (``$ATHANIEL_HOME/node``), installing
...
and their other projects; Athaniel never modifies those. When the failing npm is
one of those foreign installs, Athaniel instead provisions its own managed Node
...
from athaniel_constants import (
    bootstrap_athaniel_managed_node,
    get_athaniel_home,
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Athaniel-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$ATHANIEL_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$ATHANIEL_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Athaniel-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Athaniel-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="athaniel-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Athaniel could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Athaniel-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$ATHANIEL_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Athaniel-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_athaniel_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Athaniel-managed install, or
    ...
    if prefix is not None:
        # Athaniel owns this npm — upgrade it in place. Only an npm-range
