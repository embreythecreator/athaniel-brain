"""Dependency-light venv recovery that runs BEFORE athaniel_cli.main's imports.

The ``athaniel`` console entry point is ``athaniel_cli.main:main``.  Importing
``athaniel_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``athaniel_cli.env_loader``, ``yaml`` via ``athaniel_cli.config``, ...).  In
...
a corrupted venv.  ``athaniel_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``athaniel_cli.main`` to become
...
#
# ``athaniel update`` renames the live ``athaniel*.exe`` shims aside
# (``athaniel.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``athaniel`` on PATH, and the command that would repair it IS ``athaniel
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY athaniel
    invocation, and ``athaniel acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Athaniel already vendors
    (``~/.athaniel/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".athaniel" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``athaniel acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``athaniel_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``athaniel_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``athaniel update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `athaniel update`: CLI and Desktop retries preserve
...
# every `athaniel` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``athaniel update`` (or the self-lock preflight, #83569) left the
    ...
    ``athaniel_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``athaniel acp`` included).
    ...
    """
    try:
        from athaniel_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; athaniel will keep working from "
