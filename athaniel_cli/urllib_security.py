...
def _resolved_https_context() -> ssl.SSLContext | None:
    """Return the explicit CA context for Athaniel-owned urllib openers."""
    ca_bundle = next(
        (
            value
...
def _secure_opener_from_installed_policy(original_url: str, *, ssl_context=None):
    ...
    ``ssl_verify``) apply to this request. When it is None, Athaniel-owned
...
def open_credentialed_url(
    ...
    *,
    ...
    if opener_factory is None:
        ...
        for name, value in getattr(opener, "_athaniel_initial_addheaders", ()):
