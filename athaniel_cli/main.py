#!/usr/bin/env python3
"""
Athaniel CLI - Main entry point.

Usage:
    athaniel                     # Interactive chat (default)
    athaniel chat                # Interactive chat
    athaniel gateway             # Run gateway in foreground
    athaniel gateway start       # Start gateway as service
    athaniel gateway stop        # Stop gateway service
    athaniel gateway status      # Show gateway status
    athaniel gateway install     # Install gateway service
    athaniel gateway uninstall   # Uninstall gateway service
    athaniel setup               # Interactive setup wizard
    athaniel logout              # Clear stored authentication
    athaniel status              # Show status of all components
    athaniel cron                # Manage cron jobs
    athaniel cron list           # List cron jobs
    athaniel cron status         # Check if cron scheduler is running
    athaniel doctor              # Check configuration and dependencies
    athaniel honcho setup                    # Configure Honcho AI memory integration
    athaniel honcho status                   # Show Honcho config and connection status
    athaniel honcho sessions                 # List directory → session name mappings
    athaniel honcho map <name>               # Map current directory to a session name
    athaniel honcho peer                     # Show peer names and dialectic settings
    athaniel honcho peer --user NAME         # Set user peer name
    athaniel honcho peer --ai NAME           # Set AI peer name
    athaniel honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    athaniel honcho mode                     # Show current memory mode
    athaniel honcho mode [hybrid|honcho|local]  # Set memory mode
    athaniel honcho tokens                   # Show token budget settings
    athaniel honcho tokens --context N       # Set session.context() token cap
    athaniel honcho tokens --dialectic N     # Set dialectic result char cap
    athaniel honcho identity                 # Show AI peer identity representation
    athaniel honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    athaniel honcho migrate                  # Step-by-step migration guide: OpenClaw native → Athaniel + Honcho
    athaniel --version           Show version and update status
    athaniel update              Update to latest version
    athaniel uninstall           Uninstall Athaniel Brain
    athaniel acp                 Run as an ACP server for editor integration
    athaniel sessions browse     Interactive session picker with search

    athaniel claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: athaniel_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``athaniel_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``athaniel update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``athaniel_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, athaniel crashes on import and the user can't run
# ``athaniel update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import athaniel_bootstrap  # noqa: F401
...
from athaniel_cli._subprocess_compat import suppress_platform_ver_console
from athaniel_cli.cli_output import line_input
...
# Two lines of inline path math so ``python athaniel_cli/main.py`` (script
# mode — sys.path[0] is athaniel_cli/, not the repo root) can import the
# canonical helpers; everything else lives in athaniel_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from athaniel_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``athaniel update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from athaniel_cli.env_loader import ...`` / ``from athaniel_cli.config import
...
# package directory, so if IT can't import, nothing else in athaniel_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from athaniel_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from athaniel_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'athaniel' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``athaniel tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``athaniel.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("athaniel")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"athaniel", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"athaniel")
        # Windows: the .exe name is already ``athaniel.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* athaniel_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ATHANIEL_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".athaniel", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ATHANIEL_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``athaniel … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("ATHANIEL_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``ATHANIEL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ATHANIEL_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`athaniel --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(athaniel_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(athaniel_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``athaniel --version`` before config/logging imports."""
...
from athaniel_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from athaniel_cli.subcommands.cron import build_cron_parser
from athaniel_cli.subcommands.sync import build_sync_parser
from athaniel_cli.subcommands.gateway import build_gateway_parser
from athaniel_cli.subcommands.profile import build_profile_parser
from athaniel_cli.subcommands.model import build_model_parser
from athaniel_cli.subcommands.setup import build_setup_parser

from athaniel_cli.subcommands.whatsapp import build_whatsapp_parser
from athaniel_cli.subcommands.slack import build_slack_parser
from athaniel_cli.subcommands.login import build_login_parser
from athaniel_cli.subcommands.logout import build_logout_parser
from athaniel_cli.subcommands.auth import build_auth_parser
from athaniel_cli.subcommands.status import build_status_parser
from athaniel_cli.subcommands.pause import build_pause_parser
from athaniel_cli.subcommands.webhook import build_webhook_parser
from athaniel_cli.subcommands.hooks import build_hooks_parser
from athaniel_cli.subcommands.doctor import build_doctor_parser
from athaniel_cli.subcommands.verify import build_verify_parser
from athaniel_cli.subcommands.security import build_security_parser
from athaniel_cli.subcommands.approvals import build_approvals_parser
from athaniel_cli.subcommands.dump import build_dump_parser
from athaniel_cli.subcommands.debug import build_debug_parser
from athaniel_cli.subcommands.backup import build_backup_parser
from athaniel_cli.subcommands.import_cmd import build_import_cmd_parser
from athaniel_cli.subcommands.import_agent import build_import_agent_parser
from athaniel_cli.subcommands.config import build_config_parser
from athaniel_cli.subcommands.skin import build_skin_parser
from athaniel_cli.subcommands.console import build_console_parser
from athaniel_cli.subcommands.hello import build_hello_parser
from athaniel_cli.subcommands.update import build_update_parser
from athaniel_cli.subcommands.uninstall import build_uninstall_parser
from athaniel_cli.subcommands.dashboard import build_dashboard_parser
from athaniel_cli.subcommands.gui import build_gui_parser
from athaniel_cli.subcommands.logs import build_logs_parser
from athaniel_cli.subcommands.prompt_size import build_prompt_size_parser
from athaniel_cli.subcommands.memory import build_memory_parser
from athaniel_cli.subcommands.acp import build_acp_parser
from athaniel_cli.subcommands.tools import build_tools_parser
from athaniel_cli.subcommands.insights import build_insights_parser
from athaniel_cli.subcommands.monitoring import build_monitoring_parser
from athaniel_cli.subcommands.skills import build_skills_parser
from athaniel_cli.subcommands.pairing import build_pairing_parser
from athaniel_cli.subcommands.plugins import build_plugins_parser
from athaniel_cli.subcommands.mcp import build_mcp_parser
from athaniel_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (athaniel tools, athaniel setup, athaniel model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'athaniel {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any athaniel module import.
#
# Many modules cache ATHANIEL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ATHANIEL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.athaniel/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ATHANIEL_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `athaniel mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Athaniel' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo athaniel -p <name>` against the invoking user's home.
        ...
        candidate = home / ".athaniel" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`athaniel chat -p coder`), so keep scanning broadly.
    # The exception is command-argv passthrough regions such as `mcp add --args`.
    from athaniel_cli._parser import top_level_value_flag_sets
    ...
    # Mirrors athaniel_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ATHANIEL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.athaniel/profiles/coder or
    # /opt/data/profiles/coder).  If ATHANIEL_HOME points to the athaniel root
    # instead (e.g. systemd hardcodes ATHANIEL_HOME=/root/.athaniel), we must
    # still read active_profile — the user may have switched profiles via
    # `athaniel profile use` and the gateway should honour that choice.
    # See issue #22502.
    athaniel_home_env = os.environ.get("ATHANIEL_HOME", "")
    if profile_name is None and athaniel_home_env:
        if Path(athaniel_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the athaniel root.
    #
    ...
    # above) or pin ``ATHANIEL_HOME`` to the profile directory (step 1.5), and
    # a bare invocation means "the root ATHANIEL_HOME profile". If a supervised
    # default-profile child read active_profile here, switching the active
    # profile (e.g. via the dashboard or ``athaniel profile use``) would
    ...
    # restart routing):
    #   - ATHANIEL_SUPERVISED_CHILD: generalized marker exported by the
    #     generated systemd unit, launchd plist, and Windows Scheduled-Task
    #     launchers (#74872).
    #   - ATHANIEL_S6_SUPERVISED_CHILD: legacy s6 container marker (back-compat;
    ...
    #     predate the ATHANIEL_SUPERVISED_CHILD marker. Consulted ONLY for
    ...
    #     running unrelated athaniel commands), so honoring it globally would
    #     silently disable the sticky active_profile for those.
    #   - ATHANIEL_GATEWAY_EXTERNAL_SUPERVISOR: explicit external-supervisor
    #     opt-in (``athaniel gateway run --external-supervisor``).
    #
    ...
    def _under_gateway_supervisor() -> bool:
        if os.environ.get("ATHANIEL_SUPERVISED_CHILD"):
            return True
        if os.environ.get("ATHANIEL_S6_SUPERVISED_CHILD"):
            return True
        is_gateway_cmd = next(
        ...
        return os.environ.get(
            "ATHANIEL_GATEWAY_EXTERNAL_SUPERVISOR", ""
        ).strip().lower() in {"1", "true", "yes", "on"}

    if profile_name is None and not _under_gateway_supervisor():
        try:
            from athaniel_constants import get_default_athaniel_root

            active_path = get_default_athaniel_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ATHANIEL_HOME
    if profile_name is not None:
        try:
            from athaniel_cli.profiles import resolve_profile_env

            athaniel_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            athaniel_home = _resolve_sudo_user_profile_env(profile_name)
            if not athaniel_home:
            ...
            # A bug in profiles.py must NEVER prevent athaniel from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ATHANIEL_HOME"] = athaniel_home
...
# Windows launcher self-heal — the ``athaniel`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Athaniel root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``athaniel update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``athaniel``
...
# ``python -m athaniel_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no athaniel module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not ATHANIEL_HOME), so profile sessions heal
# the same shared dir.
if sys.platform == "win32":
    try:
        from athaniel_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.athaniel/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from athaniel_cli.config import get_athaniel_home
from athaniel_cli.env_loader import load_athaniel_dotenv
...
load_athaniel_dotenv(
    project_env=PROJECT_ROOT / ".env",
    load_external_secrets=sys.argv[1:2] != ["update"],
)

# Bridge security.redact_secrets from config.yaml → ATHANIEL_REDACT_SECRETS env
# var BEFORE athaniel_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves athaniel_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from athaniel_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_athaniel_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from athaniel_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "ATHANIEL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ATHANIEL_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `athaniel` subcommands
...
try:
    from athaniel_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from athaniel_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if athaniel_constants not importable yet
...
from athaniel_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to athaniel_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (athaniel_cli.main._model_flow_*) keep resolving unchanged.
from athaniel_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_athaniel_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ATHANIEL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ATHANIEL_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`athaniel_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from athaniel_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from athaniel_cli.config import get_env_path, get_athaniel_home, load_config
    from athaniel_cli.auth import get_auth_status

    # Determine whether Athaniel itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from athaniel_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from athaniel_cli.config import split_model_config_default
        ...
        _model_name = ""
    _has_athaniel_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from athaniel_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_athaniel_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Athaniel has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Athaniel to use their tokens.
    if _has_athaniel_config:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from athaniel_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_browse_picker(sessions: list, session_db=None) -> Optional[str]:
    ...
    def _delete_session(session_id: str) -> bool:
        if session_db is None:
            return False
        try:
            sessions_dir = get_athaniel_home() / "sessions"
        except Exception:
            sessions_dir = None
        try:
            return bool(
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``athaniel -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, athaniel_bin
        cli_args: the original CLI arguments (everything after 'athaniel')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    athaniel_bin = container_info["athaniel_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo athaniel {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo athaniel {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'athaniel sessions list' to see available sessions, or "
            ...
            try:
                from athaniel_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _print_tui_exit_summary(
    ...
    print(f"  athaniel --tui --resume {target}")
    if title:
        print(f'  athaniel --tui -c "{title}"')
    print()
    ...
    *starts* is the set of workspace keys the launch install explicitly scopes
    ...
    while stack:
        ...
        if not isinstance(entry, dict):
            continue
        # Workspace symlink (e.g. node_modules/@athaniel/ink → ui-tui/packages/…):
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @athaniel/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@athaniel" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/athaniel-ink/package.json",
    "packages/athaniel-ink/index.js",
    "packages/athaniel-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ATHANIEL_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ATHANIEL_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ATHANIEL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ATHANIEL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    athaniel_home = str(get_athaniel_home())
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ATHANIEL_HOME": athaniel_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(athaniel_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(athaniel_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if athaniel_cli_dir is None:
        athaniel_cli_dir = Path(__file__).parent
    bundled = athaniel_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``athaniel update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ATHANIEL_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Athaniel checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `athaniel update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Athaniel checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `athaniel --tui`\n"
        "If the checkout is still inconsistent, run `athaniel update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ATHANIEL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ATHANIEL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $ATHANIEL_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Athaniel installed,
        # and would pick a system Node over the managed one when both exist.
        from athaniel_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from athaniel_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ATHANIEL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ATHANIEL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ATHANIEL_TUI_DIR (e.g. `unset ATHANIEL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # athaniel_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from athaniel_constants import with_athaniel_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_athaniel_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Athaniel-managed
            ...
            from athaniel_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @athaniel/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @athaniel/ink resolves through
        # packages/athaniel-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "athaniel-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from athaniel_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("ATHANIEL_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ATHANIEL_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ATHANIEL_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ATHANIEL_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ATHANIEL_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ATHANIEL_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["ATHANIEL_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a athaniel process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from athaniel_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="athaniel-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ATHANIEL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ATHANIEL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["ATHANIEL_MODEL"] = model
        env["ATHANIEL_INFERENCE_MODEL"] = model
    if provider:
        env["ATHANIEL_TUI_PROVIDER"] = provider
        env["ATHANIEL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ATHANIEL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ATHANIEL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ATHANIEL_TUI_SKILLS"] = value
    if query:
        env["ATHANIEL_TUI_QUERY"] = query
    if image:
        env["ATHANIEL_TUI_IMAGE"] = image
    if checkpoints:
        env["ATHANIEL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ATHANIEL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ATHANIEL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ATHANIEL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ATHANIEL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ATHANIEL_ACCEPT_HOOKS"] = "1"
    ...
    # ATHANIEL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `athaniel --tui` try to
    ...
    # still set ATHANIEL_TUI_RESUME themselves.
    env.pop("ATHANIEL_TUI_RESUME", None)
    if resume_session_id:
        env["ATHANIEL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `athaniel update` so
    ...
    if code == 42:
        from athaniel_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ATHANIEL_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``athaniel kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``athaniel kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ATHANIEL_KANBAN_BOARD"):
        return
    try:
        from athaniel_cli.kanban_db import get_current_board

        os.environ["ATHANIEL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.athaniel/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Athaniel — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Athaniel still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``athaniel … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("ATHANIEL_TUI") == "1":
        return True
    try:
        from athaniel_cli.config import load_config
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'athaniel sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: athaniel --resume {_imported_id})")
    ...
    try:
        from athaniel_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from athaniel_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'athaniel doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Athaniel isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  athaniel setup")
        print()

        from athaniel_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'athaniel setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from athaniel_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.athaniel/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from athaniel_cli.config import get_athaniel_home
            skills_dir = Path(get_athaniel_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import athaniel_cli.banner as _banner_mod
    ...
    if getattr(args, "yolo", False):
        os.environ["ATHANIEL_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.athaniel/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ATHANIEL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ATHANIEL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ATHANIEL_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older athaniel_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: AthanielCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `athaniel` / `athaniel chat`.
        from athaniel_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from athaniel_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from athaniel_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from athaniel_cli.config import get_env_value, save_env_value
    from athaniel_constants import find_node_executable, with_athaniel_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Athaniel?")
        print()
    ...
    # has no creds.json.  Every subsequent `athaniel gateway` then paid a 30s
    ...
    session_dir = get_athaniel_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: athaniel gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `athaniel gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  athaniel gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '✶ Athaniel Brain'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  athaniel gateway")
        ...
        print()
        print("  Or install as a service: athaniel gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'athaniel whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``athaniel whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``athaniel_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from athaniel_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from athaniel_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from athaniel_cli.config import stamp_install_method
    from athaniel_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("✶ Athaniel post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from athaniel_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from athaniel_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``athaniel model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from athaniel_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from athaniel_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from athaniel_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("ATHANIEL_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from athaniel_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'athaniel model' for "
                "available providers, or run 'athaniel doctor' to diagnose config "
                "issues."
            )
    ...
    from athaniel_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        _PROVIDER_ALIASES,
        group_providers,
    ...
    # PROVIDER_GROUPS in athaniel_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``athaniel model``
    ...
    # matching against athaniel_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    # a leftover OPENAI_BASE_URL in ~/.athaniel/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.athaniel/.env if the active provider is not 'custom'.
    ...
    """
    from athaniel_cli.config import get_env_value, save_env_value, load_config
...
#
# Athaniel uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `athaniel model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `athaniel model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`athaniel_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from athaniel_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from athaniel_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Athaniel only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``athaniel model`` flow, then route aux tasks
    to them here.
    """
    from athaniel_cli.config import load_config
    from athaniel_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from athaniel_cli.auth import _prompt_model_selection
    from athaniel_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from athaniel_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from athaniel_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from athaniel_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Athaniel URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from athaniel_cli.curses_ui import curses_radiolist
...
# downstream call sites read `athaniel_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `athaniel` invocation — including
# fast paths like `athaniel --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on athaniel_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `athaniel` invocation, including fast paths like `athaniel --version`
...
_LAZY_COMMAND_EXPORTS = {
    ...
    "athaniel_cli.dashboard_procs": (
        "_detect_concurrent_athaniel_instances",
...
def _self():
    ...
    keeps monkeypatches on athaniel_cli.main.<name> working, exactly like a
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from athaniel_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``athaniel setup`` / ``athaniel model``.
    ...
    recover from a malformed paste without editing ``~/.athaniel/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from athaniel_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from athaniel_cli.config import save_env_value
    from athaniel_cli.secret_prompt import masked_secret_prompt
    ...
    from athaniel_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `athaniel setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from athaniel_constants import display_athaniel_home as _dhh_fn

            print(
                f"    Athaniel will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Athaniel CLI with a provider."""
    from athaniel_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from athaniel_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from athaniel_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from athaniel_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from athaniel_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.athaniel/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `athaniel sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set ATHANIEL_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "ATHANIEL_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="athaniel sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="athaniel sync now")
...
def cmd_webhook(args):
    ...
    from athaniel_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``athaniel slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: athaniel slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `athaniel slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from athaniel_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from athaniel_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from athaniel_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from athaniel_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from athaniel_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from athaniel_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `athaniel security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from athaniel_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `athaniel approvals <subcmd>`."""
    from athaniel_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from athaniel_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from athaniel_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from athaniel_cli.config import config_command
...
def cmd_skin(args):
    ...
    from athaniel_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Athaniel home directory to a zip file."""
    if getattr(args, "quick", False):
        from athaniel_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from athaniel_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Athaniel backup from a zip file."""
    from athaniel_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `athaniel --version` pre-import fast path (the `version` subcommand
    # was consolidated into `--version`).
    _startup_fast.print_fast_version_info(check_updates=check_updates)


def cmd_hello(args):
    """Print a greeting from Athaniel Brain."""
    print("Hello from Athaniel Brain.")
...
def cmd_uninstall(args):
    """Uninstall Athaniel Brain (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from athaniel_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from athaniel_cli.uninstall import run_gui_uninstall
    ...
    from athaniel_cli.uninstall import run_uninstall
...
# Update pipeline lives in athaniel_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on athaniel_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in ATHANIEL_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``athaniel update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``athaniel update``
    ...
    ``athaniel`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``athaniel update``
    ...
    ``athaniel_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "athaniel_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ATHANIEL_HOME."""
    from athaniel_constants import get_athaniel_home
    return get_athaniel_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``athaniel`` launcher present but ``athaniel_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the athaniel venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — athaniel venv python3, no nix-shell overhead
    ...
    # a athaniel venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``athaniel update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Athaniel-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_athaniel_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "athaniel_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``athaniel web``).
    ...
    # build path usable on a stock `py -m athaniel_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_athaniel_node_path())
    ...
    # as `athaniel update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (athaniel web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``athaniel update`` can unconditionally call ``athaniel desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``athaniel desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ATHANIEL_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ATHANIEL_HOME."""
    from athaniel_constants import get_athaniel_home
    return get_athaniel_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Athaniel.app/Contents/MacOS/Athaniel → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``athaniel desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``athaniel update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Athaniel.app/Contents/MacOS/Athaniel"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Athaniel.exe",
            release_dir / "win-ia32-unpacked" / "Athaniel.exe",
            release_dir / "win-arm64-unpacked" / "Athaniel.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "athaniel",
            release_dir / "linux-unpacked" / "Athaniel",
            release_dir / "linux-arm64-unpacked" / "athaniel",
            release_dir / "linux-arm64-unpacked" / "Athaniel",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Athaniel.exe
...
#
# The desktop self-update chain (Desktop → athaniel-setup --update →
# `athaniel update` → `athaniel desktop --build-only` → relaunch) rebuilds
# Athaniel.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Athaniel.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Athaniel.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 athaniel-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Athaniel.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Athaniel.exe from backup.")
        print("    Your existing version was kept and still works. Run `athaniel desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `athaniel desktop --force-build` to rebuild, or re-run the Athaniel")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Athaniel'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_athaniel_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Athaniel.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Athaniel.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Athaniel.exe: Access is
    ...
    an unrelated "Athaniel" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Athaniel.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Athaniel is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Athaniel.app/Contents/MacOS/Athaniel  ->  app bundle = .../Athaniel.app
    app = exe.parents[2]
...
def _desktop_macos_setup_tcc_identity(identity: str = "Athaniel Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Athaniel to use it.

    One-shot setup for ``athaniel desktop --setup-tcc-identity``. Creates a
    ...
    if not already_imported:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="athaniel-tcc-"))
        try:
            ...
            def _export_p12(extra_args: list) -> None:
                subprocess.run(
                    [
                        ...
                        "-out", str(p12), "-passout", "pass:athaniellocal",
                    ],
                    capture_output=True, check=True,
                )
    ...
    # Point Athaniel at the identity (config.yaml, not .env — it's not a secret).
    try:
        from athaniel_cli.config import set_config_value
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.embreythecreator.athaniel"
    )
    return True
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``athaniel desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Athaniel Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Athaniel Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `athaniel desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the ATHANIEL_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Athaniel Desktop (Linux only, best-effort).
    ...
    shell. ``athaniel uninstall --gui`` removes it.
    """
    try:
        from athaniel_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from athaniel_logging import setup_logging as _setup_logging_gui
    ...
    # with_athaniel_node_path() copies os.environ when called with no arg.
    env = with_athaniel_node_path()
    if getattr(args, "fake_boot", False):
        env["ATHANIEL_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ATHANIEL_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "athaniel_root", None):
        env["ATHANIEL_DESKTOP_ATHANIEL_ROOT"] = str(Path(args.athaniel_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ATHANIEL_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["ATHANIEL_DESKTOP_CWD"] = os.getcwd()
    ...
    # `ATHANIEL_DESKTOP_DISABLE_GPU=... athaniel desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... athaniel desktop` keep working.
    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options()
    )
    if config_disable_gpu != "auto" and "ATHANIEL_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ATHANIEL_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `ATHANIEL_DESKTOP_PASSWORD_STORE=... athaniel desktop` keeps working.
    if sys.platform == "linux" and "ATHANIEL_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["ATHANIEL_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Athaniel Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  athaniel gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Athaniel-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → athaniel-setup →
            # athaniel update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `athaniel update` path).
            nixos_env = with_athaniel_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Athaniel.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Athaniel.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Athaniel.exe, close any")
                    print("  running Athaniel desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> athaniel desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Athaniel is
                ...
                # success on a Athaniel.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Athaniel shows up
    ...
    if source_mode:
        print("→ Launching Athaniel Desktop from source build...")
    ...
    print(f"→ Launching packaged Athaniel Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in athaniel_cli/dashboard_procs.py
...
# athaniel_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "athaniel dashboard",
            "athaniel_cli.main dashboard",
            "athaniel_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "athaniel serve",
            "athaniel_cli.main serve",
            "athaniel_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "athaniel-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Athaniel installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``athaniel-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/athaniel-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but athaniel launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``athaniel update``.
    ...
    log_path = get_athaniel_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `athaniel update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $ATHANIEL_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``athaniel update``.
    ...
    protocol stream (``athaniel acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `athaniel update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``athaniel.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_athaniel_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from athaniel.exe; applying package-only first aid, "
    ...
    try:
        from athaniel_cli import _install_repair as _ir
        ...
        from athaniel_cli.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Athaniel is still running from the launcher that needs "
                "replacing. Close other Athaniel windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\athaniel.exe`` is a launcher that runs the interpreter with
    ...
    ``athaniel ...`` command holds its own shim, and an editable install run
    ...
    ``athaniel.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _athaniel_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_athaniel_launcher_locked() -> bool:
    """True when a venv ``athaniel*.exe`` shim is this process or an ancestor.
...
_UPDATE_REEXEC_ENV = "ATHANIEL_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``athaniel update``, so the whole remaining flow — the
    ...
    from athaniel_constants import venv_python_path

    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "athaniel_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from athaniel_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``athaniel update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from athaniel_constants import project_venv_dir, venv_bin_dir
...
def _athaniel_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_athaniel_exe(
    ...
    when ``athaniel update`` runs, ``athaniel.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``athaniel.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next athaniel invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Athaniel Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Athaniel Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``athaniel.exe`` re-runs itself under the venv Python before
    ...
    for shim in _athaniel_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Athaniel Desktop, exit other `athaniel` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `athaniel update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Athaniel versions queued for our shims.

    Athaniel used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _athaniel_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_athaniel_exe`` if uv didn't write replacements.

    This is the safety-critical direction. A failed *quarantine* only aborts an
    update; a failed *restore* leaves the install with no ``athaniel`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``athaniel*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``athaniel.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``athaniel.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``athaniel`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_athaniel_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `athaniel` missing from PATH after a green
...
def _quarantine_stamp_ms(stale: Path) -> int | None:
    ...
    :func:`_quarantine_running_athaniel_exe`. We neither rescue nor delete those:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``athaniel.exe.old.*`` from updates.

    Called early on every athaniel invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``athaniel`` gone from PATH:

    1. **Orphan rescue.** If ``athaniel.exe`` is missing while
       ``athaniel.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``athaniel update`` does exactly this). Leave anything inside the grace
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``athaniel.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``athaniel.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `athaniel update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from athaniel_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``athaniel.exe`` / ``athaniel-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_athaniel_exe`` for the rationale.
    ...
    if (
        ...
        # Quarantining the wrong dir means the running athaniel.exe stays locked
    ...
    # hours later inside ``athaniel update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``athaniel.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``athaniel.exe`` shim is locked during ``athaniel update``, or when uv/distlib
    skips a launcher write. The symptom is ``athaniel-brain.exe`` and
    ``athaniel-acp.exe`` present but ``athaniel.exe`` missing, so ``athaniel`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `athaniel update --force` after "
            "closing other athaniel processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m athaniel_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``athaniel update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``athaniel.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``athaniel`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `athaniel update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `athaniel update --force` after closing other athaniel processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``athaniel update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.athaniel/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``athaniel update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``athaniel update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.athaniel/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``athaniel update --gateway``) the update is already
    ...
    try:
        ...
        # athaniel_cli.config.get_athaniel_home to simulate setup failure.
        from athaniel_cli.config import get_athaniel_home as _get_athaniel_home

        logs_dir = _get_athaniel_home() / "logs"
...
def cmd_update(args):
    """Update Athaniel Brain to the latest version.
    ...
    if is_unsupported_install_method(install_method):
        print(f"⚠ {format_unsupported_install_warning(install_method)}")

    if is_managed():
        managed_error("update Athaniel Brain")
        return
    ...
    if getattr(args, "plan", False):
        ...
        # Athaniel runtime across profiles, its supervisor, and its running
        # code version — without mutating anything. Safe on a live fleet.
        from athaniel_cli.update_inventory import (
    ...
    # exit 1 errors).
    from athaniel_cli.update_contract import (
    ...
    # pip/PyPI installs have no ``.git`` checkout to pull from — check/apply
    # against PyPI instead of the git-based ``_cmd_update_check``/
    # ``_cmd_update_impl`` pair (which the "unsupported install method"
    # warning above still lets through: warn, don't block).
    if install_method == "pip":
        if getattr(args, "check", False):
            branch = _resolve_update_branch(args)
            _cmd_update_check_pip(
                branch=branch,
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `athaniel update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
    ...
    from athaniel_cli.update_lock import (
    ...
    try:
        ...
        try:
            from athaniel_cli.update_receipt import finalize_pending_update_receipt
...
def _cmd_update_pip(args):
    """Update Athaniel via pip (for PyPI installs)."""
    from athaniel_cli import __version__
    from athaniel_cli.config import is_uv_tool_install

    print(f"→ Current version: {__version__}")
    print("→ Checking PyPI for updates...")

    from athaniel_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "athaniel-brain"]
        ...
        cmd = [pipx, "upgrade", "athaniel-brain"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "athaniel-brain"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "athaniel-brain"]
    ...
    print("✓ Update complete! Restart athaniel to use the new version.")


def _cmd_update_check_pip(branch: str, branch_explicit: bool) -> None:
    """``athaniel update --check`` for pip/PyPI installs.

    pip installs have no ``.git`` checkout, so the git-based
    ``_cmd_update_check`` can't answer "any new commits?" — compare the
    installed version against PyPI's latest release instead.
    """
    from athaniel_cli.config import recommended_update_command
    from athaniel_cli.banner import check_via_pypi
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``athaniel -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from athaniel_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    from athaniel_constants import display_athaniel_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `athaniel profile` — show current profile status
        from athaniel_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_athaniel_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → athaniel -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.athaniel)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  athaniel profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     athaniel -p {name} chat")
                else:
        ...
        from athaniel_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from athaniel_constants import get_athaniel_home as _hh
        ...
        from athaniel_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `athaniel profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → athaniel -p {name}  ({wrapper})")
        print()
        ...
        from athaniel_cli.profiles import profile_exists, validate_alias_name
        ...
        from athaniel_cli.profiles import normalize_profile_name, rename_profile
        ...
        from athaniel_cli.profiles import export_profile
        ...
        from athaniel_cli.profiles import import_profile
        ...
        import tempfile
        from athaniel_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="athaniel_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  athaniel -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      athaniel -p {plan.manifest.name} chat")
        ...
        from athaniel_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `athaniel profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"athaniel -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from athaniel_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("athaniel_requires"):
            print(f"Requires:     Athaniel {data['athaniel_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from athaniel_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.athaniel_requires:
        print(f"  Requires: Athaniel {mf.athaniel_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No athaniel dashboard or serve processes running.")
        return 0

    print(f"{len(live)} athaniel dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``athaniel dashboard register`` for
    OAuth.
    ...
    try:
        from athaniel_cli.web_server import should_require_dashboard_auth
    ...
    try:
        from athaniel_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `athaniel dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    athaniel dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "ATHANIEL_DASHBOARD_OAUTH_CLIENT_ID into ~/.athaniel/.env for you.\n"
            "  Docs: https://athaniel-brain.embreythecreator.com/docs/"
    ...
    try:
        from athaniel_cli.config import load_config, save_config
        from athaniel_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
    ...
    try:
        from athaniel_cli.plugins import discover_plugins
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from athaniel_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.athaniel/desktop-ssh: a
    # literal "~/.athaniel/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of ATHANIEL_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_athaniel_home(): a non-default sticky profile (or any ATHANIEL_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_athaniel_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".athaniel" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``ATHANIEL_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``athaniel dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No athaniel dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `athaniel update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with athaniel serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with ATHANIEL_DESKTOP=1 plus
    # ATHANIEL_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # ATHANIEL_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `athaniel dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited ATHANIEL_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # ATHANIEL_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (ATHANIEL_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets ATHANIEL_SERVE_HEADLESS below.
    if os.environ.get("ATHANIEL_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("ATHANIEL_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("ATHANIEL_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("ATHANIEL_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → ATHANIEL_HOME points into
    ...
    try:
        from athaniel_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("ATHANIEL_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "athaniel_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: ATHANIEL_HOME is explicitly pinned to the
        ...
        # ATHANIEL_HOME.  We must resolve the root explicitly instead of just
        # dropping ATHANIEL_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ATHANIEL_HOME=/opt/data`), so an unset
        # ATHANIEL_HOME falls back to $HOME/.athaniel = /opt/data/.athaniel — an
        ...
        # update-button guard also misfires).  get_default_athaniel_root()
        # returns the root for both layouts: ~/.athaniel for a standard install
        ...
        try:
            from athaniel_constants import get_default_athaniel_root
            env["ATHANIEL_HOME"] = str(get_default_athaniel_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ATHANIEL_HOME) rather than block the reroute.
            env.pop("ATHANIEL_HOME", None)
    ...
    from athaniel_cli.resource_limits import apply_nofile_soft_limit
    ...
    # the same logs directory as every other Athaniel surface.
    try:
        from athaniel_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["ATHANIEL_SERVE_HEADLESS"] = "1"
    elif "ATHANIEL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ATHANIEL_WEB_DIST"])
            if "ATHANIEL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "athaniel_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ATHANIEL_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "ATHANIEL_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ATHANIEL_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ATHANIEL_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ATHANIEL_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ATHANIEL_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ATHANIEL_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ATHANIEL_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ATHANIEL_WEB_DIST: {_dist_root}")
    ...
    try:
        from athaniel_cli.mcp_startup import start_background_mcp_discovery
    ...
    from athaniel_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from athaniel_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from athaniel_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from athaniel_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from athaniel_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Athaniel log files."""
    from athaniel_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Athaniel command console."""
    from athaniel_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from athaniel_cli.models import CANONICAL_PROVIDERS as _cp
...
def _first_positional_argv() -> str | None:
    ...
    ``athaniel -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``athaniel`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``athaniel <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``athaniel photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ATHANIEL_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ATHANIEL_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from athaniel_cli.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["ATHANIEL_SAFE_MODE"] = "1"
    os.environ["ATHANIEL_IGNORE_USER_CONFIG"] = "1"
    os.environ["ATHANIEL_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    ``athaniel`` / ``athaniel -w -s foo --yolo`` / ``athaniel chat`` don't need the
    ...
    """
    if os.environ.get("ATHANIEL_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from athaniel_cli.config import get_container_exec_info
    ...
    from athaniel_cli._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("ATHANIEL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ATHANIEL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ATHANIEL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ATHANIEL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `athaniel --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from athaniel_constants import get_athaniel_home, display_athaniel_home

        mem_dir = get_athaniel_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_athaniel_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_athaniel_home()}/memories/\n")
    else:
        from athaniel_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Athaniel Brain as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from athaniel_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from athaniel_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from athaniel_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: athaniel-brain[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from athaniel_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from athaniel_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``athaniel skills trust [path]`` / ``athaniel skills untrust [path]``.
...
def cmd_pairing(args):
    from athaniel_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from athaniel_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from athaniel_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from athaniel_cli.claw import claw_command

    claw_command(args)


def cmd_absorb(args) -> int:
    """`athaniel absorb` — fold a new upstream release into the fork.
    ...
    try:
        if args.status:
            ...
            print(f"  next: `athaniel absorb --continue` to resolve, "
                  f"`athaniel absorb --verify` to re-check, "
                  f"`athaniel absorb --commit` to finalize")
            return 1 if marked else 0
        if args.cont:
        ...
        if args.commit:
            ...
            return 0
        if not args.tag:
            print("  usage: athaniel absorb <tag> | --check | --gate | --status | "
...
def _advertise_agent_env() -> None:
    ...
    (``athaniel-brain`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``ATHANIEL_AGENT`` is the Athaniel-specific marker. setdefault: never
    clobber an outer harness (e.g. Athaniel running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "athaniel-brain")
    os.environ.setdefault("ATHANIEL_AGENT", "true")



def main():
    """Main entry point for athaniel CLI."""
    # Cosmetic: make the process show up as 'athaniel' instead of 'python3.11'
    ...
    try:
        from athaniel_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``athaniel.exe.old.*`` quarantine files left by previous
    # ``athaniel update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_athaniel_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (athaniel update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``athaniel update``
    ...
    # ``athaniel skills install update``) merely defers recovery one launch;
    # under-matching (missing ``athaniel -p work update``) would race a recovery
    ...
    # never restarted the fleet. Do NOT restart here — that is ``athaniel
    ...
    # =========================================================================
    # model command  (parser built in athaniel_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from athaniel_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from athaniel_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://athaniel-brain.embreythecreator.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `athaniel model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory athaniel -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.athaniel/archive/worktree-prune/ before removal. See: "
            "https://athaniel-brain.nousresearch.com/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from athaniel_cli.worktree_cmd import cmd_worktree
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Athaniel can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    def _dispatch_browser(_args):
        from athaniel_cli.browser_connect import (
            UNSUPPORTED_CHANNEL,
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.athaniel/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://athaniel-brain.embreythecreator.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from athaniel_cli import secrets_cli as _secrets_cli
    from athaniel_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `athaniel proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://athaniel-brain.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from athaniel_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``athaniel proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from athaniel_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in athaniel_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in athaniel_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in athaniel_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `athaniel whatsapp` which sets up "
    ...
    absorb_parser = subparsers.add_parser(
        ...
        description=(
            "Fold a new upstream release into Athaniel's own core via the "
            ...
            "`upstream` remote (pip/docker installs get core updates via `athaniel "
            "update`)."
        ),
    )
    ...
    # =========================================================================
    # slack command  (parser built in athaniel_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from athaniel_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in athaniel_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in athaniel_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in athaniel_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in athaniel_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in athaniel_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in athaniel_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in athaniel_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Athaniel gateways)
    # =========================================================================
    from athaniel_cli.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    from athaniel_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from athaniel_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from athaniel_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in athaniel_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in athaniel_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in athaniel_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in athaniel_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in athaniel_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in athaniel_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in athaniel_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in athaniel_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.athaniel/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo athaniel uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from athaniel_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in athaniel_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: athaniel_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from athaniel_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in athaniel_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in athaniel_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in athaniel_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in athaniel_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in athaniel_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from athaniel_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in athaniel_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``athaniel --help``, ``athaniel logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from athaniel_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from athaniel_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Athaniel shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from athaniel_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from athaniel_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in athaniel_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in athaniel_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `athaniel computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `athaniel tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `athaniel computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Athaniel app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from athaniel_cli.tools_config import (
        ...
        if action == "status":
            import os as _os
            import subprocess
            from athaniel_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("ATHANIEL_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from athaniel_cli.tools_config import _cua_driver_env
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by ATHANIEL_CUA_DRIVER_CMD, or unset "
                            "the override and run: athaniel computer-use install --upgrade"
                        )
                    else:
                        print("    Run: athaniel computer-use install")
                    return 1
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: athaniel computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: athaniel computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: athaniel computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: athaniel computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: athaniel computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in athaniel_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <athaniel home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Athaniel",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Athaniel session store "
            "so it can be resumed with 'athaniel --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in athaniel_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on athaniel_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in athaniel_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in athaniel_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # hello command  (parser built in athaniel_cli/subcommands/hello.py)
    # =========================================================================
    build_hello_parser(subparsers, cmd_hello=cmd_hello)

    # NOTE: the `athaniel version` subcommand was removed — `athaniel --version`
    # / `-V` now carries the full output including update status.

    # =========================================================================
    # update command  (parser built in athaniel_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in athaniel_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in athaniel_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in athaniel_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in athaniel_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Athaniel-Setup.exe success screen tells users to
    # run `athaniel desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in athaniel_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in athaniel_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in athaniel_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``athaniel -c Pokemon Agent Dev`` → ``athaniel -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'athaniel -c model' where 'model' is consumed as the
    ...
    # --yolo: set ATHANIEL_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (athaniel hooks list, cron
    ...
    # ``athaniel egress start`` refusing when credential_source=bitwarden
