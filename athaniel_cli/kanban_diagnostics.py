...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``athaniel -p <profile> auth``). No HTTP side effect.
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``athaniel kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`athaniel kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"athaniel config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"athaniel config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: athaniel kanban specify {task_id}",
            payload={"command": f"athaniel kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: athaniel -p {assignee} doctor",
            payload={"command": f"athaniel -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: athaniel -p {assignee} auth",
            payload={"command": f"athaniel -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: athaniel kanban log {task_id}",
                payload={"command": f"athaniel kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_review_dependency_deadlock(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            ...
            payload={"command": f"athaniel kanban complete {task_id}"},
            suggested=True,
        ))
    if task_id and child_ids:
        actions.append(DiagnosticAction(
            ...
            payload={"command": f"athaniel kanban unlink {task_id} {child_ids[0]}"},
        ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: athaniel kanban events {task_id}",
            payload={"command": f"athaniel kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong ATHANIEL_HOME).
    ...
    works for Athaniel profiles, registered lanes, external workers, and
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "athaniel kanban diagnostics"},
        ),
    ]
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Athaniel runtime config.
