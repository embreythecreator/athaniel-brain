...
on Japanese locales, etc.).  Athaniel's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``athaniel_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Athaniel is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ATHANIEL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ATHANIEL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Athaniel-managed Git install directories so
    ...
    # already-running shells don't see the change — which means athaniel
    ...
    # install without Athaniel-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Athaniel's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Athaniel's
    ``athaniel config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Athaniel (User env var in Windows Settings, or
    export in a PowerShell profile) and Athaniel picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Athaniel-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\athaniel\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``athaniel`` from right after install) retain their old PATH.

    Any subprocess Athaniel spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Athaniel-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "athaniel", "git", "cmd"),
        os.path.join(local_appdata, "athaniel", "git", "bin"),
        os.path.join(local_appdata, "athaniel", "git", "usr", "bin"),
        # Athaniel venv Scripts directory — host of the athaniel.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes athaniel, but harmless to include.
        os.path.join(local_appdata, "athaniel", "athaniel-brain", "venv", "Scripts"),
