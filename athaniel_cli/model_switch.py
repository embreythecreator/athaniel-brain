...
- ``athaniel_cli.providers``        -- canonical provider identity + overlays
- ``athaniel_cli.model_normalize``  -- per-provider name formatting
...
from athaniel_cli.model_normalize import (
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Athaniel wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Athaniel.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Athaniel versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``athaniel model`` wizard — not a
    ...
    while ``athaniel model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Athaniel itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Athaniel itself discovered (``models_discovered: true``
...
_ATHANIEL_MODEL_WARNING = (
    "Athaniel Athaniel 3 & 4 models are NOT agentic and are not designed "
    "for use with Athaniel Brain. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Athaniel Athaniel 3 / Athaniel 4 chat families.
# The previous substring check (`"athaniel" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``athaniel-brain:qwen3-14b-ctx16k`` that just
# happen to carry "athaniel" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   embreythecreator/athaniel-3-Llama-3.1-70B, athaniel-4-405b, openrouter/athaniel3:70b
# Negative examples it must NOT match:
#   athaniel-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ATHANIEL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])athaniel[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_athaniel_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Athaniel 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ATHANIEL_NON_AGENTIC_RE.search(model_name))


def _check_athaniel_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Athaniel 3/4 chat model."""
    if is_nous_athaniel_non_agentic(model_name):
        return _ATHANIEL_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``athaniel config set model.aliases.xxx``)
...
# switch (ATHANIEL_HOME moves, so the path moves) and a config/key rotation
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    """
    try:
        from athaniel_constants import get_config_path
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from athaniel_cli.model_switch import
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from athaniel_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``athaniel_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'athaniel model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'athaniel doctor' — config issues detected:"
        ...
        from athaniel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from athaniel_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from athaniel_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from athaniel_cli.config import load_config
            from athaniel_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from athaniel_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from athaniel_cli.models import should_use_ollama_native_catalog
    ...
    if target_provider in {"nous", "nous-portal", "embreythecreator"}:
        from athaniel_cli.providers import nous_api_mode
    ...
    # athaniel_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from athaniel_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from athaniel_cli.models import normalize_opencode_base_url
    ...
    athaniel_warn = _check_athaniel_model_warning(new_model)
    if athaniel_warn:
        warnings.append(athaniel_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from athaniel_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Athaniel provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from athaniel_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from athaniel_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from athaniel_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from athaniel_cli.providers import ATHANIEL_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from athaniel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Athaniel-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for athaniel_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(athaniel_id)
        if (
            _alias_target
            and _alias_target != athaniel_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        _canonical = athaniel_id
        try:
            from providers import get_provider_profile as _gpp
            _prof = _gpp(athaniel_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != athaniel_id:
            continue
        if athaniel_id.lower() in seen:
            continue
        if athaniel_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(athaniel_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from athaniel_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(athaniel_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(athaniel_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        athaniel_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(athaniel_id)
            seen.add(athaniel_id.lower())

    # --- Section 2: Athaniel-only providers (ATHANIEL_OVERLAYS) ---
    _mdev_to_athaniel = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in ATHANIEL_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        athaniel_slug = _mdev_to_athaniel.get(pid, pid)
        if athaniel_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or athaniel_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or athaniel_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(athaniel_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(athaniel_slug)
            seen.add(pid.lower())
            seen.add(athaniel_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from athaniel_cli/models.py (OPENROUTER_MODELS,
    ...
    from athaniel_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against athaniel_id / mdev_id (section 1), pid / athaniel_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from athaniel_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by athaniel provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://athaniel-brain.embreythecreator.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Athaniel release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from athaniel_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from athaniel_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Athaniel-mapped providers ---
    from athaniel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from athaniel_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for athaniel_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip athaniel_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose athaniel_id matches the canonical profile name so
        ...
        # same athaniel_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if athaniel_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (ATHANIEL_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Athaniel may not route yet.
        ...
        if not has_creds:
            try:
                from athaniel_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(athaniel_id)
                )
        ...
        # /model picker sees the SAME list `athaniel model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(athaniel_id)
        if not model_ids:
            model_ids = curated.get(athaniel_id, [])
            if athaniel_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(athaniel_id, model_ids)
        ...
        results.append({
            ...
            "is_current": (
                ...
                or athaniel_id == current_provider
    ...
    # --- 2. Check Athaniel-only providers (nous, openai-codex, copilot, opencode-go) ---
    from athaniel_cli.providers import ATHANIEL_OVERLAYS
    from athaniel_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Athaniel provider ID.
    # ATHANIEL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Athaniel IDs ("copilot").
    _mdev_to_athaniel = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in ATHANIEL_OVERLAYS.items():
        ...
        # Resolve Athaniel slug — e.g. "github-copilot" → "copilot"
        athaniel_slug = _mdev_to_athaniel.get(pid, pid)
        if athaniel_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or athaniel_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(athaniel_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or athaniel_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(athaniel_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(athaniel_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", athaniel_slug, exc)
        ...
        # configured.
        if not has_creds and athaniel_slug == "anthropic":
            try:
                ...
                if (athaniel_creds and athaniel_creds.get("accessToken")) or \
        ...
        if athaniel_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(athaniel_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(athaniel_slug)
                model_ids = _ids if _ids else (curated.get(athaniel_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(athaniel_slug, []) or curated.get(pid, [])
        elif athaniel_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `athaniel model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from athaniel_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(athaniel_slug, []) or curated.get(pid, [])
                if athaniel_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(athaniel_slug, model_ids)
        total = len(model_ids)
        if athaniel_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": athaniel_slug,
            "name": get_label(athaniel_slug),
            "is_current": athaniel_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "athaniel",
        ...
        seen_slugs.add(athaniel_slug.lower())
        _record_builtin_endpoint(athaniel_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ATHANIEL_OVERLAYS (keeps /model in sync
    # with `athaniel model`).
    try:
        from athaniel_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Athaniel writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Athaniel's own writer uses (mirrors section-4 grouping).
            ...
            # ``athaniel model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``athaniel model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from athaniel_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Athaniel's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, athaniel_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``athaniel model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from athaniel_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`athaniel_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from athaniel_cli.models import fetch_openrouter_models
