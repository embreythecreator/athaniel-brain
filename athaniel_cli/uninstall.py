"""
Athaniel Brain Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.athaniel/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Athaniel PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing athaniel-brain or athaniel PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Athaniel Brain" comment and following line
                if '# Athaniel Brain' in line or '# athaniel-brain' in line:
                    skip_next = True
                    continue
                if skip_next and ('athaniel' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing athaniel
                if 'athaniel' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Athaniel-owned file, and
...
def remove_wrapper_script():
    """Remove the athaniel wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "athaniel",
        Path.home() / ".local" / "bin" / "athaniel-acp",
        Path.home() / ".local" / "bin" / "athaniel-brain",
        Path("/usr/local/bin/athaniel"),
        Path("/usr/local/bin/athaniel-acp"),
        Path("/usr/local/bin/athaniel-brain"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains athaniel_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'athaniel_cli' in content or 'athaniel-brain' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(athaniel_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``athaniel`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Athaniel home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (athaniel_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``athaniel gateway run`` processes
    ...
    try:
        from athaniel_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from athaniel_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ATHANIEL_HOME`` and ``ATHANIEL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\athaniel\git\cmd``, ``%LOCALAPPDATA%\athaniel\git\bin``,
#      ``%LOCALAPPDATA%\athaniel\git\usr\bin``, ``%LOCALAPPDATA%\athaniel\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\athaniel\git\`` and Node to
#      ``%LOCALAPPDATA%\athaniel\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``athaniel dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\athaniel\gateway-service\`` and sometimes
...
def _athaniel_path_markers(athaniel_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Athaniel-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the athaniel launchers and the managed uv) — only wanted when
    ...
    """
    root = str(athaniel_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare athaniel-brain install dir.
    markers = [root + "\\athaniel-brain", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if ATHANIEL_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "athaniel".  We don't want to catch
    # unrelated entries like "cathaniel-foo" or "ephermeral", so we look for
    # backslash-athaniel as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(athaniel_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Athaniel-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<athaniel_home>\\bin`` (the managed binary
    dir holding the athaniel launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _athaniel_path_markers(athaniel_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_athaniel_env_vars_windows() -> list[str]:
    """Delete ATHANIEL_HOME and ATHANIEL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ATHANIEL_HOME", "ATHANIEL_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``athaniel`` launchers install.ps1 staged in the managed
    binary dir (the default Athaniel root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``athaniel`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_athaniel_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from athaniel_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from athaniel_constants import get_default_athaniel_root

        bin_dir = get_default_athaniel_root() / "bin"
    except Exception as e:
...
def _is_default_athaniel_home(athaniel_home: Path) -> bool:
    """Return True when ``athaniel_home`` points at the default (non-profile) root."""
    try:
        from athaniel_constants import get_default_athaniel_root
        return athaniel_home.resolve() == get_default_athaniel_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ATHANIEL_HOME directory.

    We shell out to ``athaniel -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ATHANIEL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m athaniel_cli.main` so we don't depend on a `athaniel`
    #    wrapper that may be half-removed mid-uninstall.
    athaniel_invocation = [_sys.executable, "-m", "athaniel_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                athaniel_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's ATHANIEL_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``athaniel uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ATHANIEL_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from athaniel_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(athaniel_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ✶ Athaniel Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Athaniel Chat GUI installation was found.")
        print(f"  Checked: {athaniel_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Athaniel agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(athaniel_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Athaniel agent at {athaniel_home / 'athaniel-brain'}")
        print(f"  • Your config, sessions, and secrets under {athaniel_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Athaniel agent is still installed. Run 'athaniel' to use the CLI,")
    print("or 'athaniel uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.athaniel/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.athaniel/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ATHANIEL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_athaniel_home(athaniel_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.athaniel data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ✶ Athaniel Brain Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {athaniel_home / 'config.yaml'}")
    print(f"  Secrets: {athaniel_home / '.env'}")
    print(f"  Data:    {athaniel_home / 'cron/'}, {athaniel_home / 'sessions/'}, {athaniel_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ATHANIEL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Athaniel data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Athaniel code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, athaniel_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Athaniel PATH entries from shell configs / Windows User PATH")
    print("  • Athaniel wrapper scripts and Athaniel-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Athaniel config/data: {athaniel_home}")
        if _is_default_athaniel_home(athaniel_home):
        ...
        print(f"  • Keep Athaniel config/data: {athaniel_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``athaniel`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ATHANIEL_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in athaniel_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\athaniel\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (athaniel\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_athaniel_home(athaniel_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(athaniel_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Athaniel-owned PATH entries in User environment")

        log_info("Removing ATHANIEL_HOME / ATHANIEL_GIT_BASH_PATH User env vars...")
        removed_env = remove_athaniel_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Athaniel-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing athaniel command...")
    ...
    #     would dangle — `athaniel` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows athaniel launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows athaniel launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Athaniel home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Athaniel-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(athaniel_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Athaniel-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(athaniel_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ATHANIEL_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from athaniel_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(athaniel_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.athaniel/, just remove the athaniel-brain subdir
            if athaniel_home in project_root.parents or project_root.parent == athaniel_home:
    ...
    #     under ATHANIEL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(athaniel_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(athaniel_home)
    ...
    # 5. Optionally remove ~/.athaniel/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ATHANIEL_HOME dirs live under
        ...
        try:
            if athaniel_home.exists():
                shutil.rmtree(athaniel_home)
                log_success(f"Removed {athaniel_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {athaniel_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {athaniel_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {athaniel_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://athaniel-brain.embreythecreator.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://athaniel-brain.embreythecreator.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Athaniel Brain! ✶")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m athaniel_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import athaniel_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``athaniel_constants`` + ``athaniel_cli.colors``
    (and lazily ``athaniel_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m athaniel_cli.uninstall")
    parser.add_argument(
        "--mode",
