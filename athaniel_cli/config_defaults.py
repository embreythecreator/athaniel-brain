"""Default configuration data for Athaniel Brain.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from athaniel_cli/config.py. Must not import from athaniel_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["athaniel-cli"],
    # SQLite journal mode used by every Athaniel database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Athaniel server processes.
    ...
    "session": {
        # Per-terminal `athaniel -c`: each CLI session drops a breadcrumb file
        # under $ATHANIEL_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by athaniel_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `athaniel chat --run-budget N`.
        ...
        # an interactive `athaniel gateway restart` must never block for hours
        ...
        # the Athaniel-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Athaniel
        ...
        # identity slot (SOUL.md). Empty by default. The ATHANIEL_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Athaniel adds a coding operating brief
        ...
        # `athaniel status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``ATHANIEL_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Athaniel' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Athaniel uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at ATHANIEL_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `athaniel -p <bot> chat -Q` recipient
        ...
        #             ATHANIEL_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ATHANIEL_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Athaniel auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Athaniel sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Athaniel
        ...
        # bundled Athaniel image, which drops to the `athaniel` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `athaniel tools` picker's
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes Athaniel spawn
        ...
        # `athaniel doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a Athaniel-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Athaniel' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.athaniel/browser-profile/) so copied
        ...
        # approval does it run `athaniel browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Athaniel sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Athaniel should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``athaniel chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.athaniel/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: athaniel sweeps the checkpoint base at startup
        ...
        # ``athaniel checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .athaniel.md, or .cursorrules before Athaniel applies
    ...
    # Single-query (``athaniel -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Athaniel truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Athaniel' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); athaniel =
                                      # Athaniel' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Athaniel' local
    ...
    "auxiliary": {
        ...
        "suggested_follow_ups": {
            ...
            "base_url": "",
            "api_key": "",
            # Unlike title_generation's 30s, this one is charged to the tail of
            # every turn — it rides the terminal chunk rather than a background
            # thread. Failure swallows to [], so a slow provider costs the
            # suggestion row, never the response.
            "timeout": 4,
            "extra_body": {},
            "reasoning_effort": "",  # per-task thinking level: none|minimal|low|medium|high|xhigh|max|ultra (empty = provider default)
        },
        ...
        # Invoked by ``athaniel kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``athaniel kanban decompose`` and the kanban
        ...
        # ``athaniel profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `athaniel model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `athaniel` (and `athaniel chat`) launches by default:
        ...
        # REPL and `--tui` (or ATHANIEL_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `athaniel --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `athaniel -c` muscle memory.  Default off so existing
        # users aren't surprised.  ATHANIEL_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `athaniel --tui` drops a one-time hint
        ...
        # model — see athaniel_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to ATHANIEL_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/athaniel`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `athaniel pets`. Disabled until
        ...
        "pet": {
            ...
            # get_athaniel_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see athaniel_cli/web_server.py — an event-loop stall
        ...
        # ATHANIEL_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (ATHANIEL_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``ATHANIEL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ATHANIEL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ATHANIEL_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ATHANIEL_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ATHANIEL_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/athaniel``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Athaniel appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.athaniel/cache/piper-voices/
    ...
    # "Hey Athaniel" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey athaniel",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_athaniel" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.athaniel/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.athaniel/cache/delegation/
    ...
    # response. If not, Athaniel feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Athaniel auto-pauses the goal and
    ...
    "moa": {
        ...
        # <athaniel_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.athaniel/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.athaniel/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``athaniel skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``athaniel skills trust`` / ``athaniel skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${ATHANIEL_SKILL_DIR} and ${ATHANIEL_SESSION_ID} in SKILL.md
        ...
        # (`athaniel skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.athaniel/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.athaniel/.curator_backups/blobs/. Enables `athaniel curator ledger`
        # and single-mutation `athaniel curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `athaniel curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `athaniel curator run --consolidate` overrides
        ...
        # normally restored on every `athaniel update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `athaniel curator purge` deletes
        ...
        # skipped), snapshot ~/.athaniel/skills/ into
        # ~/.athaniel/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `athaniel curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for athaniel-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Athaniel Brain*" header.
    ...
    # dangerous command. -q runs export ATHANIEL_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; ATHANIEL_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Athaniel' built-in platform hint for a single messaging platform
    ...
    # ``athaniel plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.athaniel/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ATHANIEL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .athaniel config) always require human
        ...
        # longer trigger the startup banner. Add via `athaniel doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``athaniel_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Athaniel to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ATHANIEL_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ATHANIEL_CRON_MAX_PARALLEL env var.
        ...
        # ATHANIEL_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # ATHANIEL_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via ATHANIEL_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `athaniel -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `athaniel` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `athaniel kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     athaniel-brain's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Athaniel tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.athaniel/logs/.
    ...
    # update model picker lists without shipping a athaniel-brain release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://athaniel-brain.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `athaniel model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Athaniel provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal ATHANIEL_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.athaniel/sessions/sessions.json entirely.
        ...
        # the ATHANIEL_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.athaniel.gateway` that defenses 1-2 don't
        ...
        # `athaniel update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``ATHANIEL_GATEWAY_MAX_STARTS`` / ``ATHANIEL_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.athaniel/.env,
        ...
        # behavior: files must live under the Athaniel cache, under
        ...
        # user. Bridged to ATHANIEL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Athaniel
        # cache (~/.athaniel/cache/{documents,images,audio,video,screenshots})
        ...
        # to ATHANIEL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ATHANIEL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ATHANIEL_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.athaniel/state.db.
    ...
    "sessions": {
        ...
        # the default of ``athaniel sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.athaniel/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `athaniel sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `athaniel update` prints a one-line notice with
        ...
        # ~/.athaniel/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ATHANIEL_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ATHANIEL_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `athaniel sessions export`. Checked
    ...
    # ``athaniel doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `athaniel doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``athaniel update`` behaviour.
    "updates": {
        ...
        #     DBs) into <ATHANIEL_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``athaniel backup``-style zip
        #     of ATHANIEL_HOME into <ATHANIEL_HOME>/backups/, restorable with
        #     ``athaniel import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `athaniel update` does with uncommitted local changes to the
        ...
        # When `athaniel update` finds the source checkout parked on a feature
        ...
        #                         merge. `athaniel update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<ATHANIEL_HOME>/lsp/bin/`` on first use.
    ...
    # `athaniel tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.athaniel/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.athaniel/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.athaniel/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Athaniel may use this encrypted
            ...
            # ~/.athaniel/bin/ on first use.  When False you must install
            ...
            # `athaniel secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.athaniel/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Athaniel disables it for our users unless they
        # explicitly opt in here. When false (default), Athaniel sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Athaniel also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Athaniel computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Athaniel passes the
    ...
    #
    # Configure with `athaniel egress setup`.  Disabled by default — the rest of
    # Athaniel works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.athaniel/bin/ on
    ...
    # Athaniel Desktop (Electron app) launch options. These only affect
    # `athaniel desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ATHANIEL_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the ATHANIEL_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Athaniel mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'athaniel model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Athaniel auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Athaniel' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Athaniel, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @athaniel:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ATHANIEL_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #athaniel)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: athaniel-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'athaniel-brain' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Athaniel API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Athaniel API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Athaniel API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ATHANIEL_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ATHANIEL_TOOL_PROGRESS_MODE for backward
    ...
    # ATHANIEL_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "ATHANIEL_PREFILL_MESSAGES_FILE": {
    ...
    "ATHANIEL_EPHEMERAL_SYSTEM_PROMPT": {
