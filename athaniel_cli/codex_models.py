...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in athaniel_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Athaniel uses. Removed here. If OpenAI re-enables them on Codex backend,
...
def _add_context_variants(model_ids: List[str]) -> List[str]:
    ...
    The suffix is Athaniel-side only — it is stripped before the model id hits
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Athaniel openai-codex talks to the same
