"""``athaniel verify`` — detect a project's run recipe and smoke-test it.
...
``.athaniel/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``athaniel verify`` satisfies
