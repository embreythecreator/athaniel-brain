...
- ``athaniel_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``athaniel_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``athaniel_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``athaniel_cli.main`` imports
``managed_uv``, ``athaniel_constants``, and friends only in its late path; the
...
:func:`athaniel_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from athaniel_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (athaniel_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``athaniel acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # athaniel_constants is stdlib-only, so the canonical layout helpers are safe
...
#: managed binary dir (the default Athaniel root's ``bin``, next to uv.exe)
#: on the user PATH. Keep in lockstep with the launcher list in
#: scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("athaniel", "athaniel-acp")


def _venv_is_relocatable(venv_dir: Path) -> bool:
    """True when the venv's pyvenv.cfg declares ``relocatable = true``.

    uv writes the flag; ``athaniel_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``athaniel`` launchers when they vanish.

    On Windows, ``athaniel`` resolves through launchers derived from the venv
    ...
    the managed binary dir — the default Athaniel root's ``bin``
    (``%LOCALAPPDATA%\\athaniel\\bin``, next to the managed uv) — which lives
    OUTSIDE the git checkout so no git operation can ever touch it. It is
    a per-machine dir shared by every profile: ``get_athaniel_home()`` would
    point inside ``profiles\\<name>`` under ``athaniel -p``, so the anchor
    here is :func:`athaniel_constants.get_default_athaniel_root`.

    Earlier installer versions staged them at ``<checkout>\\bin`` instead —
    inside the git working tree — where ``athaniel update``'s pre-update
    ...
    nothing restored them and ``athaniel`` stopped resolving in every new
    ...
    - canonical managed binary dir: only when *root* is the managed clone
      (``root.parent == get_default_athaniel_root()``), so source checkouts
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    as ``athaniel_constants.venv_bin_dir``.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    # Per-machine anchor: the DEFAULT Athaniel root, not get_athaniel_home() —
    # under ``athaniel -p <name>`` that returns ``profiles\\<name>``, which
    ...
    try:
        home = Path(get_default_athaniel_root())
    except Exception:
        return []
    ...
    # every athaniel_cli.main process start (right after the profile
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                "  ✓ Restored athaniel launcher(s): " + ", ".join(restored),
                file=sys.stderr,
            )
    return restored
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``athaniel_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *,
    ...
    """One-time PATH migration to the ``ATHANIEL_HOME\\bin`` launcher layout.

    Runs from the ``athaniel update`` tail (and mirrors what install.ps1's
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from athaniel_constants import get_default_athaniel_root, venv_bin_dir
    ...
    if kept != entries:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                f"  ✓ athaniel launchers now resolve from {home_bin} "
...
def _quarantine_running_athaniel_exe(
    ...
    """Rename live athaniel*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``athaniel acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = (
        _quarantine_running_athaniel_exe(scripts_dir, failed_out=failed)
        if scripts_dir
        else []
    )
    if failed:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `athaniel` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``athaniel*.exe`` shims on Windows so they can be replaced
