"""``athaniel_cli/_scan_venv_blockers.py`` — Standalone venv-process scan for JSON consumption.

Invoked by the Desktop Electron app::

    venv\\Scripts\\python.exe -m athaniel_cli._scan_venv_blockers
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    athaniel_cli.main gateway run`` and the uv-side interpreter re-running the
    same argv). Reporting those as blockers dead-ends the Desktop update:
    the preflight aborts with ``venv-blocked`` *before* spawning
    ``athaniel-setup``, so the CLI updater's own
    ``_pause_windows_gateways_for_update()`` — which exists precisely to
    stop these processes (and is always active: ``athaniel-setup`` invokes
    ``athaniel update --yes --gateway``) — never gets the chance to run.
...
def main() -> None:
    ...
    try:
        from athaniel_cli.main import _detect_venv_python_processes  # noqa: PLC0415
