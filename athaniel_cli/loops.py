...
Design notes / invariants (same contract as ``athaniel_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.AthanielCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from athaniel_cli.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from athaniel_cli.goals import _warn_dropped_write
