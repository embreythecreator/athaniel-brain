"""Persistent session goals — the Ralph loop for Athaniel.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Athaniel feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.AthanielCLI`` or the gateway
...
# creation, and the first athaniel_cli.config import (journal-mode
...
def _bootstrap_session_db(home: str, done: threading.Event) -> None:
    ...
    try:
        ...
        # ATHANIEL_HOME). It then caches the wrong profile's DB under this
        # profile's key.
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current ATHANIEL_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``athaniel_home`` path so profile switches still pick up the right DB.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.athaniel/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``athaniel -p <profile> chat -q "work kanban task <id>"``). The worker's
