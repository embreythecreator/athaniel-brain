...
from athaniel_cli.model_council import model_council_moa_preset

MOA_MARKER_PREFIX = "__ATHANIEL_MOA_TURN_V1__"
...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model Athaniel agent
...
# WO-MOA/2: the four postures of the MoA faculty. `ensemble` is today's
# behavior and the default everywhere — a config with no `mode` key runs
# byte-identical to pre-WO-MOA/2. The other three are normalized/stored here
# but remain inert until the mode-aware fan-out lands in moa_loop.py.
MOA_MODES = ("ensemble", "self_consistency", "debate", "verify")


def coerce_moa_mode(value: Any) -> str:
    text = str(value or "").strip().lower().replace("-", "_")
    return text if text in MOA_MODES else "ensemble"


def _coerce_sc_fallback(value: Any) -> str:
    """self_consistency on a non-categorical task: fail loudly or degrade.

    `error` (default) refuses the turn — the contract forbids LLM-judged
    prose agreement, so a prose task simply cannot run this posture.
    `ensemble` silently degrades to the ensemble posture instead.
    """
    text = str(value or "").strip().lower()
    return text if text in ("error", "ensemble") else "error"
...
def _default_preset() -> dict[str, Any]:
    ...
    return {
        ...
        # matching single-model agent behavior. The council preset supplies
        # its own explicit values, so they ride through unchanged here.
        ...
        # Council direct-routing re-runs advisors every tool iteration so
        # advice tracks live task state (see _coerce_fanout docstring).
        "fanout": "per_iteration",
        "enabled": preset["enabled"],
        # WO-MOA/2 posture keys — inert until moa_loop.py reads them.
        "mode": "ensemble",
        "debate_rounds": 2,
        "debate_round_budget": None,
        "self_consistency_samples": 3,
        "self_consistency_fallback": "error",
    }
...
def _normalize_preset(raw: Any) -> dict[str, Any]:
    ...
    return {
        ...
        # WO-MOA/2 posture keys. All defaults reproduce pre-WO behavior
        # (mode=ensemble; the rest are inert unless a non-ensemble mode is
        # selected). debate_round_budget None = unbudgeted; the debate
        # protocol still hard-stops at debate_rounds.
        "mode": coerce_moa_mode(raw.get("mode")),
        "debate_rounds": max(1, _coerce_int(raw.get("debate_rounds"), 2)),
        "debate_round_budget": _coerce_int_or_none(raw.get("debate_round_budget")),
        "self_consistency_samples": max(
            2, _coerce_int(raw.get("self_consistency_samples"), 3)
        ),
        "self_consistency_fallback": _coerce_sc_fallback(
            raw.get("self_consistency_fallback")
        ),
    }
...
def resolve_moa_preset(config: Any, name: str | None = None) -> dict[str, Any]:
    ...
    if preset is None:
        ...
        raise MoAPresetNotFoundError(
            ...
            f"{available}. Run `athaniel moa list`."
        )
...
def exact_moa_preset_name(config: Any, text: str) -> str | None:
    ...
    ``athaniel_cli/model_switch.py``) to recognize a bare ``/model <preset>``
