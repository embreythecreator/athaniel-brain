...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``athaniel model`` CLI picker. Adds network calls
    ...
    try:
        from athaniel_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``athaniel_cli/moa_cmd.py``.
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`athaniel_cli.providers.custom_provider_aliases` — lets the frontend
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from athaniel_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `athaniel model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Athaniel' own Anthropic device flow
    ...
    try:
        ...
        athaniel_creds = read_athaniel_oauth_credentials() or {}
        if athaniel_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Athaniel.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Athaniel device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Athaniel overlay declares it keyless."""
    try:
        from athaniel_cli.providers import ATHANIEL_OVERLAYS
        overlay = ATHANIEL_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `athaniel model` to configure ({auth_type})"
        )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`athaniel_cli.model_switch.list_picker_providers`) so the
