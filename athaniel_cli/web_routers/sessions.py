...
from athaniel_state import is_malformed_db_error
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another Athaniel surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Athaniel backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `athaniel sessions stats`).
