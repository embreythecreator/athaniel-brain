...
late-binding seam in :mod:`athaniel_cli.web_deps` (``late`` for callables,
...
from athaniel_cli.web_deps import late, LateState
from athaniel_cli.web_models import (
...
_log = logging.getLogger("athaniel_cli.web_server")
...
_spawn_athaniel_action = late("_spawn_athaniel_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from athaniel_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `athaniel tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `athaniel computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``athaniel tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `athaniel tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``athaniel tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.athaniel/.env`` via ``save_env_value`` —
    the same store ``athaniel tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``athaniel tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``athaniel -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ATHANIEL_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``athaniel computer-use permissions grant`` as a background action.
