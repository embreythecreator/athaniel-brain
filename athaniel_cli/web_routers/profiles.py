...
late-binding seam in :mod:`athaniel_cli.web_deps` so tests that
...
from athaniel_cli.web_deps import late
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the ATHANIEL_HOME junk filters — through the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's ATHANIEL_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``athaniel profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ATHANIEL_HOME).
    """
    from athaniel_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``athaniel profile use``).
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (athaniel_cli.profiles
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ATHANIEL_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``athaniel profile describe
    <name> --auto``.
    ...
    try:
        from athaniel_cli import profile_describer
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps athaniel_cli.profiles.export_profile /
# import_profile (the same machinery behind `athaniel profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_athaniel_home() / "profile-exports"
        try:
