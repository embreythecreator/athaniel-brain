...
the late-binding seam in :mod:`athaniel_cli.web_deps` so tests that
``monkeypatch.setattr(web_server, "_spawn_athaniel_action", ...)`` keep
working.
"""
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from athaniel_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
