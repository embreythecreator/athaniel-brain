"""Athaniel-managed uv and Python runtime repair.

Athaniel owns its own uv binary at ``$ATHANIEL_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ATHANIEL_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Athaniel
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.athaniel-runtime/python``. A
...
from athaniel_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".athaniel-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.embreythecreator.athaniel.managed-python"
...
def managed_uv_path() -> Path:
    """Return the path where Athaniel keeps *its* uv binary.

    ``$ATHANIEL_HOME/bin/uv`` on POSIX, ``$ATHANIEL_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Athaniel-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Athaniel provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Athaniel keeps databases "
            "out of WAL mode on this SQLite build. The next `athaniel update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``athaniel update`` runs
    the call site from the *old*, already-imported ``athaniel_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``athaniel update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under ATHANIEL_HOME
    caches the last successful self-update time.
    """
    try:
        from athaniel_constants import get_athaniel_home

        stamp = get_athaniel_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``athaniel update`` so the managed copy stays current.
...
def _reload_athaniel_constants():
    """Re-execute ``athaniel_constants`` from disk and return the fresh module.

    ``athaniel update`` imports ``athaniel_constants`` from the OLD checkout,
    ...
    return importlib.reload(athaniel_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Athaniel generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `athaniel update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("athaniel_cli.main")
    ...
    if holders:
        ...
        return True, f"other Athaniel processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Athaniel-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``athaniel update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``athaniel update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$ATHANIEL_HOME/bin/`` instead of ``~/.local/bin/``.
    """
