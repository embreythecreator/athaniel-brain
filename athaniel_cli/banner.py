...
Pure display functions with no AthanielCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `athaniel ... > file`) raises
...
from athaniel_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

ATHANIEL_AGENT_LOGO = """[bold]███████╗███████╗██████╗  █████╗ ██████╗ ██╗  ██╗██╗███████╗██╗[/]
[bold]██╔════╝██╔════╝██╔══██╗██╔══██╗██╔══██╗██║  ██║██║██╔════╝██║[/]
[bold]███████╗█████╗  ██████╔╝███████║██████╔╝███████║██║█████╗  ██║[/]
[bold]╚════██║██╔══╝  ██╔══██╗██╔══██║██╔═══╝ ██╔══██║██║██╔══╝  ██║[/]
[bold]███████║███████╗██║  ██║██║  ██║██║     ██║  ██║██║███████╗███████╗[/]
...
# Face's core-idle.webp, frame 0, converted once to 30 x 13 Braille art.
# Grayscale crop (24, 24, 232, 232), Lanczos to 60 x 52 pixels;
# gamma 1.05, 95% luminance, 4 x 4 Bayer dithering, cutoff 0.06.
# Ellipse centre (29.5, 25.5), radii (24, 22), contour tolerance 0.025.
# Shorter proportions compensate for terminal line height. Dim the lower
# hemisphere to keep the crown dominant; inherit foreground for light themes.
# Braille blanks preserve alignment through Rich's centred layout. Embedded
# art has no image library or Face dependency at runtime.
ATHANIEL_SIGIL = """⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⣀⣤⢲⢮⢿⢿⢿⢿⢿⢿⢵⣖⢤⣀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⢀⣴⢿⢜⢽⢝⢝⢝⢝⢝⢝⢝⢽⢝⢽⢿⢵⢿⢦⡀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⣠⢿⢝⢕⠝⢕⠕⠅⠕⠅⠕⠅⠕⢕⠝⢕⢝⢽⢝⢽⢽⣄⠀⠀⠀⠀
...
# (e.g. nix-built athaniel — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/embreythecreator/athaniel-brain.git"
_OFFICIAL_REPO_CANONICAL = "github.com/embreythecreator/athaniel-brain"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/embreythecreator/athaniel-brain/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "athaniel-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `athaniel update`,
...
def _fetch_pypi_latest(package: str = "athaniel-brain") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Athaniel update is available.

    Two paths: if ``ATHANIEL_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = athaniel_home / ".update_check"
    embedded_rev = os.environ.get("ATHANIEL_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ATHANIEL_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/athaniel/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from athaniel_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $ATHANIEL_HOME/athaniel-brain/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = athaniel_home / "athaniel-brain"
        if not (repo_dir / ".git").exists():
...
def absorb_offer_line(repo: str) -> Optional[str]:
    ...
    try:
        ...
        if not tag:
            return None
        return f"✶ upstream {tag} available to absorb · run `athaniel absorb {tag}`"
    except Exception:
        return None


def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Athaniel git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ATHANIEL_HOME/athaniel-brain/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``athaniel_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from athaniel_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/embreythecreator/athaniel-brain/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Athaniel checkout. Cached per-process. Release URL always points at the
    canonical embreythecreator/athaniel-brain repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Athaniel Brain v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from athaniel_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built athaniel; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_athaniel_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_athaniel_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """Build and print a welcome banner with the orb on left and info on right.
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ATHANIEL_SIGIL
    except Exception:
        _bskin = None
        _hero = ATHANIEL_SIGIL
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Athaniel[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or athaniel setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Athaniel[/]")

    if os.getenv("ATHANIEL_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from athaniel_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from athaniel_cli.codex_runtime_switch import get_current_runtime
        from athaniel_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ATHANIEL_AGENT_LOGO
        console.print(_logo)
        console.print()
