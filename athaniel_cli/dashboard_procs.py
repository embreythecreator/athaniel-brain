"""Dashboard process-hygiene helpers — extracted from ``athaniel_cli/main.py``.
...
``_detect_concurrent_athaniel_instances``) are lifted verbatim. References to
helpers that STAY in ``athaniel_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``athaniel_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``athaniel_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``athaniel_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``athaniel dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``athaniel update`` replaces files on disk, the running
    ...
    ``athaniel-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Athaniel Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``athaniel serve`` as
    ...
    environment variable ``ATHANIEL_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`athaniel serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "athaniel.*dashboard"`) keeps us consistent
            # with `athaniel_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `athaniel --profile p serve --host <ip>` contains
    # neither "athaniel serve" nor "athaniel_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from athaniel_cli.process_identity import ledger_entries
...
def _athaniel_home_for_pid(pid: int) -> str | None:
    """Best-effort ``ATHANIEL_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("ATHANIEL_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"ATHANIEL_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Athaniel Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``athaniel update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], athaniel_home: str | None = None
) -> str:
    """Stable owner key: ``ATHANIEL_HOME`` when known, else ``--profile`` / ``-p``.

    ``ATHANIEL_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.athaniel`` roots) keep a resolved ``home:`` key so
    ...
    if athaniel_home:
        try:
            home_path = Path(athaniel_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(athaniel_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(athaniel_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``athaniel update``.

    Each candidate is ``(pid, argv, athaniel_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_athaniel_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``ATHANIEL_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``ATHANIEL_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``ATHANIEL_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``athaniel update`` respawn detaches with ``start_new_session=True``, so
    ...
    if own_home is None:
        try:
            ...
            own_home = str(get_athaniel_home())
        except Exception:
            own_home = ""
    ...
    for _pid, argv, athaniel_home in candidates:
        if not argv:
            continue
        if _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and athaniel_home and _normalized_home_for_compare(athaniel_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, athaniel_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``athaniel dashboard`` / ``athaniel serve`` processes.

    Called at the end of ``athaniel update`` (default ``reason``) and also
    from ``athaniel dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``athaniel update`` path), a detected
    ``athaniel-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``athaniel-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``athaniel update``'s systemd
    fleet-restart loop, which restarts ``athaniel-serve*`` units before this
    ...
    # When the Athaniel Desktop Electron app spawns this dashboard as a
    # backend child, it sets ATHANIEL_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] = set()
    raw_pid = os.environ.get("ATHANIEL_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `athaniel update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot ATHANIEL_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _athaniel_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. athaniel update's
        ...
        for pid in pids:
            try:
                ...
                if not pid_is_athaniel(
                    ...
                    failed.append((pid, "not athaniel-owned or process identity changed"))
                    continue
                result = subprocess.run(
    ...
    #    remote backend (athaniel serve) under Restart=on-failure never comes
...
def _detect_concurrent_athaniel_instances(
    ...
    The Athaniel Desktop Electron app spawns ``athaniel.EXE`` as a backend child,
    so during ``athaniel update`` the user-invoked process and the desktop's
    ...
    shims (``athaniel.exe`` / ``athaniel-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``athaniel update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``athaniel.exe``) is a separate process from the Python
    ...
    for shim in _m()._athaniel_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated athaniel.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``athaniel update``
    ...
    #      athaniel.exe sitting *under* a non-Athaniel parent (e.g. a Athaniel
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        athaniel serve --host 127.0.0.1 --port 0
        athaniel serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "athaniel" not in cmd and "athaniel_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (ATHANIEL_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("ATHANIEL_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``athaniel serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``athaniel serve`` on a Mac
...
def _athaniel_home_dir() -> Path:
    """Resolved Athaniel home (ATHANIEL_HOME override or ~/.athaniel)."""
    override = os.environ.get("ATHANIEL_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "athanielPath", "athanielHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated ATHANIEL_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{athaniel_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _athaniel_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local athaniel serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``athaniel serve`` backends with no parent.
    ...
    running under ATHANIEL_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never ATHANIEL_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports ATHANIEL_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # ATHANIEL_BACKEND_READY, so a sibling spawned seconds ago is not yet
