"""Cross-process mutual exclusion for in-flight Athaniel updates.

Three different surfaces can start an update of the same install tree:

* ``athaniel update`` from a terminal,
* the dashboard's Update button (``POST /api/athaniel/update`` →
  ``_spawn_athaniel_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``athaniel-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``athaniel update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ATHANIEL_HOME>/.athaniel-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``athaniel update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Athaniel is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``athaniel-setup`` binary under ``~/.athaniel`` is only refreshed by a full
  installer run (``copy_self_to_athaniel_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Athaniel is still running") on every
  GUI update forever, with no Athaniel process actually running.
"""
...
MARKER_NAME = ".athaniel-update-in-progress"

# Set by an orchestrating updater (the Tauri `athaniel-setup --update` flow) to
# its own pid before spawning `athaniel update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ATHANIEL_UPDATE_HANDOFF_PID"
...
# to show "Athaniel is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Athaniel home (never the context-local profile override):
    the Rust updater resolves ``$ATHANIEL_HOME`` or the platform default, and the
    ...
    return get_process_athaniel_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``athaniel update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``athaniel-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Athaniel update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `athaniel update` as a stage): we run under ITS claim
