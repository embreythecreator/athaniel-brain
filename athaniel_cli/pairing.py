...
Usage:
    athaniel pairing list              # Show all pending + approved users
    athaniel pairing approve <platform> <request-id|code>  # Approve a pairing request
    athaniel pairing revoke <platform> <user_id> # Revoke user access
    athaniel pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle athaniel pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: athaniel pairing {list|approve|revoke|clear-pending}")
        print("Run 'athaniel pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: athaniel pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.athaniel/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'athaniel pairing list' to see pending requests.\n")
