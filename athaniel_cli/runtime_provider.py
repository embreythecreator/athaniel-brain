...
from athaniel_cli import auth as auth_mod
...
from athaniel_cli import config as _config_mod
from athaniel_cli.providers import custom_provider_aliases, custom_provider_slug
from athaniel_constants import OPENROUTER_BASE_URL
from athaniel_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`athaniel_cli.config.load_config`.

    Deliberately NOT a module-level ``from athaniel_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``athaniel_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``athaniel_cli.config.load_config`` OR
    ``athaniel_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from athaniel_cli.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # host_mandated_api_mode clause in athaniel_cli/providers.py so the
...
def _fallback_api_mode(provider: str, base_url: str, model: str = "") -> str:
    ...
    if detected:
        return detected
    from athaniel_cli.providers import determine_api_mode
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from athaniel_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Athaniel' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from athaniel_cli.config import _canonical_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from athaniel_cli.models import azure_foundry_model_api_mode
        ...
        if configured_provider == provider and pool_url_is_default:
            ...
            if cfg_base_url:
                base_url = cfg_base_url
        configured_mode = _parse_api_mode(model_cfg.get("api_mode"))
        from athaniel_cli.models import opencode_provider_family
        if opencode_provider_family(provider) is not None:
            ...
            # Refs #16878.
            from athaniel_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("ATHANIEL_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/athaniel_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'athaniel doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ATHANIEL_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("ATHANIEL_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'athaniel model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.athaniel/.env or run 'athaniel model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'athaniel model')."
        )
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("ATHANIEL_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("ATHANIEL_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from athaniel_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `athaniel auth list`). If the key is
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("ATHANIEL_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "athaniel-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from athaniel_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Athaniel canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Athaniel-compatible importers).
