"""``athaniel sessions`` command — extracted from ``athaniel_cli/main.py``.
...
Helpers that stay in ``athaniel_cli.main`` (``get_athaniel_home``,
...
``athaniel_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_athaniel_home():
    return _m().get_athaniel_home()
...
#: Default age floor for `athaniel sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`athaniel sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from athaniel_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    athaniel sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from athaniel_cli.session_recovery import (
    ...
    if action == "import":
        from athaniel_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from athaniel_state import workspace_key as _ws_key
        ...
        from athaniel_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from athaniel_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_athaniel_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from athaniel_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_athaniel_home() / "session-exports"
        ...
        # `athaniel sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `athaniel sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `athaniel sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `athaniel sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: athaniel sessions pin <session_id>"
            )
            return
        ...
        # Launch athaniel --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`athaniel sessions optimize` later to reclaim freed space.)")
