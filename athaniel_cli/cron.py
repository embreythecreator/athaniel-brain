"""
Cron subcommand for athaniel CLI.
...
# commands at execution time when ``_ATHANIEL_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: athaniel gateway install", Colors.DIM))
    print(color("                    sudo athaniel gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  athaniel cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'athaniel cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `athaniel cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    ``athaniel cron incidents [--state <s>]`` lists incidents (the stored error
    is redacted and truncated at write time, safe for terminal display);
    ``athaniel cron incidents ack <id>`` closes one so its failure ping stays
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(
                color(
                    "✗ Incident ID required: athaniel cron incidents ack <incident_id>",
                    Colors.RED,
                )
            )
            return 1
    ...
    print(
        color(
            ...
            "athaniel cron incidents ack <id>",
            Colors.DIM,
        )
    )
    return 0
...
def cron_status():
    ...
    if pids or gateway_alive_via_lock:
        ...
        if hb_age is None:
            ...
            print("  If the gateway just started, wait ~60s and re-run `athaniel cron status`.")
            print("  If heartbeat never appears, restart: athaniel gateway restart")
            ...
            if pids:
                print(f"  PID: {', '.join(map(str, pids))}")
            print("  Cron jobs may NOT be firing. Restart: athaniel gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec athaniel "
                        "athaniel cron ...`). Fix ownership to match the "
        ...
        print("    athaniel gateway install    # Install as a user service")
        print("    sudo athaniel gateway install --system  # Linux servers: boot-time system service")
        print("    athaniel gateway            # Or run in foreground")
...
def _scripts_dir_for_cron() -> Path:
    ...
    Prefer ``cron.jobs.CRON_DIR.parent`` over a fresh ``get_athaniel_home()`` call
    so tests and profile-aware callers that monkeypatch cron storage inspect the
    same Athaniel home the jobs were loaded from.
    """
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside ATHANIEL_HOME/scripts: {script!r}"
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `athaniel cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (ATHANIEL_SESSION_KEY);
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _athaniel_now, rearm_oneshot

    run_at = _athaniel_now().isoformat() if args.run_now else args.run_at
    try:
...
def cron_notepad(args) -> int:
    """Handle ``athaniel cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: athaniel cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: athaniel cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: athaniel cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: athaniel cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
