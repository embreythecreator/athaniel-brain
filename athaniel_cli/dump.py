"""
Dump command for athaniel CLI.

Outputs a compact, plain-text summary of the user's Athaniel setup
...
from athaniel_cli.config import get_athaniel_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.athaniel/.env.
    ...
    exports. ``athaniel debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.athaniel_build_sha`` by the Dockerfile's
    ``ATHANIEL_GIT_SHA`` build-arg (see ``athaniel_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``athaniel dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from athaniel_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(athaniel_home: Path) -> int:
...
def _cron_summary(athaniel_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = athaniel_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"athaniel_home:      {display_athaniel_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.athaniel/.env: a managed
        ...
        # A credential added via `athaniel auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `athaniel auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["athaniel-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(athaniel_home)}")
    lines.append(f"  skills:             {_count_skills(athaniel_home)}")
