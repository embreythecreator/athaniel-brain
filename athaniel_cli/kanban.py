"""CLI for the Athaniel Kanban board — ``athaniel kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/athaniel-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from athaniel_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    athaniel_home: Optional[Path] = None,
) -> tuple[bool, str]:
    """Return ``(running, message)``.

    - ``running=True``: a gateway is alive for this ATHANIEL_HOME and its
    ...
    Used by ``athaniel kanban create`` (and callers) to warn when a task
    ...
    ``athaniel_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different ATHANIEL_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=athaniel_home, use_cache=False
        )
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`athaniel kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Athaniel profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://athaniel-brain.embreythecreator.com/docs/user-guide/features/kanban "
            "or docs/athaniel-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # ATHANIEL_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `athaniel kanban boards switch <slug>` or the "
            "ATHANIEL_KANBAN_BOARD env var). Use `athaniel kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    b_import = boards_sub.add_parser(
        ...
        description=(
            "Import a .tar.gz produced by `athaniel kanban boards export`. "
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `athaniel project list`.")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $ATHANIEL_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $ATHANIEL_PROFILE or 'user')")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $ATHANIEL_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `athaniel gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.athaniel/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $ATHANIEL_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $ATHANIEL_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``athaniel kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: athaniel kanban <action> [options]\n"
                "Run 'athaniel kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # athaniel_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using ATHANIEL_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `athaniel kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # ATHANIEL_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `athaniel kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("ATHANIEL_PROFILE_NAME", "ATHANIEL_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("ATHANIEL_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (athaniel kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``athaniel kanban boards <action>``.
    ...
    DB, so a fresh ATHANIEL_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `athaniel kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `athaniel kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `athaniel kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `athaniel kanban boards delete <slug>` (alias), the
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    from athaniel_cli import kanban_transfer
    ...
    print("Import it with `athaniel kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `athaniel kanban boards switch {res['board']}`.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.athaniel/profiles/.")
        print("Create one with `athaniel -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  athaniel gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `athaniel -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from athaniel_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``athaniel kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "athaniel kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    athaniel gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`athaniel kanban list --status ready` / "
                    f"`athaniel kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Athaniel profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from athaniel_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from athaniel_cli import kanban_decompose as decomp
