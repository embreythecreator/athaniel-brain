...
The Athaniel docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.athaniel/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/embreythecreator/athaniel-brain/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_athaniel_home() / "cache" / "model_catalog.json"
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Athaniel silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`athaniel update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``athaniel update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.athaniel/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``athaniel model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
