"""
Backup and import commands for athaniel CLI.

`athaniel backup` creates a zip archive of the entire ~/.athaniel/ directory
(excluding the athaniel-brain repo and transient files).

`athaniel import` restores from a backup zip, overlaying onto the current
ATHANIEL_HOME root.
"""
...
from athaniel_constants import get_default_athaniel_root, get_athaniel_home, display_athaniel_home
from utils import (
    _preserve_file_mode,
...
from athaniel_cli.sizefmt import format_bytes as _format_size
...
# Where ``athaniel backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``athaniel-brain`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/athaniel-brain/``
...
# ATHANIEL_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "athaniel-brain",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under ATHANIEL_HOME) —
...
# File names that ``athaniel import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ATHANIEL_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Athaniel backup slot."""
...
@contextmanager
def _backup_operation_lock(athaniel_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = athaniel_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Athaniel backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ATHANIEL_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to athaniel root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``athaniel-brain`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/athaniel-brain/`` — must be preserved.
        if part == "athaniel-brain" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ATHANIEL_HOME.
...
# to the user as a hung `athaniel update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # athaniel_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Athaniel home directory."""
    athaniel_root = get_default_athaniel_root()

    if not athaniel_root.is_dir():
        print(f"Error: Athaniel home directory not found at {athaniel_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(athaniel_root):
            _run_backup_locked(args, athaniel_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, athaniel_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"athaniel-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"athaniel-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_athaniel_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(athaniel_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(athaniel_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``athaniel-brain`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "athaniel-brain" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(athaniel_root)
    ...
    # outside ATHANIEL_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_athaniel_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: athaniel import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Athaniel backup.
    ...
    # Look for telltale files that a athaniel home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Athaniel backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.athaniel/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a athaniel dir name
        if prefix in {".athaniel", "athaniel"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``athaniel import``, which is the
    ...
    so without the chown a ``sudo athaniel import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``athaniel import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Athaniel' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_athaniel_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (athaniel_root / "config.yaml").exists()
        has_env = (athaniel_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Athaniel configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ATHANIEL_HOME.
            ...
            target = athaniel_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(athaniel_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_athaniel_home()}."
            )
        ...
        profiles_dir = athaniel_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # athaniel_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: athaniel profile list  (after installing athaniel)")

        # Guidance
        print()
        if not (athaniel_root / "athaniel-brain").is_dir():
            print("Note: The athaniel-brain codebase was not included in the backup.")
            print("  If this is a fresh install, run: athaniel update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  athaniel -p {pname} gateway install")
        ...
        try:
            from athaniel_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Athaniel configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and athaniel backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ATHANIEL_HOME).
...
# — `athaniel update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ATHANIEL_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ATHANIEL_HOME and the entry is silently
...
def _quick_snapshot_root(athaniel_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: athaniel snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(athaniel_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ATHANIEL_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``athaniel update``.
    ...
    Args:
        ...
        athaniel_home: Override for the Athaniel home directory (tests).
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_athaniel_home()
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, athaniel_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(athaniel_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for athaniel backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_athaniel_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, athaniel_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(athaniel_root):
            return _write_full_zip_backup_locked(out_path, athaniel_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, athaniel_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``athaniel_root`` to ``out_path``.
...
def _pre_update_backup_dir(athaniel_home: Optional[Path] = None) -> Path:
    home = athaniel_home or get_athaniel_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ATHANIEL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ATHANIEL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``athaniel update``) should continue even if the backup fails.
    """
    athaniel_root = athaniel_home or get_default_athaniel_root()
    if not athaniel_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(athaniel_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, athaniel_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `athaniel claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ATHANIEL_HOME under ``backups/`` before a
    ``athaniel claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``athaniel import <archive>``.  Writes to
    ``<ATHANIEL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `athaniel import` and the
