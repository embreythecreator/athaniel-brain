"""Slash command definitions and autocomplete for the Athaniel CLI.
...
from athaniel_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``athaniel_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``athaniel_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Athaniel config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Athaniel works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .athaniel/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Athaniel' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Athaniel is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.athaniel/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Athaniel Brain to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Athaniel Brain version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`athaniel_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/athaniel`` subcommand mapping, and
    ...
    """
    try:
        from athaniel_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Athaniel ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Athaniel
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``athaniel skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``athaniel skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``athaniel skills list`` and
...
# unrelated command landed. These claim their slots right after /athaniel,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /athaniel <command>).
...
# native slot, the alias spelling stays reachable via /athaniel reset).
...
# ``/athaniel <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/athaniel" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /athaniel topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /athaniel moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /athaniel debug on Slack.
#   - plan: plan-mode toggle; reached via /athaniel plan on Slack.
#   - egress: Docker-only proxy status; reachable as /athaniel egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /athaniel init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /athaniel version on
...
#   - diff: git working-tree diff; reached via /athaniel diff on Slack so it
...
#     /athaniel update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /athaniel heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /athaniel refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /athaniel pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /athaniel whoami on Slack.
...
#     /athaniel platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_ATHANIEL_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "plan", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/athaniel <verb>`` subcommand.
    ...
    can still reach them via ``/athaniel <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/athaniel`` is always reserved as the first entry so the
    legacy ``/athaniel <subcommand>`` form keeps working for anything that
    ...
    # Reserve /athaniel as the catch-all top-level command.
    entries.append(("athaniel", "Talk to Athaniel or run a subcommand", "[subcommand] [args]"))
    seen.add("athaniel")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ATHANIEL_ONLY:
            # Intentionally Slack-via-/athaniel only (see _SLACK_VIA_ATHANIEL_ONLY).
            return
    ...
    # everything except /athaniel, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://athaniel-brain.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /athaniel handler.

    Maps both canonical names and aliases so /athaniel bg do stuff works
    the same as /athaniel bg do stuff.

    Plugin-registered slash commands are included so ``/athaniel <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from athaniel_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via athaniel_cli.personality."""
        try:
