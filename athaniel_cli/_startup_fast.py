...
This module is imported by ``athaniel_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no athaniel_cli.config,
...
``athaniel_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(athaniel_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ATHANIEL_HOME?"""
    active_profile = os.path.join(athaniel_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    athaniel_home = os.environ.get("ATHANIEL_HOME", "").strip()
    if athaniel_home:
        return athaniel_home
    return os.path.join(os.path.expanduser("~"), ".athaniel")
...
def container_mode_may_be_active() -> bool:
    ...
    athaniel_home = os.environ.get("ATHANIEL_HOME", "").strip()
    if athaniel_home:
        if os.path.exists(os.path.join(athaniel_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(athaniel_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(athaniel_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".athaniel")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ATHANIEL_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``athaniel --version`` output (also used by /version).
    ...
    ``athaniel version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from athaniel_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from athaniel_cli import __release_date__, __version__

        print(f"Athaniel Brain v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from athaniel_cli.config import detect_install_method
    ...
    try:
        from athaniel_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``athaniel --version`` before the heavy import wall.
    ...
    container. Termux keeps the ATHANIEL_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("ATHANIEL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
