...
this. Capabilities govern the *host API surfaces* Athaniel hands out (which
...
until the user re-consents (``athaniel plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Athaniel may declare ids this build doesn't know; they can
