"""Migrate Athaniel' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Athaniel' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Athaniel mcp_servers.<n>.command/args/env  → codex stdio transport
  Athaniel mcp_servers.<n>.url/headers       → codex streamable_http transport
  Athaniel mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Athaniel mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Athaniel-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Athaniel' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Athaniel config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Athaniel' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, athaniel_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Athaniel MCP server config to the codex inline-table dict
    ...
    if not isinstance(athaniel_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(athaniel_cfg.get("command"))
    has_url = bool(athaniel_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(athaniel_cfg["command"])
        args = athaniel_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = athaniel_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = athaniel_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(athaniel_cfg["url"])
        headers = athaniel_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Athaniel' transport: sse hint is informational; codex auto-negotiates
        if athaniel_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in athaniel_cfg:
        try:
            out["tool_timeout_sec"] = float(athaniel_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in athaniel_cfg:
        try:
            out["startup_timeout_sec"] = float(athaniel_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if athaniel_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in athaniel_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_ATHANIEL_KEYS:
            skipped.append(f"{key} (unknown Athaniel key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ATHANIEL_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Athaniel)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Athaniel' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Athaniel' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Athaniel
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="athaniel-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ATHANIEL_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed athaniel-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.athaniel`` symlink that happens to live under ``/private/var/folders``
...
def _build_athaniel_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Athaniel' own
    ...
    so a athaniel installed under /opt/, /usr/local/, or a venv all work.
    ATHANIEL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ATHANIEL_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_athaniel_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ATHANIEL_HOME its launcher sets
    ...
    # — that would override the launcher's ATHANIEL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ATHANIEL_HOME", tmp_path) would
    ...
    athaniel_home = os.environ.get("ATHANIEL_HOME") or ""
    if athaniel_home and _looks_like_test_tempdir(athaniel_home):
        athaniel_home = ""
    if athaniel_home:
        env["ATHANIEL_HOME"] = athaniel_home
    # PYTHONPATH passes through so a worktree-launched athaniel finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["ATHANIEL_QUIET"] = "1"
    env["ATHANIEL_REDACT_SECRETS"] = env.get("ATHANIEL_REDACT_SECRETS", "true")
...
def migrate(
    athaniel_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_athaniel_tools: bool = True,
) -> MigrationReport:
    """Translate Athaniel mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        athaniel_config: full ~/.athaniel/config.yaml dict
        ...
        expose_athaniel_tools: when True (default), register Athaniel' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Athaniel for tools
    ...
    athaniel_servers = (athaniel_config or {}).get("mcp_servers") or {}
    if not isinstance(athaniel_servers, dict):
        report.errors.append(
            "mcp_servers in Athaniel config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in athaniel_servers.items():
    ...
    # Inject Athaniel' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Athaniel for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/athaniel_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_athaniel_tools:
        translated["athaniel-tools"] = _build_athaniel_tools_mcp_entry()
        if "athaniel-tools" not in report.migrated:
            report.migrated.append("athaniel-tools")
