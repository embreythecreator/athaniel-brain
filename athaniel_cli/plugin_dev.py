"""Runtime-backed validation behind ``athaniel plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary ATHANIEL_HOME
    ...
    try:
        ...
        # removes it instead of stranding a athaniel-plugin-doctor-* directory
        # until interpreter GC (or never, on a hard exit).
        temporary_home = stack.enter_context(
            tempfile.TemporaryDirectory(prefix="athaniel-plugin-doctor-")
        )
        ...
        stack.enter_context(
            patch.dict(
                os.environ,
                {
                    ...
                    "ATHANIEL_BUNDLED_PLUGINS": str(bundled),
                    "ATHANIEL_ENABLE_PROJECT_PLUGINS": "0",
                },
                clear=False,
            )
        )
    ...
    modules_before = {
        ...
        if name == "athaniel_plugins" or name.startswith("athaniel_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Athaniel discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "athaniel_plugins" or name.startswith("athaniel_plugins."))
            ):
...
def _holds_plugin(path: Path) -> bool:
    ...
    confusing error: ``athaniel plugins doctor`` with no argument defaults to
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        user_root = get_athaniel_home() / "plugins"
        ...
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".athaniel" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from athaniel_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Athaniel supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Athaniel never auto-installs plugin dependencies; "
    ...
    if schema:
        from athaniel_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Athaniel' real scanner and registration path."""
    try:
