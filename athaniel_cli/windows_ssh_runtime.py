...
def _root() -> Path:
    ...
    # helper's token, including custom ATHANIEL_HOME layouts.
    return get_default_athaniel_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, athaniel_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(athaniel_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the athaniel exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs athaniel_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "athaniel_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "athaniel_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, athaniel_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, athaniel_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves athaniel_cli's own location and prepends its parent, because the
    ...
    # the athaniel_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the athaniel_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["athanielPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Athaniel path must be absolute")
    athaniel_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(athaniel_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Athaniel Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('athaniel_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_athaniel(athaniel_path: str) -> dict[str, Any]:
    path = os.path.abspath(athaniel_path)
    if not os.path.isabs(athaniel_path) or not os.path.isfile(path):
        raise ValueError("Athaniel path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "athanielHome": str(get_default_athaniel_root()), "python": sys.executable}
    ...
    if operation == "inspect" and len(argv) == 2:
        return inspect_athaniel(argv[1])
