"""Image-authored deployment provenance for immutable Athaniel runtimes.

The published image bakes ``/etc/athaniel/image-provenance.json`` outside both
``$ATHANIEL_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/athaniel/image-provenance.json")
