"""Profile distributions — shareable, packaged Athaniel profiles via git.

A distribution is a Athaniel profile published as a git repository (or
...
* ``athaniel profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``athaniel skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``athaniel profile``, not a parallel tree):

    athaniel profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "athaniel_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".athaniel_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "athaniel-brain", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            athaniel_requires=str(data.get("athaniel_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.athaniel_requires:
            out["athaniel_requires"] = self.athaniel_requires
        if self.author:
...
def check_athaniel_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Athaniel {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``athaniel profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Athaniel profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Athaniel distribution."
        )

    # Version check up-front so we fail fast
    check_athaniel_requires(manifest.athaniel_requires, athaniel_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.athaniel).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="athaniel_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `athaniel profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`athaniel profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="athaniel_dist_update_") as tmp:
        plan = plan_install(
