"""
Doctor command for athaniel CLI.

Diagnoses issues with Athaniel Brain setup.
"""
...
from athaniel_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
ATHANIEL_HOME = get_athaniel_home()
_DHH = display_athaniel_home()  # user-facing display path (e.g. ~/.athaniel or ~/.athaniel/profiles/coder)

# Load environment variables from ~/.athaniel/.env so API key checks work
_env_path = get_env_path()
load_athaniel_dotenv(athaniel_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from athaniel_cli.colors import Colors, color
from athaniel_cli.models import _ATHANIEL_USER_AGENT
from athaniel_cli.vercel_auth import describe_vercel_auth
from athaniel_constants import OPENROUTER_MODELS_URL
...
from athaniel_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Athaniel containers"
...
def _athaniel_database_paths(athaniel_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Athaniel-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from athaniel_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, athaniel_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((athaniel_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(athaniel_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``athaniel_cli.sqlite_safe_read``).
...
def _format_db_size(db_path: Path) -> str:
    ...
    from athaniel_cli.backup import _format_size
...
def _report_database_journal_modes(
    athaniel_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from athaniel_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = athaniel_home if athaniel_home is not None else ATHANIEL_HOME
    try:
        databases = _athaniel_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Athaniel databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.athaniel/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("ATHANIEL_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ATHANIEL_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from athaniel_cli.auth import get_minimax_oauth_auth_status
...
from athaniel_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'athaniel sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'athaniel sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. ATHANIEL_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ATHANIEL_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ATHANIEL_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Athaniel Relay plugin."""
    from athaniel_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches athaniel_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``athaniel --version`` reporting a stale version while
    ...
    """
    try:
        from athaniel_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != athaniel_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'athaniel update', or set "
            "athaniel_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-athaniel and dashboard static services are up
    ...
    for static in ("main-athaniel", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `athaniel profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `athaniel doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from athaniel_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ATHANIEL_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ATHANIEL_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ATHANIEL_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.embreythecreator.athaniel"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `athaniel update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (athaniel desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `athaniel desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.embreythecreator.athaniel` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Athaniel once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Athaniel.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Athaniel.app``
    is deliberately not probed: it is the separately-signed Athaniel-Setup
    launcher (``com.embreythecreator.athaniel.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Athaniel.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from athaniel_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Athaniel will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Athaniel.app if you use Desktop), "
        "and restart them once. With Athaniel' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `athaniel`.
    os.environ.setdefault("ATHANIEL_INTERACTIVE", "1")

    # Handle `athaniel doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from athaniel_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.athaniel/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Athaniel Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`athaniel doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from athaniel_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Athaniel already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and athaniel_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.athaniel/.env (primary location for user config)
    env_path = ATHANIEL_HOME / '.env'
    if env_path.exists():
        ...
        # matches athaniel_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'athaniel setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'athaniel setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'athaniel setup' to create one")
                issues.append("Run 'athaniel setup' to create .env")
    
    # Check ~/.athaniel/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ATHANIEL_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from athaniel_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from athaniel_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'athaniel config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from athaniel_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.athaniel/.env or run 'athaniel setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'athaniel setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'athaniel config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from athaniel_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from athaniel_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'athaniel setup' to migrate config")
                else:
                    issues.append("Run 'athaniel doctor --fix' or 'athaniel setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from athaniel_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'athaniel doctor --fix'")
        except Exception:
            pass

        # Detect stale ATHANIEL_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ATHANIEL_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from athaniel_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("ATHANIEL_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ATHANIEL_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `athaniel setup` run)",
                )
                if should_fix:
                    if remove_env_value("ATHANIEL_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ATHANIEL_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ATHANIEL_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ATHANIEL_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ATHANIEL_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'athaniel doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from athaniel_cli.config import load_env as _load_env_depr
            from athaniel_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Athaniel' own device-code flow — the Codex CLI is
    ...
    if athaniel_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        athaniel_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = athaniel_home / subdir_name
    ...
    soul_path = athaniel_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Athaniel a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Athaniel Brain Persona\n\n"
                "<!-- Edit this file to customize how Athaniel communicates. -->\n\n"
                "You are Athaniel, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = athaniel_home / "memories"
    ...
    state_db_path = athaniel_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from athaniel_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'athaniel doctor --fix' "
                        "(or 'athaniel sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from athaniel_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'athaniel doctor --fix' "
                        "(or 'athaniel sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Athaniel
        ...
        try:
            from athaniel_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'athaniel sessions optimize-storage' "
    ...
    wal_path = athaniel_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'athaniel doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "athaniel"
        ...
        _cmd_link = _cmd_link_dir / "athaniel"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(athaniel not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/athaniel → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/athaniel points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/athaniel → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/athaniel — run 'athaniel doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/athaniel exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/athaniel not found",
                    "(athaniel command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/athaniel → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/athaniel symlink — run 'athaniel doctor --fix'")
    ...
    try:
        from athaniel_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'athaniel-brain[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Athaniel-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `athaniel update` does, so a session's first browser call
            ...
            # `athaniel update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `athaniel doctor` invocation.
    ...
    try:
        ...
        if _using_lightpanda_engine():
            ...
            if not _lp_used:
                ...
                check_info(
                    "Fix: pick Lightpanda in `athaniel tools` → Browser Automation, "
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ATHANIEL_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Athaniel one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'athaniel config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _ATHANIEL_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``athaniel doctor``.
    ...
    # never the legitimate source for `athaniel doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'athaniel setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ATHANIEL_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: athaniel skills list)")
    ...
    try:
        from athaniel_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = ATHANIEL_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: athaniel memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: athaniel memory setup",
                    "No Honcho API key — run 'athaniel memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run athaniel memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: athaniel memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: athaniel memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from athaniel_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "athaniel -p" in content:
                            _m = _re.search(r"athaniel -p (\S+)", content)
    ...
    # `athaniel doctor --live` (real network calls; bounded + read-only).
    try:
        from athaniel_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'athaniel doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
