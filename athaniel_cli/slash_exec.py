...
``CommandDef.execute`` (athaniel_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/athaniel_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``athaniel_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_athaniel_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: athaniel bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from athaniel_cli.commands import gateway_help_lines
