...
Athaniel is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_ATHANIEL_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `athaniel
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Athaniel' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Athaniel shells out to git from many non-interactive contexts — MCP catalog
...
def _text_names_athaniel(text: str) -> bool:
    """True when *text* names Athaniel at a path-segment / token boundary.

    A bare ``"athaniel" in text`` substring test would also match unrelated
    processes whose paths merely contain the letters (``...\\sathaniela\\...``),
    ...
    that *starts with* ``athaniel`` (``athaniel``, ``athaniel.exe``, ``athaniel_cli``,
    ``athaniel-brain``, ``athaniel-runtime``) or the hidden-dir form
    ``.athaniel``/``.athaniel-runtime``.
    """
    for token in re.split(r"[\\/\s=,;\"']+", text.lower()):
        if token.startswith("athaniel") or token.startswith(".athaniel"):
            return True
    return False


def _process_command_is_athaniel(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Athaniel code."""
    try:
        import psutil
        ...
        return _text_names_athaniel(f"{command} {executable}")
    except Exception:
        return False
...
def pid_is_athaniel(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a Athaniel process. When
    ...
    if (
        ...
        return False
    try:
        return _process_command_is_athaniel(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # ``pid_is_athaniel`` guard is for BARE pids from state files or process
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``athaniel update`` on slow-WMI machines
