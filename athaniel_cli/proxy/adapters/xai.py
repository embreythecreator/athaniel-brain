...
# xAI's public API is OpenAI-compatible for the endpoints Athaniel commonly
# uses. The Responses endpoint is included because Athaniel' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Athaniel-managed OAuth credentials."""

    auth_hint = "athaniel auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`athaniel auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`athaniel auth reset xai-oauth` or re-authenticate with "
                    "`athaniel auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `athaniel auth add xai-oauth --type oauth`."
            )
