...
Reads the user's Nous OAuth state from ``~/.athaniel/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `athaniel auth add nous` first."
                )
    ...
    # to athaniel_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
