"""CLI handlers for the ``athaniel proxy`` subcommand."""
...
from athaniel_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "athaniel proxy requires aiohttp. Run `athaniel setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"athaniel auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Athaniel proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Athaniel proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: athaniel proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``athaniel proxy <subcommand>``."""
    ...
    print(
        "athaniel proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  athaniel proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
