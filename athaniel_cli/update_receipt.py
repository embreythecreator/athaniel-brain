...
1. **Update receipt** — a machine-readable JSON record of what one
   ``athaniel update`` run discovered, did, skipped (and why), written to
   ``<ATHANIEL_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``athaniel_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``athaniel update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``athaniel update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_athaniel_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``athaniel update`` has many early-termination paths (Windows
