...
Backs ``athaniel kanban export|import``, the matching ``/boards/{slug}/export``
...
ARCHIVE_FORMAT = "athaniel-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "athaniel_version": str(athaniel_version),
            "includes": {
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(
            "archive is not a Athaniel kanban board export (no manifest.json)"
        )
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Athaniel kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Athaniel "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Athaniel and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "athaniel_version": manifest.get("athaniel_version"),
        },
        "counts": counts,
