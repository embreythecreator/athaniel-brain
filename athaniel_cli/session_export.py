...
def _render_full_markdown(sessions: List[Dict[str, Any]]) -> str:
    ...
    if len(sessions) == 1:
        ...
        lines.append("# Athaniel sessions export")
        lines.append("")
        for session in sessions:
            lines.append(f"## Session: {_heading_text(_session_title_or_id(session))}")
            lines.extend(_session_metadata_lines(session))
...
Options:
  filename   optional output name/path (default: auto-named;
             CLI saves under ~/.athaniel/sessions/saved/)
...
def render_session_for_save(session: Dict[str, Any], fmt: str) -> str:
    ...
    if fmt == "html":
        from athaniel_cli.session_export_html import generate_html_export
...
def default_save_filename(session_id: str, fmt: str) -> str:
    ...
    return f"athaniel_session_{safe_id}.{fmt}"
