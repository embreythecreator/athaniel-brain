"""On-demand worktree + branch reclaim (``athaniel worktree`` / ``/worktree prune``).

The startup pruner in ``cli._prune_stale_worktrees`` is deliberately
conservative and silent: it runs before the banner on every ``athaniel -w``
...
- **Orphaned local branches** beyond the two auto-generated prefixes the
  startup pass deletes (``athaniel/athaniel-*``, ``pr-*``): salvage lanes, port
...
- untracked-only dirt is ARCHIVED to ``~/.athaniel/archive/worktree-prune/``
...
def _archive_untracked(tree: Path, untracked: List[str]) -> Optional[Path]:
    ...
    dest = (
        Path.home() / ".athaniel" / "archive" / "worktree-prune"
        / f"{tree.name}-{stamp}"
    )
    try:
...
def audit_worktrees(repo_root: str, *, with_sizes: bool = True) -> List[TreeRecord]:
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        ...
        if lock_state == "live":
            rec("keep", "in use by a running athaniel session")
            continue
...
def audit_branches(repo_root: str) -> List[BranchRecord]:
    ...
    Generalizes the startup pass's prefix list (``athaniel/athaniel-*``/``pr-*``)
    ...
    if workers > 1:
        try:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="athaniel-branch-gc"
            ) as pool:
