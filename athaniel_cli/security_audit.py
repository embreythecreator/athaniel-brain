"""On-demand supply-chain audit for Athaniel Brain installs.

Scans three surfaces a Athaniel user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Athaniel venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.athaniel/plugins``
...
def _discover_plugins(athaniel_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.athaniel/plugins``.
...
def _discover_components(
    *,
    ...
    home = athaniel_home or Path(get_athaniel_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `athaniel security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, athaniel_home=home
    )
