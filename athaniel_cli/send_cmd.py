"""CLI subcommand: ``athaniel send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"athaniel send: {file_path} is not a text file. --file reads the "
                ...
                f'  athaniel send --to telegram "MEDIA:{file_path}"\n'
                f'  athaniel send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  athaniel send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"athaniel send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"athaniel send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"athaniel send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"athaniel send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `athaniel send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"athaniel send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `athaniel gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.athaniel/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_athaniel_env() -> None:
    """Populate ``os.environ`` from ``~/.athaniel/.env`` AND bridge top-level
    ...
    startup that ``athaniel send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.athaniel/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.athaniel/config.yaml`` into
       ...
       via ``athaniel config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``athaniel send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.athaniel/.env and ~/.athaniel/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "athaniel send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  athaniel send --to telegram \"hello\"\n"
            "  athaniel send --to discord:#ops --file report.md\n"
            "  athaniel send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "athaniel send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `athaniel send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Athaniel "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.athaniel/.env + ~/.athaniel/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  athaniel send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `athaniel send --list telegram`.",
    )
