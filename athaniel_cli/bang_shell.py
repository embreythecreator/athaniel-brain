...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("ATHANIEL_GATEWAY_SESSION"):
        return False
    if env_var_enabled("ATHANIEL_CRON_SESSION"):
        return False
    if (os.getenv("ATHANIEL_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Athaniel-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Athaniel session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
