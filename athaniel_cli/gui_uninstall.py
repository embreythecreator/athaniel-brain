"""
Athaniel Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``athaniel desktop`` / ``athaniel gui``)
     Built inside the agent checkout under ``$ATHANIEL_HOME/athaniel-brain/``:
       ...
       - ``$ATHANIEL_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Athaniel.app`` or ``~/Applications/Athaniel.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Athaniel`` (NSIS per-user)
...
the app name ("Athaniel"), separate from ``$ATHANIEL_HOME``:
  - macOS:   ``~/Library/Application Support/Athaniel``
  - Windows: ``%APPDATA%\\Athaniel``
  - Linux:   ``$XDG_CONFIG_HOME/Athaniel`` (default ``~/.config/Athaniel``)
...
``athaniel uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(athaniel_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return athaniel_home / "athaniel-brain"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Athaniel"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Athaniel"
...
def source_built_gui_artifacts(athaniel_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``athaniel desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``athaniel-brain/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(athaniel_home)
    ...
    return [
        ...
        athaniel_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Athaniel" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Athaniel.app"),
            home / "Applications" / "Athaniel.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Athaniel).
            local_base / "Programs" / "Athaniel",
            # Older / alternate layout some builds used.
            local_base / "athaniel-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Athaniel")
    else:
        ...
        # ``uninstall_gui``.
        from athaniel_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `athaniel desktop` installs. Its icon is
            ...
            data_base / "applications" / "Athaniel.desktop",
            data_base / "icons" / "hicolor" / "scalable" / "apps" / "athaniel.png",
        ]
        ...
        for size in ("256x256", "512x512", "1024x1024"):
            paths.append(data_base / "icons" / "hicolor" / size / "apps" / "athaniel.png")
    return paths


def agent_is_installed(athaniel_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ATHANIEL_HOME.
    ...
    if (agent_root / "athaniel_cli").is_dir():
        return True
...
def gui_is_installed(athaniel_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(athaniel_home):
        if p.exists():
            return True
...
def gui_install_summary(athaniel_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = athaniel_home if athaniel_home is not None else get_athaniel_home()
...
def uninstall_gui(
    athaniel_home: "Path | None" = None, *, remove_userdata: bool = True
) -> "list[Path]":
    ...
    Never touches ``athaniel-brain/athaniel_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ATHANIEL_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Athaniel disappears from the launcher.
        try:
            from athaniel_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove athaniel' or "
            "'sudo dnf remove athaniel'). AppImage builds are a single file you "
