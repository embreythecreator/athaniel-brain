"""Pydantic request/response models for the Athaniel dashboard web server.

Extracted verbatim from ``athaniel_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from athaniel_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``athaniel model`` custom flow collects.
    api_key: str = ""
...
class MoaModelSlot(BaseModel):
    provider: str = ""
    model: str = ""
    # Model Council slot metadata — declared (like reasoning_effort below) so a
    # client round-tripping the GET payload doesn't have it stripped at parse
    # time and wiped on save.
    role: str = ""
    wing: str = ""
    name: str = ""
    purpose: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `athaniel import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`athaniel -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ATHANIEL_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under ATHANIEL_HOME.
    output: str = ""
