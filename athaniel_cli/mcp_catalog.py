...
entries via ``athaniel mcp catalog`` or the interactive ``athaniel mcp picker``,
and install them with ``athaniel mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into athaniel-brain. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``athaniel mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.athaniel/.env`` (the
...
from athaniel_constants import get_athaniel_home, get_optional_mcps_dir
...
from athaniel_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.athaniel/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Athaniel install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Athaniel understands version {_MANIFEST_VERSION})"
        )
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from athaniel_cli.mcp_config import _env_key_for_server
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Athaniel is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Athaniel
        understands. Update Athaniel to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_athaniel_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.athaniel/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.athaniel/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from athaniel_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from athaniel_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    *,
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``athaniel mcp configure <name>``.
    """
    print()
    ...
    if entry.tools.default_excluded and prior_selection is None:
        if prior_exclude is not None:
            ...
            print(color(
                ...
                "config.yaml or run "
                f"`athaniel mcp configure {entry.name}` to change.",
                Colors.GREEN,
            ))
            return
    ...
    if probed is None:
        ...
        if prior_selection is not None:
            ...
            print(color(
                ...
                f"Run `athaniel mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `athaniel mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `athaniel mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `athaniel mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `athaniel auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`athaniel auth {entry.auth.provider}` if you have not "
    ...
    from athaniel_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Athaniel session to load its tools.",
        Colors.GREEN,
    ))
