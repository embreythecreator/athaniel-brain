...
from athaniel_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import athaniel_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $ATHANIEL_HOME/node is
    ...
    "browser": lambda: (
        ...
        or _has_athaniel_brain_browser()
...
def _has_athaniel_brain_browser() -> bool:
    ...
    # install.sh installs globally into $ATHANIEL_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-AthanielHome", str(get_athaniel_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = athaniel_subprocess_env(inherit_credentials=False)
