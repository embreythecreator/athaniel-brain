...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      athaniel-brain = self'.packages.default;
      athanielVenv = athaniel-brain.athanielVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "athaniel-check";
                homeDirectory = "/home/athaniel-check";
                stateVersion = "24.11";
              };
            }
            { services.athaniel-brain = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.athaniel-brain = programs;
              services.athaniel-brain = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.athaniel-brain. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.athaniel-brain);
      ...
      configKeys = pkgs.runCommand "athaniel-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${athanielVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "athaniel-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "athaniel-build-package" { } ''
          echo "PASS: package built at ${athaniel-brain}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "athaniel-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ATHANIEL_TEST = "1";
                environmentFiles = [ "/run/secrets/athaniel-env" ];
                athanielHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "athaniel" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "athaniel" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.athanielAgentSetup.data;

            failures =
              lib.optional (names != [
                "athaniel-brain"
                "athaniel-backend"
              ]) "expected athaniel-brain + athaniel-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/athaniel gateway" (argvOf "athaniel-brain")
              ) "gateway process does not run `athaniel gateway`: ${argvOf "athaniel-brain"}"
              ++ lib.optional (
                !lib.hasInfix "bin/athaniel serve" (argvOf "athaniel-backend")
              ) "backend process does not run `athaniel serve`: ${argvOf "athaniel-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "athaniel-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/athaniel-check/.athaniel" (envOf n)) names
              ) "gateway and backend must share one ATHANIEL_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ATHANIEL_HOME or null != "/home/athaniel-check/.athaniel"
              ) "programs.athaniel-brain.enable must export ATHANIEL_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "athaniel-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/athaniel-check/.athaniel/SOUL.md" activation
              ) "athanielHomeFiles must install into ATHANIEL_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ATHANIEL_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ATHANIEL_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ATHANIEL_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "athaniel-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "athaniel-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/athaniel-check";
            ...
            cases = [
              ...
              {
                name = "athanielHomeFiles needs no directory";
                ok = accepts { athanielHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "athaniel-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ATHANIEL_HOME ───────────────
        # `programs.enable` exports ATHANIEL_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.athaniel
        # while the services use the ATHANIEL_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/athaniel-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                athanielHome = "/home/athaniel-check/.athaniel-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "athaniel-desktop") cfg.home.packages;
            ...
            # second Athaniel runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "athaniel-brain") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.athaniel-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.athaniel-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one athaniel-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ATHANIEL_HOME" != "/home/athaniel-check/.athaniel-work"
              ) "the launcher must carry ATHANIEL_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ATHANIEL_HOME")})"
              ++ lib.optional (
                setValue "ATHANIEL_MANAGED" != "home-manager"
              ) "the launcher must report ATHANIEL_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ATHANIEL_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ATHANIEL_DESKTOP_ATHANIEL" != "${lib.head agentPackages}/bin/athaniel"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ATHANIEL_DESKTOP_ATHANIEL")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ATHANIEL_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ATHANIEL_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ATHANIEL_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ATHANIEL_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ATHANIEL_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "athaniel-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ATHANIEL_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ATHANIEL_HOME" wrapper
              ) "the launcher must carry ATHANIEL_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ATHANIEL_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ATHANIEL_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? athaniel-backend || cfg.launchd.agents ? athaniel-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "athaniel-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `athaniel`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.athaniel-brain.enable = true;";
              }
              {
                value = false;
                expect = "programs.athaniel-brain.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "athaniel-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "athaniel-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "athaniel" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."athaniel-brain-setup".text;

            failures =
              lib.optional (names != [
                "athaniel-brain"
                "athaniel-backend"
              ]) "expected athaniel-brain + athaniel-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/athaniel gateway" (execOf "athaniel-brain")
              ) "gateway unit does not run `athaniel gateway`: ${execOf "athaniel-brain"}"
              ++ lib.optional (
                !lib.hasInfix "bin/athaniel dashboard" (execOf "athaniel-backend")
              ) "backend unit does not run `athaniel dashboard`: ${execOf "athaniel-backend"}"
              ++ lib.optional (
                units.athaniel-brain.environment.ATHANIEL_HOME != units.athaniel-backend.environment.ATHANIEL_HOME
              ) "gateway and backend must share one ATHANIEL_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/athaniel/.athaniel/SOUL.md" activation
              ) "athanielHomeFiles must install into ATHANIEL_HOME";
          ...
          in
          pkgs.runCommand "athaniel-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs athaniel, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.athaniel-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/athaniel serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec athaniel directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "athaniel-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "athaniel-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec athaniel, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/athaniel dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "athaniel-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ATHANIEL_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                ATHANIEL_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "athaniel-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ATHANIEL_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--athaniel-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "athaniel-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "athaniel-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${athaniel-brain}/bin/athaniel || (echo "FAIL: athaniel binary missing"; exit 1)
          test -x ${athaniel-brain}/bin/athaniel-brain || (echo "FAIL: athaniel-brain binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${athaniel-brain}/bin/athaniel --version 2>&1 | grep -qi "athaniel" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "athaniel-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in athaniel athaniel-brain athaniel-acp; do
            test -x ${athaniel-brain}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "athaniel-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking athaniel --help ==="
          ${athaniel-brain}/bin/athaniel --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${athaniel-brain}/bin/athaniel --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "athaniel-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${athaniel-brain}/share/athaniel-brain/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${athaniel-brain}/share/athaniel-brain/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ATHANIEL_BUNDLED_SKILLS" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ATHANIEL_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${athaniel-brain}/share/athaniel-brain/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${athaniel-brain}/share/athaniel-brain/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ATHANIEL_OPTIONAL_SKILLS" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ATHANIEL_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "athaniel-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${athaniel-brain}/share/athaniel-brain/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${athaniel-brain}/share/athaniel-brain/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ATHANIEL_BUNDLED_PLUGINS" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ATHANIEL_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "athaniel-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${athaniel-brain}/share/athaniel-brain/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${athaniel-brain}/share/athaniel-brain/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${athaniel-brain}/share/athaniel-brain/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ATHANIEL_BUNDLED_LOCALES" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ATHANIEL_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ATHANIEL_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ATHANIEL_BUNDLED_LOCALES=${athaniel-brain}/share/athaniel-brain/locales \
            ${athanielVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ATHANIEL_BUNDLED_LOCALES set"; exit 1)
        ...
        # ATHANIEL_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "athaniel-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${athaniel-brain}/share/athaniel-brain/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${athaniel-brain}/share/athaniel-brain/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ATHANIEL_OPTIONAL_MCPS" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ATHANIEL_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${athaniel-brain}/bin/athaniel mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: athaniel mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "athaniel-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${athaniel-brain}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${athaniel-brain}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ATHANIEL_TUI_DIR" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ATHANIEL_TUI_DIR set in wrapper"
        ...
        # Verify ATHANIEL_NODE is set in wrapper and points to Node 26+
        # (Athaniel pins its toolchain to Node 26 everywhere)
        athaniel-node = pkgs.runCommand "athaniel-node-version" { } ''
          set -e
          echo "=== Checking ATHANIEL_NODE in wrapper ==="
          grep -q "ATHANIEL_NODE" ${athaniel-brain}/bin/athaniel || \
            (echo "FAIL: ATHANIEL_NODE not set in wrapper"; exit 1)
          echo "PASS: ATHANIEL_NODE present in wrapper"

          ATHANIEL_NODE=$(sed -n "s/^export ATHANIEL_NODE='\(.*\)'/\1/p" ${athaniel-brain}/bin/athaniel)
          test -x "$ATHANIEL_NODE" || (echo "FAIL: ATHANIEL_NODE=$ATHANIEL_NODE not executable"; exit 1)
          echo "PASS: ATHANIEL_NODE executable at $ATHANIEL_NODE"

          NODE_MAJOR=$("$ATHANIEL_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Athaniel requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ATHANIEL_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ATHANIEL_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "athaniel-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ATHANIEL_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ATHANIEL_MANAGED guards ==="
          check_blocked "config set" ${athaniel-brain}/bin/athaniel config set model foo
          check_blocked "config edit" ${athaniel-brain}/bin/athaniel config edit
        ...
        extra-python-packages = let
          ...
          athanielWithExtra = athaniel-brain.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "athaniel-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${athanielWithExtra}/bin/athaniel || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${athanielWithExtra}/bin/athaniel || \
          ...
          if grep -q "PYTHONPATH" ${athaniel-brain}/bin/athaniel; then
        ...
        extra-dependency-groups = let
          athanielWithGroups = athaniel-brain.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "athaniel-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${athanielWithGroups}"
          echo "venv: ${athanielWithGroups.athanielVenv}"
        ...
        messaging-variant = pkgs.runCommand "athaniel-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.athanielVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "athaniel-config-roundtrip" {
          ...
          merge_and_load() {
            local athaniel_home="$1"
            export ATHANIEL_HOME="$athaniel_home"
            ${configMergeScript} ${nixSettings} "$athaniel_home/config.yaml"
            ${athanielVenv}/bin/python3 -c '
import json, sys
