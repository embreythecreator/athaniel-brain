# nix/desktop.nix — Athaniel Desktop (Electron) app build + wrapper
#
# `athanielAgent` is the fully-built `.#default` package — it ships the
# `athaniel` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ATHANIEL_DESKTOP_ATHANIEL` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Athaniel CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # ATHANIEL_HOME and ATHANIEL_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "athaniel-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "athaniel-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = athanielNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "athaniel-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe athanielNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/athaniel-desktop $out/bin
    cp -r ${renderer}/* $out/share/athaniel-desktop/
    ...
    substituteInPlace $out/share/athaniel-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/athaniel-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ATHANIEL_DESKTOP_ATHANIEL to the absolute path of the nix-built `athaniel`
    # binary so the desktop's resolver step 4 ("existing Athaniel CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/athaniel-desktop \
      --add-flags "$out/share/athaniel-desktop" \
      --set ATHANIEL_DESKTOP_ATHANIEL "${lib.getExe athanielAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/athaniel.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../athaniel_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/athaniel-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/athaniel.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/athaniel.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Athaniel Brain";
    homepage = "https://github.com/embreythecreator/athaniel-brain";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "athaniel-desktop";
  };
}
