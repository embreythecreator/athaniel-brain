...
#
# `services.athaniel-brain` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  athanielHome, programs.athaniel-brain (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "athaniel-config-attrs";
    description = "Athaniel YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $ATHANIEL_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Athaniel Brain";
      ...
      package = mkOption {
        ...
        description = "The athaniel-brain package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Athaniel configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `athaniel config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ATHANIEL_HOME/.env. Athaniel reads that file at each start, with
          load_athaniel_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."athaniel/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Athaniel refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Athaniel reads SOUL.md and memories/ from ATHANIEL_HOME, so put those
          files in `athanielHomeFiles`.
      ...
      athanielHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ATHANIEL_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Athaniel reads SOUL.md and the memory files from ATHANIEL_HOME and not
          from the working directory. Declare those files here, or Athaniel
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the athaniel plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Athaniel discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          athaniel_brain.plugins entry-point group. Each package must be built
          with the same Python interpreter as athaniel (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in athaniel-brain's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `athaniel gateway`.";
      };
      ...
      # ── The backend: `athaniel serve` or `athaniel dashboard` ──────────────
      # `athaniel serve` and `athaniel dashboard` are the same entry point,
      # athaniel_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Athaniel Desktop
      ...
      # controls an external gateway, with `athaniel gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Athaniel
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ATHANIEL_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Athaniel Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."athaniel/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `athaniel` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.athaniel-brain.installPackage was removed. Athaniel now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.athaniel-brain.enable = ${lib.boolToString (value != false)};  # the athaniel CLI, and ATHANIEL_HOME for your shells
        programs.athaniel-brain.desktop.enable = true;  # the desktop application

      `services.athaniel-brain` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "athaniel-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "athaniel-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "athaniel-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "athaniel-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "athaniel-brain: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ATHANIEL_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.athanielHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Athaniel make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${athanielHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Athaniel
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${athanielHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${athanielHome}/config.yaml
            ${run}chmod ${modes.config} ${athanielHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "athaniel-managed" managedSystem} ${athanielHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${athanielHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${athanielHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${athanielHome}/auth.json"
        else
          ''
            if [ ! -e ${athanielHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${athanielHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree athanielHome cfg.athanielHomeFiles}
      ...
      ${run}find ${athanielHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "athaniel-brain: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${athanielHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/athaniel"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps athaniel as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "athaniel-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "athaniel-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "athaniel-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ATHANIEL_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ATHANIEL_DASHBOARD_SESSION_TOKEN

          if [ -z "$ATHANIEL_DASHBOARD_SESSION_TOKEN" ]; then
            echo "athaniel-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "athaniel-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "athaniel-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "athaniel-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "athaniel-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "athaniel-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "athaniel-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Athaniel Brain web dashboard and desktop backend"
    else
      "Athaniel Brain backend for Athaniel Desktop";

  # The environment that each Athaniel process needs, from either module.
  #
  # managedSystem gives the value of ATHANIEL_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      ATHANIEL_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give Athaniel an identity and a memory, use
          ${optionPath}.athanielHomeFiles instead. Those files go to
          ATHANIEL_HOME. Athaniel reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ATHANIEL_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
