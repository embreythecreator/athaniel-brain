...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      athanielNpmLib = self'.packages.default.passthru.athanielNpmLib;
      ...
      athanielAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "athaniel" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../athaniel} $out/bin/athaniel
          '')
        ...
        shellHook = ''
          ${athanielAgentDevShellHook}
          ${athanielNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ATHANIEL_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Athaniel Brain dev shell in $ATHANIEL_PYTHON_SRC_ROOT"
          echo "Ready. Run 'athaniel' or 'sandbox athaniel' to start."
        '';
      };
    };
}
