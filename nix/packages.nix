# nix/packages.nix — Athaniel Brain package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./athaniel-brain.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.athanielTui;
        web = full.athanielWeb;
        desktop = full.athanielDesktop;

        update-npm-lockfile = full.athanielNpmLib.updateNpmLockfile;
      };
    };
}
