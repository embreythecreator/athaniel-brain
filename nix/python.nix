...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Athaniel derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            athaniel-brain = prev.athaniel-brain.overrideAttrs (_old: {
              ATHANIEL_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $ATHANIEL_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ATHANIEL_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        athaniel-brain = prev.athaniel-brain.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "athaniel-brain-env" {
    athaniel-brain = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "athaniel-brain-editable-env" {
    athaniel-brain = dependency-groups;
  };
}
