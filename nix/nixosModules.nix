# nix/nixosModules.nix — the NixOS module for athaniel-brain
#
...
#
# Container mode: athaniel runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ATHANIEL_HOME/.env
# and read by athaniel at startup — no container recreation needed for env changes.
#
# Tool resolution: the athaniel wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.athaniel-brain;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      athaniel-brain = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      athanielHome = "${cfg.stateDir}/.athaniel";
      ...
      # ATHANIEL_HOME via addToSystemPackages, so they can save settings through the
      ...
      # athaniel user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "athaniel-container-entrypoint" ''
        set -eu

        ATHANIEL_UID="''${ATHANIEL_UID:?ATHANIEL_UID must be set}"
        ATHANIEL_GID="''${ATHANIEL_GID:?ATHANIEL_GID must be set}"

        # ── Group: ensure a group with GID=$ATHANIEL_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$ATHANIEL_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="athaniel"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$ATHANIEL_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$ATHANIEL_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$ATHANIEL_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$ATHANIEL_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="athaniel"
          TARGET_HOME="/home/athaniel"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$ATHANIEL_UID" -g "$ATHANIEL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$ATHANIEL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$ATHANIEL_UID:$ATHANIEL_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure ATHANIEL_HOME is owned by the target user.
        ...
        if [ -n "''${ATHANIEL_HOME:-}" ] && [ -d "$ATHANIEL_HOME" ]; then
          find "$ATHANIEL_HOME" \! -user "$ATHANIEL_UID" -exec chown "$ATHANIEL_UID:$ATHANIEL_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/athaniel-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/athaniel-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/athaniel ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/athaniel
          chmod 0440 /etc/sudoers.d/athaniel
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$ATHANIEL_UID" --regid="$ATHANIEL_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $ATHANIEL_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "athaniel-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        athaniel_bin=${containerDataDir}/current-package/bin/athaniel
      '';

      # Default: /var/lib/athaniel/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the athaniel group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit athanielHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.athaniel-brain =
        common.sharedOptions {
          defaultPackage = athaniel-brain;
          defaultPackageText = lib.literalExpression "athaniel-brain.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/athaniel";
              description = "State directory. Contains .athaniel/ subdir (ATHANIEL_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the athaniel CLI to environment.systemPackages and export
                ATHANIEL_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.athaniel symlink to the service
                  stateDir. These users are automatically added to the athaniel group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.athaniel-brain.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the athaniel CLI to system PATH and export ATHANIEL_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.athaniel/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.ATHANIEL_HOME = athanielHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.athaniel-brain";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.athaniel-brain.workingDirectory;
                optionPath = "services.athaniel-brain";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.athaniel-brain: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the athaniel user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.athaniel-brain: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed athaniel
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure athaniel is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${athanielHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${athanielHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."athaniel-brain-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  mkdir -p ${athanielHome}
                  mkdir -p ${cfg.stateDir}/home
                  mkdir -p ${cfg.workingDirectory}
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${athanielHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${athanielHome} ${cfg.workingDirectory}
                  ...
                  find ${athanielHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${athanielHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${athanielHome}/$_subdir"
                    chmod 2770 "${athanielHome}/$_subdir"
                    find "${athanielHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg athanielHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${athanielHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${athanielHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${athanielHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.athaniel";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${athanielHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "athaniel-brain: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.athaniel -> stateDir/.athaniel for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.athaniel-brain = {
              description = "Athaniel Brain Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $ATHANIEL_HOME/.env by the activation script. load_athaniel_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.athaniel-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.athaniel-brain = {
              description = "Athaniel Brain Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  ATHANIEL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  ATHANIEL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env ATHANIEL_UID="$ATHANIEL_UID" \
                    --env ATHANIEL_GID="$ATHANIEL_GID" \
                    --env ATHANIEL_HOME=${containerDataDir}/.athaniel \
                    --env ATHANIEL_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/athaniel gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
