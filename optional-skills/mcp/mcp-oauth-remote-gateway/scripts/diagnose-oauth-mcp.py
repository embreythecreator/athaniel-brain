...
Usage:
  ...
  <server_name> matches the files in $ATHANIEL_HOME/mcp-tokens/<server>.json etc.
...
def _athaniel_home():
    # Prefer Athaniel' own resolver (profile-safe); fall back to env then ~/.athaniel.
    try:
        from athaniel_constants import get_athaniel_home
        return str(get_athaniel_home())
    except Exception:
        return os.environ.get("ATHANIEL_HOME") or os.path.expanduser("~/.athaniel")


def _tokens_dir():
    return os.path.join(_athaniel_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "athaniel-diag", "version": "1.0"}},
        },
        headers={
