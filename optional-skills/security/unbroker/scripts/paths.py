...
All per-subject data lives under PDD_DATA_DIR (default: $ATHANIEL_HOME/unbroker),
which is the same trust boundary Athaniel uses for .env and OAuth tokens.
"""
...
def athaniel_home() -> Path:
    return Path(os.environ.get("ATHANIEL_HOME") or (Path.home() / ".athaniel"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else athaniel_home() / "unbroker"
