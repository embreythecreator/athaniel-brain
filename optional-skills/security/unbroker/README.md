...
The design is **Athaniel-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
athaniel skills install official/security/unbroker
```

Then start a new Athaniel session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Athaniel session:
...
- **PII handling.** Dossiers live under the Athaniel home directory (`0600`, optionally
