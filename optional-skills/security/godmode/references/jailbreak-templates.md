...
## 5. GODMODE FAST — Athaniel 4 405B (Zero Refusal)

**Model:** `embreythecreator/athaniel-4-405b`
**Strategy:** Athaniel models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Athaniel Brain

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.athaniel/config.yaml`:
