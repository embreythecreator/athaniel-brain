...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ATHANIEL_HOME", os.path.expanduser("~/.athaniel")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ATHANIEL_HOME", Path.home() / ".athaniel")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Athaniel config paths
# ═══════════════════════════════════════════════════════════════════

ATHANIEL_HOME = Path(os.getenv("ATHANIEL_HOME", Path.home() / ".athaniel"))
CONFIG_PATH = ATHANIEL_HOME / "config.yaml"
PREFILL_PATH = ATHANIEL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Athaniel models — already uncensored, just needs clean prompt
    "athaniel": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "athaniel" in model_lower or "nous" in model_lower:
        return "athaniel"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Athaniel config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.athaniel/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Athaniel for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Athaniel for changes to take effect.")
