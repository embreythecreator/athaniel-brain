#!/usr/bin/env python3
"""Canvas LMS API CLI for Athaniel Brain.
...
def _check_config():
    ...
    if missing:
        athaniel_env = os.path.join(
            os.environ.get("ATHANIEL_HOME", os.path.expanduser("~/.athaniel")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {athaniel_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Athaniel Brain"
    )
