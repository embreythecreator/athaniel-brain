...
Maps each role archetype to the Athaniel skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public athaniel-brain
...
## Athaniel skills relevant to video production

### Visual / rendering skills (`athaniel-brain/skills/creative/`)
...
### Design / pre-production skills (`athaniel-brain/skills/creative/`)
...
### Audio / media skills (`athaniel-brain/skills/creative/` + `skills/media/`)
...
These are **not** Athaniel skills but external CLIs / APIs that profiles invoke.
...
## Built-in Athaniel tools for media review

These are native Athaniel tools — not invoked via terminal but through their own
...
key is present in `${ATHANIEL_HOME:-~/.athaniel}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Athaniel profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ATHANIEL_HOME:-~/.athaniel}/.env` → environment variable.
...
When a new Athaniel-public video skill ships:
