...
athaniel kanban watch --tenant <project-slug>

# Snapshot of the board
athaniel kanban list --tenant <project-slug>
athaniel kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
athaniel kanban stats --tenant <project-slug>

# Visual dashboard (browser)
athaniel dashboard

# Inspect a specific task (includes comments + events)
athaniel kanban show <task-id>

# Follow a single task's event stream
athaniel kanban tail <task-id>
```

Verify available subcommands with `athaniel kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `athaniel kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `athaniel kanban block <id>` then `athaniel kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `athaniel kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `athaniel kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `athaniel profile list` (profile exists?), `athaniel status` (gateway/dispatcher up?), and verify tenant. |
...
athaniel kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
athaniel kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(athaniel kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
athaniel kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
athaniel kanban block <task-id>
athaniel kanban archive <task-id>

# Diagnose what happened
athaniel kanban show <task-id>      # task body, comments, recent events
athaniel kanban tail <task-id>      # follow the live event stream
athaniel kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `athaniel kanban create` for the director
...
while true; do
    clear
    athaniel kanban list --tenant <slug>
    echo "---"
    athaniel kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `athaniel kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `athaniel profile list` and start
