...
the actual `setup.sh` that creates the project workspace, configures Athaniel
...
2. **Create profiles** — `athaniel profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.athaniel/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `athaniel kanban create` assigned to the director
...
```bash
athaniel profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.athaniel/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.athaniel/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Athaniel
...
Each profile gets a `SOUL.md` at `~/.athaniel/profiles/<name>/SOUL.md` that
...
```bash
athaniel kanban create "Direct production of <video title>" \
    --assignee director \
...
the Athaniel `.env` (`${ATHANIEL_HOME:-$HOME/.athaniel}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _athaniel_env="${ATHANIEL_HOME:-$HOME/.athaniel}/.env"
    if grep -q "^${var}=" "$_athaniel_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_athaniel_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_athaniel_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY athaniel ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY athaniel OPENROUTER_API_KEY || exit 1
# ...
```
