#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `athaniel kanban list` and
...
def athaniel_available() -> bool:
    return shutil.which("athaniel") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed athaniel CLI."""
    try:
        out = subprocess.run(
            ["athaniel", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `athaniel kanban list`
    out = subprocess.run(
        ["athaniel", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["athaniel", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not athaniel_available():
        print("ERROR: 'athaniel' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
