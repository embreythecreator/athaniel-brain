...
### 1. Start order: editor first, Athaniel second

Athaniel probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Athaniel session.
...
restart the editor, restart the Athaniel session.
...
server fails to bind (Output Log) or Athaniel' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.athaniel/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Athaniel

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Athaniel' connection
lives in `~/.athaniel/config.yaml` via `athaniel mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Athaniel — just don't
mistake it for the Athaniel setup step.
...
explicitly warns against overlapping calls. Athaniel executes same-turn tool
...
### 9. Timeouts: Athaniel gives up before Unreal does

Athaniel' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Athaniel runs elsewhere (SSH backend,
