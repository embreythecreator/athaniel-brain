...
ATHANIEL_HOME_DIR="${ATHANIEL_HOME:-$HOME/.athaniel}"
ATHANIEL_CFG="${ATHANIEL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Athaniel config has twozero_td MCP entry ──
if [[ ! -f "$ATHANIEL_CFG" ]]; then
    echo -e " ${FAIL} Athaniel config not found at ${ATHANIEL_CFG}"
    manual_steps+=("Create ${ATHANIEL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ATHANIEL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Athaniel config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Athaniel config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ATHANIEL_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ATHANIEL_CFG} manually"); }
    manual_steps+=("Restart Athaniel session to pick up config change")
fi
