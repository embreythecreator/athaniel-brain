...
## 5. Athaniel Configuration

### Config location

`$ATHANIEL_HOME/config.yaml` (defaults to `~/.athaniel/config.yaml` when `ATHANIEL_HOME` is unset)
...
Restart the Athaniel session for changes to take effect. The MCP connection is
