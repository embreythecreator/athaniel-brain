...
$ATHANIEL_OSINT_CACHE/icij/ (default: ~/.cache/athaniel-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("ATHANIEL_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "athaniel-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "athaniel-brain osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $ATHANIEL_OSINT_CACHE/icij or ~/.cache/athaniel-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
