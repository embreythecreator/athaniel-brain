...
Usage (via cron with --no-agent):
    ...
    athaniel cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ATHANIEL_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Athaniel-Watcher/1.0"})
