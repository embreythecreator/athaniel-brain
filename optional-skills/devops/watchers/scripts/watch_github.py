...
Usage (via cron with --no-agent):
    ...
    athaniel cron create athaniel-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ATHANIEL_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name athaniel-issues --repo embreythecreator/athaniel-brain --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Athaniel .env file
(``${ATHANIEL_HOME:-~/.athaniel}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Athaniel-Watcher/1.0",
    }
