...
is typically only in `~/.athaniel/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Athaniel,
...
`athaniel-custom-providers` skill, pitfall 2). Athaniel hits this because it forwards
