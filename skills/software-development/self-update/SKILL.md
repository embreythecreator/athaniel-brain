---
name: self-update
description: "Use when the operator asks Athaniel to update itself — fold a new upstream release into the Brain (hermes-agent) or the Face (space-agent). Explains how the absorb machinery actually works, drives the full loop, and STOPS for human approval at --commit, merge, and push. Maintainer / git-install only."
version: 1.0.0
...
|---|---|---|---|
| **Brain** (this body) | `~/Oblivion/athaniel-brain` | NousResearch/hermes-agent | `athaniel absorb` (packaged, `athaniel_cli/absorb/`) |
| **Face** (web + desktop) | `~/Oblivion/athaniel-face` | agent0ai/space-agent | `python3 scripts/absorb/absorb.py` (run from that repo) |
...
1. **Transform T** (`rename_map.py`): the upstream→Athaniel rename, encoded
   ...
   commit. Brain T: Hermes/Nous → Athaniel (+ attribution rule). Face T:
...
4. **Parity gates** (`parity_report.py`) — ALL must pass before commit:
   ...
   - **divergence manifest** (`divergence.py`): machine-checked invariants
     ...
     Settings overlay, `_athaniel_version`, "Embrey The Creator / The
...
athaniel absorb --check          # ✶ banner line also signals this at launch
athaniel absorb <tag>            # merge + gates + auto-verify
# conflicts? materialize into the sidecar worktree (<repo>-absorb/ —
# the live/running tree is NEVER touched), resolve there, re-gate:
athaniel absorb --continue
athaniel absorb --verify
athaniel absorb --status         # cold-resume: where am I?
# >>> STOP — show the operator the parity report + verify verdict <<<
athaniel absorb --commit         # only on explicit human go — finalizes AND ff-installs to main
```
...
- **desktop head**: `/Applications/Athaniel.app` is a frozen packaged
