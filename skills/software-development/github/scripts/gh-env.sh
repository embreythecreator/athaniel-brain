#!/usr/bin/env bash
# GitHub environment detection helper for Athaniel Brain skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _athaniel_env="${ATHANIEL_HOME:-$HOME/.athaniel}/.env"; [ -f "$_athaniel_env" ] && grep -q "^GITHUB_TOKEN=" "$_athaniel_env" 2>/dev/null; then
