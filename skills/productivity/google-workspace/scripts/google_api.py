#!/usr/bin/env python3
"""Google Workspace API CLI for Athaniel Brain.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Athaniel-facing JSON contract and falls back to the Python client
...
from _athaniel_home import get_athaniel_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ATHANIEL_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Athaniel Brain")
