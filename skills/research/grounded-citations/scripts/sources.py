...
Ledger path resolution (first wins):
  --ledger PATH
  $ATHANIEL_CITATION_LEDGER
  $ATHANIEL_HOME/cache/citations/ledger.json
"""
...
from _athaniel_home import get_athaniel_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ATHANIEL_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_athaniel_home() / "cache" / "citations" / "ledger.json"
