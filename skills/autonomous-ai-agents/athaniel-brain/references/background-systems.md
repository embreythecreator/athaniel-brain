...
the `cronjob` tool, the `athaniel cron` CLI (`list`, `add`, `edit`,
...
User docs: https://athaniel-brain.nousresearch.com/docs/user-guide/features/cron
...
- **CLI:** `athaniel curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `athaniel curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.athaniel/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://athaniel-brain.nousresearch.com/docs/user-guide/features/curator

### Kanban (multi-agent work queue)

Durable SQLite board for multi-profile / multi-worker collaboration.
Users drive it via `athaniel kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `ATHANIEL_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `ATHANIEL_KANBAN_BOARD` pinned in env); tenant is a soft namespace
  within a board for workspace-path + memory-key isolation.

User docs: https://athaniel-brain.nousresearch.com/docs/user-guide/features/kanban
