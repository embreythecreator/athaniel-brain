...
Set via `athaniel model` (picker) or `athaniel setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://athaniel-brain.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `athaniel auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `athaniel auth add openai-codex` |
| qwen-oauth | OAuth | `athaniel auth add qwen-oauth` |
| minimax-oauth | OAuth | `athaniel auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`athaniel auth`).
Fallback chain when the primary fails: `athaniel fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`athaniel_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   athaniel config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
