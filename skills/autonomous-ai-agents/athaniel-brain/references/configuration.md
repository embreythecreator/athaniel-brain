...
Edit with `athaniel config edit` or `athaniel config set section.key value`.
Full reference: https://athaniel-brain.nousresearch.com/docs/user-guide/configuration
...
`athaniel config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `athaniel tools` (interactive) or `athaniel tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ATHANIEL_CORE_TOOLS` is the default bundle most platforms inherit).
