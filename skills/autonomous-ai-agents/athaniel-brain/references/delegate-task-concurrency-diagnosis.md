...
**three** code paths in Athaniel that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Athaniel — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Athaniel
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Athaniel is
   refusing my value."
...
athaniel config get delegation.max_concurrent_children

# 2. Did Athaniel' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.athaniel/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with athaniel on path)
...
appears, **the cap is the model**, not Athaniel.
