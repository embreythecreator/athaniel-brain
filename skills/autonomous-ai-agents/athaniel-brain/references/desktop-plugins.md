...
Write plugins for the Athaniel desktop app: statusbar items, layout panes,
...
system (`~/.athaniel/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ATHANIEL_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ATHANIEL_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Athaniel desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ATHANIEL_HOME/desktop-plugins/` (usually
  `~/.athaniel/desktop-plugins/`).

## How to Run

1. Create `$ATHANIEL_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.athaniel/...` by default, or `~/.athaniel/profiles/<profile>/...` under a
...
The ONLY import surface is `@athaniel/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Athaniel
  ...
  (`athaniel://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.athaniel/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@athaniel/plugin-sdk`, `react`, and
