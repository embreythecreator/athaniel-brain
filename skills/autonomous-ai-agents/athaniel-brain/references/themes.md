# Themes / Skins — Author a Athaniel Color Theme

Author a Athaniel **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`athaniel_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.athaniel/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Athaniel itself.
...
- Write access to the Athaniel home dir — `~/.athaniel` by default, or `$ATHANIEL_HOME`
  / the active profile's dir. Skins live in `<athaniel-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `athaniel config set`).
...
3. `write_file` it to `<athaniel-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `athaniel config set display.skin default` (or they can `/skin default`).
...
```
athaniel skin set <key> <hex>      # e.g. athaniel skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `athaniel skin set background "#08201f"` changes
only the background; `athaniel skin use <name>` / `athaniel skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.athaniel`** when a profile is active — resolve the real home
  from `$ATHANIEL_HOME` first, falling back to `~/.athaniel`.
...
- **Never hand-edit `config.yaml` to activate.** Use `athaniel config set
...
- **You apply it, not the user.** `athaniel config set display.skin <name>` is
...
- `read_file` the written `<athaniel-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `athaniel config get display.skin` and confirm it reports `<name>`.
