...
## Layer 1 — Is this thing a Athaniel plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Athaniel process**. Its LLM calls go through Athaniel's already-configured provider. | Already uses Portal if user's Athaniel is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Athaniel. That LLM call is already authenticated
via Athaniel's provider config. The plugin's own server auth (e.g.
...
What Athaniel Brain has that *feels* like OAuth — `athaniel login --provider nous`
opening a browser, user signs in, token lands in `~/.athaniel/auth.json` — is a
**Athaniel-specific browser flow**. Under the hood it produces a credential
Athaniel uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Athaniel's existing Portal credential out of `~/.athaniel/auth.json`.
...
rides on the credential Athaniel already holds.

Where this could live in Athaniel:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Athaniel itself.
...
1. First decide: Athaniel plugin (runs inside Athaniel) or separate app? If plugin,
   it already uses Portal via Athaniel's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Athaniel agent itself") is fully supported. Athaniel treats agent
...
pitch in the Tool Gateway announcement is specifically about Athaniel Brain's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Athaniel is configured to use Portal as its provider. It is
