...
Author widget apps for the Athaniel TUI (`athaniel --tui`): glanceable ambient
...
- The TUI must be in use (`athaniel --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.athaniel/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.athaniel/logs/tui_gateway_crash.log` if a widget never appears.
