...
For occasional contributors and PR authors. Full developer docs: https://athaniel-brain.nousresearch.com/docs/developer-guide/
...
Config: `~/.athaniel/config.yaml` (settings), `~/.athaniel/.env` (API keys) — both under `$ATHANIEL_HOME` when it is set.
...
`_ATHANIEL_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_athaniel_home()` for paths,
never hardcode `~/.athaniel`. For custom/local-only tools, write a plugin in
`~/.athaniel/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `athaniel_cli/commands.py`
...
- Tests auto-redirect `ATHANIEL_HOME` to temp dirs — never touch real `~/.athaniel/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/athaniel_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_athaniel_home()` from `athaniel_constants` for all paths (profile-safe)
