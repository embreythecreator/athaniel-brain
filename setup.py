...
Athaniel Brain (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ATHANIEL_NIX_BUILD=1`` on the
Athaniel package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ATHANIEL_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for athaniel-brain is not supported.\n"
    "Athaniel is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://athaniel-brain.nousresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Athaniel Nix derivation sets ATHANIEL_NIX_BUILD=1. If it does, file a bug."
)
