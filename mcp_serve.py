"""
Athaniel MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Athaniel-specific extra)

Usage:
    athaniel mcp serve
    athaniel mcp serve --verbose
...
logger = logging.getLogger("athaniel.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ATHANIEL_HOME."""
    try:
        from athaniel_constants import get_athaniel_home
        return get_athaniel_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ATHANIEL_HOME", Path.home() / ".athaniel")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from athaniel_constants import get_athaniel_home
        directory_file = get_athaniel_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ATHANIEL_HOME", Path.home() / ".athaniel")
...
class EventBridge:
    ...
    This is the Athaniel equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from athaniel_constants import get_athaniel_home
            db_file = get_athaniel_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ATHANIEL_HOME", Path.home() / ".athaniel")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Athaniel MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "athaniel",
        instructions=(
            "Athaniel Brain messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Athaniel MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
