...
def _is_dispatcher_owned_worker() -> bool:
    """False when ATHANIEL_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_athaniel_home() under the active profile.
...
#   - cli.py, athaniel_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("ATHANIEL_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by ATHANIEL_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like athaniel-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("athaniel-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (athaniel-*) include _ATHANIEL_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("athaniel-")
    ...
    # window). Core Athaniel tools (toolsets._ATHANIEL_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from athaniel_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to athaniel-brain's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from athaniel_cli.lifecycle import has_hook, invoke_hook
