"""Centralized logging setup for Athaniel Brain.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.athaniel/logs/`` (profile-aware via ``get_athaniel_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``athaniel_cli.web_server``,
    ``athaniel_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Athaniel (TUI, gateway, ``hy_memory`` server, MCP
...
from athaniel_constants import get_config_path, get_athaniel_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_athaniel_session_injector", False):
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.athaniel_home = str(get_athaniel_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.athaniel_home = ""  # type: ignore[attr-defined]
        return record

    _session_record_factory._athaniel_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``athaniel logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``athaniel logs --component gateway``.
    "gateway": ("gateway", "athaniel_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("athaniel_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "athaniel_cli.web_server",
        "athaniel_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Athaniel logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    athaniel_home
        Override for the Athaniel home directory.  Falls back to
        ``get_athaniel_home()`` (profile-aware).
    log_level
    ...
    home = athaniel_home or get_athaniel_home()
    from athaniel_constants import mkdir_under_athaniel_home
    log_dir = mkdir_under_athaniel_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_athaniel_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._athaniel_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the athaniel group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from athaniel_cli.config import is_managed
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Athaniel home.
    ...
    def __init__(
        ...
        *,
        ...
        self._athaniel_routed_log_path = Path(self.baseFilename)
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "athaniel_home", "")
        try:
    ...
    def _handler_for_home(self, home: Path) -> _ManagedRotatingFileHandler:
        with self._profile_handlers_lock:
            ...
            mkdir_under_athaniel_home(path.parent)
...
# the module header): when several Athaniel processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._athaniel_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="athaniel-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_athaniel_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_athaniel_routed_log_path", None) == resolved:
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse athaniel_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when athaniel_cli.config isn't importable (bare
        # athaniel_logging consumers).
        try:
            from athaniel_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
