"""Runtime smoke test for Docker $ATHANIEL_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the athaniel user after container boot.
...
registered later runs its log service as the dropped athaniel user and
...
def test_logs_gateways_seeded_and_athaniel_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by athaniel after boot."""
    ...
    # Both must be owned by athaniel
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=athaniel" in r.stdout, (
        f"logs/ not owned by athaniel: {r.stdout}"
    )
    assert "gateways=athaniel" in r.stdout, (
        f"logs/gateways/ not owned by athaniel: {r.stdout}"
    )
