...
Override the image with ``ATHANIEL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ATHANIEL_TEST_IMAGE", "athaniel-brain-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ATHANIEL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ATHANIEL_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged athaniel user
# ---------------------------------------------------------------------------
#
# Background: every Athaniel runtime path inside the container drops to UID
# 10000 (the ``athaniel`` user) via ``s6-setuidgid athaniel``. ``docker exec``
...
# fails (PermissionError swallowed) for athaniel, so a test that ran as root
...
# helpers below run every probe under ``-u athaniel`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: athaniel).
