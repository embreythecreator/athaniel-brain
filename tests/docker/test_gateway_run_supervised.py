...
Before the s6 migration, ``docker run embreythecreator/athaniel-brain gateway
...
crash and a supervised dashboard alongside (when ``ATHANIEL_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``ATHANIEL_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``ATHANIEL_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``athaniel`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'athaniel.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'athaniel.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → athaniel gateway run) can take
    ...
    # (main-wrapper.sh → python → athaniel gateway run) can take well over
    ...
    # top), NOT the static main-athaniel service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-athaniel
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The ATHANIEL_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``athaniel gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``athaniel gateway
    ...
    # (main-wrapper.sh → python → athaniel gateway run) can take well
    ...
    # started either — so we'd see the CMD's `athaniel gateway run`
    ...
    # `athaniel gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*athaniel.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `athaniel gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-athaniel`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``ATHANIEL_DASHBOARD=1`` is set, ``docker run <image> gateway
