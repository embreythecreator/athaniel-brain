...
def test_dashboard_service_resets_home(
    ...
    container with ATHANIEL_DASHBOARD=1 and verify the dashboard process
    ...
    the process env.
    """
    start_container(built_image, container_name, "ATHANIEL_DASHBOARD=1", "ATHANIEL_DASHBOARD_HOST=127.0.0.1")

    # Check if the dashboard process is running and inspect its HOME.
    r = docker_exec_sh(
        container_name,
        # Find the dashboard process (athaniel dashboard) and read its HOME
        # from /proc/<pid>/environ. If not running, verify the run script
        # itself exports HOME=/opt/data by grepping the script source.
        'pid=$(pgrep -f "athaniel dashboard" | head -1); '
        'if [ -n "$pid" ]; then '
        '  tr "\\0" "\\n" < /proc/$pid/environ | grep "^HOME="; '
        'else '
        '  grep -q "export HOME=/opt/data" '
        '    /opt/athaniel/docker/s6-rc.d/dashboard/run && '
        '  echo "HOME=/opt/data"; '
        'fi',
        timeout=15,
    )
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    ATHANIEL_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "ATHANIEL_DASHBOARD=1", "ATHANIEL_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to athaniel:athaniel on every boot.
    ...
    # Verify files are now owned by athaniel.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "athaniel" in r.stdout, (
        f"expected athaniel-owned files after restart, got: {r.stdout!r} — "
