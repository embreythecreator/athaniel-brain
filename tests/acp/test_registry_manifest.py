"""Tests for ACP Registry metadata shipped with Athaniel."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "athaniel-brain"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Athaniel Brain"
    assert data["description"]
    assert data["repository"] == "https://github.com/embreythecreator/athaniel-brain"
    assert data["website"].startswith("https://athaniel-brain.embreythecreator.com/")
    assert data["authors"] == ["Athaniel"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"athaniel-brain[acp]=={data['version']}"
    assert uvx["args"] == ["athaniel-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"athaniel-brain[acp]=={_pyproject_version()}"
    )
