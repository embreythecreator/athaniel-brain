...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAthanielSessionId"] == "root1"
    assert prov["rootAthanielSessionId"] == "root1"
    assert prov["parentAthanielSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_athaniel_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAthanielSessionId"] == "old"
    assert prov["rootAthanielSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAthanielSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAthanielSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"athaniel"}
    assert "sessionProvenance" in meta["athaniel"]
    assert meta["athaniel"]["sessionProvenance"]["currentAthanielSessionId"] == "root1"
