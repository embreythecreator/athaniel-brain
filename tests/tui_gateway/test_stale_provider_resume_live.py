...
def live_home(monkeypatch):
    """A REAL isolated ATHANIEL_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="athaniel-live-staleprov-"))
    home = tmp / ".athaniel"
    ...
    # athaniel_constants caches the resolved home at first read — the env var
    ...
    import athaniel_cli.config as hconfig
    ...
    # conftest also re-pins athaniel_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(athaniel_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_athaniel_home", str(home), raising=False)
    yield home, server
