    ...
    with patch.dict(
        "sys.modules",
        {
            "athaniel_constants": MagicMock(
                get_athaniel_home=MagicMock(return_value="/tmp/athaniel_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'athaniel-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'athaniel-release' patched"
    }
