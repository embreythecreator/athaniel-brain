...
``ATHANIEL_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def athaniel_root(tmp_path, monkeypatch):
    """A temp ATHANIEL_HOME root with two named profiles: 'work' and 'other'.

    Pointing ATHANIEL_HOME at a dir outside ~/.athaniel makes it the profile ROOT
    (get_default_athaniel_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "athaniel_home"
...
def test_add_lands_in_named_profile_only(athaniel_root):
    root = athaniel_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(athaniel_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(athaniel_root):
    root = athaniel_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(athaniel_root):
    root = athaniel_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(athaniel_root):
    root = athaniel_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(athaniel_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(athaniel_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(athaniel_root):
    root = athaniel_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == ATHANIEL_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
