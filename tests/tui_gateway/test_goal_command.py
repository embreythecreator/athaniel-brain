...
def athaniel_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ATHANIEL_HOME.
...
@pytest.fixture()
def server(athaniel_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ATHANIEL_HOME. Sibling test
    ...
    # module-level ``_athaniel_home = get_athaniel_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_athaniel_home", athaniel_home)
...
def test_moa_bare_returns_usage(server, session, athaniel_home):
    _write_moa_config(athaniel_home, """
moa:
