...
locally, one reached over SSH, one behind a plain URL+token, or Athaniel Cloud —
and only the first two run with ``ATHANIEL_DESKTOP=1`` in their environment.
...
model "You are chatting inside the Athaniel desktop app".
...
class TestDesktopUiToolset:
    ...
    def test_stays_off_the_core_tool_list(self):
        ...
        from toolsets import _ATHANIEL_CORE_TOOLS

        assert GUI_TOOLS.isdisjoint(_ATHANIEL_CORE_TOOLS)
...
class TestSurfaceResolution:
    ...
    def test_desktop_env_alone_does_not_grant_them(self, no_desktop_env):
        ...
        The embedded terminal pane runs `athaniel --tui` against this same
        backend; env-keyed gating handed it GUI tools it cannot answer.
        """
        no_desktop_env.setenv("ATHANIEL_DESKTOP", "1")
        assert "desktop_ui" not in server._gui_surface_toolsets("tui")
...
class TestResolverPlumbing:
    ...
    def test_explicit_env_pin_still_wins(self, no_desktop_env):
        """ATHANIEL_TUI_TOOLSETS is an operator override; surface can't re-add."""
        no_desktop_env.setenv("ATHANIEL_TUI_TOOLSETS", "web,memory")
