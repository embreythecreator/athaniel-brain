...
``athaniel chat -q`` wires the interactive prompt_toolkit clarify callback
...
inter-agent wakeups all deliver work as ``athaniel chat -q``, so an agent that
...
def test_no_choices_returns_immediate_headless_answer():
    from athaniel_cli.cli_agent_setup_mixin import _single_query_clarify_callback
...
def test_callback_signature_matches_oneshot_contract():
    ...
    from athaniel_cli.cli_agent_setup_mixin import _single_query_clarify_callback
    from athaniel_cli import oneshot
...
def test_agent_construction_gates_clarify_callback_on_single_query_mode():
    ...
    from athaniel_cli import cli_agent_setup_mixin as mixin_mod
