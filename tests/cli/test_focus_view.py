...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = AthanielCLI.__new__(AthanielCLI)
        host.model = "opus"
        ...
        with patch.object(AthanielCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(AthanielCLI, "_is_session_yolo_active", return_value=False):
            text = AthanielCLI._build_status_bar_text(host, width=width)
...
class TestCommandRegistration:
    ...
    def test_verbose_cycle_releases_focus_view(self):
        ...
        with patch("cli.save_config_value", return_value=True), patch("cli._cprint"):
            AthanielCLI._toggle_verbose(host)
