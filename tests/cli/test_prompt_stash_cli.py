...
def _make_cli(**kwargs):
    """Create a AthanielCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports athaniel_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``AthanielCLI.run()``'s local ``KeyBindings`` instance, so they are not
