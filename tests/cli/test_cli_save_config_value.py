...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_athaniel_home(), so
        # point ATHANIEL_HOME at the temp dir (the _athaniel_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "athaniel_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in ATHANIEL_HOME/config.yaml
    ...
    install whose ATHANIEL_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read ATHANIEL_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh ATHANIEL_HOME with NO config.yaml (managed/desktop first launch).
        athaniel_home = tmp_path / ".athaniel"
        athaniel_home.mkdir()
        monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((athaniel_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
