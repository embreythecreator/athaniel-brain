...
SessionDB.session_gateway_runtime (athaniel_state.py) — the round trip that
makes `athaniel --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare AthanielCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.AthanielCLI)
