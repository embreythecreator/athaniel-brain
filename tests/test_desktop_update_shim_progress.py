...
def progress(tmp_path):
    ...
    status = tmp_path / "athaniel-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Athaniel to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `athaniel update`, and reports the stage that was on screen
...
n="$(cat "$ATHANIEL_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ATHANIEL_TEST_CALLS"
for f in "$TMPDIR"/athaniel-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ATHANIEL_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ATHANIEL_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "athaniel-brain"
    (install_root / "venv" / "bin").mkdir(parents=True)
    athaniel = install_root / "venv" / "bin" / "athaniel"
    athaniel.write_text(FAKE_ATHANIEL)
    athaniel.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ATHANIEL_TEST_CAPTURE": str(capture),
        "ATHANIEL_TEST_CALLS": str(calls),
        "ATHANIEL_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".athaniel-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`athaniel update` is the longest wait in the hand-off and the one the
