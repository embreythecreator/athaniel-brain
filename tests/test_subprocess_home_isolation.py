...
Athaniel state stays profile-scoped through ATHANIEL_HOME. Host subprocesses should
...
See: https://github.com/embreythecreator/athaniel-brain/issues/25114
See: https://github.com/embreythecreator/athaniel-brain/issues/36144
See: https://github.com/embreythecreator/athaniel-brain/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for athaniel_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(athaniel_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("ATHANIEL_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        athaniel_home = real_home / ".athaniel" / "profiles" / "coder"
        profile_home = athaniel_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_REAL_HOME", str(real_home))

        from athaniel_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".athaniel" / "profiles"
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ATHANIEL_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["ATHANIEL_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(athaniel_home / "home")
        assert result["ATHANIEL_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_athaniel_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from athaniel_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from athaniel_cli.profiles import create_profile
