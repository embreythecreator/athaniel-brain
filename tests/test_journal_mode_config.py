...
def _reset_configured_delete_override_warned_paths():
    ...
    athaniel_state._delete_overridden_warned_paths.clear()
    yield
    athaniel_state._delete_overridden_warned_paths.clear()
...
def test_resolve_journal_mode_uses_real_database_config(monkeypatch, tmp_path):
    from athaniel_state import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "DELETE")
    assert resolve_journal_mode() == "delete"


def test_new_nonsecret_athaniel_env_override_is_not_exposed(monkeypatch, tmp_path):
    from athaniel_state import resolve_journal_mode

    _configure_mode(monkeypatch, tmp_path, "wal")
    monkeypatch.setenv("ATHANIEL_JOURNAL_MODE", "delete")
...
def test_real_db_openers_honor_configured_delete(monkeypatch, tmp_path):
    ...
    from athaniel_cli import kanban_db, projects_db
    ...
    discord = DiscordRecoveryStore(athaniel_home=tmp_path)
