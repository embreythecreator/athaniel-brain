...
class TestSanitizeStructureNonAscii:
    def test_sanitizes_nested_dict_structure(self):
        payload = {
            "default_headers": {
                "X-Title": "Athaniel │ Agent",
                "User-Agent": "Athaniel/1.0 🤖",
            }
        }
        assert _sanitize_structure_non_ascii(payload) is True
        assert payload["default_headers"]["X-Title"] == "Athaniel  Agent"
        assert payload["default_headers"]["User-Agent"] == "Athaniel/1.0 "
