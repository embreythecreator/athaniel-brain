...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_athaniel_attribution(mock_openai):
    """Keyless requests still identify as Athaniel (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Athaniel Brain"
    assert str(headers.get("User-Agent", "")).startswith("AthanielAgent/")
