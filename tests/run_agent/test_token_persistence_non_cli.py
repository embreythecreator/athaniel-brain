...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    athaniel_state = ModuleType("athaniel_state")
    athaniel_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "athaniel_state", athaniel_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Athaniel", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Athaniel"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Athaniel", "detail": "full"}),
        ),
    )
