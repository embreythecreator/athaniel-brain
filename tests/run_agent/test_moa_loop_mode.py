...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge athaniel system prompt"},
    ...
    assert all("huge athaniel system prompt" not in m["content"] for m in view)
...
def test_slot_runtime_anthropic_oauth_routes_through_provider_branch(monkeypatch):
    ...
    monkeypatch.setattr(
        "athaniel_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_council_direct_route_disabled_or_unlabelled_passes_through(monkeypatch):
    ...
    monkeypatch.delenv("ATHANIEL_MODEL_COUNCIL_ENABLED", raising=False)
    routed, role = moa_loop._council_route_references(
...
def test_reference_messages_flattens_cache_decorated_content():
    ...
    plain = [
        {"role": "system", "content": "athaniel system prompt"},
        {"role": "user", "content": "Can we get codex usage resets into athaniel?"},
    ]
