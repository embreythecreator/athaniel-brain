...
closure the PR changed, against a real temp ``ATHANIEL_HOME``.
"""
...
def _fake_switch_result():
    ...
    from athaniel_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "athaniel_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "athaniel_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_athaniel_home", athaniel_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_athaniel_home() / config.yaml`` — point it here.
    monkeypatch.setattr("athaniel_constants.get_athaniel_home", lambda: athaniel_home)
    monkeypatch.setattr("athaniel_cli.config.get_athaniel_home", lambda: athaniel_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_athaniel_home", default_home)
    _stub_picker_dependencies(monkeypatch)
