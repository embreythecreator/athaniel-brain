...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="AthanielBot"),
    )
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("athaniel_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_athaniel_when_only_mentions(adapter):
    """If a message contains only mention syntax, the stripped content is
    empty — fall back to the 'Athaniel' default rather than ''."""
    thread = SimpleNamespace(id=999, name="Athaniel")
    message = SimpleNamespace(
...
async def test_rename_thread_edits_only_when_current_name_matches(adapter):
    ...
    thread.edit.assert_awaited_once_with(
        ...
        reason="Athaniel semantic session title",
    )
...
def test_register_skill_command_callback_dispatches_by_name(adapter):
    ...
    with patch(
        "athaniel_cli.commands.discord_skill_commands_by_category",
