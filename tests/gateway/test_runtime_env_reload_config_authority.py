...
ATHANIEL_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ATHANIEL_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``athaniel config get terminal.backend`` still says local.
    """
    athaniel_home = tmp_path / ".athaniel"
    athaniel_home.mkdir()
    (athaniel_home / "config.yaml").write_text(
    ...
    (athaniel_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
