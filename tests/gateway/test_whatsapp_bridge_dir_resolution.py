...
(/opt/athaniel/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ATHANIEL_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_athaniel_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ATHANIEL_HOME."""
    ...
    monkeypatch.setattr(
        "athaniel_constants.get_athaniel_home", lambda: athaniel_home
    )
    ...
    expected = athaniel_home / "scripts" / "whatsapp-bridge"
