...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        athaniel_home.mkdir()
        (athaniel_home / "config.yaml").write_text(content, encoding="utf-8")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        athaniel_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/embreythecreator/athaniel-brain/issues/23994:
    reply_to_mode 'off' was ignored when sending via Athaniel-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/embreythecreator/athaniel-brain/issues/87051:
    ...
    response in a different chat lane than the Athaniel topic the session runs
    in. The no-anchor branch must prefer the Athaniel topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_athaniel_topic_thread_id(self):
