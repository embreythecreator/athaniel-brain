...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "athaniel"
        assert adapter.channel == "#athaniel-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :athaniel: hello there")
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG athaniel :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        adapter = IRCAdapter(cfg)
        adapter._current_nick = "athaniel"
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :athaniel: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "athanielbot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 athanielbot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK athanielbot-cron") for line in sent_lines)
        assert any(line.startswith("USER athanielbot-cron 0 * :Athaniel Brain (cron)")
