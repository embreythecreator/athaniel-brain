"""Tests for athaniel-api-server toolset and API server tool availability."""
...
class TestAthanielApiServerToolset:
    """Tests for the athaniel-api-server toolset definition."""


    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("athaniel-api-server")
...
class TestApiServerPlatformConfig:
    ...
    def test_default_api_server_includes_terminal_toolset(self):
        ...
        athaniel-api-server composite, so the old all-tools subset test dropped
...
class TestApiServerAdapterToolset:
    ...
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            # No platform_toolsets override — should fall back to athaniel-api-server default
    ...
    def test_create_agent_respects_config_override(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            call_kwargs = mock_agent_cls.call_args
            toolsets = call_kwargs.kwargs.get("enabled_toolsets")
            # `plan` is deliberately non-configurable (tools_config.py) and is
