...
class TestConcurrencyCap:
    ...
    def test_resolve_reads_config_value(self):
        ...
        with patch("athaniel_cli.config.load_config", return_value=cfg):
...
def _limb_rail_path() -> Path:
    return Path(os.environ["ATHANIEL_HOME"]) / "logs" / "limb_rail.jsonl"
    ...
    async def test_health_detailed_returns_ok(self, adapter):
        ...
        with patch("gateway.status.read_runtime_status", return_value={
            ...
            async with TestClient(TestServer(app)) as cli:
                ...
                assert data["platform"] == "athaniel-brain"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_versioned_athaniel_model(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == _default_athaniel_model_name()
            assert data["data"][0]["owned_by"] == "athaniel"
    ...
    def test_resolve_model_name_default_profile(self):
        """Default profile falls back to the versioned Athaniel model name."""
        with patch("athaniel_cli.profiles.get_active_profile_name", return_value="default"):
            assert APIServerAdapter._resolve_model_name("") == _default_athaniel_model_name()
    ...
    async def test_model_options_returns_shared_inventory(self, adapter, monkeypatch):
        ...
        from athaniel_cli import inventory
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "athaniel.api_server.capabilities"
            assert data["platform"] == "athaniel-brain"
            assert data["model"] == _default_athaniel_model_name()
            ...
            assert data["features"]["session_continuity_header"] == "X-Athaniel-Session-Id"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "athaniel_cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "athaniel_cli.tools_config._get_platform_tools",
            return_value={"default"},
        ), patch(
            "athaniel_cli.tools_config.get_nous_subscription_features",
            return_value=feature_snapshot,
        ) as resolve_features, patch(
            "athaniel_cli.tools_config._toolset_has_keys",
            return_value=True,
        ) as has_keys, patch(
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: athaniel.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``athaniel.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: athaniel.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: athaniel.tool.progress":
                    continue
    ...
    @pytest.mark.asyncio
    async def test_stream_emits_activity_events(self, adapter):
        """Reasoning/status callbacks stream as ``event: athaniel.activity``.

        Reasoning text must only ever travel on the activity event — never
        inside any ``chat.completion.chunk`` ``delta.content`` (reasoning-leak
        incidents #8878/#9568).
        """
        import asyncio
        import json as _json
        ...
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                ...
                thinking_cb = kwargs.get("thinking_callback")
                status_cb = kwargs.get("status_callback")
                if reasoning_cb:
                    reasoning_cb("secret reasoning chunk")
                if thinking_cb:
                    thinking_cb("🤔 Thinking...")
                    thinking_cb("")  # clear must be forwarded too
                if status_cb:
                    status_cb("lifecycle", "compressing session")
                if cb:
                    await asyncio.sleep(0.05)
                    cb("The answer.")
                return (
            ...
            assert "event: athaniel.activity" in body
            assert '"type": "reasoning.delta"' in body
            assert '"type": "status.update"' in body
            assert "compressing session" in body

            # Collect activity payloads and verify the thinking clear came through.
            activity_payloads = []
            lines = body.splitlines()
            for i, line in enumerate(lines):
                if line.strip() != "event: athaniel.activity":
                    continue
                for follow in lines[i + 1: i + 4]:
                    if follow.startswith("data: "):
                        activity_payloads.append(_json.loads(follow[len("data: "):]))
                        break
            status_texts = [
                p.get("text") for p in activity_payloads if p.get("type") == "status.update"
            ]
            assert "" in status_texts, status_texts

            # Reasoning text must never leak into any delta.content chunk.
            for line in lines:
                if line.startswith("data: ") and line.strip() != "data: [DONE]":
                    try:
                        chunk = _json.loads(line[len("data: "):])
                    ...
                    if chunk.get("object") == "chat.completion.chunk":
                        for choice in chunk.get("choices", []):
                            ...
                            assert "secret reasoning chunk" not in content

    @pytest.mark.asyncio
    @pytest.mark.parametrize("tool_name,padding,limit", [
        ("terminal", 500, 300), ("patch", 500, 64_000),
        ("write_file", 500, 64_000), ("patch", 70_000, 64_000),
    ])
    async def test_stream_tool_complete_result_preview_is_redacted(self, adapter, tool_name, padding, limit):
        """``resultPreview`` on the completed tool event is truncated and
        passed through the secret redactor — credentials in tool results
        must never reach the SSE stream verbatim."""
        import asyncio
        ...
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                ...
                tc_cb = kwargs.get("tool_complete_callback")
                if ts_cb:
                    ts_cb("call_env_1", tool_name, {"command": "env"})
                if tc_cb:
                    tc_cb(
                        "call_env_1",
                        tool_name,
                        {"command": "env"},
                        json.dumps({"success": True, "diff": "--- a/file\n+++ b/file\n@@ -1 +1 @@\n-OPENAI_KEY=sk-secretsecret12345\n+" + "x" * padding + "\nDIFF_END"}),
                    )
                if cb:
                    await asyncio.sleep(0.05)
                    cb("done.")
            ...
            assert '"resultPreview":' in body
            # Redacted (sk-… matches the known-credential patterns) and truncated.
            assert "sk-secretsecret12345" not in body
            assert ("DIFF_END" in body) == (padding < limit)
            previews = []
            for line in body.splitlines():
                if line.startswith("data: ") and line != "data: [DONE]":
                    frame = json.loads(line[6:])
                    # Activity frames use the same field regardless of tool.
                    def collect(value):
                        if isinstance(value, dict):
                            if "resultPreview" in value:
                                previews.append(value["resultPreview"])
                    ...
                    collect(frame)
            assert previews
            assert all(len(preview) <= limit for preview in previews)
            if padding < limit:
                assert json.loads(previews[0])["diff"].endswith("DIFF_END")
    ...
    async def test_successful_completion(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["model"] == "athaniel-brain"
    ...
    async def test_face_session_key_adds_platform_policy_once(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Athaniel-Session-Key": "vessel:athaniel:face:user:v2",
        ...
        assert prompt.count("athaniel-face-operator-contract-v2") == 1
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Athaniel-Session-Id": "athaniel-face-onscreen-user-v2",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "athaniel-brain", "messages": [{"role": "user", "content": "Hello"}]},
                )

        assert resp.status == 200
        prompt = mock_run.call_args.kwargs["ephemeral_system_prompt"]
        assert prompt == build_face_policy()
        assert mock_run.call_args.kwargs["session_id"] == "athaniel-face-onscreen-user-v2"
    ...
    async def test_existing_face_contract_is_not_duplicated(self, auth_adapter):
        ...
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        legacy_contract = "Athaniel Face operator contract (athaniel-face-operator-contract-v2):\nlegacy copy"
        mock_db = MagicMock()
        mock_db.get_messages_as_conversation.return_value = []
        auth_adapter._session_db = mock_db
        app = _create_app(auth_adapter)
    ...
    async def test_action_result_turn_is_fenced_and_records_telemetry(self, adapter):
        ...
        payload = json.dumps([
            {
                ...
                "error": None,
                "refusal_reason": None,
                "face": {"app": "athaniel-void", "version": "1.2.3", "session": "face-session"},
            },
            {
                "v": 1,
                "action_id": "act_two",
                ...
                "result": "",
                "error": "boom",
                "face": {"app": "athaniel-void", "session": "face-session"},
            },
        ])
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Athaniel-Turn-Type": "Action-Result"},
                    json={
                        "model": "athaniel-brain",
                        "messages": [
            ...
            assert rows[0]["face_app"] == "athaniel-void"
            assert rows[0]["face_session"] == "face-session"
            assert rows[0]["face"] == {"app": "athaniel-void", "session": "face-session"}
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Athaniel-Turn-Type": "action-result"},
                    json={"model": "athaniel-brain", "messages": [{"role": "user", "content": payload}]},
                )
    ...
    async def test_action_result_unrecognized_header_value_is_ignored(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Athaniel-Turn-Type": "action-results"},
                    json={"model": "athaniel-brain", "messages": [{"role": "user", "content": payload}]},
                )
    ...
    async def test_result_error_fallback_is_redacted(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "athaniel-brain", "input": "Hello"},
                )
    ...
    async def test_stream_task_done_callback_enqueues_eos_for_responses(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with (
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "athaniel-brain", "input": "hi", "stream": True},
                )
                assert resp.status == 200
    ...
    async def test_get_stored_response(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "athaniel-brain", "input": "Hi"},
                )
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "athaniel-brain", "input": "What is 6*7?"},
                )
...
class TestChatCompletionsAgentIncomplete:
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-Athaniel-Error", "")
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["athaniel"]["failed"] is True
...
# ---------------------------------------------------------------------------
# X-Athaniel-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_traversal_session_id_header_rejected(self, auth_adapter):
        """Security (#5958): a path-traversal X-Athaniel-Session-Id must be
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={"X-Athaniel-Session-Id": bad, "Authorization": "Bearer sk-secret"},
                        json={"model": "athaniel-brain", "messages": [{"role": "user", "content": "hi"}]},
                    )
    ...
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-Athaniel-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Athaniel-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
...
# ---------------------------------------------------------------------------
# X-Athaniel-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Athaniel-Session-Key": "agent:main:webui:dm:user-7",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "athaniel-brain", "messages": [{"role": "user", "content": "hi"}]},
                )
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Athaniel-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Athaniel-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "athaniel-brain", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Athaniel-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Athaniel-Session-Key"
...
class TestStoredSessionModelFilter:
    ...
    "no stored model" — replaying "athaniel-brain" upstream 400s. Found live
    (Aug 2026): the first cross-gateway `athaniel peer dm` against a fresh
    api_server failed every turn with "athaniel-brain is not a valid model ID".
    """
...
class TestSessionDbOffEventLoop:
    ...
    async def test_create_session_without_model_does_not_persist_virtual_alias(self, auth_adapter):
        ...
        virtual model alias (self._model_name, e.g. "athaniel-brain") as if it
        ...
        "athaniel-brain" as an invalid model identifier on every turn.
        """
        app = _create_app(auth_adapter)
    ...
    async def test_create_session_with_explicit_virtual_alias_does_not_persist_it(self, auth_adapter):
        """Sending ``model: "athaniel-brain"`` explicitly (the virtual alias
    ...
    async def test_create_session_with_provider_prefixed_virtual_alias_does_not_persist_it(self, auth_adapter):
        ...
        "openrouter::athaniel-brain" never matched self._model_name and leaked
...
class TestApiKeyStartupGuardFailsClosed:
    ...
    def _blocking_auth_import():
        ...
        def _blocked(name, *args, **kwargs):
            if name == "athaniel_cli.auth":
                raise ImportError("simulated: athaniel_cli.auth unavailable")
...
class TestCreateAgentModelRecovery:
    def test_create_agent_defaults_to_provider_catalog_model_when_empty(self, monkeypatch):
        ...
        `athaniel auth add openai-codex` without `athaniel model`) —
        ...
        monkeypatch.setattr(
            "athaniel_cli.models.get_default_model_for_provider",
    ...
    def test_create_agent_does_not_cache_virtual_alias(self, monkeypatch):
        """Write-side guard: the advertised virtual model (``athaniel-brain``)
