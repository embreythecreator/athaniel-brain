...
* ``compression.codex_app_server_auto`` semantics hold: only ``athaniel``
  lets Athaniel' threshold start a compaction; ``native``/``off`` skip
...
class LiveCodexAgent:
    ...
    def __init__(self, mode="athaniel", session=None):
...
# ---------------------------------------------------------------------------
# Core no-op regression: athaniel mode + live thread => thread/compact runs
# ---------------------------------------------------------------------------

def test_athaniel_mode_compacts_live_thread_at_rpc_boundary(tmp_path):
    agent = LiveCodexAgent(mode="athaniel")
    key = "tg:123"
...
def test_athaniel_mode_without_cached_agent_skips_without_local_compression(tmp_path):
...
@pytest.mark.parametrize("mode", ["native", "athaniel", "off"])
...
def test_force_without_live_thread_does_not_run_local_compressor(mode):
    # The #73715 branch let force=True fall through to the local Athaniel
...
def test_timeout_records_persistent_cooldown(tmp_path):
    ...
    agent = LiveCodexAgent(mode="athaniel", session=HangingSession())
    key = "tg:123"
    gw, db = _gateway(tmp_path, key, agent)
