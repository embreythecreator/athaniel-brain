...
from athaniel_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(athaniel_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <athaniel_home>/plugins/<name> and opt it in."""
    plugin_dir = athaniel_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path / "athaniel_empty"))
