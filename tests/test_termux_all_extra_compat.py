...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"athaniel-brain[termux]"' in text
    assert '"athaniel-brain[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"athaniel-brain[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
