...
def athaniel_home(tmp_path, monkeypatch):
    """Point ATHANIEL_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_load_mappings_handles_corrupt_json(athaniel_home):
...
def test_verify_checksums_signature_skips_without_gpg(athaniel_home, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(athaniel_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(athaniel_home, monkeypatch):
...
def test_ca_key_created_with_0o600(athaniel_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(athaniel_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(athaniel_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(athaniel_home):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(athaniel_home):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(athaniel_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("athaniel-mgmt-")
...
def test_reload_proxy_refuses_when_not_running(athaniel_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(athaniel_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(athaniel_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=athaniel_home / "ca.crt",
        ca_key=athaniel_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (athaniel_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = athaniel_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("athaniel-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(athaniel_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_ATHANIEL_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_ATHANIEL_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(athaniel_home, monkeypatch):
...
def test_get_status_probes_configured_bind_host(athaniel_home, monkeypatch):
