...
Uses a throwaway ATHANIEL_HOME so it never touches ~/.athaniel. No real names.
"""
...
def _fresh_home():
    """Point ATHANIEL_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_athaniel_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ATHANIEL_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ATHANIEL_HOME is set.
...
# a different get_athaniel_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ATHANIEL_HOME (throwaway): {HOME}")
