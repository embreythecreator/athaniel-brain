...
These run without Docker: the shim's ATHANIEL_TINI_SHIM_TARGET /
ATHANIEL_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["ATHANIEL_TINI_SHIM_TARGET"] = str(init)
    env["ATHANIEL_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "ATHANIEL_TINI_SHIM_TARGET" in text
