"""Guard: Athaniel-owned subprocesses must not resolve managed runtimes by bare PATH.

Athaniel installs runtimes for itself — ``uv`` at ``$ATHANIEL_HOME/bin/uv``, Node at
``$ATHANIEL_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Athaniel's own
...
* when a system copy also exists, the one Athaniel does not own wins — which is
...
``iter_athaniel_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Athaniel provisions into ATHANIEL_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Athaniel-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("athaniel_cli/update_cmd.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # athaniel_brain-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Athaniel-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ATHANIEL_HOME/bin (uv) and $ATHANIEL_HOME/node are not on an "
        ...
        "  node/npm -> athaniel_constants.find_node_executable()\n"
        "  PATH env -> athaniel_constants.iter_athaniel_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(athaniel_constants, helper))


def test_managed_uv_helpers_exist():
    from athaniel_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
