...
Field incident: an interrupted `athaniel update` (pull done, restart never ran)
...
had no way to know the fix was one `athaniel gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "athaniel gateway restart" not in msg
