...
which is already bounded by ATHANIEL_CRON_TIMEOUT — the worker thread submitted
...
timeout (ATHANIEL_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        profile_token = set_athaniel_home_override(profile_home)
        try:
            with patch("cron.scheduler._athaniel_home", None), \
                 patch("cron.scheduler._resolve_origin", return_value=None), \
                 patch("athaniel_cli.env_loader.load_athaniel_dotenv"), \
                 patch("athaniel_cli.env_loader.reset_secret_source_cache"), \
                 patch("athaniel_state.SessionDB", side_effect=make_session_db), \
                 patch(
            ...
            reset_athaniel_home_override(profile_token)
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._athaniel_home", tmp_path), \
             ...
             patch("athaniel_cli.env_loader.load_athaniel_dotenv"), \
             patch("athaniel_cli.env_loader.reset_secret_source_cache"), \
             patch("athaniel_state.SessionDB"), \
             patch(
                 "athaniel_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed ATHANIEL_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors ATHANIEL_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("ATHANIEL_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "ATHANIEL_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ATHANIEL_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._athaniel_home", tmp_path), \
                 ...
                 patch("athaniel_cli.env_loader.load_athaniel_dotenv"), \
                 patch("athaniel_cli.env_loader.reset_secret_source_cache"), \
                 patch("athaniel_state.SessionDB", side_effect=_hanging_then_capture), \
                 patch(
                     "athaniel_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._athaniel_home", tmp_path), \
             ...
             patch("athaniel_cli.env_loader.load_athaniel_dotenv"), \
             patch("athaniel_cli.env_loader.reset_secret_source_cache"), \
             patch("athaniel_state.SessionDB") as mock_db_cls, \
             patch(
