...
def test_cancel_event_terminates_script_process_tree(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_get_athaniel_home", lambda: tmp_path)
...
def test_long_running_script_refreshes_owned_claim_in_profile_store(
    ...
    monkeypatch.setattr(jobs, "_athaniel_now", lambda: current_time[0])
    ...
    with (
        ...
        patch("athaniel_state.SessionDB", return_value=MagicMock()),
    ):
