...
def cron_store(tmp_path, monkeypatch):
    ...
    (athaniel_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))
    monkeypatch.setattr(J, "ATHANIEL_DIR", athaniel_home)
    monkeypatch.setattr(J, "CRON_DIR", athaniel_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", athaniel_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", athaniel_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_athaniel_now", lambda: now):
            due = J.get_due_jobs()
