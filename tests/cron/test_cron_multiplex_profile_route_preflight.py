...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_athaniel_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "athaniel_constants.get_default_athaniel_root", lambda: root
    )
    token = set_athaniel_home_override(str(grant_home))
...
class TestRoutedSatellitePreflight:
    ...
    def test_route_for_another_profile_does_not_rescue(self, tmp_path, monkeypatch):
        ...
        token = set_athaniel_home_override(str(other_home))
        try:
    ...
    def test_primary_home_itself_skips_route_lookup(self, multiplex_homes):
        ...
        token = set_athaniel_home_override(str(root))
        try:
            assert _delivery_platform_routed_from_primary_gateway("telegram") is False
        finally:
            reset_athaniel_home_override(token)
