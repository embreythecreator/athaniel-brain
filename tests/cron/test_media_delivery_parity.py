...
text-only on manual ``athaniel cron run <job-id>``. Same box, same token, same
...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import athaniel_cli.plugins as hp_boot
    ...
    import athaniel_cli.plugins as hp
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp ATHANIEL_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_bridge_helper_exists_and_applies_config(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(home))
        for var in (
            "ATHANIEL_MEDIA_DELIVERY_STRICT",
            "ATHANIEL_MEDIA_ALLOW_DIRS",
            "ATHANIEL_MEDIA_TRUST_RECENT_FILES",
        ):
        ...
        assert os.environ.get("ATHANIEL_MEDIA_DELIVERY_STRICT") == "1"
        assert str(allow_dir) in os.environ.get("ATHANIEL_MEDIA_ALLOW_DIRS", "")
        assert os.environ.get("ATHANIEL_MEDIA_TRUST_RECENT_FILES") == "0"
    ...
    def test_standalone_filter_honors_bridged_allowlist(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(home))
        for var in ("ATHANIEL_MEDIA_DELIVERY_STRICT", "ATHANIEL_MEDIA_ALLOW_DIRS"):
            monkeypatch.delenv(var, raising=False)
    ...
    def test_deliver_result_runs_bridge(self, monkeypatch, tmp_path, media_file):
        ...
        Realistic enterprise shape: ATHANIEL_MEDIA_DELIVERY_STRICT=1 arrives via
