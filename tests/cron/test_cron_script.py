...
- Path resolution (absolute, relative to ATHANIEL_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ATHANIEL_HOME."""
    athaniel_home = tmp_path / ".athaniel"
    athaniel_home.mkdir()
    (athaniel_home / "cron").mkdir()
    (athaniel_home / "cron" / "output").mkdir()
    (athaniel_home / "scripts").mkdir()
    monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ATHANIEL_DIR", athaniel_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", athaniel_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", athaniel_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", athaniel_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Athaniel provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _ATHANIEL_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_ATHANIEL_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.athaniel/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ATHANIEL_SESSION_PLATFORM") is None
        assert os.environ.get("ATHANIEL_SESSION_CHAT_ID") is None
        assert os.environ.get("ATHANIEL_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_CRON_SCRIPT_TIMEOUT", "2")
