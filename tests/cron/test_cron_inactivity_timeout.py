...
- Unlimited timeout (ATHANIEL_CRON_TIMEOUT=0)
- Backward compat: ATHANIEL_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """ATHANIEL_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """ATHANIEL_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("ATHANIEL_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_athaniel_time_importable(self):
        """athaniel_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _athaniel_now
        assert callable(_athaniel_now)
