...
profile's ATHANIEL_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.athaniel/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active ATHANIEL_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever ATHANIEL_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    ATHANIEL_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import athaniel_constants

    monkeypatch.setattr(
        athaniel_constants, "_get_platform_default_athaniel_home", lambda: root
    )
    monkeypatch.setenv("ATHANIEL_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile ATHANIEL_HOME (<root>/profiles/<name>), the cron store
    ...
    assert athaniel_constants.get_athaniel_home().resolve() == profile_home.resolve()
    assert athaniel_constants.get_default_athaniel_root().resolve() == root.resolve()

    # cron/jobs.py computes ATHANIEL_DIR from get_athaniel_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.ATHANIEL_DIR.resolve() == profile_home.resolve()
        assert (
