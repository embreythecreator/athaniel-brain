...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ATHANIEL_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ATHANIEL_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_athaniel_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ATHANIEL_HOME": "/h", "ATHANIEL_PROFILE": "p",
        "ATHANIEL_CONFIG": "/c.yaml", "ATHANIEL_ENV": "/e",
        "ATHANIEL_DELEGATED_CHILD_CONTEXT": "1",
        # other ATHANIEL_* → dropped (broad prefix removed)
        "ATHANIEL_BASE_URL": "https://x", "ATHANIEL_INTERACTIVE": "1",
        "ATHANIEL_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "ATHANIEL_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ATHANIEL_BASE_URL", "ATHANIEL_INTERACTIVE", "ATHANIEL_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ATHANIEL_* vars."""
    import logging
