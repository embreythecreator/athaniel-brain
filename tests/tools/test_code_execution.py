...
UDS socket lifecycle, athaniel_tools generation, timeout enforcement,
...
from tools.code_execution_tool import (
    ...
    execute_code,
    generate_athaniel_tools_module,
...
class TestAthanielToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_athaniel_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)


    def test_empty_list_generates_nothing(self):
        src = generate_athaniel_tools_module([])
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_athaniel_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"athaniel_rpc\")", src)
        self.assertNotIn('os.environ.get("ATHANIEL_RPC_DIR", "/tmp/athaniel_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_athaniel_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "athaniel_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "athaniel_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/athaniel_exec_", mkdir_cmd)
        self.assertIn("ATHANIEL_RPC_DIR=/data/data/com.termux/files/usr/tmp/athaniel_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/athaniel_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/athaniel_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """ATHANIEL_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"ATHANIEL_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import athaniel_constants; print(athaniel_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("athaniel_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from athaniel_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from athaniel_tools import json_parse
...
"""
    ...
    def test_retry_helper_all_fail(self):
        ...
        code = """
from athaniel_tools import retry
def always_fail():
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_athaniel_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("ATHANIEL_RPC_SOCKET", child_env)
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching ATHANIEL_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated athaniel_tools module reads ATHANIEL_RPC_TOKEN and sends it."""
        src = generate_athaniel_tools_module(["terminal"], transport="uds")
        self.assertIn("ATHANIEL_RPC_TOKEN", src)
