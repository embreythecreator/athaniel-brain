...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("athaniel_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_SESSION_KEY", session_key)
        monkeypatch.setenv("ATHANIEL_EXEC_ASK", "1")
        monkeypatch.delenv("ATHANIEL_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("athaniel-verify-", "athaniel-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "athaniel-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/athaniel-verify-example.py",
            "rm -f /tmp/athaniel-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../athaniel-verify-example.py",
            "rm -f /tmp/a/../../tmp/athaniel-verify-example.py",
            "rm -f /var/tmp/athaniel-verify-example.py",
            "rm -f /tmp/athaniel-verify-*",
            "rm -f /tmp/athaniel-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/athaniel-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/athaniel-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\athaniel-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\athaniel-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\athaniel-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ATHANIEL_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.athaniel/.env",
            "echo x | tee $ATHANIEL_HOME/.env",
            'echo x | tee "$ATHANIEL_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestAthanielConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.athaniel/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.athaniel/config.yaml",
            "echo '  mode: off' >> ~/.athaniel/config.yaml",
            "echo x | tee ~/.athaniel/config.yaml",
            "echo x | tee $ATHANIEL_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.athaniel/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Athaniel absolute config.yaml is
        # handled by the project patterns, not the Athaniel-home rule.
        for cmd in (
            "cat ~/.athaniel/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.athaniel/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Athaniel-home prefixes must fold to ~/ and
    ~/.athaniel/ in dangerous-command detection.
    ...
    SSH, and Athaniel config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_athaniel_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ATHANIEL_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("ATHANIEL_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.athaniel/athaniel-brain && source venv/bin/activate && python -m athaniel_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m athaniel_cli.main gateway run --replace &",
            "nohup python -m athaniel_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart athaniel-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("ATHANIEL_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("ATHANIEL_CRON_SESSION", raising=False)
        monkeypatch.setenv("ATHANIEL_GATEWAY_SESSION", "1")
        monkeypatch.setenv("ATHANIEL_SESSION_PLATFORM", "telegram")
    ...
    def test_cron_session_returns_false_regardless_of_platform(self, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_CRON_SESSION", "1")
        monkeypatch.setenv("ATHANIEL_SESSION_PLATFORM", "telegram")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("ATHANIEL_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("ATHANIEL_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ATHANIEL_INTERACTIVE", raising=False)
        monkeypatch.setenv("ATHANIEL_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("ATHANIEL_SESSION_KEY", "test-api-session")
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Athaniel security config via IFS.
            "sed${IFS}-i ~/.athaniel/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep athaniel) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "athaniel.*gateway")',
            "kill -9 `pgrep athaniel`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof athaniel)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof athaniel_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof athaniel`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Athaniel service
    label achieves the same effect as `athaniel gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_athaniel_label_detected(self):
        for cmd in (
            "launchctl stop ai.athaniel.gateway",
            "launchctl kickstart -k system/ai.athaniel.gateway",
            "launchctl bootout system/ai.athaniel.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.athaniel.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Athaniel labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a athaniel-gateway identifier, so quoted prose and non-gateway athaniel
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "athaniel"/"ai.athaniel" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ATHANIEL_GATEWAY_SESSION", "ATHANIEL_CRON_SESSION",
                      "ATHANIEL_YOLO_MODE",
                      "ATHANIEL_SESSION_KEY", "ATHANIEL_INTERACTIVE")
        }
        os.environ.pop("ATHANIEL_YOLO_MODE", None)
        os.environ.pop("ATHANIEL_INTERACTIVE", None)
        # ATHANIEL_CRON_SESSION takes priority over ATHANIEL_GATEWAY_SESSION in
        ...
        os.environ.pop("ATHANIEL_CRON_SESSION", None)
        os.environ["ATHANIEL_GATEWAY_SESSION"] = "1"
        os.environ["ATHANIEL_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("athaniel_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ATHANIEL_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.athaniel.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.athaniel.gateway.plist",
    "launchctl stop ai.athaniel.gateway",
    "launchctl restart ai.athaniel.gateway",
    "launchctl bootout gui/501/ai.athaniel.gateway",
    "launchctl remove ai.athaniel.gateway",
    "launchctl disable gui/501/ai.athaniel.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_ATHANIEL_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Athaniel ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.athaniel.update-checker",
