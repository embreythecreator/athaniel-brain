...
``athaniel_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``ATHANIEL_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # ATHANIEL_* handling (#27303): only the operational allowlist passes;
        # every other ATHANIEL_* is dropped (the broad prefix was removed).
        "ATHANIEL_HOME": "/home/alice/.athaniel",        # allowlisted → kept
        "ATHANIEL_PROFILE": "default",                 # allowlisted → kept
        "ATHANIEL_INTERACTIVE": "1",                   # not allowlisted → dropped
        "ATHANIEL_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "ATHANIEL_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``athaniel_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``athaniel_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        """
        from tools.code_execution_tool import generate_athaniel_tools_module
        import tempfile, ast
        stub = generate_athaniel_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_athaniel_tools_module(["terminal"], transport="uds")
...
# let the child import athaniel_tools, a user script that printed non-ASCII
