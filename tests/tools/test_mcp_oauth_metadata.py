...
Covers:
- :class:`AthanielTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.AthanielMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ATHANIEL_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# AthanielTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = AthanielTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path))
        storage = AthanielTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (AthanielMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: AthanielTokenStorage, **context_attrs):
    ...
    provider = _ATHANIEL_PROVIDER_CLS.__new__(_ATHANIEL_PROVIDER_CLS)
    provider._athaniel_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path))
        storage = AthanielTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ATHANIEL_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path))
        storage = AthanielTokenStorage("flow-srv")
        ...
        with patch.object(
            _ATHANIEL_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
