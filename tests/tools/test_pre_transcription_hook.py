...
PROMPT = "Athaniel, Teknium, Athaniel, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("athaniel_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("athaniel_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("athaniel_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.athaniel_xai_user_agent", lambda: "test-ua",
        )
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    plugin_dir = athaniel_home / "plugins" / "stt_vocab"
