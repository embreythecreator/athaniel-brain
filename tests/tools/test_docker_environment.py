...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across athaniel processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/athaniel-snap-test123.sh"
    env._cwd_file = "/tmp/athaniel-cwd-test123.txt"
    env._cwd_marker = "__ATHANIEL_CWD_test123__"
...
def test_init_env_args_uses_athaniel_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _ATHANIEL_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_athaniel_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_athaniel_dotenv(monkeypatch):
...
def test_init_env_args_uses_athaniel_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.athaniel/.env.
    """
    env = _make_execute_only_env(["MY_SECRET"])

    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_athaniel_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_uses_active_profile_for_forwarded_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_athaniel_env_vars", lambda: {})
...
def test_egress_node_options_overrides_conflicting_ca_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: ([], {"_ATHANIEL_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca"}, []),
    )
    calls = _mock_subprocess_run(monkeypatch)
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Athaniel image, or ``gosu``/``su``
...
def test_run_command_tags_athaniel_brain_label(monkeypatch):
    """Every container athaniel-brain starts must carry the athaniel-brain=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=athaniel-brain=1`` call. Regression test
    ...
    assert "athaniel-brain=1" in labels, (
        f"athaniel-brain=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "athaniel-task-id=task_with_weird_chars" in labels, (
...
def test_labels_attribute_populated_after_init(monkeypatch):
    ...
    assert env._labels == {
        "athaniel-brain": "1",
        "athaniel-task-id": "abc",
        "athaniel-profile": "default",
        "athaniel-egress": "off",
    }
...
def test_shared_container_key_replaces_profile_identity(monkeypatch):
    ...
    assert a._labels["athaniel-profile"] == b._labels["athaniel-profile"]
    assert a._labels["athaniel-profile"] != "research"
    assert a._labels["athaniel-profile"].startswith("team_workspace-")
...
def test_distinct_shared_keys_never_collide(monkeypatch):
    ...
    assert c._labels["athaniel-profile"] != d._labels["athaniel-profile"]
    # Both stay within Docker's 63-char label-value bound.
    assert len(c._labels["athaniel-profile"]) <= 63
    assert len(d._labels["athaniel-profile"]) <= 63
...
def test_empty_shared_container_key_preserves_profile_isolation(monkeypatch):
    ...
    assert env._labels["athaniel-profile"] == "research"
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Athaniel process spawning a new container
...
def test_egress_enabled_does_not_reuse_pre_egress_container(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ["-v", "/tmp/ca:/etc/ssl/certs/athaniel-egress-ca.crt:ro"],
    ...
    def _run(cmd, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                assert any(str(part).startswith("label=athaniel-egress=") for part in cmd)
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
...
def test_extra_args_proxy_override_refuses_under_egress(monkeypatch):
    """docker_extra_args are appended after Athaniel args, so egress enforcement
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Athaniel process uses it. Without this
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("athaniel-"), "should remove the container by its generated name"
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Athaniel startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.athaniel/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.athaniel/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="athaniel-brain:latest")
