...
Athaniel exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_athaniel_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    ATHANIEL_HOME (cache/terminal), NOT tmpfs /tmp."""
    import athaniel_constants  # noqa: F401 — resolves ATHANIEL_HOME per call
    ...
    assert result == str(tmp_path / ".athaniel" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "athaniel_bg_dead1.log").exists()
