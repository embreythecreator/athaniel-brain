...
class TestWriteDenyExactPaths:
    ...
    def test_athaniel_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.athaniel/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "athaniel_root"
        ...
        # Sanity check: ATHANIEL_HOME does point to the profile dir, not the root.
        from athaniel_constants import get_athaniel_home, get_default_athaniel_root
...
class TestWriteAllowed:
    ...
    def test_athaniel_control_files_requested_writable(self):
