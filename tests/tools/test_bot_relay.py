    ...
    return [
        {
            ...
            "connection_label": "Athaniel Cloud",
            "title": "Moxie",
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "athaniel", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@athaniel", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("athaniel@cloud-1", roster)["profile"] == "default"
    assert bot_relay.resolve_remote_target("athaniel@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "athaniel" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "athaniel", "connection_id": "cloud-1",
         "connection_label": "Athaniel Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="athaniel", message="ping", agent=agent))
    assert out.get("status") == "sent"
    assert "Athaniel Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 athaniel (@athaniel): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@athaniel` — on Athaniel Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="athaniel",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path / "nope"))
...
def test_ttl_config_read_is_lazy_and_defensive(monkeypatch):
    ...
    def _boom(name, *a, **k):
        if name.startswith("athaniel_cli"):
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "athaniel", "connection_id": "cloud-1",
         "connection_label": "Athaniel Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
