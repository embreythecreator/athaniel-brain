...
class TestPlanFrameOutputSchema:
    """WO-POSTURE/1 4.4: fixed frame contract, validated by the same path."""

    def _valid(self, frame="contrarian"):
        return {
            "frame": frame, "summary": "s", "relevance": "high",
            "practicality": "medium", "rigor": "low",
            "evidence": [{"arxiv_id": "2401.1", "claim": "c"}, {"vault_id": "note:1", "claim": "d"}],
        }

    def test_valid_frame_output_passes(self):
        from tools.delegation_output_schema import plan_frame_output_schema, validate_output

        ok, errs = validate_output(json.dumps(self._valid()), plan_frame_output_schema("contrarian"))
        assert ok and errs == []

    def test_wrong_frame_bad_enum_and_refless_evidence_fail(self):
        from tools.delegation_output_schema import plan_frame_output_schema, validate_output

        schema = plan_frame_output_schema("contrarian")
        wrong_frame = {**self._valid(), "frame": "failure_modes"}
        bad_enum = {**self._valid(), "rigor": "extreme"}
        refless = {**self._valid(), "evidence": [{"claim": "no ref"}]}
        for payload in (wrong_frame, bad_enum, refless):
            ok, errs = validate_output(json.dumps(payload), schema)
            assert not ok and errs, payload

    def test_evidence_is_bounded(self):
        from tools.delegation_output_schema import (
            FRAME_EVIDENCE_MAX, plan_frame_output_schema, validate_output,
        )

        too_many = {**self._valid(), "evidence": [{"arxiv_id": str(i), "claim": "c"} for i in range(FRAME_EVIDENCE_MAX + 1)]}
        ok, _ = validate_output(json.dumps(too_many), plan_frame_output_schema("contrarian"))
        assert not ok
