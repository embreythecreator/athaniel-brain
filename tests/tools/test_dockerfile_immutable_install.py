"""Contract tests for the Docker image's immutable /opt/athaniel install tree."""
...
def test_dockerfile_makes_opt_athaniel_readonly_for_athaniel_user() -> None:
    ...
    assert "chown -R root:root /opt/athaniel" not in text
    assert "chmod -R a+rX /opt/athaniel" not in text
    assert "chmod -R a-w /opt/athaniel" not in text


def test_dockerfile_does_not_chown_install_trees_to_athaniel() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+athaniel:athaniel\s+/opt/athaniel/\.venv",
        r"chown\s+-R\s+athaniel:athaniel\s+/opt/athaniel/ui-tui",
        r"chown\s+-R\s+athaniel:athaniel\s+/opt/athaniel/gateway",
        r"chown\s+-R\s+athaniel:athaniel\s+/opt/athaniel/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/athaniel must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/athaniel/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $ATHANIEL_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the athaniel
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/athaniel/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/athaniel/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/athaniel tree.
    assert f"ENV ATHANIEL_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV ATHANIEL_LAZY_INSTALL_TARGET=/opt/athaniel" not in text

    # The seal flag must still be present — the redirect rides on top of it,
    # it does not replace it.
    assert "ENV ATHANIEL_DISABLE_LAZY_INSTALLS=1" in text

    # stage2-hook must seed + chown the target dir so first-use installs
    # succeed as the unprivileged athaniel runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$ATHANIEL_HOME/lazy-packages"' in stage2, (
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+athaniel:athaniel\s+/opt/athaniel/plugins", text
    )
