...
Older images made /opt/athaniel/ui-tui and /opt/athaniel/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ATHANIEL_TUI_DIR=/opt/athaniel/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R athaniel:athaniel /opt/athaniel/ui-tui" not in text
    assert "chown -R athaniel:athaniel /opt/athaniel/node_modules" not in text
