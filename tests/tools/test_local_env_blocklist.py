...
Verifies that Athaniel-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Athaniel secrets.

See: https://github.com/embreythecreator/athaniel-brain/issues/1002
See: https://github.com/embreythecreator/athaniel-brain/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _ATHANIEL_PROVIDER_ENV_BLOCKLIST,
    _ATHANIEL_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.athaniel/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Athaniel inference secret
        ...
        models`` run inside a Athaniel terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _ATHANIEL_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Athaniel-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "ATHANIEL_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_athaniel_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Athaniel even when the gateway PATH cannot."""
        from tools.environments.local import _sanitize_subprocess_env

        with patch(
            "tools.environments.local._resolve_athaniel_bin_dir",
    ...
    def test_bare_athaniel_path_does_not_duplicate_existing_install_dir(self):
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    (and the other BUZZ_* vars) are stripped by _ATHANIEL_PROVIDER_ENV_BLOCKLIST
    ...
    children then get the BUZZ_* vars; execute_code, athaniel_subprocess_env,
    ...
    def test_buzz_vars_stripped_without_buzz_context(self, monkeypatch):
        ...
        monkeypatch.delenv("ATHANIEL_SESSION_PLATFORM", raising=False)
        buzz_vars = {
            "BUZZ_PRIVATE_KEY": "nsec1faketestkey",
            "BUZZ_AUTH_TAG": '["tag","data","kind","sig"]',
            "BUZZ_RELAY_URL": "https://mycommunity.communities.buzz.xyz",
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        athaniel_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in athaniel-snap-<id>.sh
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _ATHANIEL_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_ATHANIEL_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_ATHANIEL_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Athaniel venv path instead of the project's own ``.venv`` —
    silently clobbering the Athaniel environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Athaniel venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.athaniel/athaniel-brain/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/athaniel",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "athaniel-brain"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Athaniel-owned entry stripping (#74817).

    The Desktop Electron app injects the Athaniel repo root and the Athaniel
    ...
    ``_strip_athaniel_owned_pythonpath`` surgically removes only the
    entries Athaniel itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Athaniel-owned entries are removed; everything else survives
        verbatim (ordering, duplicates, empty components).

        Covers: the running venv's site-packages, the repo root (computed
        independently via parents[2] so an off-by-one in _athaniel_repo_root
        cannot silently pass), duplicate Athaniel entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Athaniel ordering with an
        empty component preserved.
        """
        from tools.environments.local import _strip_athaniel_owned_pythonpath
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Athaniel-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Athaniel venv
        ...
        off-by-one in _athaniel_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Athaniel-owned entries.
        ...
        Athaniel-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Athaniel venv site-packages entries are stripped
        by the same Athaniel-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_athaniel_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        athaniel_win = f"C:\\\\Users\\\\u\\\\.athaniel\\\\athaniel-brain\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([athaniel_win, user_win]),
        }
        ...
        # the Athaniel-owned check cannot match a backslash path here.
        assert athaniel_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_athaniel_owned_paths_stripped(self):
        """On Windows, a Athaniel venv site-packages entry written with
        backslashes is stripped by the same Athaniel-owned check, while a
        ...
        _strip_athaniel_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert athaniel_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Athaniel venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Athaniel runtime producer contract.
        """
        import tools.environments.local as local

        repo_root = tmp_path / "athaniel-brain"
        ...
        monkeypatch.setattr(local, "_athaniel_repo_root_aliases", (repo_root,))
        monkeypatch.setattr(local, "_in_venv", False)
        monkeypatch.setattr(local, "_athaniel_site_packages", None)
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local._strip_athaniel_owned_pythonpath(env)
    ...
    @pytest.mark.parametrize("builder", [
        ...
        "athaniel_subprocess_env",
    ])
    def test_builders_strip_athaniel_venv_pythonpath(self, builder):
        """Every subprocess env builder applies the same sanitation contract:
        Athaniel venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.athaniel_subprocess_env()
    ...
    def test_scrub_child_env_strips_athaniel_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Athaniel venv
        site-packages entries should be stripped when
        _strip_athaniel_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_athaniel_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_athaniel_entries(self, same_env):
        """Integration: execute_code's real spawn path composes a clean PYTHONPATH.

        Seeds a contaminated inherited PYTHONPATH (Athaniel repo root + Athaniel
        ...
        * inherited Athaniel venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        athaniel_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_tool._uses_athaniel_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [athaniel_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(athaniel_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Athaniel venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from athaniel_cli.gateway_windows import _preserve_athaniel_home_path
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(configured_home))

        launcher_entry = Path(_preserve_athaniel_home_path(physical_root))
        aliases = local._build_athaniel_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "athaniel-brain"
        assert launcher_entry in aliases

        monkeypatch.setattr(local, "_athaniel_repo_root_aliases", aliases)
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with ATHANIEL_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home ATHANIEL_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "athaniel-brain"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(configured_home))
        lexical_root = configured_home / "athaniel-brain"
        ...
        # The sanitizer now runs under the re-homed (profile) ATHANIEL_HOME.
        aliases = local._build_athaniel_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\athaniel\\athaniel-brain -> C:\\...\\athaniel-brain) while the
        ...
        # repo-level link: <configured-home>/athaniel-brain -> physical repo
        try:
            _make_directory_link(configured_home / "athaniel-brain", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "athaniel-brain").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "athaniel-brain"
        ...
        for lookalike in (configured_home / "athaniel-brain", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/athaniel-brain.
        ...
        try:
            _make_directory_link(configured_root / "athaniel-brain", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "athaniel-brain"
        aliases = local._build_athaniel_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local._same_path(a, lexical_root) for a in aliases)
        assert not any(local._same_path(a, configured_home / "athaniel-brain") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._athaniel_site_packages = None
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Athaniel runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Athaniel venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "athaniel_subprocess_env":
                result = local_mod.athaniel_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/athaniel-venv",
            "VIRTUAL_ENV": "/opt/athaniel-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/athaniel-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/athaniel-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_ATHANIEL_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Athaniel (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _ATHANIEL_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _ATHANIEL_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Athaniel-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _ATHANIEL_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Athaniel, and blocklisting
        ...
        leaked_block = general_chain & _ATHANIEL_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_ATHANIEL_PROVIDER_ENV_BLOCKLIST)

    def test_claude_code_oauth_token_is_inheritable(self):
        """CLAUDE_CODE_OAUTH_TOKEN is owned by the user's Claude Code install
        (subscription OAuth), not a Athaniel inference credential. Stripping it
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _ATHANIEL_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _ATHANIEL_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_athaniel_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        athaniel-install-dir prepend (a separate concern, covered by
        TestAthanielBinDirOnPath) so a real ``athaniel`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._ATHANIEL_BIN_DIR
        local_mod._ATHANIEL_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._ATHANIEL_BIN_DIR = saved
...
class TestAthanielBinDirOnPath:
    """The athaniel install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``athaniel`` via the terminal tool must work
    even when the gateway was launched without the athaniel install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_athaniel_bin_dir / _prepend_athaniel_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._ATHANIEL_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/athaniel/bin/athaniel" if name == "athaniel" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/athaniel/bin")
        assert local_mod._resolve_athaniel_bin_dir() == "/opt/athaniel/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._ATHANIEL_BIN_DIR = None
        assert local_mod._prepend_athaniel_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_athaniel_bin_dir(self):
        """A gateway env missing the athaniel dir gets it back in the subshell PATH.

        Platform-agnostic: ``_prepend_athaniel_bin_dir`` uses ``os.pathsep`` on
        every host, so no platform flag is faked here."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._ATHANIEL_BIN_DIR = "/opt/athaniel/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/athaniel/bin"
        assert "/usr/bin" in entries


class TestAthanielInternalDynamicSecrets:
    """Dynamically-named Athaniel secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_ATHANIEL_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_athaniel_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``athaniel_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_athaniel_internal_secret
        assert _is_athaniel_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_athaniel_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_athaniel_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_athaniel_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_athaniel_internal_secret
        assert _is_athaniel_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_athaniel_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_athaniel_internal_secret
        assert _is_athaniel_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_athaniel_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_athaniel_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_athaniel_internal_secret
        assert not _is_athaniel_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_athaniel_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_athaniel_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_athaniel_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_athaniel_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_athaniel_internal_secret("PATH")
        assert not _is_athaniel_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _ATHANIEL_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _ATHANIEL_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _ATHANIEL_PROVIDER_ENV_BLOCKLIST
