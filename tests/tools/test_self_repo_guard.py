...
class TestBlocksMutationsInSourceRepo:
    ...
    def test_relative_cd_into_repo(self, repo):
        hit, _ = _detect("cd athaniel-brain && git pull", repo.parent, repo)
        assert hit is True
    ...
    def test_tilde_dash_c_path(self, repo, monkeypatch, tmp_path):
        ...
        hit, _ = _detect("git -C ~/athaniel-brain checkout main", tmp_path, repo)
        assert hit is True
...
class TestBlockMessageGuidance:
    ...
    def test_scratch_hint_honors_athaniel_home(self, repo, monkeypatch):
        monkeypatch.setenv("ATHANIEL_HOME", "/custom/athaniel-home")
        hit, msg = _detect("git rebase origin/main", repo, repo)
        assert hit is True
        assert "/custom/athaniel-home/scratch" in msg
