...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export ATHANIEL_STICKY_ENV_PROBE="sticky"; '
                'export PATH="/tmp/athaniel-session-bin:$PATH"; '
                'echo "first=$ATHANIEL_STICKY_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$ATHANIEL_STICKY_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$ATHANIEL_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
