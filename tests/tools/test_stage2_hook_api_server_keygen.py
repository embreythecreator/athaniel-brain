...
was gated on ``[ -f "$ATHANIEL_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        "set -eu\n"
        f"{env_setup}"
        f'ATHANIEL_HOME="{home}"\n'
        # In tests we run unprivileged; as_athaniel is a passthrough then.
        'as_athaniel() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Athaniel loads $ATHANIEL_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/athaniel/.env.example -> $ATHANIEL_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ATHANIEL_HOME/.env from it on first boot"
    )
