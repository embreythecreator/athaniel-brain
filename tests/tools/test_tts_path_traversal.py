...
def test_output_path_rejects_athaniel_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    athaniel_home = tmp_path / "athaniel-home"
    athaniel_home.mkdir()
    monkeypatch.setattr(file_safety, "_athaniel_home_path", lambda: athaniel_home)
    monkeypatch.setattr(file_safety, "_athaniel_root_path", lambda: athaniel_home)

    target = athaniel_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    athaniel_home = tmp_path / "athaniel-home"
    token_dir = athaniel_home / "mcp-tokens"
