...
def fake_athaniel(tmp_path, monkeypatch):
    """Build a two-profile Athaniel layout and point ATHANIEL_HOME at
    the athaniel-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_athaniel):
        from tools.file_tools import write_file_tool
        target = fake_athaniel["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_athaniel):
        ...
        from tools.file_tools import write_file_tool
        target = fake_athaniel["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_athaniel_path_unaffected(self, fake_athaniel, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_athaniel):
    ...
    def test_cross_profile_patch_bypass(self, fake_athaniel):
        from tools.file_tools import patch_tool
        target = fake_athaniel["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_athaniel):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_athaniel, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_athaniel["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ATHANIEL_HOME (set in
        ...
        assert "not found in active profile 'athaniel-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.athaniel/profiles/<name>/."""
        # Don't set ATHANIEL_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_athaniel_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_athaniel_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_athaniel):
        """When active profile is 'athaniel-security', the prompt warns
        ...
        assert "Active Athaniel profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.athaniel/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Athaniel profile: default" in src
        assert "Active Athaniel profile: {active_profile}" in src
