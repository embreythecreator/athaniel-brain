"""Cross-session ATHANIEL_SESSION_* leak guard for the local terminal backend.

Regression coverage for the bug where a terminal subprocess could observe a
*different concurrent session's* ``ATHANIEL_SESSION_KEY`` (and the other
``ATHANIEL_SESSION_*`` vars).
...
subprocess-env bridge resolves each ``ATHANIEL_SESSION_*`` from the ContextVar and,
...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, athaniel_subprocess_env
...
def test_engaged_unset_contextvar_strips_foreign_session_key(monkeypatch):
    ...
    os.environ["ATHANIEL_SESSION_KEY"], this task's ContextVar is unset, and the
    ...
    assert "ATHANIEL_SESSION_KEY" not in env, (
        "Foreign concurrent session key leaked into subprocess env: "
        f"{env.get('ATHANIEL_SESSION_KEY')!r}"
    )
...
def test_set_session_vars_engages_and_overrides_foreign_global(monkeypatch):
    ...
    assert env.get("ATHANIEL_SESSION_KEY") == "agent:main:discord:group:MY_BUGS_ROOT:111"
...
def test_unengaged_process_preserves_os_environ_fallback(monkeypatch):
    ...
    Pure single-process CLI/one-shot sets ATHANIEL_SESSION_* directly in os.environ
    ...
    monkeypatch.setenv("ATHANIEL_SESSION_KEY", "cli-session-key")
    monkeypatch.setenv("ATHANIEL_SESSION_ID", "cli-session-id")

    env = _make_run_env({})

    assert env.get("ATHANIEL_SESSION_KEY") == "cli-session-key"
    assert env.get("ATHANIEL_SESSION_ID") == "cli-session-id"
...
def test_explicit_empty_thread_id_overrides_stale_value(monkeypatch):
    ...
    no thread id binds ATHANIEL_SESSION_THREAD_ID="" and that empty value must win
    over an older non-empty value left in os.environ.
    """
    monkeypatch.setenv("ATHANIEL_SESSION_THREAD_ID", "stale-thread-from-prior-turn")
    ...
    assert env.get("ATHANIEL_SESSION_THREAD_ID") == "", (
        "Bound-empty thread id did not override the stale value: "
        f"{env.get('ATHANIEL_SESSION_THREAD_ID')!r}"
    )
    assert env.get("ATHANIEL_SESSION_KEY") == "mm:chan"
...
def test_sanitize_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "ATHANIEL_SESSION_KEY": "agent:main:discord:thread:FOREIGN_BG:FOREIGN_BG",
        "ATHANIEL_SESSION_THREAD_ID": "FOREIGN_BG",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert "ATHANIEL_SESSION_KEY" not in sanitized, (
        f"Background subprocess inherited foreign key: {sanitized.get('ATHANIEL_SESSION_KEY')!r}"
    )
    assert "ATHANIEL_SESSION_THREAD_ID" not in sanitized
...
def test_sanitize_subprocess_env_set_contextvar_wins_when_engaged():
    ...
    assert sanitized.get("ATHANIEL_SESSION_KEY") == "agent:main:discord:group:REAL_BG:222"


# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (athaniel_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_athaniel_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """athaniel_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
...
def test_athaniel_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    """A pure single-process CLI (never engaged) keeps the inherited fallback."""
    monkeypatch.setenv("ATHANIEL_SESSION_KEY", "cli-fallback-key")
    # not engaged (autouse fixture leaves _session_context_engaged False)
    env = athaniel_subprocess_env()
    assert env.get("ATHANIEL_SESSION_KEY") == "cli-fallback-key"
