...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "athaniel-in"})
...
class TestSendToPlatformChunking:
    ...
    def test_signal_long_message_is_chunked(self, monkeypatch):
        ...
        The standalone path (athaniel send / cron / MCP) speaks raw JSON-RPC via
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from athaniel"
...
class TestParseTargetRef:
    ...
    def test_explicit_targets_round_trip_chat_and_thread(self):
        cases = [
            ...
            ("matrix", "@athaniel:matrix.org", "@athaniel:matrix.org", None),
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``athaniel cron``
    runs separately from ``athaniel gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``ATHANIEL_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``ATHANIEL_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``ATHANIEL_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
