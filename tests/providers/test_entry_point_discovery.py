"""Tests for pip entry-point provider discovery (athaniel_brain.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``athaniel_brain.plugins`` entry point, supporting both a
...
class _FakeEP:
    def __init__(self, name, loader):
        self.name = name
        self.group = "athaniel_brain.plugins"
        self._loader = loader
