...
from cli import AthanielCLI, _build_compact_banner, _rich_text_from_ansi
from athaniel_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_shows_athaniel_wordmark(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Athaniel Brain v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "✶ ATHANIEL" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_athaniel(self):
        ...
        assert "NOUS ATHANIEL" not in banner
