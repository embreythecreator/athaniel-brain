...
1. ``load_athaniel_dotenv()`` auto-loads ``~/.athaniel/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_athaniel_dotenv(athaniel_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "athaniel_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
