...
from acp_adapter.server import AthanielACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["athaniel-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "athaniel_cli.config",
        mod("athaniel_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
