...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.athaniel/athaniel-brain/utils.py)
...
2. ``athaniel update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``athaniel_cli/main.py`` (the ``athaniel update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``athaniel_constants`` after the
