...
def test_required_frontmatter_fields(p):
    ...
    athaniel = (fm.get("metadata") or {}).get("athaniel") or {}
    if not (athaniel.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.athaniel.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    athaniel = (fm.get("metadata") or {}).get("athaniel") or {}
    dangling = [
        rs for rs in (athaniel.get("related_skills") or []) if rs not in _all_names()
    ]
