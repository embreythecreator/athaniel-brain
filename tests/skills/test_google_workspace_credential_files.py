...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        athaniel_home.mkdir()
        (athaniel_home / "google_token.json").write_text("{}")
        (athaniel_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.athaniel/google_token.json" in container_paths
            assert "/root/.athaniel/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
