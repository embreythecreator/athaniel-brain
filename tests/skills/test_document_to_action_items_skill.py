...
def test_frontmatter_required_fields():
    ...
    athaniel = fm["metadata"]["athaniel"]
    assert athaniel["tags"]
    assert "related_skills" in athaniel
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Athaniel Brain"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["athaniel"]["related_skills"]:
