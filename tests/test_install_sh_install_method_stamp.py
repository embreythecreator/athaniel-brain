...
($INSTALL_DIR), not into the shared $ATHANIEL_HOME.

Background (shared-$ATHANIEL_HOME bug)
------------------------------------
$ATHANIEL_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.athaniel:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$ATHANIEL_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``athaniel update``).
...
``~/.athaniel/athaniel-brain``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$ATHANIEL_HOME/\.install_method"', text), (
        "install.sh must not stamp $ATHANIEL_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `athaniel update`"
    )
