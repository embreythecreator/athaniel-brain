"""Tests for ``athaniel migrate xai`` — apply path with ruamel round-trip."""
...
class TestCrashDurability:
    ...
    def test_symlinked_config_is_replaced_in_place(self, tmp_path: Path):
        ...
        real_dir.mkdir()
        real = real_dir / "athaniel-config.yaml"
        real.write_text(
            "principal:\n"
            "  provider: xai\n"
            "  model: grok-3\n",
    ...
    def test_comments_survive_the_atomic_write(self, trap_config: Path):
        ...
        text = trap_config.read_text(encoding="utf-8")
        assert "# Athaniel config (sample)" in text
