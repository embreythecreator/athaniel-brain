...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "athaniel_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ATHANIEL_HOME.  For a standard install (ATHANIEL_HOME unset) that root is
        # the platform-native default (~/.athaniel), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from athaniel_constants import get_default_athaniel_root
        assert env.get("ATHANIEL_HOME") == str(get_default_athaniel_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ATHANIEL_DESKTOP=1) must NOT
