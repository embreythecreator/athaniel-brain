...
1. The venv-repair import probes (early recovery + `athaniel update`) must
...
2. ``athaniel doctor`` must fail the certificate check in that state, and
   ``athaniel doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `athaniel update`'s venv repair must apply
    ...
    def _run_probe_script(self, monkeypatch, tmp_path, bundle_path):
        ...
        from athaniel_cli import main as main_mod
...
# =========================================================================
# 2. athaniel doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "athaniel doctor --fix" in message
