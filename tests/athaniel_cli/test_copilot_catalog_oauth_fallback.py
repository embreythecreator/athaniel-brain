...
``athaniel auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.athaniel/.env`` — the picker was silently dropping back to
...
from athaniel_cli.models import _resolve_copilot_catalog_api_key
...
class TestCopilotCatalogApiKeyResolution:
    ...
    def test_falls_back_to_pool_oauth_token(self):
        ...
        with patch(
            ...
            "athaniel_cli.copilot_auth.exchange_copilot_token",
