"""Regression tests for #34107 — Docker UID/GID handling in ensure_athaniel_home.

When Athaniel runs in Docker with ``ATHANIEL_UID=1000`` / ``ATHANIEL_GID=911``,
the entrypoint chowns the top-level ``ATHANIEL_HOME`` once at startup. But
...
The fix is a ``_chown_to_athaniel_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_athaniel_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAthanielUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ATHANIEL_UID", "1000")
        monkeypatch.setenv("ATHANIEL_GID", "911")
        from athaniel_cli.config import _resolve_athaniel_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_athaniel_uid
# ---------------------------------------------------------------------------


class TestChownToAthanielUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ATHANIEL_UID", "1000")
        monkeypatch.setenv("ATHANIEL_GID", "911")
        from athaniel_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_athaniel_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_athaniel_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ATHANIEL_UID", raising=False)
        monkeypatch.delenv("ATHANIEL_GID", raising=False)
        from athaniel_cli import config as cfg
