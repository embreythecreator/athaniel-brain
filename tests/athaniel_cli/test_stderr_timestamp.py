"""Tests for athaniel_cli.stderr_timestamp."""

import re
import sys

from gateway.restart import EXTERNAL_GATEWAY_SUPERVISOR_ENV
from athaniel_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.athaniel.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.athaniel.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "athaniel_cli.main", "gateway", "run", "--replace"]
