...
See: embreythecreator/athaniel-brain#7622
"""
...
_GATEWAY_CMD = "python -m athaniel_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m athaniel_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("athaniel_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("athaniel_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("athaniel_cli.gateway.is_macos", return_value=True),
            patch("athaniel_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "athaniel_cli.gateway.get_launchd_label",
                return_value="ai.athaniel.gateway.myprofile",
            ),
            patch(
                "athaniel_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.athaniel.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.athaniel.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.athaniel.gateway": 123,
            "ai.athaniel.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "athaniel_cli.gateway.get_launchd_label",
                return_value="ai.athaniel.gateway",
            ),
            patch(
                "athaniel_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.athaniel.gateway", "ai.athaniel.gateway-profile-b"],
            ),
            patch(
                "athaniel_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.athaniel.gateway-unmapped\n"
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.athaniel.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("athaniel_cli.gateway.is_macos", return_value=True),
            patch("athaniel_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.athaniel.gateway.broken\n"
                    "  123  \t0\tai.athaniel.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every athaniel-gateway* unit unconditionally."""
        with (
