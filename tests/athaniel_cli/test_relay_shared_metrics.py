"""Focused tests for the Athaniel shared-metrics durable store."""
...
from athaniel_cli.observability import shared_metrics as shared_metrics_module
from athaniel_cli.observability.shared_metrics import SharedMetricsStore
from athaniel_cli.observability.shared_metrics_contract import (
...
LEGACY_SCHEMA_PATH = SCHEMA_PATH.with_name("athaniel.shared_metrics.v1.schema.json")
...
def _resource(
    athaniel_version: str = "test-version",
    *,
    os_family: str = "linux",
    ...
    return {
        ...
        "athaniel_version": athaniel_version,
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "athaniel.shared_metrics.v2"
...
def test_package_schema_matches_the_model_call_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "athaniel.shared_metrics.v2"
...
def test_client_active_mark_accepts_only_an_empty_allowlisted_payload():
    event = SimpleNamespace(
        ...
        scope_category=None,
        metadata={
            "athaniel.metrics.schema_version": "athaniel.metrics.event.v2",
        },
        data={},
    )
    ...
    wrong_schema.metadata["athaniel.metrics.schema_version"] = "unknown"
...
def test_v1_package_schema_retains_the_legacy_model_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "athaniel.shared_metrics.v1"
...
def test_auxiliary_logical_scope_projects_one_normalized_terminal_route():
    event = SimpleNamespace(
        ...
        metadata={
            ...
            "athaniel.call_role": "auxiliary:compression",
        },
    )
    ...
    event.metadata["athaniel.call_role"] = "primary"
...
def test_tool_subscriber_contract_accepts_only_bounded_events():
    terminal = SimpleNamespace(
        ...
        category="tool",
        category_profile={},
        name="athaniel.tool_call",
        scope_category="end",
        metadata={"athaniel.metrics.schema_version": "athaniel.metrics.event.v2"},
        data={
...
def test_skill_subscriber_contract_accepts_only_bounded_marks():
    ...
    assert skill_counter(load) == ("athaniel.skill.load.count", load.data)
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        connection.execute(
            ...
            CREATE TABLE counter_aggregates (
                ...
                athaniel_version TEXT NOT NULL,
...
def test_store_exports_task_started_and_terminal_counters(tmp_path):
    ...
    store.record_counter("athaniel.task_run.finished", terminal, _resource())
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE athaniel_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT athaniel_version FROM counter_aggregates"
            ).fetchall()
        }
