...
2. ``athaniel_cli.main._refresh_windows_gateway_launchers`` — ``athaniel
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "athaniel_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("athaniel_cli.config.get_athaniel_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: athaniel update regenerates launchers
# ---------------------------------------------------------------------------
