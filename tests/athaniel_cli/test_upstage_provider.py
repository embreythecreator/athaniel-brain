...
Regression guard for the bug where `athaniel model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`athaniel_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in ATHANIEL_OVERLAYS
        overlay = ATHANIEL_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `athaniel_cli/main.py` (cmd_model): read
