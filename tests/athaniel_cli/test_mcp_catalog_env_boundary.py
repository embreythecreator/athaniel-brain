...
@pytest.fixture
def catalog_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _isolate_athaniel_home):
    ...
    monkeypatch.setenv("ATHANIEL_OPTIONAL_MCPS", str(catalog))
...
def test_catalog_rejects_undeclared_key_before_any_write_or_install(
    ...
    import athaniel_cli.mcp_catalog as mcp_catalog
...
def test_catalog_cannot_declare_reserved_control_key(
    ...
    response = client.post(
        "/api/mcp/catalog/install",
        headers=HEADERS,
        json={"name": "demo", "env": {"ATHANIEL_YOLO_MODE": "1"}},
    )
    ...
    assert installs == []
    env_path = catalog_env / ".env"
    assert not env_path.exists() or "ATHANIEL_YOLO_MODE" not in env_path.read_text(
        encoding="utf-8"
    )
...
def test_rejected_copilot_controls_do_not_change_live_resolvers(
    ...
    monkeypatch.setenv("ATHANIEL_COPILOT_ACP_COMMAND", "/opt/trusted/copilot")
    monkeypatch.setenv("ATHANIEL_COPILOT_ACP_ARGS", "--acp --stdio")
    expected_command = _resolve_command()
    expected_args = _resolve_args()

    attempts = {
        "ATHANIEL_COPILOT_ACP_COMMAND": "/tmp/attacker-command",
        "ATHANIEL_COPILOT_ACP_ARGS": "--malicious-transport",
    }
...
def test_preexisting_copilot_controls_remain_usable(
    ...
    monkeypatch.setenv("ATHANIEL_COPILOT_ACP_COMMAND", "parent-placeholder")
    monkeypatch.setenv("ATHANIEL_COPILOT_ACP_ARGS", "--parent-placeholder")
    (catalog_env / ".env").write_text(
        "ATHANIEL_COPILOT_ACP_COMMAND=/opt/operator/copilot\n"
        "ATHANIEL_COPILOT_ACP_ARGS=--acp --stdio --operator-mode\n",
        encoding="utf-8",
    )

    load_athaniel_dotenv(
        athaniel_home=catalog_env,
