"""Tests for athaniel_cli/terminal_breadcrumbs.py — per-terminal ``athaniel -c``.

Covers terminal id derivation (tty vs env vars vs none), breadcrumb
write/read roundtrip under a temp ATHANIEL_HOME, stale-session fallback
...
from athaniel_cli import terminal_breadcrumbs as tb
...
def test_breadcrumb_roundtrip(athaniel_home, monkeypatch, no_terminal_env):
    ...
    files = list((athaniel_home / "terminal-sessions").iterdir())
    assert [f.name for f in files] == ["tty-dev-pts-7"]


def test_write_skipped_without_terminal_identity(athaniel_home, monkeypatch, no_terminal_env):
    _fake_no_tty(monkeypatch)
    tb.write_breadcrumb("20260815_120000_abc123")
    assert not (athaniel_home / "terminal-sessions").exists()
    assert tb.read_breadcrumb() is None


def test_two_terminals_do_not_clobber(athaniel_home, monkeypatch, no_terminal_env):
...
def test_stale_breadcrumb_ignored_and_pruned(athaniel_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = athaniel_home / "terminal-sessions"
...
def test_corrupt_breadcrumb_returns_none(athaniel_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = athaniel_home / "terminal-sessions"
    directory.mkdir(parents=True)
...
def test_resolve_picks_this_terminals_session(athaniel_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(athaniel_home, "20260815_100000_aaaaaa")
    _make_session(athaniel_home, "20260815_110000_bbbbbb")  # newer, other terminal
    tb.write_breadcrumb("20260815_100000_aaaaaa")
    assert tb.resolve_breadcrumb_session() == "20260815_100000_aaaaaa"


def test_resolve_falls_back_when_session_deleted(athaniel_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(athaniel_home, "20260815_110000_bbbbbb")
    tb.write_breadcrumb("20260815_100000_deleted")  # never existed / deleted
    assert tb.resolve_breadcrumb_session() is None


def test_resolve_projects_through_compression_chain(athaniel_home, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(athaniel_home, "20260815_100000_parent")
...
def test_config_gate_off_disables_writes_and_resolution(
    athaniel_home, monkeypatch, no_terminal_env
):
