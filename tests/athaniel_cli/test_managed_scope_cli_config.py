...
athaniel_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `athaniel config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_athaniel_home = get_athaniel_home()`` at import time (module
    singleton), so monkeypatching ATHANIEL_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ATHANIEL_HOME.)
    """
    import cli

    cli._athaniel_home = home
