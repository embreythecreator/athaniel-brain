...
Complements tests/athaniel_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/athaniel_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/athaniel_cli/test_kanban_db.py.
    monkeypatch.setenv("ATHANIEL_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    athaniel subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("ATHANIEL_KANBAN_TASK") == tid
    assert env.get("ATHANIEL_PROFILE") == "some-profile"
...
def test_cli_daemon_help_marks_deprecated():
    ...
    from athaniel_cli import kanban as kb_cli
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `athaniel kanban --help` output"
    )
...
def test_gateway_dispatcher_disables_corrupt_board_without_traceback(
    ...
    import athaniel_cli.config as _cfg_mod
    import athaniel_cli.kanban_db as _kb
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``athaniel_cli.kanban_db`` fresh and uses that single
