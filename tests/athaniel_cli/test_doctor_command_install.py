"""Tests for the Command Installation check in athaniel doctor."""
...
import athaniel_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ATHANIEL_HOME + PROJECT_ROOT for doctor tests."""
    ...
    athaniel_bin = venv_bin_dir / "athaniel"
    athaniel_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    ...
    return home, project, athaniel_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, athaniel_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "athaniel"
        wrong_target = tmp_path / "wrong_athaniel"
        ...
        assert cmd_link.resolve() == athaniel_bin.resolve()
