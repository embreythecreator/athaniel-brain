"""Tests for the unified provider catalog (athaniel_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``athaniel model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from athaniel_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
