"""Tests for the per-profile Projects store (athaniel_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Athaniel Brain", folders=["/tmp/athaniel"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "athaniel-brain"
    assert proj.name == "Athaniel Brain"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/athaniel"
    assert [f.path for f in proj.folders] == ["/tmp/athaniel"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "athaniel-brain").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ATHANIEL_HOME.
