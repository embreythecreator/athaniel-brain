...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /athaniel <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_athaniel_norm = {_norm(n) for n in _SLACK_VIA_ATHANIEL_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_athaniel_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `athaniel slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "athaniel_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
class TestTelegramMenuCommands:
    ...
    def test_configured_priority_promotes_skill_into_last_menu_slot(
        ...
        with (
            patch("athaniel_cli.commands.telegram_bot_commands", return_value=fake_core),
            patch("agent.skill_commands.get_skill_commands", return_value=fake_cmds),
            patch("athaniel_cli.plugins.get_plugin_commands", return_value=fake_plugins),
            patch("tools.skills_tool.SKILLS_DIR", local_dir),
            patch("athaniel_cli.commands._telegram_command_menu_config", return_value=menu_cfg),
        ):
    ...
    def test_scalar_configured_priority_is_accepted_as_one_command(self):
        ...
        from unittest.mock import patch
        from athaniel_cli.commands import _telegram_effective_priority
        ...
        with patch("athaniel_cli.config.read_raw_config", return_value=raw_config):
...
from athaniel_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch athaniel_cli.plugins.get_plugin_commands() to a fixed dict."""
        from athaniel_cli import plugins as _plugins_mod
