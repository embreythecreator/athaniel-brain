...
base deps) during ``athaniel update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point athaniel_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("athaniel_cli.main._resolve_install_target_python", return_value=py), \
             patch("athaniel_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("athaniel_cli.main._run_install_with_heartbeat"):

            from athaniel_cli.main import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("athaniel_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("athaniel_cli.main._run_install_with_heartbeat") as mock_install:
            from athaniel_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `athaniel update`.
        ...
        with patch("athaniel_cli.main._is_windows", return_value=True):
            from athaniel_cli.main import _resolve_install_target_python
