"""Tests for the Linux XDG desktop entry installed by ``athaniel desktop``."""
...
from athaniel_cli import linux_desktop_entry as lde
...
def xdg_home(tmp_path, monkeypatch) -> Path:
    ...
    # ~/.local/bin/athaniel on the dev machine.
...
def test_install_writes_entry_with_absolute_exec_and_icon(
    ...
    athaniel_bin = tmp_path / "bin" / "athaniel"
    athaniel_bin.parent.mkdir()
    athaniel_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "athaniel_cli.relaunch.resolve_athaniel_bin", lambda: str(athaniel_bin)
    )
    ...
    assert entry == xdg_home / "applications" / "athaniel.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `athaniel` would not resolve.
    assert values["Exec"] == f"{athaniel_bin} desktop"
...
def test_install_prefers_themed_icon_from_hicolor(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "athaniel.png"
    assert dest.is_file()
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "athaniel_cli.relaunch.resolve_athaniel_bin", lambda: "/usr/bin/athaniel"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("athaniel_cli.relaunch.resolve_athaniel_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m athaniel_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `athaniel`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(
    ...
    athaniel_bin = tmp_path / "bin" / "athaniel"
    athaniel_bin.parent.mkdir()
    athaniel_bin.write_text(
        "#!/usr/bin/env python3\nimport athaniel_cli\n", encoding="utf-8"
    )
    athaniel_bin.chmod(0o755)
    monkeypatch.setattr(
        "athaniel_cli.relaunch.resolve_athaniel_bin", lambda: str(athaniel_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(athaniel_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    athaniel_bin.parent.mkdir()
    athaniel_bin.write_text(
        '#!/bin/bash\nexec /opt/athaniel/venv/bin/python "$@"\n', encoding="utf-8"
    )
    athaniel_bin.chmod(0o755)
    monkeypatch.setattr(
        "athaniel_cli.relaunch.resolve_athaniel_bin", lambda: str(athaniel_bin)
    ...
    assert exec_line == f"{athaniel_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    athaniel_bin.parent.mkdir()
    interpreter = os.path.abspath(sys.executable)
    athaniel_bin.write_text(f"#!{interpreter}\nimport athaniel_cli\n", encoding="utf-8")
    athaniel_bin.chmod(0o755)
    monkeypatch.setattr(
        "athaniel_cli.relaunch.resolve_athaniel_bin", lambda: str(athaniel_bin)
    )
...
def test_exec_converges_from_repo_script_argv0_to_installed_wrapper(
    ...
    Launching with argv[0] = <checkout>/athaniel (what the broken entry
    ...
    root = _make_project(tmp_path)
    repo_script = root / "athaniel"  # checkout-internal launcher candidate
    repo_script.write_text(
        "#!/usr/bin/env python3\nimport athaniel_cli\n", encoding="utf-8"
    )
    repo_script.chmod(0o755)
    wrapper = tmp_path / "installed" / "bin" / "athaniel"
    ...
    monkeypatch.setattr(
        "shutil.which", lambda name: str(wrapper) if name == "athaniel" else None
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m athaniel_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m athaniel_cli.main desktop` fallback. Persisting the
    ...
    def fake_resolve():
        # Mirror resolve_athaniel_bin's chain: argv[0] → relative → PATH → None.
        return sys.argv[0] if sys.argv[0] else None

    monkeypatch.setattr("athaniel_cli.relaunch.resolve_athaniel_bin", fake_resolve)
...
def test_exec_uses_known_wrapper_when_path_lookup_misses(
    ...
    # checkout's athaniel script (the aidiyet check requires it to target
    # the writing checkout).
    known_wrapper = tmp_path / "known-home" / ".local" / "bin" / "athaniel"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "athaniel"} "$@"\n',
        encoding="utf-8",
    )
...
def test_exec_rejects_known_wrapper_from_another_checkout(
    ...
    On machines with multiple installs over time, ~/.local/bin/athaniel may
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "user",
            {},
            "HOME-SET-BY-TEST/.local/bin/athaniel",
            id="user-layout",
        ),
        pytest.param(
            "termux",
            {"PREFIX": "PREFIX-SET-BY-TEST"},
            "PREFIX-SET-BY-TEST/bin/athaniel",
            id="termux-prefix-first",
        ),
        pytest.param(
            "root-fhs",
            {"__EUID0__": "1"},
            "/usr/local/bin/athaniel",
            id="root-fhs",
        ),
        pytest.param(
            "non-root-no-fhs",
...
def test_known_wrapper_candidates_cover_installer_layouts(
    ...
    if layout == "root-fhs":
        ...
        assert candidates.index("/usr/local/bin/athaniel") < candidates.index(
            f"{sentinel_home}/.local/bin/athaniel"
        )
    if layout == "non-root-no-fhs":
        # Non-root euid: /usr/local/bin must be excluded outright.
        assert "/usr/local/bin/athaniel" not in candidates
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Athaniel is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "athaniel"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "athaniel"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("athaniel_cli.relaunch.resolve_athaniel_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_can_import_probe_runs_and_caches(tmp_path):
    ...
    deliberately NOT asserted - a CI host with athaniel pip-installed
    ...
    try:
        first = lde._can_import_athaniel_cli(real)
        assert isinstance(first, bool)
        assert str(real) in lde._probe_cache
        t0 = time.monotonic()
        second = lde._can_import_athaniel_cli(real)
...
def test_exec_falls_back_to_running_interpreter_when_probe_fails(
    ...
    monkeypatch.setattr(lde, "_can_import_athaniel_cli", lambda p: False)
...
def test_wrapper_ownership_accepts_shim_via_symlinked_home(tmp_path, monkeypatch):
    ...
    lexical_checkout = home_link / "athaniel-brain"
    (home_real / "athaniel-brain").mkdir()

    shim = home_link / ".local" / "bin" / "athaniel"
    shim.parent.mkdir(parents=True)
    shim.write_text(
        f"#!/bin/bash\n"
        f"exec {lexical_checkout}/venv/bin/python "
        f'{lexical_checkout}/athaniel "$@"\n',
        encoding="utf-8",
    )
...
def test_needs_interpreter_case_insensitive_match(tmp_path, monkeypatch):
    ...
    console_script = venv_bin / "athaniel"
...
def test_probe_skips_wrapper_with_escaping_python_shebang(
    ...
    broken_wrapper = xdg_home / ".local" / "bin" / "athaniel"
...
def test_probe_accepts_shell_launcher_wrapper(tmp_path, xdg_home, monkeypatch):
    ...
    good_wrapper = xdg_home / ".local" / "bin" / "athaniel"
