...
Covers:
 - Normal path (tokens already in Athaniel auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Athaniel
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `athaniel auth openai-codex`. The old
...
@pytest.fixture()
def athaniel_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Athaniel auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(athaniel_auth_only_env):
    """openai-codex appears when tokens are already in Athaniel auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Athaniel
    auth store."""
    athaniel_home = tmp_path / ".athaniel"
    athaniel_home.mkdir()
