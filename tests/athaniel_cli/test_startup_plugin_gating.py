...
``athaniel_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``athaniel --help``, ``athaniel --version``,
``athaniel logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from athaniel_cli._parser import build_top_level_parser, top_level_value_flag_sets
...
def _live_subcommand_names() -> set[str]:
    """Run ``athaniel --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from athaniel_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["athaniel", "--help"]
    buf = io.StringIO()
    try:
...
def test_reasoning_value_is_not_misclassified_as_subcommand(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["athaniel", "--reasoning", "high", "chat", "hello"],
    )
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``athaniel photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain athaniel-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level athaniel <name> CLI command via PluginContext.
