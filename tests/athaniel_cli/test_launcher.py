"""Tests for the top-level `./athaniel` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./athaniel` should use `athaniel_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "athaniel"
    called = []

    fake_main_module = types.ModuleType("athaniel_cli.main")

    def fake_main():
        called.append("athaniel_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "athaniel_cli.main", fake_main_module)
    ...
    assert called == ["athaniel_cli.main"]
