"""Tests for the update check mechanism in athaniel_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "athaniel-brain"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("athaniel_cli.banner.subprocess.run") as mock_run:
...
def test_check_via_local_git_fetch_failure_returns_none(tmp_path, monkeypatch):
    ...
    def mock_git_stdout(args, *, cwd, timeout=5):
        if args[:2] == ["remote", "get-url"]:
            return "https://github.com/embreythecreator/athaniel-brain.git"
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path))
    monkeypatch.delenv("ATHANIEL_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "athaniel_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "athaniel_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_athaniel_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via athaniel_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to athaniel_home / "athaniel-brain". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to athaniel_home / "athaniel-brain".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("athaniel_cli"):
