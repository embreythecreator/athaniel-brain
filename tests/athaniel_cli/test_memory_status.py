"""Tests for `athaniel memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from athaniel_cli.memory_setup import cmd_status
    ...
    with patch("athaniel_cli.config.load_config", return_value=config):
        with patch("athaniel_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
