"""Tests for --ignore-user-config and --ignore-rules flags on `athaniel chat`.
...
files. In Athaniel the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.athaniel/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AthanielCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("ATHANIEL_IGNORE_USER_CONFIG", "ATHANIEL_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``ATHANIEL_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<athaniel_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With ATHANIEL_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._athaniel_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("ATHANIEL_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AthanielCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting ATHANIEL_IGNORE_RULES=1 flips AthanielCLI.ignore_rules True."""
        monkeypatch.setenv("ATHANIEL_IGNORE_RULES", "1")

        # Import AthanielCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AthanielCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AthanielCLI)
        # Replicate the exact logic from cli.py AthanielCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("ATHANIEL_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``athaniel_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("ATHANIEL_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("ATHANIEL_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("ATHANIEL_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("ATHANIEL_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "ATHANIEL_IGNORE_USER_CONFIG" not in os.environ
        assert "ATHANIEL_IGNORE_RULES" not in os.environ
