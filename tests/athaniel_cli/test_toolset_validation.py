"""Unit tests for athaniel_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Athaniel.
"""
...
# A representative set of real toolset names. `athaniel` is deliberately absent —
# that is the corruption #38798 reported (`athaniel-cli` rewritten to `athaniel`).
_KNOWN = {
    "athaniel-cli",
    "athaniel-telegram",
    "athaniel-discord",
...
def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'athaniel' instead of 'athaniel-cli'.
    warnings = validate_platform_toolsets({"cli": ["athaniel"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'athaniel'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'athaniel-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["athaniel-cli"], "discord": ["bogus"]}
...
def test_empty_list_on_a_platform_warns_even_when_others_are_valid():
    ...
    cfg = {
        "cli": [],
        "telegram": ["athaniel-telegram"],
        "discord": ["athaniel-discord"],
    }
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_empty_list_warns_for_each_affected_platform():
    cfg = {"cli": [], "discord": [], "telegram": ["athaniel-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_null_platform_warns_even_when_others_are_valid():
    ...
    cfg = {"cli": None, "telegram": ["athaniel-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)

    assert any(
        "platform 'cli'" in w
        and "null toolset value" in w
        and "falling back to 'athaniel-cli'" in w
        for w in warnings
    )
    assert not any("zero valid toolsets" in w for w in warnings)
...
def test_null_platform_uses_canonical_default_for_alias():
    cfg = {"whatsapp_cloud": None, "telegram": ["athaniel-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)

    assert any(
        "platform 'whatsapp_cloud'" in w
        and "falling back to 'athaniel-whatsapp'" in w
        for w in warnings
    )
    assert not any("zero valid toolsets" in w for w in warnings)
...
def test_scalar_platform_value_warns_but_uses_platform_default():
    cfg = {"cli": "bogus", "telegram": ["athaniel-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_platform_restricted_toolset_warns_when_other_platform_is_valid():
    cfg = {"telegram": ["discord"], "cli": ["athaniel-cli"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
...
def test_null_plugin_platform_uses_synthetic_default():
    ...
    try:
        cfg = {platform: None, "telegram": ["athaniel-telegram"]}
        warnings = validate_platform_toolsets(cfg, _is_valid)

        assert resolve_toolset(f"athaniel-{platform}")
        assert any(
            f"platform '{platform}'" in w
            and f"falling back to 'athaniel-{platform}'" in w
            for w in warnings
        )
        assert not any("zero valid toolsets" in w for w in warnings)
    finally:
...
def test_populated_platforms_produce_no_empty_list_warning():
    cfg = {"cli": ["athaniel-cli"], "telegram": ["athaniel-telegram"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert warnings == []
