"""Tests for _purge_stale_athaniel_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `athaniel update`.

Field failure (2026-08-20, Teknium's Linux box): `athaniel update` pulled a
checkout where athaniel_cli/gateway.py newly imports `line_input` from
athaniel_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from athaniel_cli.gateway
...
the Athaniel package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Athaniel modules from the cache; later
...
def test_purge_evicts_athaniel_prefixed_modules():
    victims = [
        "athaniel_cli.cli_output",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_athaniel_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_athaniel_modules()
    assert sys.modules.get("athaniel_cli.update_cmd") is update_cmd
    assert sys.modules.get("athaniel_cli.main") is cli_main
    assert "athaniel_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["athaniel_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_athaniel_modules()
    finally:
        sys.modules.pop("athaniel_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from athaniel_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
