...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``athaniel_cli._subprocess_compat.pid_is_athaniel`` fails closed on foreign
  processes and identity mismatches.
- ``athaniel_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsAthanielLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Athaniel process (bare python sleeper in a temp-ish argv)
        must never be judged safe for taskkill."""
        from athaniel_cli._subprocess_compat import pid_is_athaniel

        proc = _spawn_sleeper()
        try:
            # sys.executable in CI lives under a uv/hostedtoolcache path with
            # no 'athaniel' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "athaniel" in sys.executable.lower():
                pytest.skip("interpreter path names athaniel; probe would match")
            assert pid_is_athaniel(proc.pid) is False
        finally:
            _cleanup(proc)

    def test_stale_fingerprint_is_refused_even_for_athaniel_argv(self):
        ...
        try:
            ...
            assert (
                pid_is_athaniel(proc.pid, expected_start_time=live + 12345) is False
            )
        finally:
            _cleanup(proc)
    ...
    def test_nonexistent_pid_is_refused(self):
        ...
        assert pid_is_athaniel(2**24) is False
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from athaniel_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
