"""Tests for athaniel_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `athaniel setup` and `athaniel import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("athaniel_constants.is_container", lambda: container)
...
class TestEnsureGatewayService:
    ...
    def test_no_service_manager_is_noop(self, monkeypatch, capsys):
        ...
        assert gateway_mod.ensure_gateway_service() is False
        out = capsys.readouterr().out
        assert "athaniel gateway" in out
