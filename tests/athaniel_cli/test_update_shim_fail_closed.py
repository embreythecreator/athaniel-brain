...
The bug class: on Windows, when `athaniel.exe`/sibling shims could not be
...
import athaniel_cli._install_repair as ir
import athaniel_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("athaniel", "athaniel-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # athaniel.exe cannot be renamed; athaniel-gateway.exe can
    real_rename = Path.rename

    def deny_athaniel(self, target):
        if self.name == "athaniel.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_athaniel)
    monkeypatch.setattr(cli_main, "_athaniel_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "athaniel.exe" in exc_info.value.failed_shims
    # The successful rename (athaniel-gateway.exe) was rolled back.
    assert (scripts / "athaniel-gateway.exe").exists()
    assert not list(scripts.glob("athaniel-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("athaniel",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("athaniel",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = cli_main.ShimQuarantineError(["athaniel.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "athaniel.exe" in out
