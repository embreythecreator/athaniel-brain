"""ensure_athaniel_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".athaniel"
    second = tmp_path / "b" / ".athaniel"

    monkeypatch.setenv("ATHANIEL_HOME", str(first))
    cfg.ensure_athaniel_home()

    # Profile switch: ATHANIEL_HOME moves → the new path is ensured too.
    monkeypatch.setenv("ATHANIEL_HOME", str(second))
    cfg.ensure_athaniel_home()
