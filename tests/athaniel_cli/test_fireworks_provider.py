...
from athaniel_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS, normalize_provider
...
class TestFireworksAliases:
    ...
    def test_providers_normalize_provider(self, alias):
        from athaniel_cli.providers import normalize_provider as normalize_in_providers
...
class TestFireworksOverlay:
    def test_overlay_exists(self):
        ...
        assert "fireworks" in ATHANIEL_OVERLAYS
        overlay = ATHANIEL_OVERLAYS["fireworks"]
...
class TestFireworksAuxiliary:
    ...
    def test_client_sends_attribution_headers(self, monkeypatch):
        ...
        assert headers["HTTP-Referer"] == "https://athaniel-brain.nousresearch.com"
        assert headers["X-Title"] == "Athaniel Brain"
        assert kwargs["base_url"] == "https://api.fireworks.ai/inference/v1"

    def test_client_sends_athaniel_user_agent(self, monkeypatch):
        ...
        headers = kwargs.get("default_headers", {})
        assert headers["User-Agent"].startswith("AthanielAgent/")
