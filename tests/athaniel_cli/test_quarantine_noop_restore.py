...
``athaniel*.exe`` shims aside (``athaniel.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``athaniel`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("athaniel", "athaniel-brain", "athaniel-acp", "athaniel-gateway"):
...
# ---------------------------------------------------------------------------
# athaniel_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "athaniel.exe" in names, "athaniel.exe must be restored after a no-op install"
    assert "athaniel-acp.exe" in names
    assert "athaniel-gateway.exe" in names
    assert (scripts / "athaniel.exe").read_bytes() == b"MZ-old-athaniel"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "athaniel.exe").read_bytes() == b"MZ-new-athaniel"
...
# ---------------------------------------------------------------------------
# athaniel_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "athaniel.exe" in names, "athaniel.exe must be restored after a no-op recovery install"
    assert (scripts / "athaniel.exe").read_bytes() == b"MZ-old-athaniel"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "athaniel.exe").write_bytes(b"MZ-new-athaniel")
