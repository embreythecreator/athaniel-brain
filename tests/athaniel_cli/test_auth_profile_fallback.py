...
When ``ATHANIEL_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.athaniel/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.athaniel            (has its own auth.json fixture)
    * Profile     -> tmp_path/.athaniel/profiles/coder   (active, ATHANIEL_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Athaniel is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ATHANIEL_HOME != ~/.athaniel, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    athaniel_home = tmp_path / "classic"
    athaniel_home.mkdir()
