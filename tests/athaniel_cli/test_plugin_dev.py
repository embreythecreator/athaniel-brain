...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from athaniel_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``athaniel plugins doctor`` with no argument defaults to ``.``.

    Before the manifest guard, that copied the whole working directory into
    a temporary ATHANIEL_HOME — running it from ``$HOME`` copied the home
    directory, cloud-storage placeholders included.
    """
    import os
    import shutil

    from athaniel_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = athaniel_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_athaniel_home", lambda: athaniel_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a athaniel-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded athaniel-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("athaniel-plugin-doctor-*"))
