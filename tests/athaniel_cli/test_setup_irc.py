"""Tests for IRC gateway configuration via `athaniel setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Athaniel install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Athaniel install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#athaniel")
            monkeypatch.setenv("IRC_NICKNAME", "athaniel-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `athaniel setup gateway` experience with IRC present."""
