"""Tests for SIGHUP protection and stdout mirroring in ``athaniel update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``athaniel_cli/main.py``.  These exist so
that ``athaniel update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("ATHANIEL_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("athaniel_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== athaniel-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("ATHANIEL_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("athaniel_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import athaniel_cli.config as _cfg
        if hasattr(_cfg, "_ATHANIEL_HOME_CACHE"):
            _cfg._ATHANIEL_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "athaniel update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_athaniel_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "athaniel_cli.config.get_athaniel_home", _boom, raising=True
        )
