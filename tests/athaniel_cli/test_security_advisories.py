"""Tests for athaniel_cli.security_advisories.
...
import athaniel_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ATHANIEL_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full athaniel_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "athaniel_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
