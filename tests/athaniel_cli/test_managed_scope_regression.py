...
def athaniel_home(tmp_path, monkeypatch):
    ...
    # /etc/athaniel on the dev/CI box can't influence the test.
    monkeypatch.setenv("ATHANIEL_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(athaniel_home, monkeypatch):
...
def test_env_expansion_in_user_config(athaniel_home, monkeypatch):
    from athaniel_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_athaniel_dotenv(athaniel_home=str(home))
