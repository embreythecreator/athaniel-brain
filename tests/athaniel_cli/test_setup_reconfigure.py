...
- Bare `athaniel setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `athaniel setup --quick` runs the narrower "fill in missing items" flow.
- `athaniel setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("athaniel_cli.setup.ensure_athaniel_home", {}),
        ("athaniel_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("athaniel_cli.config.is_managed", {"return_value": False}),
        ("athaniel_cli.setup.load_config", {"return_value": {}}),
        ("athaniel_cli.setup.save_config", {}),
        ("athaniel_cli.setup.get_env_value", {"return_value": None}),
        ("athaniel_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("athaniel_cli.setup._print_setup_summary", {}),
        ("athaniel_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("athaniel_cli.setup.load_config", {"return_value": {}}),
        ("athaniel_cli.setup.save_config", {}),
        ("athaniel_cli.auth.get_active_provider", {"return_value": None}),
        ("athaniel_cli.setup.get_env_value", {"return_value": None}),
        ("athaniel_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `athaniel setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="athaniel_cli.setup.prompt_choice",
                quick="athaniel_cli.setup._run_quick_setup",
                model="athaniel_cli.setup.setup_model_provider",
                terminal="athaniel_cli.setup.setup_terminal_backend",
                agent="athaniel_cli.setup.setup_agent_settings",
                gateway="athaniel_cli.setup.setup_gateway",
                tools="athaniel_cli.setup.setup_tools",
            )
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from athaniel_cli.setup import run_setup_wizard
            from athaniel_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("athaniel_cli.setup.prompt_choice", {"return_value": 0}),
                first="athaniel_cli.setup._run_first_time_quick_setup",
            )
            from athaniel_cli.setup import run_setup_wizard
            from athaniel_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("athaniel_cli.setup.prompt_choice", {"return_value": 2}),
                blank="athaniel_cli.setup._run_blank_slate_setup",
            )
            from athaniel_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "athaniel_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["athaniel", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
