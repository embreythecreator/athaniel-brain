...
This rewrites files Athaniel does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``ATHANIEL_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("ATHANIEL_HOME", str(home / ".athaniel"))
    return home


class TestHappyPath:
    def test_athaniel_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
        ...
        assert "# Athaniel Brain" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/ATHANIEL_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Athaniel Brain" not in real.read_text(encoding="utf-8")
