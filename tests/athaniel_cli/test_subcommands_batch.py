...
def test_config_get_unset_subcommands_parse():
    """`athaniel config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="athaniel")
...
# ── deprecated `athaniel login` fails gracefully, not with argparse error ────
#
# `athaniel login` is a removed command; its handler (`login_command` in
# `athaniel_cli/auth.py`) prints a deprecation notice pointing at `athaniel auth` /
# `athaniel model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `athaniel --help`.
