"""Tests for athaniel_cli.gateway."""
...
import athaniel_cli.gateway as gateway


_BREAKAWAY_MARKER = "_ATHANIEL_GATEWAY_BREAKAWAY"
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /athaniel-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``ATHANIEL_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``ATHANIEL_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(*, replace, verbosity):
            assert "_ATHANIEL_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["ATHANIEL_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "ATHANIEL_HOME": str(tmp_path),
            "ATHANIEL_GATEWAY_DETACHED": "1",
            "ATHANIEL_GATEWAY_EXIT_DIAG": "1",
            "ATHANIEL_GATEWAY_MAX_STARTS": "0",
    ...
    from athaniel_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["ATHANIEL_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "ATHANIEL_HOME": str(tmp_path),
        "ATHANIEL_GATEWAY_EXIT_DIAG": "0",
        "ATHANIEL_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("ATHANIEL_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("athaniel_constants.is_container", lambda: True)
    monkeypatch.setattr("athaniel_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("athaniel_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_athaniel_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "athaniel-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test ATHANIEL_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="ATHANIEL_HOME=/home/alice/.athaniel"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_athaniel_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_athaniel_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "athaniel-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "athaniel_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "athaniel_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Athaniel Desktop opens (``athaniel serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time Athaniel Desktop opens
    (``athaniel serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `athaniel gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "athaniel-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "athaniel-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "athaniel-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "athaniel-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "athaniel_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """Athaniel_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        # hardcoded "AthanielGateway" would leave the guard dormant on every
        # standard install (task name is Athaniel_Gateway / Athaniel_Gateway_<p>).
        import athaniel_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "Athaniel_Gateway_testprof"
        )
        queried = []
        ...
        assert result is False
        assert killed_pids == []
        assert queried == ["Athaniel_Gateway_testprof"]
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(self, monkeypatch):
        ...
        assert gateway._windows_scheduled_task_running("AthanielGateway") is False
        assert gateway._windows_scheduled_task_supervises("AthanielGateway") is False
        assert gateway._windows_scheduled_task_state("AthanielGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("Athaniel_Gateway") is expected, state
            assert gateway._windows_scheduled_task_running("Athaniel_Gateway") is (state == "Running")
...
def test_find_windows_gateway_services_maps_verified_pid_tree(monkeypatch):
    ...
    fake_psutil = SimpleNamespace(
        win_service_iter=lambda: [
            FakeService("AthanielGateway", 100),
...
def test_find_windows_gateway_services_rejects_transitional_ancestor(monkeypatch):
    ...
    class FakeService:
        def as_dict(self):
            return {"name": "AthanielGateway", "pid": 100, "status": "stop_pending"}
