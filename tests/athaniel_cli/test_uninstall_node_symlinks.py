"""Tests for athaniel_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ATHANIEL_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Athaniel-managed node dir.
"""
...
def _make_athaniel_node(athaniel_home: Path) -> Path:
    """Create a fake $ATHANIEL_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = athaniel_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    athaniel_home = fake_home / ".athaniel"
    _make_athaniel_node(athaniel_home)
    ...
    removed = uninstall.remove_node_symlinks(athaniel_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    athaniel_home = fake_home / ".athaniel"
    node_bin = _make_athaniel_node(athaniel_home)
