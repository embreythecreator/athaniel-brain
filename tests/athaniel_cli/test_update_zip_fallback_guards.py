...
install``. A locked ``athaniel.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(athaniel_main, "_is_windows", lambda: True)
    exc = _cpe([r"C:\venv\Scripts\uv.exe", "pip", "install", "-e", "."])
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M athaniel_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(athaniel_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
    ...
    with patch("urllib.request.urlretrieve") as download:
        with pytest.raises(SystemExit) as exc_info:
            athaniel_main._update_via_zip(SimpleNamespace(branch=None))
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.athaniel-update-staging/")
    assert is_artifact("?? cli.py.athaniel-update-staging")
    assert is_artifact("?? tools.athaniel-update-old/")
    ...
    assert not is_artifact(" M athaniel_cli/update_cmd.py")
    assert not is_artifact("?? notes.athaniel-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.athaniel-update-staging/\n?? cli.py.athaniel-update-old\n"),
    )
    assert (
