"""``athaniel`` must survive git operations on the checkout (launcher layout).

The Windows ``athaniel`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``ATHANIEL_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``athaniel-brain\\bin``) was swept by ``athaniel update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``athaniel`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``athaniel update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``athaniel_constants.venv_bin_dir``), so these
...
from athaniel_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: ATHANIEL_HOME/athaniel-brain/venv/Scripts + launchers."""
    home = tmp_path / "athaniel"
    root = home / "athaniel-brain"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under ATHANIEL_HOME gains nothing anywhere."""
    home = tmp_path / "athaniel-home"
    home.mkdir()
    monkeypatch.setenv("ATHANIEL_HOME", str(home))
    root = tmp_path / "src" / "athaniel-brain"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``athaniel -p <name>`` ATHANIEL_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("ATHANIEL_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `athaniel update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "athaniel.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "athaniel.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\athaniel\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/ATHANIEL/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``athaniel update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/athaniel.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/athaniel.exe is not gitignored — athaniel update's autostash "
