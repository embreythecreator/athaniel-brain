...
The notice prints the most recent curator run summary on `athaniel update`,
...
session start) so users normally never see the rename map. `athaniel update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(athaniel_main)
