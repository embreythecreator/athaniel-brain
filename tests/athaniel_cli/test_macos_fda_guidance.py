...
reports the state and prints the one-switch setup; `athaniel setup` surfaces
...
from athaniel_cli.setup import _print_macos_fda_tip
...
class TestSetupFdaTip:
    ...
    def test_tip_printed_when_denied(self, monkeypatch, tmp_path):
        ...
        assert "Full Disk Access" in out
        assert "Privacy_AllFiles" in out
        assert "survives every Athaniel update" in out
