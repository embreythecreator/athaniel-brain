"""Cross-process update mutual exclusion (``athaniel_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``athaniel
...
dashboard-spawned ``athaniel update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".athaniel-update-in-progress"


def test_marker_path_follows_process_athaniel_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* ATHANIEL_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".athaniel-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``athaniel update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Athaniel is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``athaniel-setup`` under ``~/.athaniel`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("athaniel_cli.update_lock._pid_alive", lambda pid: True)
