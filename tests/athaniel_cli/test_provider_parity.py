...
the SAME provider universe as ``athaniel model`` (the CLI/TUI picker).
...
def test_every_athaniel_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `athaniel model` universe."""
    ...
    assert not missing, (
        "providers shown in `athaniel model` but not configurable in the desktop "
