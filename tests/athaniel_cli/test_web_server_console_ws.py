"""Dashboard Athaniel Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_athaniel_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from athaniel_cli.console_engine import ConsoleResult, AthanielConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(AthanielConsoleEngine, "execute", slow_execute)
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert complete["prompt"] == "athaniel> "
