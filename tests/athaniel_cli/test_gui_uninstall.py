"""Tests for athaniel_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import athaniel_cli.gui_uninstall as gu


def _make_agent(athaniel_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = athaniel_home / "athaniel-brain"
    (agent_root / "athaniel_cli").mkdir(parents=True)
    (agent_root / "athaniel_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(athaniel_home: Path) -> None:
    """Create the source-built GUI artifacts a `athaniel desktop` run produces."""
    desktop = athaniel_home / "athaniel-brain" / "apps" / "desktop"
    ...
    (athaniel_home / "athaniel-brain" / "node_modules").mkdir(parents=True)
    (athaniel_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(athaniel_home: Path) -> None:
    (athaniel_home / "config.yaml").write_text("x: 1\n")
    (athaniel_home / ".env").write_text("KEY=secret\n")
    (athaniel_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(athaniel_home)
    _make_gui_build(athaniel_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(athaniel_home)
    ...
    assert summary["athaniel_home"] == str(athaniel_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `athaniel desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    athaniel_home = tmp_path / ".athaniel"
    _make_agent(athaniel_home)
    icon = lde.icon_path(athaniel_home / "athaniel-brain")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(athaniel_home)
    ...
    assert lde.icon_path(athaniel_home / "athaniel-brain").exists()
    # The agent itself survives a GUI uninstall.
    assert (athaniel_home / "athaniel-brain" / "athaniel_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".athaniel")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import athaniel_cli.uninstall as uninstall
