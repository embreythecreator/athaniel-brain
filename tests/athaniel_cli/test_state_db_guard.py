...
from athaniel_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ATHANIEL_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def athaniel_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("athaniel_cli.config") or mod == "athaniel_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, athaniel_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, athaniel_home, capsys, monkeypatch):
