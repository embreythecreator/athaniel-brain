...
BOOK_FILES = {
    ...
    "athaniel_cli/__init__.py": '__version__ = "0.16.9"\n',
}
...
def test_commit_bookkeeping_and_state_clear(tmp_path, monkeypatch):
    ...
    assert '__version__ = "0.18.0"' in show("athaniel_cli/__init__.py")
    assert driver.state(repo) is None
...
def test_commit_version_bumps_from_ours_head_not_merged(tmp_path, monkeypatch):
    ...
    # independent Athaniel line must still bump from OUR previous HEAD
    ...
    init = subprocess.run(["git", "-C", repo, "show", f"{oid}:athaniel_cli/__init__.py"],
                          capture_output=True, text=True, check=True).stdout
...
def test_cli_commit_passes_skip_verify(tmp_path, monkeypatch, capsys):
    from athaniel_cli import main as m
    from athaniel_cli.absorb import driver as d
    repo = _mkrepo(tmp_path)
    _git(repo, "remote", "add", "upstream", "https://example.invalid/x.git")
    monkeypatch.chdir(repo)
    seen = {}
