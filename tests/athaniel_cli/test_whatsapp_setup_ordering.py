...
Before the fix, ``athaniel whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``athaniel gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(athaniel_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from athaniel_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("athaniel_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``athaniel whatsapp`` with an existing paired session and
