"""Tests for athaniel_cli.cron command handling."""
...
from athaniel_cli import cron as cron_cli
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("athaniel_cli.gateway.find_gateway_pids", lambda: [])
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "athaniel_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`athaniel cron run` must not report 'failed' when the run was dispatched
