...
Athaniel.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors athaniel_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Athaniel.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("athaniel_cli.main._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Athaniel.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("athaniel_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("athaniel_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `athaniel desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Athaniel.exe cannot launch
    must exit nonzero (so athaniel-setup's retry-once kicks in) and must restore
    ...
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "Athaniel.exe", PE_AMD64)

    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    pack_ok = subprocess.CompletedProcess(["npm", "run", "pack"], 0)

    with patch("athaniel_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("athaniel_cli.main._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("athaniel_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         patch("athaniel_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("athaniel_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("athaniel_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("athaniel_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("athaniel_cli.main.subprocess.run", return_value=pack_ok), \
