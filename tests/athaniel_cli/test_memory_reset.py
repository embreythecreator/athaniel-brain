"""Tests for the `athaniel memory reset` CLI command.
...
- Profile-scoped reset (uses ATHANIEL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ATHANIEL_HOME with memory files."""
    athaniel_home = tmp_path / ".athaniel"
    memories = athaniel_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAthaniel repo is at ~/.athaniel/athaniel-brain\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `athaniel memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        athaniel_home = tmp_path / ".athaniel"
        (athaniel_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))
