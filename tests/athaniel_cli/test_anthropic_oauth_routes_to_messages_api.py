...
* ``athaniel_cli.providers.determine_api_mode`` correctly mapped
  ``api.anthropic.com`` to ``anthropic_messages``.
* ``athaniel_cli.runtime_provider._detect_api_mode_for_url`` did NOT, so
...
class TestExplicitRuntimeForAnthropic:
    ...
    Exercised whenever the user (or a Athaniel subcommand) passes an
...
class TestPoolEntryForAnthropic:
    ...
    has added an OAuth credential via ``athaniel auth add anthropic
