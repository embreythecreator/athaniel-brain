"""Tests for ``athaniel peer`` — cross-machine bot-to-bot DMs."""
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "athaniel.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``athaniel peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``athaniel peer
