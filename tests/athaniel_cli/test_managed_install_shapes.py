...
def athaniel_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ATHANIEL_MANAGED", raising=False)
    return home
...
def test_env_var_names_the_managing_system(athaniel_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("ATHANIEL_MANAGED", raising=False)
    else:
        monkeypatch.setenv("ATHANIEL_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    athaniel_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the ATHANIEL_MANAGED of the service."""
    (athaniel_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("ATHANIEL_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(athaniel_home, monkeypatch):
    (athaniel_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("ATHANIEL_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    athaniel_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("ATHANIEL_MANAGED", managed_value)
    ...
    # `athaniel update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "athaniel update"
...
def test_unmanaged_install_offers_no_update_command(athaniel_home, monkeypatch):
    monkeypatch.delenv("ATHANIEL_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(athaniel_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = athaniel_home / ".managed"
