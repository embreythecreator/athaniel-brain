"""Tests for ``athaniel doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from athaniel_cli import doctor_live
from athaniel_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `athaniel doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("athaniel_cli.doctor.ATHANIEL_HOME", tmp_path / "home")
        monkeypatch.setattr("athaniel_cli.doctor.PROJECT_ROOT", tmp_path / "root")
