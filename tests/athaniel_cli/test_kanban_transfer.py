"""Tests for kanban board export / import (``athaniel_cli.kanban_transfer``).
...
from athaniel_cli import kanban_transfer as kt
from athaniel_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ATHANIEL_KANBAN_HOME", str(root))
        for var in ("ATHANIEL_KANBAN_DB", "ATHANIEL_KANBAN_WORKSPACES_ROOT",
                    "ATHANIEL_KANBAN_ATTACHMENTS_ROOT", "ATHANIEL_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Athaniel"):
