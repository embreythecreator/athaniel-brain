...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ATHANIEL_HOME with .container-mode file."""
    athaniel_home = tmp_path / ".athaniel"
    athaniel_home.mkdir()
    monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))
    monkeypatch.delenv("ATHANIEL_DEV", raising=False)

    container_mode = athaniel_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=athaniel-brain\n"
        "exec_user=athaniel\n"
        "athaniel_bin=/data/current-package/bin/athaniel\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("athaniel_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "athaniel-brain"
    assert info["exec_user"] == "athaniel"
    assert info["athaniel_bin"] == "/data/current-package/bin/athaniel"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from athaniel_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "athaniel"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "athaniel-brain" in cmd
    assert "/data/current-package/bin/athaniel" in cmd
    assert "chat" in cmd
