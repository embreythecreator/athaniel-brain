...
def main_mod(monkeypatch):
    import athaniel_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_athaniel_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from athaniel_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """athaniel -z bypasses AthanielCLI, but recall still needs SessionDB."""
    from athaniel_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "athaniel_state", mod("athaniel_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "athaniel_cli.config",
        mod("athaniel_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "athaniel_cli.models",
        mod("athaniel_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "athaniel_cli.tools_config",
        mod("athaniel_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ATHANIEL_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/athaniel-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["ATHANIEL_MODEL"] == "nous/athaniel-test"
    assert env["ATHANIEL_INFERENCE_MODEL"] == "nous/athaniel-test"
    assert env["ATHANIEL_TUI_PROVIDER"] == "nous"
    assert env["ATHANIEL_INFERENCE_PROVIDER"] == "nous"
    assert env["ATHANIEL_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ATHANIEL_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("athaniel-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_athaniel_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("ATHANIEL_TUI_DIR", raising=False)
