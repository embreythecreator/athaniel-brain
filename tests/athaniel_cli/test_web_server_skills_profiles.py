...
profile's ATHANIEL_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedHubActions:
    def test_hub_install_spawns_with_profile_flag(
        ...
        """Hub installs must go through a fresh ``athaniel -p <profile>``
        ...
        monkeypatch.setattr(web_server, "_spawn_athaniel_action", _fake_spawn)
        resp = client.post(
