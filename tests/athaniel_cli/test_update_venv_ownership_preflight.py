"""Venv ownership preflight for ``athaniel update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo athaniel`` contains root-owned files
(e.g. ``site-packages/athaniel_brain-*.dist-info/INSTALLER``). A later normal
``athaniel update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/athaniel`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "athaniel").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "athaniel_brain-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "athaniel_brain-1.0.0.dist-info" / "INSTALLER"
    )
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    athaniel_bin = str(venv / "bin" / "athaniel")
    real_uid = update_cmd._path_uid
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == athaniel_bin else real_uid(p),
    )
    with pytest.raises(SystemExit) as exc:
        update_cmd._refuse_update_if_venv_foreign_owned(tmp_path)
    assert exc.value.code == 1
    out = capsys.readouterr().out
    assert athaniel_bin in out
