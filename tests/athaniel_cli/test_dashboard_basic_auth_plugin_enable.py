...
from athaniel_cli.dashboard_auth import clear_providers, list_providers
from athaniel_cli.plugins import PluginManager, discover_plugins
...
class TestBasicProviderLoadsAfterUnblock:
    def test_disabled_basic_blocks_registration(self, athaniel_home, monkeypatch):
    ...
    def test_unblock_then_rediscover_registers_provider(
        self, athaniel_home, monkeypatch,
    ):
        password_hash = basic_plugin.hash_password("hunter2")
        cfg = {
            "dashboard": {
        ...
        _write_config(athaniel_home, cfg)
