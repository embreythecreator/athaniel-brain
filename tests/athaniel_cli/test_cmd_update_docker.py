"""Tests for ``athaniel update`` / ``--check`` inside the Docker container.
...
fix, ``athaniel update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Athaniel, not an update to the running container, so the message
...
from athaniel_cli.main import _cmd_update_check, cmd_update
...
@patch("athaniel_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``athaniel update`` inside Docker → friendly message + exit 2, no git calls.
    ...
    assert "docker pull embreythecreator/athaniel-brain:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    """
    from athaniel_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker pull embreythecreator/athaniel-brain:latest" in msg
    ...
    assert "ATHANIEL_HOME" in msg or "/opt/data" in msg, (
