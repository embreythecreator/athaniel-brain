"""Tests for athaniel_cli configuration management."""
...
class TestGetAthanielHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ATHANIEL_HOME", None)
            home = get_athaniel_home()
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\athaniel — see
                # athaniel_constants._get_platform_default_athaniel_home.
                ...
                assert home == base / "athaniel"
            else:
                assert home == Path.home() / ".athaniel"


class TestEnsureAthanielHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from athaniel_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Athaniel Brain, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``athaniel logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from athaniel_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from athaniel_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHANIEL_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHANIEL_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHANIEL_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ATHANIEL_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("athaniel_cli.config.sanitize_env_file", return_value=2),
            patch(
                "athaniel_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("athaniel_cli.config.read_raw_config", return_value={}),
            patch("athaniel_cli.config.get_missing_env_vars", return_value=[]),
            patch("athaniel_cli.config.get_missing_config_fields", return_value=[]),
            patch("athaniel_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_removed_tavily_var_not_in_env_vars_by_version(self):
        ...
        from athaniel_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ATHANIEL_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `athaniel setup`) lets a
        ...
        via config.yaml; ATHANIEL_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from athaniel_cli.config import OPTIONAL_ENV_VARS
        assert "ATHANIEL_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `athaniel setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `athaniel setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_athaniel_home(self):
        from athaniel_cli.config_migrations import support_floor_message
        from athaniel_constants import display_athaniel_home

        msg = support_floor_message()
        assert f"{display_athaniel_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from athaniel_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Athaniel runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Athaniel
    process on next startup via the dotenv → ``os.environ`` chain in
    ``athaniel_cli/env_loader.py``.
    ...
    def _athaniel_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ATHANIEL_SPOTIFY_CLIENT_ID",
            "ATHANIEL_QWEN_BASE_URL",
            "ATHANIEL_MAX_ITERATIONS",
        ],
    )
    def test_athaniel_integration_keys_still_writable(self, allowed_key):
        """``ATHANIEL_*`` overall is NOT blocked.
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from athaniel_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"ATHANIEL_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["ATHANIEL_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Athaniel_Yolo_Mode", "ATHANIEL_YOLO_MODE"),
            ("Athaniel_Optional_Mcps", "ATHANIEL_OPTIONAL_MCPS"),
            ("Athaniel_Copilot_Acp_Command", "ATHANIEL_COPILOT_ACP_COMMAND"),
            ("Athaniel_Copilot_Acp_Args", "ATHANIEL_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from athaniel_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from athaniel_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from athaniel_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ATHANIEL_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: athaniel\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "athaniel"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import athaniel_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`athaniel config set/unset` must exit cleanly (no traceback) when the
