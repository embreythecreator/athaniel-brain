...
subparser, placing the flag BEFORE the subcommand (e.g. 'athaniel --yolo chat')
...
def _build_parser():
    """Build the athaniel argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("athaniel_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "athaniel_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets ATHANIEL_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `athaniel gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: athaniel_cli.main is a very heavy
    # import (previously 11 separate `python -m athaniel_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `athaniel <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `athaniel -t web chat` silently dropped the
    ...
    Uses the real `athaniel_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `athaniel <flag> <value> chat`:\n  "
