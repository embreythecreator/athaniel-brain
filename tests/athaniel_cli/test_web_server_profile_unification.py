...
profile switcher can target any profile's ATHANIEL_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via ATHANIEL_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_athaniel_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_athaniel_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import athaniel_cli.mcp_config as mcp_config
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same ATHANIEL_HOME the rest
        ...
        monkeypatch.setattr(
            "athaniel_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_athaniel_home()))
            return None
        ...
        assert resp.json()["athaniel_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_athaniel_home(self, isolated_profiles, monkeypatch):
        import athaniel_cli.web_server as web_server

        monkeypatch.setattr(
            "athaniel_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["ATHANIEL_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "ATHANIEL_TUI_GATEWAY_URL" not in env
...
class TestProfileScopedAudio:
    ...
    def test_transcribe_runs_inside_target_profile_home(
        ...
        def _fake_transcribe(path):
            ...
            seen["home"] = str(get_athaniel_home())
