...
Covers:
- Defense 1: gateway stop/restart refuse when _ATHANIEL_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "athaniel gateway uninstall",
        "athaniel  gateway  restart",         # double spaces
        "Athaniel Gateway Restart".lower(),   # mixed case handled
        "ATHANIEL GATEWAY RESTART",           # uppercase
    ])
    def test_athaniel_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.athaniel.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.athaniel/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l athaniel-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.athaniel.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.athaniel.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.athaniel.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.athaniel/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.athaniel.gateway-apollo:/a.plist' "
            "'ai.athaniel.gateway-cronus:/c.plist' 'ai.athaniel.gateway-plutus:/p.plist' "
            "'ai.athaniel.gateway:/Users/botuser/Library/LaunchAgents/ai.athaniel.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated athaniel note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.athaniel.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.athaniel.gateway',
        "launchctl kick'start' -k gui/501/ai.athaniel.gateway",
        "launchctl kick\\start -k gui/501/ai.athaniel.gateway",
        'launchctl "kickstart" -k gui/501/ai.athaniel.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.athaniel.gateway',
        "launchctl dis\\able gui/501/ai.athaniel.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.athaniel."gateway"',
        # Same class on the systemctl and athaniel-CLI branches.
        'systemctl re"start" athaniel-gateway',
        'athaniel gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.athaniel.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.athaniel.update-checker',
        'systemctl re"start" athaniel-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "athaniel cron list",
        "athaniel update",
        "athaniel config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `athaniel gateway start` is benign — starting a gateway from inside a
        ...
        "athaniel gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway athaniel
        # services must not be falsely blocked (the old `.*athaniel` matched any
        # athaniel token).
        "launchctl unload ai.athaniel.update-checker.plist",
        "launchctl restart ai.athaniel.daemon",
        ...
        "systemctl restart athaniel-meta.service",
        "systemctl restart athaniel-cron-helper",
        ...
        "echo after the athaniel gateway restarted cleanly",
        "the athaniel gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "athaniel skill view gateway-notes && echo athaniel gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `athaniel` is a path component
        # there, not a command.
        "cat '/docs/athaniel gateway restart-notes.md'",
        "less /home/user/notes/athaniel gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "athaniel gateway restart",
        "athaniel gateway restart; echo done",
        "athaniel gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;athaniel gateway restart",
        "true && athaniel gateway stop",
        "echo $(athaniel gateway restart)",
        "echo `athaniel gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.athaniel.gateway"])',
        'subprocess.run(["athaniel", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.athaniel.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["athaniel", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: athaniel gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `athaniel -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ATHANIEL_HOME/ATHANIEL_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ATHANIEL_PROFILE", "zeus")
        monkeypatch.delenv("ATHANIEL_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "athaniel -p zeus gateway stop",
        "athaniel -p zeus gateway restart",
        "athaniel --profile zeus gateway restart",
        "athaniel --profile zeus gateway stop",
        "athaniel --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "athaniel -v -p zeus gateway restart",
        "athaniel -p zeus -v gateway restart",
        "athaniel --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "athaniel -p venus gateway stop",
        "athaniel -p venus gateway restart",
        "athaniel --profile venus gateway restart",
        "athaniel --profile=venus gateway stop",
        "athaniel -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "athaniel -p zeus gateway start",
        "athaniel -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("athaniel gateway restart")
        assert _contains_gateway_lifecycle_command("athaniel gateway stop")

    def test_athaniel_home_derived_profile(self, monkeypatch):
        # Without ATHANIEL_PROFILE the guard falls back to the ATHANIEL_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("ATHANIEL_PROFILE", raising=False)
        monkeypatch.delenv("ATHANIEL_PROFILE_NAME", raising=False)
        import athaniel_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("athaniel -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("athaniel -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("athaniel -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("athaniel gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_athaniel_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade athaniel then run athaniel gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_athaniel_gateway_once.sh). The script must live under
        # ATHANIEL_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path / ".athaniel"))
        scripts_dir = tmp_path / ".athaniel" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ATHANIEL_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ATHANIEL_GATEWAY=1.

    Issue #37453: systemctl --user restart athaniel-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart athaniel-gateway",
        "systemctl --user restart athaniel-gateway",
        "systemctl stop athaniel-gateway.service",
        "athaniel gateway restart",
        "athaniel gateway uninstall",
        "launchctl kickstart gui/501/ai.athaniel.gateway",
        "launchctl bootout gui/501/ai.athaniel.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.athaniel.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.athaniel/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.athaniel.gateway.restart-once.plist",
        "pkill -f athaniel.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart athaniel-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.athaniel.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-athaniel label: label-independent detection is the point
        # (#62891 second reproduction used `ai.athaniel.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.athaniel.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ATHANIEL_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ATHANIEL_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_ATHANIEL_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="athaniel gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["athaniel gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.athaniel.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.athaniel.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-athaniel systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run athaniel gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("athaniel gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeathaniel gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ATHANIEL_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("athaniel gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/athaniel\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 athaniel gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_athaniel_home()` falls back to Path.home(), which raises
        RuntimeError when neither ATHANIEL_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart athaniel-gateway' .",
        "grep -rn 'athaniel gateway restart' ./logs",
        "rg 'athaniel gateway restart' ../archive",
        "grep -c 'systemctl stop athaniel-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM athaniel_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell athaniel gateway restart"',
        'sqlite3 ./db ".system systemctl restart athaniel-gateway"',
        'psql ./x -c "\\! systemctl restart athaniel-gateway"',
        "grep -r 'athaniel gateway restart' . | sh",
        "grep -r 'athaniel gateway restart' ./logs | bash",
        "grep -r 'athaniel gateway restart' . | sudo sh",
        "grep -r 'x' . ; athaniel gateway restart",
        "grep -r 'x' . && systemctl restart athaniel-gateway",
        'grep -r "$(athaniel gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart athaniel-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell athaniel gateway restart"',
        'sqlite3 db ".system systemctl restart athaniel-gateway"',
        'psql -c "\\! systemctl restart athaniel-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    athaniel_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run athaniel gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path / ".athaniel"))
        (tmp_path / ".athaniel").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run athaniel gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop athaniel-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart athaniel-gateway' /var/log/syslog",
        "rg 'athaniel gateway restart' /home/user/.athaniel/logs/",
        "journalctl -u athaniel-gateway --grep 'systemctl restart athaniel-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'athaniel_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell athaniel gateway restart"',
        'psql -c "\\! systemctl restart athaniel-gateway"',
        "grep 'systemctl restart athaniel-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart athaniel-gateway",
        'grep "$(systemctl restart athaniel-gateway)" f',
        "grep 'restart' log; systemctl restart athaniel-gateway",
        'sqlite3 db "SELECT 1"; athaniel gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "athaniel gateway restart",
        "sudo systemctl stop athaniel-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart athaniel-gateway%'\""
        )
