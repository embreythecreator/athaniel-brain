...
``athaniel_cli.config`` while the older ``config.py`` does not export them.
Construction of ``AthanielCLI`` then dies before the agent-setup mixin can
print ``partial_update_hint``. ``cmd_chat`` is the load-bearing catch:
bare ``athaniel`` and ``athaniel chat`` (including the fast-chat launch path)
all go through it.
"""
...
from athaniel_constants import emit_partial_update_hint, partial_update_hint
...
def _missing_config_name_error(name: str = "resolve_turn_limit") -> ImportError:
    exc = ImportError(
        f"cannot import name '{name}' from 'athaniel_cli.config'"
    )
    exc.name = "athaniel_cli.config"
    return exc
...
def test_emit_hint_for_missing_resolve_turn_limit():
    ...
    assert "athaniel update" in text
...
def test_emit_hint_for_missing_split_model_config_default():
    ...
    buf = io.StringIO()
    assert emit_partial_update_hint(exc, file=buf) is True
    assert "athaniel update" in buf.getvalue()
