"""Tests for the post-update *import* guard in ``athaniel update``.
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not athaniel_main's. Both modules expose
    ...
    syntax_ok, _, _ = athaniel_main._validate_critical_files_syntax(tmp_path)
    ...
    ok, module, error = athaniel_main._validate_critical_modules_import(tmp_path)
    assert ok is False
...
def test_import_guard_passes_on_consistent_tree(monkeypatch, tmp_path):
    ...
    assert athaniel_main._validate_critical_modules_import(tmp_path) == (True, None, None)
...
def test_import_guard_ignores_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, _, _ = athaniel_main._validate_critical_modules_import(tmp_path)
    assert ok is True
...
def test_import_guard_can_report_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, module, error = athaniel_main._validate_critical_modules_import(
...
def test_import_guard_does_not_accept_forged_static_marker(monkeypatch, tmp_path):
    ...
    (tmp_path / "consumer.py").write_text(
        "import os, sys\n"
        "sys.stdout.write('__ATHANIEL_IMPORT_HEALTH__[]')\n"
        "sys.stdout.flush()\n"
        "os._exit(7)\n"
    )
    monkeypatch.setattr(update_cmd, "_UPDATE_CRITICAL_MODULES", ("consumer",))
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("athaniel update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``athaniel update`` can run under a different Python than the install's.
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "athanielx"])
...
@pytest.mark.parametrize("modname", ["tools.todo_tool", "agent.context_compressor",
                                     "athaniel_constants", "athaniel_cli.config", "cli"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `athanielx`
    (third-party) matched the probe's loose `startswith("athaniel")`. A user
    ...
    """
    from athaniel_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "athanielx"):
