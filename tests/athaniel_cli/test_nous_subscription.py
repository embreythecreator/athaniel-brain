...
from athaniel_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import athaniel_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "athaniel_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `athaniel model` overwrote it to `web.backend: nous`.
    """
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "athaniel_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `athaniel tools`
# ---------------------------------------------------------------------------
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("athaniel_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "athaniel_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_athaniel_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Athaniel Node dir even when
    ...
    monkeypatch.setattr(
        "athaniel_constants.with_athaniel_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "athaniel_constants.agent_browser_runnable",
