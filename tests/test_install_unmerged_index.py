...
The ``athaniel update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m athaniel_cli.main gateway run`` — image name
    ``pythonw``, not ``athaniel.exe`` — so the ``taskkill /IM athaniel.exe`` guard
    ...
    # The athaniel.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM athaniel.exe' in text
