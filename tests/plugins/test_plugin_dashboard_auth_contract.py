...
plugin SDK (``window.__ATHANIEL_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-Athaniel-Session-Token`` header) and gated OAuth
(``athaniel_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__ATHANIEL_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__ATHANIEL_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + athaniel-achievements are bundled today; assert at least one is
