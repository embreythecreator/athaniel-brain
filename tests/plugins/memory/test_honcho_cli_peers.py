"""Regression tests for #76414: `athaniel honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("athaniel.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("athaniel_work"). The lookup
...
a real honcho.json (temp ATHANIEL_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "athaniel": {"peerName": "alice", "aiPeer": "athaniel"},
            "athaniel_work": {"peerName": "alice", "aiPeer": "athaniel"},
            "athaniel_my_profile": {"peerName": "bob", "aiPeer": "athaniel"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "athaniel_cli.profiles.list_profiles",
        ...
        assert host == "athaniel_work"  # not "athaniel.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'athaniel_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("athaniel.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["athaniel_work"]
        cfg["hosts"]["athaniel.work"] = {"peerName": "carol", "aiPeer": "athaniel"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "athaniel_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'athaniel.work' (or '(not set)')."""
        monkeypatch.setattr(
            "athaniel_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "athaniel.work" not in out
        ...
        assert "alice" in work_line and "athaniel" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "athaniel.new" not in out  # well-formed key, no dot-form leak
