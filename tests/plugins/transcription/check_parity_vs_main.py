...
the legacy ``ATHANIEL_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "athaniel-brain-main"
...
assert (PR_DIR / "tools" / "transcription_tools.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a athaniel-brain checkout"
)
...
# Isolated ATHANIEL_HOME so the config write is hermetic.
home = tempfile.mkdtemp()
os.environ["ATHANIEL_HOME"] = home
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'athaniel-brain-main' checkout pinned to "
