...
binary download are exercised manually by `athaniel secrets bitwarden
...
def athaniel_home(tmp_path, monkeypatch):
    """Point Athaniel at an isolated home directory."""
    home = tmp_path / ".athaniel"
    home.mkdir()
    monkeypatch.setenv("ATHANIEL_HOME", str(home))
    # Some modules cache get_athaniel_home; clear if needed.
    import athaniel_constants
    if hasattr(athaniel_constants, "_ATHANIEL_HOME_CACHE"):
        athaniel_constants._ATHANIEL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(athaniel_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from athaniel_cli.env_loader import _apply_external_secret_sources
