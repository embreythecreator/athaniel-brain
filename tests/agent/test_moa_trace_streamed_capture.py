...
path via ``athaniel chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ATHANIEL_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ATHANIEL_HOME at a temp dir and turn moa.save_traces on."""
    athaniel_home = tmp_path / ".athaniel"
    athaniel_home.mkdir()
    monkeypatch.setenv("ATHANIEL_HOME", str(athaniel_home))

    # save_moa_turn reads config via athaniel_cli.config.load_config; stub it to
    ...
    return athaniel_home / "moa-traces"
