"""Regression tests for the codex_app_server → Athaniel UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_athaniel_tools_mcp_server_emits_bare_tool_name(self):
        """The athaniel-tools MCP server wraps Athaniel' own tools for codex;
        ...
        mcp.athaniel-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "athaniel-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "athaniel-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "athaniel agent docs"
        assert calls[0].args[3] == {"query": "athaniel agent docs"}
