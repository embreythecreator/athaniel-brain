...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_athaniel_home``,
    ``get_default_athaniel_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``ATHANIEL_HOME`` and no resolver mocks.
    """

    def test_real_athaniel_home_under_profiles_renders_correct_paths(
        ...
        assert get_athaniel_home() == profile_home
        assert get_default_athaniel_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Athaniel profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """ATHANIEL_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".athaniel"
        ...
        assert "Active Athaniel profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "ATHANIEL_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "ATHANIEL_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_athaniel_home", lambda: Path("/athaniel"))

    # Production renders this as str(get_athaniel_home()) + "/profiles/<name>/",
    # and str(Path("/athaniel")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/athaniel".
    _home_str = str(Path("/athaniel"))
    expected_profile = (
    ...
    with (
        ...
        patch("athaniel_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("athaniel_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under ATHANIEL_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from athaniel_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("athaniel_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from athaniel_time import now as athaniel_now
        sid = athaniel_now().strftime("%Y%m%d_%H%M%S_fresh")
