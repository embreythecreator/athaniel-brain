...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ATHANIEL_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="athaniel_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".athaniel"))
    prev_home = os.environ.get("ATHANIEL_HOME")
    os.environ["ATHANIEL_HOME"] = os.path.join(test_home, ".athaniel")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("athaniel_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["ATHANIEL_HOME"] = prev_home
