...
thread that did not bind the ATHANIEL_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_athaniel_home()``,
so an unbound build thread fell back to ``~/.athaniel`` (default) and leaked
default's full skills index + "Active Athaniel profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient ATHANIEL_HOME, on a
    ...
    def build():
        # No set_athaniel_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_athaniel_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    athaniel ROOT (get_default_athaniel_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: ATHANIEL_HOME env points at the profile dir itself.
    monkeypatch.setenv("ATHANIEL_HOME", str(bot_home))
...
def test_profile_name_correct_when_ambient_is_another_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATHANIEL_HOME", str(other_home))
