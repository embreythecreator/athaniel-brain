...
``athaniel_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``ATHANIEL_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "athaniel_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "athaniel_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / athaniel_subprocess_env /
