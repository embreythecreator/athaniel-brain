...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"athaniel-brain-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "athaniel-brain-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "athaniel-brain-dev"
...
# `athaniel curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `athaniel curator pin <umbrella>` in the rename block so
