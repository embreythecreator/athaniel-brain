...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("athaniel_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated ATHANIEL_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="athaniel_api_content_")
