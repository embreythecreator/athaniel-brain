...
def athaniel_home(tmp_path, monkeypatch):
    """Real on-disk ATHANIEL_HOME so ``load_pool()`` re-reads what it persisted."""
...
def test_persisted_claude_code_row_carries_no_token_material(
    athaniel_home, claude_credentials
):
    ...
    rows = [r for r in _persisted_rows(athaniel_home) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(athaniel_home, claude_credentials):
...
def test_empty_oauth_entry_is_never_leased(athaniel_home, claude_credentials):
...
def test_athaniel_pkce_row_still_syncs_from_the_pool_store(monkeypatch):
    """The borrowed-source refusal must not disable pool-owned adoption.

    ``athaniel_pkce`` *is* pool-owned, so its persisted row keeps its tokens and
