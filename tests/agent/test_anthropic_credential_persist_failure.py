...
``~/.athaniel/.anthropic_oauth.json`` for ``athaniel_pkce`` ones.  Those singletons
...
def athaniel_home(tmp_path, monkeypatch):
    """Real on-disk ATHANIEL_HOME so ``load_pool()`` re-reads what we persisted."""
    ...
    monkeypatch.setattr(
        "athaniel_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_athaniel_oauth_writer_raises_instead_of_swallowing(athaniel_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_athaniel_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    athaniel_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
# ---------------------------------------------------------------------------
# Pool path: athaniel_pkce
# ---------------------------------------------------------------------------


def test_pool_athaniel_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "athaniel_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
