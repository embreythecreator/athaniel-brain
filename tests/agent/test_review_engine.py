...
def test_load_review_credentials_cfg_missing_section(monkeypatch):
    monkeypatch.setattr(
        "athaniel_cli.config.load_config_readonly", lambda: {"auxiliary": {}}
    )
    assert re_mod._load_review_credentials_cfg() is None
...
def test_collect_skills_from_preloaded_prompt_and_history():
    ...
    parent.ephemeral_system_prompt = (
        ...
        '"athaniel-brain-dev" skill preloaded. Treat its instructions as '
    ...
    msgs = [
        {"role": "assistant", "content": "", "tool_calls": [
            ...
            {"function": {"name": "skill_view",
                          "arguments": '{"name": "athaniel-brain-dev", '
    ...
    assert names == ["athaniel-brain-dev", "github-pr-workflow"]
...
def test_briefing_includes_loaded_skills_instruction():
    ...
    _, context = build_review_task(snap, "", ["athaniel-brain-dev", "xitter"])
    assert "athaniel-brain-dev, xitter" in context
...
def test_start_review_threads_loaded_skills_into_context(monkeypatch):
    ...
    parent = _fake_parent()
    parent.ephemeral_system_prompt = (
        'session with the "athaniel-brain-dev" skill preloaded.'
    )
    msgs = [
    ...
    assert result["status"] == "dispatched"
    assert "athaniel-brain-dev" in built["context"]
