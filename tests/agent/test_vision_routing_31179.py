...
def isolated_home(monkeypatch):
    """Temp ATHANIEL_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="athaniel_test_31179_")
    athaniel_home = os.path.join(test_home, ".athaniel")
    os.makedirs(athaniel_home)
...
def _fresh_modules():
    """Drop cached athaniel modules so each test reloads against current env."""
    _drop_reload_targets()
