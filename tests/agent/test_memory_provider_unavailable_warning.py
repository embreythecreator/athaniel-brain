"""Regression tests for embreythecreator/athaniel-brain#2765.
...
service that didn't inherit ``~/.athaniel/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "athaniel memory status" in msg
