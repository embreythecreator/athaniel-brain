...
class TestCacheFileReadBlocking:
    """Internal Athaniel cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        athaniel_home = tmp_path / ".athaniel"
        cache = athaniel_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._athaniel_home_path", return_value=athaniel_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Athaniel cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        athaniel_home = tmp_path / ".athaniel"
        hub = athaniel_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_athaniel_home(self, tmp_path):
        """The env basename guard does not depend on ATHANIEL_HOME resolution."""
        athaniel_home = tmp_path / ".athaniel"
        athaniel_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        athaniel_home = tmp_path / ".athaniel"
        cache = athaniel_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
