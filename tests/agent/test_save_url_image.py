...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ATHANIEL_HOME under tmp_path."""
    monkeypatch.setenv("ATHANIEL_HOME", str(tmp_path / ".athaniel"))
    (tmp_path / ".athaniel").mkdir()

    # Force the constants/image cache helpers to re-read ATHANIEL_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("athaniel_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_athaniel_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ATHANIEL_HOME — gateway cleanup
