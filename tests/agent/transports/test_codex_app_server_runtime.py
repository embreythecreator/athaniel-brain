...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Athaniel passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ATHANIEL_HOME", "/users/alice/.athaniel/profiles/backend-worker")
        monkeypatch.setenv("ATHANIEL_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ATHANIEL_KANBAN_DB",
            "/users/alice/.athaniel/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.athaniel/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through athaniel_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Athaniel secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "ATHANIEL_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
