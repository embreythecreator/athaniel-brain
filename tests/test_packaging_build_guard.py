...
def _build_artifact(kind: str, tmp_path, *, nix_build: bool) -> subprocess.CompletedProcess[str]:
    ...
    if nix_build:
        env["ATHANIEL_NIX_BUILD"] = "1"
    else:
        env.pop("ATHANIEL_NIX_BUILD", None)
...
def test_artifact_build_rejects_nix_development_shell_environment(kind, tmp_path):
    ...
    assert "Building wheels or sdists for athaniel-brain is not supported" in result.stderr


@pytest.mark.parametrize(
    ("kind", "artifact_glob"),
    [("sdist", "athaniel_brain-*.tar.gz"), ("wheel", "athaniel_brain-*.whl")],
)
