...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/athaniel/.overview.md") == "viking://user/athaniel"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/athaniel/memories/profile.md") == "viking://user/athaniel/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_athaniel_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATHANIEL_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp ATHANIEL_HOME
        athaniel_home = tmp_path / "athaniel_test"
        athaniel_home.mkdir(exist_ok=True)
        config_yaml = athaniel_home / "config.yaml"
...
""",
    ...
    def test_dashboard_shaped_string_values_are_typed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            openviking_plugin,
            "_load_athaniel_openviking_config",
            lambda: {
                "recall_limit": "12",
                "recall_score_threshold": "0.42",
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/athaniel"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/athaniel/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/athaniel/.overview.md"
        assert result["resolved_uri"] == "viking://user/athaniel"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/athaniel/memories/a.md",
            "viking://user/athaniel/memories/b.md",
            "viking://user/athaniel/memories/c.md",
            "viking://user/athaniel/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/athaniel/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/athaniel"}),
        ]


class TestOpenVikingAutoRecallPrefetch:
    @pytest.mark.parametrize("peer", ["", "athaniel"])
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch, peer):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/user/peers/athaniel/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/user/peers/athaniel/memories/e2e-full.md"]
        ...
        assert all(
            headers.get("user-agent") == f"openviking-memory-athaniel/{_ATHANIEL_VERSION}"
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/athaniel/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/athaniel/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/athaniel"}))

        assert result["path"] == "viking://user/athaniel"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/athaniel/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/athaniel/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/athaniel"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="athaniel"):
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "athaniel")
        ...
        assert alice_uri.startswith("viking://user/alice/peers/athaniel/")
        assert bob_uri.startswith("viking://user/bob/peers/athaniel/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/default/peers/athaniel/memories/"
        )
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting athaniel for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this Athaniel run" not in message, message
