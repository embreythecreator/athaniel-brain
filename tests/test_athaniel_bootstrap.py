"""Tests for athaniel_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Athaniel entry point
(athaniel, athaniel-brain, athaniel-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Athaniel entry point imports athaniel_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported athaniel_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("athaniel_bootstrap", None)
    import athaniel_bootstrap  # noqa: WPS433
    return athaniel_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Athaniel CLI
...
class TestEntryPointsImportBootstrap:
    """Every Athaniel entry point must import athaniel_bootstrap as its
    ...
    # Entry points that invoke Athaniel as a process.  Each one must
    # import athaniel_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "athaniel_cli/main.py",   # athaniel CLI (console_script)
        "run_agent.py",          # athaniel-brain (console_script)
        "acp_adapter/entry.py",  # athaniel-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import athaniel_bootstrap' and that
        ...
        half-finished ``athaniel update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``athaniel_bootstrap`` as a top-level module) leaves athaniel
        ...
        """
        # Resolve relative to the athaniel-brain repo root.  Tests live
        # at tests/test_athaniel_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # athaniel start even when athaniel_bootstrap hasn't been
        ...
        assert first_import_name == "athaniel_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'athaniel_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import athaniel_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Athaniel's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ATHANIEL_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ATHANIEL_PYTHON_SRC_ROOT"] = env
            elif "ATHANIEL_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ATHANIEL_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/athaniel")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ATHANIEL_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ATHANIEL_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/athaniel", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Athaniel root.  The guard must relocate Athaniel to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/athaniel"])
        assert result[0] == "/opt/athaniel"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Athaniel root.
        assert result.index("/opt/athaniel") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ATHANIEL_PYTHON_SRC_ROOT"] = "/env/athaniel"
            hb.harden_import_path()
            assert sys.path[0] == "/env/athaniel"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ATHANIEL_PYTHON_SRC_ROOT", None)
