...
- ``athaniel verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``athaniel verify --json`` when the workspace
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        athaniel_dir = project / ".athaniel"
        athaniel_dir.mkdir()
        (athaniel_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(athaniel_home):
    project = _workspace(athaniel_home)
    ...
    assert status["evidence"]["canonical_command"] == "athaniel verify"


def test_record_verify_run_records_failure(athaniel_home):
    project = _workspace(athaniel_home)
...
def test_cli_passing_run_writes_ledger_evidence(athaniel_home, capsys):
    project = _workspace(athaniel_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(athaniel_home, capsys):
    project = _workspace(athaniel_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(athaniel_home, capsys):
...
def test_cli_run_uses_athaniel_session_id_env(athaniel_home, capsys, monkeypatch):
    monkeypatch.setenv("ATHANIEL_SESSION_ID", "sess-42")
    project = _workspace(athaniel_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> athaniel verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(athaniel_home, capsys):
    project = _workspace(athaniel_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `athaniel verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_athaniel_verify_when_recipe_has_start(athaniel_home):
    project = _workspace(athaniel_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "athaniel verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_athaniel_verify_when_manifest_exists(athaniel_home):
    # No start script, but a saved .athaniel/environment.json qualifies.
    project = _workspace(
        athaniel_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(athaniel_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(athaniel_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "athaniel verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(athaniel_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(athaniel_home, capsys):
    project = _workspace(athaniel_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(athaniel_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(athaniel_home, capsys):
    project = _workspace(athaniel_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
