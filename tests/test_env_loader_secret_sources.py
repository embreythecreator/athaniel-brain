"""Tests for the secret-source tracking in ``athaniel_cli.env_loader``.

These cover the small public surface that lets `athaniel model` / `athaniel setup`
...
from athaniel_cli import env_loader  # noqa: E402
...
def test_cold_profile_hydration_seeds_op_env_bootstrap(tmp_path, monkeypatch):
    ...
    load_athaniel_dotenv() reads <home>/.op.env for OP_SERVICE_ACCOUNT_TOKEN
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_athaniel_dotenv()`` is called at module-import time from several
    hot modules (cli.py, athaniel_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per ATHANIEL_HOME per process.
    """
    ...
    # cli.py, athaniel_cli/main.py, run_agent.py, trajectory_compressor.py,
