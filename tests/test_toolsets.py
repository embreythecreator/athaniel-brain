...
class TestToolsetConsistency:
    ...
    def test_athaniel_platforms_share_core_tools(self):
        """All athaniel-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on athaniel-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["athaniel-cli", "athaniel-telegram", "athaniel-discord", "athaniel-whatsapp", "athaniel-slack", "athaniel-signal", "athaniel-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_athaniel_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("athaniel-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("athaniel-cli")
        second = resolve_toolset("athaniel-cli")
        ...
        assert (
            "athaniel-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("athaniel-cli", include_registry=False)
        second = resolve_toolset("athaniel-cli", include_registry=False)
        assert first == second
