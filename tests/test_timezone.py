"""
Tests for timezone support (athaniel_time module + integration points).
...
import athaniel_time


def _reset_athaniel_time_cache():
    """Reset the athaniel_time module cache."""
    athaniel_time.reset_cache()


# =========================================================================
# athaniel_time.now() — core helper
# =========================================================================

class TestAthanielTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_athaniel_time_cache()

    def teardown_method(self):
        _reset_athaniel_time_cache()
        os.environ.pop("ATHANIEL_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["ATHANIEL_TIMEZONE"] = "Asia/Kolkata"
        result = athaniel_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["ATHANIEL_TIMEZONE"] = "UTC"
        result = athaniel_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["ATHANIEL_TIMEZONE"] = "America/New_York"
        result = athaniel_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ATHANIEL_TIMEZONE"] = "Europe/London"
        tz = athaniel_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching ATHANIEL_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("ATHANIEL_TIMEZONE", raising=False)

        monkeypatch.setenv("ATHANIEL_HOME", str(first_home))
        assert str(athaniel_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch ATHANIEL_HOME in one process.
        monkeypatch.setenv("ATHANIEL_HOME", str(second_home))
        assert str(athaniel_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_athaniel_home_override(str(homes[key]))
                try:
                    tz = athaniel_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ATHANIEL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ATHANIEL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ATHANIEL_TIMEZONE=" + os.environ.get("ATHANIEL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ATHANIEL_TIMEZONE=NOT_SET" in result["output"], (
            "ATHANIEL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ATHANIEL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ATHANIEL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=athaniel_tz) which shifted
        absolute time when system-local tz != Athaniel tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Athaniel tz
        ...
        # Use a Athaniel timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _athaniel_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ATHANIEL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_athaniel_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Athaniel timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ATHANIEL_TIMEZONE"] = "US/Eastern"
        _reset_athaniel_time_cache()
