...
1. `athaniel secrets bitwarden setup --help` works (dispatch path)
2. `athaniel update --check` works (update path)
3. `athaniel secrets bitwarden disable` works (handler execution)
4. `athaniel secrets onepassword status` works (lazy backend loads on demand)
...
def _run_athaniel(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run athaniel CLI as a subprocess from repo root.
    ...
    connectivity — but ``athaniel update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-athaniel-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-athaniel-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "athaniel_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`athaniel secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_athaniel(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`athaniel secrets bitwarden disable` must exit 0."""
        result = _run_athaniel(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`athaniel secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_athaniel(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`athaniel secrets onepassword setup --help` must exit 0."""
        result = _run_athaniel(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `athaniel update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`athaniel update --check` must not load cryptography._rust."""
        result = _run_athaniel(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['athaniel','update','--check']
...
sys.argv = ['athaniel', 'update', '--check']

import athaniel_cli.main as m
...
# resolves the athaniel_cli.main module's lazily re-exported attribute —
# so the patch must land on athaniel_cli.main._cmd_update_check.
with patch('athaniel_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
