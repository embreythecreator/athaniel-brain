...
``athaniel_cli.sqlite_safe_read``'s registry permanently over-counted as well.
...
def _live_count(path) -> int:
    ...
    import athaniel_cli.sqlite_safe_read as mod
...
def test_read_open_failure_backs_off_but_recovers(db):
    ...
    from athaniel_state import _READ_OPEN_RETRY_SECONDS
...
def test_peak_live_connections_bounded_under_simultaneous_burst(db):
    ...
    """
    from athaniel_state import _READ_POOL_MAX
...
def test_peak_is_bounded_across_two_SessionDBs_on_one_path(db):
    ...
    from athaniel_state import SessionDB, _READ_POOL_MAX
...
# And Athaniel's SQLite descriptors are only ever a share of the fd table: the
...
def test_peak_is_bounded_across_many_database_files(tmp_path):
    ...
    from athaniel_state import SessionDB, _READ_POOL_MAX, _READ_POOL_PROCESS_MAX
    ...
    try:
        ...
        for d in dbs:
            d.close()
        assert athaniel_state._process_read_permits.acquire(blocking=False), (
            "close() stranded a process permit"
        )
        athaniel_state._process_read_permits.release()
...
def test_no_read_connection_is_opened_without_descriptor_headroom(db, monkeypatch):
    ...
    monkeypatch.setattr(athaniel_state, "_fd_soft_limit", lambda: 256)
    monkeypatch.setattr(athaniel_state, "_open_fd_count", lambda: 250)
    monkeypatch.setattr(athaniel_state, "_fd_usage_cache", (0.0, None))
    ...
    assert athaniel_state._read_open_denied_fd_headroom > 0, (
        "the guard fired without leaving a trace to diagnose it from"
    )

    monkeypatch.setattr(athaniel_state, "_open_fd_count", lambda: 10)
    monkeypatch.setattr(athaniel_state, "_fd_usage_cache", (0.0, None))
...
def test_fd_headroom_guard_fails_open_where_it_cannot_measure(monkeypatch):
    ...
    monkeypatch.setattr(athaniel_state, "_fd_soft_limit", lambda: None)
    assert athaniel_state._fd_headroom_ok() is True

    # A probe that could not get a descriptor of its own is evidence, not
    # absence of evidence.
    monkeypatch.setattr(athaniel_state, "_fd_soft_limit", lambda: 256)
    monkeypatch.setattr(athaniel_state, "_open_fd_count", lambda: -1)
    monkeypatch.setattr(athaniel_state, "_fd_usage_cache", (0.0, None))
    assert athaniel_state._fd_headroom_ok() is False
...
def test_duplicate_handles_on_one_path_are_reported(db, caplog):
    ...
    from athaniel_state import SessionDB, _HANDLES_PER_PATH_WARN
