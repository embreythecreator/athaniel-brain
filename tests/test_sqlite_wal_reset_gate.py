...
Athaniel must not *enable* multi-process WAL on SQLite builds that still contain
...
def _reset_wal_reset_bug_warnings():
    athaniel_state._wal_reset_bug_warned_paths.clear()
    yield
    athaniel_state._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            athaniel_state, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )
...
class TestNoDowngradeUnderConcurrentOpeners:
    ...
    def test_configured_delete_refuses_when_probe_blocked(
        ...
        monkeypatch.setattr(athaniel_state, "resolve_journal_mode", lambda: "delete")
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(athaniel_state, "sqlite_source_id", lambda: "testid-abc")
