...
#
# Installs a real, earlier Athaniel the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `athaniel`.
#
...
# Keep sandbox state out of the default .athaniel-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".athaniel-sandbox-e2e-$ROUTE"
export ATHANIEL_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/athaniel/.athaniel/athaniel-brain"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/athaniel-brain.git"
...
LOG_DIR="${ATHANIEL_E2E_LOG_DIR:-$(mktemp -d -t athaniel-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED athaniel accept FLAG on `athaniel update`?
#
...
# that silently rots. `athaniel update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "athaniel update --help 2>&1" | grep -qF -- "$flag"
}
...
require_athaniel_works() {
  local when="$1" out
  out="$(in_sandbox "athaniel --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "athaniel --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "athaniel runs $when"
}

# ── install the earlier Athaniel ─────────────────────────────────────────────
...
require_athaniel_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: athaniel update'
    ...
    # athaniel which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="athaniel update --yes"
    else
      update_cmd="athaniel update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "athaniel update failed ($update_cmd)"
    fi
    require_landed_on_target 'athaniel update'
    require_athaniel_works 'after athaniel update'
    ;;
  installer)
    ...
    require_athaniel_works 'after installer re-run'
    ;;
esac
