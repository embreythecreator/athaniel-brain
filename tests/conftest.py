"""Shared fixtures for the athaniel-brain test suite.
...
2. **Isolated ATHANIEL_HOME.** ATHANIEL_HOME points to a per-test tempdir so
   code reading ``~/.athaniel/*`` via ``get_athaniel_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".athaniel"`` instead of the canonical
   ``get_athaniel_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ATHANIEL_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ATHANIEL_HOME before ANY test module is imported ──────────────────
# `athaniel_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_athaniel_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.athaniel/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ATHANIEL_HOME, but fixtures run
...
# the REAL Athaniel root — capture it BEFORE the sandbox rewires ATHANIEL_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.athaniel (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ATHANIEL_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ATHANIEL_HOME = os.environ.get("ATHANIEL_HOME", "")


def _athaniel_home_points_at_production(value: str) -> bool:
    """True when a pre-set ATHANIEL_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ATHANIEL_HOME=~/.athaniel``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``athaniel_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.athaniel`` — the escape vector that landed pytest
    ...
    ATHANIEL_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".athaniel").resolve()
    except Exception:
        return True
...
if _athaniel_home_points_at_production(os.environ.get("ATHANIEL_HOME", "")):
    _SESSION_ATHANIEL_HOME = tempfile.mkdtemp(prefix="athaniel-test-home-")
    os.environ["ATHANIEL_HOME"] = _SESSION_ATHANIEL_HOME
    atexit.register(shutil.rmtree, _SESSION_ATHANIEL_HOME, True)
...
# like a real CLI") — which used to disarm athaniel_state's live-DB guard in
# the child at the same moment the child lost the ATHANIEL_HOME redirect.
# ATHANIEL_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# athaniel_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ATHANIEL_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ATHANIEL_TEST_ISOLATION"] = os.environ.get("ATHANIEL_HOME", "") or "1"

#: ATHANIEL_HOME as it stood when conftest was imported - i.e. before any test
...
ATHANIEL_HOME_AT_CONFTEST_IMPORT = os.environ.get("ATHANIEL_HOME", "")
...
# ATHANIEL_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ATHANIEL_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``athaniel_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Athaniel agent session inherits them and
    # athaniel_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "ATHANIEL_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "ATHANIEL_INTERACTIVE",
    "ATHANIEL_QUIET",
    "ATHANIEL_TOOL_PROGRESS",
    "ATHANIEL_TOOL_PROGRESS_MODE",
    "ATHANIEL_MAX_ITERATIONS",
    "ATHANIEL_SESSION_PLATFORM",
    "ATHANIEL_SESSION_CHAT_ID",
    "ATHANIEL_SESSION_CHAT_NAME",
    "ATHANIEL_SESSION_CHAT_TYPE",
    "ATHANIEL_SESSION_THREAD_ID",
    "ATHANIEL_SESSION_SOURCE",
    "ATHANIEL_SESSION_KEY",
    "ATHANIEL_GATEWAY_SESSION",
    "ATHANIEL_CRON_SESSION",
    "_ATHANIEL_GATEWAY",
    "ATHANIEL_PLATFORM",
    "ATHANIEL_MODEL",
    "ATHANIEL_INFERENCE_MODEL",
    "ATHANIEL_INFERENCE_PROVIDER",
    "ATHANIEL_TUI_PROVIDER",
    "ATHANIEL_MANAGED",
    "ATHANIEL_MANAGED_DIR",
    "ATHANIEL_DEV",
    "ATHANIEL_CONTAINER",
    "ATHANIEL_EPHEMERAL_SYSTEM_PROMPT",
    "ATHANIEL_TIMEZONE",
    "ATHANIEL_REDACT_SECRETS",
    "ATHANIEL_BACKGROUND_NOTIFICATIONS",
    "ATHANIEL_EXEC_ASK",
    "ATHANIEL_HOME_MODE",
    "ATHANIEL_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.athaniel/kanban.db instead of the per-test ATHANIEL_HOME.
    "ATHANIEL_KANBAN_DB",
    "ATHANIEL_KANBAN_BOARD",
    "ATHANIEL_KANBAN_HOME",
    "ATHANIEL_KANBAN_WORKSPACES_ROOT",
    "ATHANIEL_KANBAN_LOGS_ROOT",
    "ATHANIEL_KANBAN_TASK",
    "ATHANIEL_KANBAN_WORKSPACE",
    "ATHANIEL_KANBAN_RUN_ID",
    "ATHANIEL_KANBAN_CLAIM_LOCK",
    "ATHANIEL_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "ATHANIEL_DELEGATED_CHILD_CONTEXT",
    "ATHANIEL_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "athaniel" host.
    "ATHANIEL_HONCHO_HOST",
    ...
    "ATHANIEL_DASHBOARD_OAUTH_CLIENT_ID",
    "ATHANIEL_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ATHANIEL_HOME to per-test tempdirs so code that
    reads ``~/.athaniel/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ATHANIEL_* vars that could change test semantics.
    for name in _ATHANIEL_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ATHANIEL_HONCHO_HOST", "athaniel")

    # 3. Redirect ATHANIEL_HOME to a per-test tempdir. Code that reads
    #    ``~/.athaniel/*`` via ``get_athaniel_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.athaniel/*`` via
    #    ``Path.home() / ".athaniel"`` instead of ``get_athaniel_home()``
    #    is a bug to fix at the callsite.
    fake_athaniel_home = tmp_path / "athaniel_test"
    fake_athaniel_home.mkdir()
    (fake_athaniel_home / "sessions").mkdir()
    (fake_athaniel_home / "cron").mkdir()
    (fake_athaniel_home / "memories").mkdir()
    (fake_athaniel_home / "skills").mkdir()
    monkeypatch.setenv("ATHANIEL_HOME", str(fake_athaniel_home))
    ...
    # athaniel_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ATHANIEL_TEST_ISOLATION", str(fake_athaniel_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ATHANIEL_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. athaniel_state computes ``DEFAULT_DB_PATH = get_athaniel_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from athaniel_state import ...``) that
    ...
    athaniel_state_mod = sys.modules.get("athaniel_state")
    if athaniel_state_mod is not None and hasattr(athaniel_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            athaniel_state_mod, "DEFAULT_DB_PATH", fake_athaniel_home / "state.db"
        )
    ...
    monkeypatch.setenv("ATHANIEL_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.athaniel/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import athaniel_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ATHANIEL_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_athaniel_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from athaniel_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.athaniel. This autouse
...
# allow-list because test-level fixtures legitimately move ATHANIEL_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ATHANIEL_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ATHANIEL_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Athaniel root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ATHANIEL_HOME and not _athaniel_home_points_at_production(
        _PRE_SANDBOX_ATHANIEL_HOME
    ):
        # ATHANIEL_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_athaniel_root().resolve()
    # No pre-existing ATHANIEL_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".athaniel").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.athaniel`` captured at import time. Hermetic tests that legitimately
    move ATHANIEL_HOME to sibling tempdirs are unaffected.

    Only patches when ``athaniel_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("athaniel_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import athaniel_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.athaniel. See #69283."
        )
...
# ``athaniel_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Athaniel root. This fixture wires the
# test-side knobs:
...
#     installs where ATHANIEL_HOME is not ~/.athaniel) into the guard's
...
# so subprocess children that import athaniel_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("athaniel_state")
    ...
    if _PRE_SANDBOX_ATHANIEL_HOME and not _athaniel_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ATHANIEL_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Athaniel home override redirects every later
    # ``get_athaniel_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from athaniel_constants import get_athaniel_home_override, set_athaniel_home_override

        if get_athaniel_home_override() is not None:
            set_athaniel_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal athaniel config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``athaniel-gateway`` process
...
# correlated with a ``tests/athaniel_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> athaniel-gateway``
...
def _wal_is_usable() -> bool:
    """True when Athaniel will actually put a database into WAL mode here.

    Athaniel refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Athaniel can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Athaniel managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``athaniel_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_athaniel_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_athaniel_home`` fixture
    redirects ``ATHANIEL_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.athaniel/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``athaniel_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ATHANIEL_VOICE_TTS"] = "1"``. The
...
#      ``athaniel_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``athaniel_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``athaniel_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``athaniel_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Athaniel runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.athaniel``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Athaniel falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Athaniel uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart athaniel-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting athaniel/python patterns are also blocked.
    """
    ...
    def _matches_athaniel_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ATHANIEL_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_athaniel_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch athaniel-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m athaniel_cli.main".
                if (
                    "athaniel" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live athaniel-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting athaniel/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `athaniel update` (or the
        # equivalent `python -m athaniel_cli.main update`).  These commands
        ...
        if "update" in low and (
            # athaniel update / athaniel update --gateway / setsid bash -c ... athaniel update
            ("athaniel" in low and "update" in low.split())
            or
            # python -m athaniel_cli.main update --gateway
            ("athaniel_cli" in low and "update" in low.split())
            or
            # venv/bin/athaniel update  (absolute path variant used in tests)
            (".venv/bin/athaniel" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`athaniel update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ATHANIEL_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import athaniel_cli.voice as _voice
    except Exception:
