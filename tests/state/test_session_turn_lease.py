...
from athaniel_state import SessionDB, SessionTurnLeaseLostError
    ...
    def flaky_acquire(*args, **kwargs):
        ...
        if attempts["n"] == 1:
            raise sqlite3.OperationalError(
                "database is locked (another Athaniel process held the "
...
def test_non_expired_turn_lease_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(
        athaniel_state, "psutil", SimpleNamespace(pid_exists=pid_exists)
    )
...
def test_yield_request_marks_wake_holder_only(tmp_path):
    """Operator-held leases are never asked to yield; wake-held leases are."""
    db = SessionDB(tmp_path / "state.db")
    db.create_session("s", source="test")
    holder = f"pid={os.getpid()}:turn=wake"

    assert db.try_acquire_session_turn_lease("s", holder, ttl_seconds=5, kind="wake")
    assert not db.session_turn_yield_requested("s", holder)
    assert db.request_session_turn_yield("s") is True
    assert db.session_turn_yield_requested("s", holder) is True
    # Holder-scoped: a different holder string reads False.
    assert not db.session_turn_yield_requested("s", "pid=1:turn=other")
    db.release_session_turn_lease("s", holder)

    op_holder = f"pid={os.getpid()}:turn=op"
    assert db.try_acquire_session_turn_lease("s", op_holder, ttl_seconds=5)
    assert db.request_session_turn_yield("s") is False
    assert not db.session_turn_yield_requested("s", op_holder)
    db.release_session_turn_lease("s", op_holder)


def test_operator_acquire_wait_requests_yield_from_wake_holder(tmp_path):
    """A blocked operator waiter marks the wake holder within its wait."""
    db = SessionDB(tmp_path / "state.db")
    db.create_session("s", source="test")
    wake_holder = f"pid={os.getpid()}:turn=wake"
    assert db.try_acquire_session_turn_lease("s", wake_holder, ttl_seconds=30, kind="wake")

    acquired = db.acquire_session_turn_lease(
        "s",
        f"pid={os.getpid()}:turn=operator",
        ttl_seconds=5,
        wait_seconds=0.3,
        poll_interval_seconds=0.02,
        kind="operator",
        request_yield_on_wait=True,
    )
    assert acquired is False  # wake holder never released in this test
    assert db.session_turn_yield_requested("s", wake_holder) is True
    db.release_session_turn_lease("s", wake_holder)


def test_successor_lease_does_not_inherit_yield_request(tmp_path):
    """A fresh lease row starts with yield_requested = 0."""
    db = SessionDB(tmp_path / "state.db")
    db.create_session("s", source="test")
    first = f"pid={os.getpid()}:turn=wake1"
    assert db.try_acquire_session_turn_lease("s", first, ttl_seconds=5, kind="wake")
    assert db.request_session_turn_yield("s")
    db.release_session_turn_lease("s", first)

    second = f"pid={os.getpid()}:turn=wake2"
    assert db.try_acquire_session_turn_lease("s", second, ttl_seconds=5, kind="wake")
    assert db.session_turn_yield_requested("s", second) is False
    db.release_session_turn_lease("s", second)


def test_machine_waiter_does_not_request_yield(tmp_path):
    """Only operator waiters ask; a wake waiter behind a wake holder does not."""
    db = SessionDB(tmp_path / "state.db")
    db.create_session("s", source="test")
    wake_holder = f"pid={os.getpid()}:turn=wake"
    assert db.try_acquire_session_turn_lease("s", wake_holder, ttl_seconds=30, kind="wake")
    ...
    assert not db.acquire_session_turn_lease(
        "s",
        f"pid={os.getpid()}:turn=wake-waiter",
        ttl_seconds=5,
        wait_seconds=0.2,
        poll_interval_seconds=0.02,
        kind="wake",
        request_yield_on_wait=False,
    )
    assert db.session_turn_yield_requested("s", wake_holder) is False
    db.release_session_turn_lease("s", wake_holder)
