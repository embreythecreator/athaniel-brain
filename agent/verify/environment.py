...
The manifest lives at ``<project>/.athaniel/environment.json`` and is the
...
MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".athaniel") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``athaniel verify``.
    """
