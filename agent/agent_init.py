...
from athaniel_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.athaniel/.env``. See embreythecreator/athaniel-brain#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'athaniel memory status'. Note: "
        "systemd/gateway services do not inherit ~/.athaniel/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from athaniel_cli.providers import ATHANIEL_OVERLAYS, get_provider

        overlay = ATHANIEL_OVERLAYS.get(provider)
    ...
    try:
        ...
        from athaniel_cli.models import normalize_provider as normalize_model_provider
        from athaniel_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: athaniel config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ATHANIEL_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_athaniel_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ATHANIEL_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Athaniel never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .athaniel.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ATHANIEL_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.athaniel/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"nous", "nous-portal", "embreythecreator"}:
        ...
        try:
            from athaniel_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent._cache_ttl = "5m"
    try:
        from athaniel_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind ATHANIEL_DEV_CREDITS.
    ...
    # both live under ~/.athaniel/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from athaniel_logging import setup_logging, setup_verbose_logging
    setup_logging(athaniel_home=_ra()._athaniel_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that athaniel_logging.setup_logging() does not
        ...
        # handler level inside athaniel_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from athaniel_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from athaniel_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = athaniel_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `athaniel model` to "
                        "select a provider, or run `athaniel setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different ATHANIEL_HOME after
    ...
    # worker (kanban_show tool is present iff ATHANIEL_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["ATHANIEL_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.athaniel/sessions/ alongside gateway sessions
    athaniel_home = get_athaniel_home()
    agent.logs_dir = athaniel_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.athaniel/sessions/session_{sid}.json)
    ...
    try:
        from athaniel_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `athaniel --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from athaniel_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Athaniel behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "athaniel"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Athaniel' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "athaniel", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, athaniel, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from athaniel_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from athaniel_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from athaniel_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Athaniel Brain.  Choose a model with at least "
    ...
    # Nous Athaniel 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from athaniel_cli.model_switch import _check_athaniel_model_warning

            _athaniel_warn = _check_athaniel_model_warning(agent.model or "")
            if _athaniel_warn:
                _user_msg = (
                    "⚠ Athaniel Athaniel 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_athaniel_warn)
        except Exception:
            pass
