...
def list_providers(*, scope: Optional[str] = None) -> List[BrowserProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or athaniel_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[BrowserProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or athaniel_home_key(), {}).get(key) or _providers.get(key)
...
def registry_generation(*, scope: Optional[str] = None) -> tuple[int, int]:
    ...
    active_scope = scope or athaniel_home_key()
    with _lock:
...
def _resolve(configured: Optional[str]) -> Optional[BrowserProvider]:
    ...
    plugins added under ``~/.athaniel/plugins/browser/<vendor>/`` are subject
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(athaniel_home_key(), {}))
