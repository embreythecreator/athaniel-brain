...
(``gateway/platforms/webhook.py``): inbound wakes Athaniel when the world
changes; outbound tells the world when Athaniel does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Athaniel-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``ATHANIEL_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.athaniel/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/athaniel-events
          ...
          secret_env: ATHANIEL_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `athaniel hooks list`
...
Headers::
    ...
    User-Agent:              Athaniel-Agent-Outbound-Webhook
    X-Athaniel-Event:          <hook event name>
    X-Athaniel-Delivery:       <delivery_id>
    X-Athaniel-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("ATHANIEL_SAFE_MODE"):
        logger.info("ATHANIEL_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Athaniel-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Athaniel-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
