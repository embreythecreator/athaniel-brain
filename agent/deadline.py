...
(https://github.com/embreythecreator/athaniel-brain/issues/85125).
...
* :func:`resolve_timeout` — one config-first resolution path for timeout
  ...
  so new surfaces stop inventing ``ATHANIEL_*_TIMEOUT`` env vars (".env is for
...
* ``None`` timeout means unbounded, and non-positive resolved values are
  ...
  ``ATHANIEL_CONCURRENT_TOOL_TIMEOUT_S`` convention).
"""
...
class DeadlineExpired(TimeoutError):
    ...
    was Athaniel's own bound — error classification must not attribute it to
...
def clamp_timeout(timeout: Optional[float]) -> Optional[float]:
    ...
    * Non-positive values become ``None`` (unbounded) — matching the existing
      ``ATHANIEL_CONCURRENT_TOOL_TIMEOUT_S`` "0 disables the bound" convention.
