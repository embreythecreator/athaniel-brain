...
token`` all expose exactly this contract. Athaniel already accepts a callable
...
def _mint(command: str, label: str) -> tuple[str, Optional[float]]:
    ...
    if stdout.lstrip().startswith("{"):
        ...
        if isinstance(payload, dict):
            ...
            # Imported lazily: athaniel_cli.auth imports from agent.* at module
            # level, so a top-level import here would risk a cycle.
            from athaniel_cli.auth import _parse_iso_timestamp
