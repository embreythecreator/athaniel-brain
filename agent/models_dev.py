...
Data resolution order:
  ...
  2. Disk cache (~/.athaniel/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Athaniel ↔ models.dev
# ---------------------------------------------------------------------------

# Athaniel provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Athaniel' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Athaniel ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_athaniel_ids(mdev_id: str) -> List[str]:
    """Return the Athaniel provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for athaniel_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(athaniel_id)
...
def _get_cache_path() -> Path:
    ...
    return get_athaniel_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from athaniel_constants import get_athaniel_home
    return get_athaniel_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``athaniel config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Athaniel provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Athaniel id.
    for athaniel_id in _models_dev_to_athaniel_ids(provider_key):
        if athaniel_id != provider_key:
            candidates.append(athaniel_id)
...
def _get_provider_models(
    ...
    """Resolve a Athaniel provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``athaniel model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from athaniel_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Athaniel agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Athaniel provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Athaniel ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Athaniel or models.dev provider ID.  Tries exact match then
