...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ATHANIEL_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Athaniel runtime / deployment
    "ATHANIEL_HOME", "ATHANIEL_PROFILE", "ATHANIEL_GATEWAY_LOCK_DIR",
    "ATHANIEL_MAX_ITERATIONS", "ATHANIEL_MAX_TOKENS", "ATHANIEL_API_TIMEOUT",
    "ATHANIEL_REDACT_SECRETS", "ATHANIEL_NOUS_TIMEOUT_SECONDS",
    "_ATHANIEL_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ATHANIEL_KANBAN_DB", "ATHANIEL_KANBAN_WORKSPACES_ROOT", "ATHANIEL_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ATHANIEL_KANBAN_",
    "ATHANIEL_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Athaniel writes
    ...
    ``athaniel_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Athaniel parser: save_env_value
    ...
    from athaniel_cli.config import _parse_env_value
...
def build_profile_secret_scope(athaniel_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(athaniel_home)
    secrets = load_env_file(home / ".env")

    try:
        from athaniel_cli.env_loader import get_secret_source_values
