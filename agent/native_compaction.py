...
Athaniel' support is deliberately narrow (live verification, Aug 2026):
...
Ownership model: Athaniel' local compression stays fully armed as the
...
def _warn_native_compaction_suppressed_by_checkpoint_gate() -> None:
    ...
    logger.warning(
        ...
        "checkpoint-aware Athaniel compressor stays authoritative."
    )
...
def native_compaction_context_management(
    ...
    *,
    ...
    # the server replaces older context. Keep the checkpoint-aware Athaniel
...
def _is_summary_item(item: Any) -> bool:
    """True when *item* is a canonical Athaniel compression-summary message.
...
def prune_pre_checkpoint_items(
    ...
    - Compression summary messages (``_is_summary_item``, the canonical
      ...
      byte/character-sliced: Athaniel summaries carry structural framing
