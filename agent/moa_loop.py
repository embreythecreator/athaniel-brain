...
MoA-enabled; the normal Athaniel agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from athaniel_cli.moa_config import coerce_privacy_filter
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Athaniel boilerplate, not advisory signal).
...
def _council_fallback_spares(
    ...
    try:
        from athaniel_cli.config import load_config
        from athaniel_cli.model_council import fallback_chain_for_role
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Athaniel agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Athaniel agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Athaniel agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
def build_reference_guidance(
    ...
    *,
    ...
    """Assemble the aggregator's reference-context guidance block.

    Pure and deterministic — extracted from ``create()`` so the golden
    trace-replay gate (WO-MOA/2) can rebuild the block from a captured
    trace's reference outputs and byte-diff it against what the aggregator
    actually received. ``joined`` is the pre-joined (and possibly redacted /
    degraded-annotated) reference-response body; ``references`` supplies the
    label roster for the header. The output format is LOAD-BEARING: any
    change here shows up as a byte diff in the golden replay test.
    """
    return (
        "[Mixture of Agents reference context]\n"
        f"Preset: {preset_name}\n"
        f"Aggregator/acting model: {aggregator_label}\n"
        f"References: {', '.join(label for label, _ in references)}\n\n"
...
class MoAChatCompletions:
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        # WO-MOA/2 inert dispatch: resolve the preset's posture through the
        # mode registry (all modes currently route to this ensemble path —
        # behavior unchanged) and count it for the soak metric that gates
        # posture-logic PRs. Protected by the golden trace-replay gate.
        try:
            from agent.moa_modes import resolve_mode

            self._resolved_mode = resolve_mode(preset)
        except Exception:  # pragma: no cover - dispatch must never break a turn
            self._resolved_mode = "ensemble"
        ...
        if reference_outputs and not successful_outputs:
            ...
            guidance = build_reference_guidance(
                ...
                [(label, text) for label, text, _usage in _agg_refs],
                joined=joined,
            )
            _attach_reference_guidance(agg_messages, guidance)
...
def build_moa_facade(agent, preset_name: Any = None) -> MoAClient:
    ...
    # WO-MOA/2: bind the plan-lifecycle backends to this agent+preset. Both
    # surfaces (the convene_debate tool and the /plan approve gate) have a
    # session but no agent, so this single construction point is where an
    # agent-bound fan-out becomes reachable. Registration alone changes no
    # behavior: debate only runs when the model calls convene_debate, and
    # the verify gate stays off until plan.verify_gate is configured.
    try:
        from athaniel_cli.config import load_config as _load_config
        from athaniel_cli.moa_config import resolve_moa_preset

        _preset = resolve_moa_preset(
            (_load_config() or {}).get("moa") or {}, resolved_preset
        )
    except Exception:  # pragma: no cover - registration is best-effort
        _preset = None
    if _preset:
        try:
            from agent.plan_cove import make_verify_fn
            from agent.plan_verify import register_verify_backend
            from tools.convene_debate_tool import (
                make_debate_backend,
                register_debate_backend,
            )

            register_verify_backend(make_verify_fn(agent, _preset))
            register_debate_backend(make_debate_backend(agent, _preset))
        except Exception as exc:  # pragma: no cover - never break client build
            logger.debug("Plan-lifecycle backend registration skipped: %s", exc)
