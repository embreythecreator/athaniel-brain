...
Every Athaniel request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Athaniel Brain and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=athaniel-brain",
        "client=athaniel-client-v<__version__>",
    ]

The version is sourced live from ``athaniel_cli.__version__`` so it auto-aligns
...
``athaniel_cli.__version__`` is the canonical source of truth.
"""
...
def _athaniel_version() -> str:
    """Return the current Athaniel release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``athaniel_cli`` cannot be imported (should
...
def athaniel_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=athaniel-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=athaniel-client-v{_athaniel_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Athaniel session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Athaniel conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.athaniel/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Athaniel
    ...
    """
    tags = ["product=athaniel-brain", athaniel_client_tag()]
