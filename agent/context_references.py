...
from athaniel_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_ATHANIEL_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    athaniel_home = get_athaniel_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(athaniel_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(athaniel_home / rel for rel in _SENSITIVE_ATHANIEL_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Athaniel path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.athaniel/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Athaniel path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
