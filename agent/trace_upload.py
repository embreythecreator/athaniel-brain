"""Upload a Athaniel session transcript to Hugging Face as an agent trace.

Athaniel stores sessions in its own SQLite store (``athaniel_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``athaniel trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Athaniel' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "athaniel-traces"
_ATHANIEL_VERSION = "athaniel-brain"
...
# ---------------------------------------------------------------------------
# Conversion: Athaniel OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Athaniel' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Athaniel conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.athaniel/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `athaniel trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/athaniel-traces`` dataset. Returns a
