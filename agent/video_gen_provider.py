...
as ``kind: backend``) or ``~/.athaniel/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``athaniel tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ATHANIEL_HOME/cache/videos/``, creating parents as needed."""
    from athaniel_constants import get_athaniel_home

    path = get_athaniel_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ATHANIEL_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$ATHANIEL_HOME/cache/videos/``.
