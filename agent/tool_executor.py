...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Athaniel process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``ATHANIEL_CONCURRENT_TOOL_TIMEOUT_S`` env var
    ...
    return resolve_timeout(
        ...
        env_var="ATHANIEL_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Athaniel process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Athaniel policy and dispatch exactly once."""
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        ...
        if block_message is None:
            _policy_msg = policy_deny_message(
                getattr(agent, "_execution_policy", None), function_name
            )
            if _policy_msg is not None:
                # Execution-policy deny (plan mode): non-halting — synthesizes
                # an error result for this call only; other calls in the
                # batch and the turn itself continue.
                block_message = _policy_msg
                block_error_type = "execution_policy_block"
    ...
    def _athaniel_pipeline(relay_args: dict[str, Any]) -> Any:
        ...
        return run_tool_execution_middleware(
            ...
            capture_agent=agent,  # WO-POSTURE/1 1.5 — popped host-side, never reaches plugins
            task_id=effective_task_id or "",
            session_id=getattr(agent, "session_id", "") or "",
            tool_call_id=tool_call_id or "",
            turn_id=getattr(agent, "_current_turn_id", "") or "",
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``ATHANIEL_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
