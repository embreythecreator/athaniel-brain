...
cache problem must never block Athaniel startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Athaniel home used for cache paths.

    ``home_path`` is whatever ``load_athaniel_dotenv()`` already resolved;
    falling back to ``$ATHANIEL_HOME`` / ``~/.athaniel`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from athaniel_constants import get_athaniel_home

        home_path = get_athaniel_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<athaniel_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ATHANIEL_HOME is ever made traversable.
            try:
