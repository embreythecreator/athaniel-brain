...
credentials at process startup, _after_ ~/.athaniel/.env has loaded.
...
Currently bundled:
  ...
  - ``bitwarden`` — Bitwarden Secrets Manager (`bws` CLI).  See
    ...
    ``athaniel_cli.secrets_cli`` for the user-facing setup wizard.
  - ``onepassword`` — 1Password ``op://`` secret references (`op` CLI).
    See ``agent.secret_sources.onepassword`` for the integration and
    ``athaniel_cli.onepassword_secrets_cli`` for the user-facing commands.
