...
``~/.athaniel/.env`` has loaded and BEFORE the rest of Athaniel reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per ATHANIEL_HOME) by the orchestrator in
...
_SOURCE_ENVIRONMENT = ContextVar("athaniel_secret_source_environment", default=None)
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``athaniel secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``athaniel secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved ATHANIEL_HOME.
        """
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``athaniel secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``athaniel secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `athaniel secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `athaniel secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `athaniel secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `athaniel secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
...
# codes that must not reach Athaniel' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Athaniel knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Athaniel
      output.
