...
``athaniel_cli.env_loader._apply_external_secret_sources()``.
...
def get_source(name: str, *, scope: Optional[str] = None) -> Optional[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or athaniel_home_key(), {}).get(
            name
...
def list_sources(*, scope: Optional[str] = None) -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(scope or athaniel_home_key(), {}))
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(athaniel_home_key(), {}))
        return list(merged.values())
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``athaniel`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's ATHANIEL_HOME is ``~/.athaniel/profiles/<name>``; the
    default profile (``~/.athaniel``) returns "".
    """
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(
        secrets_cfg, scope=athaniel_home_key(home_path)
    )
    if not enabled:
        return report
