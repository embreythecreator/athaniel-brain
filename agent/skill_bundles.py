...
Storage
-------
Bundles live in ``~/.athaniel/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ATHANIEL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``athaniel bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ATHANIEL_HOME.

    Honors ``ATHANIEL_BUNDLES_DIR`` for tests; falls back to
    ``<ATHANIEL_HOME>/skill-bundles``.
    """
    override = os.environ.get("ATHANIEL_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_athaniel_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `athaniel bundles` CLI subcommand.
# ---------------------------------------------------------------------------
