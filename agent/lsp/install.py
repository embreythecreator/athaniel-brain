...
appropriate.  All installs go to a Athaniel-owned bin staging dir,
``<ATHANIEL_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``athaniel lsp
  status``.
...
# ``<ATHANIEL_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `athaniel lsp status` reports the host's presence.
...
def athaniel_lsp_bin_dir() -> Path:
    """Return the Athaniel-owned bin staging dir for LSP servers."""
    from athaniel_constants import get_athaniel_home

    p = get_athaniel_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(athaniel_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $ATHANIEL_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Athaniel installed and
    ...
    if npm is None:
        ...
        return None
    staging = athaniel_lsp_bin_dir().parent  # <ATHANIEL_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = athaniel_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a athaniel-owned target dir.
    ...
    console script.
    """
    pip_target = athaniel_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = athaniel_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``athaniel lsp status`` CLI to give users a quick
