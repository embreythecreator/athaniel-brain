"""Coding-context awareness — base Athaniel, every interactive surface.

When the user runs Athaniel inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Athaniel shifts into a **coding posture**. This module is the
...
from athaniel_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("athaniel.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "athaniel", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _ATHANIEL_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``ATHANIEL_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Athaniel should operate in its coding posture right now."""
