...
_ATHANIEL_MD_NAMES = (".athaniel.md", "ATHANIEL.md")


def _find_athaniel_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.athaniel.md`` or ``ATHANIEL.md``.
    ...
    # could pick up a .athaniel.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ATHANIEL_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Athaniel Brain, created by Embrey The Creator / The Voice. Be direct: match the "
...
ATHANIEL_AGENT_HELP_GUIDANCE = (
    ...
    # precedence. Injected only when skill_view exists AND the athaniel-brain
    # skill is actually installed (see system_prompt.py slot resolution).
    "You run on Athaniel Brain (by Athaniel). When the user needs help with "
    "Athaniel itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://athaniel-brain.embreythecreator.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. The `athaniel-brain` skill has the actual commands and proven "
    "workflows — load it with skill_view(name='athaniel-brain') before configuring, "
    "modifying, or troubleshooting Athaniel so you don't guess or invent workarounds."
)
...
ATHANIEL_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Athaniel Brain (by Athaniel). When the user needs help with "
    "Athaniel itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://athaniel-brain.nousresearch.com/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.athaniel/kanban.db`. Your task id is in "
    "`$ATHANIEL_KANBAN_TASK`; your workspace is `$ATHANIEL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ATHANIEL_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ATHANIEL_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ATHANIEL_KANBAN_BRANCH:-wt/$ATHANIEL_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ATHANIEL_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`athaniel profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `athaniel kanban <verb>` for board operations. Use "
...
#
# The athaniel-brain runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to athaniel-brain's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Athaniel appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Athaniel floating over another application, so the
    user is rarely asking about Athaniel — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Athaniel "
        ...
        "rather than anything inside Athaniel. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Athaniel terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the Athaniel desktop app, a graphical chat "
        ...
        "data-athaniel-send=\"prompt\" on any clickable element (or "
        "window.athaniel.send(\"prompt\")) sends that prompt as a hidden user "
    ...
    "api_server": (
        "You're responding through the API server. Its consumers render "
        "GitHub-flavored markdown (the Face thread, Word chat): headings, bullet "
        "and numbered lists, tables, code fences, and links all display properly. "
        "Use that structure whenever it makes the answer easier to scan — a list "
        "for parallel items, a table for comparisons, a fence for code or commands "
        "— and plain prose when a sentence suffices. Conversational, not padded. "
        "Diagrams: when the answer needs an architecture, workflow, sequence, "
        "dataflow, or lifecycle diagram, use the archify skill and put the "
        "validated JSON in a ```archify <type> fence inside the answer — the "
        "Face renders it inline as a figure. "
        "Charts: when the answer needs a bar, line, area, point, or arc chart of "
        "data you have, put a Vega-Lite JSON spec in a ```chart fence inside the "
        "answer — the Face renders it inline, monochrome; do not set colours or "
        "scale ranges in the spec. "
        "Math: inline as $...$ and display as $$...$$ (the Face renders KaTeX); "
        "never \\( \\) or \\[ \\], and use \\cr for matrix row breaks, since "
        "markdown eats those backslashes. "
        "Sequence, state, and gantt diagrams go in a ```mermaid fence "
        "(sequenceDiagram / stateDiagram-v2 / gantt; the Face renders them "
        "inline in ink) — architecture, workflow, dataflow, and lifecycle stay "
        "archify, and no other mermaid kinds render. "
        "Citations: when an answer rests on web pages or files you read, mark the "
        "claims with [1], [2] and END the answer with one ```sources fence holding "
        "a JSON array of {\"id\", \"title\", \"url\", \"snippet\"} — the Face turns "
        "the markers into citation chips and the fence into a collapsed source list. "
        "PDF and CSV files you produce or find: write them as markdown links "
        "(http(s) URLs or app paths like ~/library/files/x.csv) — the Face previews "
        "them inline under the sentence. "
...
# container / remote host rather than on the machine where Athaniel itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Athaniel runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Athaniel itself is running. The host OS, home, and cwd "
                f"of the Athaniel process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Athaniel "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Athaniel
    ...
    extra = (os.getenv("ATHANIEL_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_athaniel_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ATHANIEL_PLATFORM") or os.environ.get("ATHANIEL_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.athaniel/skills/`` directory.  External dirs
    ...
    # ATHANIEL_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_athaniel_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.athaniel/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_athaniel_home_override(_home_token)
...
def load_soul_md(
    ...
    """Load SOUL.md from ATHANIEL_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the ATHANIEL_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from athaniel_cli.config import ensure_athaniel_home
        ensure_athaniel_home()
    except Exception as e:
        logger.debug("Could not ensure ATHANIEL_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_athaniel_home()
...
def _load_athaniel_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.athaniel.md / ATHANIEL.md — walk to git root."""
    athaniel_md_path = _find_athaniel_md(cwd_path)
    if not athaniel_md_path:
        return ""
    try:
        content = athaniel_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = athaniel_md_path.name
        try:
            rel = str(athaniel_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".athaniel.md", context_length=context_length,
            read_path=str(athaniel_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", athaniel_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .athaniel.md / ATHANIEL.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ATHANIEL_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Athaniel install/source
    ...
    # explicitly configured cwd is honored verbatim — the Athaniel tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Athaniel in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Athaniel install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_athaniel_md(cwd_path, context_length)
    ...
    # SOUL.md from ATHANIEL_HOME only — skip when already loaded as identity
    if not skip_soul:
