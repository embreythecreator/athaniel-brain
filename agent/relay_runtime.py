"""Profile-scoped NeMo Relay runtimes owned by the Athaniel agent core."""
...
SESSION_SCOPE = "athaniel.session"
TURN_SCOPE = "athaniel.turn"
LOGICAL_LLM_SCOPE = "athaniel.logical_llm_call"
RUNTIME_SCHEMA_KEY = "athaniel.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "athaniel.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "athaniel.relay.runtime_instance"
RELAY_PLUGINS_EXECUTION_CONSUMER = "athaniel.nemo_relay.plugins"
...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when Athaniel finalization forwards runtime metadata. Filter to
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Athaniel session."""
    ...
    def run_in_session(
        ...
        **kwargs: Any,
        ...
        with self._lock:
            ...
            if runtime is None:
                raise RuntimeError("Athaniel Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "Athaniel Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "Athaniel could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside Athaniel native ownership; leaving it unchanged and "
                    "disabling Athaniel-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "Athaniel Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # Athaniel only enters Relay's initialization path after an
                ...
                logger.warning(
                    "Athaniel Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
            ...
            logger.info(
                ...
                "hosted by this Athaniel process."
            )
            return state
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Athaniel Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "Athaniel Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Athaniel-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    ...
                    old_handle,
                    output={"athaniel.session.segment_reason": reason},
                    metadata={
                        RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                        RUNTIME_INSTANCE_KEY: self.runtime_id,
                    },
                ...
                logger.warning(
                    "Athaniel Relay segment close failed (session=%s segment=%d); "
            ...
            scope_metadata = {
                RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                RUNTIME_INSTANCE_KEY: self.runtime_id,
                "athaniel.session.segment": session.segment,
                "athaniel.session.segment_reason": reason,
            }
            parent_handle = None
            if session.parent_session_id:
                with self._sessions_lock:
            ...
            try:
                ...
                logger.warning(
                    "Athaniel Relay segment open failed (session=%s segment=%d); "
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Athaniel Relay session without creating one."""
        with self._sessions_lock:
            if self._closing:
                return None
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Athaniel session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Athaniel Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Athaniel Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Athaniel Relay runtime is shutting down")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Athaniel session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before Athaniel authorizes a tool call."""
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "Athaniel Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "Athaniel Relay drained %d orphaned scope(s) before closing %s",
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Athaniel Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        if has_active_operations:
            thread = threading.Thread(
                ...
                name=f"athaniel-nemo-relay-shutdown-{self.runtime_id[:8]}",
                daemon=True,
            )
            try:
                thread.start()
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "Athaniel Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Athaniel Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Athaniel Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Athaniel profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Athaniel Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one Athaniel turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "athaniel_relay_turn", default=None
)

# Depth of managed Relay callbacks executing on the current logical call path.
# Set >0 while the native Relay pipeline is mid-dispatch of a Athaniel callback
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "athaniel_relay_managed_callback_depth", default=0
)
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Athaniel core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Athaniel Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"athaniel.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "Athaniel Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Athaniel Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Athaniel turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Athaniel turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Athaniel Relay segment rotation failed", exc_info=True
                )
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "athaniel.execution_surface": lease.platform or "unknown",
                    },
                    timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning("Athaniel Relay turn initialization failed", exc_info=True)
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "Athaniel Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
                ...
                try:
                    ...
                    logger.warning(
                        "Athaniel Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "Athaniel Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Athaniel Relay compaction notification failed", exc_info=True
            )
    ...
    def _finish_logical_calls(
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            logger.warning("Athaniel Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a Athaniel tool). Nested managed execution here is structurally
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Athaniel session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Athaniel Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at Athaniel's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Athaniel core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Athaniel Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Athaniel session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Athaniel Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Athaniel Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Athaniel session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Athaniel Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active Athaniel profile."""
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "Athaniel [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "Athaniel Relay plugin configuration could not be loaded from "
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="athaniel-nemo-relay-plugin-lifecycle",
        daemon=True,
    )
    thread.start()
    thread.join()
