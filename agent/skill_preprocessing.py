...
# Matches ${ATHANIEL_SKILL_DIR} / ${ATHANIEL_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${ATHANIEL_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(ATHANIEL_SKILL_DIR|ATHANIEL_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${ATHANIEL_SKILL_DIR} / ${ATHANIEL_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "ATHANIEL_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "ATHANIEL_SESSION_ID" and session_id:
