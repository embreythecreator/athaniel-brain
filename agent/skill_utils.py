...
from athaniel_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.athaniel/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    ATHANIEL_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ATHANIEL_KANBAN_TASK`` / ``ATHANIEL_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ATHANIEL_KANBAN_TASK") or os.getenv("ATHANIEL_KANBAN_BOARD"):
        ...
        # The Athaniel Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``athaniel_cli.config`` on the
...
# `athaniel-brain` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Athaniel itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Athaniel, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"athaniel-brain"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ATHANIEL_PLATFORM`` or
            ``ATHANIEL_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("ATHANIEL_PLATFORM")
        or get_session_env("ATHANIEL_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``athaniel config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``athaniel`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.athaniel/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``athaniel`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ATHANIEL_HOME, not cwd
        if not p.is_absolute():
            p = (athaniel_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.athaniel/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.athaniel/skills`` at the git root) are
...
#   <root>/.athaniel/skills/   — Athaniel-native location
...
# a one-line "run `athaniel skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when ATHANIEL_HOME itself lives inside a git checkout:
    ``<root>/.athaniel/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``athaniel skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`athaniel skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under ATHANIEL_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_athaniel_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.athaniel/skills/`` (including via symlinks) or configured
    ...
    # ~/.athaniel/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Athaniel can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    athaniel = metadata.get("athaniel") or {}
    if not isinstance(athaniel, dict):
        athaniel = {}
    return {
        "fallback_for_toolsets": athaniel.get("fallback_for_toolsets", []),
        "requires_toolsets": athaniel.get("requires_toolsets", []),
        "fallback_for_tools": athaniel.get("fallback_for_tools", []),
        "requires_tools": athaniel.get("requires_tools", []),
        ...
        "session_platforms": athaniel.get("session_platforms", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    athaniel = metadata.get("athaniel")
    if not isinstance(athaniel, dict):
        return []
    raw = athaniel.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Athaniel metadata, VCS, virtualenv/dependency, cache, and skill
