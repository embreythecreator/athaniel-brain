...
# Wire-name used when Athaniel keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Athaniel's configured provider
# (Firecrawl / Exa / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "athaniel_web_search"
...
# rename on the wire (athaniel_<name>), map back in normalize_response so
# Athaniel dispatch is unaffected.
...
# Athaniel's progressive-disclosure bridge registers exactly that literal
...
_RESERVED_TOOL_ALIAS_PREFIX = "athaniel_"
...
# named ``athaniel_tool_search`` is never silently rewritten to ``tool_search``
# unless THIS request actually emitted that alias (#95003 review contract).
_LEGACY_ALIAS_FALLBACK = {
    **_RESERVED_ALIAS_TO_NAME,
    "athaniel_web_search": "web_search",
}
...
def _alias_reserved_tools(
    ...
    a legitimate user/plugin/MCP tool already named ``athaniel_<x>`` is
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Athaniel web providers.
        #
        ...
        #    keep Athaniel dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``athaniel_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo THIS request's wire aliases before Athaniel dispatch.
                ...
                # ``athaniel_tool_search`` etc. is dispatched as itself when
