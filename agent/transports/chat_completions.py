...
_XAI_TOOL_SEARCH_ALIAS = "athaniel_tool_search"
...
def _rename_tool_search_bridge_for_xai(
    ...
    occupies ``athaniel_tool_search``, the bridge takes a ``_2``/``_3``
...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    ...
    Athaniel' internal effort set extends the wire vocabulary with ``ultra``
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Athaniel/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from Athaniel'
    ...
    # is stricter (low/high). Clamp Athaniel' wider effort set to what each
...
def _raise_gemini_thinking_max_tokens(
    ...
    global Athaniel cap of 4096 is enough for visible text, but Ultra/high
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        - Athaniel-internal scaffolding markers — any top-level message key
        ...
        for msg_idx, msg in enumerate(messages):
            ...
            # Drop all Athaniel-internal scaffolding markers (``_``-prefixed).
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if message_tool_calls:
            ...
            for tc in message_tool_calls:
                ...
                # preserve an explicit blank name for Athaniel's recovery path.
                ...
                # ``athaniel_tool_search`` dispatches as itself when no alias
