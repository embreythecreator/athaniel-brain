...
Owns one Codex thread per Athaniel session. Drives `turn/start`, consumes
...
# Athaniel' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ATHANIEL_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Athaniel/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Athaniel session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ATHANIEL_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ATHANIEL_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="athaniel",
            client_title="Athaniel Brain",
            client_version=_get_athaniel_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Athaniel' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Athaniel' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own athaniel-tools callback we
            # auto-accept — the user already approved Athaniel' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Athaniel
        approval-mode/timeout logic. The Athaniel-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Athaniel approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Athaniel approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Athaniel returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Athaniel approval mode/timeout resolution
    ...
    # "prompt expired"; the Athaniel-side messaging already distinguishes them.
    return "decline"
...
def _get_athaniel_version() -> str:
    """Best-effort Athaniel version string for codex's userAgent line."""
    try:
