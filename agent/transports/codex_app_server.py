...
projection into Athaniel' display, approval bridging, transcript projection into
...
"codex_app_server"`. Athaniel' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Athaniel secret — gateway
        ...
        spawn_env = athaniel_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ATHANIEL_KANBAN_TASK"):
            kanban_db = spawn_env.get("ATHANIEL_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "ATHANIEL_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ATHANIEL_HOME", os.path.expanduser("~/.athaniel")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "athaniel",
        client_title: str = "Athaniel Brain",
