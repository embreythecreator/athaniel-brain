...
``kind: backend``) or ``~/.athaniel/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`athaniel_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.athaniel/.env``) so credentials set
    through Athaniel' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``athaniel tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``athaniel tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``athaniel_cli/tools_config.py`` to inject this provider as a
