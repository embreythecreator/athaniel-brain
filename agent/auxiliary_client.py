...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.athaniel/auth.json active provider)
...
from athaniel_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``ATHANIEL_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `athaniel update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Athaniel owns auxiliary retry + provider/model fallback policy (the
    ...
    # Athaniel sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Athaniel control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="athaniel-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Athaniel triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from athaniel_cli.auth import resolve_api_key_provider_credentials
        from athaniel_cli.models import fetch_models_with_pricing
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://athaniel-brain.embreythecreator.com",
    "X-Title": "Athaniel Brain",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ATHANIEL_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ATHANIEL_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ATHANIEL_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ATHANIEL_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "AthanielAgent",
}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://athaniel-brain.nousresearch.com",
    "X-Title": "Athaniel Brain",
...
# in lockstep with athaniel_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``athaniel_cli.__version__`` is
...
_AUTH_JSON_PATH = get_athaniel_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from athaniel_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("ATHANIEL_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Athaniel-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Athaniel plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``athaniel_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.athaniel/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from athaniel_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from athaniel_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``athaniel auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``athaniel_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Athaniel auth store.
    ...
    try:
        from athaniel_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from athaniel_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from athaniel_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if not _aux_probe_active():
        ...
        try:
            from athaniel_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `athaniel setup` or `athaniel model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=athaniel_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`athaniel_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from athaniel_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second athaniel
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``athaniel config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from athaniel_cli.nous_account import get_nous_portal_account_info
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_athaniel_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``athaniel_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._athaniel_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Athaniel' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `athaniel model` but the
    #    old OPENAI_BASE_URL lingers in ~/.athaniel/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: athaniel model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.athaniel/.env",
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_athaniel_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_athaniel_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = athaniel_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Athaniel owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: athaniel auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: athaniel model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: athaniel model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: athaniel doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("athaniel_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from athaniel_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(athaniel_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`athaniel_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Athaniel will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nous", "nous-portal", "embreythecreator"}:
            from athaniel_cli.providers import nous_api_mode
    ...
    if _provider_for_portal in {"nous", "nous-portal", "embreythecreator"}:
    ...
    # Anthropic Messages adapters translate Athaniel reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nous_on_messages = False
        if provider_norm in {"nous", "nous-portal", "embreythecreator"}:
            from athaniel_cli.providers import nous_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Athaniel reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `athaniel model`."
                    )
