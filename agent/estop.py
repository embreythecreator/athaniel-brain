...
``athaniel pause`` writes a sentinel file at ``$ATHANIEL_HOME/ESTOP``;
``athaniel resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Athaniel is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.athaniel/ESTOP``.
...
def _athaniel_home() -> Path:
    """Resolve the active ATHANIEL_HOME (profile-aware) at call time."""
    try:
...
def _canonical_root() -> Path:
    """Fleet-wide Athaniel root, even when this process is a profile gateway.

    Profile gateways launch with ATHANIEL_HOME=~/.athaniel/profiles/<name>.
    ``athaniel pause`` from an operator seat writes ~/.athaniel/ESTOP. If we
    ...
    try:
        ...
        return Path(get_default_athaniel_root())
    except Exception:
        return Path(os.path.expanduser("~/.athaniel"))


def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `athaniel pause`."""
    return _athaniel_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    Engaged if ANY candidate sentinel exists: the process ATHANIEL_HOME
    (profile-local) or the fleet canonical root (~/.athaniel). Fail SAFE on
...
def disengage() -> bool:
    ...
    ``athaniel resume`` from a profile gateway still clears an operator pause
    written at ~/.athaniel/ESTOP.
    """
    lifted = False
    for path in _candidate_sentinel_paths():
        try:
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Athaniel is paused ({reason}). New work is on hold; "
            "run `athaniel resume` to pick things back up."
        )
    return (
        "⏸️ Athaniel is paused. New work is on hold; "
        "run `athaniel resume` to pick things back up."
    )
