...
See the ``athaniel-brain-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Athaniel through a client
    shim, and a shim that cannot carry Athaniel tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_ATHANIEL_TOOL_CALLS = False``
    ...
    for candidate in (client, type(client) if client is not None else None):
        if candidate is None:
            continue
        supported = getattr(candidate, "SUPPORTS_ATHANIEL_TOOL_CALLS", None)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Athaniel, e.g. 'athaniel-brain').\n"
    "  • Hub-installed skills (installed via 'athaniel skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'athaniel curator pin'). You are an "
    ...
    "'athaniel curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'athaniel skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'athaniel curator pin'). Pin blocks "
    ...
    "reply and recommend 'athaniel curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("ATHANIEL_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def build_cache_parity_fork(
    ...
    *,
    ...
    # scratch (fresh _athaniel_now() timestamp, fresh
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Athaniel tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit Athaniel tool calls, "
