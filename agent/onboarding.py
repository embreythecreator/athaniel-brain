...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Athaniel starts and finds ``~/.openclaw/``.

    Points users at ``athaniel claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``athaniel claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Athaniel, run "
        "`athaniel claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `athaniel claw cleanup` (renames it to ~/.openclaw.pre-migration — "
