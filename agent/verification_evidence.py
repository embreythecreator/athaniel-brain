...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("athaniel-verify-", "athaniel-ad-hoc-")
...
def _db_path() -> Path:
    return get_athaniel_home() / "verification_evidence.db"
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "athaniel verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``athaniel verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``athaniel verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="athaniel verify",
        kind="verify",
