...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``ATHANIEL_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``ATHANIEL_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``ATHANIEL_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("ATHANIEL_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown ATHANIEL_DEV_SUBSCRIPTION_FIXTURE: {name}")
