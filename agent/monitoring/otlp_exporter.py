...
configured under ``monitoring.export.otlp``. Lets an operator stream Athaniel
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install athaniel-brain[otlp]``), imported lazily so the
...
def _require_sdk(*, auto_install: bool = True, prompt: bool = True):
    ...
    try:
        ...
        raise OTLPUnavailable(
            ...
            "    pip install 'athaniel-brain[otlp]'\n"
            f"(import error: {e})"
        )
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"athaniel.event": kind or "unknown"}
    keep_by_kind = {
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'athaniel-brain[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
