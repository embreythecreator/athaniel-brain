...
def _athaniel_home_path() -> Path:
    """Resolve the active ATHANIEL_HOME (profile-aware) without circular imports."""
    try:
        from athaniel_constants import get_athaniel_home  # local import to avoid cycles
        return get_athaniel_home()
    except Exception:
        return Path(os.path.expanduser("~/.athaniel"))


def _athaniel_root_path() -> Path:
    """Resolve the Athaniel root dir (always the parent of any profile, never per-profile)."""
    try:
        from athaniel_constants import get_default_athaniel_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    athaniel_home = _athaniel_home_path()
    athaniel_root = _athaniel_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(athaniel_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(athaniel_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(athaniel_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(athaniel_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(athaniel_home / "cache" / "bws_cache.enc.json"),
            str(athaniel_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved ATHANIEL_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("ATHANIEL_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    athaniel_dirs = []
    for base in (_athaniel_home_path(), _athaniel_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in athaniel_dirs:
                athaniel_dirs.append(real)
        except Exception:
            continue

    for base_real in athaniel_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside ATHANIEL_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Athaniel path.

    Three categories are blocked:

      * Internal Athaniel cache files under ``ATHANIEL_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ATHANIEL_HOME and the global Athaniel
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.athaniel/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ATHANIEL_HOME (profile-aware) AND the global
    # Athaniel root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ATHANIEL_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    athaniel_dirs: list[Path] = []
    for base in (_athaniel_home_path(), _athaniel_root_path()):
        try:
            real = base.resolve()
            if real not in athaniel_dirs:
    ...
    for hd in athaniel_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Athaniel cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # ATHANIEL_HOME or <root>.
    ...
    for hd in athaniel_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Athaniel credential store "
    ...
    for hd in athaniel_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Athaniel MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Athaniel MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in athaniel_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Athaniel real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Athaniel real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Athaniel read (see
...
#
# Athaniel profiles are separate ATHANIEL_HOME dirs under
...
#
# Reference: May 2026 incident where a athaniel-security profile session
# edited skills under both ``~/.athaniel/profiles/athaniel-security/skills/``
# AND ``~/.athaniel/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ATHANIEL_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ATHANIEL_HOME.

    ``~/.athaniel``              -> ``"default"``
    ``~/.athaniel/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _athaniel_home_path().resolve()
        root_real = _athaniel_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Athaniel scope, or is inside
...
#
#   <ATHANIEL_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.athaniel/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.athaniel/…`` segment and warns
# regardless of which Athaniel profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.athaniel/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.athaniel`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.athaniel/…`` and returns the
    index where the inner Athaniel-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .athaniel / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".athaniel":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Athaniel state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.athaniel``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.athaniel`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Athaniel resolver to
    succeed, so it works correctly even when called from contexts where
    ATHANIEL_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Athaniel process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ATHANIEL_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.athaniel/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.athaniel/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ATHANIEL_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Athaniel state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Athaniel "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ATHANIEL_HOME. Use the "
