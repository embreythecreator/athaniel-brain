...
The parent Athaniel process may itself be a Kanban dispatcher worker with
ATHANIEL_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's ATHANIEL_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "athaniel_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "ATHANIEL_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``ATHANIEL_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$ATHANIEL_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``ATHANIEL_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``ATHANIEL_KANBAN_*`` vars while losing the ContextVar in
