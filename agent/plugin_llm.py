...
Plugins built on Athaniel Brain often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Athaniel
...
:class:`~athaniel_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Athaniel' main config
...
handles every provider, fallback chain, and per-task override Athaniel
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`athaniel_cli.plugins.PluginContext`
