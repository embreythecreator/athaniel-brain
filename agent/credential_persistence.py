...
``auth.json``.  They intentionally have no dependency on ``athaniel_cli.auth`` so
...
# Sources Athaniel owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Athaniel-owned OAuth/device-code state)
