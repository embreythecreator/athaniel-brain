"""OpenAI-shape bridge shared by Athaniel' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Athaniel as
...
the agent's *own* tool notifications. Athaniel' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Athaniel' perspective).
...
of its own (Copilot) forwards everything Athaniel offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Athaniel'
agent-level tools, because re-offering the overlapping ones makes Athaniel re-run
...
class StreamChunks(list):
    ...
    Athaniel reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``athaniel_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Athaniel'
