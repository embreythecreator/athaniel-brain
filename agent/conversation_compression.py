...
# Mirror athaniel_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``athaniel_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["ATHANIEL_SESSION_ID"] = child_session_id
    try:
...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    receive one normalized host contract instead of having to infer Athaniel
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `athaniel setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Athaniel "
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Athaniel's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Athaniel' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|athaniel|off).
    # The memory-provider context handoff below is intentionally Athaniel-only:
    ...
    try:
            ...
            try:
                ...
                if in_place:
                    ...
                    if callable(_parent_row_reader):
                        try:
                            from athaniel_state_common import is_automatic_end_reason
                    ...
                    # whose thread lacks the ATHANIEL_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("ATHANIEL_SESSION_SOURCE", "cli"),
                        model=agent.model,
                    ...
                    try:
                        from athaniel_cli.goals import migrate_goal_to_session
                        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
                    except Exception as _goal_err:
                        logger.debug("Could not migrate goal on compression: %s", _goal_err)
                    # Same boundary hazard for /heartbeat state — carry it too.
                    try:
                        from athaniel_cli.heartbeat import migrate_heartbeat_to_session
        ...
        # engines (e.g. athaniel-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See athaniel-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Athaniel' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Athaniel' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "athaniel", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "athaniel":
        logger.info(
    ...
    # way the Athaniel path below does. An active cooldown means a recent
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="athaniel_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
