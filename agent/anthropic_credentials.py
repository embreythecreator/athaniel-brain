...
3. ``~/.athaniel/.anthropic_oauth.json`` (Athaniel PKCE login)
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in athaniel_cli/runtime_provider.py.
    """
...
class CredentialPersistError(RuntimeError):
    ...
    ``~/.athaniel/.anthropic_oauth.json`` for ``athaniel_pkce``).
...
#     authority boundary of ``claude_code``/``athaniel_pkce`` is the shared
#     singleton file, which other Athaniel processes/profiles read with fresh
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    ...
    return source_path.with_name(source_path.name + ".athaniel-spent-rotations.json")
...
def spent_rotation_source_path(source: Any) -> Optional[Path]:
    ...
    if source == "athaniel_pkce":
        return _get_athaniel_oauth_file()
    return None
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by Athaniel so sibling "
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every Athaniel profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Athaniel notices an expired token, Claude Code may have already rotated it.
    ...
    try:
        from athaniel_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float

        refresh_timeout_seconds = env_float(
            "ATHANIEL_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20
        )
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Athaniel historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``athaniel models``) never mutates ``~/.athaniel/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Athaniel)
      ...
      5. Anthropic credential_pool OAuth entry (~/.athaniel/auth.json)
    ...
    # 1. Athaniel-managed OAuth/setup token env var
    ...
    # 5. Athaniel credential_pool OAuth entry.
...
# ── Athaniel-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.athaniel/.anthropic_oauth.json (our own file).
...
def _get_athaniel_oauth_file() -> Path:
    return get_athaniel_home() / ".anthropic_oauth.json"
...
def run_athaniel_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Athaniel-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Athaniel with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from athaniel_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_athaniel_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Athaniel-managed OAuth credentials from ~/.athaniel/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Athaniel OAuth credentials: %s", e)
    return None


def _write_athaniel_oauth_credentials(
    access_token: str,
    refresh_token: Optional[str],
    expires_at_ms: Optional[int],
) -> None:
    """Write refreshed athaniel_pkce tokens back to ~/.athaniel/.anthropic_oauth.json.

    Without this, a successful pool-level refresh of a ``athaniel_pkce``-sourced
    ...
    try:
        ...
        logger.error(
            "Failed to write refreshed Athaniel OAuth credentials to %s: %s", oauth_file, e
        )
