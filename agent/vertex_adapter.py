"""Vertex AI (Google Cloud) adapter for Athaniel Brain.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows Athaniel to use Gemini models via Google Cloud with
...
# who installed plain `athaniel-brain` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Athaniel at Vertex.
    ...
    Athaniel up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Athaniel-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Athaniel-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
