...
Athaniel keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Athaniel's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Athaniel)
        ...
        with httpx.Client(timeout=timeout) as client:
            resp = client.post(
                ...
                headers={
                    ...
                    "X-Goog-Api-Client": f"athaniel-brain/{_ATHANIEL_VERSION}",
                },
            )
...
_FREE_TIER_GUIDANCE = (
    ...
    "for Gemini Flash models). Athaniel typically makes 3-10 API calls per user turn, "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "GOOGLE_API_KEY in ~/.athaniel/.env and restart your session. "
    "Details: https://ai.google.dev/gemini-api/docs/api-key"
)


class GeminiAPIError(Exception):
    """Error shape compatible with Athaniel retry/error classification."""
...
def _thinking_requests_output_headroom(thinking_config: Any) -> bool:
    ...
    Athaniel ``max_tokens`` of 4096/16384 is enough for visible text, but
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.athaniel/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `athaniel setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": f"athaniel-brain/{_ATHANIEL_VERSION} (gemini-native)",
            "X-Goog-Api-Client": f"athaniel-brain/{_ATHANIEL_VERSION}",
        }
