...
def _state_file() -> Path:
    return get_athaniel_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.athaniel/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `athaniel update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``athaniel curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``athaniel update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``athaniel curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `athaniel curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Athaniel' background skill CURATOR. This is an "
    ...
    "into ~/.athaniel/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: athaniel-config-*, athaniel-dashboard-*, gateway-*, codex-*, "
    ...
    "`athaniel curator rollback` restores a hollow skill (issue #96962).\n\n"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.athaniel/logs/curator/``)
    ...
    authored skill data in ``~/.athaniel/skills/``.

    ``ensure_athaniel_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_athaniel_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every athaniel install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: athaniel curator status
        keep an umbrella stable: athaniel curator pin document-tools
    ...
    lines.append("full report: athaniel curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: athaniel curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.athaniel/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.athaniel/skills/.archive/` for "
            "safety and can be restored via `athaniel curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.athaniel/skills/.archive/`. "
            "Restore any via `athaniel curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `athaniel curator restore <name>`")
    lines.append("- All archives live under `~/.athaniel/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``athaniel curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `athaniel curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `athaniel update` only if "
        ...
        # recorded in state so `athaniel curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``athaniel model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `athaniel model` → auxiliary
    ...
    try:
        ...
        review_agent = AIAgent(
            ...
            **_agent_kwargs,
            ...
            # followed snapshotted an already-stripped package and ``athaniel
