...
Providers live in ``~/.athaniel/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.athaniel/cache`` files land
  inside the backend (e.g. ``"~/.athaniel"`` for home-synced backends,
  ``"/root/.athaniel"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_athaniel_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Athaniel cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``athaniel setup`` after this backend is selected.
