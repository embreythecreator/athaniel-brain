...
upstream prefix cache warm.  See ``athaniel-brain-dev``'s
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- athaniel-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ATHANIEL_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ATHANIEL_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the ATHANIEL_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from athaniel_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from athaniel_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound ATHANIEL_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Athaniel never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ATHANIEL_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the ATHANIEL_HOME ContextVar
    ...
    # Pointer to the docs (and, when it exists, the athaniel-brain skill) for
    # user questions about Athaniel itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the athaniel-brain skill not installed — so the
    ...
    stable_parts.append(ATHANIEL_AGENT_HELP_GUIDANCE_NO_SKILLS)
    ...
    # ATHANIEL_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # the athaniel-brain skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- athaniel-brain:" in skills_prompt:
        stable_parts[_help_guidance_slot] = ATHANIEL_AGENT_HELP_GUIDANCE
    ...
    # Coding posture (base Athaniel, any interactive coding surface in a code
    ...
    # `athaniel -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['athaniel-bots']).
    ...
    # Active-profile hint — names the Athaniel profile the agent is running
    # under so it doesn't conflate ~/.athaniel/skills/ (default profile) with
    # ~/.athaniel/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient ATHANIEL_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_athaniel_root(): get_athaniel_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_athaniel_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_athaniel_root())
    else:
        _home_str = _root_str = str(get_athaniel_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Athaniel profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_athaniel_root()), which in ambient
        # profile mode is NOT get_athaniel_home().
        profile_home = _home_str
        default_root = get_default_athaniel_root()
        post_workspace_parts.append(
            f"Active Athaniel profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Athaniel). Every other surface (desktop chat panel,
        ...
        if getattr(agent, "_context_cwd_is_launch_artifact", False):
            ...
            # then reject Athaniel's bundled contributor AGENTS.md (#97448).
            context_cwd = None
    ...
    from athaniel_time import get_timezone as _athaniel_tz, now as _athaniel_now
    now = _athaniel_now()
