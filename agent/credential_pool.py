...
import athaniel_cli.auth as auth_mod
...
# un-throttled log fires on *every* selection. On Windows several Athaniel
...
# readiness handshake ("Timed out connecting to Athaniel backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``athaniel_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.athaniel/auth.json`` / profile equivalent), so it works for
        every *pool-owned* Anthropic source - ``athaniel_pkce`` and
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``athaniel model`` / ``athaniel auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Athaniel process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``athaniel`` invocation that defaults to the active provider
        (e.g. setup wizard, ``athaniel auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Athaniel
        ...
        # Previously "anthropic" was excluded from this tuple: two Athaniel
        ...
        # "claude_code" (athaniel_pkce, dashboard-issued manual entries) —
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Athaniel profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Athaniel process) that might
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        ``athaniel_pkce`` sources the singleton file — not ``auth.json`` — is the
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ATHANIEL_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ATHANIEL_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ATHANIEL_XAI_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "xai-oauth"
            else "ATHANIEL_ANTHROPIC_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("athaniel_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # Same rationale for the singleton source athaniel_pkce:
                # _seed_from_singletons() reads ~/.athaniel/.anthropic_oauth.json
                ...
                # Do not use endswith here: manual:athaniel_pkce is already
                # pool-owned, and creating a singleton for it would introduce
                # a second authority for the same refresh-token family.
                elif entry.source == "athaniel_pkce":
                    try:
                        from agent.anthropic_credentials import _write_athaniel_oauth_credentials
                        _write_athaniel_oauth_credentials(
                            refreshed["access_token"],
                            refreshed["refresh_token"],
                            refreshed["expires_at_ms"],
                        )
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.athaniel/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token — single-use tokens consumed by another Athaniel
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("athaniel_pkce"),
                        )
                ...
                # Backstop for non-claude_code sources (athaniel_pkce,
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Athaniel process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Athaniel profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `athaniel model` / `athaniel auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `athaniel model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``athaniel auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `athaniel auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `athaniel auth remove <provider> <N>` is stable across all source types.
    try:
        from athaniel_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Athaniel PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `athaniel setup` ("Claude
        ...
        # Claude Code CLI, athaniel_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.athaniel/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `athaniel setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"athaniel_pkce", "claude_code"}
            ]
        ...
        for source_name, creds in (
            ("athaniel_pkce", read_athaniel_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `athaniel auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (athaniel auth remove copilot gh_cli) must not pay either on
        ...
        # Athaniel auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from athaniel_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.athaniel/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `athaniel auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `athaniel auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Athaniel auth store.  Without this gate the removal is instantly
        ...
        # Athaniel owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `athaniel auth openai-codex`.
        ...
        # When the user logs in via ``athaniel model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``athaniel auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from athaniel_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.athaniel/.env over os.environ — the user's config file is the
# authoritative source for Athaniel credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_athaniel_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `athaniel auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.athaniel/.env.
    ...
    try:
        from athaniel_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from athaniel_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.athaniel/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `athaniel auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Athaniel
