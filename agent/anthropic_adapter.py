"""Anthropic Messages API adapter for Athaniel Brain.

Translates between Athaniel's internal OpenAI-style message format and
...
# Athaniel effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to athaniel's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to athaniel's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + AthanielAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://athaniel-brain.nousresearch.com")
        headers.setdefault("X-Title", "Athaniel Brain")
        headers.setdefault("User-Agent", f"AthanielAgent/{_ATHANIEL_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Athaniel loads that into the process env from
    # ``~/.athaniel/.env``). The result is dual auth —
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Athaniel Brain", "Claude Code")
                text = text.replace("Athaniel agent", "Claude Code")
                text = text.replace("athaniel-brain", "claude-code")
                text = text.replace("Athaniel", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Athaniel-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Athaniel surfaces in its CLI. We
