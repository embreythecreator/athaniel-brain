#!/usr/bin/env python3
"""
SQLite State Store for Athaniel Brain.
...
# transitive dependency via athaniel_state_common). run_agent keeps its own
# predating copy — athaniel_state cannot import run_agent (circular) — guarded
...
from athaniel_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, cast

from athaniel_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from athaniel_state_portability import SessionPortabilityMixin
from athaniel_state_schema import SessionSchemaMixin
from athaniel_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``athaniel -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_athaniel_home() / "state.db"
...
#
# The ceilings above bound Athaniel's SQLite descriptors, which is only ever part
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.athaniel`` even when a test fixture later
    redirects ``ATHANIEL_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_athaniel_home()`` fresh so a runtime
       ``ATHANIEL_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_athaniel_home() / "state.db"
...
# ~/.athaniel/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects ATHANIEL_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without ATHANIEL_HOME, a stale worktree
# without the re-pin, or a developer shell that exports ATHANIEL_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "ATHANIEL_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.athaniel``).  The test conftest injects the pre-sandbox production
#: root here so custom-``ATHANIEL_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Athaniel root for the guard.

    Deliberately avoids ``Path.home()`` / ``athaniel_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``athaniel_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "athaniel"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "athaniel"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".athaniel"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``ATHANIEL_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Athaniel test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``ATHANIEL_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "ATHANIEL_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``ATHANIEL_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``athaniel`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Athaniel home *root*.
    ...
    under ``~/.athaniel/athaniel-brain/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``ATHANIEL_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``ATHANIEL_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Athaniel root {root}). "
                "Tests must run against a temporary ATHANIEL_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ATHANIEL_HOME. If this test genuinely needs the live "
...
# athaniel_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``athaniel sessions optimize`` (FTS merge + VACUUM)
    ...
    ``athaniel_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``athaniel_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``athaniel update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Athaniel-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Athaniel"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Athaniel-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `athaniel doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``athaniel_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see athaniel_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with athaniel_cli.config.
        from athaniel_cli.config import cfg_get, load_config_readonly
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``athaniel doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Athaniel host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``athaniel serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Athaniel's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``athaniel_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        st = db_path.stat()
        try:
            from athaniel_cli.sqlite_safe_read import (
                LiveConnectionError,
                offline_file_access,
            )
        except ImportError:
            # Scaffold/embed installs ship athaniel_state without athaniel_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``athaniel_cli.sqlite_safe_read``).
    ...
    try:
        from athaniel_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Athaniel home tree (``get_athaniel_home()``) — the safe repair scope:
      Athaniel owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``athaniel_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_athaniel_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Athaniel needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (athaniel_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_athaniel_state.py:600-632)
        ...
        probe_session_id = f"_athaniel_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Athaniel deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (athaniel gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.athaniel/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``athaniel sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ATHANIEL_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_athaniel_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ATHANIEL_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship athaniel_state without athaniel_cli). A
    ...
    """
    try:
        from athaniel_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "athaniel_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch athaniel_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``athaniel_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from athaniel_cli.backup import is_zeroed_sqlite_file
    ...
    if size < 0:
        return False
    from athaniel_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `athaniel snapshot list` / "
                "`athaniel snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (athaniel doctor, dashboards) ──────────
...
_ATHANIEL_CMDLINE_MARKERS = ("athaniel_cli.main", "athaniel_cli/main", "athaniel serve",
                           "athaniel-brain", "athaniel gateway", "athaniel chat")


def _looks_like_athaniel(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Athaniel process?
    ...
    We only flag processes that look like Athaniel, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _ATHANIEL_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple athaniel processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Athaniel
    ...
    # `athaniel update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `athaniel snapshot list` / "
                        f"`athaniel snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`athaniel db optimize`),
            ...
            # ``athaniel_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `athaniel update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `athaniel sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Athaniel process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Athaniel process —
                        # only flag uninspectable holders that look like
                        # another Athaniel instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_athaniel(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from athaniel_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `athaniel sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`athaniel sessions optimize-storage`) ──
    #
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_athaniel_root().resolve()
        ...
        def _do(conn):
            ...
            conn.execute(
                """INSERT INTO sessions (
                   ...
                   parent_session_id, fork_message_id, cwd, profile_name, git_repo_root,
                   ...
                   ON CONFLICT(id) DO UPDATE SET
                       ...
                       fork_message_id = COALESCE(sessions.fork_message_id, excluded.fork_message_id),
    ...
    def fork_session(self, source_id: str, fork_id: str, *, title=None,
                     fork_message_id=None, anchor=None, exclude_anchor=False) -> str:
        """Publish a transcript fork and its lineage in one write transaction.

        An optional exact display-content anchor selects the cutoff. Never
        silently fork the wrong turn if a Face cache is stale or compacted.
        Regenerate excludes the anchor user row; the next completion sends it.
        The original remains resumable and every original message is retained.
        """
        title = self.sanitize_title(title)

        def _do(conn):
            source = conn.execute("SELECT * FROM sessions WHERE id = ?", (source_id,)).fetchone()
            if source is None:
                raise ValueError("Source session no longer exists")
            if conn.execute("SELECT 1 FROM sessions WHERE id = ?", (fork_id,)).fetchone():
                raise ValueError("Fork session already exists")
            self._check_transcript_write_guards(
                conn, source_id, None, reject_active_turn_lease=True,
                reject_active_compression_lock=True,
            )
            if title and conn.execute("SELECT 1 FROM sessions WHERE title = ?", (title,)).fetchone():
                raise ValueError("Fork title is already in use")
            messages = [dict(row) for row in conn.execute(
                "SELECT * FROM messages WHERE session_id = ? AND active = 1 ORDER BY id", (source_id,)
            )]
            for message in messages:
                message["content"] = self._decode_content(message.get("content"))
            if anchor is not None:
                role, content, occurrence = anchor.get("role"), anchor.get("content"), anchor.get("occurrence", 1)
                if role not in ("user", "assistant") or not isinstance(content, str) or not content.strip():
                    raise ValueError("A nonempty user or assistant fork anchor is required")
                if type(occurrence) is not int or occurrence < 1:
                    raise ValueError("Invalid fork anchor occurrence")
                def anchor_text(message):
                    value = message.get("content")
                    if isinstance(value, list):
                        value = "\n".join(part.get("text", "") for part in value if isinstance(part, dict) and part.get("type") == "text")
                    if not isinstance(value, str):
                        return ""
                    if role == "user" and value.startswith("_____user\n"):
                        value = value[len("_____user\n"):]
                    return value.strip()
                matches = [i for i, message in enumerate(messages)
                           if message.get("role") == role and anchor_text(message) == content.strip()]
                if len(matches) < occurrence:
                    raise ValueError("Fork point is no longer in the active Brain transcript; reload the session")
                cutoff = matches[occurrence - 1]
                if exclude_anchor and role != "user":
                    raise ValueError("Regeneration must start at a user message")
                # Never leave an assistant tool-call without its result.
                if not exclude_anchor and messages[cutoff].get("tool_calls"):
                    while cutoff + 1 < len(messages) and messages[cutoff + 1].get("role") == "tool":
                        cutoff += 1
                messages = messages[:cutoff + (0 if exclude_anchor else 1)]
            elif exclude_anchor or fork_message_id:
                raise ValueError("A fork anchor is required")
            config = json.loads(source["model_config"] or "{}")
            parent_id = source["parent_session_id"] if fork_message_id and source["fork_message_id"] == fork_message_id else source_id
            config["_branched_from"] = parent_id
            conn.execute(
                """INSERT INTO sessions (id, source, model, model_config, system_prompt,
                   system_prompt_hash, parent_session_id, fork_message_id, started_at,
                   title, title_source, cwd, profile_name, git_repo_root, git_branch,
                   user_id, chat_id, chat_type, thread_id)
                   VALUES (?, 'api_server', ?, ?, ?, ?, ?, ?, ?, ?, 'derived', ?, ?, ?, ?, ?, ?, ?, ?)""",
                (fork_id, source["model"], json.dumps(config), source["system_prompt"],
                 source["system_prompt_hash"], parent_id, fork_message_id, time.time(), title,
                 source["cwd"], source["profile_name"], source["git_repo_root"], source["git_branch"],
                 source["user_id"], source["chat_id"], source["chat_type"], source["thread_id"]),
            )
            count, tools = self._insert_message_rows(conn, fork_id, messages)
            conn.execute("UPDATE sessions SET message_count = ?, tool_call_count = ? WHERE id = ?",
                         (count, tools, fork_id))
            conn.execute("UPDATE sessions SET ended_at = ?, end_reason = 'branched' WHERE id = ?",
                         (time.time(), source_id))
    ...
    # offline repair path behind ``athaniel sessions repair-routing``.
    ...
    def try_acquire_session_turn_lease(
        ...
        *,
        ...
        ``kind`` classifies the holder (WO-SESSION/YIELD-1): ``operator`` for
        real people, ``wake``/``background`` for async-delegation self-posts
        and other machine turns, which must honor yield requests.
        """
        if not session_id or not holder:
            return False
        now = time.time()
        expires_at = now + max(0.1, float(ttl_seconds))
        lease_kind = kind if kind in ("operator", "wake", "background") else "operator"
        ...
        def _do(conn):
            ...
            conn.execute(
                ...
                "(conversation_id, holder, acquired_at, expires_at, kind, yield_requested) "
                "VALUES (?, ?, ?, ?, ?, 0)",
                (conversation_id, holder, now, expires_at, lease_kind),
            )
            owner = conn.execute(
    ...
    def acquire_session_turn_lease(
        ...
        *,
        ...
        kind: str = "operator",
        request_yield_on_wait: bool = False,
    ) -> bool:
        ...
        ``request_yield_on_wait`` (WO-SESSION/YIELD-1): an operator waiter
        asks any wake/background holder to yield — set once on the first
        failed acquire and re-set on each notice tick, so a takeover by a
        NEW wake holder mid-wait is asked to yield too. Best-effort: a
        locked write is retried on the next tick, never raised.
        """
        ...
        def _request_yield() -> None:
            if not request_yield_on_wait:
                return
            try:
                self.request_session_turn_yield(session_id)
            except Exception:
                logger.debug(
                    "session turn yield request failed; will retry on next notice",
                    exc_info=True,
                )
    ...
    def request_session_turn_yield(
        self,
        session_id: str,
        *,
        from_kinds: tuple = ("wake", "background"),
    ) -> bool:
        """Ask the current wake/background lease holder to yield the session.

        WO-SESSION/YIELD-1: only machine-class holders are asked — an
        operator-held lease is never marked, so operator-vs-operator
        contention stays FIFO. Returns True when a holder was marked.
        """
        if not session_id:
            return False
        kinds = tuple(k for k in from_kinds if k in ("wake", "background"))
        if not kinds:
            return False
        ...
        def _do(conn):
            ...
            cursor = conn.execute(
                "UPDATE session_turn_leases SET yield_requested = 1 "
                f"WHERE conversation_id = ? AND kind IN ({placeholders})",
                (conversation_id, *kinds),
            )
            return cursor.rowcount > 0

        return bool(self._execute_write(_do, patience_s=0.5))

    def session_turn_yield_requested(self, session_id: str, holder: str) -> bool:
        """Return True when ``holder``'s lease has a pending yield request.

        Read-only and holder-scoped: a successor lease (fresh row,
        ``yield_requested = 0``) never inherits a stale request. Errors
        report False — the poller retries on its next tick.
        """
        if not session_id or not holder:
            return False
        try:
            with self._read_ctx() as conn:
                conversation_id = self._session_turn_lease_key_on_conn(conn, session_id)
                row = conn.execute(
                    "SELECT yield_requested FROM session_turn_leases "
                    "WHERE conversation_id = ? AND holder = ?",
        ...
        if row is None:
            return False
        value = row["yield_requested"] if isinstance(row, sqlite3.Row) else row[0]
        return bool(value)
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``athaniel --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``athaniel serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: athaniel_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: athaniel_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``athaniel -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``athaniel sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``ATHANIEL_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Athaniel, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Athaniel session.

        A Athaniel session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Athaniel session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `athaniel sessions vacuum` runs in a transient
    ...
    def set_execution_policy(
        self, session_id: str, policy: dict, *, expected_revision: int = None
    ) -> bool:
        """Upsert the execution-policy dict for a session.

        With ``expected_revision`` the write is conditional: the stored row
        must still carry that revision or nothing is written and ``False`` is
        returned. The read-modify-write above this is guarded only by
        in-process locks, so the CLI and the gateway — separate processes over
        one state.db — can otherwise interleave and drop a revision pointer.
        """
        ...
        def _do(conn):
            ...
            cursor = conn.execute(
                ...
                UPDATE execution_policy
                 ...
                 WHERE session_id = ?
                   AND COALESCE(json_extract(policy_json, '$.revision'), 0) = ?
                """,
                (payload, now, str(session_id), int(expected_revision)),
            )
            if cursor.rowcount:
                return
            # No row yet is a legitimate first write when the caller expected 0.
            if int(expected_revision) == 0:
                cursor = conn.execute(
                    ...
                    SELECT ?, ?, ?
                     WHERE NOT EXISTS (
                        SELECT 1 FROM execution_policy WHERE session_id = ?
                     )
                    """,
                    (str(session_id), payload, now, str(session_id)),
                )
                if cursor.rowcount:
                    return
            applied = False
    ...
    # ------------------------------------------------------------------
    # Seal grants (WO-FACE/CHAT-1 P0-1) — agent/seal_approval.py
    # ------------------------------------------------------------------

    def _ensure_seal_grant_table(self, conn) -> None:
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS seal_grant (
                session_id TEXT PRIMARY KEY,
    ...
    def get_seal_grant(self, session_id: str) -> Optional[dict]:
        """Return the stored Seal grant state for a session, or None."""
        with self._lock:
            try:
                row = self._conn.execute(
                    "SELECT grant_json FROM seal_grant WHERE session_id = ?",
                    (str(session_id),),
                ).fetchone()
            except sqlite3.OperationalError as e:
                # Table not created yet = genuinely no grants. Anything else
                # (locked DB) must RAISE so SealApprovalStore._load can retry:
                # swallowing a lock here would drop a pending grant, and a
                # dropped grant looks to the operator like a card that silently
                # vanished.
                if "no such table" in str(e).lower():
                    return None
                raise
        if row is None:
            return None
        raw = row["grant_json"] if isinstance(row, sqlite3.Row) else row[0]
        try:
            data = json.loads(raw)
        except (TypeError, ValueError):
            return None
    ...
    def set_seal_grant(self, session_id: str, state: dict) -> bool:
        """Upsert the Seal grant state for a session.

        Deliberately a plain upsert with no compare-and-swap, unlike
        set_execution_policy. The correctness guard for Seal is the nonce check
        in SealApprovalStore.approve, and a lost update there fails CLOSED — the
        stale nonce is refused and the action is re-requested. Adding CAS would
        buy nothing a refusal does not already cover.
        """
        payload = json.dumps(state)
        now = time.time()

        def _do(conn):
            self._ensure_seal_grant_table(conn)
            conn.execute(
                """
                INSERT INTO seal_grant (session_id, grant_json, updated_at)
                VALUES (?, ?, ?)
                ON CONFLICT(session_id) DO UPDATE SET
    ...
    def clear_seal_grant(self, session_id: str) -> None:
        """Delete the Seal grant row for a session."""
        def _do(conn):
            try:
                conn.execute(
                    "DELETE FROM seal_grant WHERE session_id = ?",
                    (str(session_id),),
                )
            except sqlite3.OperationalError:
                pass
