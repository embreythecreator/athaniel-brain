...
Supersedes: PR #31781 (prompt_toolkit `athaniel profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `athaniel profile wizard` in the terminal.
...
needs, and a profile is just a ATHANIEL_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.athaniel/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — ATHANIEL_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with ATHANIEL_HOME override |
...
### Seam #1 — hub-skill install cannot use the ATHANIEL_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = ATHANIEL_HOME / "skills"` at **module
import time**. The context-local `set_athaniel_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_athaniel_action`
runs `python -m athaniel_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_athaniel_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's ATHANIEL_HOME bound
...
spawned detached (`_spawn_athaniel_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, ATHANIEL_HOME override)
3. write selected MCP servers (synchronous, ATHANIEL_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `athaniel -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, ATHANIEL_HOME override
...
page afterward. Mirror `_write_profile_model`'s ATHANIEL_HOME-override helper for
...
- Backend E2E with isolated ATHANIEL_HOME: POST a full create body
