...
`athaniel config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.athaniel/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ATHANIEL_HOME`, secret scope, and
