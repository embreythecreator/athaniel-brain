...
Athaniel supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.athaniel/config.yaml`:
...
Or set the env var: `ATHANIEL_KANBAN_DISPATCH_IN_GATEWAY=false`
