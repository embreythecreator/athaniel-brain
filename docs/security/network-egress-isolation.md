...
When running Athaniel inside Docker, the default `network_mode: host` gives the
...
The Athaniel [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ athaniel-brain │   │ athaniel-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   ATHANIEL_UID=$(id -u) ATHANIEL_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=athaniel,athaniel-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://athaniel-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Athaniel trust model and vulnerability reporting
