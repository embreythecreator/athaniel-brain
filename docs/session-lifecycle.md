...
The canonical transcript store is SQLite via `SessionDB` (from `athaniel_state`). The
...
Counts consecutive restarts via a JSON file (`{ATHANIEL_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `athaniel update`, `athaniel gateway restart`,
or `/restart`.
