...
This document is the formal interface between the **Athaniel gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `athaniel gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`athaniel gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `athaniel` CLI.
