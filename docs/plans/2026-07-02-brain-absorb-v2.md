...
**Goal:** Harden `athaniel absorb` with a machine-checked divergence gate, an auto-verify battery, a real conflict-resolution flow, and automated bookkeeping — human stays the `--commit` gate.

**Architecture:** Extend the existing `athaniel_cli/absorb/` modules in place (approved spec: `docs/specs/2026-07-02-brain-absorb-v2-design.md`). Two new modules (`divergence.py`, `verify.py`), guard/bookkeeping upgrades to `driver.py` and `parity_report.py`, four new CLI flags in `main.py`, and a rewrite of the repo-local `absorb-upstream` skill.
...
- Run everything from the repo root `~/Oblivion/athaniel-brain` with `.venv/bin/python`. `rebrand_tree.py` and `parity_report.py` use ambient-cwd git (no `-C`), so process cwd must be the repo — existing constraint, keep it.
...
**Files:**
- Create: `athaniel_cli/absorb/divergence.py`
- Test: `tests/athaniel_cli/test_absorb_divergence.py`
...
Create `tests/athaniel_cli/test_absorb_divergence.py`:
...
GOOD = {
    "gateway/platforms/whatsapp_common.py": 'DEFAULT_REPLY_PREFIX = "✶ *Athaniel Brain*"\n',
    "gateway/overlay/brain_settings.py": "# overlay\n",
    "gateway/platforms/api_server.py": "def _athaniel_version():\n    return 'dev'\n",
    "agent/prompt_builder.py": "# created by Embrey The Creator / The Voice\n",
    "athaniel_cli/default_soul.py": "# created by Embrey The Creator / The Voice\n",
}
...
Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_divergence.py -q`
...
Each entry pins one deliberate Athaniel divergence from upstream Hermes so a
...
INVARIANTS: list[tuple[str, str, str | None]] = [
    ...
    ("gateway/platforms/api_server.py", "contains", "_athaniel_version"),
    ("agent/prompt_builder.py", "contains", "Embrey The Creator / The Voice"),
    ("athaniel_cli/default_soul.py", "contains", "Embrey The Creator / The Voice"),
]
...
Run: `.venv/bin/python -c "from athaniel_cli.absorb import divergence; print(divergence.check('.', 'HEAD'))"`
...
```bash
git add athaniel_cli/absorb/divergence.py tests/athaniel_cli/test_absorb_divergence.py
...
**Files:**
- Modify: `athaniel_cli/absorb/parity_report.py` (function `report`, lines 56–73; function `main`, lines 76–102)
- Modify: `athaniel_cli/absorb/driver.py:94` (the `parity_report.report(...)` call in `absorb()`)
- Test: `tests/athaniel_cli/test_absorb_parity.py` (extend)
...
Append to `tests/athaniel_cli/test_absorb_parity.py`:
...
Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_parity.py -q`
Expected: 2 new tests FAIL with `KeyError: 'divergence_violations'`

- [ ] **Step 3: Implement**

In `athaniel_cli/absorb/parity_report.py`, add the import at the top (after `import sys`):
...
def report(merged: str, theirs: str, head: str, repo: str = ".") -> dict:
    """Classify the merged tree; structured result for `athaniel absorb`.
...
In `athaniel_cli/absorb/driver.py:94`, pass the repo through:
...
Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_parity.py tests/athaniel_cli/test_absorb_driver.py -q`
...
```bash
git add athaniel_cli/absorb/parity_report.py athaniel_cli/absorb/driver.py tests/athaniel_cli/test_absorb_parity.py
...
**Files:**
- Modify: `athaniel_cli/absorb/driver.py` (imports; `absorb()` lines 66–98; `abort()` lines 114–116; new helpers after `_git`)
- Test: `tests/athaniel_cli/test_absorb_driver.py` (extend)
...
Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_driver.py -q`
Expected: new tests FAIL (`AttributeError: module ... has no attribute 'state'`, etc.)

- [ ] **Step 3: Implement**

In `athaniel_cli/absorb/driver.py`:
...
```python
    ...
    if drift:
        raise AbsorbRefused(
            ...
            "athaniel_cli/absorb/divergence.py first:\n" + "\n".join(drift))
```
...
**Files:**
- Create: `athaniel_cli/absorb/verify.py`
- Test: `tests/athaniel_cli/test_absorb_verify.py`
...
Create `tests/athaniel_cli/test_absorb_verify.py`:

```python
import subprocess
from athaniel_cli.absorb import verify
...
def test_green_tree_passes(tmp_path):
    repo = _mkrepo(tmp_path, {
        "mod.py": "X = 1\n",
        "tests/athaniel_cli/test_absorb_driver.py": "def test_ok():\n    assert True\n",
    })
...
def test_failing_targeted_test_fails(tmp_path):
    ...
    merged = _commit_tree(
        repo, {"tests/athaniel_cli/test_absorb_driver.py":
               "def test_bad():\n    assert False\n"})
    res = verify.run(repo, merged)
...
Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_verify.py -q`
...
```python
"""Post-merge verification battery for `athaniel absorb`.
...
TARGETED_TESTS = [
    "tests/athaniel_cli/test_absorb_driver.py",
    "tests/athaniel_cli/test_absorb_detect.py",
    "tests/athaniel_cli/test_absorb_parity.py",
    "tests/athaniel_cli/test_absorb_divergence.py",
    "tests/athaniel_cli/test_absorb_verify.py",
    "tests/athaniel_cli/test_banner.py",
    "tests/athaniel_cli/test_build_info.py",
    "tests/athaniel_cli/test_config.py",
...
```bash
git add athaniel_cli/absorb/verify.py tests/athaniel_cli/test_absorb_verify.py
...
**Files:**
- Modify: `athaniel_cli/absorb/driver.py` (import; end of `absorb()`; new functions after `absorb()`)
- Test: `tests/athaniel_cli/test_absorb_driver.py` (extend)
...
def materialize(repo: str) -> str:
    ...
    st = state(repo)
    if not st:
        raise AbsorbRefused("no absorb in flight — run `athaniel absorb <tag>` first")
...
**Files:**
- Modify: `athaniel_cli/absorb/driver.py` (replace `commit()`, lines now after `verify_current()`; add `_bookkeep_tree()` and `_changelog_insert()` helpers; add `import datetime`, `import os`, `import tempfile` to the module imports)
- Test: `tests/athaniel_cli/test_absorb_driver.py` (extend)
...
In `athaniel_cli/absorb/driver.py`, add three imports to the existing block (`re` and `subprocess` are already there — do not duplicate):
...
def commit(repo: str, tag: str | None = None, skip_verify: bool = False) -> str:
    ...
    if head != st["ours_head"]:
        raise AbsorbRefused("HEAD moved since the absorb was prepared — "
                            "re-run `athaniel absorb` or --abort first")
    merged = st["merged"]
    ...
    if not skip_verify and not st["verify_ok"]:
        raise AbsorbRefused("verify battery not green — fix and re-run "
                            "`athaniel absorb --verify`, or pass --skip-verify (human call)")
...
Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_driver.py tests/athaniel_cli/test_absorb_detect.py tests/athaniel_cli/test_absorb_parity.py tests/athaniel_cli/test_absorb_divergence.py tests/athaniel_cli/test_absorb_verify.py -q`
Expected: all pass
...
**Files:**
- Modify: `athaniel_cli/main.py` — `cmd_absorb` (line ~11569) and the absorb parser block (lines ~11834–11852)
- Test: `tests/athaniel_cli/test_absorb_driver.py` (extend the CLI test; fix the existing namespace)
...
In `tests/athaniel_cli/test_absorb_driver.py`, first UPDATE the existing `test_cli_absorb_gate_runs` namespace to carry the new attributes (it will otherwise crash with `AttributeError` once `cmd_absorb` reads them):
...
In `athaniel_cli/main.py`, parser block (after the `--abort` line ~11851), add:
...
Run: `.venv/bin/python -m athaniel_cli.main absorb --help` — expect the four new flags listed.
Run: `.venv/bin/python -m athaniel_cli.main absorb --status` — expect `✓ no absorb in flight`.

- [ ] **Step 6: Commit**

```bash
git add athaniel_cli/main.py tests/athaniel_cli/test_absorb_driver.py
...
5. If parity shows conflicts: `athaniel absorb --continue` materializes the merge on
...
In the CLI section (around line 312 where `athaniel absorb v2026.7.0` appears), extend the flow to:

```sh
athaniel absorb --status                # resume state, if any
athaniel absorb v2026.7.0               # branch + parity + AUTO verify battery
athaniel absorb --continue              # materialize conflicts into the working tree
# ...resolve conflict files in place...
athaniel absorb --verify                # snapshot + re-run parity/divergence/battery
athaniel absorb --commit                # human step: guards + bookkeeping + finalize
athaniel absorb --abort                 # rollback at any point
```

Immediately after that flow block, add these two paragraphs verbatim:

```markdown
**Divergence manifest.** `athaniel_cli/absorb/divergence.py` pins our genuine
...
**Verify battery.** `athaniel_cli/absorb/verify.py` materializes the merged tree
...
```markdown
Conflicts are resolved in place: `athaniel absorb --continue` materializes the
merge into the working tree, and `athaniel absorb --verify` snapshots your
resolutions and re-runs parity + the verify battery. Bookkeeping is automatic:
`athaniel absorb --commit` bumps the minor version in `pyproject.toml`, rewrites
this table, and prepends the `CHANGELOG.md` entry — no hand edits.
```

- [ ] **Step 4: Full absorb + neighbors test run**

Run: `.venv/bin/python -m pytest tests/athaniel_cli/test_absorb_driver.py tests/athaniel_cli/test_absorb_detect.py tests/athaniel_cli/test_absorb_parity.py tests/athaniel_cli/test_absorb_divergence.py tests/athaniel_cli/test_absorb_verify.py tests/athaniel_cli/test_banner.py tests/athaniel_cli/test_build_info.py -q`
Expected: all pass

- [ ] **Step 5: Real-repo smoke**

Run: `.venv/bin/python -m athaniel_cli.main absorb --gate`
Expected: `✓ gate passed (0 stray tokens)`

Run: `.venv/bin/python -c "from athaniel_cli.absorb import divergence; v = divergence.check('.', 'HEAD'); print(v or 'divergence manifest intact')"`
