...
has to be thrown away or summarized. Athaniel has always done this in one batch:
...
Athaniel folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Athaniel's normal session flush is
...
With `micro_compact` unset or `false` Athaniel behaves exactly as it always has:
...
Defaults to `$ATHANIEL_HOME/logs/agent.log`. It reports passes, outcome mix, net
