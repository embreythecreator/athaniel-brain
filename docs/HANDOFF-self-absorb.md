...
**Repo:** `~/Oblivion/athaniel-brain` · **Branch:** `absorb/v2026.6.19`
...
2. **Built the "self-absorb" feature** (`athaniel absorb` command + detection + an agentic skill) so
   Athaniel can do future absorbs itself. All 6 plan tasks landed as commits and are verified
...
c1c9efbbd  feat(absorb): athaniel absorb subcommand + dispatch          ← self-absorb task 3
f2dc03898  feat(absorb): python driver with guardrails + fidelity gate   ← self-absorb task 2
dded3bff0  refactor(absorb): package harness into athaniel_cli/absorb   ← self-absorb task 1
bb32a7bf8  docs: session handoff for absorb + self-absorb feature
cbdb60744  docs(plan): Athaniel self-absorb implementation plan
6c8d461e9  docs(spec): Athaniel self-absorb design (athaniel absorb + skill)
...
To get back here in a fresh session: `cd ~/Oblivion/athaniel-brain && git checkout absorb/v2026.6.19`.
...
- `rename_map.py` — the transform **T** (the "rebrand touch core"). Two token families + carve-outs:
  - **HERMES family:** `Hermes Agent`→`Athaniel Brain`, `hermes_agent`→`athaniel_brain`,
    `hermes-agent`→`athaniel-brain`, `Hermes`→`Athaniel`, `hermes`→`athaniel` (paths + content).
  - **NOUS family:** `Nous Research`→`Athaniel`, `NousResearch`→`embreythecreator`.
    **Never** touches the `Nous Portal` inference *provider* / `nous_account` / `NOUS_*`
    (real external service), nor the domain `nousresearch.com` — that is where the real
    Nous services live (OAuth, inference, Tool Gateway suffix), so rebranding it aims live
    third-party endpoints at a domain we own but don't serve. It *was* rebranded until
    2026-07-31; `_DOMAIN_SENTINEL` in `swap_text()` now protects it.
  - **ATTRIBUTION:** `created by Athaniel`/`created by Hermes` → `created by Embrey The Creator`.
...
| `import athaniel_cli.main` | OK |
| `./athaniel --version` | `Athaniel Brain v0.17.0 (2026.6.19)` — versioned-banner feature intact |
| `./athaniel --help` | new `whatsapp-cloud` subcommand present |
...
| Full core bulk run (`athaniel_cli`+`agent`+`run_agent`+`cli`+`acp_adapter`+`athaniel_state`+`cron`) | 14,532 passed / **337 failed** |
...
- I ran 14k tests in **one in-process pytest** (`-o addopts=""`), disabling the project's per-file
  ...
  in isolation**.
- This **machine/session** has a real Anthropic OAuth token in the env + real `~/.athaniel` config
...
The user wanted Athaniel able to update its own core base from upstream. Brainstormed → spec → plan
→ **built**. All 6 plan tasks landed (commits `dded3bff0`..`791fcbe3c`) and are verified passing.

- **Spec:** `docs/specs/2026-06-29-athaniel-self-absorb-design.md`
- **Plan:** `docs/plans/2026-06-29-athaniel-self-absorb.md` (6 TDD tasks — all complete)

**What shipped + where it lives now:**
- Harness moved `scripts/absorb/*.sh` → importable **`athaniel_cli/absorb/`** (`rename_map.py`,
  `rebrand_tree.py`, `parity_report.py`, `driver.py`, `detect.py`; 715 lines). `scripts/absorb/README.md`
  is now just a pointer.
- **`athaniel absorb`** subcommand with `--check` / `--gate` / `--commit` / `--abort` (+ `[tag]`,
  `--base`). Run the gate via `athaniel absorb --gate` (NOT the old `scripts/absorb/absorb.sh --gate`).
...
- Tests: `tests/athaniel_cli/test_absorb_{driver,detect,parity}.py` — **12 passing** (clean HOME).

**Verified 2026-06-30:** gate → 0 stray tokens · 12 absorb tests pass · `athaniel --version` →
`v0.17.0 (2026.6.19) … +12 carried commits` · `athaniel absorb --check` → up to date with upstream.

**Locked decisions:**
- **Layered**: a deterministic `athaniel absorb` command does the mechanics + safety rails; a skill
  wraps it so Athaniel drives it agentically, **pausing for human approval before committing**.
...
do it; they get core updates via published releases (`athaniel update`, which already exists and is
...
**Plan shape (6 tasks):**
1. Repackage harness `scripts/absorb/` → importable `athaniel_cli/absorb/`.
...
3. Wire `athaniel absorb` subcommand (`_parser.py:build_top_level_parser`, `main.py` dispatch).
...
- Subcommand registration: `athaniel_cli/_parser.py` → `build_top_level_parser()` (subparsers added
  ~line 246+).
- Dispatch: `athaniel_cli/main.py` (mirror the `elif action == "update":` site ~line 10621);
  detection pattern in `_cmd_update_check` (~line 7881).
- Banner offer surface: `athaniel_cli/banner.py` → `check_for_updates()` (~line 265), caches in
  `~/.athaniel/.update_check`.
...
The next upstream absorb is now one command: **`athaniel absorb v2026.7.0`** (add `--base v2026.6.19`
to override the merge base). It produces an `absorb/<tag>` branch + parity report and stops;
`athaniel absorb --commit` finalizes once parity is READY, `--abort` tears the branch down.
...
1. ~~**Merge `absorb/v2026.6.19` → `main`?** And push?~~ — **DONE.** Fast-forwarded `main`
   (`683ba08e3 → 7e4ad83fa`) and pushed to `origin` (embreythecreator/athaniel-brain). The whole
...
Nothing left open. Next upstream bump is one command: `athaniel absorb v2026.7.0`.
