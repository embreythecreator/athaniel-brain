...
`athaniel doctor` reports the explicit repair command.
...
ATHANIEL_HOME="$HOME/.athaniel" athaniel sessions repair --check-only
ATHANIEL_HOME="$HOME/.athaniel" athaniel sessions repair
```
...
```bash
ATHANIEL_HOME="$HOME/.athaniel" athaniel sessions repair --check-only
sqlite3 "$HOME/.athaniel/state.db" \
