...
Athaniel middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Athaniel'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Athaniel calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Athaniel retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Athaniel guardrails,
...
- `telemetry_schema_version`: currently `athaniel.observer.v1`
- `middleware_schema_version`: currently `athaniel.middleware.v1`
...
Athaniel stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Athaniel runs
them as a nested chain in registration order. Middleware failures are fail-open:
Athaniel logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Athaniel applies middleware in this order:
...
For each tool call, Athaniel applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Athaniel pre-execution path against the effective arguments:
...
For isolated local testing, use one `ATHANIEL_HOME` for plugin enablement and the
agent run:

```bash
export ATHANIEL_HOME=/tmp/athaniel-middleware-test
mkdir -p "$ATHANIEL_HOME"
athaniel plugins enable <plugin-name>
athaniel chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run athaniel plugins enable <plugin-name>
uv run athaniel chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["athaniel_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/athaniel-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Athaniel expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Athaniel treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Athaniel preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Athaniel does not convert downstream
