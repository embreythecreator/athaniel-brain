...
How Athaniel folds a new upstream **Hermes Agent** release into its own rebranded
core, reproducibly, without hand-porting a 1,000+-file diff. This is the reference
for the `athaniel_cli/absorb/` package and the `athaniel absorb` command.

> **Audience:** maintainers of `embreythecreator/athaniel-brain` working on a
> git/source checkout. This capability does **not** exist for pip/docker installs —
> they get core updates via `athaniel update` (latest *published* release), which is
...
`athaniel-brain` is a **white fork** of `NousResearch/hermes-agent` imported as a
...
renamed to `Athaniel`/`embreythecreator`, **including file and directory names**
(`hermes_cli/` → `athaniel_cli/`, `hermes_state.py` → `athaniel_state.py`, …). A raw
...
Because BASE, THEIRS and OURS now all speak Athaniel, the merge's conflicts
...
The operator attribution fix (`created by Hermes/Athaniel` →
...
|---|---|---|
| BASE (`attribution=False`) | `…by Athaniel` | rename only, no attribution rule |
...
athaniel_cli/main.py        # cmd_absorb() + the `absorb` subparser (the CLI surface)
athaniel_cli/banner.py      # absorb_offer_line() — proactive launch-time offer
skills/software-development/absorb-upstream/SKILL.md   # the agentic loop
scripts/absorb/README.md     # pointer only ("moved to athaniel_cli/absorb/")
tests/athaniel_cli/test_absorb_{driver,detect,parity}.py
...
HERMES_RULES        # product rename, applied to content + (a subset to) paths
  "Hermes Agent" → "Athaniel Brain"     # display wordmark — must precede bare token
...
NOUS_RULES          # vendor/brand/org rename (most-specific first)
  ...
  # NOT rebranded, and actively protected by _DOMAIN_SENTINEL in swap_text():
  "nousresearch.com" → "nousresearch.com"   # real third-party service domain
...
PATH_RULES          # case-aware path tokens (total swap; 643/643 hermes paths renamed, 0 kept)
  hermes_agent→athaniel_brain, hermes-agent→athaniel-brain,
  HERMES→ATHANIEL, Hermes→Athaniel, hermes→athaniel
```
...
| `swap_text(text, athaniel_path, attribution=True) -> str` | apply the families + (optional) attribution to a blob body; carve-outs keyed on the **already-path-swapped** path |
...
- the boot banner / figlet **ATHANIEL** wordmark + Seraphim sigil
...
CLI form (also usable standalone): `python -m athaniel_cli.absorb.rebrand_tree <ref>
...
`athaniel_cli/absorb/` (the harness names the tokens it rewrites, so it self-excludes).
...
| `gate(repo, base_ref) -> (passed, detail)` | the **fidelity gate**: `T(base, attribution=False)` then `git grep` for stray `hermes`/`nousresearch` outside `ALLOWED_STRAY`; passes iff zero. Token-level, domain-aware: `nousresearch.com` occurrences don't count (protected carve-out, see `_DOMAIN_SENTINEL`) — only `hermes` or non-domain `nousresearch` are stray. Same rule in `parity_report.grep_stray` |
...
launch: `✶ upstream Hermes <tag> available to absorb · run \`athaniel absorb <tag>\``
(wrapped so it can never break the banner).

---

## 9. The CLI — `athaniel absorb`

```
athaniel absorb [tag] [--base BASE] [--check] [--gate] [--commit] [--abort]
```
...
`skills/software-development/absorb-upstream/SKILL.md` lets Athaniel drive an absorb
...
athaniel absorb --continue              # materialize conflicts into the sidecar worktree
...
athaniel absorb --commit                # human step: guards + bookkeeping + finalize + ff-install to main
...
> restart the gateway** (`launchctl kickstart -k gui/$(id -u)/ai.athaniel.gateway` on
...
| `branch absorb/<tag> already exists` | a prior run left it — `athaniel absorb <tag> --abort` first. |
...
| `/v1/models` advertises an old version after a bump | `_athaniel_version()` reads `importlib.metadata` first; the editable-install dist-info is stale. `VIRTUAL_ENV=$PWD/venv uv pip install -e . --no-deps` then restart (the `venv` has no `pip`; use `uv`). |
...
```
Transform T            athaniel_cli/absorb/rename_map.py      swap_path / swap_text
Apply T → tree         athaniel_cli/absorb/rebrand_tree.py    build_rebranded_tree(ref, attribution=)
Classify merge         athaniel_cli/absorb/parity_report.py   report(merged, theirs, head)
Orchestrate + guard    athaniel_cli/absorb/driver.py          gate / absorb / commit / abort
Detect new tags        athaniel_cli/absorb/detect.py          latest_absorbable(repo)
CLI                    athaniel_cli/main.py:cmd_absorb         athaniel absorb …
Launch offer           athaniel_cli/banner.py                  absorb_offer_line(repo)
Agentic loop           skills/software-development/absorb-upstream/SKILL.md
Provenance / base tag  UPSTREAM_BASE.md
Tests                  tests/athaniel_cli/test_absorb_{driver,detect,parity}.py
```
