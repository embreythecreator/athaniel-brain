# Athaniel Observer Hooks

Athaniel observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Athaniel' planner, model providers, memory, tool registry,
...
Athaniel also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "athaniel.observer.v1"
```

Hook callbacks are fail-open. Athaniel catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Athaniel only builds
...
The native NeMo Relay SDK integration maps Athaniel session, turn, LLM, and tool
