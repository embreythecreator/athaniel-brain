...
Athaniel includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Athaniel and does not require a Athaniel observability plugin. Athaniel remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Athaniel execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `athaniel-brain[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Athaniel `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ATHANIEL_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ATHANIEL_NEMO_RELAY_ATOF_*` and `ATHANIEL_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Athaniel does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Athaniel requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Athaniel installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Athaniel process so configured interceptors can
...
Collection remains off unless Athaniel policy enables it:
...
opt-in. Set `ATHANIEL_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Athaniel does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Athaniel
...
retain the same `session_id` and add `athaniel.session.segment` plus
`athaniel.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Athaniel
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Athaniel process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Athaniel logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Athaniel process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Athaniel
process and share the same policy boundary.

Run profiles in separate Athaniel processes when they require different trust
...
Athaniel core owns one Relay host and one isolated Relay session scope per Athaniel
session. Core lifecycle producers use
`athaniel_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Athaniel plugin registration. Shared-metrics marks must
...
```text
Athaniel turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Athaniel shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Athaniel sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Athaniel used for the logical call, such as
...
New calls use `athaniel.model_route.count`. The previous
`athaniel.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `athaniel.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Athaniel
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Athaniel processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `athaniel.task_run`, parented to
the owning Athaniel session. The start counter contains only bounded execution
...
provider attempts for the same Athaniel API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Athaniel tool-call
...
cancellations. Active task ownership follows the task ID if Athaniel rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`athaniel.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Athaniel
...
tool or plugin names. Athaniel does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `athaniel.tool_approval` marks and
...
Successful skill mutations emit `athaniel.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `athaniel.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Athaniel derives reuse and patch-generation
...
```text
$ATHANIEL_HOME/telemetry/shared_metrics/metrics.sqlite3
$ATHANIEL_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Athaniel version,
...
schema field name, its current scope is one `ATHANIEL_HOME`, so it is more
...
Deleting `$ATHANIEL_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ATHANIEL_HOME`. To reset it, stop Athaniel
processes and remove `$ATHANIEL_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Athaniel CLI turn against the deterministic local model server:
