...
Athaniel gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Athaniel's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `athaniel.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `athaniel.platform.up/degraded` with bounded `error_code` attributes |
...
`athaniel.gateway.active_agents`, `athaniel.gateway.background_work`, and
`athaniel.gateway.background_delegations` are complementary. `active_agents`
...
```bash
athaniel monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'athaniel-brain[otlp]'`),
...
a plain `athaniel gateway run`: the exporter lives in the gateway process, so
...
`athaniel.gateway.up`, `athaniel.platform.up`, and `athaniel.platform.degraded`.
...
absent_over_time(athaniel_gateway_up[5m])

# Locally owned bridge is explicitly down.
athaniel_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
athaniel_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
athaniel_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
athaniel_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(athaniel_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `athaniel.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
athaniel.status = failed|unknown
athaniel.delivery_outcome = failed|not_configured
athaniel.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `athaniel.job_key`;
...
Athaniel Brain-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/athaniel_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/athaniel_otel_capture.jsonl --wait 8
...
`athaniel.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
athaniel monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `athaniel monitoring` CLI intentionally exposes `status` only. This first
release covers only Athaniel Brain-owned service-health and operational-diagnostic
signals, including Athaniel Brain-owned Relay transport health. Team Gateway's
