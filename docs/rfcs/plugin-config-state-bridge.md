...
or change arbitrary Athaniel configuration.
...
small host fact requested by the RFC. Settings writes use Athaniel'
...
`ATHANIEL_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<ATHANIEL_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Athaniel home on every operation.
...
The implementation is covered with real temporary-Athaniel-home tests for:
...
- [Issue #64227](https://github.com/embreythecreator/athaniel-brain/issues/64227)
- [RFC PR #58542](https://github.com/embreythecreator/athaniel-brain/pull/58542) by Topher Ross
