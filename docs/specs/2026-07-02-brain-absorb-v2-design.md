...
**Date:** 2026-07-02 · **Status:** approved · **Repo:** `~/Oblivion/athaniel-brain`
**Predecessor:** `docs/HANDOFF-self-absorb.md`, `docs/absorb-harness.md` (v1, shipped 2026-06-30)

## Context

`athaniel absorb` v1 (`athaniel_cli/absorb/`, ~620 lines, stdlib + git plumbing) does the
...
- **Autonomy boundary (locked):** Athaniel auto-verifies and auto-resolves only mechanical
  steps; **a human always runs `--commit`**. `main` is never touched; nothing pushes.
- **Non-goal:** Face absorb (`~/Oblivion/athaniel-face`, upstream `agent0ai/space-agent`)
  — deliberately split into a follow-up spec that reuses this hardened pattern.

## 1. Divergence manifest — new `athaniel_cli/absorb/divergence.py`
...
| `gateway/platforms/api_server.py` | contains | `_athaniel_version` (versioned model name) |
...
## 2. Auto-verify battery — new `athaniel_cli/absorb/verify.py`
...
3. The **targeted hermetic test set** (the set that ran 295-green during the v2026.6.19
   absorb): `tests/athaniel_cli/test_absorb_*.py`, banner, build_info, brain_settings,
...
- `athaniel absorb --continue` — switch to `absorb/<tag>` and materialize the merged
...
- `athaniel absorb --verify` — if the working tree is a materialized absorb, snapshot it
...
- `athaniel absorb --status` — print the in-flight absorb (tag, branch, parity summary,
...
Extend the hermetic suite (`tests/athaniel_cli/test_absorb_{driver,detect,parity}.py`,
