...
athaniel-face `0eef06a` (space_action.js, both agent surfaces, brain_chat
header forwarding, operator contract v2).
Work order: `sys_brain_hermes_spine_seating` — seat the Brain as sole agent loop.

## Problem

Face (athaniel-face) today runs a full in-app agent loop: it assembles prompts,
...
3. Face validates, (if required) approval-gates, executes, and sends the
   **result envelope** back as the next turn message with
   `x-athaniel-turn-type: action-result` — it is *input to the Brain's loop*,
...
```json
{
  ...
  "error": null,
  "refusal_reason": null,
  "face": { "app": "athaniel-void", "version": "…", "session": "<ward-session>" }
}
```
...
| A1.3 ✅ | Prompt ownership: Face stops injecting system prompts / skills / promptinclude / personality / memory files; operator contract + protocol + space.* brief now Brain-side platform policy (`gateway/platforms/face_policy.py`, keyed on `vessel:athaniel:face:*` / `athaniel-face-*`; idempotent for old clients). Face still forwards the user's own custom instructions. | Brain owns context |
| A1.4 ✅ | State ownership: no local history replay — brain-only requests carry only the current turn; continuity via `X-Athaniel-Session-Id`; Face history is UI-render-only; memory includes no longer reach requests | Kill condition met: no long-term model-facing state or independent reasoning loop in Face (face `3f356a4` + brain `4a77a1e9f`, 2026-07-05) |
...
3. Result truncation budget: **32768 chars** (`RESULT_TRUNCATION_LIMIT` in
   ...
   only, never full contents (`ATHANIEL_HOME/logs/limb_rail.jsonl`).
