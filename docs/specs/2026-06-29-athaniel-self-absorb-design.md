# Design: Athaniel self-absorb — `athaniel absorb` + skill
...
The goal of this work: make absorbing upstream a **first-class capability of Athaniel
itself** — a deterministic `athaniel absorb` subcommand, proactive detection of new
upstream releases, and a skill so Athaniel can *drive* the absorb agentically (resolve
conflicts, run tests) while pausing for human approval before committing.

### The two distinct "updates" (load-bearing distinction)

- **`athaniel update`** (exists) — exits + relaunches to pull the latest *published*
  release. The *install* side: "get the newest Athaniel Brain Embrey shipped."
...
`athaniel update` to receive it.* Self-absorb is therefore a **maintainer capability that
...
- **Autonomy:** layered — a deterministic command does the mechanics + safety rails; a
  skill wraps it so Athaniel can drive it, pausing for approval before committing.
...
- **`detect.py`** — `git ls-remote upstream --tags`, find latest tag newer than the
  recorded base, cache the result with a TTL (in `.git/` or `~/.athaniel/`) so launch
  latency is unaffected.

## The `athaniel absorb` command

```
athaniel absorb --check          # detection only: newer upstream tag? (cheap, cached)
athaniel absorb --gate           # fidelity gate: does T still reproduce HEAD?
athaniel absorb <tag>            # gate → rebrand → 3-way merge onto absorb/<tag>
                                  #   → parity report. STOPS before committing.
athaniel absorb <tag> --commit   # finalize once clean + parity READY (after review)
athaniel absorb --abort          # delete the absorb branch, restore — one-step rollback
```

`athaniel absorb <tag>` is **dry-by-default**: it produces the merged branch + parity
...
```
✶ upstream Hermes v2026.7.0 available to absorb · run `athaniel absorb v2026.7.0`
```
...
It teaches Athaniel the loop:

1. `athaniel absorb --check`; if a newer tag exists, offer it.
2. `athaniel absorb <tag>`; read the parity report.
...
versioned model name, the "Embrey The Creator" attribution) so Athaniel knows what to
...
- A dry-run integration test of `athaniel absorb --check` and `--gate`.
- Follows existing patterns: `tests/athaniel_cli/test_absorb_*.py`.

## Install-type behavior matrix

| Install type | detection | `athaniel absorb` | how they get core updates |
|---|---|---|---|
| git/source + `upstream` remote (maintainer) | shows offer | full capability | self-absorb, then publish |
| git/source, no `upstream` remote | silent | refuses, explains how to add the remote | `athaniel update` |
| pip / docker / managed | silent | refuses with guidance | `athaniel update` (published release) |

## Phasing

1. **Repackage + command** — move harness into `athaniel_cli/absorb/`, port `driver.py`,
   wire `athaniel absorb` (`--check`/`--gate`/`<tag>`/`--commit`/`--abort`) with all
   guardrails. Tests.
...
- Changes to the existing `athaniel update` install-update behavior.
