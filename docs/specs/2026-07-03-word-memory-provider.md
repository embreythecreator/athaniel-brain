...
procrastinate jobs, Ward-authenticated REST on :5055) in as a Athaniel Brain
...
- **Config** (`get_config_schema` / `save_config`, stored under `memory.word.*`
  ...
  `Athaniel Memory`), `search_type` (`text` default | `vector` — vector
...
- New files only under `plugins/memory/word/`, `tests/plugins/`, plus the
  static provider list in the `athaniel memory` CLI help text and README
  touch-ups. Do not modify divergence-manifest files
  (`athaniel_cli/absorb/divergence.py` invariants) or absorb harness code.
...
1. `.venv/bin/python -m pytest tests/plugins/test_word_plugin.py
   ...
   `tests/plugins/` + `tests/athaniel_cli/` still green.
2. `discover_memory_providers()` lists `word`; `athaniel memory status`/help
   text includes it.
