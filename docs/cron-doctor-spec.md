...
a provider run fails, or delivery starts failing. `athaniel cron list` shows some of
...
Add `athaniel cron doctor` as a read-only diagnostic command that summarizes cron
...
- `athaniel cron doctor` returns `0` and prints a healthy message when active jobs
...
- It returns `1` and prints grouped job-level issues when any active job has:
  ...
  - script path missing/outside `ATHANIEL_HOME/scripts`, or
