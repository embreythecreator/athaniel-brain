...
Athaniel Brain 使用 SQLite 数据库（`~/.athaniel/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。

源文件：`athaniel_state.py`
...
多个 athaniel 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.athaniel/state.db
...
默认路径：`~/.athaniel/state.db`

该路径由 `athaniel_constants.get_athaniel_home()` 推导，默认解析为 `~/.athaniel/`，或 `ATHANIEL_HOME` 环境变量的值。
