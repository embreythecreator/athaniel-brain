...
ACP 适配器将 Athaniel 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
`acp_adapter/tools.py` 将 Athaniel 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 Athaniel 的运行时解析器：

- `acp_adapter/auth.py`
- `athaniel_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 Athaniel provider/凭据。它还始终通告一个终端 setup 认证方法（`athaniel-setup`，参数 `--setup`），以便首次运行的 ACP 客户端在启动正常 ACP 会话前可以打开 Athaniel 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.athaniel/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `athaniel-acp` toolset 定义
- `athaniel_cli/main.py` — `athaniel acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `athaniel-acp` 脚本