---
title: "Bot Mode"
description: "Turn your Athaniel profiles into a roster of named Bots — each with its own chat, role, model, memory, skills, and avatar. Bots run routines, share group chats, and message each other."
---

# Bot Mode

**Bot Mode** turns your [Athaniel profiles](./profiles.md) into a roster of named **Bots**. Each Bot has its own role, model, memory, skills, and avatar; Bots run recurring routines, deliberate together in group chats, and message each other directly. Build a specialist Bot once and it is there forever, one click away.
...
There is no new primitive to learn: a Bot **is** a Athaniel profile — isolated config, memory, skills, credentials, and chat history under `~/.athaniel/profiles/<name>/`. Bot Mode is a UI over that primitive, so everything you do in it is visible from the CLI too: `athaniel -p <bot> chat` opens the same agent, and Bot routines appear in `athaniel cron list`. No core patches, no background daemons, no extra storage.
:::
...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair Athaniel knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile.
...
- **A pixel pet** — a companion from the [petdex gallery](./features/pets.md) that bounces beside the avatar while the Bot is busy. Run `athaniel pets` in a terminal to explore the gallery.
...
Every gateway you register in **Settings → Connections** — local, remote URL, SSH, Athaniel Cloud, docker — is a persistent line the Desktop holds open, and Bot Mode uses those lines for messaging automatically. No extra setup:
...
```bash
athaniel peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
athaniel peer list
athaniel peer dm spark < /tmp/dm.txt        # message body from a file (nothing shell-interpreted)
athaniel peer dm spark/researcher < /tmp/dm.txt   # named profile on a multiplexed peer
athaniel peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
athaniel peer status spark run_abc123
athaniel peer stop spark run_abc123
```

`athaniel peer dm` delivers into the remote agent's canonical Bot Chat over the peer's existing API server, runs one agent turn there, and prints the reply on stdout — the exact cross-machine twin of the local `athaniel -p <bot> chat` command.
...
Requirements: the peer machine runs the `api_server` gateway platform with a strong `API_SERVER_KEY`; reachability is your network's business (LAN, Tailscale, VPN). The key is a credential and lives in `~/.athaniel/.env` as `ATHANIEL_PEER_<NAME>_KEY`; peer names/URLs live in `config.yaml` under `bot_peers`.
...
| --- | --- |
| Chat with a Bot | `athaniel -p <bot> chat` |
| A Bot's files, skills, memory | `~/.athaniel/profiles/<bot>/` |
| Routines | `athaniel cron list` (jobs named `[bot:<name>] …`) |
| Create / inspect profiles | `athaniel profile create`, `athaniel profile list` |
