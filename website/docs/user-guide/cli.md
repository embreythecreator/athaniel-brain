...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Athaniel Brain terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Athaniel Brain's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `athaniel setup --portal` — and you're ready to `athaniel chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Athaniel also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `athaniel --tui` — see the [TUI](tui.md) guide.
:::
...
athaniel chat --query-file prompt.txt
athaniel chat --query-file - < prompt.txt

# With a specific model
athaniel chat --model "anthropic/claude-sonnet-4"

# With a specific provider
athaniel chat --provider nous        # Use Nous Portal
athaniel chat --provider openrouter  # Force OpenRouter
...
athaniel chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
athaniel --continue             # Resume the most recent CLI session (-c)
athaniel --resume <session_id>  # Resume a specific session by ID (-r)
athaniel --resume latest        # Resume the most recent session (same as -c)
athaniel --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
athaniel chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
athaniel -w                         # Interactive mode in worktree
athaniel -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`athaniel -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
athaniel worktree list              # audit: age, size, verdict, reason per tree
athaniel worktree prune             # remove safe trees + delete merged branches
athaniel worktree prune --dry-run   # show the plan without changing anything
athaniel worktree prune --trees-only     # leave local branches alone
athaniel worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running athaniel session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.athaniel/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `athaniel -w` for days — no longer accumulate merged scratch trees
...
The `athaniel plugins` commands manage native Athaniel plugins and portable Agent
...
athaniel plugins disable <plugin-name>
athaniel plugins update <plugin-name>
athaniel plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Athaniel currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Athaniel CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Athaniel CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ATHANIEL_YOLO_MODE` is on (either `athaniel --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Athaniel to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.athaniel/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Athaniel supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Athaniel replies |
...
```bash
athaniel -s athaniel-brain-dev,github-auth
athaniel chat -s github-pr-workflow -s github-auth
```

Athaniel loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.athaniel/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.athaniel/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Athaniel..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Athaniel also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Athaniel recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Athaniel — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Athaniel is working, Athaniel prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Athaniel to the background — just like any terminal process. The shell prints a confirmation:

```
Athaniel Brain has been suspended. Run `fg` to bring Athaniel Brain back.
```
...
Resume this session with:
  athaniel --resume 20260225_143052_a1b2c3
...
```bash
athaniel --continue                          # Resume the most recent CLI session
athaniel -c                                  # Short form
athaniel -c "my project"                     # Resume a named session (latest in lineage)
athaniel --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
athaniel --resume "refactoring auth"         # Resume by title
athaniel --resume latest                     # Resume the most recent session (same as -c)
athaniel --resume latest --in ./my-project   # Latest session for ./my-project's workspace
athaniel -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `athaniel sessions rename <id> <title>` from the command line. Use `athaniel sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Athaniel's SQLite state database under `~/.athaniel/state.db`. The database keeps:
...
Athaniel immediately confirms the task and gives you back the prompt:
