...
title: "Git Worktrees"
description: "Run multiple Athaniel agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Athaniel Brain is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Athaniel so each session has a clean, isolated working directory.

## Why Use Worktrees with Athaniel?

Athaniel treats the **current working directory** as the project root:

- CLI: the directory where you run `athaniel` or `athaniel chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.athaniel/config.yaml`
...
Athaniel creates `.worktrees/my-experiment/` inside the repo (branch
`athaniel/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `athaniel-<id>`
...
git worktree add ../repo-feature feature/athaniel-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/athaniel-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Athaniel there:

```bash
cd ../repo-feature

# Start Athaniel in the worktree
athaniel
```
...
git worktree add ../repo-experiment-a feature/athaniel-a
git worktree add ../repo-experiment-b feature/athaniel-b
```
...
Each Athaniel process:

- Works on its own branch (`feature/athaniel-a` vs `feature/athaniel-b`).
...
- Athaniel checkpoint data under `~/.athaniel/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Athaniel experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/athaniel-checkpoints-docs`, `feature/athaniel-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Athaniel from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `athaniel -w` (Automatic Worktree Mode)

Athaniel has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `athaniel/athaniel-<hash>`).
...
```bash
athaniel -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `athaniel -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Athaniel session its own clean checkout.
