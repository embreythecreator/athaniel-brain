...
description: "Chat with Athaniel from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Athaniel from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Athaniel](/guides/use-voice-mode-with-athaniel).
...
```mermaid
flowchart TB
    subgraph Gateway["Athaniel Gateway"]
...
For group chats, hooks, and automation flows, Athaniel supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Athaniel keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Athaniel does not hide failures just because the text resembles a silence token.
...
```bash
athaniel gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
athaniel gateway              # Run in foreground
athaniel gateway setup        # Configure messaging platforms interactively
athaniel gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo athaniel gateway install --system   # Linux only: install a boot-time system service
athaniel gateway start        # Start the default service
athaniel gateway stop         # Stop the default service
athaniel gateway status       # Check default service status
athaniel gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.athaniel/config.yaml"
gateway:
...
```bash
athaniel gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Athaniel sends heartbeats
...
| `/update` | Update Athaniel Brain to the latest version |
...
with the `session_reset` section in `~/.athaniel/config.yaml`:
...
Configure per-platform overrides in `~/.athaniel/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.athaniel/gateway-config.yaml`:
...
athaniel pairing approve telegram XKGH5N7P

# Other pairing commands:
athaniel pairing list          # View pending + approved users
athaniel pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Athaniel appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.athaniel/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.athaniel/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `ATHANIEL_HOME`:

- `~/.athaniel/status_phrases.yaml` or any `*.yaml` in `~/.athaniel/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to ATHANIEL_HOME
...
Off by default. When enabled, Athaniel prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.athaniel/config.yaml`:
...
```bash
ATHANIEL_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
athaniel gateway install               # Install as user service
athaniel gateway start                 # Start the service
athaniel gateway stop                  # Stop the service
athaniel gateway status                # Check status
journalctl --user -u athaniel-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo athaniel gateway install --system
sudo athaniel gateway start --system
sudo athaniel gateway status --system
journalctl -u athaniel-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Athaniel installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit athaniel-gateway` (or `sudo systemctl edit athaniel-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `athaniel update`. When `athaniel update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart athaniel-gateway` command (it never blocks on an interactive password prompt).
...
```bash
athaniel gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `athaniel update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo athaniel update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/athaniel-gateway`:

```
athaniel ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed athaniel-gateway*, /usr/bin/systemctl --no-ask-password start athaniel-gateway*, /usr/bin/systemctl --no-ask-password restart athaniel-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Athaniel will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Athaniel installations on the same machine (with different `ATHANIEL_HOME` directories), each gets its own systemd service name. The default `~/.athaniel` uses `athaniel-gateway`; other installations use `athaniel-gateway-<hash>`. The `athaniel gateway` commands automatically target the correct service for your current `ATHANIEL_HOME`.
:::

### macOS (launchd)

```bash
athaniel gateway install               # Install as launchd agent
athaniel gateway start                 # Start the service
athaniel gateway stop                  # Stop the service
athaniel gateway status                # Check status
tail -f ~/.athaniel/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.athaniel.gateway.plist`. It includes three environment variables:
...
- **ATHANIEL_HOME** — scopes the gateway to your Athaniel installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `athaniel gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ATHANIEL_HOME` directory gets its own launchd label. The default `~/.athaniel` uses `ai.athaniel.gateway`; other installations use `ai.athaniel.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `athaniel-cli` | Full access |
| Telegram | `athaniel-telegram` | Full tools including terminal |
| Discord | `athaniel-discord` | Full tools including terminal |
| WhatsApp | `athaniel-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `athaniel-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `athaniel-slack` | Full tools including terminal |
| Google Chat | `athaniel-google_chat` | Full tools including terminal |
| Signal | `athaniel-signal` | Full tools including terminal |
| SMS | `athaniel-sms` | Full tools including terminal |
| Email | `athaniel-email` | Full tools including terminal |
| Home Assistant | `athaniel-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `athaniel-mattermost` | Full tools including terminal |
| Matrix | `athaniel-matrix` | Full tools including terminal |
| DingTalk | `athaniel-dingtalk` | Full tools including terminal |
| Feishu/Lark | `athaniel-feishu` | Full tools including terminal |
| WeCom | `athaniel-wecom` | Full tools including terminal |
| WeCom Callback | `athaniel-wecom-callback` | Full tools including terminal |
| Weixin | `athaniel-weixin` | Full tools including terminal |
| BlueBubbles | `athaniel-bluebubbles` | Full tools including terminal |
| QQBot | `athaniel-qqbot` | Full tools including terminal |
| Yuanbao | `athaniel-yuanbao` | Full tools including terminal |
| Microsoft Teams | `athaniel-teams` | Full tools including terminal |
| API Server | `athaniel-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `athaniel-webhook` | Full tools including terminal |
| Raft | `athaniel-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.athaniel/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
