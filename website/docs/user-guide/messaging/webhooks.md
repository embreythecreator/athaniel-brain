...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Athaniel Brain — Webhooks Tutorial"
...
1. Enable via `athaniel gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `athaniel webhook subscribe`
...
| `script` | No | Filter/transform script under `~/.athaniel/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
| `skills` | No | List of skill names to load for the agent run. |
| `toolsets` | No | List of toolset keys (e.g. `["terminal", "file", "web"]`) that **replaces** the platform-level webhook toolset for runs triggered by this route only. Manual config edit only — not settable via `athaniel webhook subscribe`, so agent-created subscriptions cannot self-grant elevated tools. Names are validated the same way as `platform_toolsets` entries (unknown or platform-restricted names are dropped). See [Per-route toolsets](#per-route-toolsets). |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.athaniel/data/todoist/watchlist.json"
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.athaniel/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.athaniel/scripts/todoist-athaniel-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "athaniel" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
- Empty stdout, exact `[SILENT]`, `{"__athaniel_ignore__": true}`, timeout, missing script, or nonzero exit code returns HTTP 200 with `{"status":"ignored","reason":"script"}`.
...
Add the `github-pr` route to your `~/.athaniel/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Athaniel processes the event, and posts a review comment on the PR.
...
```bash
athaniel webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `athaniel webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
athaniel webhook subscribe github-issues \
  --events "issues" \
...
```bash
athaniel webhook list
```

### Remove a subscription

```bash
athaniel webhook remove github-issues
```

### Test a subscription

```bash
athaniel webhook test github-issues
athaniel webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.athaniel/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `athaniel webhook subscribe` command.
...
For dynamic subscriptions, add the `toolsets` key by editing `~/.athaniel/webhook_subscriptions.json` directly:
...
- `athaniel webhook subscribe` deliberately does **not** accept a toolsets flag. Granting elevated tools is a manual config-file edit, so an agent creating its own subscription at runtime cannot self-grant `terminal`.
...
- Run the gateway in foreground to see logs: `athaniel gateway run`
