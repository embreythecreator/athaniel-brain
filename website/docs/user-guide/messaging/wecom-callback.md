...
Connect Athaniel to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Athaniel supports two WeCom integration modes:
...
> Run `athaniel gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Athaniel decrypts the message, queues it for the agent
...
(Use `athaniel gateway start` only after `athaniel gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Athaniel and the token the
...
in `~/.athaniel/.env` values around `=` will also break signature checks. After
fixing, restart `athaniel gateway run`.
...
Check `athaniel gateway run` logs for the bound host/port. If the adapter bound to
