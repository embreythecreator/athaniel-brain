...
description: Control your smart home with Athaniel Brain via Home Assistant integration.
...
Athaniel Brain integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Athaniel Brain"
5. Copy the token
...
Athaniel Brain registers four tools for smart home control:
...
Configure which events the agent sees in `~/.athaniel/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Athaniel Brain".
...
The adapter reads credentials from `~/.athaniel/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Athaniel profile
...
running Athaniel — `curl -H "Authorization: Bearer <token>" <url>/api/` should
