...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Athaniel A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Athaniel over HTTP.

It interoperates with any A2A-compliant peer — another Athaniel, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Athaniel ↔ Athaniel across machines** — let your desktop agent hand tasks to a Athaniel on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Athaniel so other frameworks' agents can send it tasks.
...
```bash
athaniel gateway setup      # pick A2A
```
...
```bash
athaniel tools enable a2a --platform cli        # CLI/TUI sessions
athaniel tools enable a2a --platform telegram   # or any messaging platform
athaniel tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Athaniel serves:
...
- **Audit log** — every exchange appends to `~/.athaniel/a2a_audit.jsonl`.
