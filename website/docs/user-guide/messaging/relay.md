---
sidebar_position: 30
title: "Athaniel Relay"
description: "Connect Athaniel to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Athaniel Relay (Connector)
...
Athaniel Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.athaniel/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.athaniel/.env`:
...
`athaniel gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.athaniel/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `athaniel auth add nous` (or `athaniel setup`) and retry.
...
startup. Restart it (`athaniel gateway restart`).
