...
Connect Athaniel to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Athaniel — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Athaniel runs it in a
...
athaniel photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Athaniel Brain` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.athaniel/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.athaniel/.env`
...
(device token, dashboard project id) lives in `~/.athaniel/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Athaniel
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Athaniel replies with a pairing code. Approve it with:

```bash
athaniel pairing approve photon <CODE>
```

Use `athaniel pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.athaniel/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.athaniel/.env`):
...
By default Athaniel responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Athaniel` and
`@Athaniel agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Athaniel will reply.

## Status & troubleshooting

```bash
athaniel photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Athaniel uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run athaniel photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `athaniel photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `athaniel photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `athaniel photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Athaniel sends images, voice
  ...
  media.
- **Native polls are supported.** Athaniel sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Athaniel, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Athaniel-sent messages
...
- **Message effects are supported.** Athaniel sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `athaniel send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<athaniel-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Athaniel is set
...
| `PHOTON_MENTION_PATTERNS` | Athaniel wake words  | JSON list / comma / newline regex patterns for group mentions |
