...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Athaniel Brain as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Athaniel Brain integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Athaniel opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Athaniel responds to every message. |
| Group chats | Athaniel responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Athaniel will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `athaniel gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Athaniel runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Athaniel behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Athaniel starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Athaniel responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Athaniel responds to @mentions from any user in any group. |
| `allowlist` | Athaniel only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Athaniel ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Athaniel read all group traffic without requiring an @mention:
...
Athaniel auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Athaniel ignores messages sent by other bots. Enable bot-to-bot messaging when you want Athaniel to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Athaniel. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `athaniel update --gateway` needs confirmation, the adapter records the selected answer in Athaniel's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Athaniel reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.athaniel/feishu_comment_rules.json` (pairing grants in `~/.athaniel/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
You can invite the Athaniel Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, Athaniel can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends Athaniel the meeting invitation event.
- Athaniel extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, Athaniel drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so Athaniel can reply to the inviter.
...
|---------|---------|---------|
| Quiet period | `ATHANIEL_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `ATHANIEL_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `ATHANIEL_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `ATHANIEL_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.athaniel/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `ATHANIEL_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `ATHANIEL_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `ATHANIEL_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `ATHANIEL_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `ATHANIEL_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `ATHANIEL_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `athaniel gateway setup` |
| `Another local Athaniel gateway is already using this Feishu app_id` | Only one Athaniel instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Athaniel can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `athaniel-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
