...
sidebar_position: 12
title: "Google Chat"
description: "Set up Athaniel Brain as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Athaniel Brain to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Athaniel
...
> Run `athaniel gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Athaniel can read (e.g.,
`~/.athaniel/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a athaniel restart)
...
- Principal: `athaniel-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Athaniel calls
...
- **App name**: whatever you want users to see ("Athaniel" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/athaniel-chat-events`.
...
`ADDED_TO_SPACE` event that Athaniel uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Athaniel

Add the Google Chat section to `~/.athaniel/.env`:
...
Send "hola" in the test DM. The bot posts a "Athaniel is thinking…" marker, then
...
`~/.athaniel/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Athaniel is thinking…").
...
Thread support: when a user replies inside a thread, Athaniel detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Athaniel session.
...
3. Download the JSON. Move it onto the host that runs Athaniel.
4. Register the client with Athaniel (run under the profile you want it scoped to):
...
athaniel -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Athaniel home (e.g.
`~/.athaniel/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.athaniel/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.athaniel/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Athaniel isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `athaniel gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Athaniel will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.athaniel/google_chat_user_tokens/<sanitized_email>.json` (filesystem
