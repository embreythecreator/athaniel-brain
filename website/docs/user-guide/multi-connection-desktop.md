...
# Connecting Desktop to Many Athaniel Instances

Register every Athaniel backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Athaniel Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Athaniel gateway…"**) deep-links straight to
...
list of Athaniel gateways. Its intro says it plainly: *"Manage this device and
every Athaniel gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Athaniel runtime managed by this app." | automatic |
| **Remote gateway** | "A Athaniel gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Athaniel install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Athaniel Cloud** | "A hosted instance discovered through your Athaniel Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Athaniel Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Athaniel Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Athaniel Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Athaniel Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `athaniel serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Athaniel agents
...
connection is registered) dispatches `athaniel update` to every eligible
...
- **Athaniel Cloud** instances are skipped with a *"Managed by Athaniel Cloud"*
...
panel, ⌘K **Update Athaniel**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `athaniel serve` is running on the remote host, the
...
- **"Update Athaniel Desktop to chat with agents on other connections"** — the
