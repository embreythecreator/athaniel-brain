...
A **profile distribution** packages a complete Athaniel agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Athaniel has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `athaniel profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `athaniel profile update` pulls new versions | Re-send the file |
...
| Made with | `athaniel profile install` / `update` | `/export` and `/import`, or `athaniel profile export` / `import` |
...
Before distributions, sharing a Athaniel agent meant sending someone:
...
```bash
athaniel profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `athaniel profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Athaniel in 2026, that's already true.
...
:::caution
**Athaniel does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `athaniel profile install` or `athaniel profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
athaniel profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.athaniel/profiles/research-bot/SOUL.md
...
Create `~/.athaniel/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
athaniel_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.athaniel/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.athaniel/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `athaniel profile update research-bot` will pull the latest.
...
```bash
athaniel profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.athaniel/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
athaniel profile install github.com/you/research-bot

# Full HTTPS
athaniel profile install https://github.com/you/research-bot.git

# SSH
athaniel profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
athaniel profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
athaniel profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
athaniel profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
athaniel profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
athaniel profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Athaniel distribution.
...
```bash
cp ~/.athaniel/profiles/research-bot/.env.EXAMPLE ~/.athaniel/profiles/research-bot/.env
...
```bash
athaniel profile info research-bot
```
...
`athaniel profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
athaniel profile update research-bot
```
...
```bash
athaniel profile delete research-bot
```
...
Path:    ~/.athaniel/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `athaniel profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.athaniel/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
athaniel profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `athaniel profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.athaniel/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/athaniel-polymarket-trader.git
git push -u origin main --tags

# Anyone
athaniel profile install github.com/you/athaniel-polymarket-trader --alias
```
...
You built Athaniel-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
athaniel profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
athaniel profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `athaniel profile info <name>` and hold off on updates until you're ready.
:::
...
athaniel profile info research-bot | grep Version
...
```yaml
# ~/.athaniel/profiles/research-bot/local/my-overrides.yaml
...
athaniel profile delete research-bot --yes
athaniel profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
athaniel profile update research-bot --force-config --yes
```
...
athaniel profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.athaniel/profiles/forked-research-bot/
...
athaniel profile install ~/.athaniel/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
athaniel profile delete research-bot-test --yes
athaniel profile install ~/.athaniel/profiles/research-bot --name research-bot-test
```
...
```bash
athaniel profile export research-bot
athaniel profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
athaniel profile import ./research-bot.tar.gz
athaniel profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `athaniel -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.athaniel`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.athaniel`) is exported through an allow-list — only known Athaniel artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.athaniel/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `athaniel -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `athaniel profile` — not a parallel command tree.
...
- Reserved profile names (`athaniel`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`athaniel profile export` / `import`](../reference/profile-commands.md#athaniel-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Athaniel](../guides/use-soul-with-athaniel.md) — authoring personalities
