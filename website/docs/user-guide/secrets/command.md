...
secrets:
  command:
    ...
    # command: "pass show athaniel/env"
    # command: "secret-tool lookup service athaniel-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Athaniel' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Athaniel discards its stderr on purpose) |
