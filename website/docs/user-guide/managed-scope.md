...
`~/.athaniel/config.yaml`, `~/.athaniel/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/athaniel`:

```text
/etc/athaniel/
├── config.yaml     # managed config layer (wins over ~/.athaniel/config.yaml)
└── .env            # managed env layer (wins over ~/.athaniel/.env + shell)
```
...
The location can be relocated with the `ATHANIEL_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ATHANIEL_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Athaniel.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ATHANIEL_MANAGED_DIR=/opt/org/athaniel-policy
```

:::warning
A user who can set `ATHANIEL_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `athaniel doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/athaniel/config.yaml` (managed) | `/etc/athaniel/.env` (managed) |
| 2 | `~/.athaniel/config.yaml` (user) | `~/.athaniel/.env` (user) |
...
```bash
athaniel config        # shows a header naming the managed source + the pinned keys
athaniel doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Athaniel refuses and names the source:

```bash
$ athaniel config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/athaniel/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `athaniel config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/athaniel

# Pin some config values for every user on this machine
sudo tee /etc/athaniel/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/athaniel/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/athaniel
sudo chmod 0644 /etc/athaniel/config.yaml /etc/athaniel/.env
```

Changes take effect on the next Athaniel start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`athaniel doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Athaniel as `root`), managed scope is advisory.
