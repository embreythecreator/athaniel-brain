...
If you only run one Athaniel agent, you don't need this page — see
...
[Connecting Desktop to Many Athaniel Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Athaniel agents that should all
...
(`ai.athaniel.gateway-<name>.plist`) or systemd user service
(`athaniel-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
athaniel profile create coder
athaniel profile create personal-bot
athaniel profile create research
...
```bash
athaniel config set gateway.multiplex_profiles true
athaniel gateway restart
```

Equivalently, in the default profile's `~/.athaniel/config.yaml`:
...
You do **not** run `athaniel gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `athaniel gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.athaniel/profiles/coder/.env`; the default listener key is rejected.
...
home). `athaniel status` reports the multiplexer and the profiles it serves;
`athaniel status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/athaniel-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: athaniel-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    athaniel gateway "$action"
  else
    athaniel -p "$profile" gateway "$action"
  fi
}
...
```bash
athaniel-gateways start      # start every configured profile
athaniel-gateways stop       # stop every configured profile
athaniel-gateways restart    # restart all
athaniel-gateways status     # status across all
athaniel-gateways list       # delegates to `athaniel gateway list`
```

:::tip
The `default` profile is targeted with `athaniel gateway <action>` (no `-p`),
not `athaniel -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `athaniel -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.athaniel.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/athaniel-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.athaniel.gateway.plist` /
`athaniel-gateway.service`.
...
```bash
# Default profile
tail -f ~/.athaniel/logs/gateway.log
tail -f ~/.athaniel/logs/gateway.error.log

# Named profile
tail -f ~/.athaniel/profiles/<name>/logs/gateway.log
tail -f ~/.athaniel/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.athaniel/logs/gateway.log ~/.athaniel/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
athaniel logs -f                  # follow default profile
athaniel -p coder logs -f         # follow one profile
athaniel logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
athaniel profile list             # profiles + model + gateway state
athaniel-gateways status          # full status across every profile
launchctl list | grep athaniel    # macOS — PIDs and labels
systemctl --user list-units 'athaniel-gateway-*'   # Linux — units
```
...
```
~/.athaniel/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.athaniel/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
athaniel config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.athaniel/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=athaniel --why="gateways running" \
  sleep infinity &
...
`athaniel-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.athaniel/.env ~/.athaniel/profiles/*/.env
```

## Updating the code

`athaniel update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `athaniel gateway start` after a previous `athaniel gateway stop`. The
...
```bash
ps -ef | grep "athaniel_cli.*-p <profile>"
cat ~/.athaniel/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.athaniel.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.athaniel.gateway-<profile>.plist

# Linux
systemctl --user restart athaniel-gateway-<profile>.service
```

### Health check

```bash
athaniel doctor                  # default profile
athaniel -p <profile> doctor     # one profile
```
