---
sidebar_position: 3
title: "Athaniel Desktop"
description: "The native Athaniel desktop app — a polished experience for chatting with Athaniel, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Athaniel Desktop

The Athaniel desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Athaniel Brain core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `athaniel` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Athaniel has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`athaniel`) and **[TUI](./tui.md)** (`athaniel --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`athaniel dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [Athaniel Desktop product page](https://athaniel-brain.nousresearch.com/desktop), or follow the [installation instructions for Athaniel Desktop](../getting-started/installation.md).

If you already have Athaniel installed, simply run
...
- **The same conversation history** as every other Athaniel surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Athaniel instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Athaniel Desktop to a remote backend](./features/web-dashboard.md#connecting-athaniel-desktop-to-a-remote-backend).

#### Repository discovery

Athaniel Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Athaniel sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `athaniel desktop --cwd <path>` (or the `ATHANIEL_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Athaniel to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `athaniel model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Athaniel management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#athaniel-cron).
- **Profiles** — switch between [Athaniel profiles](./profiles.md) (isolated config/skills/sessions).
...
roster where every [Athaniel profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Athaniel cron). The roster lives in the left
...
each other's Bot Chats directly (`athaniel -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Athaniel, and more.
...
The [manual update process](https://athaniel-brain.embreythecreator.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Athaniel agent, your config, and your chats stay. (Same as `athaniel uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `athaniel uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `athaniel uninstall --full`.)
...
You can do the same from the terminal — `athaniel uninstall --gui` for the GUI alone, or `athaniel uninstall` / `athaniel uninstall --full` for the agent too.

:::note
Running `athaniel uninstall --gui` from a **source checkout** (a `athaniel desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `athaniel desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `athaniel desktop`

To launch via the CLI, simply run `athaniel desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `athaniel update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `ATHANIEL_DESKTOP_CWD`)           |
| `--athaniel-root PATH` | Override the Athaniel source root the app uses (sets `ATHANIEL_DESKTOP_ATHANIEL_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `athaniel` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Athaniel Brain runtime into `ATHANIEL_HOME` (`~/.athaniel`, or `%LOCALAPPDATA%\athaniel` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `ATHANIEL_DESKTOP_ATHANIEL_ROOT`, then a completed managed install, then a probed `athaniel` on `PATH` (unless `--ignore-existing` / `ATHANIEL_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `ATHANIEL_DESKTOP_ATHANIEL` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `athaniel serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `athaniel --tui`. The desktop app is **self-contained**: it runs its own `athaniel serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Athaniel backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `athaniel serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Athaniel Cloud** — sign in once to Athaniel Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every Athaniel gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Athaniel Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Athaniel gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Athaniel Instances](./multi-connection-desktop.md).
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Athaniel](./features/web-dashboard.md#default-provider-nous-research).

### On the backend (the remote machine)

Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.athaniel/.env` (the secrets file, mode 0600):
...
athaniel serve --host 0.0.0.0 --port 9119
```

Keep that `athaniel serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `athaniel serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `ATHANIEL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.athaniel/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/athaniel` work if you front it with a reverse proxy)
2. **Sign in** — the app detects which provider the backend advertises and adapts the button. For a username/password backend it shows a **Sign in** button that opens a credential form (enter the credentials from step 1). For an OAuth backend it shows **Sign in with `<provider>`** (e.g. *Sign in with Athaniel*), which runs the provider's browser sign-in. Either way the app ends up with an authenticated session against the backend.
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `ATHANIEL_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `ATHANIEL_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ATHANIEL_DASHBOARD_BASIC_AUTH_USERNAME` / `ATHANIEL_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.athaniel/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `ATHANIEL_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Athaniel Desktop to a remote backend](./features/web-dashboard.md#connecting-athaniel-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Athaniel Desktop](../reference/environment-variables.md#web-dashboard--athaniel-desktop).
...
`$ATHANIEL_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
- **Open logs** — opens `ATHANIEL_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `athaniel debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `ATHANIEL_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.athaniel/athaniel-brain/.athaniel-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.athaniel/athaniel-brain/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.embreythecreator.athaniel
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.athaniel/athaniel-brain/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
ATHANIEL_DESKTOP_ATHANIEL_ROOT=/path/to/clone npm run dev
ATHANIEL_HOME=/tmp/throwaway npm run dev
...
(Desktop, then Downloads, then Documents, ...) as Athaniel touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Athaniel' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Athaniel.app** if you use Desktop.
3. Fully quit and relaunch them once.

`athaniel doctor` reports whether the current terminal context already has the
grant, and `athaniel setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.embreythecreator.athaniel   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
Athaniel. Grants are stable from then on.
...
certificate once and tell Athaniel to use it. The one-shot command does
...
```bash
athaniel desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Athaniel Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `athaniel config set desktop.macos_signing_identity "Athaniel Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Athaniel Local Signing`). The command is idempotent —
...
`tccutil reset All com.embreythecreator.athaniel` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `athaniel --tui` and the dashboard chat tab
