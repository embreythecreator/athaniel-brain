---
title: "Athaniel Brain Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Athaniel Brain Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Athaniel Brain Skill Authoring
...
| Path | `skills/software-development\athaniel-brain-skill-authoring` |
| Version | `2.0.0` |
| Author | Athaniel Brain |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `athaniel-brain`, `conventions`, `skill-md` |
...
# Authoring Athaniel Brain Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.athaniel/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the athaniel-brain repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with athaniel-brain
...
- Don't use for: personal skills in `~/.athaniel/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `athaniel skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Athaniel Brain
license: MIT
...
- Credit the **human first**, then "Athaniel Brain" as secondary collaborator: `Ben Barclay (benbarclay), Athaniel Brain`.
- Never `author: Athaniel Brain` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Athaniel Brain`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.athaniel/skills/`.
...
|---|---|
| Athaniel tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.athaniel/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Athaniel Brain` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.athaniel.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Athaniel tools
