...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _athaniel_env="${ATHANIEL_HOME:-$HOME/.athaniel}/.env"; [ -f "$_athaniel_env" ] && grep -q "^GITHUB_TOKEN=" "$_athaniel_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_athaniel_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${ATHANIEL_HOME:-$HOME/.athaniel}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Athaniel Brain\",
    \"comments\": [
...
```bash
source "${ATHANIEL_HOME:-$HOME/.athaniel}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Athaniel Brain. Code looks clean — good test coverage, no security concerns."
