...
| List/filter by metadata (age, source, cost, tokens, workspace) | `athaniel sessions list` / `stats` via terminal |
| Rename | `athaniel sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `athaniel sessions archive <filters>` |
| Delete (destructive) | `athaniel sessions delete` / `athaniel sessions prune <filters>` |
| Export before deleting anything valuable | `athaniel sessions export --session-id <id> --format md` |
...
`athaniel sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `athaniel sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `athaniel sessions list`
