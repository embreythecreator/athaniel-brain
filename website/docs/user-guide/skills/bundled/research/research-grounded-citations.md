...
Ledger location: `$ATHANIEL_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `ATHANIEL_CITATION_LEDGER`.

## How to Run

```bash
S=~/.athaniel/skills/research/grounded-citations/scripts/sources.py
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `ATHANIEL_CITATION_LEDGER`) if their
