...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/devops/watchers` |
...
All three live in `$ATHANIEL_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$ATHANIEL_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $ATHANIEL_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${ATHANIEL_HOME:-~/.athaniel}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $ATHANIEL_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name athaniel-issues --repo embreythecreator/athaniel-brain --scope issues
```

Poll an arbitrary JSON API:

```bash
python $ATHANIEL_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$ATHANIEL_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $ATHANIEL_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $ATHANIEL_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$ATHANIEL_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
