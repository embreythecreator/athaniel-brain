...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ATHANIEL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ATHANIEL_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ATHANIEL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client athaniel
```
...
```bash
PUBLISH="${ATHANIEL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client athaniel
```
...
```bash
DRIVE="${ATHANIEL_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `athaniel`)    |
