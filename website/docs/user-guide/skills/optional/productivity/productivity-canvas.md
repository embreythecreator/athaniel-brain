...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Athaniel Brain"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ATHANIEL_HOME:-~/.athaniel}/.env`:
...
```bash
CANVAS="python $ATHANIEL_HOME/skills/productivity/canvas/scripts/canvas_api.py"
