...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/productivity/siyuan` |
...
3. Store it in `${ATHANIEL_HOME:-~/.athaniel}/.env`:
   ```
...
```yaml
# In ~/.athaniel/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
