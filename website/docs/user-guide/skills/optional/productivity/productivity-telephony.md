...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/productivity/telephony` |
...
This optional skill gives Athaniel practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ATHANIEL_HOME:-~/.athaniel}/.env`
...
It does **not** turn Athaniel into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Athaniel memory
...
### 1) "I want Athaniel to own a real phone number"
Use **Twilio**.
...
- pairs well with Athaniel `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${ATHANIEL_HOME:-~/.athaniel}/.env`
...
```bash
SCRIPT="$(find ~/.athaniel/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
athaniel skills search telephony
athaniel skills install official/productivity/telephony
```
...
Then save credentials into Athaniel:
...
3. Buy it and save it into `${ATHANIEL_HOME:-~/.athaniel}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Athaniel calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Athaniel's existing `text_to_speech` support.

Use this when:
- you want the call to use Athaniel's configured TTS voice rather than Twilio `<Say>`
...
Recommended Athaniel TTS -> Twilio Play workflow:

1. Generate the audio with Athaniel `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Athaniel to create the message audio with `text_to_speech`
...
Important note:
- Athaniel TTS is great for prerecorded outbound messages
...
- Athaniel STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Athaniel memory.
...
- Do not store arbitrary third-party phone numbers in Athaniel memory.
...
3. persist that number to `${ATHANIEL_HOME:-~/.athaniel}/.env`
