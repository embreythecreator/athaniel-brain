...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/web-development/har-derived-api-client` |
...
Covers **every Athaniel browser pathway**: the default local `browser_navigate`
...
- For the CDP path (`har_capture_cdp.py`): a reachable CDP endpoint. On Athaniel,
...
| Browser pathway | How Athaniel reaches it | Capturer |
|---|---|---|
...
Rule of thumb: **if Athaniel *launched* the browser, use `har_capture.py`; if it
...
```bash
# 1a. Capture, LOCAL browser (Athaniel launched it)
...
har_capture.py <url> <out.har> [--wait S] [--headed] [--action SPEC ...]
  ...
  use when Athaniel LAUNCHED the browser (local browser_navigate default)
...
0. **Pick the capturer by pathway** (see How to Run table). Launched-locally → `har_capture.py`; reached over CDP → `har_capture_cdp.py`. On Athaniel, `/browser connect` tells you the CDP endpoint when a cloud/remote backend is active.
...
- **Wrong capturer = empty/no HAR.** `har_capture.py` on a cloud/CDP backend records nothing (it launches its own local browser instead of the one you meant). `har_capture_cdp.py` needs the endpoint; on Athaniel get it from `/browser connect` or `BROWSER_CDP_URL`. Match the capturer to the pathway (How to Run table).
...
- **CDP capture doesn't close the browser.** `har_capture_cdp.py` attaches to a browser it doesn't own and leaves it running — correct for cloud/remote sessions Athaniel manages. Don't add a close; let the owning backend tear it down.
