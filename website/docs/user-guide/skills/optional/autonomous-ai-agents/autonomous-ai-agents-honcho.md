---
title: "Honcho — Configure and troubleshoot Honcho memory for Athaniel"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Athaniel"
---
...
Configure and troubleshoot Honcho memory for Athaniel.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Athaniel

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Athaniel profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/athaniel#running-honcho-locally-with-athaniel

### Verify

```bash
athaniel honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Athaniel profile's AI peer
...
Honcho models conversations as interactions between **peers**. Athaniel creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Athaniel instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Athaniel run |
| `global` | Single session across all directories |

Manual override: `athaniel honcho map my-project-name`
...
Each Athaniel profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block athaniel.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `athaniel.coder` host block in `honcho.json`
...
```bash
athaniel honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Athaniel when Honcho memory is active.
...
Config file: `$ATHANIEL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `athaniel honcho setup`. Ensure `memory.provider: honcho` is in `~/.athaniel/config.yaml`.

### Memory not persisting across sessions
Check `athaniel honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `athaniel profile create <name> --clone`. For existing profiles: `athaniel honcho sync`.
...
| Command | Description |
|---------|-------------|
| `athaniel honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `athaniel honcho status` | Show resolved config, connection test, peer info for active profile |
| `athaniel honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `athaniel honcho disable` | Disable Honcho for the active profile |
| `athaniel honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `athaniel honcho peers` | Show peer identities across all profiles |
| `athaniel honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `athaniel honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `athaniel honcho sessions` | List known directory-to-session-name mappings |
| `athaniel honcho map <name>` | Map current working directory to a Honcho session name |
| `athaniel honcho identity` | Seed AI peer identity or show both peer representations |
| `athaniel honcho sync` | Create host blocks for all Athaniel profiles that don't have one yet |
| `athaniel honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Athaniel + Honcho |
| `athaniel memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `athaniel memory status` | Show active memory provider and config |
| `athaniel memory off` | Disable external memory provider |
