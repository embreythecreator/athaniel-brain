...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/software-development/grill-me` |
| Path | `optional-skills/software-development\grill-me` |
| Version | `2.0.0` |
| Author | Rafael Zendron (rafaumeu) + Matt Pocock (mattpocock/skills, grilling) + Athaniel Brain |
| License | MIT |
...
1. **Asking questions out of dependency order.** A question that depends on
   ...
   later round.
2. **Skipping the codebase.** Find facts in code with Athaniel tools instead of
   asking the user.
