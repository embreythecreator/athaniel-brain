...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/payments/mpp-agent` |
...
Wraps the Machine Payments Protocol (MPP, https://mpp.dev) clients so Athaniel can pay for per-request API access against servers that respond with `HTTP 402 Payment Required`.
