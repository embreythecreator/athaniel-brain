---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Athaniel"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Athaniel"
---
...
Import an OpenClaw setup (memories, skills) into Athaniel.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Athaniel Brain (Athaniel) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Athaniel`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Athaniel Migration

Use this skill when a user wants to move their OpenClaw setup into Athaniel Brain with minimal manual cleanup.
...
```bash
athaniel claw migrate              # Full interactive migration
...
athaniel claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `athaniel setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_athaniel.py` to:

- import `SOUL.md` into the Athaniel home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Athaniel memory entries
- merge OpenClaw command approval patterns into Athaniel `command_allowlist`
- migrate Athaniel-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Athaniel working-directory configuration
- copy OpenClaw skills into `~/.athaniel/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Athaniel workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.athaniel/tts/`
- archive non-secret docs that do not have a direct Athaniel destination
...
- `scripts/openclaw_to_athaniel.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py`

Do not guess a shorter path like `~/.athaniel/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.athaniel/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Athaniel-compatible secrets, currently:
...
Athaniel CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Athaniel. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Athaniel workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Athaniel skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.athaniel/skills/migration/openclaw-migration/scripts/openclaw_to_athaniel.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Athaniel unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Athaniel targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Athaniel destination. Unsupported auth blobs must still be reported as skipped.
...
- Athaniel persona state imported
- Athaniel memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.athaniel/skills/openclaw-imports/`
