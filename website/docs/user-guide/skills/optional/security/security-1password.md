...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Athaniel Brain |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Athaniel terminal calls (desktop app flow only)
...
### Service Account (recommended for Athaniel)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ATHANIEL_HOME:-~/.athaniel}/.env` (the skill will prompt for this on first load).
...
## Athaniel Execution Pattern (desktop app flow)

Athaniel terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/athaniel-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/athaniel-op.sock"
