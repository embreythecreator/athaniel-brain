...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp\mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Athaniel Brain |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`athaniel-brain`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athaniel-brain), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Athaniel Gateway

## Overview

Athaniel' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Athaniel process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Athaniel runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Athaniel.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Athaniel' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Athaniel is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Athaniel** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
Athaniel' native MCP OAuth client (`tools/mcp_oauth.py`):

1. Picks a free local port `P`.
2. Registers a dynamic OAuth client with the AS, sending `redirect_uri = http://127.0.0.1:P/callback`.
3. Starts an HTTP server on `127.0.0.1:P` **inside the Athaniel process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When Athaniel runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the Athaniel process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the athaniel user, an
empty or missing tokens directory (`$ATHANIEL_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Athaniel detects a remote session it prints two
...
Both require an interactive terminal to the Athaniel host. The rest of this skill
is for when there is NO interactive TTY — Athaniel running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Athaniel Dashboard (try this BEFORE manual token surgery)

A remote Athaniel gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `athaniel dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'athaniel dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "ATHANIEL_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`ATHANIEL_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$ATHANIEL_HOME/.env`. Those
...
Athaniel' `AthanielTokenStorage` would have written, so on `/reload-mcp` Athaniel finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "ATHANIEL|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Athaniel' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find ATHANIEL_HOME and the config path

```bash
ATHANIEL_HOME=$(python3 -c 'from athaniel_constants import get_athaniel_home; print(get_athaniel_home())')
echo "config: $ATHANIEL_HOME/config.yaml"
echo "tokens: $ATHANIEL_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Athaniel Brain (manual OAuth)",
...
### 8. Write tokens in Athaniel' exact schema

`tools/mcp_oauth.py::AthanielTokenStorage` expects two files under
`$ATHANIEL_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "athaniel-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Athaniel (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Athaniel sees `auth: oauth`, calls `AthanielTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Athaniel process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Athaniel needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Athaniel does natively).
...
- **Don't claim Athaniel doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Athaniel. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Athaniel' config.
