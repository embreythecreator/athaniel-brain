...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/mcp/fastmcp` |
...
| Related skills | [`athaniel-brain`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athaniel-brain), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Athaniel or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Athaniel. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.athaniel/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.athaniel/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Athaniel integration, either:

- configure the server in `~/.athaniel/config.yaml` using the `native-mcp` skill, or
...
### Athaniel cannot see the deployed server

The server-building part may be correct while the Athaniel config is not. Load the `native-mcp` skill and configure the server in `~/.athaniel/config.yaml`, then restart Athaniel.
