...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Athaniel Kanban pipeline that
...
3. **Generates** a setup script that creates Athaniel profiles, project workspace, and the initial kanban task
...
For mapping role → which Athaniel skills + toolsets it loads, see
...
3. Creates each Athaniel profile via `athaniel profile create --clone`
...
7. Fires the initial `athaniel kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/embreythecreator/athaniel-brain/blob/main/optional-skills/creative\kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
athaniel kanban watch --tenant <project-tenant>     # live events
athaniel kanban list  --tenant <project-tenant>     # board snapshot
athaniel dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/embreythecreator/athaniel-brain/blob/main/optional-skills/creative\kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/embreythecreator/athaniel-brain/blob/main/optional-skills/creative\kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `athaniel kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${ATHANIEL_HOME:-~/.athaniel}/.env` or the user's secret store.
