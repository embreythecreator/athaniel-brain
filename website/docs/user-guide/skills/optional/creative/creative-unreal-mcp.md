...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Athaniel MCP catalog. The
...
Two halves, in this order: the editor side must be up before Athaniel connects.
...
### One-time, Athaniel side

    athaniel mcp install unreal-engine
...
`~/.athaniel/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Athaniel — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Athaniel connects
...
2. Start the Athaniel session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Athaniel
they appear as:

| Athaniel tool | Purpose |
|---|---|
...
then happens at `athaniel mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Athaniel runs batched calls
...
- **Start order matters.** Editor + server up first, then the Athaniel
...
- **Timeouts on long operations.** Athaniel' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.athaniel/config.yaml` for
