...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/creative/sketch` |
| Path | `optional-skills/creative\sketch` |
| Version | `1.0.1` |
| Author | Athaniel Brain (adapted from gsd-build/get-shit-done) |
| License | MIT |
...
If `gsd-sketch` shows up as a sibling skill (installed via `npx get-shit-done-cc --athaniel`), you can use **`gsd-sketch`** for the fuller workflow: persistent `.planning/sketches/` with MANIFEST, frontier mode analysis, consistency audits across past sketches, and integration with the rest of GSD. This skill is the lightweight standalone version — one-off sketching without the state machinery.
...
**Verify variants visually — use Athaniel' browser tools.** Don't just write HTML and hope it renders; load each variant and look at it:
...
Adapted from the GSD (Get Shit Done) project's `/gsd-sketch` workflow — MIT © 2025 Lex Christopherson ([gsd-build/get-shit-done](https://github.com/gsd-build/get-shit-done)). The upstream GSD repo is now **archived/unmaintained** on GitHub; the `get-shit-done-cc` npm package still installs (`npx get-shit-done-cc --athaniel --global`) and ships persistent sketch state, theme/variant pattern references, and consistency-audit workflows, but treat it as an archived community project.
