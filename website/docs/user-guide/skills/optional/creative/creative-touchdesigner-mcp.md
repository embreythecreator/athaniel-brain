...
| | |
|---|---|
| Source | Optional — install with `athaniel skills install official/creative/touchdesigner-mcp` |
...
```
Athaniel Brain -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${ATHANIEL_HOME:-$HOME/.athaniel}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Athaniel config (if missing)
...
3. **Restart Athaniel session** to pick up the new MCP server
