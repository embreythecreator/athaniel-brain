...
The `execute_code` tool lets the agent write Python scripts that call Athaniel tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Athaniel over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from athaniel_tools import ...`
2. Athaniel generates a `athaniel_tools.py` stub module with RPC functions
3. Athaniel opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Athaniel
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from athaniel_tools import web_search, web_extract
...
```python
from athaniel_tools import search_files, read_file
import json
...
```python
from athaniel_tools import search_files, read_file, patch
...
```python
from athaniel_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.athaniel/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Athaniel's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Athaniel's own python) |
...
### `ATHANIEL_*` variables in the child

The child process receives only a small, fixed set of operational `ATHANIEL_*`
...
(plus `ATHANIEL_RPC_DIR` / `ATHANIEL_RPC_SOCKET` / `TZ` / `HOME`, which Athaniel
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `ATHANIEL_`
through to the child. That broad prefix was removed for security hardening: it
could leak `ATHANIEL_*`-named configuration that doesn't match a secret substring
(for example `ATHANIEL_BASE_URL`, `ATHANIEL_KANBAN_DB`, or a `ATHANIEL_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `ATHANIEL_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Athaniel-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `ATHANIEL_*`
variables, Athaniel emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`athaniel logs --level
DEBUG`, or check `~/.athaniel/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted ATHANIEL_* var(s)` if a script behaves
as though a `ATHANIEL_*` variable is missing.

Athaniel always writes the script and the auto-generated `athaniel_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Athaniel tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

## Platform Support

Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, Athaniel automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
