...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Athaniel's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Athaniel instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
athaniel memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$ATHANIEL_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing Athaniel at a self-hosted Honcho server, `athaniel honcho setup` (and `athaniel memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `athaniel` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Athaniel syncs them back at session init.
...
The `athaniel honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `athaniel memory setup honcho` (or run `athaniel memory setup` and pick it from the list); the `athaniel honcho` subcommand then appears on the next invocation.

```bash
athaniel memory setup honcho    # Configure Honcho directly (works before activation)
athaniel honcho status          # Connection status, config, and key settings
athaniel honcho setup           # Redirects to `athaniel memory setup` (post-activation alias)
athaniel honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
athaniel honcho peer            # Show or update peer names + dialectic reasoning level
athaniel honcho mode            # Show or set recall mode (hybrid/context/tools)
athaniel honcho tokens          # Show or set token budget for context and dialectic
athaniel honcho identity        # Seed or show the AI peer's Honcho identity
athaniel honcho sync            # Sync Honcho config to all existing profiles
athaniel honcho peers           # Show peer identities across all profiles
athaniel honcho sessions        # List known Honcho session mappings
athaniel honcho map             # Map current directory to a Honcho session name
athaniel honcho enable          # Enable Honcho for the active profile
athaniel honcho disable         # Disable Honcho for the active profile
athaniel honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `athaniel honcho`

If you previously used the standalone `athaniel honcho setup`:
...
No re-login or re-setup needed. Run `athaniel memory setup` and select "honcho" — the wizard detects your existing config.
