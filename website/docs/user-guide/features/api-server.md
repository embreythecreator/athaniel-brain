...
sidebar_position: 14
title: "API Server"
description: "Expose athaniel-brain as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes athaniel-brain as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to athaniel-brain and use it as a backend.

Your agent handles requests with its full toolset (terminal, file operations, web search, memory, skills) and returns the final response. When streaming, tool progress indicators appear inline so frontends can show what the agent is doing.

:::tip One backend covers models + tools
Athaniel itself needs a configured provider and tool backends for the API server to be useful. A [Nous Portal](/user-guide/features/tool-gateway) subscription handles both — 300+ models plus web/image/TTS/browser via the Tool Gateway. Run `athaniel setup --portal` once before starting the API server and frontends like Open WebUI or LobeChat get a fully tool-equipped backend.
:::
...
# Optional: only if a browser must call Athaniel directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "athaniel-brain", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus Athaniel' custom `athaniel.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: Athaniel emits `event: athaniel.tool.progress` for tool-start visibility without polluting persisted assistant text.
- **Responses**: Athaniel emits spec-native `function_call` and `function_call_output` output items during the SSE stream, so clients can render structured tool UI in real time.
...
Tool calls in the `output` array were already executed server-side by the Athaniel agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `athaniel-brain` for the default profile). Required by most frontends for model discovery.

`/v1/models` is intentionally the cheap OpenAI-compat surface. It does **not**
enumerate every authenticated provider/model combination Athaniel can route to,
and it does not do pricing or capability enrichment.

### GET /api/model/options

Athaniel-aware clients can request the same curated provider/model inventory used
...
Normal opens are intentionally conservative for custom providers: Athaniel probes
...
authenticated UI needs the richer Athaniel-specific picker metadata.
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running Athaniel version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.

## Browser-extension control

Athaniel can route browser tools through an authenticated extension that controls
the browser session associated with the current Athaniel session. The feature is
...
register only for an existing server session. Athaniel derives the controller
...
Athaniel preserves the existing browser backend. Once the gateway binds a
...
Athaniel never retries the same action through another backend.
...
2. Athaniel returns a single-use ticket with a 30-second TTL and the filtered,
   server-bound controller scope.
3. Open `GET /v1/browser-control/ws` with both WebSocket subprotocols:
   `athaniel-browser-control-v1` and
   `athaniel-browser-control-ticket.<ticket>`.
...
Athaniel sends `browser.controller.command` frames containing `command_id`,
...
Authenticated clients can override Athaniel' default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the Athaniel provider slug to resolve credentials/runtime for this turn
...
alias, Athaniel rejects the request with `400` instead of silently remixing route
...
Requests that include an explicit `provider` — and the Athaniel-native
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, Athaniel surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.

For safely retryable creation, send an `Idempotency-Key` header (1–255 visible ASCII characters). Athaniel durably reserves the key before starting work. An identical retry returns the original `run_id` with HTTP 202 and `Idempotency-Replayed: true`, including after a gateway restart and after the run has completed, failed, or been cancelled. Reusing the same key with a different JSON payload returns HTTP 409 with code `idempotency_key_conflict`. Keys are isolated by authenticated API profile/credential and retained for 24 hours after their last status update; clients should use unique, unguessable keys and must not reuse them for unrelated operations. Requests without the header retain the legacy behavior and always create a new run.

When `session_id` identifies an existing Athaniel session and no explicit
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while Athaniel asks the active agent to stop at the next safe interruption point.
...
Create a new scheduled job. Body accepts the same shape as `athaniel cron` — prompt, schedule, skills, provider override, delivery target.
...
External UIs can manage Athaniel sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
## Long-term memory scoping (`X-Athaniel-Session-Key`)

Multi-user frontends like Open WebUI need a stable per-channel identifier for long-term memory (Honcho, etc.) that is **independent** of the transcript-scoped `X-Athaniel-Session-Id` (which rotates on `/new`). Pass `X-Athaniel-Session-Key` on `/v1/chat/completions`, `/v1/responses`, or `/v1/runs` and Athaniel threads it through to `AIAgent(gateway_session_key=...)`, where the Honcho memory provider uses it to derive a stable scope.

```http
POST /v1/chat/completions HTTP/1.1
Authorization: Bearer ***
X-Athaniel-Session-Id: transcript-alpha
X-Athaniel-Session-Key: agent:main:webui:dm:user-42
```

Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-Athaniel-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior Athaniel had before.

## System Prompt Handling

When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), athaniel-brain **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call Athaniel directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.athaniel/profiles/<profile>/.env`). The default
...
:::warning Security
The API server gives full access to athaniel-brain's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `athaniel-brain` for default profile. |

### config.yaml

The same settings can live in `~/.athaniel/config.yaml` under a nested `gateway.api_server:` section:
...
To give multiple users their own isolated Athaniel instance (separate config, memory, skills), use [profiles](/user-guide/profiles):

```bash
# Create a profile per user
athaniel profile create alice
athaniel profile create bob
...
cat >> ~/.athaniel/profiles/alice/.env <<EOF
API_SERVER_ENABLED=true
API_SERVER_PORT=8643
API_SERVER_KEY=alice-secret
EOF

cat >> ~/.athaniel/profiles/bob/.env <<EOF
API_SERVER_ENABLED=true
...
athaniel -p alice gateway &
athaniel -p bob gateway &
```
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Athaniel instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
