...
Athaniel Brain includes a full browser automation toolset with multiple backend options:
...
- **Lightpanda local engine** via [Lightpanda](https://lightpanda.io) — a headless browser built from scratch in Zig for machines; instant start up, 16x lower memory and 9x faster than Chrome. Works in Browser Use mode (Athaniel spawns it, no Chromium or Node needed) and with the built-in tools (automatic Chrome fallback for actions it doesn't support yet)
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `athaniel setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `athaniel model` or `athaniel tools`.
:::
...
**This is the default browser mode**: when `browser.backend` is unset and the `browser-use` CLI is runnable (installed, or available through `uvx`), the agent gets the single `browser_exec` tool. If the CLI can't run, Athaniel falls back to the built-in browser tools automatically.

The mode is a **driver** that composes with your configured browser backend: it drives your local Chrome, a Nous-subscription cloud browser, Browserbase, Firecrawl, or Browser Use cloud browsers — whichever browser source is selected in `athaniel tools` → Browser Automation. The one exception is Camofox, which has no CDP endpoint for the harness to attach to; Camofox setups automatically keep the built-in browser tools.
...
When a cloud provider is configured, Athaniel auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`athaniel setup tools → Browser Automation`
...
When enabled, Athaniel copies your default browser's **active** profile — the one
...
`~/.athaniel/browser-profile/<browser>/`, then launches your **real browser
...
When you turn the toggle back off, Athaniel deletes the snapshot store
(`~/.athaniel/browser-profile/`) on the next browser use, so the copied
...
an exclusive (deny-all) lock, so Athaniel cannot copy them while the browser is
...
Set `browser.real_profile_autoclose: true` to let Athaniel **offer to close the
browser for you** when it's holding the profile. Even with this on, Athaniel never
closes it automatically — when the profile is locked it always stops and the
agent asks you first; only on your approval does it run `athaniel browser
...
(e.g. a background/tray instance relaunched), Athaniel stays blocked and tells you
...
Or configure via `athaniel tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `athaniel tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.athaniel/config.yaml`:
...
Then fully restart Athaniel so the new config is picked up.

:::warning Nested path matters
Athaniel reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Athaniel ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Athaniel silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Athaniel profile, so different Athaniel profiles get different browser profiles (profile isolation).

##### What Athaniel does not do
- It does not force persistence on the Camofox server. Athaniel only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Athaniel cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Athaniel and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Athaniel after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Athaniel derives the stable `userId` from the profile-scoped directory `~/.athaniel/browser_auth/camofox/` (or the equivalent under `$ATHANIEL_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Athaniel profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Athaniel to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Athaniel uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Athaniel calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Athaniel skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Athaniel does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Athaniel issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Athaniel adopts the most recently created one in that group.
3. Otherwise, Athaniel adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Athaniel falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Athaniel does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Athaniel to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Athaniel generates a per-task `session_key` (`task_<id>`) — Athaniel will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Athaniel can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Athaniel runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Athaniel automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
Lightpanda is a **local engine** (a browser source, like "Local Browser"), not a cloud provider. Install the binary and put it on your `PATH` (see the [Lightpanda installation guide](https://lightpanda.io/docs/run-locally/installation/one-liner)), then pick **Lightpanda** in `athaniel tools` → Browser Automation, or set:
...
- **Browser Use mode (the default).** Athaniel launches `lightpanda serve --host 127.0.0.1 --port <free>` itself — one process per `browser_exec` session name (or per task) — and points the Browser Use CLI at it. No Chromium, Playwright or Node.js is needed. The process is reaped after `browser.inactivity_timeout`, on exit, and by the orphan sweep if Athaniel crashes. Lightpanda has no graphical renderer, so `capture_screenshot()` is unavailable and the tool description tells the model to work text-first; it also holds one page per session, so the model is told to call `new_tab()` once and `goto_url()` afterwards (tracked upstream in [lightpanda-io/browser#1962](https://github.com/lightpanda-io/browser/issues/1962)).
- **Built-in browser tools** (`/browser use off`). Athaniel drives Lightpanda through `agent-browser --engine lightpanda` over CDP, the same way it drives local Chrome, with **automatic Chrome fallback**: Lightpanda handles the actions it supports (navigate, snapshot, click, type, scroll, back, press, eval) and Athaniel transparently retries on Chrome for anything it doesn't. Screenshots and `browser_vision` are routed straight to Chrome.

**When the engine is ignored.** `browser.engine` is the lowest-precedence browser setting: a cloud provider (including the Nous subscription browser — and on never-configured setups, any `BROWSERBASE_API_KEY` / `BROWSER_USE_API_KEY` in `~/.athaniel/.env` auto-selects one), Camofox, a `browser.cdp_url` / `/browser connect` override, or `browser.use_real_profile` all take precedence. Picking Lightpanda in `athaniel tools` writes `cloud_provider: local` for you; `/browser status` and `athaniel doctor` report when the engine is configured but shadowed, and by what.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Athaniel browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Athaniel from the terminal (`athaniel` or `athaniel chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.athaniel/chrome-debug \
  --no-first-run \
...
Then launch the Athaniel CLI and run `/browser connect`.
...
If Athaniel runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Athaniel itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Athaniel talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Athaniel MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Athaniel can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Browser Use mode (default) "lightpanda" makes Athaniel spawn `lightpanda serve`;
...
# Extra Chromium launch flags (comma- or newline-separated). Athaniel auto-injects
...
# Athaniel doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `athaniel config set toolsets '["athaniel-cli", "browser"]'`.
:::
...
You can also run `athaniel config set browser.snapshot_threshold 30000`. The setting applies to both explicit `browser_snapshot` calls and the automatic snapshot returned after navigation, including the Camofox backend (minimum 1000). Restart the current Athaniel session after changing it so the browser config cache reloads.
...
```
Type "athaniel agent" into the search field @e3
```
...
Screenshots are stored in `~/.athaniel/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.athaniel/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Athaniel automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages are truncated at `browser.snapshot_threshold` (default 15,000 characters, matching `web_extract`; no LLM summarization); the complete snapshot is saved to `~/.athaniel/cache/web/` and the output points at it for `read_file` paging
