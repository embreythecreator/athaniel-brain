...
Athaniel Brain ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
athaniel memory setup      # interactive picker + configuration
athaniel memory status     # check what's active
athaniel memory off        # disable external provider
```

You can also select the active memory provider via `athaniel plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.athaniel/config.yaml`:
...
When a memory provider is active, Athaniel automatically:
...
**Setup Wizard:**
```bash
athaniel memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `athaniel honcho setup` command still works (it now redirects to `athaniel memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$ATHANIEL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ATHANIEL_HOME/honcho.json` > `~/.athaniel/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/embreythecreator/athaniel-brain/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/athaniel).
...
:::tip Migrating from `athaniel honcho`
If you previously used `athaniel honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Athaniel profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Athaniel profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Athaniel profile. Host key `athaniel` → default; `athaniel.<profile>` for others. |
...
```bash
athaniel profile create coder --clone
```

`--clone` creates a `athaniel.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
athaniel honcho sync
```

Scans every Athaniel profile, creates host blocks for any profile without one, inherits settings from the default `athaniel` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `athaniel memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/embreythecreator/athaniel-brain/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/athaniel).
...
athaniel memory setup    # select "openviking"
# Or manually:
athaniel config set memory.provider openviking
```

`athaniel memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.athaniel/.env`, and for named profiles use
`~/.athaniel/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Athaniel sends no peer ID and writes
...
Set `memory.openviking.agent: athaniel` to restore the old peer-scoped writes.
...
Athaniel sends `User-Agent: openviking-memory-athaniel/<version>` on OpenViking
...
**Setup (Platform):**
```bash
athaniel memory setup    # select "mem0" → "Platform"
# Or manually:
athaniel config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.athaniel/.env
```

**Setup (OSS):**
```bash
athaniel memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
athaniel memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
athaniel memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
athaniel memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
athaniel memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.athaniel/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.athaniel/.env
```
...
**Config:** `$ATHANIEL_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.athaniel/.env`.
...
| `user_id` | `athaniel-user` | User identifier |
| `agent_id` | `athaniel` | Agent identifier |
...
**Switching modes:** Re-run `athaniel memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
athaniel memory setup    # select "hindsight"
# Or manually:
athaniel config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.athaniel/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p athaniel ui start`

**Config:** `$ATHANIEL_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `athaniel` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Athaniel Brain and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/embreythecreator/athaniel-brain/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
athaniel memory setup    # select "holographic"
# Or manually:
athaniel config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.athaniel-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ATHANIEL_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
athaniel memory setup    # select "retaindb"
# Or manually:
athaniel config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.athaniel/.env
```
...
# Then configure Athaniel
athaniel memory setup    # select "byterover"
# Or manually:
athaniel config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ATHANIEL_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=athaniel), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
athaniel memory setup    # select "supermemory"
# Or manually:
athaniel config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.athaniel/.env
```
...
Before running `athaniel memory setup`, set `base_url` in
`$ATHANIEL_HOME/supermemory.json`:
...
Then run `athaniel memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$ATHANIEL_HOME/supermemory.json`
...
| `container_tag` | `athaniel` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `athaniel-{identity}` → `athaniel-coder`) to isolate memories per Athaniel profile
...
| **Requires** | `pip install athaniel-memori` + `athaniel-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install athaniel-memori
athaniel-memori install
athaniel config set memory.provider memori
athaniel memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `athaniel-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ATHANIEL_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ATHANIEL_HOME/` so each profile has its own credentials
