...
On a brand-new install (or the first time a pre-curator install ticks after `athaniel update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `athaniel curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.athaniel/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `athaniel model`:**

```bash
athaniel model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`athaniel model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
athaniel curator status         # last run, counts, pinned list, LRU top 5
athaniel curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
athaniel curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
athaniel curator run --background  # fire-and-forget: start the run in a background thread
athaniel curator run --dry-run  # preview only — report without any mutations
athaniel curator backup         # take a manual snapshot of ~/.athaniel/skills/
athaniel curator rollback       # restore from the newest snapshot
athaniel curator rollback --list     # list available snapshots
athaniel curator rollback --id <ts>  # restore a specific snapshot
athaniel curator rollback -y         # skip the confirmation prompt
athaniel curator pause          # stop runs until resumed
athaniel curator resume
athaniel curator pin <skill>    # never auto-transition this skill
athaniel curator unpin <skill>
athaniel curator adopt <skill>    # hand an unmanaged skill to the curator
athaniel curator adopt --all-unmanaged   # hand over every unmanaged skill
athaniel curator list-unmanaged   # itemize skills with no provenance marker
athaniel curator restore <skill>  # move an archived skill back to active
athaniel curator list-archived    # list skills currently in ~/.athaniel/skills/.archive/
athaniel curator archive <skill>  # manually archive a single skill now
athaniel curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
athaniel curator ledger           # list the per-mutation audit ledger (all actors)
athaniel curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
athaniel curator rollback <entry-id>  # undo a single mutation from the ledger
athaniel curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```

## Backups and rollback

Before every real curator pass, Athaniel takes a tar.gz snapshot of `~/.athaniel/skills/` at `~/.athaniel/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
athaniel curator rollback        # restore newest snapshot (with confirmation)
athaniel curator rollback -y     # skip the prompt
athaniel curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Athaniel takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `athaniel curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `athaniel curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`athaniel curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.athaniel/skills/.curator_ledger.jsonl`:
...
- **before/after** — per-file `{path, sha256}` manifests. File contents are stored content-addressed (deduped by hash) under `~/.athaniel/.curator_backups/blobs/`, so a hundred entries touching the same unchanged file cost one blob.

```bash
athaniel curator ledger                  # newest 20 entries
athaniel curator ledger --skill my-skill --limit 50
athaniel curator rollback <entry-id>     # restore that one mutation's before-state
```

Single-entry rollback restores exactly the files that mutation touched (and removes files it created) from the blob store — nothing else in the skills tree moves. Like whole-tree rollback, it takes a safety ledger entry of the current state first and **fails closed**: if the safety capture can't be written, nothing is changed. Because foreground deletes are ledgered too, `athaniel curator rollback <entry-id>` can resurrect a hard-deleted skill.
...
Archived skills are kept forever by default. If you want `~/.athaniel/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
athaniel curator purge --dry-run   # preview what would be deleted
athaniel curator purge             # delete archives older than the TTL (with confirmation)
athaniel curator purge --days 90   # one-off TTL override
```

## What "agent-created" means

The curator only manages skills explicitly marked as **agent-created** in
`~/.athaniel/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.athaniel/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.athaniel/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Athaniel at an external skill
...
**To see which skills the curator actually manages**, run `athaniel curator status`.
...
`athaniel curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `athaniel curator adopt <name>` hands one over
```
...
```bash
athaniel curator list-unmanaged                    # itemize them, with reasons
athaniel curator adopt <name> [<name> ...]         # hand specific skills over
athaniel curator adopt --all-unmanaged --dry-run   # preview the full list
athaniel curator adopt --all-unmanaged             # hand over everything (prompts)
athaniel curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `athaniel curator adopt` changes the policy, and
...
**wrote** it — Athaniel edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `athaniel curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `athaniel curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `athaniel curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
athaniel curator pin <skill>
athaniel curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.athaniel/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.athaniel/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.athaniel/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.athaniel/logs/curator/`:

```
~/.athaniel/logs/curator/
└── 20260429-111512/
...
```bash
athaniel curator restore <skill-name>
```

This moves the skill back from `~/.athaniel/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.athaniel/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `athaniel curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/embreythecreator/athaniel-brain/issues/7816) — original proposal and design discussion
