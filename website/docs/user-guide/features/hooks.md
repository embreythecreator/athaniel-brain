...
Athaniel has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.athaniel/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.athaniel/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.athaniel/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.athaniel/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".athaniel" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.athaniel/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.athaniel/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".athaniel" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.athaniel/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.athaniel/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/athaniel-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.athaniel/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Athaniel does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.athaniel/BOOT.md` with natural-language startup instructions.
...
Create `~/.athaniel/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `athaniel cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.athaniel/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.athaniel/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.athaniel/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.athaniel/hooks/boot-md/handler.py`**

```python
"""Run ~/.athaniel/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".athaniel" / "BOOT.md"
...
```bash
athaniel logs --follow --level INFO | grep boot-md
```
...
Delete `~/.athaniel/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Athaniel shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.athaniel/hooks/`
...
- Runtime event-name validity comes from `athaniel_cli.plugins.VALID_HOOKS`. `athaniel hooks list` lists configured shell/outbound hooks, not every available event; `athaniel hooks test <event>` reports the valid set only when an invalid event is supplied.
...
| `session_id` | `str` | Current Athaniel session id |
...
If a `pre_tool_call` callback exceeds `plugins.hook_callback_timeout` (or is still running from a previous timed-out fire), Athaniel **fails closed**: the tool is blocked with a timeout message rather than proceeding without a policy decision.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Athaniel's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

The clean user-message `content` remains unchanged. For replay and prompt-cache stability, Athaniel may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "Athaniel, Teknium, Athaniel, kanban"
...
Declare shell-script hooks in your `~/.athaniel/config.yaml` and Athaniel will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`athaniel_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.athaniel/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.athaniel/agent-hooks/` (by convention) | `~/.athaniel/plugins/<name>/` | `~/.athaniel/hooks/<name>/` |
...
Each time the event fires, Athaniel spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Athaniel-canonical

// Modify a pre_tool_call — rewrite tool args before dispatch:
{"action": "modify", "args": {"new_string": "fixed content"}}         // Athaniel-canonical
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/.athaniel/agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `athaniel hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.athaniel/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.athaniel/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.athaniel/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.athaniel/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.athaniel/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.athaniel/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Athaniel event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.athaniel/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.athaniel/agent-hooks/log-orchestration.sh
log=~/.athaniel/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Athaniel sees it, then persists the decision to `~/.athaniel/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `athaniel --accept-hooks chat`)
2. `ATHANIEL_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.athaniel/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `athaniel hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.athaniel/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/athaniel/.athaniel/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `athaniel hooks list`.

### The `athaniel hooks` CLI

| Command | What it does |
|---------|--------------|
| `athaniel hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `athaniel hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `athaniel hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `athaniel hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.athaniel/agent-hooks/` so the path is easy to audit.
- Re-run `athaniel hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake Athaniel when the world changes; outbound webhooks tell the world when Athaniel does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and Athaniel POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* Athaniel instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.athaniel/config.yaml`:

```yaml
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      url: https://ci.example.com/athaniel-events
      events: [on_session_end, subagent_stop]
      secret_env: ATHANIEL_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
      timeout: 10                           # per-attempt seconds (1–60)

    - name: tool-monitor
      url: https://metrics.example.com/hooks/athaniel
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.athaniel/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `athaniel hooks list`).
...
| `X-Athaniel-Event` | The hook event name |
| `X-Athaniel-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-Athaniel-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-Athaniel-Delivery` header) — remember recently seen ids and skip duplicates. Athaniel retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `ATHANIEL_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`athaniel hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
