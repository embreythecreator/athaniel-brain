...
Athaniel Brain includes two model-callable web tools backed by multiple providers:

- **`web_search`** — search the web and return ranked results
- **`web_extract`** — fetch and extract readable content from one or more URLs

Both are configured through a single backend selection. Providers are chosen via `athaniel tools` or set directly in `config.yaml`.
...
| **xAI (Grok)** | `XAI_API_KEY` or `athaniel auth add xai-oauth` | ✔ | — | Paid (SuperGrok or per-token) |

Brave Search, DDGS, and xAI are **search-only** — pair any of them with Firecrawl/Keenable/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let Athaniel lazy-install it on first use). xAI runs Grok's server-side `web_search` tool on the Responses API — results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all model output (see the [trust-model caveat](#xai-grok) below).
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, web search and extract are available through the **[Tool Gateway](tool-gateway.md)** via managed Firecrawl — no API key needed. New installs can run `athaniel setup --portal` to log in and turn on all gateway tools at once; existing installs can flip just web via `athaniel tools`.
:::
...
| `web_extract` — same URL, same format, same provider | Full text stored under `~/.athaniel/cache/web/` | Shared across CLI, gateway, cron, and subagent processes |
...
### Quick setup via `athaniel tools`

Run `athaniel tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** — just point Athaniel at a running SearXNG instance.
...
To enable JSON responses for Athaniel, add the following override:
...
Then select SearXNG as the search backend in `~/.athaniel/config.yaml`:
...
Or set via `athaniel tools` → Web Search & Extract → SearXNG.
...
With this config, Athaniel uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.
...
If no backend has **ever** been selected (no `web.backend` / per-capability key written by you or `athaniel tools`), Athaniel picks the first available one based on which credentials are set:
...
Run `athaniel setup` to see which web backend is detected:
...
source ~/.athaniel/athaniel-brain/.venv/bin/activate
...
```bash
athaniel skills install official/research/searxng-search
```
