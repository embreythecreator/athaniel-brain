...
Type `@` followed by a reference to inject content directly into your message. Athaniel expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Athaniel env: `$ATHANIEL_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ATHANIEL_HOME/skills/.hub/`
