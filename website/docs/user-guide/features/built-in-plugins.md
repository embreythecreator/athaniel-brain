...
description: "Plugins shipped with Athaniel Brain that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Athaniel ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.athaniel/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Athaniel Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.athaniel/plugins/<name>/`
3. **Project** — `./.athaniel/plugins/<name>/` (requires `ATHANIEL_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `athaniel_brain.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `athaniel memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `athaniel plugins list` and the interactive `athaniel plugins` UI), but none load until you explicitly enable them:

```bash
athaniel plugins enable disk-cleanup
```

Or via `~/.athaniel/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Athaniel. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
athaniel plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `athaniel plugins enable <name>`.
...
| `athaniel-achievements` | dashboard tab | Steam-style collectible badges generated from your real Athaniel session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `athaniel memory` and `athaniel plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ATHANIEL_HOME` or `/tmp/athaniel-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ATHANIEL_HOME | always | Never |
...
**State** — everything lives at `$ATHANIEL_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ATHANIEL_HOME` or `/tmp/athaniel-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `athaniel plugins enable disk-cleanup` (or check the box in `athaniel plugins`).

**Disabling again:** `athaniel plugins disable disk-cleanup`.
...
**Enabling:** `athaniel plugins enable security-guidance` (or check the box in `athaniel plugins`).

**Disabling again:** `athaniel plugins disable security-guidance`.
...
Traces Athaniel turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Athaniel' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `athaniel logs`.
...
```bash
athaniel tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Athaniel and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
athaniel plugins enable observability/langfuse
```

Then put the credentials in `~/.athaniel/.env`:

```bash
ATHANIEL_LANGFUSE_PUBLIC_KEY=pk-lf-...
ATHANIEL_LANGFUSE_SECRET_KEY=sk-lf-...
ATHANIEL_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Athaniel turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ATHANIEL_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Athaniel session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `athaniel chat` session lives under one Langfuse session.

**Verify:**

```bash
athaniel plugins list                 # observability/langfuse should show "enabled"
athaniel chat -q "hello"              # check the Langfuse UI for a "Athaniel turn" trace
```
...
| `ATHANIEL_LANGFUSE_RELEASE` | — | Release/version tag |
| `ATHANIEL_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ATHANIEL_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ATHANIEL_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Athaniel-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Athaniel-prefixed wins when both are set.
...
**Disabling:** `athaniel plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Athaniel plugin. Do not run `athaniel plugins enable observability/nemo_relay`; Athaniel core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `ATHANIEL_NEMO_RELAY_PLUGINS_TOML` to that file before starting Athaniel. The policy is process-wide for every profile hosted by that Athaniel process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `ATHANIEL_NEMO_RELAY_ATOF_*` and `ATHANIEL_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `athaniel doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `athaniel.session.segment` plus `athaniel.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.athaniel/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
athaniel plugins enable google_meet
athaniel meet setup   # preflight: playwright, chromium, auth file
athaniel meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `athaniel plugins disable google_meet`. Any saved transcripts stay in `~/.athaniel/workspace/meetings/` until you remove them.

### athaniel-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Athaniel session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Athaniel feature changes.

**How it works:**

- Scans your entire `~/.athaniel/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ATHANIEL_HOME/plugins/athaniel-achievements/state.json`
...
| Secret | Hidden until Athaniel detects the first related signal in your history |

**API** — routes mount under `/api/plugins/athaniel-achievements/`:
...
**State files** — live under `$ATHANIEL_HOME/plugins/athaniel-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Athaniel updates. |
...
**Enabling:** Nothing to enable — `athaniel-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `athaniel dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/athaniel-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.athaniel/plugins/athaniel-achievements/` that ships no dashboard. The plugin's state files under `$ATHANIEL_HOME/plugins/athaniel-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Athaniel plugin — see [Build a Athaniel Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.athaniel/plugins/<name>/`
- Manifest source is reported as `bundled` in `athaniel plugins list`
