...
When Athaniel Brain runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.athaniel/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.athaniel/config.yaml` (switchable per session
via `/personality`).
...
If you use Athaniel' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.athaniel/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Athaniel Brain's deliverable
