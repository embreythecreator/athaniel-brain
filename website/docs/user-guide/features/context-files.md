...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .athaniel.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Athaniel Brain automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Athaniel instance and is loaded from `ATHANIEL_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.athaniel.md** / **ATHANIEL.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this Athaniel instance | `ATHANIEL_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.athaniel.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
When your working directory sits inside a git repository, Athaniel loads a **merged chain** of `AGENTS.md` files at session start: the git-root `AGENTS.md` first, then the `AGENTS.md` in every intermediate directory down to your working directory. Deeper files appear later in the prompt, so more specific guidance takes precedence. Each file gets its own provenance header (e.g. `## ../../AGENTS.md`), and identical copies along the chain are deduplicated.
...
- `~/.athaniel/SOUL.md`
- or `$ATHANIEL_HOME/SOUL.md` if you run Athaniel with a custom home directory

Important details:

- Athaniel seeds a default `SOUL.md` automatically if one does not exist yet
- Athaniel loads `SOUL.md` only from `ATHANIEL_HOME`
- Athaniel does not probe the working directory for `SOUL.md`
...
Athaniel is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.athaniel.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Athaniel.
...
1. **Scan working directory** — checks for `.athaniel.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
