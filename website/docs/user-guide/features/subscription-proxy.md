...
chat completions — use your Athaniel-managed provider subscription as their
...
| Use case | "Use Athaniel as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Athaniel stores
the refresh token in `~/.athaniel/auth.json` — the same place all Athaniel
...
Model:      Athaniel-4-70B    # or Athaniel-4.3-36B, Athaniel-4-405B
```
...
interface in `athaniel_cli/proxy/adapters/`.
...
If you see `not logged in`, run `athaniel portal`. If you see
...
`athaniel_cli/proxy/adapters/<provider>.py` and registering it in
