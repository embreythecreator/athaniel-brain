...
Athaniel can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ATHANIEL_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Athaniel renders the sprite at full fidelity when your
...
athaniel pets list
athaniel pets list cat

# Install a pet and make it active in one step
athaniel pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
athaniel pets show

# Check your setup
athaniel pets doctor
```

## `athaniel pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `athaniel pets list [query] [--limit N]` |
| List installed pets | `athaniel pets list --installed` |
| Install a pet | `athaniel pets install <slug> [--select] [--force]` |
| Set the active pet | `athaniel pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `athaniel pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `athaniel pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `athaniel pets off` |
| Remove an installed pet | `athaniel pets remove <slug>` |
| Diagnose setup | `athaniel pets doctor` |

`athaniel pets show` flags:
...
Beyond installing pre-made pets from the gallery, Athaniel can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `athaniel pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `athaniel tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ATHANIEL_PET_IMAGE_PROVIDER` env var (e.g. `ATHANIEL_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Athaniel is
...
Run `athaniel pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Athaniel uses its own
  profile-scoped `<ATHANIEL_HOME>/pets/` instead — install through `athaniel pets`.

## See also

- The [`athaniel-brain` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athaniel-brain.md)
