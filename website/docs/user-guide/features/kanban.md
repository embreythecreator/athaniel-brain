...
description: "Durable SQLite-backed task board for coordinating multiple Athaniel profiles"
---
...
Athaniel Kanban is a durable task board, shared across all your Athaniel profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.athaniel/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.athaniel/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `athaniel kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `athaniel kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/athaniel-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `ATHANIEL_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
athaniel kanban boards list

# Create a new board.
athaniel kanban boards create atm10-server \
    --name "ATM10 Server" \
...
athaniel kanban --board atm10-server list
athaniel kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
athaniel kanban boards switch atm10-server
athaniel kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
athaniel kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `ATHANIEL_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.athaniel/kanban/current` — the slug persisted by `athaniel kanban
   boards switch`.
4. `default`.
...
`athaniel dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<athaniel-home>/kanban/attachments/<task_id>/` for the default board, or
  `<athaniel-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `ATHANIEL_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
athaniel kanban init

# 2. Start the gateway (hosts the embedded dispatcher)
athaniel gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
athaniel kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
athaniel kanban watch

# 5. See the board (you)
athaniel kanban list
athaniel kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `athaniel kanban show t_abcd`.
...
Override the config flag at runtime via `ATHANIEL_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `athaniel gateway
...
until one comes up — `athaniel kanban create` warns about this at creation
time.

Running `athaniel kanban daemon` as a separate process is **deprecated**;
...
athaniel kanban create "nightly ops review" \
    --assignee ops \
...
```bash
athaniel kanban complete t_abc t_def t_hij --result "batch wrap"
athaniel kanban archive  t_abc t_def t_hij
athaniel kanban unblock  t_abc t_def
athaniel kanban block    t_abc "need input" --ids t_def t_hij
```
...
**Workers do not shell out to `athaniel kanban`.** When the dispatcher spawns a worker it sets `ATHANIEL_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `athaniel kanban` CLI.
...
kanban_show()                                     # no args — uses ATHANIEL_KANBAN_TASK
...
### Why tools instead of shelling to `athaniel kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `athaniel kanban complete` *inside* the container, where `athaniel` isn't installed and `~/.athaniel/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.athaniel/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `athaniel chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `ATHANIEL_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/athaniel_cli/test_kanban_db.py -q"],
...
2. `cd $ATHANIEL_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, Athaniel injects up to two
...
guard is active only for dispatcher-spawned workers (`ATHANIEL_KANBAN_TASK` is
set) and can be disabled with `ATHANIEL_KANBAN_STOP_NUDGE=0`.
...
```bash
athaniel kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

athaniel kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `athaniel skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
athaniel kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
athaniel kanban set-model t_abcd claude-opus-4.6 --provider anthropic
athaniel kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.athaniel/profiles/<name>/`, and the dispatcher injects the profile-scoped `ATHANIEL_HOME` when it spawns `athaniel -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.athaniel/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.athaniel/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.athaniel/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `athaniel kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
athaniel kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `athaniel kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `athaniel kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `athaniel profile create --description "..."`, `athaniel profile describe <name> --text "..."`, `athaniel profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.athaniel/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `athaniel profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.athaniel/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Athaniel dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
athaniel kanban init                                     # create kanban.db + print daemon hint
athaniel kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
athaniel kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
athaniel kanban show <id> [--json]
athaniel kanban assign <id> <profile>                    # or 'none' to unassign
athaniel kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
athaniel kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
athaniel kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
athaniel kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
athaniel kanban diagnostics [--json]                     # board health snapshot (alias: diag)
athaniel kanban link <parent_id> <child_id>
athaniel kanban unlink <parent_id> <child_id>
athaniel kanban claim <id> [--ttl SECONDS]
athaniel kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
athaniel kanban complete <id>... [--result "..."]
athaniel kanban block <id> "<reason>" [--ids <id>...]
athaniel kanban unblock <id>...
athaniel kanban archive <id>...

athaniel kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
athaniel kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
athaniel kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

athaniel kanban tail <id>                                # follow a single task's event stream
athaniel kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
athaniel kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
athaniel kanban runs <id> [--json]                       # attempt history (one row per run)
athaniel kanban assignees [--json]                       # profiles on disk + per-assignee task counts
athaniel kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
athaniel kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `athaniel gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
athaniel kanban stats [--json]                           # per-status + per-assignee counts
athaniel kanban log <id> [--tail BYTES]                  # worker log from ~/.athaniel/kanban/logs/
athaniel kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
athaniel kanban notify-list [<id>] [--json]
athaniel kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
athaniel kanban context <id>                             # what a worker sees
athaniel kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
athaniel kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
athaniel kanban create "nightly backup audit" \
...
Every `athaniel kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `athaniel chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `athaniel_cli.kanban.run_slash()` entry point that reuses the `athaniel kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `athaniel kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.athaniel/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`athaniel kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `athaniel kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/athaniel-kanban-v1-spec.pdf`.
...
_metadata_: `{"changed_files": ["athaniel_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
athaniel kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
[`agent-merge-conflict-arbiter` optional skill](https://github.com/embreythecreator/athaniel-brain/blob/main/optional-skills/autonomous-ai-agents/agent-merge-conflict-arbiter/SKILL.md)
...
```
hotspot: athaniel_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
athaniel kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
athaniel kanban notify-list
athaniel kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
guide](https://github.com/embreythecreator/athaniel-brain/blob/main/docs/kanban/multi-gateway.md)),
...
```bash
athaniel kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
athaniel kanban runs t_abcd
...
**Bulk close caveat.** `athaniel kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`athaniel kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `athaniel kanban heartbeat $TASK` to signal liveness during long operations. |
...
`athaniel kanban tail <id>` shows these for a single task. `athaniel kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.athaniel/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/athaniel-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
