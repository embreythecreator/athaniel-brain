...
Athaniel Brain can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Athaniel integration. It
...
**Fresh installs already have the driver.** The Athaniel installer
...
- **`athaniel tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
athaniel computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `athaniel computer-use
status` to verify the install.

Already have cua-driver? Athaniel reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `athaniel update`, and the first
`computer_use` call of a session, Athaniel checks the local version and
...
selected with `ATHANIEL_CUA_DRIVER_CMD` stays
under your control, so Athaniel reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Athaniel autodetection is a
planned cua-driver follow-up, so currently point Athaniel at that directory or
...
interface. The built-in toolset provides Athaniel actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `athaniel computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Athaniel host identity. |
...
```
athaniel -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.athaniel/config.yaml`.

## Permission modes and logged-in browser profiles

Athaniel maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Athaniel session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Athaniel approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Athaniel risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.athaniel/cua-manifest.yaml
```
...
for the format). Athaniel launches a private runtime with
...
identity instead of resetting with every Athaniel build), and Athaniel verifies
...
resetting or closing the Athaniel session, cancellation cleanup, or process exit
closes that transport session. Athaniel also stops private runtimes that it
launched for bounded or unrestricted access. One Athaniel
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Athaniel host identity.
...
## `athaniel computer-use doctor` — your first triage stop

`athaniel computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ athaniel computer-use doctor
...
Athaniel run declares a public cua-driver **session name** (something like
`athaniel-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Athaniel disables it automatically where it is a known failure
...
Athaniel keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Athaniel-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Athaniel
autodetection is a planned cua-driver follow-up, so currently point Athaniel at
...
These are **platform deep dives, not duplicates of the Athaniel skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Athaniel
...
image capture also saves a bounded, shareable copy under Athaniel' image cache
...
Athaniel applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.athaniel/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Athaniel applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Athaniel agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Athaniel agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Athaniel warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Athaniel from inside the RDP / console
...
```
ATHANIEL_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ATHANIEL_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Athaniel disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Athaniel sets
...
When it's on, `athaniel computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Athaniel at a binary you built from source instead
of the published release. Athaniel resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ATHANIEL_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Athaniel testing (see notes).
...
### Option B — point Athaniel straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ATHANIEL_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ATHANIEL_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ATHANIEL_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Athaniel is using your build

- `athaniel computer-use status` prints the resolved binary path and
  version.
- `athaniel computer-use doctor` confirms the binary is reachable and
...
- **Athaniel spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Athaniel YOLO, Athaniel instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Athaniel
  change needed either way.
- **Local builds skip the version check.** Athaniel warns when the
...
**First action when anything's off: run `athaniel computer-use doctor`.**
...
Run `athaniel computer-use install` to fetch the cua-driver binary, or
run `athaniel tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `athaniel computer-use doctor` will
...
- **Athaniel-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Athaniel `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Athaniel autodetection is a
  planned follow-up; currently point Athaniel at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Athaniel among them)
