...
Athaniel Brain can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
athaniel chat --checkpoints
```

Or enable globally in `~/.athaniel/config.yaml`:
...
| Command | Description |
|---------|-------------|
| `athaniel checkpoints` | Show total size, project count, per-project breakdown |
| `athaniel checkpoints status` | Same as bare `checkpoints` |
| `athaniel checkpoints list` | Alias for `status` |
| `athaniel checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `athaniel checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `athaniel checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Athaniel detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.athaniel/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/athaniel/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.athaniel/checkpoints/ at startup
  ...
  # `athaniel checkpoints prune` command below, with a confirmation prompt.
  auto_prune: true
  retention_days: 7
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `athaniel checkpoints prune` manually.
...
Athaniel responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.athaniel/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/athaniel-brain                                        20       2h ago  live
...
Clear with: athaniel checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
athaniel checkpoints prune --retention-days 3 --max-size-mb 200
```
...
`/rollback <N>` restores only the files Athaniel itself changed. Every successful
...
Athaniel last wrote (you edited it afterwards, or Athaniel never touched it) is
...
If the ledger is empty (a store created before this feature, or Athaniel hasn't
...
- **Directory scope** — Athaniel skips overly broad directories (root `/`, home `$HOME`).
...
~/.athaniel/checkpoints/
  ...
  │   ├── refs/athaniel/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `athaniel checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.athaniel/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.athaniel/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
athaniel checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `athaniel chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `athaniel checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Athaniel session in its own worktree/branch, with checkpoints as an extra layer.
