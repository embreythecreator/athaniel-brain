...
This tutorial walks you through setting up a Telegram bot powered by Athaniel Brain that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Athaniel Brain installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.athaniel/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Athaniel itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Athaniel Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_athaniel_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Athaniel Brain. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.athaniel/.env`:
...
```
[Gateway] Starting Athaniel Gateway...
...
# View live logs
journalctl --user -u athaniel-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `athaniel gateway install` to update the plist.
:::
...
```bash
athaniel gateway stop && athaniel gateway start
```
...
athaniel pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.athaniel/.env`:
...
Control how much detail the bot shows when using tools. In `~/.athaniel/config.yaml`:
...
Customize how the bot communicates by editing `~/.athaniel/SOUL.md`:

For a full guide, see [Use SOUL.md with Athaniel](/guides/use-soul-with-athaniel).
...
```bash
# From the CLI
athaniel cron list          # View all scheduled jobs
athaniel cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u athaniel-gateway` (Linux) or `~/.athaniel/logs/gateway.log` (macOS) |
| Cron job output | `~/.athaniel/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.athaniel/cron/jobs.json` |
| Pairing data | `~/.athaniel/pairing/` |
| Session history | `~/.athaniel/sessions/` |
