...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `athaniel send`."
---

# Pipe Script Output to Messaging Platforms

`athaniel send` is a small, scriptable CLI that pushes a message to any
messaging platform Athaniel is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | athaniel send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `athaniel
...
athaniel send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
athaniel send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
athaniel send --list

# Filter by platform
athaniel send --list telegram
```
...
Any platform Athaniel ships adapters for works as a target:
...
`athaniel send` resolves the message body in this order:

1. **Positional argument** — `athaniel send --to telegram "hi"`
2. **`--file PATH`** — `athaniel send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | athaniel send --to telegram`

When stdin is a TTY (no pipe), Athaniel does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  athaniel send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `athaniel send` reuses your Athaniel config, the same script works on
any host where Athaniel is installed — no need to export bot tokens into
...
`athaniel send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  athaniel send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | athaniel send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.athaniel/bin/athaniel send \
...
```bash
./train.py --epochs 200 && \
  athaniel send --to telegram "training done" || \
  athaniel send --to telegram "training failed (exit $?)"
```
...
athaniel send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(athaniel send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `athaniel send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `athaniel send` calls
the platform's REST endpoint directly using credentials from
`~/.athaniel/.env` and `~/.athaniel/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `athaniel gateway start` and retry.
...
# Machine-readable
athaniel send --list --json
```

The listing is built from `~/.athaniel/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`athaniel gateway
...
| Approach | Multi-platform | Reuses Athaniel creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `athaniel send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`athaniel send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `athaniel send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `athaniel send` shares with cron delivery.
