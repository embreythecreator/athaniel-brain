---
sidebar_position: 6
title: "Use MCP with Athaniel"
description: "A practical guide to connecting MCP servers to Athaniel Brain, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Athaniel

This guide shows how to actually use MCP with Athaniel Brain in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Athaniel tool
- you want Athaniel to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Athaniel to internal APIs, databases, or company systems without modifying Athaniel core

Do not use MCP when:
- a built-in Athaniel tool already solves the job well
...
- Athaniel remains the agent
- MCP servers contribute tools
- Athaniel discovers those tools at startup or reload time
...
If you installed Athaniel with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Athaniel banner/status should show MCP integration when configured
- ask Athaniel what tools it has available
...
## WSL2: bridge Athaniel in WSL to Windows Chrome

This is the practical setup when:

- Athaniel runs inside WSL2
...
In this setup, Athaniel does **not** connect to Chrome directly. Instead:

- Athaniel runs in WSL
- Athaniel starts a local stdio MCP server
...
```text
Athaniel (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Athaniel stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Athaniel core browser transport
...
```bash
athaniel mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
athaniel mcp test chrome-devtools-win
```

Then start a fresh Athaniel session or run:
...
Once loaded, Athaniel can use the MCP-prefixed browser tools directly. For example:
...
If Athaniel runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Athaniel from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Athaniel from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Athaniel:
...
2. Athaniel-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Athaniel will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Athaniel to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Athaniel to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Athaniel remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
athaniel mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
athaniel mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `athaniel mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Athaniel to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Athaniel to merge, publish, deploy, or spawn runtimes.
...
Now Athaniel can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Athaniel core.
...
Because Athaniel now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
