...
```bash
athaniel teams-pipeline validate
```
...
```bash
athaniel teams-pipeline token-health
athaniel teams-pipeline token-health --force-refresh
```
...
```bash
athaniel teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
athaniel teams-pipeline maintain-subscriptions
athaniel teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Athaniel cron (recommended if you already run the Athaniel gateway)

Athaniel ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.athaniel/scripts/`. First create the script:

```bash
mkdir -p ~/.athaniel/scripts
cat > ~/.athaniel/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec athaniel teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.athaniel/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
athaniel cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/athaniel-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Athaniel Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=athaniel
EnvironmentFile=/etc/athaniel/env
ExecStart=/usr/local/bin/athaniel teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/athaniel-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Athaniel Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now athaniel-teams-pipeline-maintain.timer
systemctl list-timers athaniel-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/athaniel teams-pipeline maintain-subscriptions >> /var/log/athaniel/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.athaniel/.env` at the top of a wrapper script that crontab calls.
...
```bash
athaniel teams-pipeline subscriptions   # should show expirationDateTime advanced
athaniel teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
athaniel teams-pipeline list
athaniel teams-pipeline list --status failed
athaniel teams-pipeline show <job-id>
```

### Replay a stored job

```bash
athaniel teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
athaniel teams-pipeline fetch --meeting-id <meeting-id>
athaniel teams-pipeline fetch --join-web-url "<join-url>"
athaniel teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `athaniel teams-pipeline maintain-subscriptions --dry-run`
- inspect `athaniel teams-pipeline list --status failed`
...
- run `athaniel teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `athaniel teams-pipeline run`
...
- [ ] `athaniel teams-pipeline validate` returns an OK snapshot
- [ ] `athaniel teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Athaniel cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
