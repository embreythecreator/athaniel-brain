...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Athaniel Brain — no API key required"
---

# MiniMax OAuth

Athaniel Brain supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Athaniel automatically refreshes your session.
...
# → Athaniel opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.athaniel/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.athaniel/.env
```
...
```bash
athaniel auth add minimax-oauth --no-browser
```

Athaniel will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Athaniel implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Athaniel generates a PKCE verifier / challenge pair and a random state value.
...
4. Athaniel polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.athaniel/auth.json` under the `minimax-oauth` key.
...
```bash
athaniel config set model.default MiniMax-M2.7
athaniel config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.athaniel/config.yaml` will contain entries similar to:
...
```bash
athaniel --provider minimax-oauth    # canonical
athaniel --provider minimax-portal   # alias
athaniel --provider minimax-global   # alias
athaniel --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `athaniel setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
athaniel --provider minimax-oauth
```
...
Athaniel refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Athaniel marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Athaniel marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `athaniel auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Athaniel raises a timeout error.

**Fix:** re-run `athaniel auth add minimax-oauth` (or `athaniel model`). The flow starts fresh.

### State mismatch (possible CSRF)

Athaniel detected that the `state` value returned by the authorization server does not match what it sent.
...
If `athaniel` cannot open a browser window, use `--no-browser`:

```bash
athaniel auth add minimax-oauth --no-browser
```

Athaniel prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `athaniel model` and select MiniMax (OAuth), or run `athaniel auth add minimax-oauth`.
...
```bash
athaniel auth logout minimax-oauth
```
...
- [athaniel doctor](../reference/cli-commands.md)
