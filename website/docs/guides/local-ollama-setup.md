---
sidebar_position: 9
title: "Run Athaniel Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running Athaniel Brain entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run Athaniel Locally with Ollama — Zero API Cost
...
You'll set up Athaniel Brain running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, Athaniel works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- Athaniel connected to Ollama as a custom endpoint
...
```bash
# ~/.athaniel/.env
ATHANIEL_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Athaniel is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full Athaniel experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside Athaniel with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Alternatively, edit `~/.athaniel/config.yaml` directly:
...
Athaniel will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. Athaniel requires at least 64,000 tokens for agentic work with tools:
...
Then update your Athaniel config to use `gemma4-64k` as the model name.
...
Once Athaniel works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.

### Telegram

1. Create a bot via [@BotFather](https://t.me/BotFather) and get the token
2. Add to your `~/.athaniel/config.yaml`:
...
- **Widen the API timeout** — set `ATHANIEL_API_TIMEOUT=1800` in `~/.athaniel/.env` (see [What You Need](#what-you-need)).
- **Measure and trim the fixed prompt** — run `athaniel prompt-size` for a byte breakdown of the system prompt and tool schemas, then disable unused toolsets with `athaniel tools` and uninstall skills you don't need with `athaniel skills`.
...
- **Terminal commands** — Athaniel wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below Athaniel' 64K minimum unless you configure them
