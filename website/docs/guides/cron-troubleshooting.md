...
description: "Diagnose and fix common Athaniel cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`athaniel gateway` for foreground, or `athaniel gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `athaniel cron tick`.
...
```bash
date
athaniel cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.athaniel/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.athaniel/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.athaniel/.env` |
...
| `local` | Write access to `~/.athaniel/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `athaniel cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Athaniel Cloud / Team Gateway)
...
```bash
athaniel skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `athaniel skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `athaniel skills list`.
...
2. `~/.athaniel/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `athaniel cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Athaniel config directory). Verify:
```bash
ls ~/.athaniel/scripts/your-script.py   # Must exist
athaniel cron edit <job_id> --script ~/.athaniel/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `athaniel skills install <skill-name>`.
...
```bash
ps aux | grep athaniel
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.athaniel/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.athaniel/cron/jobs.json
chmod 600 ~/.athaniel/cron/jobs.json   # Your user should own it
```
...
```bash
athaniel cron list                    # Show all jobs, states, next_run times
athaniel cron run <job_id>            # Schedule for next tick (for testing)
athaniel cron edit <job_id>           # Fix configuration issues
athaniel logs                         # View recent Athaniel logs
athaniel skills list                  # Verify installed skills
```
...
1. Run the job with `athaniel cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.athaniel/logs/agent.log` for scheduler messages and `~/.athaniel/logs/errors.log` for warnings
3. Open an issue at [github.com/embreythecreator/athaniel-brain](https://github.com/embreythecreator/athaniel-brain) with:
