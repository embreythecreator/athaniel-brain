...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `athaniel setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Athaniel spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.athaniel/cron/output/` and you can read them anytime.
:::
...
Athaniel will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Athaniel Brain
```
...
Before creating cron jobs, ensure Athaniel has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Athaniel what you want:
...
Athaniel will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Athaniel to delegate each topic to a sub-agent:
...
Athaniel will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
