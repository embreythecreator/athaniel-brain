...
Copy-paste blueprints for common automation patterns. Each blueprint uses Athaniel's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`athaniel webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `athaniel webhook subscribe`) |
...
```bash
athaniel cron create "0 2 * * *" \
  "You are a project manager triaging the embreythecreator/athaniel-brain GitHub repo.

1. Run: gh issue list --repo embreythecreator/athaniel-brain --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
athaniel webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
athaniel cron create "0 9 * * 1" \
  "Scan the embreythecreator/athaniel-brain repo for documentation drift.

1. Run: gh pr list --repo embreythecreator/athaniel-brain --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (athaniel_cli/commands.py, athaniel_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (athaniel_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
athaniel cron create "0 6 * * *" \
  "Run a dependency security audit on the athaniel-brain project.

1. cd ~/.athaniel/athaniel-brain && source .venv/bin/activate
...
```bash
athaniel webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
athaniel webhook subscribe alert-triage \
...
```python title="~/.athaniel/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Athaniel-Monitor/1.0"})
...
```bash
athaniel cron create "every 30m" \
  ...
  --script ~/.athaniel/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
athaniel cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Athaniel Brain development." \
...
```bash
athaniel webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
athaniel webhook subscribe stripe-payments \
...
```bash
athaniel cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the athaniel-brain codebase.
