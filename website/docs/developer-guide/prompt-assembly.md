...
description: "How Athaniel builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Athaniel deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.athaniel.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.athaniel/SOUL.md)
You are Athaniel, an AI assistant created by Embrey The Creator / The Voice.
...
The platform hint (Layer 10 above) is the per-surface guidance Athaniel
...
`SOUL.md` lives at `~/.athaniel/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_athaniel_home() / "SOUL.md"
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_athaniel_md(cwd_path)       # 1. .athaniel.md / ATHANIEL.md (walks to git root)
    ...
    # SOUL.md from ATHANIEL_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.athaniel.md`, `ATHANIEL.md` | CWD up to git root | Athaniel-native project config |
...
- **YAML frontmatter stripped** — `.athaniel.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Athaniel concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.athaniel.md` / `ATHANIEL.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Athaniel already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.athaniel/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.athaniel/MEMORY.md` and `~/.athaniel/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.athaniel.md`, `ATHANIEL.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Athaniel.
- Ephemeral overlays such as `ATHANIEL_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Athaniel assembles prompts for everyone, change Python and treat it as a code contribution
