...
sidebar_position: 1
title: "Architecture"
description: "Athaniel Brain internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Athaniel Brain internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ athaniel_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AthanielCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── athaniel_state.py           # SQLite session/state database with FTS5
├── athaniel_constants.py       # ATHANIEL_HOME, profile-aware paths
...
│
├── athaniel_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `athaniel` subcommands (large file)
...
│   ├── skills_config.py      # athaniel skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # athaniel tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # athaniel gateway start/stop
│
...
```text
User input → AthanielCLI.process_input()
...
Three discovery sources: `~/.athaniel/plugins/` (user), `.athaniel/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `athaniel plugins` or `config.yaml`.
...
Exposes Athaniel as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`athaniel -p <name>`) gets its own ATHANIEL_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
