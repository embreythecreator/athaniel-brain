...
Athaniel Brain saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Athaniel function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `athaniel` CLI
