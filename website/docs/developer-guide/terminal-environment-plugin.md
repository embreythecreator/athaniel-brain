...
Athaniel runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.athaniel/plugins/`, registering a backend the user selects
...
| `athaniel setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `athaniel status` / `athaniel doctor` | `doctor_checks()` |
...
```python title="~/.athaniel/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.athaniel"    # where synced cache files land, or None
...
```yaml title="~/.athaniel/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
athaniel plugins enable acmebox
athaniel config set terminal.backend acmebox
```
