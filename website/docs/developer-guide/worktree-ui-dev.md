...
| Variable | Meaning |
|----------|---------|
| `ATHANIEL_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ATHANIEL_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ATHANIEL_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Athaniel itself — they're private to these helpers. The variables Athaniel *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `athaniel --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_athaniel_root)" || { echo "htui: not in a Athaniel checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ATHANIEL_MAIN_CHECKOUT/.venv/bin/python" -m athaniel_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ATHANIEL_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_athaniel_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ATHANIEL_TUI_DIR` are mutually exclusive
`ATHANIEL_TUI_DIR` points Athaniel at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `athaniel --tui --dev` exits with an error. Run `unset ATHANIEL_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_athaniel_root)" || { echo "hgui: not in a Athaniel checkout" >&2; return 1; }
  deps="${ATHANIEL_GUI_DEPS_CHECKOUT:-$ATHANIEL_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _athaniel_link_deps "$desktop" "$deps/apps/desktop"
    _athaniel_link_deps "$root" "$deps"
  else
  ...
  trap '_athaniel_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ATHANIEL_DESKTOP_ATHANIEL_ROOT="$root" \
    ATHANIEL_DESKTOP_PYTHON="$ATHANIEL_MAIN_CHECKOUT/.venv/bin/python" \
    ATHANIEL_DESKTOP_IGNORE_EXISTING=1 \
    ATHANIEL_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ATHANIEL_DESKTOP_ATHANIEL_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `athaniel`. |
| `ATHANIEL_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ATHANIEL_DESKTOP_IGNORE_EXISTING` | Ignores any `athaniel` on `PATH` so it can't shadow the worktree. |
| `ATHANIEL_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Athaniel checkout.
_athaniel_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/athaniel_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_athaniel_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'athaniel_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `athaniel --tui --dev` and the `ATHANIEL_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/embreythecreator/athaniel-brain/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ATHANIEL_*` variable Athaniel reads
