...
description: "How to build a memory provider plugin for Athaniel Brain"
---

# Building a Memory Provider Plugin

Memory provider plugins give Athaniel Brain persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `athaniel plugins`.
:::

## Installation Layouts

Athaniel discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Athaniel. Closed to new providers — see [CONTRIBUTING](https://github.com/embreythecreator/athaniel-brain/blob/main/CONTRIBUTING.md). |
| User | `$ATHANIEL_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.athaniel/plugins/<name>/` | Opt-in via `ATHANIEL_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `athaniel_brain.memory_providers` entry point | `pip install`, nothing to copy. |
...
Athaniel, in `$ATHANIEL_HOME/plugins/<name>/` when installed by a user, or in
`./.athaniel/plugins/<name>/` for a project-local one:
...
`athaniel_brain.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."athaniel_brain.memory_providers"]
...
package layout. No copy under `$ATHANIEL_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Athaniel reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `athaniel <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          athaniel_home (str): Active ATHANIEL_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `athaniel memory setup` | **Yes** |
| `save_config(values, athaniel_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Athaniel
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Athaniel
...
`get_config_schema()` returns a list of field descriptors used by `athaniel memory setup`:
...
Every field in `get_config_schema()` is prompted during `athaniel memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$ATHANIEL_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, athaniel_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(athaniel_home) / "my-provider.json"
...
the `messages` parameter; Athaniel will continue calling them with the legacy
signature.
...
All storage paths **must** use the `athaniel_home` kwarg from `initialize()`, not hardcoded `~/.athaniel`:

```python
# CORRECT — profile-scoped
from athaniel_constants import get_athaniel_home
data_dir = get_athaniel_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.athaniel/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `athaniel my-provider status`, `athaniel my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `athaniel <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `athaniel --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: athaniel my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the athaniel my-provider argparse tree.
