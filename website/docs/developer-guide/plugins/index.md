...
title: "Build a Athaniel Plugin"
description: "Step-by-step guide to building a complete Athaniel plugin with tools, hooks, data files, and skills"
---

# Build a Athaniel Plugin

This guide walks through building a complete Athaniel plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Athaniel has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `ATHANIEL_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.athaniel/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `athaniel skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `embreythecreator/athaniel-brain`. Users install them into `~/.athaniel/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Athaniel Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/embreythecreator/athaniel-brain/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Athaniel can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Athaniel already owns. It does not replace native `plugin.yaml` plus
...
```bash
athaniel plugins install owner/repository --no-enable
athaniel plugins list
athaniel plugins enable <plugin-name>
```
...
Athaniel validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Athaniel.
...
Portable `streamable-http` entries are routed through Athaniel' existing native
...
same full-trust posture as other installed Athaniel plugins.
...
records it as Published. Athaniel keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Athaniel does not expose a
...
continue to work after a normal Athaniel upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Athaniel inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Athaniel releases can therefore load a plugin whose manifest contains
...
Athaniel enforces this contract with frozen external-plugin fixtures discovered
from an isolated `ATHANIEL_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.athaniel/plugins/calculator
cd ~/.athaniel/plugins/calculator
```

### Validate with Plugin Doctor

`athaniel plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Athaniel itself. It reports invalid hook names, callbacks that do
...
```bash
athaniel plugins doctor . --ci
```

Doctor uses a temporary `ATHANIEL_HOME`, restores plugin registration state after
...
This tells Athaniel: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `athaniel_cli/plugin_capabilities.py` for the
...
`athaniel_brain.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`athaniel_brain.plugins` entry point:

```toml
[project.entry-points."athaniel_brain.plugins"]
calculator = "my_pkg:register"

[project.entry-points."athaniel_brain.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Athaniel reads these from installed metadata without importing your code, so
`athaniel plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Athaniel understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Athaniel validates them, and `athaniel plugins install` / `athaniel plugins doctor` surface missing ones with a `pip install` hint, but Athaniel **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Athaniel' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/embreythecreator/athaniel-brain/issues/64165) and
[#15220](https://github.com/embreythecreator/athaniel-brain/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Athaniel may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `athaniel my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Athaniel continues fine
...
Use plugin-relative config keys for user-visible behavior. Athaniel resolves them
...
`<ATHANIEL_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `ATHANIEL_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
ATHANIEL_PLUGINS_DEBUG=1 athaniel plugins list
```
...
The same logs are always written to `~/.athaniel/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
athaniel logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `athaniel plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.athaniel/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `athaniel plugins update` / `remove` git-pull or delete it — your
...
# <athaniel home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.athaniel/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.athaniel/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `athaniel plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Athaniel into installing arbitrary packages — only specs Athaniel itself ships are eligible |
...
auditable in `~/.athaniel/logs/agent.log`. Plugins load after built-in
...
does not ship with Athaniel core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `athaniel plugins list`
...
loader, the plugin is disabled and Athaniel continues. The gate exists
...
You normally never edit this key by hand. `athaniel plugins enable <name>`
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $ATHANIEL_HOME/hook_outputs
```
...
- **The system prompt is Athaniel's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `athaniel_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Athaniel sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`athaniel.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Athaniel still loads on an older one.

### Register CLI commands

Plugins can add their own `athaniel <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for athaniel my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: athaniel my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for athaniel my-plugin."""
...
After registration, users can run `athaniel my-plugin status`, `athaniel my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `athaniel name` in a terminal |
...
- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `ATHANIEL_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `athaniel <subcommand>` (e.g. `athaniel kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "athaniel kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Athaniel from a hook.
...
Athaniel has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.athaniel/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `athaniel gateway status` without SDK instantiation.
...
Athaniel also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Athaniel without any Python plugin. Declare them in `~/.athaniel/config.yaml`:
...
Athaniel connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.athaniel/hooks/<name>/`:

```yaml
# ~/.athaniel/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.athaniel/hooks/long-task-alert/handler.py
...
athaniel skills search my-workflow --source myorg/skills-repo
athaniel skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install athaniel-plugin-calculator
# Plugin auto-discovered on next athaniel startup
```
...
```python
# Wrong — will break if Athaniel passes extra context
def handler(args):
    ...
