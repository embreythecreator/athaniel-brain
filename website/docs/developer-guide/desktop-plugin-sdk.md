...
title: "Desktop Plugin SDK (@athaniel/plugin-sdk)"
description: "Extend the native Athaniel Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Athaniel Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `AthanielPlugin`.
It imports one module — `@athaniel/plugin-sdk` — and gets everything: the app's
...
`$ATHANIEL_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Athaniel. This page is the **native
desktop app** (`athaniel desktop`) SDK — the `@athaniel/plugin-sdk` module and
`$ATHANIEL_HOME/desktop-plugins/`. The **web dashboard** (`athaniel dashboard`) has
its own, unrelated plugin system on `window.__ATHANIEL_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Athaniel Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ATHANIEL_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ATHANIEL_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `AthanielPlugin` contract, appear in **Settings → Plugins**,
...
Create `$ATHANIEL_HOME/desktop-plugins/hello/plugin.js` (that's `~/.athaniel/...`
by default, or `~/.athaniel/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.athaniel/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@athaniel/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Athaniel' }),
      jsx('div', {
...
The only importable specifiers are `@athaniel/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `AthanielPlugin`:

```ts
interface AthanielPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `athaniel.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@athaniel/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@athaniel/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@athaniel/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@athaniel/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@athaniel/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@athaniel/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@athaniel/plugin-sdk'
...
`window.athaniel.send('get-price eth')` (or a declarative
`<button data-athaniel-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@athaniel/plugin-sdk'
...
for requests; `targetProfile` is the backend Athaniel profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Athaniel
...
ctx.os.notify({
  ...
  // Body click → focus Athaniel + navigate. Same vocabulary as OS deep links:
  activate: 'athaniel://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'athaniel://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `athaniel://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `athaniel://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@athaniel/plugin-sdk'
...
```javascript
import { queryClient } from '@athaniel/plugin-sdk'
...
a package into `~/.athaniel/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.athaniel/plugins` is not reachable as a filesystem —
...
the `athaniel://` scheme — a plain anchor on your website or README:
...
`athaniel-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Athaniel plugin and declare it in a
`manifest.json`:
...
athaniel-brain codebase directly (`athaniel_state`, `athaniel_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.athaniel/`)
...
(`athaniel.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `AthanielPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@athaniel/plugin-sdk` alias.
2. It's still lint-fenced to `@athaniel/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@athaniel/plugin-sdk`, `react`,
...
| Plugin contract | `AthanielPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `AthanielOpenTarget`, `Contribution` |
...
### Agents: the `athaniel-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`athaniel-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ATHANIEL_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `athaniel logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@athaniel/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.athaniel/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.athaniel/logs/errors.log` for
