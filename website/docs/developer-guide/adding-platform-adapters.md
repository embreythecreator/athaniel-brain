...
This guide covers adding a new messaging platform to the Athaniel gateway. A platform adapter connects Athaniel to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.athaniel/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Athaniel code. Your plugin is a directory with two files:

```
~/.athaniel/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `athaniel config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-athaniel-config) below).
...
With `provides_tools` declared, Athaniel imports only `tools.py` during plugin
...
`athaniel tools enable my_platform --platform cli`, or by listing the toolset
...
| `athaniel config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `athaniel status` | Shows plugin platforms with `(plugin)` tag |
| `athaniel gateway setup` | Plugin platforms appear in setup menu |
| `athaniel tools` / `athaniel skills` | Plugin platforms in per-platform config |
...
direct `athaniel send --to ...` and cron `deliver=platform:...` by declaring send
...
def register(ctx):
    ctx.register_platform(
        ...
        # May be a regular function or async def. Athaniel awaits any awaitable
...
Most users set up a platform by dropping env vars into `~/.athaniel/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `athaniel gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `athaniel cron run` separate from `athaniel gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `athaniel config`

`athaniel_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Athaniel Brain.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #athaniel)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Athaniel core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`athaniel_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`athaniel_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`athaniel_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`athaniel_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`athaniel_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`athaniel_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"athaniel-newplat"` toolset definition with `_ATHANIEL_CORE_TOOLS`
2. **`toolsets.py`** — Add `"athaniel-newplat"` to the `"athaniel-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | athaniel-newplat toolset |
