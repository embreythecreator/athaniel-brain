...
Plugins can launch and supervise fresh Athaniel child sessions without importing
...
active agent turn fails closed with `No active Athaniel parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Athaniel can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Athaniel's existing unsafe-tool block remains enforced.
