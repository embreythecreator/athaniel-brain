...
Athaniel Brain uses a SQLite database (`~/.athaniel/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `athaniel_state.py`


## Architecture Overview

```
~/.athaniel/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `athaniel_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `athaniel_state.py` for the exact SQL.
...
Multiple athaniel processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.athaniel/state.db
...
Default path: `~/.athaniel/state.db`

This is derived from `athaniel_constants.get_athaniel_home()` which resolves to
`~/.athaniel/` by default, or the value of `ATHANIEL_HOME` environment variable.
