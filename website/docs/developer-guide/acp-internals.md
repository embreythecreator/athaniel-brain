...
The ACP adapter wraps Athaniel' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
athaniel acp / athaniel-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.athaniel/.env
  -> configure stderr logging
  -> construct AthanielACPAgent
...
- `allow_once` -> Athaniel `once`
- `allow_always` -> Athaniel `always`
- reject options -> Athaniel `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Athaniel tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["athaniel-acp"])
...
Instead it reuses Athaniel' runtime resolver:

- `acp_adapter/auth.py`
- `athaniel_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Athaniel provider/credentials. It also always advertises a terminal setup auth method (`athaniel-setup`, args `--setup`) so first-run ACP clients can open Athaniel' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.athaniel/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `athaniel-acp` toolset definition
- `athaniel_cli/main.py` — `athaniel acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `athaniel-acp` script
