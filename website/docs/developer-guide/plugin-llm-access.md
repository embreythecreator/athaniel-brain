...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Athaniel already has applies. The plugin never sees a
...
athaniel-brain itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Athaniel provider id (e.g. "openrouter")
...
using Athaniel built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Athaniel provider
        # (must be one Athaniel already knows about — same names as
        # `athaniel model` and config.yaml model.provider).
        ...
        # plugin sends — Athaniel does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.athaniel/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Athaniel' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Athaniel subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/embreythecreator/athaniel-brain/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/embreythecreator/athaniel-brain/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/embreythecreator/athaniel-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/embreythecreator/athaniel-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
