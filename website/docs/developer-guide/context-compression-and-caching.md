...
Athaniel Brain uses a dual compression system and Anthropic prompt caching to
...
Configure via `athaniel plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Athaniel has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|athaniel|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `athaniel`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Athaniel raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$ATHANIEL_HOME`
...
```bash
athaniel config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
athaniel config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Athaniel keeps the
...
`gpt-5.4-900k`). These are Athaniel-side aliases: the suffix is stripped before
...
thread context, so Athaniel' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Athaniel
  records the resulting compaction events (compression counters, session
  events). Set `athaniel` to let Athaniel' compression threshold initiate
  app-server compaction, or `off` to disable Athaniel-initiated automatic
  compaction entirely (codex may still compact natively).

Athaniel' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Athaniel' summary compressor.
...
encrypted `compaction` output item. Athaniel captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Athaniel
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Athaniel can avoid; user-facing docs for `/model`, fallback providers, and
