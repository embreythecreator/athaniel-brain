...
description: "How Athaniel detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Athaniel' browser tooling:
...
(`athaniel-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Athaniel `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `athaniel-acp`, `athaniel-api-server`, and core toolsets (gated on CDP reachability)
- `athaniel_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
