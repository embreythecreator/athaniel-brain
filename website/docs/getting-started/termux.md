...
description: "Run Athaniel Brain directly on an Android phone with Termux"
---

# Athaniel on Android with Termux
...
Athaniel Brain can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Athaniel from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
This APT repository is **community-maintained by `@adybag14-cyber` and is not an official embreythecreator distribution**. embreythecreator does not build, sign, host, or audit these packages. Enabling the repository means trusting the contributor-operated repository and its signing key. Termux itself remains a Tier 2 / best-effort platform.
:::

For users who prefer a native package-manager install rather than building Python/Rust dependencies on the phone, a community-maintained APT repository is available. The repository bootstrap and packaging sources are published in [`adybag14-cyber/termux-python`](https://github.com/adybag14-cyber/termux-python), with the Athaniel package build in [`adybag14-cyber/termux-athaniel`](https://github.com/adybag14-cyber/termux-athaniel).

Install the repository key/source and Athaniel with:

```bash
curl -fsSL https://raw.githubusercontent.com/adybag14-cyber/termux-python/main/scripts/setup_apt_repo.sh | bash
pkg install athaniel-brain
```
...
APT-managed Athaniel installs are marked with install method `apt`. Athaniel therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade athaniel-brain
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Athaniel runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Athaniel now ships a Termux-aware installer path:
...
- links `athaniel` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `athaniel` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/athaniel" "$PREFIX/bin/athaniel"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `athaniel` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.athaniel/.env`.
...
### `athaniel doctor` says ripgrep or Node is missing
