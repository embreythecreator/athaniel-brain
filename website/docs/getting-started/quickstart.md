---
sidebar_position: 1
title: "Athaniel Brain Quickstart"
description: "Your first conversation with Athaniel Brain — from install to chatting in under 5 minutes"
---

# Athaniel Brain Quickstart

This guide gets you from zero to a working Athaniel setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Athaniel Brain Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Athaniel Brain Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Athaniel for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Athaniel working on my machine | `athaniel setup` | Run a real chat and verify it responds |
| I already know my provider | `athaniel model` | Save the config, then start chatting |
| I want a bot or always-on setup | `athaniel gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `athaniel model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `athaniel model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Athaniel cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Athaniel Brain
### With the Athaniel Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Athaniel Desktop installer](https://athaniel-brain.embreythecreator.com/) from our website and run it.

### Without Athaniel Desktop:
For a command-line only install without Athaniel Desktop, run:
...
The single most important setup step. Use `athaniel model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `athaniel setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `athaniel update`. Re-enable anything later with `athaniel tools`, seed skills with `athaniel skills opt-in --sync`, or tune settings with `athaniel setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `athaniel model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `athaniel model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `athaniel model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `athaniel_cli/models.py` may change between releases) | `athaniel model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `athaniel model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `athaniel model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `athaniel model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `athaniel model` (requires `copilot` CLI + `copilot login`) |
...
Athaniel Brain requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `athaniel model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Athaniel separates secrets from normal config:

- **Secrets and tokens** → `~/.athaniel/.env`
- **Non-secret settings** → `~/.athaniel/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
athaniel config set model anthropic/claude-opus-4.6
athaniel config set terminal.backend docker
athaniel config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
athaniel            # classic CLI
athaniel --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Athaniel ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `athaniel` vs `athaniel --tui`.
:::
...
- Athaniel replies without error
...
```bash
athaniel --continue    # Resume the most recent session
athaniel -c            # Short form
```
...
```bash
athaniel gateway setup    # Interactive platform configuration
```
...
- `athaniel tools` — tune tool access per platform
- `athaniel skills` — browse and install reusable workflows
...
```bash
athaniel config set terminal.backend docker    # Docker isolation
athaniel config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `athaniel egress setup && athaniel egress start`; `athaniel setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Athaniel install directory (the curl installer placed it at
# ~/.athaniel/athaniel-brain on Linux/macOS or %LOCALAPPDATA%\athaniel\athaniel-brain on Windows):
cd ~/.athaniel/athaniel-brain
...
Skills are on-demand instruction documents that teach Athaniel how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Athaniel ships with a catalog of bundled skills already installed in `~/.athaniel/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
athaniel skills browse                      # list everything available
athaniel skills search kubernetes           # find skills by keyword
athaniel skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `athaniel skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Athaniel ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.athaniel/athaniel-brain && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Athaniel opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `athaniel model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `athaniel gateway setup` and check `athaniel gateway status` |
| `athaniel --continue` can't find old session | Switched profiles or session never saved | Check `athaniel sessions list` and confirm you're in the right profile |
...
| `athaniel doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `athaniel model`
3. `athaniel setup`
4. `athaniel sessions list`
5. `athaniel --continue`
6. `athaniel gateway status`
...
| `athaniel tools` | Configure which tools are enabled per platform |
| `athaniel setup` | Full setup wizard (configures everything at once) |
| `athaniel doctor` | Diagnose issues |
| `athaniel update` | Update to latest version |
| `athaniel gateway` | Start the messaging gateway |
| `athaniel --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-athaniel-to-another-machine)** — `athaniel backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
