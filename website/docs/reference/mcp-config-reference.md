...
description: "Reference for Athaniel Brain MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Values resolve from the active profile's secret scope (falling back to the process environment), so put the secret in `~/.athaniel/.env`. An unset variable keeps its literal placeholder.
...
Athaniel may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Athaniel only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Athaniel does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Athaniel uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, client identification, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.athaniel/mcp-tokens/<server>.json` and reused across sessions
- Token refresh is automatic; re-authorization only happens when refresh fails
- Only applies to HTTP/StreamableHTTP transport (`url`-based servers)

### Client identification: CIMD and DCR

Athaniel identifies itself to authorization servers with a **Client ID Metadata Document** (CIMD), the mechanism the MCP `2026-07-28` spec adopted in place of Dynamic Client Registration. The document is published at
`https://embreythecreator.github.io/athaniel-brain/docs/oauth/client-metadata.json`, and that URL *is* the `client_id` — the authorization server fetches it to learn Athaniel' name, logo, and permitted redirect URIs. Nothing is registered per install, and nothing is user-specific.
...
The document declares a fixed set of loopback redirect URIs, and the spec requires the redirect URI in an authorization request to be an *exact string match* against one of them — so a CIMD flow cannot use the random high port Athaniel normally picks. Athaniel therefore pins the callback to one of ports `27890`–`27894`.

That pin has to be chosen before the server's capabilities are known, because the redirect URI is fixed at the start of the flow while the server's metadata only arrives partway through. So Athaniel pins the port for any flow that *could* end up using CIMD, and reverts to a random port for the rest:

- A server Athaniel has connected to before, whose cached metadata does not advertise CIMD, keeps the random port it has always used.
- A server Athaniel has never reached gets a pinned port on that first login, since guessing is the only way CIMD can ever be used.
...
If a server fetches the document and refuses it at the *token* endpoint (`invalid_client`), Athaniel logs the rejection, records it under `~/.athaniel/mcp-tokens/<server>.cimd-off`, and uses DCR for that server from then on.

A server that cannot fetch or validate the document at all aborts at the *authorization* endpoint instead, before any redirect happens. There is no signal Athaniel can observe there, so the browser shows an invalid-client error and the login times out after five minutes. The timeout message names the document and points at `cimd: false`. Running `athaniel mcp login <server>` clears the recorded rejection, so a corrected document gets another chance.
...
## Add to Athaniel link

MCP vendors and docs can offer a one-click **"Add to Athaniel"** button that opens the Athaniel desktop app with a pre-filled server config, mirroring Cursor's `cursor://anysphere.cursor-deeplink/mcp/install` scheme:

```text
athaniel://mcp/install?name=NAME&config=BASE64
```
...
const link = `athaniel://mcp/install?name=example&config=${btoa(JSON.stringify(config))
