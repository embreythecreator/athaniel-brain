...
description: "Frequently asked questions and solutions to common issues with Athaniel Brain"
---
...
### What LLM providers work with Athaniel?

Athaniel Brain works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[Nous Portal](/integrations/nous-portal)** — Athaniel's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `athaniel auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `athaniel model` or by editing `~/.athaniel/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Athaniel in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Athaniel inside WSL2
...
- let Athaniel use the resulting MCP browser tools

This is more reliable than trying to force Athaniel core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Athaniel](../guides/use-mcp-with-athaniel.md#wsl2-bridge-athaniel-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Athaniel Brain does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.athaniel/`.

### Can I use it offline / with local models?

Yes. Run `athaniel model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Athaniel minimum; set this to match your server's actual context window
```
...
Athaniel persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Athaniel — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Athaniel auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `ATHANIEL_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Athaniel Brain itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Athaniel Brain instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Athaniel programmatically:
...
#### `athaniel: command not found` after installation
...
**Cause:** Athaniel requires Python 3.11 or newer.
...
**Cause:** Athaniel builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Athaniel runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Athaniel auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.athaniel/config.yaml`:
...
sudo rm /usr/local/bin/athaniel
...
#### The agent says "Athaniel policy" or "Athaniel guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Athaniel runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Athaniel enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Athaniel does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Athaniel tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Athaniel execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for Athaniel' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `athaniel model` from your terminal to add new providers:

```bash
# Exit the Athaniel chat session first (Ctrl+C or /quit)
...
After adding a new provider via `athaniel model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `athaniel model` (from terminal) |
| Enter/change API keys | `athaniel model` (from terminal) |
...
athaniel config show

# Re-configure your provider
athaniel model

# Or set directly
athaniel config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.athaniel/.env` for conflicting entries.
:::
...
# Set a valid model
athaniel config set ATHANIEL_MODEL anthropic/claude-opus-4.7

# Or specify per-session
athaniel chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `athaniel chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Athaniel detected the wrong context length for your model.
...
athaniel chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Athaniel may have the wrong context length for your model. Check what it detected:
...
**Cause:** Athaniel detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Athaniel never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `athaniel chat`
...
cat ~/.athaniel/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `athaniel gateway setup`
- Check gateway logs: `cat ~/.athaniel/logs/gateway.log | tail -50`
...
Configure in `~/.athaniel/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.athaniel/athaniel-brain && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `athaniel gateway start` fails
...
tmux new -s athaniel 'athaniel gateway run'
# Reattach later: tmux attach -t athaniel

# Option 3: Background via nohup
nohup athaniel gateway run > ~/.athaniel/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'athaniel gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `athaniel gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
athaniel gateway install    # Re-snapshots your current PATH
athaniel gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.athaniel.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `athaniel chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `athaniel chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`athaniel prompt-size`](/reference/cli-commands#athaniel-prompt-size) to measure it, then trim: disable toolsets you don't use (`athaniel tools`) and uninstall or disable skills you don't need (`athaniel skills`).
...
cd ~/.athaniel/athaniel-brain && uv pip install -e ".[mcp]"
...
Verify your `~/.athaniel/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
athaniel config show | grep -A 12 mcp_servers

# Restart Athaniel or reload MCP after config changes
athaniel chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Athaniel](/guides/use-mcp-with-athaniel)
...
:::warning
If an MCP server crashes mid-request, Athaniel will report a timeout. Check the server's own logs (not just Athaniel logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `athaniel profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Athaniel home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `athaniel update`?

`athaniel update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `athaniel update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.athaniel/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Athaniel can route subagents to a different model automatically. Set this in `~/.athaniel/config.yaml`:
...
Now when you tell Athaniel "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Athaniel do this?

**Current limitation:** Athaniel profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Athaniel reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `athaniel skills config` settings don't seem to take effect.

**Solution:** Use `athaniel skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`athaniel gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Athaniel creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Athaniel to another machine
...
1. Install Athaniel Brain on the new machine:
   ```bash
   curl -fsSL https://athaniel-brain.embreythecreator.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   athaniel backup
   ```
   This creates a zip of your entire `~/.athaniel/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/athaniel-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/athaniel-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   athaniel import ~/athaniel-backup-<timestamp>.zip
   ```

4. On the new machine, run `athaniel setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
athaniel profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
athaniel profile import ./work-backup.tar.gz work
```
...
### `athaniel backup` vs `athaniel profile export`

| Feature | `athaniel backup` | `athaniel profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.athaniel` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='athaniel-brain' ~/.athaniel/ newmachine:~/.athaniel/
```

:::tip
`athaniel backup` produces a consistent snapshot even while Athaniel is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Athaniel installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Athaniel-specific issue — it's a shell config permissions problem.
...
athaniel config show | head -20

# Re-run model selection
athaniel model

# Or test with a known-good model
athaniel chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/embreythecreator/athaniel-brain/issues)
2. **Ask the community:** [Athaniel Discord](https://discord.gg/embreythecreator)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Athaniel version (`athaniel --version`), and the full error message
