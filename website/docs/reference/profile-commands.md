...
This page covers all commands related to [Athaniel profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `athaniel profile` without a subcommand shows help.
...
## `athaniel profile use`

```bash
athaniel profile use <name>
```

Sets `<name>` as the active profile. All subsequent `athaniel` commands (without `-p`) will use this profile.
...
athaniel profile use default
```

## `athaniel profile create`

```bash
athaniel profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `athaniel profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `athaniel update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.athaniel`), use `athaniel skills opt-out` / `athaniel skills opt-in`. |
...
athaniel profile create work2 --clone-from work

# Clone everything from a specific profile
athaniel profile create work2-backup --clone-from work --clone-all
```

## `athaniel profile describe`

```bash
athaniel profile describe [<name>] [options]
```
...
athaniel profile describe researcher

# Set it explicitly
athaniel profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
athaniel profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
athaniel profile describe --all --auto
```

## `athaniel profile delete`

```bash
athaniel profile delete <name> [options]
```
...
```bash
athaniel profile delete mybot
athaniel profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.athaniel`) cannot be deleted — use `athaniel uninstall` to remove everything.
:::

## `athaniel profile show`

```bash
athaniel profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Athaniel home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ athaniel profile show work
Profile: work
Path:    ~/.athaniel/profiles/work
...
## `athaniel profile alias`

```bash
athaniel profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Athaniel installation.
...
```bash
athaniel profile alias work
# Creates/updates ~/.local/bin/work

athaniel profile alias work --name mywork
# Creates ~/.local/bin/mywork

athaniel profile alias work --remove
# Removes the wrapper script
```

## `athaniel profile rename`

```bash
athaniel profile rename <old-name> <new-name>
```
...
```bash
athaniel profile rename mybot assistant
# ~/.athaniel/profiles/mybot → ~/.athaniel/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `athaniel profile export`

```bash
athaniel profile export <name> [options]
```
...
```bash
athaniel profile export work
# Creates work.tar.gz in the current directory

athaniel profile export work -o ./work-2026-03-29.tar.gz
```
...
## `athaniel profile import`

```bash
athaniel profile import <archive> [options]
```
...
```bash
athaniel profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

athaniel profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `athaniel -p telemetry`). |
...
athaniel profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
athaniel profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
athaniel profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
athaniel profile install ./telemetry/
```

### `athaniel profile update`

```bash
athaniel profile update <name> [--force-config] [--yes]
```
...
### `athaniel profile info`

```bash
athaniel profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Athaniel version, author, env var requirements, the source URL/path, and
...
`athaniel profile list` also shows the distribution name and version in a
`Distribution` column, and `athaniel profile show <name>` / `delete <name>`
...
athaniel profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
athaniel profile install https://github.com/your-org/internal-assistant.git
```
...
`athaniel_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Athaniel version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Athaniel can clone from.
3. Tell recipients to run `athaniel profile install <your-repo-url>`.
...
## `athaniel -p` / `athaniel --profile`

```bash
athaniel -p <name> <command> [options]
athaniel --profile <name> <command> [options]
```

Global flag to run any Athaniel command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
athaniel -p work chat -q "Check the server status"
athaniel --profile dev gateway start
athaniel -p personal skills list
athaniel -p work config edit
```

## `athaniel completion`

```bash
athaniel completion <shell>
```
...
- `athaniel profile <TAB>` — subcommands (list, use, create, etc.)
- `athaniel profile use <TAB>` — profile names
- `athaniel -p <TAB>` — profile names
