...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/embreythecreator/athaniel-brain/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    athaniel_meta = (fm.get("metadata") or {}).get("athaniel") or {}
    tags = athaniel_meta.get("tags") or []
    related = athaniel_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `athaniel skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Athaniel Brain"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Athaniel ships with a large built-in skill library copied into `~/.athaniel/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Athaniel also syncs bundled skills on `athaniel update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.athaniel/skills/` tree, it is still shipped with Athaniel; restore it with `athaniel skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with athaniel-brain — install via athaniel skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with athaniel-brain under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "athaniel skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "athaniel skills install official/blockchain/solana",
        "athaniel skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "athaniel skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
