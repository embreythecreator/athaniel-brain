# Athaniel Brain Security Policy

This document describes Athaniel Brain's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/embreythecreator/athaniel-brain/security/advisories/new)
or **security@embreythecreator.com**. Do not open public issues for
security vulnerabilities. **Athaniel Brain does not operate a bug
bounty program.**
...
- Environment details (`athaniel --version`, commit SHA, OS, Python
  version).
...
Athaniel Brain is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Athaniel Brain,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Athaniel Brain access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Athaniel Brain's documentation
...
Athaniel Brain supports two OS-level isolation postures. They address
...
Athaniel Brain supports this in two ways:

- **Athaniel Brain's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Athaniel Brain's in-process heuristics
...
Athaniel Brain filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Athaniel Brain
itself. Bugs in Athaniel Brain's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Athaniel Brain does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Athaniel Brain's own documentation, or reasonable
  operator expectations would predict — including cases where
  Athaniel Brain has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  athaniel-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Athaniel Brain's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Athaniel Brain. Bugs in Athaniel
...
- Athaniel Brain includes supply-chain guards for MCP server
